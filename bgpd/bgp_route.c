...
static struct bgp_node *bgp_afi_node_get(struct bgp_table *table, afi_t afi, safi_t safi, struct prefix *p, struct prefix_rd *prd) {
	struct bgp_node *rn;
...
static void bgp_pcount_adjust(struct bgp_node *rn, struct bgp_info *ri) {
	...
	if(!BGP_INFO_COUNTABLE(ri) && CHECK_FLAG(ri->flags, BGP_INFO_COUNTED)) {
		...
		if(ri->peer->pcount[table->afi][table->safi]) {
			...
			zlog_warn("%s: Asked to decrement 0 prefix count for peer %s", __func__, ri->peer->host);
			zlog_backtrace(LOG_WARNING);
			zlog_warn("%s: Please report to Quagga bugzilla", __func__);
		}
	} else if(BGP_INFO_COUNTABLE(ri) && !CHECK_FLAG(ri->flags, BGP_INFO_COUNTED)) {
...
void bgp_info_unset_flag(struct bgp_node *rn, struct bgp_info *ri, u_int32_t flag) {
...
static int bgp_info_cmp(struct bgp *bgp, struct bgp_info *new, struct bgp_info *exist, afi_t afi, safi_t safi) {
   ...
   */
	...
	if(bgp_flag_check(bgp, BGP_FLAG_ALWAYS_COMPARE_MED) || (bgp_flag_check(bgp, BGP_FLAG_MED_CONFED) && confed_as_route) || aspath_cmp_left(newattr->aspath, existattr->aspath)
	   || aspath_cmp_left_confed(newattr->aspath, existattr->aspath) || internal_as_route) {
	...
	if(new_sort == BGP_PEER_EBGP && (exist_sort == BGP_PEER_IBGP || exist_sort == BGP_PEER_CONFED)) {
		return -1;
	}
	if(exist_sort == BGP_PEER_EBGP && (new_sort == BGP_PEER_IBGP || new_sort == BGP_PEER_CONFED)) {
		return 1;
	}
	...
	if(!bgp_flag_check(bgp, BGP_FLAG_COMPARE_ROUTER_ID) && new_sort == BGP_PEER_EBGP && exist_sort == BGP_PEER_EBGP) {
...
static enum filter_type bgp_input_filter(struct peer *peer, struct prefix *p, struct attr *attr, afi_t afi, safi_t safi) {
	struct bgp_filter *filter;

	filter = &peer->filter[afi][safi];

#define FILTER_EXIST_WARN(F, f, filter) \
	if(BGP_DEBUG(update, UPDATE_IN) && !(F##_IN(filter))) plog_warn(peer->log, "%s: Could not find configured input %s-list %s!", peer->host, #f, F##_IN_NAME(filter));
...
static enum filter_type bgp_output_filter(struct peer *peer, struct prefix *p, struct attr *attr, afi_t afi, safi_t safi) {
	struct bgp_filter *filter;

	filter = &peer->filter[afi][safi];

#define FILTER_EXIST_WARN(F, f, filter) \
	if(BGP_DEBUG(update, UPDATE_OUT) && !(F##_OUT(filter))) plog_warn(peer->log, "%s: Could not find configured output %s-list %s!", peer->host, #f, F##_OUT_NAME(filter));
...
static int bgp_community_filter(struct peer *peer, struct attr *attr) {
	if(attr->community) {
		...
		/* NO_EXPORT check. */
		if(peer->sort == BGP_PEER_EBGP && community_include(attr->community, COMMUNITY_NO_EXPORT)) {
			return 1;
		}
...
static int bgp_input_modifier(struct peer *peer, struct prefix *p, struct attr *attr, afi_t afi, safi_t safi) {
	struct bgp_filter *filter;
...
static int bgp_export_modifier(struct peer *rsclient, struct peer *peer, struct prefix *p, struct attr *attr, afi_t afi, safi_t safi) {
	struct bgp_filter *filter;
	struct bgp_info info;
	route_map_result_t ret;
...
static int bgp_import_modifier(struct peer *rsclient, struct peer *peer, struct prefix *p, struct attr *attr, afi_t afi, safi_t safi) {
	struct bgp_filter *filter;
	struct bgp_info info;
	route_map_result_t ret;
...
static int bgp_announce_check(struct bgp_info *ri, struct peer *peer, struct prefix *p, struct attr *attr, afi_t afi, safi_t safi) {
	int ret;
	...
	/* Transparency check. */
	if(CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT) && CHECK_FLAG(from->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT)) {
		transparent = 1;
	} else {
		transparent = 0;
	}
	...
	if(riattr->flag & ATTR_FLAG_BIT(BGP_ATTR_ORIGINATOR_ID)) {
		if(IPV4_ADDR_SAME(&peer->remote_id, &riattr->extra->originator_id)) {
			if(BGP_DEBUG(filter, FILTER)) {
				zlog(peer->log, LOG_DEBUG, "%s [Update:SEND] %s/%d originator-id is same as remote router-id", peer->host, inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN), p->prefixlen);
			}
			return 0;
		}
	}
	...
	if(CHECK_FLAG(peer->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_RM_ADV) && (CHECK_FLAG(peer->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_SM_RCV) || CHECK_FLAG(peer->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_SM_OLD_RCV))) {
	...
	if(bgp_output_filter(peer, p, riattr, afi, safi) == FILTER_DENY) {
		if(BGP_DEBUG(filter, FILTER)) {
			zlog(peer->log, LOG_DEBUG, "%s [Update:SEND] %s/%d is filtered", peer->host, inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN), p->prefixlen);
		}
		return 0;
	}
	...
	if(aspath_loop_check(riattr->aspath, peer->as)) {
		if(BGP_DEBUG(filter, FILTER)) {
			zlog(peer->log, LOG_DEBUG, "%s [Update:SEND] suppress announcement to peer AS %u is AS path.", peer->host, peer->as);
		}
		return 0;
	}
	...
	if(CHECK_FLAG(bgp->config, BGP_CONFIG_CONFEDERATION)) {
		if(aspath_loop_check(riattr->aspath, bgp->confed_id)) {
			if(BGP_DEBUG(filter, FILTER)) {
				zlog(peer->log, LOG_DEBUG, "%s [Update:SEND] suppress announcement to peer AS %u is AS path.", peer->host, bgp->confed_id);
			}
			return 0;
		}
	}
	...
	if((peer->sort == BGP_PEER_IBGP || peer->sort == BGP_PEER_CONFED) && (!(attr->flag & ATTR_FLAG_BIT(BGP_ATTR_LOCAL_PREF)))) {
	...
	if(peer && from && peer->sort == BGP_PEER_IBGP && from->sort == BGP_PEER_IBGP && (!(attr->flag & ATTR_FLAG_BIT(BGP_ATTR_ORIGINATOR_ID)))) {
...
#define NEXTHOP_IS_V4 ((safi != SAFI_ENCAP && p->family == AF_INET) || (safi == SAFI_ENCAP && attr->extra->mp_nexthop_len == 4))

#define NEXTHOP_IS_V6 ((safi != SAFI_ENCAP && p->family == AF_INET6) || (safi == SAFI_ENCAP && attr->extra->mp_nexthop_len == 16))

	/* next-hop-set */
	if(transparent || (reflect && !CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_NEXTHOP_SELF_ALL))
	   || (CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_NEXTHOP_UNCHANGED) && ((NEXTHOP_IS_V4 && attr->nexthop.s_addr) || (NEXTHOP_IS_V6 && !IN6_IS_ADDR_UNSPECIFIED(&attr->extra->mp_nexthop_global))))) {
		/* NEXT-HOP Unchanged. */
	} else if(CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_NEXTHOP_SELF) || (NEXTHOP_IS_V4 && attr->nexthop.s_addr == 0) || (NEXTHOP_IS_V6 && IN6_IS_ADDR_UNSPECIFIED(&attr->extra->mp_nexthop_global)) || (peer->sort == BGP_PEER_EBGP && (bgp_multiaccess_check_v4(attr->nexthop, peer) == 0))) {
		/* Set IPv4 nexthop. */
		if(NEXTHOP_IS_V4) {
			if((safi == SAFI_MPLS_VPN) || (safi == SAFI_ENCAP)) {
				memcpy(&attr->extra->mp_nexthop_global_in, &peer->nexthop.v4, IPV4_MAX_BYTELEN);
			} else {
	...
	if(p->family == AF_INET6 && safi != SAFI_ENCAP) {
		...
		/* Default nexthop_local treatment for non-RS-Clients */
			...
			if(peer->shared_network && !IN6_IS_ADDR_UNSPECIFIED(&peer->nexthop.v6_local)) {
				memcpy(&attr->extra->mp_nexthop_local, &peer->nexthop.v6_local, IPV6_MAX_BYTELEN);
				attr->extra->mp_nexthop_len = 32;
			}
	...
	if(peer->sort == BGP_PEER_EBGP && peer_af_flag_check(peer, afi, safi, PEER_FLAG_REMOVE_PRIVATE_AS) && aspath_private_as_check(attr->aspath)) {
	...
	if(ROUTE_MAP_OUT_NAME(filter) || (ri->extra && ri->extra->suppress)) {
	 ...
	 of the reflected IBGP routes, unless configured to allow it */
		if((from->sort == BGP_PEER_IBGP && peer->sort == BGP_PEER_IBGP) && !bgp_flag_check(bgp, BGP_FLAG_RR_ALLOW_OUTBOUND_POLICY)) {
...
static int bgp_announce_check_rsclient(struct bgp_info *ri, struct peer *rsclient, struct prefix *p, struct attr *attr, afi_t afi, safi_t safi) {
	int ret;
	char buf[SU_ADDRSTRLEN];
	struct bgp_filter *filter;
	struct bgp_info info;
	...
	/* Default route check.  */
	if(CHECK_FLAG(rsclient->af_sflags[afi][safi], PEER_STATUS_DEFAULT_ORIGINATE)) {
		if(p->family == AF_INET && p->u.prefix4.s_addr == INADDR_ANY) {
			return 0;
		} else if(p->family == AF_INET6 && p->prefixlen == 0) {
			return 0;
	...
	/* If the attribute has originator-id and it is same as remote
     peer's id. */
	if(riattr->flag & ATTR_FLAG_BIT(BGP_ATTR_ORIGINATOR_ID)) {
		if(IPV4_ADDR_SAME(&rsclient->remote_id, &riattr->extra->originator_id)) {
			if(BGP_DEBUG(filter, FILTER)) {
				zlog(rsclient->log, LOG_DEBUG, "%s [Update:SEND] %s/%d originator-id is same as remote router-id", rsclient->host, inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN), p->prefixlen);
			}
			return 0;
		}
	}
	...
	/* ORF prefix-list filter check */
	if(CHECK_FLAG(rsclient->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_RM_ADV) && (CHECK_FLAG(rsclient->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_SM_RCV) || CHECK_FLAG(rsclient->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_SM_OLD_RCV))) {
	...
	if(bgp_output_filter(rsclient, p, riattr, afi, safi) == FILTER_DENY) {
		if(BGP_DEBUG(filter, FILTER)) {
			zlog(rsclient->log, LOG_DEBUG, "%s [Update:SEND] %s/%d is filtered", rsclient->host, inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN), p->prefixlen);
		}
		return 0;
	}
	...
	if(aspath_loop_check(riattr->aspath, rsclient->as)) {
		if(BGP_DEBUG(filter, FILTER)) {
			zlog(rsclient->log, LOG_DEBUG, "%s [Update:SEND] suppress announcement to peer AS %u is AS path.", rsclient->host, rsclient->as);
		}
		return 0;
	}
#endif /* BGP_SEND_ASPATH_CHECK */
	...
	/* next-hop-set */
	if((p->family == AF_INET && attr->nexthop.s_addr == 0) || (p->family == AF_INET6 && IN6_IS_ADDR_UNSPECIFIED(&attr->extra->mp_nexthop_global))) {
		/* Set IPv4 nexthop. */
		if(p->family == AF_INET) {
			if((safi == SAFI_MPLS_VPN) || (safi == SAFI_ENCAP)) {
				memcpy(&attr->extra->mp_nexthop_global_in, &rsclient->nexthop.v4, IPV4_MAX_BYTELEN);
			} else {
		...
		if(p->family == AF_INET6) {
			/* IPv6 global nexthop must be included. */
			memcpy(&attr->extra->mp_nexthop_global, &rsclient->nexthop.v6_global, IPV6_MAX_BYTELEN);
			attr->extra->mp_nexthop_len = 16;
		}
	}
	...
	if(p->family == AF_INET6) {
		...
		/* Default nexthop_local treatment for RS-Clients */
			...
			if(rsclient->shared_network && from->shared_network && (rsclient->ifindex == from->ifindex)) {
				if(IN6_IS_ADDR_LINKLOCAL(&attre->mp_nexthop_local)) {
					attre->mp_nexthop_len = 32;
				} else {
					attre->mp_nexthop_len = 16;
			...
			/* Set link-local address for shared network peer. */
			else if(rsclient->shared_network && IN6_IS_ADDR_LINKLOCAL(&rsclient->nexthop.v6_local)) {
				memcpy(&attre->mp_nexthop_local, &rsclient->nexthop.v6_local, IPV6_MAX_BYTELEN);
				attre->mp_nexthop_len = 32;
			}
	...
	/* If this is EBGP peer and remove-private-AS is set.  */
	if(rsclient->sort == BGP_PEER_EBGP && peer_af_flag_check(rsclient, afi, safi, PEER_FLAG_REMOVE_PRIVATE_AS) && aspath_private_as_check(attr->aspath)) {
		attr->aspath = aspath_empty_get();
	}
...
static void bgp_best_selection(struct bgp *bgp, struct bgp_node *rn, struct bgp_info_pair *result, afi_t afi, safi_t safi) {
	...
	if(rn->info == NULL) {
		...
		zlog_warn("%s: Called for route_node %s with no routing entries!", __func__, prefix2str(&(bgp_node_to_rnode(rn)->p), buf, sizeof(buf)));
		return;
	}
		...
		for(ri1 = rn->info; ri1; ri1 = ri1->next) {
			...
			if(ri1->next) {
				for(ri2 = ri1->next; ri2; ri2 = ri2->next) {
					...
					if(ri2->peer && ri2->peer != bgp->peer_self && !CHECK_FLAG(ri2->peer->sflags, PEER_STATUS_NSF_WAIT)) {
						if(ri2->peer->status != Established) {
							continue;
						}
					}

					if(aspath_cmp_left(ri1->attr->aspath, ri2->attr->aspath) || aspath_cmp_left_confed(ri1->attr->aspath, ri2->attr->aspath)) {
						if(CHECK_FLAG(ri2->flags, BGP_INFO_SELECTED)) {
							old_select = ri2;
						}
						if((cmpret = bgp_info_cmp(bgp, ri2, new_select, afi, safi)) == -1) {
	...
	for(ri = rn->info; (ri != NULL) && (nextri = ri->next, 1); ri = nextri) {
		...
		if(ri->peer && ri->peer != bgp->peer_self && !CHECK_FLAG(ri->peer->sflags, PEER_STATUS_NSF_WAIT)) {
		...
		if((cmpret = bgp_info_cmp(bgp, ri, new_select, afi, safi)) == -1) {
			...
			new_select = ri;
		} else if(cmpret == 1 && do_mpath && bgp_flag_check(bgp, BGP_FLAG_DETERMINISTIC_MED)) {
...
static int bgp_process_announce_selected(struct peer *peer, struct bgp_info *selected, struct bgp_node *rn, afi_t afi, safi_t safi) {
	struct prefix *p;
	struct attr attr;
...
static wq_item_status bgp_process_rsclient(struct work_queue *wq, void *data) {
...
static wq_item_status bgp_process_main(struct work_queue *wq, void *data) {
	...
	if(old_select && old_select == new_select && !CHECK_FLAG(rn->flags, BGP_NODE_USER_CLEAR)) {
		if(!CHECK_FLAG(old_select->flags, BGP_INFO_ATTR_CHANGED)) {
			if(CHECK_FLAG(old_select->flags, BGP_INFO_IGP_CHANGED) || CHECK_FLAG(old_select->flags, BGP_INFO_MULTIPATH_CHG)) {
	...
	/* FIB update. */
	if((safi == SAFI_UNICAST || safi == SAFI_MULTICAST) && (!bgp->name && !bgp_option_check(BGP_OPT_NO_FIB))) {
...
static void bgp_process_queue_init(void) {
	bm->process_main_queue = work_queue_new(bm->master, "process_main_queue");
	bm->process_rsclient_queue = work_queue_new(bm->master, "process_rsclient_queue");
...
void bgp_process(struct bgp *bgp, struct bgp_node *rn, afi_t afi, safi_t safi) {
	...
	if((bm->process_main_queue == NULL) || (bm->process_rsclient_queue == NULL)) {
	...
	switch(bgp_node_table(rn)->type) {
		case BGP_TABLE_MAIN: work_queue_add(bm->process_main_queue, pqnode); break;
		case BGP_TABLE_RSCLIENT: work_queue_add(bm->process_rsclient_queue, pqnode); break;
	}
...
static int bgp_maximum_prefix_restart_timer(struct thread *thread) {
	...
	if(BGP_DEBUG(events, EVENTS)) {
		zlog_debug("%s Maximum-prefix restart timer expired, restore peering", peer->host);
	}
...
int bgp_maximum_prefix_overflow(struct peer *peer, afi_t afi, safi_t safi, int always) {
	...
	if(peer->pcount[afi][safi] > peer->pmax[afi][safi]) {
		if(CHECK_FLAG(peer->af_sflags[afi][safi], PEER_STATUS_PREFIX_LIMIT) && !always) {
			return 0;
		}
		...
		zlog(peer->log, LOG_INFO,
		     ...
		     "limit %ld",
		     afi_safi_print(afi, safi), peer->host, peer->pcount[afi][safi], peer->pmax[afi][safi]);
...

		{
			...
			bgp_notify_send_with_data(peer, BGP_NOTIFY_CEASE, BGP_NOTIFY_CEASE_MAX_PREFIX, ndata, 7);
		}
		...
		if(peer->pmax_restart[afi][safi]) {
			...
			if(BGP_DEBUG(events, EVENTS)) {
				zlog_debug("%s Maximum-prefix restart timer started for %d secs", peer->host, peer->v_pmax_restart);
			}

			BGP_TIMER_ON(peer->t_pmax_restart, bgp_maximum_prefix_restart_timer, peer->v_pmax_restart);
		}
	...
	if(peer->pcount[afi][safi] > (peer->pmax[afi][safi] * peer->pmax_threshold[afi][safi] / 100)) {
		...
		zlog(peer->log, LOG_INFO, "%%MAXPFX: No. of %s prefix received from %s reaches %ld, max %ld", afi_safi_print(afi, safi), peer->host, peer->pcount[afi][safi], peer->pmax[afi][safi]);
 ...
 */
static void bgp_rib_remove(struct bgp_node *rn, struct bgp_info *ri, struct peer *peer, afi_t afi, safi_t safi) {
...
static void bgp_rib_withdraw(struct bgp_node *rn, struct bgp_info *ri, struct peer *peer, afi_t afi, safi_t safi, struct prefix_rd *prd) {
   ...
   */
	if(CHECK_FLAG(peer->bgp->af_flags[afi][safi], BGP_CONFIG_DAMPENING) && peer->sort == BGP_PEER_EBGP) {
		if((status = bgp_damp_withdraw(ri, rn, afi, safi, 0)) == BGP_DAMP_SUPPRESSED) {
			bgp_aggregate_decrement(peer->bgp, &rn->p, ri, afi, safi);
			return;
		}
	}
...
static struct bgp_info *info_make(int type, int sub_type, struct peer *peer, struct attr *attr, struct bgp_node *rn) {
	struct bgp_info *new;
...
static void bgp_update_rsclient(struct peer *rsclient, afi_t afi, safi_t safi, struct attr *attr, struct peer *peer, struct prefix *p, int type, int sub_type, struct prefix_rd *prd, u_char *tag) {
	struct bgp_node *rn;
	struct bgp *bgp;
	...
	if(attr->flag & ATTR_FLAG_BIT(BGP_ATTR_ORIGINATOR_ID) && IPV4_ADDR_SAME(&rsclient->remote_id, &attr->extra->originator_id)) {
	...
	/* Apply export policy. */
	if(CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT) && bgp_export_modifier(rsclient, peer, p, &new_attr, afi, safi) == RMAP_DENY) {
	...
	if((afi == AFI_IP) && ((safi == SAFI_UNICAST) || safi == SAFI_MULTICAST)) {
		...
		if(new_attr.nexthop.s_addr == 0 || IPV4_CLASS_DE(ntohl(new_attr.nexthop.s_addr))) {
	...
	if(ri) {
		...
		if(!CHECK_FLAG(ri->flags, BGP_INFO_REMOVED) && attrhash_cmp(ri->attr, attr_new)) {
			if(BGP_DEBUG(update, UPDATE_IN)) {
				zlog(peer->log, LOG_DEBUG, "%s rcvd %s/%d for RS-client %s...duplicate ignored", peer->host, inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN), p->prefixlen, rsclient->host);
			}
		...
		/* Received Logging. */
		if(BGP_DEBUG(update, UPDATE_IN)) {
			zlog(peer->log, LOG_DEBUG, "%s rcvd %s/%d for RS-client %s", peer->host, inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN), p->prefixlen, rsclient->host);
		}
...
filtered:
	...
	if(BGP_DEBUG(update, UPDATE_IN)) {
		zlog(peer->log, LOG_DEBUG, "%s rcvd UPDATE about %s/%d -- DENIED for RS-client %s due to: %s", peer->host, inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN), p->prefixlen, rsclient->host, reason);
	}
...
static void bgp_withdraw_rsclient(struct peer *rsclient, afi_t afi, safi_t safi, struct peer *peer, struct prefix *p, int type, int sub_type, struct prefix_rd *prd, u_char *tag) {
	struct bgp_node *rn;
	struct bgp_info *ri;
	char buf[SU_ADDRSTRLEN];
	...
	if(ri && !CHECK_FLAG(ri->flags, BGP_INFO_HISTORY)) {
		...
		zlog(peer->log, LOG_DEBUG, "%s Can't find the route %s/%d", peer->host, inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN), p->prefixlen);
	}
...
static int bgp_update_main(struct peer *peer, struct prefix *p, struct attr *attr, afi_t afi, safi_t safi, int type, int sub_type, struct prefix_rd *prd, u_char *tag, int soft_reconfig) {
	int ret;
	...
	/* When peer's soft reconfiguration enabled.  Record input packet in
     Adj-RIBs-In.  */
	if(!soft_reconfig && CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_SOFT_RECONFIG) && peer != bgp->peer_self) {
	...
	/* AS path loop check. */
	if(aspath_loop_check(attr->aspath, bgp->as) > peer->allowas_in[afi][safi] || (CHECK_FLAG(bgp->config, BGP_CONFIG_CONFEDERATION) && aspath_loop_check(attr->aspath, bgp->confed_id) > peer->allowas_in[afi][safi])) {
		reason = "as-path contains our own AS;";
		goto filtered;
	}

	/* Route reflector originator ID check.  */
	if(attr->flag & ATTR_FLAG_BIT(BGP_ATTR_ORIGINATOR_ID) && IPV4_ADDR_SAME(&bgp->router_id, &attr->extra->originator_id)) {
		reason = "originator is us;";
		goto filtered;
	}
   ...
   * the attr (which takes over the memory references) */
	...
	if(afi == AFI_IP && safi == SAFI_UNICAST) {
	 ...
	 must not be my own address.  */
		if(new_attr.nexthop.s_addr == 0 || IPV4_CLASS_DE(ntohl(new_attr.nexthop.s_addr)) || bgp_nexthop_self(&new_attr)) {
			reason = "martian next-hop;";
			bgp_attr_flush(&new_attr);
			goto filtered;
		}
	...
	if(ri) {
       ...
       */
		...
		if((afi == AFI_IP || afi == AFI_IP6) && safi == SAFI_UNICAST) {
			if(peer->sort == BGP_PEER_EBGP && peer_ttl(peer) == 1 && !CHECK_FLAG(peer->flags, PEER_FLAG_DISABLE_CONNECTED_CHECK)) {
				connected = 1;
			} else {
				connected = 0;
			}
...
filtered:
	if(BGP_DEBUG(update, UPDATE_IN)) {
		zlog(peer->log, LOG_DEBUG, "%s rcvd UPDATE about %s/%d -- DENIED due to: %s", peer->host, inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN), p->prefixlen, reason);
	}
...
int bgp_update(struct peer *peer, struct prefix *p, struct attr *attr, afi_t afi, safi_t safi, int type, int sub_type, struct prefix_rd *prd, u_char *tag, int soft_reconfig) {
	...
	ret = bgp_update_main(peer, p, attr, afi, safi, type, sub_type, prd, tag, soft_reconfig);
	...
	for(ALL_LIST_ELEMENTS(bgp->rsclient, node, nnode, rsclient)) {
		if(CHECK_FLAG(rsclient->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT)) {
			bgp_update_rsclient(rsclient, afi, safi, attr, peer, p, type, sub_type, prd, tag);
		}
	}
...
int bgp_withdraw(struct peer *peer, struct prefix *p, struct attr *attr, afi_t afi, safi_t safi, int type, int sub_type, struct prefix_rd *prd, u_char *tag) {
	struct bgp *bgp;
	char buf[SU_ADDRSTRLEN];
	struct bgp_node *rn;
	struct bgp_info *ri;
   ...
   * if there was no entry, we don't need to do anything more. */
	if(CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_SOFT_RECONFIG) && peer != bgp->peer_self) {
		if(!bgp_adj_in_unset(rn, peer)) {
			if(BGP_DEBUG(update, UPDATE_IN)) {
				zlog(peer->log, LOG_DEBUG,
				     "%s withdrawing route %s/%d "
				     "not in adj-in",
				     peer->host, inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN), p->prefixlen);
			}
			bgp_unlock_node(rn);
			return 0;
		}
	...
	/* Logging. */
	if(BGP_DEBUG(update, UPDATE_IN)) {
		zlog(peer->log, LOG_DEBUG, "%s rcvd UPDATE about %s/%d -- withdrawn", peer->host, inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN), p->prefixlen);
	}
	...
	if(peer->default_rmap[afi][safi].name) {
		...
		for(rn = bgp_table_top(bgp->rib[afi][safi]); rn; rn = bgp_route_next(rn)) {
			for(ri = rn->info; ri; ri = ri->next) {
				...
				ret = route_map_apply(peer->default_rmap[afi][safi].map, &rn->p, RMAP_BGP, &info);
...
static void bgp_announce_table(struct peer *peer, afi_t afi, safi_t safi, struct bgp_table *table, int rsclient) {
	struct bgp_node *rn;
	struct bgp_info *ri;
	struct attr attr;
	struct attr_extra extra;
	...
	if((safi != SAFI_MPLS_VPN) && (safi != SAFI_ENCAP) && CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_DEFAULT_ORIGINATE)) {
	...
	for(rn = bgp_table_top(table); rn; rn = bgp_route_next(rn)) {
		for(ri = rn->info; ri; ri = ri->next) {
			if(CHECK_FLAG(ri->flags, BGP_INFO_SELECTED) && ri->peer != peer) {
				if((rsclient) ? (bgp_announce_check_rsclient(ri, peer, &rn->p, &attr, afi, safi)) : (bgp_announce_check(ri, peer, &rn->p, &attr, afi, safi))) {
...
static void bgp_soft_reconfig_table_rsclient(struct peer *rsclient, afi_t afi, safi_t safi, struct bgp_table *table, struct prefix_rd *prd) {
	struct bgp_node *rn;
	...
	for(rn = bgp_table_top(table); rn; rn = bgp_route_next(rn)) {
		for(ain = rn->adj_in; ain; ain = ain->next) {
			...
			bgp_update_rsclient(rsclient, afi, safi, ain->attr, ain->peer, &rn->p, ZEBRA_ROUTE_BGP, BGP_ROUTE_NORMAL, prd, tag);
		}
	}
}

void bgp_soft_reconfig_rsclient(struct peer *rsclient, afi_t afi, safi_t safi) {
	struct bgp_table *table;
	struct bgp_node *rn;
	...
	if((safi != SAFI_MPLS_VPN) && (safi != SAFI_ENCAP)) {
		...
		for(rn = bgp_table_top(rsclient->bgp->rib[afi][safi]); rn; rn = bgp_route_next(rn)) {
			if((table = rn->info) != NULL) {
...
static void bgp_soft_reconfig_table(struct peer *peer, afi_t afi, safi_t safi, struct bgp_table *table, struct prefix_rd *prd) {
	int ret;
	struct bgp_node *rn;
	struct bgp_adj_in *ain;
	...
	for(rn = bgp_table_top(table); rn; rn = bgp_route_next(rn)) {
		for(ain = rn->adj_in; ain; ain = ain->next) {
			if(ain->peer == peer) {
				...
				ret = bgp_update(peer, &rn->p, ain->attr, afi, safi, ZEBRA_ROUTE_BGP, BGP_ROUTE_NORMAL, prd, tag, 1);
...
static wq_item_status bgp_clear_route_node(struct work_queue *wq, void *data) {
	...
	for(ri = rn->info; ri; ri = ri->next) {
		if(ri->peer == peer || cnq->purpose == BGP_CLEAR_ROUTE_MY_RSCLIENT) {
			...
			if(CHECK_FLAG(peer->sflags, PEER_STATUS_NSF_WAIT) && peer->nsf[afi][safi] && !CHECK_FLAG(ri->flags, BGP_INFO_STALE) && !CHECK_FLAG(ri->flags, BGP_INFO_UNUSEABLE)) {
...
static void bgp_clear_route_table(struct peer *peer, afi_t afi, safi_t safi, struct bgp_table *table, struct peer *rsclient, enum bgp_clear_route_type purpose) {
	struct bgp_node *rn;
       ...
       */
		...
		for(ri = rn->info; ri; ri = ri->next) {
			if(ri->peer == peer || purpose == BGP_CLEAR_ROUTE_MY_RSCLIENT) {
				...
				bgp_table_lock(bgp_node_table(rn));
				bgp_lock_node(rn);
				cnq = XCALLOC(MTYPE_BGP_CLEAR_NODE_QUEUE, sizeof(struct bgp_clear_node_queue));
				cnq->rn = rn;
				cnq->purpose = purpose;
...
void bgp_clear_route(struct peer *peer, afi_t afi, safi_t safi, enum bgp_clear_route_type purpose) {
	struct bgp_node *rn;
	struct bgp_table *table;
	struct peer *rsclient;
	struct listnode *node, *nnode;
 ...
 */
static void bgp_drain_workqueue_immediate(struct work_queue *wq) {
	if(!wq) {
		return;
	}
 ...
 */
void bgp_peer_clear_node_queue_drain_immediate(struct peer *peer) {
	if(!peer) {
		return;
	}
 ...
 */
void bgp_process_queues_drain_immediate(void) {
...
static void bgp_cleanup_table(struct bgp_table *table, safi_t safi) {
	...
	for(rn = bgp_table_top(table); rn; rn = bgp_route_next(rn)) {
		for(ri = rn->info; ri; ri = next) {
			next = ri->next;
			if(CHECK_FLAG(ri->flags, BGP_INFO_SELECTED) && ri->type == ZEBRA_ROUTE_BGP && ri->sub_type == BGP_ROUTE_NORMAL) {
...
/* Parse NLRI stream.  Withdraw NLRI is recognized by NULL attr
   value. */
int bgp_nlri_parse_ip(struct peer *peer, struct attr *attr, struct bgp_nlri *packet) {
	u_char *pnt;
	u_char *lim;
	struct prefix p;
	int psize;
	...
	for(; pnt < lim; pnt += psize) {
		...
		if(packet->afi == AFI_IP && packet->safi == SAFI_UNICAST) {
	      ...
	      */
				zlog(peer->log, LOG_ERR, "%s: IPv4 unicast NLRI is multicast address %s, ignoring", peer->host, inet_ntoa(p.u.prefix4));
				continue;
			}
		}
		...
		if(packet->afi == AFI_IP6 && packet->safi == SAFI_UNICAST) {
			if(IN6_IS_ADDR_LINKLOCAL(&p.u.prefix6)) {
				...
				zlog(peer->log, LOG_ERR, "%s: IPv6 unicast NLRI is link-local address %s, ignoring", peer->host, inet_ntop(AF_INET6, &p.u.prefix6, buf, BUFSIZ));
				continue;
			}
			if(IN6_IS_ADDR_MULTICAST(&p.u.prefix6)) {
				char buf[BUFSIZ];

				zlog(peer->log, LOG_ERR, "%s: IPv6 unicast NLRI is multicast address %s, ignoring", peer->host, inet_ntop(AF_INET6, &p.u.prefix6, buf, BUFSIZ));
				continue;
			}
		}
		...
		/* Normal process. */
		if(attr) {
			ret = bgp_update(peer, &p, attr, packet->afi, packet->safi, ZEBRA_ROUTE_BGP, BGP_ROUTE_NORMAL, NULL, NULL, 0);
		} else {
			ret = bgp_withdraw(peer, &p, attr, packet->afi, packet->safi, ZEBRA_ROUTE_BGP, BGP_ROUTE_NORMAL, NULL, NULL);
		}
...
static void bgp_static_withdraw_rsclient(struct bgp *bgp, struct peer *rsclient, struct prefix *p, afi_t afi, safi_t safi) {
	struct bgp_node *rn;
	struct bgp_info *ri;
	...
	for(ri = rn->info; ri; ri = ri->next) {
		if(ri->peer == bgp->peer_self && ri->type == ZEBRA_ROUTE_BGP && ri->sub_type == BGP_ROUTE_STATIC) {
			break;
		}
	}
...
static void bgp_static_update_rsclient(struct peer *rsclient, struct prefix *p, struct bgp_static *bgp_static, afi_t afi, safi_t safi) {
	struct bgp_node *rn;
	struct bgp_info *ri;
	struct bgp_info *new;
	struct bgp_info info;
	...
	if(bgp_import_modifier(rsclient, bgp->peer_self, p, &new_attr, afi, safi) == RMAP_DENY) {
		/* This BGP update is filtered.  Log the reason then update BGP entry.  */
		if(BGP_DEBUG(update, UPDATE_IN)) {
			zlog(rsclient->log, LOG_DEBUG, "Static UPDATE about %s/%d -- DENIED for RS-client %s due to: import-policy", inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN), p->prefixlen, rsclient->host);
		}
	...
	if(ri) {
		if(attrhash_cmp(ri->attr, attr_new) && !CHECK_FLAG(ri->flags, BGP_INFO_REMOVED)) {
			bgp_unlock_node(rn);
			bgp_attr_unintern(&attr_new);
			aspath_unintern(&attr.aspath);
			bgp_attr_extra_free(&attr);
	...
	/* Make new BGP info. */
	new = info_make(ZEBRA_ROUTE_BGP, BGP_ROUTE_STATIC, bgp->peer_self, attr_new, rn);
	/* Nexthop reachability check. */
	if(bgp_flag_check(bgp, BGP_FLAG_IMPORT_CHECK)) {
...
static void bgp_static_update_main(struct bgp *bgp, struct prefix *p, struct bgp_static *bgp_static, afi_t afi, safi_t safi) {
	struct bgp_node *rn;
	struct bgp_info *ri;
	struct bgp_info *new;
	struct bgp_info info;
 ...
 */
static void bgp_static_withdraw_safi(struct bgp *bgp, struct prefix *p, afi_t afi, safi_t safi, struct prefix_rd *prd, u_char *tag) {
	struct bgp_node *rn;
	struct bgp_info *ri;
...
static void bgp_static_update_safi(struct bgp *bgp, struct prefix *p, struct bgp_static *bgp_static, afi_t afi, safi_t safi) {
	struct bgp_node *rn;
	struct bgp_info *new;
	struct attr *attr_new;
	struct attr attr = { 0 };
...
static int bgp_static_set(struct vty *vty, struct bgp *bgp, const char *ip_str, afi_t afi, safi_t safi, const char *rmap, int backdoor) {
	int ret;
	struct prefix p;
	struct bgp_static *bgp_static;
	struct bgp_node *rn;
	...
	if(afi == AFI_IP6 && IN6_IS_ADDR_LINKLOCAL(&p.u.prefix6)) {
		vty_out(vty, "%% Malformed prefix (link-local address)%s", VTY_NEWLINE);
		return CMD_WARNING;
	}
...
static int bgp_static_unset(struct vty *vty, struct bgp *bgp, const char *ip_str, afi_t afi, safi_t safi) {
	int ret;
	struct prefix p;
	struct bgp_static *bgp_static;
	struct bgp_node *rn;
				...
				if(rn->info != NULL) {
					if((safi == SAFI_MPLS_VPN) || (safi == SAFI_ENCAP)) {
						...
						for(rm = bgp_table_top(table); rm; rm = bgp_route_next(rm)) {
							bgp_static = rn->info;
							bgp_static_withdraw_safi(bgp, &rm->p, AFI_IP, safi, (struct prefix_rd *) &rn->p, bgp_static->tag);
							bgp_static_free(bgp_static);
							rn->info = NULL;
							bgp_unlock_node(rn);
						}
 ...
 */
int bgp_static_set_safi(safi_t safi, struct vty *vty, const char *ip_str, const char *rd_str, const char *tag_str, const char *rmap_str) {
	int ret;
	struct prefix p;
	struct prefix_rd prd;
	struct bgp *bgp;
	...
	prn = bgp_node_get(bgp->route[AFI_IP][safi], (struct prefix *) &prd);
	if(prn->info == NULL) {
...
/* Configure static BGP network. */
int bgp_static_unset_safi(safi_t safi, struct vty *vty, const char *ip_str, const char *rd_str, const char *tag_str) {
	int ret;
	struct bgp *bgp;
	struct prefix p;
	struct prefix_rd prd;
...
DEFUN(bgp_network, bgp_network_cmd, "network A.B.C.D/M",
      "Specify a network to announce via BGP\n"
      "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n") {
	return bgp_static_set(vty, vty->index, argv[0], AFI_IP, bgp_node_safi(vty), NULL, 0);
}

DEFUN(bgp_network_route_map, bgp_network_route_map_cmd, "network A.B.C.D/M route-map WORD",
      "Specify a network to announce via BGP\n"
      "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n"
      "Route-map to modify the attributes\n"
      "Name of the route map\n") {
	return bgp_static_set(vty, vty->index, argv[0], AFI_IP, bgp_node_safi(vty), argv[1], 0);
}

DEFUN(bgp_network_backdoor, bgp_network_backdoor_cmd, "network A.B.C.D/M backdoor",
      "Specify a network to announce via BGP\n"
      "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n"
...
DEFUN(bgp_network_mask, bgp_network_mask_cmd, "network A.B.C.D mask A.B.C.D",
      "Specify a network to announce via BGP\n"
      "Network number\n"
      "Network mask\n"
      "Network mask\n") {
	...
	return bgp_static_set(vty, vty->index, prefix_str, AFI_IP, bgp_node_safi(vty), NULL, 0);
}

DEFUN(bgp_network_mask_route_map, bgp_network_mask_route_map_cmd, "network A.B.C.D mask A.B.C.D route-map WORD",
      "Specify a network to announce via BGP\n"
      "Network number\n"
      "Network mask\n"
      "Network mask\n"
	...
	return bgp_static_set(vty, vty->index, prefix_str, AFI_IP, bgp_node_safi(vty), argv[2], 0);
}

DEFUN(bgp_network_mask_backdoor, bgp_network_mask_backdoor_cmd, "network A.B.C.D mask A.B.C.D backdoor",
      "Specify a network to announce via BGP\n"
      "Network number\n"
      "Network mask\n"
      "Network mask\n"
...
DEFUN(bgp_network_mask_natural, bgp_network_mask_natural_cmd, "network A.B.C.D",
      "Specify a network to announce via BGP\n"
      "Network number\n") {
	int ret;
	char prefix_str[BUFSIZ];
...
DEFUN(bgp_network_mask_natural_route_map, bgp_network_mask_natural_route_map_cmd, "network A.B.C.D route-map WORD",
      "Specify a network to announce via BGP\n"
      "Network number\n"
      "Route-map to modify the attributes\n"
      "Name of the route map\n") {
	...
	return bgp_static_set(vty, vty->index, prefix_str, AFI_IP, bgp_node_safi(vty), argv[1], 0);
}

DEFUN(bgp_network_mask_natural_backdoor, bgp_network_mask_natural_backdoor_cmd, "network A.B.C.D backdoor",
      "Specify a network to announce via BGP\n"
      "Network number\n"
      "Specify a BGP backdoor route\n") {
	int ret;
...
DEFUN(no_bgp_network, no_bgp_network_cmd, "no network A.B.C.D/M",
      NO_STR "Specify a network to announce via BGP\n"
	     "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n") {
	return bgp_static_unset(vty, vty->index, argv[0], AFI_IP, bgp_node_safi(vty));
}

ALIAS(no_bgp_network, no_bgp_network_route_map_cmd, "no network A.B.C.D/M route-map WORD",
      NO_STR "Specify a network to announce via BGP\n"
	     "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n"
	     "Route-map to modify the attributes\n"
	     "Name of the route map\n")

ALIAS(no_bgp_network, no_bgp_network_backdoor_cmd, "no network A.B.C.D/M backdoor",
      NO_STR "Specify a network to announce via BGP\n"
	     "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n"
	     "Specify a BGP backdoor route\n")

DEFUN(no_bgp_network_mask, no_bgp_network_mask_cmd, "no network A.B.C.D mask A.B.C.D",
      NO_STR "Specify a network to announce via BGP\n"
	     "Network number\n"
	     "Network mask\n"
	     "Network mask\n") {
	...
	return bgp_static_unset(vty, vty->index, prefix_str, AFI_IP, bgp_node_safi(vty));
}

ALIAS(no_bgp_network_mask, no_bgp_network_mask_route_map_cmd, "no network A.B.C.D mask A.B.C.D route-map WORD",
      NO_STR "Specify a network to announce via BGP\n"
	     "Network number\n"
	     "Network mask\n"
	     "Network mask\n"
...
ALIAS(no_bgp_network_mask, no_bgp_network_mask_backdoor_cmd, "no network A.B.C.D mask A.B.C.D backdoor",
      NO_STR "Specify a network to announce via BGP\n"
	     "Network number\n"
	     "Network mask\n"
	     "Network mask\n"
...
ALIAS(no_bgp_network_mask_natural, no_bgp_network_mask_natural_route_map_cmd, "no network A.B.C.D route-map WORD",
      NO_STR "Specify a network to announce via BGP\n"
	     "Network number\n"
	     "Route-map to modify the attributes\n"
	     "Name of the route map\n")

ALIAS(no_bgp_network_mask_natural, no_bgp_network_mask_natural_backdoor_cmd, "no network A.B.C.D backdoor",
      NO_STR "Specify a network to announce via BGP\n"
	     "Network number\n"
	     "Specify a BGP backdoor route\n")

DEFUN(ipv6_bgp_network, ipv6_bgp_network_cmd, "network X:X::X:X/M",
      "Specify a network to announce via BGP\n"
      "IPv6 prefix <network>/<length>\n") {
	return bgp_static_set(vty, vty->index, argv[0], AFI_IP6, bgp_node_safi(vty), NULL, 0);
}

DEFUN(ipv6_bgp_network_route_map, ipv6_bgp_network_route_map_cmd, "network X:X::X:X/M route-map WORD",
      "Specify a network to announce via BGP\n"
      "IPv6 prefix <network>/<length>\n"
      "Route-map to modify the attributes\n"
      "Name of the route map\n") {
	return bgp_static_set(vty, vty->index, argv[0], AFI_IP6, bgp_node_safi(vty), argv[1], 0);
}

DEFUN(no_ipv6_bgp_network, no_ipv6_bgp_network_cmd, "no network X:X::X:X/M",
      NO_STR "Specify a network to announce via BGP\n"
	     "IPv6 prefix <network>/<length>\n") {
	return bgp_static_unset(vty, vty->index, argv[0], AFI_IP6, bgp_node_safi(vty));
}

ALIAS(no_ipv6_bgp_network, no_ipv6_bgp_network_route_map_cmd, "no network X:X::X:X/M route-map WORD",
      NO_STR "Specify a network to announce via BGP\n"
	     "IPv6 prefix <network>/<length>\n"
	     "Route-map to modify the attributes\n"
	     "Name of the route map\n")

ALIAS(ipv6_bgp_network, old_ipv6_bgp_network_cmd, "ipv6 bgp network X:X::X:X/M",
      IPV6_STR BGP_STR "Specify a network to announce via BGP\n"
		       "IPv6 prefix <network>/<length>, e.g., 3ffe::/16\n")

ALIAS(no_ipv6_bgp_network, old_no_ipv6_bgp_network_cmd, "no ipv6 bgp network X:X::X:X/M",
      NO_STR IPV6_STR BGP_STR "Specify a network to announce via BGP\n"
			      "IPv6 prefix <network>/<length>, e.g., 3ffe::/16\n")

/* stubs for removed AS-Pathlimit commands, kept for config compatibility */
ALIAS_DEPRECATED(
	bgp_network, bgp_network_ttl_cmd, "network A.B.C.D/M pathlimit <0-255>",
	"Specify a network to announce via BGP\n"
	"IP prefix <network>/<length>, e.g., 35.0.0.0/8\n"
...
ALIAS_DEPRECATED(
	bgp_network_backdoor, bgp_network_backdoor_ttl_cmd, "network A.B.C.D/M backdoor pathlimit <0-255>",
	"Specify a network to announce via BGP\n"
	"IP prefix <network>/<length>, e.g., 35.0.0.0/8\n"
	"Specify a BGP backdoor route\n"
	"AS-Path hopcount limit attribute\n"
...
ALIAS_DEPRECATED(
	bgp_network_mask, bgp_network_mask_ttl_cmd, "network A.B.C.D mask A.B.C.D pathlimit <0-255>",
	"Specify a network to announce via BGP\n"
	"Network number\n"
	"Network mask\n"
	"Network mask\n"
...
ALIAS_DEPRECATED(
	bgp_network_mask_backdoor, bgp_network_mask_backdoor_ttl_cmd, "network A.B.C.D mask A.B.C.D backdoor pathlimit <0-255>",
	"Specify a network to announce via BGP\n"
	"Network number\n"
	"Network mask\n"
	"Network mask\n"
...
ALIAS_DEPRECATED(
	bgp_network_mask_natural, bgp_network_mask_natural_ttl_cmd, "network A.B.C.D pathlimit <0-255>",
	"Specify a network to announce via BGP\n"
	"Network number\n"
	"AS-Path hopcount limit attribute\n"
	"AS-Pathlimit TTL, in number of AS-Path hops\n"
...
ALIAS_DEPRECATED(
	bgp_network_mask_natural_backdoor, bgp_network_mask_natural_backdoor_ttl_cmd, "network A.B.C.D backdoor pathlimit <1-255>",
	"Specify a network to announce via BGP\n"
	"Network number\n"
	"Specify a BGP backdoor route\n"
	"AS-Path hopcount limit attribute\n"
...
ALIAS_DEPRECATED(
	no_bgp_network, no_bgp_network_ttl_cmd, "no network A.B.C.D/M pathlimit <0-255>",
	NO_STR "Specify a network to announce via BGP\n"
	       "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n"
	       "AS-Path hopcount limit attribute\n"
	       "AS-Pathlimit TTL, in number of AS-Path hops\n"
...
ALIAS_DEPRECATED(
	no_bgp_network, no_bgp_network_backdoor_ttl_cmd, "no network A.B.C.D/M backdoor pathlimit <0-255>",
	NO_STR "Specify a network to announce via BGP\n"
	       "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n"
	       "Specify a BGP backdoor route\n"
	       "AS-Path hopcount limit attribute\n"
...
ALIAS_DEPRECATED(
	no_bgp_network, no_bgp_network_mask_ttl_cmd, "no network A.B.C.D mask A.B.C.D pathlimit <0-255>",
	NO_STR "Specify a network to announce via BGP\n"
	       "Network number\n"
	       "Network mask\n"
	       "Network mask\n"
...
ALIAS_DEPRECATED(
	no_bgp_network_mask, no_bgp_network_mask_backdoor_ttl_cmd, "no network A.B.C.D mask A.B.C.D  backdoor pathlimit <0-255>",
	NO_STR "Specify a network to announce via BGP\n"
	       "Network number\n"
	       "Network mask\n"
	       "Network mask\n"
...
ALIAS_DEPRECATED(
	no_bgp_network_mask_natural, no_bgp_network_mask_natural_ttl_cmd, "no network A.B.C.D pathlimit <0-255>",
	NO_STR "Specify a network to announce via BGP\n"
	       "Network number\n"
	       "AS-Path hopcount limit attribute\n"
	       "AS-Pathlimit TTL, in number of AS-Path hops\n"
...
ALIAS_DEPRECATED(
	no_bgp_network_mask_natural, no_bgp_network_mask_natural_backdoor_ttl_cmd, "no network A.B.C.D backdoor pathlimit <0-255>",
	NO_STR "Specify a network to announce via BGP\n"
	       "Network number\n"
	       "Specify a BGP backdoor route\n"
	       "AS-Path hopcount limit attribute\n"
...
ALIAS_DEPRECATED(
	ipv6_bgp_network, ipv6_bgp_network_ttl_cmd, "network X:X::X:X/M pathlimit <0-255>",
	"Specify a network to announce via BGP\n"
	"IPv6 prefix <network>/<length>\n"
	"AS-Path hopcount limit attribute\n"
	"AS-Pathlimit TTL, in number of AS-Path hops\n"
...
ALIAS_DEPRECATED(
	no_ipv6_bgp_network, no_ipv6_bgp_network_ttl_cmd, "no network X:X::X:X/M pathlimit <0-255>",
	NO_STR "Specify a network to announce via BGP\n"
	       "IPv6 prefix <network>/<length>\n"
	       "AS-Path hopcount limit attribute\n"
	       "AS-Pathlimit TTL, in number of AS-Path hops\n"
...
static void bgp_aggregate_route(struct bgp *bgp, struct prefix *p, struct bgp_info *rinew, afi_t afi, safi_t safi, struct bgp_info *del, struct bgp_aggregate *aggregate) {
	struct bgp_table *table;
	struct bgp_node *top;
	struct bgp_node *rn;
	u_char origin;
	...
	for(rn = bgp_node_get(table, p); rn; rn = bgp_route_next_until(rn, top)) {
		if(rn->p.prefixlen > p->prefixlen) {
			...
			for(ri = rn->info; ri; ri = ri->next) {
				...
				if(!IPV4_ADDR_SAME(&ri->attr->nexthop, &nexthop) || ri->attr->med != med) {
					if(aspath) {
						aspath_free(aspath);
					}
					if(community) {
				...
				if(ri->sub_type != BGP_ROUTE_AGGREGATE) {
					...
					if(aggregate->as_set) {
						...
						if(ri->attr->community) {
							if(community) {
								commerge = community_merge(community, ri->attr->community);
	...
	if(rinew) {
		...
		if(aggregate->as_set) {
			...
			if(rinew->attr->community) {
				if(community) {
					commerge = community_merge(community, rinew->attr->community);
					community = community_uniq_sort(commerge);
					community_free(commerge);
				} else {
	...
	if(aggregate->count > 0) {
		rn = bgp_node_get(table, p);
		new = info_make(ZEBRA_ROUTE_BGP, BGP_ROUTE_AGGREGATE, bgp->peer_self, bgp_attr_aggregate_intern(bgp, origin, aspath, community, aggregate->as_set, atomic_aggregate), rn);
		SET_FLAG(new->flags, BGP_INFO_VALID);
...
static void bgp_aggregate_add(struct bgp *bgp, struct prefix *p, afi_t afi, safi_t safi, struct bgp_aggregate *aggregate) {
	struct bgp_table *table;
	struct bgp_node *top;
	struct bgp_node *rn;
	struct bgp_info *new;
...
void bgp_aggregate_delete(struct bgp *bgp, struct prefix *p, afi_t afi, safi_t safi, struct bgp_aggregate *aggregate) {
	...
	for(ri = rn->info; ri; ri = ri->next) {
		if(ri->peer == bgp->peer_self && ri->type == ZEBRA_ROUTE_BGP && ri->sub_type == BGP_ROUTE_AGGREGATE) {
			break;
		}
	}
...
static int bgp_aggregate_unset(struct vty *vty, const char *prefix_str, afi_t afi, safi_t safi) {
	int ret;
	struct prefix p;
	struct bgp_node *rn;
	struct bgp *bgp;
	...
	if(!rn) {
		vty_out(vty, "%% There is no aggregate-address configuration.%s", VTY_NEWLINE);
		return CMD_WARNING;
	}
...
static int bgp_aggregate_set(struct vty *vty, const char *prefix_str, afi_t afi, safi_t safi, u_char summary_only, u_char as_set) {
	int ret;
	struct prefix p;
	struct bgp_node *rn;
	struct bgp *bgp;
...
DEFUN(aggregate_address, aggregate_address_cmd, "aggregate-address A.B.C.D/M",
      "Configure BGP aggregate entries\n"
      "Aggregate prefix\n") {
	return bgp_aggregate_set(vty, argv[0], AFI_IP, bgp_node_safi(vty), 0, 0);
}

DEFUN(aggregate_address_mask, aggregate_address_mask_cmd, "aggregate-address A.B.C.D A.B.C.D",
      "Configure BGP aggregate entries\n"
      "Aggregate address\n"
      "Aggregate mask\n") {
	int ret;
	...
	return bgp_aggregate_set(vty, prefix_str, AFI_IP, bgp_node_safi(vty), 0, 0);
}

DEFUN(aggregate_address_summary_only, aggregate_address_summary_only_cmd, "aggregate-address A.B.C.D/M summary-only",
      "Configure BGP aggregate entries\n"
      "Aggregate prefix\n"
      "Filter more specific routes from updates\n") {
	return bgp_aggregate_set(vty, argv[0], AFI_IP, bgp_node_safi(vty), AGGREGATE_SUMMARY_ONLY, 0);
}

DEFUN(aggregate_address_mask_summary_only, aggregate_address_mask_summary_only_cmd, "aggregate-address A.B.C.D A.B.C.D summary-only",
      "Configure BGP aggregate entries\n"
      "Aggregate address\n"
      "Aggregate mask\n"
      "Filter more specific routes from updates\n") {
	...
	return bgp_aggregate_set(vty, prefix_str, AFI_IP, bgp_node_safi(vty), AGGREGATE_SUMMARY_ONLY, 0);
}

DEFUN(aggregate_address_as_set, aggregate_address_as_set_cmd, "aggregate-address A.B.C.D/M as-set",
      "Configure BGP aggregate entries\n"
      "Aggregate prefix\n"
      "Generate AS set path information\n") {
	return bgp_aggregate_set(vty, argv[0], AFI_IP, bgp_node_safi(vty), 0, AGGREGATE_AS_SET);
}

DEFUN(aggregate_address_mask_as_set, aggregate_address_mask_as_set_cmd, "aggregate-address A.B.C.D A.B.C.D as-set",
      "Configure BGP aggregate entries\n"
      "Aggregate address\n"
      "Aggregate mask\n"
      "Generate AS set path information\n") {
	...
	return bgp_aggregate_set(vty, prefix_str, AFI_IP, bgp_node_safi(vty), 0, AGGREGATE_AS_SET);
}

DEFUN(aggregate_address_as_set_summary, aggregate_address_as_set_summary_cmd, "aggregate-address A.B.C.D/M as-set summary-only",
      "Configure BGP aggregate entries\n"
      "Aggregate prefix\n"
      "Generate AS set path information\n"
      "Filter more specific routes from updates\n") {
	return bgp_aggregate_set(vty, argv[0], AFI_IP, bgp_node_safi(vty), AGGREGATE_SUMMARY_ONLY, AGGREGATE_AS_SET);
}

ALIAS(aggregate_address_as_set_summary, aggregate_address_summary_as_set_cmd, "aggregate-address A.B.C.D/M summary-only as-set",
      "Configure BGP aggregate entries\n"
      "Aggregate prefix\n"
      "Filter more specific routes from updates\n"
      "Generate AS set path information\n")

DEFUN(aggregate_address_mask_as_set_summary, aggregate_address_mask_as_set_summary_cmd, "aggregate-address A.B.C.D A.B.C.D as-set summary-only",
      "Configure BGP aggregate entries\n"
      "Aggregate address\n"
      "Aggregate mask\n"
      "Generate AS set path information\n"
	...
	return bgp_aggregate_set(vty, prefix_str, AFI_IP, bgp_node_safi(vty), AGGREGATE_SUMMARY_ONLY, AGGREGATE_AS_SET);
}

ALIAS(aggregate_address_mask_as_set_summary, aggregate_address_mask_summary_as_set_cmd, "aggregate-address A.B.C.D A.B.C.D summary-only as-set",
      "Configure BGP aggregate entries\n"
      "Aggregate address\n"
      "Aggregate mask\n"
      "Filter more specific routes from updates\n"
...
DEFUN(no_aggregate_address, no_aggregate_address_cmd, "no aggregate-address A.B.C.D/M",
      NO_STR "Configure BGP aggregate entries\n"
	     "Aggregate prefix\n") {
	return bgp_aggregate_unset(vty, argv[0], AFI_IP, bgp_node_safi(vty));
}

ALIAS(no_aggregate_address, no_aggregate_address_summary_only_cmd, "no aggregate-address A.B.C.D/M summary-only",
      NO_STR "Configure BGP aggregate entries\n"
	     "Aggregate prefix\n"
	     "Filter more specific routes from updates\n")

ALIAS(no_aggregate_address, no_aggregate_address_as_set_cmd, "no aggregate-address A.B.C.D/M as-set",
      NO_STR "Configure BGP aggregate entries\n"
	     "Aggregate prefix\n"
	     "Generate AS set path information\n")

ALIAS(no_aggregate_address, no_aggregate_address_as_set_summary_cmd, "no aggregate-address A.B.C.D/M as-set summary-only",
      NO_STR "Configure BGP aggregate entries\n"
	     "Aggregate prefix\n"
	     "Generate AS set path information\n"
	     "Filter more specific routes from updates\n")

ALIAS(no_aggregate_address, no_aggregate_address_summary_as_set_cmd, "no aggregate-address A.B.C.D/M summary-only as-set",
      NO_STR "Configure BGP aggregate entries\n"
	     "Aggregate prefix\n"
	     "Filter more specific routes from updates\n"
	     "Generate AS set path information\n")

DEFUN(no_aggregate_address_mask, no_aggregate_address_mask_cmd, "no aggregate-address A.B.C.D A.B.C.D",
      NO_STR "Configure BGP aggregate entries\n"
	     "Aggregate address\n"
	     "Aggregate mask\n") {
	int ret;
...
ALIAS(no_aggregate_address_mask, no_aggregate_address_mask_summary_only_cmd, "no aggregate-address A.B.C.D A.B.C.D summary-only",
      NO_STR "Configure BGP aggregate entries\n"
	     "Aggregate address\n"
	     "Aggregate mask\n"
	     "Filter more specific routes from updates\n")

ALIAS(no_aggregate_address_mask, no_aggregate_address_mask_as_set_cmd, "no aggregate-address A.B.C.D A.B.C.D as-set",
      NO_STR "Configure BGP aggregate entries\n"
	     "Aggregate address\n"
	     "Aggregate mask\n"
	     "Generate AS set path information\n")

ALIAS(no_aggregate_address_mask, no_aggregate_address_mask_as_set_summary_cmd, "no aggregate-address A.B.C.D A.B.C.D as-set summary-only",
      NO_STR "Configure BGP aggregate entries\n"
	     "Aggregate address\n"
	     "Aggregate mask\n"
	     "Generate AS set path information\n"
...
ALIAS(no_aggregate_address_mask, no_aggregate_address_mask_summary_as_set_cmd, "no aggregate-address A.B.C.D A.B.C.D summary-only as-set",
      NO_STR "Configure BGP aggregate entries\n"
	     "Aggregate address\n"
	     "Aggregate mask\n"
	     "Filter more specific routes from updates\n"
...
DEFUN(ipv6_aggregate_address, ipv6_aggregate_address_cmd, "aggregate-address X:X::X:X/M",
      "Configure BGP aggregate entries\n"
      "Aggregate prefix\n") {
	return bgp_aggregate_set(vty, argv[0], AFI_IP6, SAFI_UNICAST, 0, 0);
}

DEFUN(ipv6_aggregate_address_summary_only, ipv6_aggregate_address_summary_only_cmd, "aggregate-address X:X::X:X/M summary-only",
      "Configure BGP aggregate entries\n"
      "Aggregate prefix\n"
      "Filter more specific routes from updates\n") {
	return bgp_aggregate_set(vty, argv[0], AFI_IP6, SAFI_UNICAST, AGGREGATE_SUMMARY_ONLY, 0);
}

DEFUN(no_ipv6_aggregate_address, no_ipv6_aggregate_address_cmd, "no aggregate-address X:X::X:X/M",
      NO_STR "Configure BGP aggregate entries\n"
	     "Aggregate prefix\n") {
	return bgp_aggregate_unset(vty, argv[0], AFI_IP6, SAFI_UNICAST);
}

DEFUN(no_ipv6_aggregate_address_summary_only, no_ipv6_aggregate_address_summary_only_cmd, "no aggregate-address X:X::X:X/M summary-only",
      NO_STR "Configure BGP aggregate entries\n"
	     "Aggregate prefix\n"
	     "Filter more specific routes from updates\n") {
	return bgp_aggregate_unset(vty, argv[0], AFI_IP6, SAFI_UNICAST);
...
ALIAS(ipv6_aggregate_address, old_ipv6_aggregate_address_cmd, "ipv6 bgp aggregate-address X:X::X:X/M",
      IPV6_STR BGP_STR "Configure BGP aggregate entries\n"
		       "Aggregate prefix\n")

ALIAS(ipv6_aggregate_address_summary_only, old_ipv6_aggregate_address_summary_only_cmd, "ipv6 bgp aggregate-address X:X::X:X/M summary-only",
      IPV6_STR BGP_STR "Configure BGP aggregate entries\n"
		       "Aggregate prefix\n"
		       "Filter more specific routes from updates\n")

ALIAS(no_ipv6_aggregate_address, old_no_ipv6_aggregate_address_cmd, "no ipv6 bgp aggregate-address X:X::X:X/M",
      NO_STR IPV6_STR BGP_STR "Configure BGP aggregate entries\n"
			      "Aggregate prefix\n")

ALIAS(no_ipv6_aggregate_address_summary_only, old_no_ipv6_aggregate_address_summary_only_cmd, "no ipv6 bgp aggregate-address X:X::X:X/M summary-only",
      NO_STR IPV6_STR BGP_STR "Configure BGP aggregate entries\n"
			      "Aggregate prefix\n"
			      "Filter more specific routes from updates\n")

/* Redistribute route treatment. */
void bgp_redistribute_add(struct prefix *p, const struct in_addr *nexthop, const struct in6_addr *nexthop6, u_int32_t metric, u_char type, route_tag_t tag) {
	struct bgp *bgp;
	struct listnode *node, *nnode;
	struct bgp_info *new;
	struct bgp_info *bi;
	...
	for(ALL_LIST_ELEMENTS(bm->bgp, node, nnode, bgp)) {
		...
		if(bgp->redist[afi][type]) {
			...
			new = info_make(type, BGP_ROUTE_REDISTRIBUTE, bgp->peer_self, new_attr, bn);
			SET_FLAG(new->flags, BGP_INFO_VALID);
...
static void route_vty_out_route(struct prefix *p, struct vty *vty) {
	...
	if(p->family == AF_INET) {
		...
		if((IN_CLASSC(destination) && p->prefixlen == 24) || (IN_CLASSB(destination) && p->prefixlen == 16) || (IN_CLASSA(destination) && p->prefixlen == 8) || p->u.prefix4.s_addr == 0) {
			/* When mask is natural, mask is not displayed. */
		} else {
			len += vty_out(vty, "/%d", p->prefixlen);
		}
	} else {
		len = vty_out(vty, "%s/%d", inet_ntop(p->family, &p->u.prefix, buf, BUFSIZ), p->prefixlen);
	}
...
static void route_vty_short_status_out(struct vty *vty, struct bgp_info *binfo) {
	...
	if(CHECK_FLAG(binfo->flags, BGP_INFO_REMOVED)) {
		...
		vty_out(vty, "s");
	} else if(CHECK_FLAG(binfo->flags, BGP_INFO_VALID) && !CHECK_FLAG(binfo->flags, BGP_INFO_HISTORY)) {
		vty_out(vty, "*");
	} else {
		vty_out(vty, " ");
	}
       ...
       */
		if((safi == SAFI_ENCAP) || (safi == SAFI_MPLS_VPN)) {
			if(attr->extra) {
				...
				switch(af) {
					case AF_INET: vty_out(vty, "%s", inet_ntop(af, &attr->extra->mp_nexthop_global_in, buf, BUFSIZ)); break;
					case AF_INET6: vty_out(vty, "%s", inet_ntop(af, &attr->extra->mp_nexthop_global, buf, BUFSIZ)); break;
					default: vty_out(vty, "?");
				}
			} else {
				vty_out(vty, "?");
			...
			if(p->family == AF_INET) {
				...
				len = vty_out(vty, "%s", inet_ntop(AF_INET6, &attr->extra->mp_nexthop_global, buf, BUFSIZ));
				len = 16 - len;
				if(len < 1) {
...
void route_vty_out_tag(struct vty *vty, struct prefix *p, struct bgp_info *binfo, int display, safi_t safi) {
	struct attr *attr;
	u_int32_t label = 0;
	...
	if(attr) {
		if(p->family == AF_INET) {
			...
			if(attr->extra->mp_nexthop_len == 16) {
				vty_out(vty, "%s", inet_ntop(AF_INET6, &attr->extra->mp_nexthop_global, buf, BUFSIZ));
			} else if(attr->extra->mp_nexthop_len == 32) {
				vty_out(vty, "%s(%s)", inet_ntop(AF_INET6, &attr->extra->mp_nexthop_global, buf, BUFSIZ), inet_ntop(AF_INET6, &attr->extra->mp_nexthop_local, buf1, BUFSIZ));
			}
		}
	}
...
/* dampening route */
static void damp_route_vty_out(struct vty *vty, struct prefix *p, struct bgp_info *binfo, int display, safi_t safi) {
	struct attr *attr;
	int len;
...
/* flap route */
static void flap_route_vty_out(struct vty *vty, struct prefix *p, struct bgp_info *binfo, int display, safi_t safi) {
	struct attr *attr;
	...
	vty_out(vty, "%s ", peer_uptime(bdi->start_time, timebuf, BGP_UPTIME_LEN));

	if(CHECK_FLAG(binfo->flags, BGP_INFO_DAMPED) && !CHECK_FLAG(binfo->flags, BGP_INFO_HISTORY)) {
		vty_out(vty, "%s ", bgp_damp_reuse_time_vty(vty, binfo, timebuf, BGP_UPTIME_LEN));
	} else {
		vty_out(vty, "%*s ", 8, " ");
	}
...
static void route_vty_out_detail(struct vty *vty, struct bgp *bgp, struct prefix *p, struct bgp_info *binfo, afi_t afi, safi_t safi) {
	...
	if(attr) {
		...
		if(CHECK_FLAG(attr->flag, ATTR_FLAG_BIT(BGP_ATTR_AGGREGATOR))) {
			vty_out(vty, ", (aggregated by %u %s)", attr->extra->aggregator_as, inet_ntoa(attr->extra->aggregator_addr));
		}
		...
		if(p->family == AF_INET) {
			vty_out(vty, "    %s", ((safi == SAFI_MPLS_VPN) || (safi == SAFI_ENCAP)) ? inet_ntoa(attr->extra->mp_nexthop_global_in) : inet_ntoa(attr->nexthop));
		} else {
			assert(attr->extra);
			vty_out(vty, "    %s", inet_ntop(AF_INET6, &attr->extra->mp_nexthop_global, buf, INET6_ADDRSTRLEN));
		}

		if(binfo->peer == bgp->peer_self) {
			vty_out(vty, " from %s ", p->family == AF_INET ? "0.0.0.0" : "::");
		...
		if(attr->extra && attr->extra->mp_nexthop_len == 32) {
			vty_out(vty, "    (%s)%s", inet_ntop(AF_INET6, &attr->extra->mp_nexthop_local, buf, INET6_ADDRSTRLEN), VTY_NEWLINE);
		}
		...
		if(binfo->peer != bgp->peer_self) {
			if(binfo->peer->as == binfo->peer->local_as) {
				...
				vty_out(vty, ", %s", (bgp_confederation_peers_check(bgp, binfo->peer->as) ? "confed-external" : "external"));
			}
		...
		if(CHECK_FLAG(binfo->flags, BGP_INFO_MULTIPATH) || (CHECK_FLAG(binfo->flags, BGP_INFO_SELECTED) && bgp_info_mpath_count(binfo))) {
		...
		if(attr->community) {
			vty_out(vty, "      Community: %s%s", attr->community->str, VTY_NEWLINE);
		}

		/* Line 5 display Extended-community */
		if(attr->flag & ATTR_FLAG_BIT(BGP_ATTR_EXT_COMMUNITIES)) {
			vty_out(vty, "      Extended Community: %s%s", attr->extra->ecommunity->str, VTY_NEWLINE);
		}

		/* Line 6 display Large community */
		if(attr->flag & ATTR_FLAG_BIT(BGP_ATTR_LARGE_COMMUNITIES)) {
			vty_out(vty, "      Large Community: %s%s", attr->extra->lcommunity->str, VTY_NEWLINE);
		}

		/* Line 7 display Originator, Cluster-id */
		if((attr->flag & ATTR_FLAG_BIT(BGP_ATTR_ORIGINATOR_ID)) || (attr->flag & ATTR_FLAG_BIT(BGP_ATTR_CLUSTER_LIST))) {
			assert(attr->extra);
			if(attr->flag & ATTR_FLAG_BIT(BGP_ATTR_ORIGINATOR_ID)) {
				vty_out(vty, "      Originator: %s", inet_ntoa(attr->extra->originator_id));
			}
			...
			if(attr->flag & ATTR_FLAG_BIT(BGP_ATTR_CLUSTER_LIST)) {
				...
				for(i = 0; i < attr->extra->cluster->length / 4; i++) {
					vty_out(vty, "%s ", inet_ntoa(attr->extra->cluster->list[i]));
				}
			}
			vty_out(vty, "%s", VTY_NEWLINE);
		}
...
static int bgp_show_table(struct vty *vty, struct bgp_table *table, struct in_addr *router_id, enum bgp_show_type type, void *output_arg) {
	struct bgp_info *ri;
	struct bgp_node *rn;
	int header = 1;
	int display;
	...
	for(rn = bgp_table_top(table); rn; rn = bgp_route_next(rn)) {
		if(rn->info != NULL) {
			...
			for(ri = rn->info; ri; ri = ri->next) {
				total_count++;
				if(type == bgp_show_type_flap_statistics || type == bgp_show_type_flap_address || type == bgp_show_type_flap_prefix || type == bgp_show_type_flap_cidr_only || type == bgp_show_type_flap_regexp
				   || type == bgp_show_type_flap_filter_list || type == bgp_show_type_flap_prefix_list || type == bgp_show_type_flap_prefix_longer || type == bgp_show_type_flap_route_map
				   || type == bgp_show_type_flap_neighbor || type == bgp_show_type_dampend_paths || type == bgp_show_type_damp_neighbor) {
					if(!(ri->extra && ri->extra->damp_info)) {
						continue;
					}
				}
				if(type == bgp_show_type_regexp || type == bgp_show_type_flap_regexp) {
				...
				if(type == bgp_show_type_prefix_list || type == bgp_show_type_flap_prefix_list) {
				...
				if(type == bgp_show_type_filter_list || type == bgp_show_type_flap_filter_list) {
				...
				if(type == bgp_show_type_route_map || type == bgp_show_type_flap_route_map) {
				...
				if(type == bgp_show_type_neighbor || type == bgp_show_type_flap_neighbor || type == bgp_show_type_damp_neighbor) {
				...
				if(type == bgp_show_type_cidr_only || type == bgp_show_type_flap_cidr_only) {
					u_int32_t destination;
				...
				if(type == bgp_show_type_prefix_longer || type == bgp_show_type_flap_prefix_longer) {
				...
				if(type == bgp_show_type_community) {
					...
					if(!ri->attr->community || !community_match(ri->attr->community, com)) {
						continue;
					}
				}
				if(type == bgp_show_type_community_exact) {
					...
					if(!ri->attr->community || !community_cmp(ri->attr->community, com)) {
						continue;
					}
				}
				...
				if(type == bgp_show_type_lcommunity) {
					...
					if(!ri->attr->extra || !ri->attr->extra->lcommunity || !lcommunity_match(ri->attr->extra->lcommunity, lcom)) {
						continue;
					}
				}
				if(type == bgp_show_type_lcommunity_list) {
					...
					if(!ri->attr->extra || !lcommunity_list_match(ri->attr->extra->lcommunity, list)) {
						continue;
					}
				}
				...
				if(type == bgp_show_type_flap_address || type == bgp_show_type_flap_prefix) {
					struct prefix *p = output_arg;
				...
				if(type == bgp_show_type_dampend_paths || type == bgp_show_type_damp_neighbor) {
					if(!CHECK_FLAG(ri->flags, BGP_INFO_DAMPED) || CHECK_FLAG(ri->flags, BGP_INFO_HISTORY)) {
						continue;
					}
				}
	...
	if(output_count == 0) {
		...
		vty_out(vty, "%sDisplayed  %ld out of %ld total prefixes%s", VTY_NEWLINE, output_count, total_count, VTY_NEWLINE);
	}

	return CMD_SUCCESS;
}

static int bgp_show(struct vty *vty, struct bgp *bgp, afi_t afi, safi_t safi, enum bgp_show_type type, void *output_arg) {
	struct bgp_table *table;
...
static void route_vty_out_detail_header(struct vty *vty, struct bgp *bgp, struct bgp_node *rn, struct prefix_rd *prd, afi_t afi, safi_t safi) {
	struct bgp_info *ri;
	struct prefix *p;
	struct peer *peer;
	struct listnode *node, *nnode;
	...
	p = &rn->p;
	vty_out(vty, "BGP routing table entry for %s%s%s/%d%s", (printrd ? prefix_rd2str(prd, buf1, RD_ADDRSTRLEN) : ""), printrd ? ":" : "", inet_ntop(p->family, &p->u.prefix, buf2, INET6_ADDRSTRLEN), p->prefixlen, VTY_NEWLINE);
...
static int bgp_show_route_in_table(struct vty *vty, struct bgp *bgp, struct bgp_table *rib, const char *ip_str, afi_t afi, safi_t safi, struct prefix_rd *prd, int prefix_check, enum bgp_path_type pathtype) {
	int ret;
	int header;
	int display = 0;
	struct prefix match;
			...
			if((table = rn->info) != NULL) {
				...
				if((rm = bgp_node_match(table, &match)) != NULL) {
					...
					for(ri = rm->info; ri; ri = ri->next) {
						if(header) {
							route_vty_out_detail_header(vty, bgp, rm, (struct prefix_rd *) &rn->p, AFI_IP, safi);

							header = 0;
						}
						display++;

						if(pathtype == BGP_PATH_ALL || (pathtype == BGP_PATH_BESTPATH && CHECK_FLAG(ri->flags, BGP_INFO_SELECTED))
						   || (pathtype == BGP_PATH_MULTIPATH && (CHECK_FLAG(ri->flags, BGP_INFO_MULTIPATH) || CHECK_FLAG(ri->flags, BGP_INFO_SELECTED)))) {
...
static int bgp_show_route(struct vty *vty, const char *view_name, const char *ip_str, afi_t afi, safi_t safi, struct prefix_rd *prd, int prefix_check, enum bgp_path_type pathtype) {
	struct bgp *bgp;
	...
	return bgp_show_route_in_table(vty, bgp, bgp->rib[afi][safi], ip_str, afi, safi, prd, prefix_check, pathtype);
}

/* BGP route print out function. */
DEFUN(show_ip_bgp, show_ip_bgp_cmd, "show ip bgp", SHOW_STR IP_STR BGP_STR) {
	return bgp_show(vty, NULL, AFI_IP, SAFI_UNICAST, bgp_show_type_normal, NULL);
}

DEFUN(show_ip_bgp_ipv4, show_ip_bgp_ipv4_cmd, "show ip bgp ipv4 (unicast|multicast)",
      SHOW_STR IP_STR BGP_STR "Address family\n"
...
DEFUN(show_ip_bgp_route_pathtype, show_ip_bgp_route_pathtype_cmd, "show ip bgp A.B.C.D (bestpath|multipath)",
      SHOW_STR IP_STR BGP_STR "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n"
...
DEFUN(show_bgp_ipv4_safi_route_pathtype, show_bgp_ipv4_safi_route_pathtype_cmd, "show bgp ipv4 (unicast|multicast) A.B.C.D (bestpath|multipath)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n"
		       "Display only the bestpath\n"
...
DEFUN(show_ip_bgp_ipv4_route, show_ip_bgp_ipv4_route_cmd, "show ip bgp ipv4 (unicast|multicast) A.B.C.D",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
...
DEFUN(show_ip_bgp_vpnv4_rd_route, show_ip_bgp_vpnv4_rd_route_cmd, "show ip bgp vpnv4 rd ASN:nn_or_IP-address:nn A.B.C.D",
      SHOW_STR IP_STR BGP_STR "Display VPNv4 NLRI specific information\n"
...
DEFUN(show_ip_bgp_prefix, show_ip_bgp_prefix_cmd, "show ip bgp A.B.C.D/M", SHOW_STR IP_STR BGP_STR "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n") {
	return bgp_show_route(vty, NULL, argv[0], AFI_IP, SAFI_UNICAST, NULL, 1, BGP_PATH_ALL);
}

DEFUN(show_ip_bgp_prefix_pathtype, show_ip_bgp_prefix_pathtype_cmd, "show ip bgp A.B.C.D/M (bestpath|multipath)",
      SHOW_STR IP_STR BGP_STR "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n"
			      "Display only the bestpath\n"
			      "Display only multipaths\n") {
	if(strncmp(argv[1], "b", 1) == 0) {
...
DEFUN(show_ip_bgp_ipv4_prefix, show_ip_bgp_ipv4_prefix_cmd, "show ip bgp ipv4 (unicast|multicast) A.B.C.D/M",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n") {
...
DEFUN(show_ip_bgp_ipv4_prefix_pathtype, show_ip_bgp_ipv4_prefix_pathtype_cmd, "show ip bgp ipv4 (unicast|multicast) A.B.C.D/M (bestpath|multipath)",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n"
...
ALIAS(show_ip_bgp_ipv4_prefix_pathtype, show_bgp_ipv4_safi_prefix_pathtype_cmd, "show bgp ipv4 (unicast|multicast) A.B.C.D/M (bestpath|multipath)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n"
...
DEFUN(show_ip_bgp_vpnv4_all_prefix, show_ip_bgp_vpnv4_all_prefix_cmd, "show ip bgp vpnv4 all A.B.C.D/M",
      SHOW_STR IP_STR BGP_STR "Display VPNv4 NLRI specific information\n"
			      "Display information about all VPNv4 NLRIs\n"
			      "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n") {
...
DEFUN(show_ip_bgp_vpnv4_rd_prefix, show_ip_bgp_vpnv4_rd_prefix_cmd, "show ip bgp vpnv4 rd ASN:nn_or_IP-address:nn A.B.C.D/M",
      SHOW_STR IP_STR BGP_STR "Display VPNv4 NLRI specific information\n"
			      "Display information for a route distinguisher\n"
			      "VPN Route Distinguisher\n"
			      "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n") {
...
DEFUN(show_ip_bgp_view, show_ip_bgp_view_cmd, "show ip bgp view WORD",
...
DEFUN(show_ip_bgp_view_route, show_ip_bgp_view_route_cmd, "show ip bgp view WORD A.B.C.D",
      SHOW_STR IP_STR BGP_STR "BGP view\n"
			      "View name\n"
			      "Network in the BGP routing table to display\n") {
...
DEFUN(show_ip_bgp_view_prefix, show_ip_bgp_view_prefix_cmd, "show ip bgp view WORD A.B.C.D/M",
      SHOW_STR IP_STR BGP_STR "BGP view\n"
			      "View name\n"
			      "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n") {
...
DEFUN(show_bgp, show_bgp_cmd, "show bgp", SHOW_STR BGP_STR) {
	return bgp_show(vty, NULL, AFI_IP6, SAFI_UNICAST, bgp_show_type_normal, NULL);
}

ALIAS(show_bgp, show_bgp_ipv6_cmd, "show bgp ipv6", SHOW_STR BGP_STR "Address family\n")

/* old command */
DEFUN(show_ipv6_bgp, show_ipv6_bgp_cmd, "show ipv6 bgp", SHOW_STR IP_STR BGP_STR) {
	return bgp_show(vty, NULL, AFI_IP6, SAFI_UNICAST, bgp_show_type_normal, NULL);
}

DEFUN(show_bgp_route, show_bgp_route_cmd, "show bgp X:X::X:X", SHOW_STR BGP_STR "Network in the BGP routing table to display\n") {
	return bgp_show_route(vty, NULL, argv[0], AFI_IP6, SAFI_UNICAST, NULL, 0, BGP_PATH_ALL);
}

DEFUN(show_bgp_ipv4_safi, show_bgp_ipv4_safi_cmd, "show bgp ipv4 (unicast|multicast)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n") {
	if(strncmp(argv[0], "m", 1) == 0) {
...
DEFUN(show_bgp_ipv4_safi_route, show_bgp_ipv4_safi_route_cmd, "show bgp ipv4 (unicast|multicast) A.B.C.D",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Network in the BGP routing table to display\n") {
...
DEFUN(show_bgp_route_pathtype, show_bgp_route_pathtype_cmd, "show bgp X:X::X:X (bestpath|multipath)",
      SHOW_STR BGP_STR "Network in the BGP routing table to display\n"
		       "Display only the bestpath\n"
		       "Display only multipaths\n") {
	if(strncmp(argv[1], "b", 1) == 0) {
...
ALIAS(show_bgp_route_pathtype, show_bgp_ipv6_route_pathtype_cmd, "show bgp ipv6 X:X::X:X (bestpath|multipath)",
      SHOW_STR BGP_STR "Address family\n"
		       "Network in the BGP routing table to display\n"
		       "Display only the bestpath\n"
		       "Display only multipaths\n")

DEFUN(show_bgp_ipv6_safi_route_pathtype, show_bgp_ipv6_safi_route_pathtype_cmd, "show bgp ipv6 (unicast|multicast) X:X::X:X (bestpath|multipath)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Network in the BGP routing table to display\n"
...
DEFUN(show_bgp_ipv4_vpn_route, show_bgp_ipv4_vpn_route_cmd, "show bgp ipv4 vpn A.B.C.D",
      SHOW_STR BGP_STR "Address Family\n"
...
DEFUN(show_bgp_ipv6_vpn_route, show_bgp_ipv6_vpn_route_cmd, "show bgp ipv6 vpn X:X::X:X",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display VPN NLRI specific information\n"
		       "Network in the BGP routing table to display\n") {
...
DEFUN(show_bgp_ipv4_vpn_rd_route, show_bgp_ipv4_vpn_rd_route_cmd, "show bgp ipv4 vpn rd ASN:nn_or_IP-address:nn A.B.C.D",
      SHOW_STR BGP_STR IP_STR "Display VPN NLRI specific information\n"
			      "Display information for a route distinguisher\n"
			      "VPN Route Distinguisher\n"
			      "Network in the BGP routing table to display\n") {
	int ret;
...
DEFUN(show_bgp_ipv6_vpn_rd_route, show_bgp_ipv6_vpn_rd_route_cmd, "show bgp ipv6 vpn rd ASN:nn_or_IP-address:nn X:X::X:X",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display VPN NLRI specific information\n"
		       "Display information for a route distinguisher\n"
		       "VPN Route Distinguisher\n"
...
DEFUN(show_bgp_prefix_pathtype, show_bgp_prefix_pathtype_cmd, "show bgp X:X::X:X/M (bestpath|multipath)",
      SHOW_STR BGP_STR "IPv6 prefix <network>/<length>\n"
		       "Display only the bestpath\n"
		       "Display only multipaths\n") {
	if(strncmp(argv[1], "b", 1) == 0) {
...
ALIAS(show_bgp_prefix_pathtype, show_bgp_ipv6_prefix_pathtype_cmd, "show bgp ipv6 X:X::X:X/M (bestpath|multipath)",
      SHOW_STR BGP_STR "Address family\n"
		       "IPv6 prefix <network>/<length>\n"
		       "Display only the bestpath\n"
		       "Display only multipaths\n")

DEFUN(show_bgp_ipv6_safi_prefix_pathtype, show_bgp_ipv6_safi_prefix_pathtype_cmd, "show bgp ipv6 (unicast|multicast) X:X::X:X/M (bestpath|multipath)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "IPv6 prefix <network>/<length>, e.g., 3ffe::/16\n"
...
DEFUN(show_bgp_ipv4_encap_route, show_bgp_ipv4_encap_route_cmd, "show bgp ipv4 encap A.B.C.D",
...
DEFUN(show_bgp_ipv6_encap_route, show_bgp_ipv6_encap_route_cmd, "show bgp ipv6 encap X:X::X:X",
      SHOW_STR BGP_STR IP6_STR "Display ENCAP NLRI specific information\n"
			       "Network in the BGP routing table to display\n") {
	return bgp_show_route(vty, NULL, argv[0], AFI_IP6, SAFI_ENCAP, NULL, 0, BGP_PATH_ALL);
}

DEFUN(show_bgp_ipv4_safi_rd_route, show_bgp_ipv4_safi_rd_route_cmd, "show bgp ipv4 (encap|vpn) rd ASN:nn_or_IP-address:nn A.B.C.D",
      SHOW_STR BGP_STR "Address Family\n"
		       "Address Family Modifier\n"
		       "Address Family Modifier\n"
		       "Display information for a route distinguisher\n"
...
DEFUN(show_bgp_ipv6_safi_rd_route, show_bgp_ipv6_safi_rd_route_cmd, "show bgp ipv6 (encap|vpn) rd ASN:nn_or_IP-address:nn X:X::X:X",
      SHOW_STR BGP_STR "Address Family\n"
		       "Address Family Modifier\n"
		       "Address Family Modifier\n"
		       "Display information for a route distinguisher\n"
...
DEFUN(show_bgp_ipv4_prefix, show_bgp_ipv4_prefix_cmd, "show bgp ipv4 A.B.C.D/M", SHOW_STR BGP_STR IP_STR "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n") {
	return bgp_show_route(vty, NULL, argv[0], AFI_IP, SAFI_UNICAST, NULL, 1, BGP_PATH_ALL);
}

DEFUN(show_bgp_ipv4_safi_prefix, show_bgp_ipv4_safi_prefix_cmd, "show bgp ipv4 (unicast|multicast) A.B.C.D/M",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n") {
...
DEFUN(show_bgp_ipv4_vpn_prefix, show_bgp_ipv4_vpn_prefix_cmd, "show bgp ipv4 vpn A.B.C.D/M",
      SHOW_STR BGP_STR IP_STR "Display VPN NLRI specific information\n"
			      "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n") {
	return bgp_show_route(vty, NULL, argv[0], AFI_IP, SAFI_MPLS_VPN, NULL, 1, BGP_PATH_ALL);
}

DEFUN(show_bgp_ipv6_vpn_prefix, show_bgp_ipv6_vpn_prefix_cmd, "show bgp ipv6 vpn X:X::X:X/M",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display VPN NLRI specific information\n"
		       "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n") {
...
DEFUN(show_bgp_ipv4_encap_prefix, show_bgp_ipv4_encap_prefix_cmd, "show bgp ipv4 encap A.B.C.D/M",
      SHOW_STR BGP_STR IP_STR "Display ENCAP NLRI specific information\n"
...
DEFUN(show_bgp_ipv6_encap_prefix, show_bgp_ipv6_encap_prefix_cmd, "show bgp ipv6 encap X:X::X:X/M",
      SHOW_STR BGP_STR IP_STR "Display ENCAP NLRI specific information\n"
			      "Display information about ENCAP NLRIs\n"
			      "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n") {
...
DEFUN(show_bgp_ipv4_safi_rd_prefix, show_bgp_ipv4_safi_rd_prefix_cmd, "show bgp ipv4 (encap|vpn) rd ASN:nn_or_IP-address:nn A.B.C.D/M",
      SHOW_STR BGP_STR "Address Family\n"
		       "Address Family Modifier\n"
		       "Address Family Modifier\n"
		       "Display information for a route distinguisher\n"
...
DEFUN(show_bgp_ipv6_safi_rd_prefix, show_bgp_ipv6_safi_rd_prefix_cmd, "show bgp ipv6 (encap|vpn) rd ASN:nn_or_IP-address:nn X:X::X:X/M",
      SHOW_STR BGP_STR "Address Family\n"
		       "Address Family Modifier\n"
		       "Address Family Modifier\n"
		       "Display information for a route distinguisher\n"
...
DEFUN(show_bgp_afi_safi_view, show_bgp_afi_safi_view_cmd, "show bgp view WORD (ipv4|ipv6) (encap|mulicast|unicast|vpn)",
...
DEFUN(show_bgp_view_afi_safi_route, show_bgp_view_afi_safi_route_cmd, "show bgp view WORD (ipv4|ipv6) (encap|multicast|unicast|vpn) A.B.C.D",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address Family\n"
		       "Address Family\n"
...
DEFUN(show_bgp_view_afi_safi_prefix, show_bgp_view_afi_safi_prefix_cmd, "show bgp view WORD (ipv4|ipv6) (encap|multicast|unicast|vpn) A.B.C.D/M",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address Family\n"
		       "Address Family\n"
...
/* new001 */
DEFUN(show_bgp_afi, show_bgp_afi_cmd, "show bgp (ipv4|ipv6)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address family\n") {
	afi_t afi;
	...
	return bgp_show(vty, NULL, afi, SAFI_UNICAST, bgp_show_type_normal, NULL);
}

DEFUN(show_bgp_ipv6_safi, show_bgp_ipv6_safi_cmd, "show bgp ipv6 (unicast|multicast)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n") {
	if(strncmp(argv[0], "m", 1) == 0) {
		return bgp_show(vty, NULL, AFI_IP6, SAFI_MULTICAST, bgp_show_type_normal, NULL);
	}

	return bgp_show(vty, NULL, AFI_IP6, SAFI_UNICAST, bgp_show_type_normal, NULL);
}

DEFUN(show_bgp_ipv6_route, show_bgp_ipv6_route_cmd, "show bgp ipv6 X:X::X:X",
      SHOW_STR BGP_STR "Address family\n"
		       "Network in the BGP routing table to display\n") {
	return bgp_show_route(vty, NULL, argv[0], AFI_IP6, SAFI_UNICAST, NULL, 0, BGP_PATH_ALL);
}

DEFUN(show_bgp_ipv6_safi_route, show_bgp_ipv6_safi_route_cmd, "show bgp ipv6 (unicast|multicast) X:X::X:X",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Network in the BGP routing table to display\n") {
...
/* old command */
DEFUN(show_ipv6_bgp_route, show_ipv6_bgp_route_cmd, "show ipv6 bgp X:X::X:X", SHOW_STR IP_STR BGP_STR "Network in the BGP routing table to display\n") {
	return bgp_show_route(vty, NULL, argv[0], AFI_IP6, SAFI_UNICAST, NULL, 0, BGP_PATH_ALL);
}

DEFUN(show_bgp_prefix, show_bgp_prefix_cmd, "show bgp X:X::X:X/M", SHOW_STR BGP_STR "IPv6 prefix <network>/<length>\n") {
	return bgp_show_route(vty, NULL, argv[0], AFI_IP6, SAFI_UNICAST, NULL, 1, BGP_PATH_ALL);
}

/* new002 */
DEFUN(show_bgp_ipv6_prefix, show_bgp_ipv6_prefix_cmd, "show bgp ipv6 X:X::X:X/M",
      SHOW_STR BGP_STR "Address family\n"
		       "IPv6 prefix <network>/<length>, e.g., 3ffe::/16\n") {
	return bgp_show_route(vty, NULL, argv[0], AFI_IP6, SAFI_UNICAST, NULL, 1, BGP_PATH_ALL);
}

DEFUN(show_bgp_ipv6_safi_prefix, show_bgp_ipv6_safi_prefix_cmd, "show bgp ipv6 (unicast|multicast) X:X::X:X/M",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "IPv6 prefix <network>/<length>, e.g., 3ffe::/16\n") {
...
/* old command */
DEFUN(show_ipv6_bgp_prefix, show_ipv6_bgp_prefix_cmd, "show ipv6 bgp X:X::X:X/M", SHOW_STR IP_STR BGP_STR "IPv6 prefix <network>/<length>, e.g., 3ffe::/16\n") {
	return bgp_show_route(vty, NULL, argv[0], AFI_IP6, SAFI_UNICAST, NULL, 1, BGP_PATH_ALL);
}

DEFUN(show_bgp_view, show_bgp_view_cmd, "show bgp view WORD",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n") {
	struct bgp *bgp;
...
DEFUN(show_bgp_view_ipv6, show_bgp_view_ipv6_cmd, "show bgp view WORD ipv6",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n") {
	struct bgp *bgp;
...
DEFUN(show_bgp_view_route, show_bgp_view_route_cmd, "show bgp view WORD X:X::X:X",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Network in the BGP routing table to display\n") {
...
DEFUN(show_bgp_view_ipv6_route, show_bgp_view_ipv6_route_cmd, "show bgp view WORD ipv6 X:X::X:X",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "Network in the BGP routing table to display\n") {
...
/* old command */
DEFUN(show_ipv6_mbgp, show_ipv6_mbgp_cmd, "show ipv6 mbgp", SHOW_STR IP_STR MBGP_STR) {
	return bgp_show(vty, NULL, AFI_IP6, SAFI_MULTICAST, bgp_show_type_normal, NULL);
}

/* old command */
DEFUN(show_ipv6_mbgp_route, show_ipv6_mbgp_route_cmd, "show ipv6 mbgp X:X::X:X", SHOW_STR IP_STR MBGP_STR "Network in the MBGP routing table to display\n") {
	return bgp_show_route(vty, NULL, argv[0], AFI_IP6, SAFI_MULTICAST, NULL, 0, BGP_PATH_ALL);
}

/* old command */
DEFUN(show_ipv6_mbgp_prefix, show_ipv6_mbgp_prefix_cmd, "show ipv6 mbgp X:X::X:X/M", SHOW_STR IP_STR MBGP_STR "IPv6 prefix <network>/<length>, e.g., 3ffe::/16\n") {
	return bgp_show_route(vty, NULL, argv[0], AFI_IP6, SAFI_MULTICAST, NULL, 1, BGP_PATH_ALL);
}

DEFUN(show_bgp_view_prefix, show_bgp_view_prefix_cmd, "show bgp view WORD X:X::X:X/M",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "IPv6 prefix <network>/<length>\n") {
...
DEFUN(show_bgp_view_ipv6_prefix, show_bgp_view_ipv6_prefix_cmd, "show bgp view WORD ipv6 X:X::X:X/M",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "IPv6 prefix <network>/<length>\n") {
...
static int bgp_show_regexp(struct vty *vty, int argc, const char **argv, afi_t afi, safi_t safi, enum bgp_show_type type) {
	int i;
	struct buffer *b;
	char *regstr;
	int first;
	...
	if(!regex) {
		vty_out(vty, "Can't compile regexp %s%s", argv[0], VTY_NEWLINE);
		return CMD_WARNING;
	}
...
DEFUN(show_ip_bgp_regexp, show_ip_bgp_regexp_cmd, "show ip bgp regexp .LINE",
      SHOW_STR IP_STR BGP_STR "Display routes matching the AS path regular expression\n"
			      "A regular-expression to match the BGP AS paths\n") {
	return bgp_show_regexp(vty, argc, argv, AFI_IP, SAFI_UNICAST, bgp_show_type_regexp);
}

DEFUN(show_ip_bgp_flap_regexp, show_ip_bgp_flap_regexp_cmd, "show ip bgp flap-statistics regexp .LINE",
      SHOW_STR IP_STR BGP_STR "Display flap statistics of routes\n"
			      "Display routes matching the AS path regular expression\n"
			      "A regular-expression to match the BGP AS paths\n") {
	return bgp_show_regexp(vty, argc, argv, AFI_IP, SAFI_UNICAST, bgp_show_type_flap_regexp);
}

ALIAS(show_ip_bgp_flap_regexp, show_ip_bgp_damp_flap_regexp_cmd, "show ip bgp dampening flap-statistics regexp .LINE",
      SHOW_STR IP_STR BGP_STR "Display detailed information about dampening\n"
			      "Display flap statistics of routes\n"
			      "Display routes matching the AS path regular expression\n"
			      "A regular-expression to match the BGP AS paths\n")

DEFUN(show_ip_bgp_ipv4_regexp, show_ip_bgp_ipv4_regexp_cmd, "show ip bgp ipv4 (unicast|multicast) regexp .LINE",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Display routes matching the AS path regular expression\n"
			      "A regular-expression to match the BGP AS paths\n") {
	if(strncmp(argv[0], "m", 1) == 0) {
		return bgp_show_regexp(vty, argc, argv, AFI_IP, SAFI_MULTICAST, bgp_show_type_regexp);
	}

	return bgp_show_regexp(vty, argc, argv, AFI_IP, SAFI_UNICAST, bgp_show_type_regexp);
}

DEFUN(show_bgp_regexp, show_bgp_regexp_cmd, "show bgp regexp .LINE",
      SHOW_STR BGP_STR "Display routes matching the AS path regular expression\n"
		       "A regular-expression to match the BGP AS paths\n") {
	return bgp_show_regexp(vty, argc, argv, AFI_IP6, SAFI_UNICAST, bgp_show_type_regexp);
}

/* old command */
DEFUN(show_ipv6_bgp_regexp, show_ipv6_bgp_regexp_cmd, "show ipv6 bgp regexp .LINE",
      SHOW_STR IP_STR BGP_STR "Display routes matching the AS path regular expression\n"
			      "A regular-expression to match the BGP AS paths\n") {
	return bgp_show_regexp(vty, argc, argv, AFI_IP6, SAFI_UNICAST, bgp_show_type_regexp);
}
...
/* old command */
DEFUN(show_ipv6_mbgp_regexp, show_ipv6_mbgp_regexp_cmd, "show ipv6 mbgp regexp .LINE",
      SHOW_STR IP_STR BGP_STR "Display routes matching the AS path regular expression\n"
			      "A regular-expression to match the MBGP AS paths\n") {
	return bgp_show_regexp(vty, argc, argv, AFI_IP6, SAFI_MULTICAST, bgp_show_type_regexp);
}

DEFUN(show_bgp_ipv4_safi_flap_regexp, show_bgp_ipv4_safi_flap_regexp_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) flap-statistics regexp .LINE",
      SHOW_STR BGP_STR IP_STR "Address Family Modifier\n"
			      "Address Family Modifier\n"
			      "Address Family Modifier\n"
			      "Address Family Modifier\n"
			      "Display flap statistics of routes\n"
	...
	return bgp_show_regexp(vty, argc - 1, argv + 1, AFI_IP, safi, bgp_show_type_flap_regexp);
}

ALIAS(show_bgp_ipv4_safi_flap_regexp, show_bgp_ipv4_safi_damp_flap_regexp_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) dampening flap-statistics regexp .LINE",
      SHOW_STR BGP_STR IP_STR "Address Family Modifier\n"
			      "Address Family Modifier\n"
			      "Address Family Modifier\n"
			      "Address Family Modifier\n"
...
DEFUN(show_bgp_ipv6_safi_flap_regexp, show_bgp_ipv6_safi_flap_regexp_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) flap-statistics regexp .LINE",
      SHOW_STR BGP_STR IPV6_STR "Address Family Modifier\n"
				"Address Family Modifier\n"
				"Address Family Modifier\n"
				"Address Family Modifier\n"
				"Display flap statistics of routes\n"
	...
	return bgp_show_regexp(vty, argc - 1, argv + 1, AFI_IP6, safi, bgp_show_type_flap_regexp);
}

ALIAS(show_bgp_ipv6_safi_flap_regexp, show_bgp_ipv6_safi_damp_flap_regexp_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) dampening flap-statistics regexp .LINE",
      SHOW_STR BGP_STR IPV6_STR "Address Family Modifier\n"
				"Address Family Modifier\n"
				"Address Family Modifier\n"
				"Address Family Modifier\n"
...
DEFUN(show_bgp_ipv4_safi_regexp, show_bgp_ipv4_safi_regexp_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) regexp .LINE",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
	...
	return bgp_show_regexp(vty, argc - 1, argv + 1, AFI_IP, safi, bgp_show_type_regexp);
}

DEFUN(show_bgp_ipv6_safi_regexp, show_bgp_ipv6_safi_regexp_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) regexp .LINE",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
	...
	return bgp_show_regexp(vty, argc - 1, argv + 1, AFI_IP6, safi, bgp_show_type_regexp);
}

DEFUN(show_bgp_ipv6_regexp, show_bgp_ipv6_regexp_cmd, "show bgp ipv6 regexp .LINE",
      SHOW_STR BGP_STR "Address family\n"
		       "Display routes matching the AS path regular expression\n"
		       "A regular-expression to match the BGP AS paths\n") {
	return bgp_show_regexp(vty, argc, argv, AFI_IP6, SAFI_UNICAST, bgp_show_type_regexp);
...
static int bgp_show_prefix_list(struct vty *vty, const char *prefix_list_str, afi_t afi, safi_t safi, enum bgp_show_type type) {
...
DEFUN(show_ip_bgp_prefix_list, show_ip_bgp_prefix_list_cmd, "show ip bgp prefix-list WORD",
      SHOW_STR IP_STR BGP_STR "Display routes conforming to the prefix-list\n"
			      "IP prefix-list name\n") {
	return bgp_show_prefix_list(vty, argv[0], AFI_IP, SAFI_UNICAST, bgp_show_type_prefix_list);
}

DEFUN(show_ip_bgp_flap_prefix_list, show_ip_bgp_flap_prefix_list_cmd, "show ip bgp flap-statistics prefix-list WORD",
      SHOW_STR IP_STR BGP_STR "Display flap statistics of routes\n"
			      "Display routes conforming to the prefix-list\n"
			      "IP prefix-list name\n") {
	return bgp_show_prefix_list(vty, argv[0], AFI_IP, SAFI_UNICAST, bgp_show_type_flap_prefix_list);
}

ALIAS(show_ip_bgp_flap_prefix_list, show_ip_bgp_damp_flap_prefix_list_cmd, "show ip bgp dampening flap-statistics prefix-list WORD",
      SHOW_STR IP_STR BGP_STR "Display detailed information about dampening\n"
			      "Display flap statistics of routes\n"
			      "Display routes conforming to the prefix-list\n"
			      "IP prefix-list name\n")

DEFUN(show_ip_bgp_ipv4_prefix_list, show_ip_bgp_ipv4_prefix_list_cmd, "show ip bgp ipv4 (unicast|multicast) prefix-list WORD",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Display routes conforming to the prefix-list\n"
			      "IP prefix-list name\n") {
	if(strncmp(argv[0], "m", 1) == 0) {
		return bgp_show_prefix_list(vty, argv[1], AFI_IP, SAFI_MULTICAST, bgp_show_type_prefix_list);
	}

	return bgp_show_prefix_list(vty, argv[1], AFI_IP, SAFI_UNICAST, bgp_show_type_prefix_list);
}

DEFUN(show_bgp_prefix_list, show_bgp_prefix_list_cmd, "show bgp prefix-list WORD",
      SHOW_STR BGP_STR "Display routes conforming to the prefix-list\n"
		       "IPv6 prefix-list name\n") {
	return bgp_show_prefix_list(vty, argv[0], AFI_IP6, SAFI_UNICAST, bgp_show_type_prefix_list);
}

ALIAS(show_bgp_prefix_list, show_bgp_ipv6_prefix_list_cmd, "show bgp ipv6 prefix-list WORD",
      SHOW_STR BGP_STR "Address family\n"
		       "Display routes conforming to the prefix-list\n"
		       "IPv6 prefix-list name\n")

/* old command */
DEFUN(show_ipv6_bgp_prefix_list, show_ipv6_bgp_prefix_list_cmd, "show ipv6 bgp prefix-list WORD",
      SHOW_STR IPV6_STR BGP_STR "Display routes matching the prefix-list\n"
				"IPv6 prefix-list name\n") {
	return bgp_show_prefix_list(vty, argv[0], AFI_IP6, SAFI_UNICAST, bgp_show_type_prefix_list);
}

/* old command */
DEFUN(show_ipv6_mbgp_prefix_list, show_ipv6_mbgp_prefix_list_cmd, "show ipv6 mbgp prefix-list WORD",
      SHOW_STR IPV6_STR MBGP_STR "Display routes matching the prefix-list\n"
				 "IPv6 prefix-list name\n") {
	return bgp_show_prefix_list(vty, argv[0], AFI_IP6, SAFI_MULTICAST, bgp_show_type_prefix_list);
}

DEFUN(show_bgp_ipv4_prefix_list, show_bgp_ipv4_prefix_list_cmd, "show bgp ipv4 prefix-list WORD",
      SHOW_STR BGP_STR IP_STR "Display routes conforming to the prefix-list\n"
			      "IP prefix-list name\n") {
	return bgp_show_prefix_list(vty, argv[0], AFI_IP, SAFI_UNICAST, bgp_show_type_prefix_list);
}

DEFUN(show_bgp_ipv4_safi_flap_prefix_list, show_bgp_ipv4_safi_flap_prefix_list_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) flap-statistics prefix-list WORD",
      SHOW_STR BGP_STR IP_STR "Address Family Modifier\n"
			      "Address Family Modifier\n"
			      "Address Family Modifier\n"
			      "Address Family Modifier\n"
	...
	return bgp_show_prefix_list(vty, argv[1], AFI_IP, safi, bgp_show_type_flap_prefix_list);
}

ALIAS(show_bgp_ipv4_safi_flap_prefix_list, show_bgp_ipv4_safi_damp_flap_prefix_list_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) dampening flap-statistics prefix-list WORD",
      SHOW_STR BGP_STR IP_STR "Address Family Modifier\n"
			      "Address Family Modifier\n"
			      "Address Family Modifier\n"
			      "Address Family Modifier\n"
...
DEFUN(show_bgp_ipv6_safi_flap_prefix_list, show_bgp_ipv6_safi_flap_prefix_list_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) flap-statistics prefix-list WORD",
      SHOW_STR BGP_STR IPV6_STR "Address Family Modifier\n"
				"Address Family Modifier\n"
				"Address Family Modifier\n"
				"Address Family Modifier\n"
	...
	return bgp_show_prefix_list(vty, argv[1], AFI_IP6, safi, bgp_show_type_flap_prefix_list);
}
ALIAS(show_bgp_ipv6_safi_flap_prefix_list, show_bgp_ipv6_safi_damp_flap_prefix_list_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) dampening flap-statistics prefix-list WORD",
      SHOW_STR BGP_STR IPV6_STR "Address Family Modifier\n"
				"Address Family Modifier\n"
				"Address Family Modifier\n"
				"Address Family Modifier\n"
...
DEFUN(show_bgp_ipv4_safi_prefix_list, show_bgp_ipv4_safi_prefix_list_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) prefix-list WORD",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
	...
	return bgp_show_prefix_list(vty, argv[1], AFI_IP, safi, bgp_show_type_prefix_list);
}

DEFUN(show_bgp_ipv6_safi_prefix_list, show_bgp_ipv6_safi_prefix_list_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) prefix-list WORD",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
	...
	return bgp_show_prefix_list(vty, argv[1], AFI_IP6, safi, bgp_show_type_prefix_list);
}

static int bgp_show_filter_list(struct vty *vty, const char *filter, afi_t afi, safi_t safi, enum bgp_show_type type) {
...
DEFUN(show_ip_bgp_filter_list, show_ip_bgp_filter_list_cmd, "show ip bgp filter-list WORD",
      SHOW_STR IP_STR BGP_STR "Display routes conforming to the filter-list\n"
			      "Regular expression access list name\n") {
	return bgp_show_filter_list(vty, argv[0], AFI_IP, SAFI_UNICAST, bgp_show_type_filter_list);
}

DEFUN(show_ip_bgp_flap_filter_list, show_ip_bgp_flap_filter_list_cmd, "show ip bgp flap-statistics filter-list WORD",
      SHOW_STR IP_STR BGP_STR "Display flap statistics of routes\n"
			      "Display routes conforming to the filter-list\n"
			      "Regular expression access list name\n") {
	return bgp_show_filter_list(vty, argv[0], AFI_IP, SAFI_UNICAST, bgp_show_type_flap_filter_list);
}

ALIAS(show_ip_bgp_flap_filter_list, show_ip_bgp_damp_flap_filter_list_cmd, "show ip bgp dampening flap-statistics filter-list WORD",
      SHOW_STR IP_STR BGP_STR "Display detailed information about dampening\n"
			      "Display flap statistics of routes\n"
			      "Display routes conforming to the filter-list\n"
			      "Regular expression access list name\n")

DEFUN(show_ip_bgp_ipv4_filter_list, show_ip_bgp_ipv4_filter_list_cmd, "show ip bgp ipv4 (unicast|multicast) filter-list WORD",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Display routes conforming to the filter-list\n"
			      "Regular expression access list name\n") {
	if(strncmp(argv[0], "m", 1) == 0) {
		return bgp_show_filter_list(vty, argv[1], AFI_IP, SAFI_MULTICAST, bgp_show_type_filter_list);
	}

	return bgp_show_filter_list(vty, argv[1], AFI_IP, SAFI_UNICAST, bgp_show_type_filter_list);
}

DEFUN(show_bgp_filter_list, show_bgp_filter_list_cmd, "show bgp filter-list WORD",
      SHOW_STR BGP_STR "Display routes conforming to the filter-list\n"
		       "Regular expression access list name\n") {
	return bgp_show_filter_list(vty, argv[0], AFI_IP6, SAFI_UNICAST, bgp_show_type_filter_list);
}

/* old command */
DEFUN(show_ipv6_bgp_filter_list, show_ipv6_bgp_filter_list_cmd, "show ipv6 bgp filter-list WORD",
      SHOW_STR IPV6_STR BGP_STR "Display routes conforming to the filter-list\n"
				"Regular expression access list name\n") {
	return bgp_show_filter_list(vty, argv[0], AFI_IP6, SAFI_UNICAST, bgp_show_type_filter_list);
}

/* old command */
DEFUN(show_ipv6_mbgp_filter_list, show_ipv6_mbgp_filter_list_cmd, "show ipv6 mbgp filter-list WORD",
      SHOW_STR IPV6_STR MBGP_STR "Display routes conforming to the filter-list\n"
				 "Regular expression access list name\n") {
	return bgp_show_filter_list(vty, argv[0], AFI_IP6, SAFI_MULTICAST, bgp_show_type_filter_list);
}

DEFUN(show_ip_bgp_dampening_info, show_ip_bgp_dampening_params_cmd, "show ip bgp dampening parameters",
      SHOW_STR IP_STR BGP_STR "Display detailed information about dampening\n"
			      "Display detail of configured dampening parameters\n") {
	return bgp_show_dampening_parameters(vty, AFI_IP, SAFI_UNICAST);
}

DEFUN(show_bgp_ipv4_filter_list, show_bgp_ipv4_filter_list_cmd, "show bgp ipv4 filter-list WORD",
      SHOW_STR BGP_STR IP_STR "Display routes conforming to the filter-list\n"
			      "Regular expression access list name\n") {
	return bgp_show_filter_list(vty, argv[0], AFI_IP, SAFI_UNICAST, bgp_show_type_filter_list);
}

DEFUN(show_bgp_ipv4_safi_flap_filter_list, show_bgp_ipv4_safi_flap_filter_list_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) flap-statistics filter-list WORD",
      SHOW_STR BGP_STR IP_STR "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
	...
	return bgp_show_filter_list(vty, argv[1], AFI_IP, safi, bgp_show_type_flap_filter_list);
}

ALIAS(show_bgp_ipv4_safi_flap_filter_list, show_bgp_ipv4_safi_damp_flap_filter_list_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) dampening flap-statistics filter-list WORD",
      SHOW_STR BGP_STR IP_STR "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
...
DEFUN(show_bgp_ipv6_safi_flap_filter_list, show_bgp_ipv6_safi_flap_filter_list_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) flap-statistics filter-list WORD",
      SHOW_STR BGP_STR IPV6_STR "Address Family modifier\n"
				"Address Family modifier\n"
				"Address Family modifier\n"
				"Address Family modifier\n"
	...
	return bgp_show_filter_list(vty, argv[1], AFI_IP6, safi, bgp_show_type_flap_filter_list);
}
ALIAS(show_bgp_ipv6_safi_flap_filter_list, show_bgp_ipv6_safi_damp_flap_filter_list_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) dampening flap-statistics filter-list WORD",
      SHOW_STR BGP_STR IPV6_STR "Address Family modifier\n"
				"Address Family modifier\n"
				"Address Family modifier\n"
				"Address Family modifier\n"
...
DEFUN(show_bgp_ipv4_safi_filter_list, show_bgp_ipv4_safi_filter_list_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) filter-list WORD",
      SHOW_STR BGP_STR "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Display routes conforming to the filter-list\n"
	...
	return bgp_show_filter_list(vty, argv[1], AFI_IP, safi, bgp_show_type_filter_list);
}

DEFUN(show_bgp_ipv6_safi_filter_list, show_bgp_ipv6_safi_filter_list_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) filter-list WORD",
      SHOW_STR BGP_STR "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
	...
	return bgp_show_filter_list(vty, argv[1], AFI_IP6, safi, bgp_show_type_filter_list);
}

DEFUN(show_bgp_ipv6_filter_list, show_bgp_ipv6_filter_list_cmd, "show bgp ipv6 filter-list WORD",
      SHOW_STR BGP_STR "Address family\n"
		       "Display routes conforming to the filter-list\n"
		       "Regular expression access list name\n") {
	return bgp_show_filter_list(vty, argv[0], AFI_IP6, SAFI_UNICAST, bgp_show_type_filter_list);
...
DEFUN(show_ip_bgp_ipv4_dampening_parameters, show_ip_bgp_ipv4_dampening_parameters_cmd, "show ip bgp ipv4 (unicast|multicast) dampening parameters",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Display detailed information about dampening\n"
...
DEFUN(show_ip_bgp_ipv4_dampening_flap_stats, show_ip_bgp_ipv4_dampening_flap_stats_cmd, "show ip bgp ipv4 (unicast|multicast) dampening flap-statistics",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Display detailed information about dampening\n"
			      "Display flap statistics of routes\n") {
	if(strncmp(argv[0], "m", 1) == 0) {
		return bgp_show(vty, NULL, AFI_IP, SAFI_MULTICAST, bgp_show_type_flap_statistics, NULL);
	}

	return bgp_show(vty, NULL, AFI_IP, SAFI_MULTICAST, bgp_show_type_flap_statistics, NULL);
}

DEFUN(show_ip_bgp_ipv4_dampening_dampd_paths, show_ip_bgp_ipv4_dampening_dampd_paths_cmd, "show ip bgp ipv4 (unicast|multicast) dampening dampened-paths",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Display detailed information about dampening\n"
	...
	if(strncmp(argv[0], "m", 1) == 0) {
		return bgp_show(vty, NULL, AFI_IP, SAFI_MULTICAST, bgp_show_type_dampend_paths, NULL);
	}

	return bgp_show(vty, NULL, AFI_IP, SAFI_MULTICAST, bgp_show_type_dampend_paths, NULL);
}

static int bgp_show_route_map(struct vty *vty, const char *rmap_str, afi_t afi, safi_t safi, enum bgp_show_type type) {
	struct route_map *rmap;

	rmap = route_map_lookup_by_name(rmap_str);
	if(!rmap) {
		vty_out(vty, "%% %s is not a valid route-map name%s", rmap_str, VTY_NEWLINE);
		return CMD_WARNING;
	}
...
DEFUN(show_ip_bgp_route_map, show_ip_bgp_route_map_cmd, "show ip bgp route-map WORD",
      SHOW_STR IP_STR BGP_STR "Display routes matching the route-map\n"
			      "A route-map to match on\n") {
	return bgp_show_route_map(vty, argv[0], AFI_IP, SAFI_UNICAST, bgp_show_type_route_map);
}

DEFUN(show_ip_bgp_flap_route_map, show_ip_bgp_flap_route_map_cmd, "show ip bgp flap-statistics route-map WORD",
      SHOW_STR IP_STR BGP_STR "Display flap statistics of routes\n"
			      "Display routes matching the route-map\n"
			      "A route-map to match on\n") {
	return bgp_show_route_map(vty, argv[0], AFI_IP, SAFI_UNICAST, bgp_show_type_flap_route_map);
}

ALIAS(show_ip_bgp_flap_route_map, show_ip_bgp_damp_flap_route_map_cmd, "show ip bgp dampening flap-statistics route-map WORD",
      SHOW_STR IP_STR BGP_STR "Display detailed information about dampening\n"
			      "Display flap statistics of routes\n"
			      "Display routes matching the route-map\n"
			      "A route-map to match on\n")

DEFUN(show_ip_bgp_ipv4_route_map, show_ip_bgp_ipv4_route_map_cmd, "show ip bgp ipv4 (unicast|multicast) route-map WORD",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Display routes matching the route-map\n"
			      "A route-map to match on\n") {
	if(strncmp(argv[0], "m", 1) == 0) {
		return bgp_show_route_map(vty, argv[1], AFI_IP, SAFI_MULTICAST, bgp_show_type_route_map);
	}

	return bgp_show_route_map(vty, argv[1], AFI_IP, SAFI_UNICAST, bgp_show_type_route_map);
}

DEFUN(show_bgp_route_map, show_bgp_route_map_cmd, "show bgp route-map WORD",
      SHOW_STR BGP_STR "Display routes matching the route-map\n"
		       "A route-map to match on\n") {
...
DEFUN(show_ip_bgp_cidr_only, show_ip_bgp_cidr_only_cmd, "show ip bgp cidr-only", SHOW_STR IP_STR BGP_STR "Display only routes with non-natural netmasks\n") {
	return bgp_show(vty, NULL, AFI_IP, SAFI_UNICAST, bgp_show_type_cidr_only, NULL);
}

DEFUN(show_ip_bgp_flap_cidr_only, show_ip_bgp_flap_cidr_only_cmd, "show ip bgp flap-statistics cidr-only",
      SHOW_STR IP_STR BGP_STR "Display flap statistics of routes\n"
			      "Display only routes with non-natural netmasks\n") {
	return bgp_show(vty, NULL, AFI_IP, SAFI_UNICAST, bgp_show_type_flap_cidr_only, NULL);
}

ALIAS(show_ip_bgp_flap_cidr_only, show_ip_bgp_damp_flap_cidr_only_cmd, "show ip bgp dampening flap-statistics cidr-only",
      SHOW_STR IP_STR BGP_STR "Display detailed information about dampening\n"
			      "Display flap statistics of routes\n"
			      "Display only routes with non-natural netmasks\n")

DEFUN(show_ip_bgp_ipv4_cidr_only, show_ip_bgp_ipv4_cidr_only_cmd, "show ip bgp ipv4 (unicast|multicast) cidr-only",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Display only routes with non-natural netmasks\n") {
	if(strncmp(argv[0], "m", 1) == 0) {
		return bgp_show(vty, NULL, AFI_IP, SAFI_MULTICAST, bgp_show_type_cidr_only, NULL);
	}

	return bgp_show(vty, NULL, AFI_IP, SAFI_UNICAST, bgp_show_type_cidr_only, NULL);
}

DEFUN(show_ip_bgp_community_all, show_ip_bgp_community_all_cmd, "show ip bgp community", SHOW_STR IP_STR BGP_STR "Display routes matching the communities\n") {
	return bgp_show(vty, NULL, AFI_IP, SAFI_UNICAST, bgp_show_type_community_all, NULL);
}

DEFUN(show_ip_bgp_ipv4_community_all, show_ip_bgp_ipv4_community_all_cmd, "show ip bgp ipv4 (unicast|multicast) community",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Display routes matching the communities\n") {
	if(strncmp(argv[0], "m", 1) == 0) {
		return bgp_show(vty, NULL, AFI_IP, SAFI_MULTICAST, bgp_show_type_community_all, NULL);
	}
...
DEFUN(show_bgp_community_all, show_bgp_community_all_cmd, "show bgp community", SHOW_STR BGP_STR "Display routes matching the communities\n") {
	return bgp_show(vty, NULL, AFI_IP6, SAFI_UNICAST, bgp_show_type_community_all, NULL);
}

ALIAS(show_bgp_community_all, show_bgp_ipv6_community_all_cmd, "show bgp ipv6 community",
      SHOW_STR BGP_STR "Address family\n"
		       "Display routes matching the communities\n")

/* old command */
DEFUN(show_ipv6_bgp_community_all, show_ipv6_bgp_community_all_cmd, "show ipv6 bgp community", SHOW_STR IPV6_STR BGP_STR "Display routes matching the communities\n") {
	return bgp_show(vty, NULL, AFI_IP6, SAFI_UNICAST, bgp_show_type_community_all, NULL);
}

/* old command */
DEFUN(show_ipv6_mbgp_community_all, show_ipv6_mbgp_community_all_cmd, "show ipv6 mbgp community", SHOW_STR IPV6_STR MBGP_STR "Display routes matching the communities\n") {
	return bgp_show(vty, NULL, AFI_IP6, SAFI_MULTICAST, bgp_show_type_community_all, NULL);
}

/* large-community */
DEFUN(show_ip_bgp_lcommunity_all, show_ip_bgp_lcommunity_all_cmd, "show ip bgp large-community", SHOW_STR IP_STR BGP_STR "Display routes matching the large-communities\n") {
	return bgp_show(vty, NULL, AFI_IP, SAFI_UNICAST, bgp_show_type_lcommunity_all, NULL);
}

DEFUN(show_ip_bgp_ipv4_lcommunity_all, show_ip_bgp_ipv4_lcommunity_all_cmd, "show ip bgp ipv4 (unicast|multicast) large-community",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Display routes matching the large-communities\n") {
	if(strncmp(argv[0], "m", 1) == 0) {
		return bgp_show(vty, NULL, AFI_IP, SAFI_MULTICAST, bgp_show_type_lcommunity_all, NULL);
	}

	return bgp_show(vty, NULL, AFI_IP, SAFI_UNICAST, bgp_show_type_lcommunity_all, NULL);
}

DEFUN(show_bgp_lcommunity_all, show_bgp_lcommunity_all_cmd, "show bgp large-community", SHOW_STR BGP_STR "Display routes matching the large-communities\n") {
	return bgp_show(vty, NULL, AFI_IP6, SAFI_UNICAST, bgp_show_type_lcommunity_all, NULL);
}

ALIAS(show_bgp_lcommunity_all, show_bgp_ipv6_lcommunity_all_cmd, "show bgp ipv6 large-community",
      SHOW_STR BGP_STR "Address family\n"
		       "Display routes matching the large-communities\n")

/* old command */
DEFUN(show_ipv6_bgp_lcommunity_all, show_ipv6_bgp_lcommunity_all_cmd, "show ipv6 bgp large-community", SHOW_STR IPV6_STR BGP_STR "Display routes matching the large-communities\n") {
	return bgp_show(vty, NULL, AFI_IP6, SAFI_UNICAST, bgp_show_type_lcommunity_all, NULL);
}

/* old command */
DEFUN(show_ipv6_mbgp_lcommunity_all, show_ipv6_mbgp_lcommunity_all_cmd, "show ipv6 mbgp large-community", SHOW_STR IPV6_STR MBGP_STR "Display routes matching the large-communities\n") {
	return bgp_show(vty, NULL, AFI_IP6, SAFI_MULTICAST, bgp_show_type_lcommunity_all, NULL);
}

DEFUN(show_bgp_ipv4_route_map, show_bgp_ipv4_route_map_cmd, "show bgp ipv4 route-map WORD",
      SHOW_STR BGP_STR IP_STR "Display routes matching the route-map\n"
			      "A route-map to match on\n") {
	return bgp_show_route_map(vty, argv[0], AFI_IP, SAFI_UNICAST, bgp_show_type_route_map);
}

DEFUN(show_bgp_ipv4_safi_flap_route_map, show_bgp_ipv4_safi_flap_route_map_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) flap-statistics route-map WORD",
      SHOW_STR BGP_STR IP_STR "Address Family Modifier\n"
			      "Address Family Modifier\n"
			      "Address Family Modifier\n"
			      "Address Family Modifier\n"
	...
	return bgp_show_route_map(vty, argv[1], AFI_IP, safi, bgp_show_type_flap_route_map);
}

ALIAS(show_bgp_ipv4_safi_flap_route_map, show_bgp_ipv4_safi_damp_flap_route_map_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) dampening flap-statistics route-map WORD",
      SHOW_STR BGP_STR IP_STR "Address Family Modifier\n"
			      "Address Family Modifier\n"
			      "Address Family Modifier\n"
			      "Address Family Modifier\n"
...
DEFUN(show_bgp_ipv6_safi_flap_route_map, show_bgp_ipv6_safi_flap_route_map_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) flap-statistics route-map WORD",
      SHOW_STR BGP_STR IPV6_STR "Address Family Modifier\n"
				"Address Family Modifier\n"
				"Address Family Modifier\n"
				"Address Family Modifier\n"
	...
	return bgp_show_route_map(vty, argv[1], AFI_IP6, safi, bgp_show_type_flap_route_map);
}
ALIAS(show_bgp_ipv6_safi_flap_route_map, show_bgp_ipv6_safi_damp_flap_route_map_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) dampening flap-statistics route-map WORD",
      SHOW_STR BGP_STR IPV6_STR "Address Family Modifier\n"
				"Address Family Modifier\n"
				"Address Family Modifier\n"
				"Address Family Modifier\n"
...
DEFUN(show_bgp_ipv4_safi_route_map, show_bgp_ipv4_safi_route_map_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) route-map WORD",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
	...
	return bgp_show_route_map(vty, argv[1], AFI_IP, safi, bgp_show_type_route_map);
}

DEFUN(show_bgp_ipv6_safi_route_map, show_bgp_ipv6_safi_route_map_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) route-map WORD",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
	...
	return bgp_show_route_map(vty, argv[1], AFI_IP6, safi, bgp_show_type_route_map);
}

DEFUN(show_bgp_ipv6_route_map, show_bgp_ipv6_route_map_cmd, "show bgp ipv6 route-map WORD",
      SHOW_STR BGP_STR "Address family\n"
		       "Display routes matching the route-map\n"
		       "A route-map to match on\n") {
	return bgp_show_route_map(vty, argv[0], AFI_IP6, SAFI_UNICAST, bgp_show_type_route_map);
...
DEFUN(show_bgp_ipv4_cidr_only, show_bgp_ipv4_cidr_only_cmd, "show bgp ipv4 cidr-only", SHOW_STR BGP_STR IP_STR "Display only routes with non-natural netmasks\n") {
	return bgp_show(vty, NULL, AFI_IP, SAFI_UNICAST, bgp_show_type_cidr_only, NULL);
}

DEFUN(show_bgp_ipv4_safi_flap_cidr_only, show_bgp_ipv4_safi_flap_cidr_only_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) flap-statistics cidr-only",
      SHOW_STR BGP_STR "Address Family\n"
		       "Address Family Modifier\n"
		       "Address Family Modifier\n"
		       "Address Family Modifier\n"
...
ALIAS(show_bgp_ipv4_safi_flap_cidr_only, show_bgp_ipv4_safi_damp_flap_cidr_only_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) dampening flap-statistics cidr-only",
      SHOW_STR BGP_STR "Address Family\n"
		       "Address Family Modifier\n"
		       "Address Family Modifier\n"
		       "Address Family Modifier\n"
...
DEFUN(show_bgp_ipv4_safi_cidr_only, show_bgp_ipv4_safi_cidr_only_cmd, "show bgp ipv4 (unicast|multicast) cidr-only",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Display only routes with non-natural netmasks\n") {
...
/* new046 */
DEFUN(show_bgp_afi_safi_community_all, show_bgp_afi_safi_community_all_cmd, "show bgp (ipv4|ipv6) (encap|multicast|unicast|vpn) community",
      SHOW_STR BGP_STR "Address family\n"
		       "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
...
DEFUN(show_bgp_afi_community_all, show_bgp_afi_community_all_cmd, "show bgp (ipv4|ipv6) community",
      SHOW_STR BGP_STR "Address family\n"
		       "Address family\n"
		       "Display routes matching the communities\n") {
	afi_t afi;
...
static int bgp_show_community(struct vty *vty, const char *view_name, int argc, const char **argv, int exact, afi_t afi, safi_t safi) {
	...
	rv = bgp_show(vty, bgp, afi, safi, (exact ? bgp_show_type_community_exact : bgp_show_type_community), com);
	community_free(com);
	return rv;
}

DEFUN(show_ip_bgp_community, show_ip_bgp_community_cmd, "show ip bgp community (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR IP_STR BGP_STR "Display routes matching the communities\n"
			      "community number\n"
...
ALIAS(show_ip_bgp_community, show_ip_bgp_community2_cmd, "show ip bgp community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR IP_STR BGP_STR "Display routes matching the communities\n"
			      "community number\n"
			      "Do not send outside local AS (well-known community)\n"
			      "Do not advertise to any peer (well-known community)\n"
...
ALIAS(show_ip_bgp_community, show_ip_bgp_community3_cmd, "show ip bgp community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR IP_STR BGP_STR "Display routes matching the communities\n"
			      "community number\n"
			      "Do not send outside local AS (well-known community)\n"
			      "Do not advertise to any peer (well-known community)\n"
...
DEFUN(show_ip_bgp_ipv4_community, show_ip_bgp_ipv4_community_cmd, "show ip bgp ipv4 (unicast|multicast) community (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Display routes matching the communities\n"
...
ALIAS(show_ip_bgp_ipv4_community, show_ip_bgp_ipv4_community2_cmd, "show ip bgp ipv4 (unicast|multicast) community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Display routes matching the communities\n"
...
ALIAS(show_ip_bgp_ipv4_community, show_ip_bgp_ipv4_community3_cmd, "show ip bgp ipv4 (unicast|multicast) community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Display routes matching the communities\n"
...
DEFUN(show_ip_bgp_community_exact, show_ip_bgp_community_exact_cmd, "show ip bgp community (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR IP_STR BGP_STR "Display routes matching the communities\n"
			      "community number\n"
			      "Do not send outside local AS (well-known community)\n"
			      "Do not advertise to any peer (well-known community)\n"
...
ALIAS(show_ip_bgp_community_exact, show_ip_bgp_community2_exact_cmd, "show ip bgp community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR IP_STR BGP_STR "Display routes matching the communities\n"
			      "community number\n"
			      "Do not send outside local AS (well-known community)\n"
			      "Do not advertise to any peer (well-known community)\n"
...
ALIAS(show_ip_bgp_community_exact, show_ip_bgp_community3_exact_cmd, "show ip bgp community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR IP_STR BGP_STR "Display routes matching the communities\n"
			      "community number\n"
			      "Do not send outside local AS (well-known community)\n"
			      "Do not advertise to any peer (well-known community)\n"
...
ALIAS(show_ip_bgp_community_exact, show_ip_bgp_community4_exact_cmd,
...
DEFUN(show_ip_bgp_ipv4_community_exact, show_ip_bgp_ipv4_community_exact_cmd, "show ip bgp ipv4 (unicast|multicast) community (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Display routes matching the communities\n"
...
ALIAS(show_ip_bgp_ipv4_community_exact, show_ip_bgp_ipv4_community2_exact_cmd, "show ip bgp ipv4 (unicast|multicast) community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Display routes matching the communities\n"
...
ALIAS(show_ip_bgp_ipv4_community_exact, show_ip_bgp_ipv4_community3_exact_cmd,
...
ALIAS(show_ip_bgp_ipv4_community_exact, show_ip_bgp_ipv4_community4_exact_cmd,
...
DEFUN(show_bgp_community, show_bgp_community_cmd, "show bgp community (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR BGP_STR "Display routes matching the communities\n"
		       "community number\n"
		       "Do not send outside local AS (well-known community)\n"
		       "Do not advertise to any peer (well-known community)\n"
		       "Do not export to next AS (well-known community)\n") {
...
ALIAS(show_bgp_community, show_bgp_ipv6_community_cmd, "show bgp ipv6 community (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR BGP_STR "Address family\n"
		       "Display routes matching the communities\n"
		       "community number\n"
		       "Do not send outside local AS (well-known community)\n"
...
ALIAS(show_bgp_community, show_bgp_community2_cmd, "show bgp community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR BGP_STR "Display routes matching the communities\n"
		       "community number\n"
		       "Do not send outside local AS (well-known community)\n"
		       "Do not advertise to any peer (well-known community)\n"
...
ALIAS(show_bgp_community, show_bgp_ipv6_community2_cmd, "show bgp ipv6 community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR BGP_STR "Address family\n"
		       "Display routes matching the communities\n"
		       "community number\n"
		       "Do not send outside local AS (well-known community)\n"
...
ALIAS(show_bgp_community, show_bgp_community3_cmd, "show bgp community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR BGP_STR "Display routes matching the communities\n"
		       "community number\n"
		       "Do not send outside local AS (well-known community)\n"
		       "Do not advertise to any peer (well-known community)\n"
...
ALIAS(show_bgp_community, show_bgp_ipv6_community3_cmd, "show bgp ipv6 community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR BGP_STR "Address family\n"
		       "Display routes matching the communities\n"
		       "community number\n"
		       "Do not send outside local AS (well-known community)\n"
...
ALIAS(show_bgp_community, show_bgp_community4_cmd, "show bgp community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR BGP_STR "Display routes matching the communities\n"
		       "community number\n"
		       "Do not send outside local AS (well-known community)\n"
		       "Do not advertise to any peer (well-known community)\n"
...
ALIAS(show_bgp_community, show_bgp_ipv6_community4_cmd,
...
/* old command */
DEFUN(show_ipv6_bgp_community, show_ipv6_bgp_community_cmd, "show ipv6 bgp community (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR IPV6_STR BGP_STR "Display routes matching the communities\n"
				"community number\n"
				"Do not send outside local AS (well-known community)\n"
				"Do not advertise to any peer (well-known community)\n"
				"Do not export to next AS (well-known community)\n") {
...
/* old command */
ALIAS(show_ipv6_bgp_community, show_ipv6_bgp_community2_cmd, "show ipv6 bgp community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR IPV6_STR BGP_STR "Display routes matching the communities\n"
				"community number\n"
				"Do not send outside local AS (well-known community)\n"
				"Do not advertise to any peer (well-known community)\n"
...
/* old command */
ALIAS(show_ipv6_bgp_community, show_ipv6_bgp_community3_cmd, "show ipv6 bgp community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR IPV6_STR BGP_STR "Display routes matching the communities\n"
				"community number\n"
				"Do not send outside local AS (well-known community)\n"
				"Do not advertise to any peer (well-known community)\n"
...
/* old command */
ALIAS(show_ipv6_bgp_community, show_ipv6_bgp_community4_cmd,
...
DEFUN(show_bgp_community_exact, show_bgp_community_exact_cmd, "show bgp community (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR BGP_STR "Display routes matching the communities\n"
		       "community number\n"
		       "Do not send outside local AS (well-known community)\n"
		       "Do not advertise to any peer (well-known community)\n"
...
ALIAS(show_bgp_community_exact, show_bgp_ipv6_community_exact_cmd, "show bgp ipv6 community (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR BGP_STR "Address family\n"
		       "Display routes matching the communities\n"
		       "community number\n"
		       "Do not send outside local AS (well-known community)\n"
...
ALIAS(show_bgp_community_exact, show_bgp_community2_exact_cmd, "show bgp community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR BGP_STR "Display routes matching the communities\n"
		       "community number\n"
		       "Do not send outside local AS (well-known community)\n"
		       "Do not advertise to any peer (well-known community)\n"
...
ALIAS(show_bgp_community_exact, show_bgp_ipv6_community2_exact_cmd, "show bgp ipv6 community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR BGP_STR "Address family\n"
		       "Display routes matching the communities\n"
		       "community number\n"
		       "Do not send outside local AS (well-known community)\n"
...
ALIAS(show_bgp_community_exact, show_bgp_community3_exact_cmd, "show bgp community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR BGP_STR "Display routes matching the communities\n"
		       "community number\n"
		       "Do not send outside local AS (well-known community)\n"
		       "Do not advertise to any peer (well-known community)\n"
...
ALIAS(show_bgp_community_exact, show_bgp_ipv6_community3_exact_cmd, "show bgp ipv6 community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR BGP_STR "Address family\n"
		       "Display routes matching the communities\n"
		       "community number\n"
		       "Do not send outside local AS (well-known community)\n"
...
ALIAS(show_bgp_community_exact, show_bgp_community4_exact_cmd,
...
/* old command */
DEFUN(show_ipv6_bgp_community_exact, show_ipv6_bgp_community_exact_cmd, "show ipv6 bgp community (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR IPV6_STR BGP_STR "Display routes matching the communities\n"
				"community number\n"
				"Do not send outside local AS (well-known community)\n"
				"Do not advertise to any peer (well-known community)\n"
...
/* old command */
ALIAS(show_ipv6_bgp_community_exact, show_ipv6_bgp_community2_exact_cmd, "show ipv6 bgp community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR IPV6_STR BGP_STR "Display routes matching the communities\n"
				"community number\n"
				"Do not send outside local AS (well-known community)\n"
				"Do not advertise to any peer (well-known community)\n"
...
/* old command */
ALIAS(show_ipv6_bgp_community_exact, show_ipv6_bgp_community3_exact_cmd, "show ipv6 bgp community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR IPV6_STR BGP_STR "Display routes matching the communities\n"
				"community number\n"
				"Do not send outside local AS (well-known community)\n"
				"Do not advertise to any peer (well-known community)\n"
...
/* old command */
ALIAS(show_ipv6_bgp_community_exact, show_ipv6_bgp_community4_exact_cmd,
...
/* old command */
DEFUN(show_ipv6_mbgp_community, show_ipv6_mbgp_community_cmd, "show ipv6 mbgp community (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR IPV6_STR MBGP_STR "Display routes matching the communities\n"
				 "community number\n"
				 "Do not send outside local AS (well-known community)\n"
				 "Do not advertise to any peer (well-known community)\n"
				 "Do not export to next AS (well-known community)\n") {
...
/* old command */
ALIAS(show_ipv6_mbgp_community, show_ipv6_mbgp_community2_cmd, "show ipv6 mbgp community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR IPV6_STR MBGP_STR "Display routes matching the communities\n"
				 "community number\n"
				 "Do not send outside local AS (well-known community)\n"
				 "Do not advertise to any peer (well-known community)\n"
...
/* old command */
ALIAS(show_ipv6_mbgp_community, show_ipv6_mbgp_community3_cmd, "show ipv6 mbgp community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR IPV6_STR MBGP_STR "Display routes matching the communities\n"
				 "community number\n"
				 "Do not send outside local AS (well-known community)\n"
				 "Do not advertise to any peer (well-known community)\n"
...
/* old command */
ALIAS(show_ipv6_mbgp_community, show_ipv6_mbgp_community4_cmd,
...
/* old command */
DEFUN(show_ipv6_mbgp_community_exact, show_ipv6_mbgp_community_exact_cmd, "show ipv6 mbgp community (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR IPV6_STR MBGP_STR "Display routes matching the communities\n"
				 "community number\n"
				 "Do not send outside local AS (well-known community)\n"
				 "Do not advertise to any peer (well-known community)\n"
...
/* old command */
ALIAS(show_ipv6_mbgp_community_exact, show_ipv6_mbgp_community2_exact_cmd, "show ipv6 mbgp community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR IPV6_STR MBGP_STR "Display routes matching the communities\n"
				 "community number\n"
				 "Do not send outside local AS (well-known community)\n"
				 "Do not advertise to any peer (well-known community)\n"
...
/* old command */
ALIAS(show_ipv6_mbgp_community_exact, show_ipv6_mbgp_community3_exact_cmd, "show ipv6 mbgp community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR IPV6_STR MBGP_STR "Display routes matching the communities\n"
				 "community number\n"
				 "Do not send outside local AS (well-known community)\n"
				 "Do not advertise to any peer (well-known community)\n"
...
/* old command */
ALIAS(show_ipv6_mbgp_community_exact, show_ipv6_mbgp_community4_exact_cmd,
...
DEFUN(show_bgp_ipv4_community, show_bgp_ipv4_community_cmd, "show bgp ipv4 community (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR BGP_STR IP_STR "Display routes matching the communities\n"
			      "community number\n"
			      "Do not send outside local AS (well-known community)\n"
			      "Do not advertise to any peer (well-known community)\n"
			      "Do not export to next AS (well-known community)\n") {
...
ALIAS(show_bgp_ipv4_community, show_bgp_ipv4_community2_cmd, "show bgp ipv4 community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR BGP_STR IP_STR "Display routes matching the communities\n"
			      "community number\n"
			      "Do not send outside local AS (well-known community)\n"
			      "Do not advertise to any peer (well-known community)\n"
...
ALIAS(show_bgp_ipv4_community, show_bgp_ipv4_community3_cmd, "show bgp ipv4 community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR BGP_STR IP_STR "Display routes matching the communities\n"
			      "community number\n"
			      "Do not send outside local AS (well-known community)\n"
			      "Do not advertise to any peer (well-known community)\n"
...
DEFUN(show_bgp_ipv4_safi_community, show_bgp_ipv4_safi_community_cmd, "show bgp ipv4 (unicast|multicast) community (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Display routes matching the communities\n"
...
ALIAS(show_bgp_ipv4_safi_community, show_bgp_ipv4_safi_community2_cmd, "show bgp ipv4 (unicast|multicast) community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Display routes matching the communities\n"
...
ALIAS(show_bgp_ipv4_safi_community, show_bgp_ipv4_safi_community3_cmd, "show bgp ipv4 (unicast|multicast) community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Display routes matching the communities\n"
...
ALIAS(show_bgp_ipv4_safi_community, show_bgp_ipv4_safi_community4_cmd,
...
DEFUN(show_bgp_view_afi_safi_community_all, show_bgp_view_afi_safi_community_all_cmd, "show bgp view WORD (ipv4|ipv6) (unicast|multicast) community",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "Address family\n"
...
DEFUN(show_bgp_view_afi_safi_community, show_bgp_view_afi_safi_community_cmd, "show bgp view WORD (ipv4|ipv6) (unicast|multicast) community (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "Address family\n"
...
ALIAS(show_bgp_view_afi_safi_community, show_bgp_view_afi_safi_community2_cmd, "show bgp view WORD (ipv4|ipv6) (unicast|multicast) community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "Address family\n"
...
ALIAS(show_bgp_view_afi_safi_community, show_bgp_view_afi_safi_community3_cmd,
...
ALIAS(show_bgp_view_afi_safi_community, show_bgp_view_afi_safi_community4_cmd,
...
DEFUN(show_bgp_ipv4_community_exact, show_bgp_ipv4_community_exact_cmd, "show bgp ipv4 community (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR BGP_STR IP_STR "Display routes matching the communities\n"
			      "community number\n"
			      "Do not send outside local AS (well-known community)\n"
			      "Do not advertise to any peer (well-known community)\n"
...
ALIAS(show_bgp_ipv4_community_exact, show_bgp_ipv4_community2_exact_cmd, "show bgp ipv4 community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR BGP_STR IP_STR "Display routes matching the communities\n"
			      "community number\n"
			      "Do not send outside local AS (well-known community)\n"
			      "Do not advertise to any peer (well-known community)\n"
...
ALIAS(show_bgp_ipv4_community_exact, show_bgp_ipv4_community3_exact_cmd, "show bgp ipv4 community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR BGP_STR IP_STR "Display routes matching the communities\n"
			      "community number\n"
			      "Do not send outside local AS (well-known community)\n"
			      "Do not advertise to any peer (well-known community)\n"
...
ALIAS(show_bgp_ipv4_community_exact, show_bgp_ipv4_community4_exact_cmd,
...
DEFUN(show_bgp_ipv4_safi_community4_exact, show_bgp_ipv4_safi_community_exact_cmd, "show bgp ipv4 (unicast|multicast) community (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Display routes matching the communities\n"
...
ALIAS(show_bgp_ipv4_safi_community4_exact, show_bgp_ipv4_safi_community2_exact_cmd, "show bgp ipv4 (unicast|multicast) community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Display routes matching the communities\n"
...
ALIAS(show_bgp_ipv4_safi_community4_exact, show_bgp_ipv4_safi_community3_exact_cmd,
...
ALIAS(show_bgp_ipv4_safi_community4_exact, show_bgp_ipv4_safi_community4_exact_cmd,
...
DEFUN(show_bgp_ipv6_safi_community, show_bgp_ipv6_safi_community_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) community (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address family modifier\n"
		       "Address family modifier\n"
		       "Address family modifier\n"
...
ALIAS(show_bgp_ipv6_safi_community, show_bgp_ipv6_safi_community2_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address family modifier\n"
		       "Address family modifier\n"
		       "Address family modifier\n"
...
ALIAS(show_bgp_ipv6_safi_community, show_bgp_ipv6_safi_community3_cmd,
...
ALIAS(show_bgp_ipv6_safi_community, show_bgp_ipv6_safi_community4_cmd,
...
DEFUN(show_bgp_ipv6_safi_community_exact, show_bgp_ipv6_safi_community_exact_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) community (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR BGP_STR "Address family\n"
		       "Address family modifier\n"
		       "Address family modifier\n"
		       "Address family modifier\n"
...
ALIAS(show_bgp_community_exact, show_bgp_ipv6_safi_community2_exact_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) community (AA:NN|local-AS|no-advertise|no-export) (AA:NN|local-AS|no-advertise|no-export) exact-match",
      SHOW_STR BGP_STR "Address family\n"
		       "Address family modifier\n"
		       "Address family modifier\n"
		       "Address family modifier\n"
...
ALIAS(show_bgp_community_exact, show_bgp_ipv6_safi_community3_exact_cmd,
...
ALIAS(show_bgp_community_exact, show_bgp_ipv6_safi_community4_exact_cmd,
...
static int bgp_show_community_list(struct vty *vty, const char *com, int exact, afi_t afi, safi_t safi) {
	...
	return bgp_show(vty, NULL, afi, safi, (exact ? bgp_show_type_community_list_exact : bgp_show_type_community_list), list);
}

DEFUN(show_ip_bgp_community_list, show_ip_bgp_community_list_cmd, "show ip bgp community-list (<1-500>|WORD)",
...
DEFUN(show_ip_bgp_ipv4_community_list, show_ip_bgp_ipv4_community_list_cmd, "show ip bgp ipv4 (unicast|multicast) community-list (<1-500>|WORD)",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
...
DEFUN(show_ip_bgp_community_list_exact, show_ip_bgp_community_list_exact_cmd, "show ip bgp community-list (<1-500>|WORD) exact-match",
      SHOW_STR IP_STR BGP_STR "Display routes matching the community-list\n"
			      "community-list number\n"
			      "community-list name\n"
			      "Exact match of the communities\n") {
...
DEFUN(show_ip_bgp_ipv4_community_list_exact, show_ip_bgp_ipv4_community_list_exact_cmd, "show ip bgp ipv4 (unicast|multicast) community-list (<1-500>|WORD) exact-match",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Display routes matching the community-list\n"
...
DEFUN(show_bgp_community_list, show_bgp_community_list_cmd, "show bgp community-list (<1-500>|WORD)",
      SHOW_STR BGP_STR "Display routes matching the community-list\n"
		       "community-list number\n"
		       "community-list name\n") {
	return bgp_show_community_list(vty, argv[0], 0, AFI_IP6, SAFI_UNICAST);
}

ALIAS(show_bgp_community_list, show_bgp_ipv6_community_list_cmd, "show bgp ipv6 community-list (<1-500>|WORD)",
      SHOW_STR BGP_STR "Address family\n"
		       "Display routes matching the community-list\n"
		       "community-list number\n"
		       "community-list name\n")
...
/* old command */
DEFUN(show_ipv6_bgp_community_list, show_ipv6_bgp_community_list_cmd, "show ipv6 bgp community-list WORD",
...
/* old command */
DEFUN(show_ipv6_mbgp_community_list, show_ipv6_mbgp_community_list_cmd, "show ipv6 mbgp community-list WORD",
...
DEFUN(show_bgp_community_list_exact, show_bgp_community_list_exact_cmd, "show bgp community-list (<1-500>|WORD) exact-match",
      SHOW_STR BGP_STR "Display routes matching the community-list\n"
		       "community-list number\n"
		       "community-list name\n"
		       "Exact match of the communities\n") {
...
ALIAS(show_bgp_community_list_exact, show_bgp_ipv6_community_list_exact_cmd, "show bgp ipv6 community-list (<1-500>|WORD) exact-match",
      SHOW_STR BGP_STR "Address family\n"
		       "Display routes matching the community-list\n"
		       "community-list number\n"
		       "community-list name\n"
...
/* old command */
DEFUN(show_ipv6_bgp_community_list_exact, show_ipv6_bgp_community_list_exact_cmd, "show ipv6 bgp community-list WORD exact-match",
      SHOW_STR IPV6_STR BGP_STR "Display routes matching the community-list\n"
				"community-list name\n"
				"Exact match of the communities\n") {
	return bgp_show_community_list(vty, argv[0], 1, AFI_IP6, SAFI_UNICAST);
...
/* old command */
DEFUN(show_ipv6_mbgp_community_list_exact, show_ipv6_mbgp_community_list_exact_cmd, "show ipv6 mbgp community-list WORD exact-match",
      SHOW_STR IPV6_STR MBGP_STR "Display routes matching the community-list\n"
				 "community-list name\n"
				 "Exact match of the communities\n") {
...
DEFUN(show_bgp_ipv4_community_list, show_bgp_ipv4_community_list_cmd, "show bgp ipv4 community-list (<1-500>|WORD)",
      SHOW_STR BGP_STR IP_STR "Display routes matching the community-list\n"
			      "community-list number\n"
			      "community-list name\n") {
	return bgp_show_community_list(vty, argv[0], 0, AFI_IP, SAFI_UNICAST);
}

DEFUN(show_bgp_ipv4_safi_community_list, show_bgp_ipv4_safi_community_list_cmd, "show bgp ipv4 (unicast|multicast) community-list (<1-500>|WORD)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Display routes matching the community-list\n"
...
DEFUN(show_bgp_ipv4_community_list_exact, show_bgp_ipv4_community_list_exact_cmd, "show bgp ipv4 community-list (<1-500>|WORD) exact-match",
      SHOW_STR BGP_STR IP_STR "Display routes matching the community-list\n"
			      "community-list number\n"
			      "community-list name\n"
			      "Exact match of the communities\n") {
...
DEFUN(show_bgp_ipv4_safi_community_list_exact, show_bgp_ipv4_safi_community_list_exact_cmd, "show bgp ipv4 (unicast|multicast) community-list (<1-500>|WORD) exact-match",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Display routes matching the community-list\n"
...
DEFUN(show_bgp_ipv6_safi_community_list, show_bgp_ipv6_safi_community_list_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) community-list (<1-500>|WORD)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address family modifier\n"
		       "Address family modifier\n"
		       "Address family modifier\n"
...
DEFUN(show_bgp_ipv6_safi_community_list_exact, show_bgp_ipv6_safi_community_list_exact_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) community-list (<1-500>|WORD) exact-match",
      SHOW_STR BGP_STR "Address family\n"
		       "Address family modifier\n"
		       "Address family modifier\n"
		       "Address family modifier\n"
...
DEFUN(show_bgp_afi_lcommunity_all, show_bgp_afi_lcommunity_all_cmd, "show bgp (ipv4|ipv6) large-community",
      SHOW_STR BGP_STR "Address family\n"
		       "Address family\n"
		       "Display routes matching the large-communities\n") {
	afi_t afi;
...
static int bgp_show_lcommunity(struct vty *vty, const char *view_name, int argc, const char **argv, afi_t afi, safi_t safi) {
...
DEFUN(show_ip_bgp_lcommunity, show_ip_bgp_lcommunity_cmd, "show ip bgp large-community (AA:BB:CC)",
...
ALIAS(show_ip_bgp_lcommunity, show_ip_bgp_lcommunity2_cmd, "show ip bgp large-community (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR IP_STR BGP_STR "Display routes matching the large-communities\n"
			      "large-community number\n"
			      "large-community number\n")

ALIAS(show_ip_bgp_lcommunity, show_ip_bgp_lcommunity3_cmd, "show ip bgp large-community (AA:BB:CC) (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR IP_STR BGP_STR "Display routes matching the large-communities\n"
			      "largecommunity number\n"
			      "largecommunity number\n"
			      "largecommunity number\n")

ALIAS(show_ip_bgp_lcommunity, show_ip_bgp_lcommunity4_cmd, "show ip bgp large-community (AA:BB:CC) (AA:BB:CC) (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR IP_STR BGP_STR "Display routes matching the large-communities\n"
			      "large-community number\n"
			      "large-community number\n"
			      "large-community number\n"
...
DEFUN(show_ip_bgp_ipv4_lcommunity, show_ip_bgp_ipv4_lcommunity_cmd, "show ip bgp ipv4 (unicast|multicast) large-community (AA:BB:CC)",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Display routes matching the large-communities\n"
...
ALIAS(show_ip_bgp_ipv4_lcommunity, show_ip_bgp_ipv4_lcommunity2_cmd, "show ip bgp ipv4 (unicast|multicast) large-community (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Display routes matching the large-communities\n"
...
ALIAS(show_ip_bgp_ipv4_lcommunity, show_ip_bgp_ipv4_lcommunity3_cmd, "show ip bgp ipv4 (unicast|multicast) large-community (AA:BB:CC) (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Display routes matching the large-communities\n"
...
ALIAS(show_ip_bgp_ipv4_lcommunity, show_ip_bgp_ipv4_lcommunity4_cmd, "show ip bgp ipv4 (unicast|multicast) large-community (AA:BB:CC) (AA:BB:CC) (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Display routes matching the large-communities\n"
...
DEFUN(show_bgp_lcommunity, show_bgp_lcommunity_cmd, "show bgp large-community (AA:BB:CC)",
...
ALIAS(show_bgp_lcommunity, show_bgp_ipv6_lcommunity_cmd, "show bgp ipv6 large-community (AA:BB:CC)",
      SHOW_STR BGP_STR "Address family\n"
		       "Display routes matching the large-communities\n"
		       "large-community number\n")

ALIAS(show_bgp_lcommunity, show_bgp_lcommunity2_cmd, "show bgp large-community (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR BGP_STR "Display routes matching the large-communities\n"
		       "large-community number\n"
		       "large-community number\n")

ALIAS(show_bgp_lcommunity, show_bgp_ipv6_lcommunity2_cmd, "show bgp ipv6 large-community (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR BGP_STR "Address family\n"
		       "Display routes matching the large-communities\n"
		       "large-community number\n"
		       "large-community number\n")

ALIAS(show_bgp_lcommunity, show_bgp_lcommunity3_cmd, "show bgp large-community (AA:BB:CC) (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR BGP_STR "Display routes matching the large-communities\n"
		       "large-community number\n"
		       "large-community number\n"
		       "large-community number\n")

ALIAS(show_bgp_lcommunity, show_bgp_ipv6_lcommunity3_cmd, "show bgp ipv6 large-community (AA:BB:CC) (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR BGP_STR "Address family\n"
		       "Display routes matching the large-communities\n"
		       "large-community number\n"
		       "large-community number\n"
...
ALIAS(show_bgp_lcommunity, show_bgp_lcommunity4_cmd, "show bgp large-community (AA:BB:CC) (AA:BB:CC) (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR BGP_STR "Display routes matching the large-communities\n"
		       "large-community number\n"
		       "large-community number\n"
		       "large-community number\n"
...
ALIAS(show_bgp_lcommunity, show_bgp_ipv6_lcommunity4_cmd, "show bgp ipv6 large-community (AA:BB:CC) (AA:BB:CC) (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR BGP_STR "Address family\n"
		       "Display routes matching the large-communities\n"
		       "large-community number\n"
		       "large-community number\n"
...
/* old command */
DEFUN(show_ipv6_bgp_lcommunity, show_ipv6_bgp_lcommunity_cmd, "show ipv6 bgp large-community (AA:BB:CC)",
      SHOW_STR IPV6_STR BGP_STR "Display routes matching the large-communities\n"
				"large-community number\n") {
	return bgp_show_lcommunity(vty, NULL, argc, argv, AFI_IP6, SAFI_UNICAST);
}

/* old command */
ALIAS(show_ipv6_bgp_lcommunity, show_ipv6_bgp_lcommunity2_cmd, "show ipv6 bgp large-community (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR IPV6_STR BGP_STR "Display routes matching the large-communities\n"
				"large-community number\n"
				"large-community number\n")

/* old command */
ALIAS(show_ipv6_bgp_lcommunity, show_ipv6_bgp_lcommunity3_cmd, "show ipv6 bgp large-community (AA:BB:CC) (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR IPV6_STR BGP_STR "Display routes matching the large-communities\n"
				"large-community number\n"
				"large-community number\n"
				"large-community number\n")
...
/* old command */
ALIAS(show_ipv6_bgp_lcommunity, show_ipv6_bgp_lcommunity4_cmd, "show ipv6 bgp large-community (AA:BB:CC) (AA:BB:CC) (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR IPV6_STR BGP_STR "Display routes matching the large-communities\n"
				"large-community number\n"
				"large-community number\n"
				"large-community number\n"
...
/* old command */
DEFUN(show_ipv6_mbgp_lcommunity, show_ipv6_mbgp_lcommunity_cmd, "show ipv6 mbgp large-community (AA:BB:CC)",
      SHOW_STR IPV6_STR MBGP_STR "Display routes matching the large-communities\n"
				 "large-community number\n") {
	return bgp_show_lcommunity(vty, NULL, argc, argv, AFI_IP6, SAFI_MULTICAST);
}

/* old command */
ALIAS(show_ipv6_mbgp_lcommunity, show_ipv6_mbgp_lcommunity2_cmd, "show ipv6 mbgp large-community (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR IPV6_STR MBGP_STR "Display routes matching the large-communities\n"
				 "large-community number\n"
				 "large-community number\n")

/* old command */
ALIAS(show_ipv6_mbgp_lcommunity, show_ipv6_mbgp_lcommunity3_cmd, "show ipv6 mbgp large-community (AA:BB:CC) (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR IPV6_STR MBGP_STR "Display routes matching the large-communities\n"
				 "large-community number\n"
				 "large-community number\n"
				 "large-community number\n")
...
/* old command */
ALIAS(show_ipv6_mbgp_lcommunity, show_ipv6_mbgp_lcommunity4_cmd, "show ipv6 mbgp laarge-community (AA:BB:CC) (AA:BB:CC) (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR IPV6_STR MBGP_STR "Display routes matching the large-communities\n"
				 "large-community number\n"
				 "large-community number\n"
				 "large-community number\n"
...
DEFUN(show_bgp_ipv4_lcommunity, show_bgp_ipv4_lcommunity_cmd, "show bgp ipv4 large-community (AA:BB:CC)",
      SHOW_STR BGP_STR IP_STR "Display routes matching the large-communities\n"
			      "large-community number\n") {
	return bgp_show_lcommunity(vty, NULL, argc, argv, AFI_IP, SAFI_UNICAST);
}

ALIAS(show_bgp_ipv4_lcommunity, show_bgp_ipv4_lcommunity2_cmd, "show bgp ipv4 large-community (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR BGP_STR IP_STR "Display routes matching the large-communities\n"
			      "large-community number\n"
			      "large-community number\n")

ALIAS(show_bgp_ipv4_lcommunity, show_bgp_ipv4_lcommunity3_cmd, "show bgp ipv4 large-community (AA:BB:CC) (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR BGP_STR IP_STR "Display routes matching the large-communities\n"
			      "large-community number\n"
			      "large-community number\n"
			      "large-community number\n")

ALIAS(show_bgp_ipv4_lcommunity, show_bgp_ipv4_lcommunity4_cmd, "show bgp ipv4 large-community (AA:BB:CC) (AA:BB:CC) (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR BGP_STR IP_STR "Display routes matching the large-communities\n"
			      "large-community number\n"
			      "large-community number\n"
			      "large-community number\n"
...
DEFUN(show_bgp_ipv4_safi_lcommunity, show_bgp_ipv4_safi_lcommunity_cmd, "show bgp ipv4 (unicast|multicast) large-community (AA:BB:CC)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Display routes matching the large-communities\n"
...
ALIAS(show_bgp_ipv4_safi_lcommunity, show_bgp_ipv4_safi_lcommunity2_cmd, "show bgp ipv4 (unicast|multicast) large-community (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Display routes matching the large-communities\n"
...
ALIAS(show_bgp_ipv4_safi_lcommunity, show_bgp_ipv4_safi_lcommunity3_cmd, "show bgp ipv4 (unicast|multicast) large-community (AA:BB:CC) (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Display routes matching the large-communities\n"
...
ALIAS(show_bgp_ipv4_safi_lcommunity, show_bgp_ipv4_safi_lcommunity4_cmd, "show bgp ipv4 (unicast|multicast) large-community (AA:BB:CC) (AA:BB:CC) (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Display routes matching the large-communities\n"
...
DEFUN(show_bgp_view_afi_safi_lcommunity_all, show_bgp_view_afi_safi_lcommunity_all_cmd, "show bgp view WORD (ipv4|ipv6) (unicast|multicast) large-community",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "Address family\n"
...
DEFUN(show_bgp_view_afi_safi_lcommunity, show_bgp_view_afi_safi_lcommunity_cmd, "show bgp view WORD (ipv4|ipv6) (unicast|multicast) large-community (AA:BB:CC)",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "Address family\n"
...
ALIAS(show_bgp_view_afi_safi_lcommunity, show_bgp_view_afi_safi_lcommunity2_cmd, "show bgp view WORD (ipv4|ipv6) (unicast|multicast) large-community (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "Address family\n"
...
ALIAS(show_bgp_view_afi_safi_lcommunity, show_bgp_view_afi_safi_lcommunity3_cmd, "show bgp view WORD (ipv4|ipv6) (unicast|multicast) large-community (AA:BB:CC) (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "Address family\n"
...
ALIAS(show_bgp_view_afi_safi_lcommunity, show_bgp_view_afi_safi_lcommunity4_cmd, "show bgp view WORD (ipv4|ipv6) (unicast|multicast) large-community (AA:BB:CC) (AA:BB:CC) (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "Address family\n"
...
DEFUN(show_bgp_ipv6_safi_lcommunity, show_bgp_ipv6_safi_lcommunity_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) large-community AA:BB:CC",
      SHOW_STR BGP_STR "Address family\n"
		       "Address family modifier\n"
		       "Address family modifier\n"
		       "Address family modifier\n"
...
ALIAS(show_bgp_ipv6_safi_lcommunity, show_bgp_ipv6_safi_lcommunity2_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) large-community (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address family modifier\n"
		       "Address family modifier\n"
		       "Address family modifier\n"
...
ALIAS(show_bgp_ipv6_safi_lcommunity, show_bgp_ipv6_safi_lcommunity3_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) large-community (AA:BB:CC) (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address family modifier\n"
		       "Address family modifier\n"
		       "Address family modifier\n"
...
ALIAS(show_bgp_ipv6_safi_lcommunity, show_bgp_ipv6_safi_lcommunity4_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) large-community (AA:BB:CC) (AA:BB:CC) (AA:BB:CC) (AA:BB:CC)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address family modifier\n"
		       "Address family modifier\n"
		       "Address family modifier\n"
...
static int bgp_show_lcommunity_list(struct vty *vty, const char *lcom, afi_t afi, safi_t safi) {
	struct community_list *list;

	list = community_list_lookup(bgp_clist, lcom, LARGE_COMMUNITY_LIST_MASTER);
	if(list == NULL) {
		vty_out(vty, "%% %s is not a valid large-community-list name%s", lcom, VTY_NEWLINE);
		return CMD_WARNING;
	}
...
DEFUN(show_ip_bgp_lcommunity_list, show_ip_bgp_lcommunity_list_cmd, "show ip bgp large-community-list (<1-500>|WORD)",
      SHOW_STR IP_STR BGP_STR "Display routes matching the large-community-list\n"
			      "large-community-list number\n"
			      "large-community-list name\n") {
	return bgp_show_lcommunity_list(vty, argv[0], AFI_IP, SAFI_UNICAST);
}

DEFUN(show_ip_bgp_ipv4_lcommunity_list, show_ip_bgp_ipv4_lcommunity_list_cmd, "show ip bgp ipv4 (unicast|multicast) large-community-list (<1-500>|WORD)",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
...
DEFUN(show_bgp_lcommunity_list, show_bgp_lcommunity_list_cmd, "show bgp large-community-list (<1-500>|WORD)",
      SHOW_STR BGP_STR "Display routes matching the large-community-list\n"
		       "large-community-list number\n"
		       "large-community-list name\n") {
	return bgp_show_lcommunity_list(vty, argv[0], AFI_IP6, SAFI_UNICAST);
}

ALIAS(show_bgp_lcommunity_list, show_bgp_ipv6_lcommunity_list_cmd, "show bgp ipv6 large-community-list (<1-500>|WORD)",
      SHOW_STR BGP_STR "Address family\n"
		       "Display routes matching the large-community-list\n"
		       "large-community-list number\n"
		       "large-community-list name\n")
...
/* old command */
DEFUN(show_ipv6_bgp_lcommunity_list, show_ipv6_bgp_lcommunity_list_cmd, "show ipv6 bgp large-community-list WORD",
      SHOW_STR IPV6_STR BGP_STR "Display routes matching the large-community-list\n"
				"large-community-list name\n") {
	return bgp_show_lcommunity_list(vty, argv[0], AFI_IP6, SAFI_UNICAST);
}

/* old command */
DEFUN(show_ipv6_mbgp_lcommunity_list, show_ipv6_mbgp_lcommunity_list_cmd, "show ipv6 mbgp large-community-list WORD",
      SHOW_STR IPV6_STR MBGP_STR "Display routes matching the large-community-list\n"
				 "large-community-list name\n") {
	return bgp_show_lcommunity_list(vty, argv[0], AFI_IP6, SAFI_MULTICAST);
}

DEFUN(show_bgp_ipv4_lcommunity_list, show_bgp_ipv4_lcommunity_list_cmd, "show bgp ipv4 large-community-list (<1-500>|WORD)",
      SHOW_STR BGP_STR IP_STR "Display routes matching the large-community-list\n"
			      "large-community-list number\n"
			      "large-community-list name\n") {
	return bgp_show_lcommunity_list(vty, argv[0], AFI_IP, SAFI_UNICAST);
}

DEFUN(show_bgp_ipv4_safi_lcommunity_list, show_bgp_ipv4_safi_lcommunity_list_cmd, "show bgp ipv4 (unicast|multicast) large-community-list (<1-500>|WORD)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Display routes matching the large-community-list\n"
...
DEFUN(show_bgp_ipv6_safi_lcommunity_list, show_bgp_ipv6_safi_lcommunity_list_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) large-community-list (<1-500>|WORD)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address family modifier\n"
		       "Address family modifier\n"
		       "Address family modifier\n"
...
static int bgp_show_prefix_longer(struct vty *vty, const char *prefix, afi_t afi, safi_t safi, enum bgp_show_type type) {
	int ret;
	struct prefix *p;
...
DEFUN(show_ip_bgp_prefix_longer, show_ip_bgp_prefix_longer_cmd, "show ip bgp A.B.C.D/M longer-prefixes",
      SHOW_STR IP_STR BGP_STR "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n"
			      "Display route and more specific routes\n") {
	return bgp_show_prefix_longer(vty, argv[0], AFI_IP, SAFI_UNICAST, bgp_show_type_prefix_longer);
}

DEFUN(show_ip_bgp_flap_prefix_longer, show_ip_bgp_flap_prefix_longer_cmd, "show ip bgp flap-statistics A.B.C.D/M longer-prefixes",
      SHOW_STR IP_STR BGP_STR "Display flap statistics of routes\n"
			      "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n"
			      "Display route and more specific routes\n") {
	return bgp_show_prefix_longer(vty, argv[0], AFI_IP, SAFI_UNICAST, bgp_show_type_flap_prefix_longer);
}

ALIAS(show_ip_bgp_flap_prefix_longer, show_ip_bgp_damp_flap_prefix_longer_cmd, "show ip bgp dampening flap-statistics A.B.C.D/M longer-prefixes",
      SHOW_STR IP_STR BGP_STR "Display detailed information about dampening\n"
			      "Display flap statistics of routes\n"
			      "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n"
			      "Display route and more specific routes\n")

DEFUN(show_ip_bgp_ipv4_prefix_longer, show_ip_bgp_ipv4_prefix_longer_cmd, "show ip bgp ipv4 (unicast|multicast) A.B.C.D/M longer-prefixes",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n"
			      "Display route and more specific routes\n") {
	if(strncmp(argv[0], "m", 1) == 0) {
		return bgp_show_prefix_longer(vty, argv[1], AFI_IP, SAFI_MULTICAST, bgp_show_type_prefix_longer);
	}

	return bgp_show_prefix_longer(vty, argv[1], AFI_IP, SAFI_UNICAST, bgp_show_type_prefix_longer);
}

DEFUN(show_ip_bgp_flap_address, show_ip_bgp_flap_address_cmd, "show ip bgp flap-statistics A.B.C.D",
      SHOW_STR IP_STR BGP_STR "Display flap statistics of routes\n"
			      "Network in the BGP routing table to display\n") {
	return bgp_show_prefix_longer(vty, argv[0], AFI_IP, SAFI_UNICAST, bgp_show_type_flap_address);
}

ALIAS(show_ip_bgp_flap_address, show_ip_bgp_damp_flap_address_cmd, "show ip bgp dampening flap-statistics A.B.C.D",
      SHOW_STR IP_STR BGP_STR "Display detailed information about dampening\n"
			      "Display flap statistics of routes\n"
			      "Network in the BGP routing table to display\n")

DEFUN(show_ip_bgp_flap_prefix, show_ip_bgp_flap_prefix_cmd, "show ip bgp flap-statistics A.B.C.D/M",
      SHOW_STR IP_STR BGP_STR "Display flap statistics of routes\n"
			      "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n") {
	return bgp_show_prefix_longer(vty, argv[0], AFI_IP, SAFI_UNICAST, bgp_show_type_flap_prefix);
}

ALIAS(show_ip_bgp_flap_prefix, show_ip_bgp_damp_flap_prefix_cmd, "show ip bgp dampening flap-statistics A.B.C.D/M",
      SHOW_STR IP_STR BGP_STR "Display detailed information about dampening\n"
			      "Display flap statistics of routes\n"
			      "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n")

DEFUN(show_bgp_prefix_longer, show_bgp_prefix_longer_cmd, "show bgp X:X::X:X/M longer-prefixes",
      SHOW_STR BGP_STR "IPv6 prefix <network>/<length>\n"
		       "Display route and more specific routes\n") {
	return bgp_show_prefix_longer(vty, argv[0], AFI_IP6, SAFI_UNICAST, bgp_show_type_prefix_longer);
}

/* old command */
DEFUN(show_ipv6_bgp_prefix_longer, show_ipv6_bgp_prefix_longer_cmd, "show ipv6 bgp X:X::X:X/M longer-prefixes",
      SHOW_STR IPV6_STR BGP_STR "IPv6 prefix <network>/<length>, e.g., 3ffe::/16\n"
				"Display route and more specific routes\n") {
	return bgp_show_prefix_longer(vty, argv[0], AFI_IP6, SAFI_UNICAST, bgp_show_type_prefix_longer);
}

/* old command */
DEFUN(show_ipv6_mbgp_prefix_longer, show_ipv6_mbgp_prefix_longer_cmd, "show ipv6 mbgp X:X::X:X/M longer-prefixes",
      SHOW_STR IPV6_STR MBGP_STR "IPv6 prefix <network>/<length>, e.g., 3ffe::/16\n"
				 "Display route and more specific routes\n") {
	return bgp_show_prefix_longer(vty, argv[0], AFI_IP6, SAFI_MULTICAST, bgp_show_type_prefix_longer);
}

DEFUN(show_bgp_ipv4_prefix_longer, show_bgp_ipv4_prefix_longer_cmd, "show bgp ipv4 A.B.C.D/M longer-prefixes",
      SHOW_STR BGP_STR IP_STR "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n"
			      "Display route and more specific routes\n") {
	return bgp_show_prefix_longer(vty, argv[0], AFI_IP, SAFI_UNICAST, bgp_show_type_prefix_longer);
}

DEFUN(show_bgp_ipv4_safi_flap_prefix_longer, show_bgp_ipv4_safi_flap_prefix_longer_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) flap-statistics A.B.C.D/M longer-prefixes",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
	...
	return bgp_show_prefix_longer(vty, argv[1], AFI_IP, safi, bgp_show_type_flap_prefix_longer);
}

ALIAS(show_bgp_ipv4_safi_flap_prefix_longer, show_bgp_ipv4_safi_damp_flap_prefix_longer_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) dampening flap-statistics A.B.C.D/M longer-prefixes",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
...
DEFUN(show_bgp_ipv6_safi_flap_prefix_longer, show_bgp_ipv6_safi_flap_prefix_longer_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) flap-statistics X:X::X:X/M longer-prefixes",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
	...
	return bgp_show_prefix_longer(vty, argv[1], AFI_IP6, safi, bgp_show_type_flap_prefix_longer);
}
ALIAS(show_bgp_ipv6_safi_flap_prefix_longer, show_bgp_ipv6_safi_damp_flap_prefix_longer_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) dampening flap-statistics X:X::X:X/M longer-prefixes",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
...
DEFUN(show_bgp_ipv4_safi_prefix_longer, show_bgp_ipv4_safi_prefix_longer_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) A.B.C.D/M longer-prefixes",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
	...
	return bgp_show_prefix_longer(vty, argv[1], AFI_IP, safi, bgp_show_type_prefix_longer);
}

DEFUN(show_bgp_ipv6_safi_prefix_longer, show_bgp_ipv6_safi_prefix_longer_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) X:X::X:X/M longer-prefixes",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
...
DEFUN(show_bgp_ipv4_safi_flap_address, show_bgp_ipv4_safi_flap_address_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) flap-statistics A.B.C.D",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
	...
	return bgp_show_prefix_longer(vty, argv[1], AFI_IP, safi, bgp_show_type_flap_address);
}
ALIAS(show_bgp_ipv4_safi_flap_address, show_bgp_ipv4_safi_damp_flap_address_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) dampening flap-statistics A.B.C.D",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
...
DEFUN(show_bgp_ipv6_flap_address, show_bgp_ipv6_flap_address_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) flap-statistics A.B.C.D",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
...
ALIAS(show_bgp_ipv6_flap_address, show_bgp_ipv6_damp_flap_address_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) dampening flap-statistics A.B.C.D",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
...
DEFUN(show_bgp_ipv4_safi_flap_prefix, show_bgp_ipv4_safi_flap_prefix_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) flap-statistics A.B.C.D/M",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
	...
	return bgp_show_prefix_longer(vty, argv[0], AFI_IP, safi, bgp_show_type_flap_prefix);
}

ALIAS(show_bgp_ipv4_safi_flap_prefix, show_bgp_ipv4_safi_damp_flap_prefix_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) dampening flap-statistics A.B.C.D/M",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
...
DEFUN(show_bgp_ipv6_safi_flap_prefix, show_bgp_ipv6_safi_flap_prefix_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) flap-statistics X:X::X:X/M",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
	...
	return bgp_show_prefix_longer(vty, argv[0], AFI_IP6, safi, bgp_show_type_flap_prefix);
}

ALIAS(show_bgp_ipv6_safi_flap_prefix, show_bgp_ipv6_safi_damp_flap_prefix_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) dampening flap-statistics X:X::X:X/M",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
...
DEFUN(show_bgp_ipv6_prefix_longer, show_bgp_ipv6_prefix_longer_cmd, "show bgp ipv6 X:X::X:X/M longer-prefixes",
      SHOW_STR BGP_STR "Address family\n"
		       "IPv6 prefix <network>/<length>\n"
		       "Display route and more specific routes\n") {
	return bgp_show_prefix_longer(vty, argv[0], AFI_IP6, SAFI_UNICAST, bgp_show_type_prefix_longer);
...
static struct peer *peer_lookup_in_view(struct vty *vty, const char *view_name, const char *ip_str) {
	int ret;
	struct bgp *bgp;
	struct peer *peer;
	union sockunion su;
...
static int bgp_table_stats_walker(struct thread *t) {
	...
	switch(top->p.family) {
		case AF_INET: space = IPV4_MAX_BITLEN; break;
		case AF_INET6: space = IPV6_MAX_BITLEN; break;
	}
	...
	for(rn = top; rn; rn = bgp_route_next(rn)) {
      ...
      ts->counts[BGP_STATS_AVGPLEN]
		...
		for(ri = rn->info; ri; ri = ri->next) {
			...
			if(ri->attr && (CHECK_FLAG(ri->attr->flag, ATTR_FLAG_BIT(BGP_ATTR_ATOMIC_AGGREGATE)))) {
...
static int bgp_table_stats(struct vty *vty, struct bgp *bgp, afi_t afi, safi_t safi) {
	...
	if(!bgp->rib[afi][safi]) {
		vty_out(vty, "%% No RIB exists for the specified AFI(%d)/SAFI(%d) %s", afi, safi, VTY_NEWLINE);
		return CMD_WARNING;
	}
	...
	vty_out(vty, "BGP %s RIB statistics%s%s", afi_safi_print(afi, safi), VTY_NEWLINE, VTY_NEWLINE);
		...
		switch(i) {
          ...
          case BGP_STATS_AVGPLEN:
			...
			case BGP_STATS_ASPATH_TOTSIZE:
				vty_out(vty, "%-30s: ", table_stats_strs[i]);
				vty_out(vty, "%12.2f", ts.counts[i] ? (float) ts.counts[i] / (float) ts.counts[BGP_STATS_ASPATH_COUNT] : 0);
				break;
			case BGP_STATS_TOTPLEN:
				vty_out(vty, "%-30s: ", table_stats_strs[i]);
				vty_out(vty, "%12.2f", ts.counts[i] ? (float) ts.counts[i] / (float) ts.counts[BGP_STATS_PREFIXES] : 0);
				break;
			case BGP_STATS_SPACE:
				...
				vty_out(vty, "%12.2f%s", 100 * (float) ts.counts[BGP_STATS_SPACE] / (float) ((uint64_t) 1UL << ts.counts[BGP_STATS_MAXBITLEN]), VTY_NEWLINE);
				vty_out(vty, "%30s: ", "/8 equivalent ");
				vty_out(vty, "%12.2f%s", (float) ts.counts[BGP_STATS_SPACE] / (float) (1UL << (ts.counts[BGP_STATS_MAXBITLEN] - 8)), VTY_NEWLINE);
				if(ts.counts[BGP_STATS_MAXBITLEN] < 25) {
					break;
				}
				vty_out(vty, "%30s: ", "/24 equivalent ");
				vty_out(vty, "%12.2f", (float) ts.counts[BGP_STATS_SPACE] / (float) (1UL << (ts.counts[BGP_STATS_MAXBITLEN] - 24)));
				break;
			default: vty_out(vty, "%-30s: ", table_stats_strs[i]); vty_out(vty, "%12llu", ts.counts[i]);
		}
...
static int bgp_table_stats_vty(struct vty *vty, const char *name, const char *afi_str, const char *safi_str) {
	struct bgp *bgp;
	afi_t afi;
	safi_t safi;
	...
	if(strncmp(afi_str, "ipv", 3) == 0) {
		if(strncmp(afi_str, "ipv4", 4) == 0) {
			...
			vty_out(vty, "%% Invalid address family %s%s", afi_str, VTY_NEWLINE);
			return CMD_WARNING;
		}
		switch(safi_str[0]) {
			case 'm': safi = SAFI_MULTICAST; break;
			case 'u': safi = SAFI_UNICAST; break;
			case 'v': safi = SAFI_MPLS_VPN; break;
			case 'e': safi = SAFI_ENCAP; break;
			default: vty_out(vty, "%% Invalid subsequent address family %s%s", safi_str, VTY_NEWLINE); return CMD_WARNING;
		}
	} else {
		vty_out(vty, "%% Invalid address family \"%s\"%s", afi_str, VTY_NEWLINE);
		return CMD_WARNING;
...
DEFUN(show_bgp_statistics, show_bgp_statistics_cmd, "show bgp (ipv4|ipv6) (encap|multicast|unicast|vpn) statistics",
      SHOW_STR BGP_STR "Address family\n"
		       "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
...
ALIAS(show_bgp_statistics, show_bgp_statistics_vpnv4_cmd, "show bgp (ipv4) (vpnv4) statistics",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "BGP RIB advertisement statistics\n")

DEFUN(show_bgp_statistics_view, show_bgp_statistics_view_cmd, "show bgp view WORD (ipv4|ipv6) (encap|multicast|unicast|vpn) statistics",
      SHOW_STR BGP_STR "BGP view\n"
		       "Address family\n"
		       "Address family\n"
		       "Address Family modifier\n"
...
ALIAS(show_bgp_statistics_view, show_bgp_statistics_view_vpnv4_cmd, "show bgp view WORD (ipv4) (vpnv4) statistics",
      SHOW_STR BGP_STR "BGP view\n"
		       "Address family\n"
		       "Address Family modifier\n"
		       "BGP RIB advertisement statistics\n")
...
static const char *pcount_strs[] = {
	[PCOUNT_ADJ_IN] = "Adj-in", [PCOUNT_DAMPED] = "Damped", [PCOUNT_REMOVED] = "Removed",	    [PCOUNT_HISTORY] = "History", [PCOUNT_STALE] = "Stale",
	[PCOUNT_VALID] = "Valid",   [PCOUNT_ALL] = "All RIB",	[PCOUNT_COUNTED] = "PfxCt counted", [PCOUNT_PFCNT] = "Useable",	  [PCOUNT_MAX] = NULL,
};
	...
	for(rn = bgp_table_top(pc->table); rn; rn = bgp_route_next(rn)) {
		...
		for(ri = rn->info; ri; ri = ri->next) {
			...
			if(CHECK_FLAG(ri->flags, BGP_INFO_COUNTED)) {
				...
				if(CHECK_FLAG(ri->flags, BGP_INFO_UNUSEABLE)) {
					plog_warn(peer->log, "%s [pcount] %s/%d is counted but flags 0x%x", peer->host, inet_ntop(rn->p.family, &rn->p.u.prefix, buf, SU_ADDRSTRLEN), rn->p.prefixlen, ri->flags);
				}
			} else {
				if(!CHECK_FLAG(ri->flags, BGP_INFO_UNUSEABLE)) {
					plog_warn(peer->log, "%s [pcount] %s/%d not counted but flags 0x%x", peer->host, inet_ntop(rn->p.family, &rn->p.u.prefix, buf, SU_ADDRSTRLEN), rn->p.prefixlen, ri->flags);
				}
			}
		}
	}
...
static int bgp_peer_counts(struct vty *vty, struct peer *peer, afi_t afi, safi_t safi) {
	struct peer_pcounts pcounts = { .peer = peer };
	unsigned int i;

	if(!peer || !peer->bgp || !peer->afc[afi][safi] || !peer->bgp->rib[afi][safi]) {
   ...
   */
	...
	vty_out(vty, "Prefix counts for %s, %s%s", peer->host, afi_safi_print(afi, safi), VTY_NEWLINE);
	vty_out(vty, "PfxCt: %ld%s", peer->pcount[afi][safi], VTY_NEWLINE);
	vty_out(vty, "%sCounts from RIB table walk:%s%s", VTY_NEWLINE, VTY_NEWLINE, VTY_NEWLINE);
	...
	if(pcounts.count[PCOUNT_PFCNT] != peer->pcount[afi][safi]) {
		vty_out(vty, "%s [pcount] PfxCt drift!%s", peer->host, VTY_NEWLINE);
		vty_out(vty, "Please report this bug, with the above command output%s", VTY_NEWLINE);
	}

	return CMD_SUCCESS;
}

DEFUN(show_ip_bgp_neighbor_prefix_counts, show_ip_bgp_neighbor_prefix_counts_cmd, "show ip bgp neighbors (A.B.C.D|X:X::X:X) prefix-counts",
      SHOW_STR IP_STR BGP_STR "Detailed information on TCP and BGP neighbor connections\n"
...
DEFUN(show_bgp_ipv6_neighbor_prefix_counts, show_bgp_ipv6_neighbor_prefix_counts_cmd, "show bgp ipv6 neighbors (A.B.C.D|X:X::X:X) prefix-counts",
      SHOW_STR BGP_STR "Address family\n"
...
DEFUN(show_ip_bgp_ipv4_neighbor_prefix_counts, show_ip_bgp_ipv4_neighbor_prefix_counts_cmd, "show ip bgp ipv4 (unicast|multicast) neighbors (A.B.C.D|X:X::X:X) prefix-counts",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Detailed information on TCP and BGP neighbor connections\n"
...
DEFUN(show_ip_bgp_vpnv4_neighbor_prefix_counts, show_ip_bgp_vpnv4_neighbor_prefix_counts_cmd, "show ip bgp vpnv4 all neighbors (A.B.C.D|X:X::X:X) prefix-counts",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Detailed information on TCP and BGP neighbor connections\n"
...
DEFUN(show_bgp_ipv4_safi_neighbor_prefix_counts, show_bgp_ipv4_safi_neighbor_prefix_counts_cmd, "show bgp ipv4 (unicast|multicast) neighbors (A.B.C.D|X:X::X:X) prefix-counts",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
...
DEFUN(show_bgp_ipv6_safi_neighbor_prefix_counts, show_bgp_ipv6_safi_neighbor_prefix_counts_cmd, "show bgp ipv6 (unicast|multicast) neighbors (A.B.C.D|X:X::X:X) prefix-counts",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
...
DEFUN(show_ip_bgp_encap_neighbor_prefix_counts, show_ip_bgp_encap_neighbor_prefix_counts_cmd, "show ip bgp encap all neighbors (A.B.C.D|X:X::X:X) prefix-counts",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Detailed information on TCP and BGP neighbor connections\n"
...
static void show_adj_route(struct vty *vty, struct peer *peer, afi_t afi, safi_t safi, int in) {
	struct bgp_table *table;
	struct bgp_adj_in *ain;
	...
	if(!in && CHECK_FLAG(peer->af_sflags[afi][safi], PEER_STATUS_DEFAULT_ORIGINATE)) {
		vty_out(vty, "BGP table version is 0, local router ID is %s%s", inet_ntoa(bgp->router_id), VTY_NEWLINE);
		vty_out(vty, BGP_SHOW_SCODE_HEADER, VTY_NEWLINE, VTY_NEWLINE);
		vty_out(vty, BGP_SHOW_OCODE_HEADER, VTY_NEWLINE, VTY_NEWLINE);

		vty_out(vty, "Originating default network 0.0.0.0%s%s", VTY_NEWLINE, VTY_NEWLINE);
		header1 = 0;
	}
	...
	if(output_count != 0) {
		vty_out(vty, "%sTotal number of prefixes %ld%s", VTY_NEWLINE, output_count, VTY_NEWLINE);
	}
}

static int peer_adj_routes(struct vty *vty, struct peer *peer, afi_t afi, safi_t safi, int in) {
...
DEFUN(show_ip_bgp_view_neighbor_advertised_route, show_ip_bgp_view_neighbor_advertised_route_cmd, "show ip bgp view WORD neighbors (A.B.C.D|X:X::X:X) advertised-routes",
      SHOW_STR IP_STR BGP_STR "BGP view\n"
			      "View name\n"
			      "Detailed information on TCP and BGP neighbor connections\n"
			      "Neighbor to display information about\n"
...
ALIAS(show_ip_bgp_view_neighbor_advertised_route, show_ip_bgp_neighbor_advertised_route_cmd, "show ip bgp neighbors (A.B.C.D|X:X::X:X) advertised-routes",
      SHOW_STR IP_STR BGP_STR "Detailed information on TCP and BGP neighbor connections\n"
			      "Neighbor to display information about\n"
			      "Neighbor to display information about\n"
			      "Display the routes advertised to a BGP neighbor\n")

DEFUN(show_ip_bgp_ipv4_neighbor_advertised_route, show_ip_bgp_ipv4_neighbor_advertised_route_cmd, "show ip bgp ipv4 (unicast|multicast) neighbors (A.B.C.D|X:X::X:X) advertised-routes",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Detailed information on TCP and BGP neighbor connections\n"
...
DEFUN(show_bgp_view_neighbor_advertised_route, show_bgp_view_neighbor_advertised_route_cmd, "show bgp view WORD neighbors (A.B.C.D|X:X::X:X) advertised-routes",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
...
DEFUN(show_bgp_view_neighbor_received_routes, show_bgp_view_neighbor_received_routes_cmd, "show bgp view WORD neighbors (A.B.C.D|X:X::X:X) received-routes",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
...
ALIAS(show_bgp_view_neighbor_advertised_route, show_bgp_neighbor_advertised_route_cmd, "show bgp neighbors (A.B.C.D|X:X::X:X) advertised-routes",
      SHOW_STR BGP_STR "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
		       "Neighbor to display information about\n"
		       "Display the routes advertised to a BGP neighbor\n")

ALIAS(show_bgp_view_neighbor_advertised_route, show_bgp_ipv6_neighbor_advertised_route_cmd, "show bgp ipv6 neighbors (A.B.C.D|X:X::X:X) advertised-routes",
      SHOW_STR BGP_STR "Address family\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
		       "Neighbor to display information about\n"
...
/* old command */
ALIAS(show_bgp_view_neighbor_advertised_route, ipv6_bgp_neighbor_advertised_route_cmd, "show ipv6 bgp neighbors (A.B.C.D|X:X::X:X) advertised-routes",
      SHOW_STR IPV6_STR BGP_STR "Detailed information on TCP and BGP neighbor connections\n"
				"Neighbor to display information about\n"
				"Neighbor to display information about\n"
				"Display the routes advertised to a BGP neighbor\n")

/* old command */
DEFUN(ipv6_mbgp_neighbor_advertised_route, ipv6_mbgp_neighbor_advertised_route_cmd, "show ipv6 mbgp neighbors (A.B.C.D|X:X::X:X) advertised-routes",
      SHOW_STR IPV6_STR MBGP_STR "Detailed information on TCP and BGP neighbor connections\n"
				 "Neighbor to display information about\n"
				 "Neighbor to display information about\n"
				 "Display the routes advertised to a BGP neighbor\n") {
	struct peer *peer;
...
DEFUN(show_ip_bgp_view_neighbor_received_routes, show_ip_bgp_view_neighbor_received_routes_cmd, "show ip bgp view WORD neighbors (A.B.C.D|X:X::X:X) received-routes",
      SHOW_STR IP_STR BGP_STR "BGP view\n"
			      "View name\n"
			      "Detailed information on TCP and BGP neighbor connections\n"
			      "Neighbor to display information about\n"
...
ALIAS(show_ip_bgp_view_neighbor_received_routes, show_ip_bgp_neighbor_received_routes_cmd, "show ip bgp neighbors (A.B.C.D|X:X::X:X) received-routes",
      SHOW_STR IP_STR BGP_STR "Detailed information on TCP and BGP neighbor connections\n"
			      "Neighbor to display information about\n"
			      "Neighbor to display information about\n"
			      "Display the received routes from neighbor\n")

ALIAS(show_bgp_view_neighbor_received_routes, show_bgp_ipv6_neighbor_received_routes_cmd, "show bgp ipv6 neighbors (A.B.C.D|X:X::X:X) received-routes",
      SHOW_STR BGP_STR "Address family\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
		       "Neighbor to display information about\n"
...
DEFUN(show_bgp_neighbor_received_prefix_filter, show_bgp_neighbor_received_prefix_filter_cmd, "show bgp neighbors (A.B.C.D|X:X::X:X) received prefix-filter",
      SHOW_STR BGP_STR "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
		       "Neighbor to display information about\n"
...
/* old command */
ALIAS(show_bgp_view_neighbor_received_routes, ipv6_bgp_neighbor_received_routes_cmd, "show ipv6 bgp neighbors (A.B.C.D|X:X::X:X) received-routes",
      SHOW_STR IPV6_STR BGP_STR "Detailed information on TCP and BGP neighbor connections\n"
				"Neighbor to display information about\n"
				"Neighbor to display information about\n"
				"Display the received routes from neighbor\n")
...
/* old command */
DEFUN(ipv6_mbgp_neighbor_received_routes, ipv6_mbgp_neighbor_received_routes_cmd, "show ipv6 mbgp neighbors (A.B.C.D|X:X::X:X) received-routes",
      SHOW_STR IPV6_STR MBGP_STR "Detailed information on TCP and BGP neighbor connections\n"
				 "Neighbor to display information about\n"
				 "Neighbor to display information about\n"
				 "Display the received routes from neighbor\n") {
...
DEFUN(show_bgp_view_neighbor_received_prefix_filter, show_bgp_view_neighbor_received_prefix_filter_cmd, "show bgp view WORD neighbors (A.B.C.D|X:X::X:X) received prefix-filter",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
...
DEFUN(show_ip_bgp_ipv4_neighbor_received_routes, show_ip_bgp_ipv4_neighbor_received_routes_cmd, "show ip bgp ipv4 (unicast|multicast) neighbors (A.B.C.D|X:X::X:X) received-routes",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Detailed information on TCP and BGP neighbor connections\n"
...
DEFUN(show_bgp_ipv4_safi_neighbor_advertised_route, show_bgp_ipv4_safi_neighbor_advertised_route_cmd, "show bgp ipv4 (multicast|unicast) neighbors (A.B.C.D|X:X::X:X) advertised-routes",
      SHOW_STR BGP_STR "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
...
DEFUN(show_bgp_ipv6_safi_neighbor_advertised_route, show_bgp_ipv6_safi_neighbor_advertised_route_cmd, "show bgp ipv6 (multicast|unicast) neighbors (A.B.C.D|X:X::X:X) advertised-routes",
      SHOW_STR BGP_STR "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
...
DEFUN(show_bgp_view_ipv6_neighbor_advertised_route, show_bgp_view_ipv6_neighbor_advertised_route_cmd, "show bgp view WORD ipv6 neighbors (A.B.C.D|X:X::X:X) advertised-routes",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
...
DEFUN(show_bgp_view_ipv6_neighbor_received_routes, show_bgp_view_ipv6_neighbor_received_routes_cmd, "show bgp view WORD ipv6 neighbors (A.B.C.D|X:X::X:X) received-routes",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
...
DEFUN(show_bgp_ipv4_safi_neighbor_received_routes, show_bgp_ipv4_safi_neighbor_received_routes_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) neighbors (A.B.C.D|X:X::X:X) received-routes",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
...
DEFUN(show_bgp_ipv6_safi_neighbor_received_routes, show_bgp_ipv6_safi_neighbor_received_routes_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) neighbors (A.B.C.D|X:X::X:X) received-routes",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
...
DEFUN(show_bgp_view_afi_safi_neighbor_adv_recd_routes, show_bgp_view_afi_safi_neighbor_adv_recd_routes_cmd, "show bgp view WORD (ipv4|ipv6) (unicast|multicast) neighbors (A.B.C.D|X:X::X:X) (advertised-routes|received-routes)",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "Address family\n"
...
DEFUN(show_ip_bgp_neighbor_received_prefix_filter, show_ip_bgp_neighbor_received_prefix_filter_cmd, "show ip bgp neighbors (A.B.C.D|X:X::X:X) received prefix-filter",
      SHOW_STR IP_STR BGP_STR "Detailed information on TCP and BGP neighbor connections\n"
			      "Neighbor to display information about\n"
			      "Neighbor to display information about\n"
			      "Display information received from a BGP neighbor\n"
...
DEFUN(show_ip_bgp_ipv4_neighbor_received_prefix_filter, show_ip_bgp_ipv4_neighbor_received_prefix_filter_cmd, "show ip bgp ipv4 (unicast|multicast) neighbors (A.B.C.D|X:X::X:X) received prefix-filter",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Detailed information on TCP and BGP neighbor connections\n"
...
ALIAS(show_bgp_view_neighbor_received_routes, show_bgp_neighbor_received_routes_cmd, "show bgp neighbors (A.B.C.D|X:X::X:X) received-routes",
      SHOW_STR BGP_STR "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
		       "Neighbor to display information about\n"
		       "Display the received routes from neighbor\n")

DEFUN(show_bgp_ipv4_safi_neighbor_received_prefix_filter, show_bgp_ipv4_safi_neighbor_received_prefix_filter_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) neighbors (A.B.C.D|X:X::X:X) received prefix-filter",
      SHOW_STR BGP_STR IP_STR "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
...
DEFUN(show_bgp_ipv6_safi_neighbor_received_prefix_filter, show_bgp_ipv6_safi_neighbor_received_prefix_filter_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) neighbors (A.B.C.D|X:X::X:X) received prefix-filter",
      SHOW_STR BGP_STR IP_STR "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
...
DEFUN(show_bgp_ipv6_neighbor_received_prefix_filter, show_bgp_ipv6_neighbor_received_prefix_filter_cmd, "show bgp ipv6 neighbors (A.B.C.D|X:X::X:X) received prefix-filter",
      SHOW_STR BGP_STR "Address family\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
		       "Neighbor to display information about\n"
...
DEFUN(show_bgp_view_ipv6_neighbor_received_prefix_filter, show_bgp_view_ipv6_neighbor_received_prefix_filter_cmd, "show bgp view WORD ipv6 neighbors (A.B.C.D|X:X::X:X) received prefix-filter",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
...
static int bgp_show_neighbor_route(struct vty *vty, struct peer *peer, afi_t afi, safi_t safi, enum bgp_show_type type) {
	if(!peer || !peer->afc[afi][safi]) {
		vty_out(vty, "%% No such neighbor or address family%s", VTY_NEWLINE);
		return CMD_WARNING;
	}
...
DEFUN(show_ip_bgp_neighbor_routes, show_ip_bgp_neighbor_routes_cmd, "show ip bgp neighbors (A.B.C.D|X:X::X:X) routes",
      SHOW_STR IP_STR BGP_STR "Detailed information on TCP and BGP neighbor connections\n"
			      "Neighbor to display information about\n"
			      "Neighbor to display information about\n"
	...
	return bgp_show_neighbor_route(vty, peer, AFI_IP, SAFI_UNICAST, bgp_show_type_neighbor);
}

DEFUN(show_ip_bgp_neighbor_flap, show_ip_bgp_neighbor_flap_cmd, "show ip bgp neighbors (A.B.C.D|X:X::X:X) flap-statistics",
      SHOW_STR IP_STR BGP_STR "Detailed information on TCP and BGP neighbor connections\n"
			      "Neighbor to display information about\n"
			      "Neighbor to display information about\n"
	...
	return bgp_show_neighbor_route(vty, peer, AFI_IP, SAFI_UNICAST, bgp_show_type_flap_neighbor);
}

DEFUN(show_ip_bgp_neighbor_damp, show_ip_bgp_neighbor_damp_cmd, "show ip bgp neighbors (A.B.C.D|X:X::X:X) dampened-routes",
      SHOW_STR IP_STR BGP_STR "Detailed information on TCP and BGP neighbor connections\n"
			      "Neighbor to display information about\n"
			      "Neighbor to display information about\n"
	...
	return bgp_show_neighbor_route(vty, peer, AFI_IP, SAFI_UNICAST, bgp_show_type_damp_neighbor);
}

DEFUN(show_ip_bgp_ipv4_neighbor_routes, show_ip_bgp_ipv4_neighbor_routes_cmd, "show ip bgp ipv4 (unicast|multicast) neighbors (A.B.C.D|X:X::X:X) routes",
      SHOW_STR IP_STR BGP_STR "Address family\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Detailed information on TCP and BGP neighbor connections\n"
	...
	if(strncmp(argv[0], "m", 1) == 0) {
		return bgp_show_neighbor_route(vty, peer, AFI_IP, SAFI_MULTICAST, bgp_show_type_neighbor);
	}

	return bgp_show_neighbor_route(vty, peer, AFI_IP, SAFI_UNICAST, bgp_show_type_neighbor);
}

DEFUN(show_ip_bgp_view_rsclient, show_ip_bgp_view_rsclient_cmd, "show ip bgp view WORD rsclient (A.B.C.D|X:X::X:X)",
      SHOW_STR IP_STR BGP_STR "BGP view\n"
			      "View name\n"
	...
	if(!peer->afc[AFI_IP][SAFI_UNICAST]) {
		vty_out(vty, "%% Activate the neighbor for the address family first%s", VTY_NEWLINE);
		return CMD_WARNING;
	}

	if(!CHECK_FLAG(peer->af_flags[AFI_IP][SAFI_UNICAST], PEER_FLAG_RSERVER_CLIENT)) {
		vty_out(vty, "%% Neighbor is not a Route-Server client%s", VTY_NEWLINE);
		return CMD_WARNING;
	}
...
ALIAS(show_ip_bgp_view_rsclient, show_ip_bgp_rsclient_cmd, "show ip bgp rsclient (A.B.C.D|X:X::X:X)", SHOW_STR IP_STR BGP_STR "Information about Route Server Client\n" NEIGHBOR_ADDR_STR)

DEFUN(show_bgp_view_ipv4_safi_rsclient, show_bgp_view_ipv4_safi_rsclient_cmd, "show bgp view WORD ipv4 (unicast|multicast) rsclient (A.B.C.D|X:X::X:X)",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "Address Family modifier\n"
	...
	if(!CHECK_FLAG(peer->af_flags[AFI_IP][safi], PEER_FLAG_RSERVER_CLIENT)) {
		vty_out(vty, "%% Neighbor is not a Route-Server client%s", VTY_NEWLINE);
		return CMD_WARNING;
	}
...
ALIAS(show_bgp_view_ipv4_safi_rsclient, show_bgp_ipv4_safi_rsclient_cmd, "show bgp ipv4 (unicast|multicast) rsclient (A.B.C.D|X:X::X:X)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Information about Route Server Client\n" NEIGHBOR_ADDR_STR)

DEFUN(show_ip_bgp_view_rsclient_route, show_ip_bgp_view_rsclient_route_cmd, "show ip bgp view WORD rsclient (A.B.C.D|X:X::X:X) A.B.C.D",
      SHOW_STR IP_STR BGP_STR "BGP view\n"
			      "View name\n"
	...
	return bgp_show_route_in_table(vty, bgp, peer->rib[AFI_IP][SAFI_UNICAST], (argc == 3) ? argv[2] : argv[1], AFI_IP, SAFI_UNICAST, NULL, 0, BGP_PATH_ALL);
}

ALIAS(show_ip_bgp_view_rsclient_route, show_ip_bgp_rsclient_route_cmd, "show ip bgp rsclient (A.B.C.D|X:X::X:X) A.B.C.D",
      SHOW_STR IP_STR BGP_STR "Information about Route Server Client\n" NEIGHBOR_ADDR_STR "Network in the BGP routing table to display\n")

DEFUN(show_bgp_ipv4_safi_neighbor_flap, show_bgp_ipv4_safi_neighbor_flap_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) neighbors (A.B.C.D|X:X::X:X) flap-statistics",
      SHOW_STR BGP_STR "Address Family Modifier\n"
		       "Address Family Modifier\n"
		       "Address Family Modifier\n"
		       "Address Family Modifier\n"
	...
	return bgp_show_neighbor_route(vty, peer, AFI_IP, safi, bgp_show_type_flap_neighbor);
}

DEFUN(show_bgp_ipv6_safi_neighbor_flap, show_bgp_ipv6_safi_neighbor_flap_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) neighbors (A.B.C.D|X:X::X:X) flap-statistics",
      SHOW_STR BGP_STR "Address Family Modifier\n"
		       "Address Family Modifier\n"
		       "Address Family Modifier\n"
		       "Address Family Modifier\n"
	...
	return bgp_show_neighbor_route(vty, peer, AFI_IP6, safi, bgp_show_type_flap_neighbor);
}

DEFUN(show_bgp_ipv4_safi_neighbor_damp, show_bgp_ipv4_safi_neighbor_damp_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) neighbors (A.B.C.D|X:X::X:X) dampened-routes",
      SHOW_STR BGP_STR "Address Family Modifier\n"
		       "Address Family Modifier\n"
		       "Address Family Modifier\n"
		       "Address Family Modifier\n"
	...
	return bgp_show_neighbor_route(vty, peer, AFI_IP, safi, bgp_show_type_damp_neighbor);
}

DEFUN(show_bgp_ipv6_safi_neighbor_damp, show_bgp_ipv6_safi_neighbor_damp_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) neighbors (A.B.C.D|X:X::X:X) dampened-routes",
      SHOW_STR BGP_STR "Address Family Modifier\n"
		       "Address Family Modifier\n"
		       "Address Family Modifier\n"
		       "Address Family Modifier\n"
	...
	return bgp_show_neighbor_route(vty, peer, AFI_IP6, safi, bgp_show_type_damp_neighbor);
}

DEFUN(show_bgp_ipv4_safi_neighbor_routes, show_bgp_ipv4_safi_neighbor_routes_cmd, "show bgp ipv4 (multicast|unicast) neighbors (A.B.C.D|X:X::X:X) routes",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
	...
	return bgp_show_neighbor_route(vty, peer, AFI_IP, safi, bgp_show_type_neighbor);
}

DEFUN(show_bgp_ipv6_safi_neighbor_routes, show_bgp_ipv6_safi_neighbor_routes_cmd, "show bgp ipv6 (multicast|unicast) neighbors (A.B.C.D|X:X::X:X) routes",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
	...
	return bgp_show_neighbor_route(vty, peer, AFI_IP6, safi, bgp_show_type_neighbor);
}

DEFUN(show_bgp_view_ipv4_safi_rsclient_route, show_bgp_view_ipv4_safi_rsclient_route_cmd, "show bgp view WORD ipv4 (unicast|multicast) rsclient (A.B.C.D|X:X::X:X) A.B.C.D",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "Address Family modifier\n"
	...
	return bgp_show_route_in_table(vty, bgp, peer->rib[AFI_IP][safi], (argc == 4) ? argv[3] : argv[2], AFI_IP, safi, NULL, 0, BGP_PATH_ALL);
}

ALIAS(show_bgp_view_ipv4_safi_rsclient_route, show_bgp_ipv4_safi_rsclient_route_cmd, "show bgp ipv4 (unicast|multicast) rsclient (A.B.C.D|X:X::X:X) A.B.C.D",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Information about Route Server Client\n" NEIGHBOR_ADDR_STR "Network in the BGP routing table to display\n")

DEFUN(show_bgp_view_ipv4_safi_rsclient_prefix, show_bgp_view_ipv4_safi_rsclient_prefix_cmd, "show bgp view WORD ipv4 (unicast|multicast) rsclient (A.B.C.D|X:X::X:X) A.B.C.D/M",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "Address Family modifier\n"
	...
	return bgp_show_route_in_table(vty, bgp, peer->rib[AFI_IP][safi], (argc == 4) ? argv[3] : argv[2], AFI_IP, safi, NULL, 1, BGP_PATH_ALL);
}

DEFUN(show_ip_bgp_view_rsclient_prefix, show_ip_bgp_view_rsclient_prefix_cmd, "show ip bgp view WORD rsclient (A.B.C.D|X:X::X:X) A.B.C.D/M",
      SHOW_STR IP_STR BGP_STR "BGP view\n"
			      "View name\n"
			      "Information about Route Server Client\n" NEIGHBOR_ADDR_STR "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n") {
	struct bgp *bgp;
	...
	return bgp_show_route_in_table(vty, bgp, peer->rib[AFI_IP][SAFI_UNICAST], (argc == 3) ? argv[2] : argv[1], AFI_IP, SAFI_UNICAST, NULL, 1, BGP_PATH_ALL);
}

ALIAS(show_ip_bgp_view_rsclient_prefix, show_ip_bgp_rsclient_prefix_cmd, "show ip bgp rsclient (A.B.C.D|X:X::X:X) A.B.C.D/M",
      SHOW_STR IP_STR BGP_STR "Information about Route Server Client\n" NEIGHBOR_ADDR_STR "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n")

ALIAS(show_bgp_view_ipv4_safi_rsclient_prefix, show_bgp_ipv4_safi_rsclient_prefix_cmd, "show bgp ipv4 (unicast|multicast) rsclient (A.B.C.D|X:X::X:X) A.B.C.D/M",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Information about Route Server Client\n" NEIGHBOR_ADDR_STR "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n")

DEFUN(show_bgp_view_ipv6_neighbor_routes, show_bgp_view_ipv6_neighbor_routes_cmd, "show bgp view WORD ipv6 neighbors (A.B.C.D|X:X::X:X) routes",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
	...
	return bgp_show_neighbor_route(vty, peer, AFI_IP6, SAFI_UNICAST, bgp_show_type_neighbor);
}

DEFUN(show_bgp_view_neighbor_damp, show_bgp_view_neighbor_damp_cmd, "show bgp view WORD neighbors (A.B.C.D|X:X::X:X) dampened-routes",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
	...
	return bgp_show_neighbor_route(vty, peer, AFI_IP6, SAFI_UNICAST, bgp_show_type_damp_neighbor);
}

DEFUN(show_bgp_view_ipv6_neighbor_damp, show_bgp_view_ipv6_neighbor_damp_cmd, "show bgp view WORD ipv6 neighbors (A.B.C.D|X:X::X:X) dampened-routes",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
...
DEFUN(show_bgp_view_ipv6_neighbor_flap, show_bgp_view_ipv6_neighbor_flap_cmd, "show bgp view WORD ipv6 neighbors (A.B.C.D|X:X::X:X) flap-statistics",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
	...
	return bgp_show_neighbor_route(vty, peer, AFI_IP6, SAFI_UNICAST, bgp_show_type_flap_neighbor);
}

DEFUN(show_bgp_view_neighbor_flap, show_bgp_view_neighbor_flap_cmd, "show bgp view WORD neighbors (A.B.C.D|X:X::X:X) flap-statistics",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
...
ALIAS(show_bgp_view_neighbor_flap, show_bgp_neighbor_flap_cmd, "show bgp neighbors (A.B.C.D|X:X::X:X) flap-statistics",
      SHOW_STR BGP_STR "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
		       "Neighbor to display information about\n"
		       "Display flap statistics of the routes learned from neighbor\n")

ALIAS(show_bgp_view_neighbor_damp, show_bgp_neighbor_damp_cmd, "show bgp neighbors (A.B.C.D|X:X::X:X) dampened-routes",
      SHOW_STR BGP_STR "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
		       "Neighbor to display information about\n"
		       "Display the dampened routes received from neighbor\n")

DEFUN(show_bgp_view_neighbor_routes, show_bgp_view_neighbor_routes_cmd, "show bgp view WORD neighbors (A.B.C.D|X:X::X:X) routes",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
...
ALIAS(show_bgp_view_neighbor_routes, show_bgp_neighbor_routes_cmd, "show bgp neighbors (A.B.C.D|X:X::X:X) routes",
      SHOW_STR BGP_STR "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
		       "Neighbor to display information about\n"
		       "Display routes learned from neighbor\n")

ALIAS(show_bgp_view_neighbor_routes, show_bgp_ipv6_neighbor_routes_cmd, "show bgp ipv6 neighbors (A.B.C.D|X:X::X:X) routes",
      SHOW_STR BGP_STR "Address family\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
		       "Neighbor to display information about\n"
...
/* old command */
ALIAS(show_bgp_view_neighbor_routes, ipv6_bgp_neighbor_routes_cmd, "show ipv6 bgp neighbors (A.B.C.D|X:X::X:X) routes",
      SHOW_STR IPV6_STR BGP_STR "Detailed information on TCP and BGP neighbor connections\n"
				"Neighbor to display information about\n"
				"Neighbor to display information about\n"
				"Display routes learned from neighbor\n")
...
/* old command */
DEFUN(ipv6_mbgp_neighbor_routes, ipv6_mbgp_neighbor_routes_cmd, "show ipv6 mbgp neighbors (A.B.C.D|X:X::X:X) routes",
      SHOW_STR IPV6_STR MBGP_STR "Detailed information on TCP and BGP neighbor connections\n"
				 "Neighbor to display information about\n"
				 "Neighbor to display information about\n"
				 "Display routes learned from neighbor\n") {
	...
	return bgp_show_neighbor_route(vty, peer, AFI_IP6, SAFI_MULTICAST, bgp_show_type_neighbor);
}

ALIAS(show_bgp_view_neighbor_flap, show_bgp_ipv6_neighbor_flap_cmd, "show bgp ipv6 neighbors (A.B.C.D|X:X::X:X) flap-statistics",
      SHOW_STR BGP_STR "Address family\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
		       "Neighbor to display information about\n"
...
ALIAS(show_bgp_view_neighbor_damp, show_bgp_ipv6_neighbor_damp_cmd, "show bgp ipv6 neighbors (A.B.C.D|X:X::X:X) dampened-routes",
      SHOW_STR BGP_STR "Address family\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
		       "Neighbor to display information about\n"
...
DEFUN(show_bgp_view_rsclient, show_bgp_view_rsclient_cmd, "show bgp view WORD rsclient (A.B.C.D|X:X::X:X)",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Information about Route Server Client\n" NEIGHBOR_ADDR_STR) {
	struct bgp_table *table;
	...
	if(!CHECK_FLAG(peer->af_flags[AFI_IP6][SAFI_UNICAST], PEER_FLAG_RSERVER_CLIENT)) {
		vty_out(vty, "%% Neighbor is not a Route-Server client%s", VTY_NEWLINE);
		return CMD_WARNING;
	}
...
ALIAS(show_bgp_view_rsclient, show_bgp_rsclient_cmd, "show bgp rsclient (A.B.C.D|X:X::X:X)", SHOW_STR BGP_STR "Information about Route Server Client\n" NEIGHBOR_ADDR_STR)

DEFUN(show_bgp_view_ipv4_rsclient, show_bgp_view_ipv4_rsclient_cmd, "show bgp view WORD ipv4 rsclient (A.B.C.D|X:X::X:X)",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address Family\n"
		       "Information about Route Server Client\n" NEIGHBOR_ADDR_STR2) {
...
DEFUN(show_bgp_view_ipv6_rsclient, show_bgp_view_ipv6_rsclient_cmd, "show bgp view WORD ipv6 rsclient (A.B.C.D|X:X::X:X)",
      SHOW_STR BGP_STR "BGP view\n"
		       "BGP view name\n"
		       "Address Family\n"
		       "Information about Route Server Client\n" NEIGHBOR_ADDR_STR2) {
...
ALIAS(show_bgp_view_ipv4_rsclient, show_bgp_ipv4_rsclient_cmd, "show bgp ipv4 rsclient (A.B.C.D|X:X::X:X)",
      SHOW_STR BGP_STR "Address Family\n"
		       "Information about Route Server Client\n" NEIGHBOR_ADDR_STR2)

ALIAS(show_bgp_view_ipv6_rsclient, show_bgp_ipv6_rsclient_cmd, "show bgp ipv6 rsclient (A.B.C.D|X:X::X:X)",
      SHOW_STR BGP_STR "Address Family\n"
		       "Information about Route Server Client\n" NEIGHBOR_ADDR_STR2)

DEFUN(show_bgp_view_ipv6_safi_rsclient, show_bgp_view_ipv6_safi_rsclient_cmd, "show bgp view WORD ipv6 (unicast|multicast) rsclient (A.B.C.D|X:X::X:X)",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "Address Family modifier\n"
	...
	if(!CHECK_FLAG(peer->af_flags[AFI_IP6][safi], PEER_FLAG_RSERVER_CLIENT)) {
		vty_out(vty, "%% Neighbor is not a Route-Server client%s", VTY_NEWLINE);
		return CMD_WARNING;
	}
...
ALIAS(show_bgp_view_ipv6_safi_rsclient, show_bgp_ipv6_safi_rsclient_cmd, "show bgp ipv6 (unicast|multicast) rsclient (A.B.C.D|X:X::X:X)",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Information about Route Server Client\n" NEIGHBOR_ADDR_STR)

DEFUN(show_bgp_view_rsclient_route, show_bgp_view_rsclient_route_cmd, "show bgp view WORD rsclient (A.B.C.D|X:X::X:X) X:X::X:X",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Information about Route Server Client\n" NEIGHBOR_ADDR_STR "Network in the BGP routing table to display\n") {
	struct bgp *bgp;
	...
	return bgp_show_route_in_table(vty, bgp, peer->rib[AFI_IP6][SAFI_UNICAST], (argc == 3) ? argv[2] : argv[1], AFI_IP6, SAFI_UNICAST, NULL, 0, BGP_PATH_ALL);
}

DEFUN(show_bgp_view_ipv6_rsclient_route, show_bgp_view_ipv6_rsclient_route_cmd, "show bgp view WORD ipv6 rsclient (A.B.C.D|X:X::X:X) X:X::X:X",
      SHOW_STR BGP_STR "BGP view\n"
		       "BGP view name\n"
		       "IP6_STR"
		       "Information about Route Server Client\n" NEIGHBOR_ADDR_STR "Network in the BGP routing table to display\n") {
...
ALIAS(show_bgp_view_ipv6_rsclient_route, show_bgp_rsclient_route_cmd, "show bgp rsclient (A.B.C.D|X:X::X:X) X:X::X:X",
      SHOW_STR BGP_STR "Information about Route Server Client\n" NEIGHBOR_ADDR_STR "Network in the BGP routing table to display\n")

ALIAS(show_bgp_view_ipv6_rsclient_route, show_bgp_ipv6_rsclient_route_cmd, "show bgp ipv6 rsclient (A.B.C.D|X:X::X:X) X:X::X:X",
      SHOW_STR BGP_STR IP6_STR "Information about Route Server Client\n" NEIGHBOR_ADDR_STR "Network in the BGP routing table to display\n")

DEFUN(show_bgp_view_ipv6_safi_rsclient_route, show_bgp_view_ipv6_safi_rsclient_route_cmd, "show bgp view WORD ipv6 (unicast|multicast) rsclient (A.B.C.D|X:X::X:X) X:X::X:X",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "Address Family modifier\n"
	...
	return bgp_show_route_in_table(vty, bgp, peer->rib[AFI_IP6][safi], (argc == 4) ? argv[3] : argv[2], AFI_IP6, safi, NULL, 0, BGP_PATH_ALL);
}

ALIAS(show_bgp_view_ipv6_safi_rsclient_route, show_bgp_ipv6_safi_rsclient_route_cmd, "show bgp ipv6 (unicast|multicast) rsclient (A.B.C.D|X:X::X:X) X:X::X:X",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Information about Route Server Client\n" NEIGHBOR_ADDR_STR "Network in the BGP routing table to display\n")

DEFUN(show_bgp_view_rsclient_prefix, show_bgp_view_rsclient_prefix_cmd, "show bgp view WORD rsclient (A.B.C.D|X:X::X:X) X:X::X:X/M",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
	...
	return bgp_show_route_in_table(vty, bgp, peer->rib[AFI_IP6][SAFI_UNICAST], (argc == 3) ? argv[2] : argv[1], AFI_IP6, SAFI_UNICAST, NULL, 1, BGP_PATH_ALL);
}

DEFUN(show_bgp_view_ipv6_rsclient_prefix, show_bgp_view_ipv6_rsclient_prefix_cmd, "show bgp view WORD ipv6 rsclient (A.B.C.D|X:X::X:X) X:X::X:X/M",
      SHOW_STR BGP_STR "BGP view\n"
...
ALIAS(show_bgp_view_ipv6_rsclient_prefix, show_bgp_rsclient_prefix_cmd, "show bgp rsclient (A.B.C.D|X:X::X:X) X:X::X:X/M",
      SHOW_STR BGP_STR "Information about Route Server Client\n" NEIGHBOR_ADDR_STR "IPv6 prefix <network>/<length>, e.g., 3ffe::/16\n")

ALIAS(show_bgp_view_ipv6_rsclient_prefix, show_bgp_ipv6_rsclient_prefix_cmd, "show bgp ipv6 rsclient (A.B.C.D|X:X::X:X) X:X::X:X/M",
      SHOW_STR BGP_STR "Information about Route Server Client\n" NEIGHBOR_ADDR_STR "IPv6 prefix <network>/<length>, e.g., 3ffe::/16\n")

DEFUN(show_bgp_view_ipv6_safi_rsclient_prefix, show_bgp_view_ipv6_safi_rsclient_prefix_cmd, "show bgp view WORD ipv6 (unicast|multicast) rsclient (A.B.C.D|X:X::X:X) X:X::X:X/M",
      SHOW_STR BGP_STR "BGP view\n"
		       "View name\n"
		       "Address family\n"
		       "Address Family modifier\n"
	...
	return bgp_show_route_in_table(vty, bgp, peer->rib[AFI_IP6][safi], (argc == 4) ? argv[3] : argv[2], AFI_IP6, safi, NULL, 1, BGP_PATH_ALL);
}

ALIAS(show_bgp_view_ipv6_safi_rsclient_prefix, show_bgp_ipv6_safi_rsclient_prefix_cmd, "show bgp ipv6 (unicast|multicast) rsclient (A.B.C.D|X:X::X:X) X:X::X:X/M",
      SHOW_STR BGP_STR "Address family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Information about Route Server Client\n" NEIGHBOR_ADDR_STR "IP prefix <network>/<length>, e.g., 3ffe::/16\n")
...
static int bgp_distance_unset(struct vty *vty, const char *distance_str, const char *ip_str, const char *access_list_str) {
	int ret;
	struct prefix p;
	u_char distance;
	struct bgp_node *rn;
...
u_char ipv6_bgp_distance_apply(struct prefix *p, struct bgp_info *rinfo, struct bgp *bgp) {
	struct bgp_node *rn;
...
DEFUN(bgp_distance, bgp_distance_cmd, "distance bgp <1-255> <1-255> <1-255>",
...
DEFUN(no_bgp_distance, no_bgp_distance_cmd, "no distance bgp <1-255> <1-255> <1-255>",
...
ALIAS(no_bgp_distance, no_bgp_distance2_cmd, "no distance bgp",
      NO_STR "Define an administrative distance\n"
	     "BGP distance\n")

DEFUN(bgp_distance_source, bgp_distance_source_cmd, "distance <1-255> A.B.C.D/M",
      "Define an administrative distance\n"
...
DEFUN(no_bgp_distance_source, no_bgp_distance_source_cmd, "no distance <1-255> A.B.C.D/M",
      NO_STR "Define an administrative distance\n"
	     "Administrative distance\n"
	     "IP source prefix\n") {
...
DEFUN(bgp_distance_source_access_list, bgp_distance_source_access_list_cmd, "distance <1-255> A.B.C.D/M WORD",
      "Define an administrative distance\n"
      "Administrative distance\n"
      "IP source prefix\n"
      "Access list name\n") {
...
DEFUN(no_bgp_distance_source_access_list, no_bgp_distance_source_access_list_cmd, "no distance <1-255> A.B.C.D/M WORD",
      NO_STR "Define an administrative distance\n"
	     "Administrative distance\n"
	     "IP source prefix\n"
	     "Access list name\n") {
...
#ifdef HAVE_IPV6
DEFUN(ipv6_bgp_distance, ipv6_bgp_distance_cmd, "distance bgp <1-255> <1-255> <1-255>",
      "Define an administrative distance\n"
      "BGP distance\n"
      "Distance for routes external to the AS\n"
      "Distance for routes internal to the AS\n"
...
DEFUN(no_ipv6_bgp_distance, no_ipv6_bgp_distance_cmd, "no distance bgp <1-255> <1-255> <1-255>",
      NO_STR "Define an administrative distance\n"
	     "BGP distance\n"
	     "Distance for routes external to the AS\n"
	     "Distance for routes internal to the AS\n"
...
ALIAS(no_ipv6_bgp_distance, no_ipv6_bgp_distance2_cmd, "no distance bgp",
      NO_STR "Define an administrative distance\n"
	     "BGP distance\n")

DEFUN(ipv6_bgp_distance_source, ipv6_bgp_distance_source_cmd, "distance <1-255> X:X::X:X/M",
      "Define an administrative distance\n"
      "Administrative distance\n"
      "IP source prefix\n") {
	bgp_distance_set(vty, argv[0], argv[1], NULL);
...
DEFUN(no_ipv6_bgp_distance_source, no_ipv6_bgp_distance_source_cmd, "no distance <1-255> X:X::X:X/M",
      NO_STR "Define an administrative distance\n"
	     "Administrative distance\n"
	     "IP source prefix\n") {
	bgp_distance_unset(vty, argv[0], argv[1], NULL);
...
DEFUN(ipv6_bgp_distance_source_access_list, ipv6_bgp_distance_source_access_list_cmd, "distance <1-255> X:X::X:X/M WORD",
      "Define an administrative distance\n"
      "Administrative distance\n"
      "IP source prefix\n"
      "Access list name\n") {
...
DEFUN(no_ipv6_bgp_distance_source_access_list, no_ipv6_bgp_distance_source_access_list_cmd, "no distance <1-255> X:X::X:X/M WORD",
      NO_STR "Define an administrative distance\n"
	     "Administrative distance\n"
	     "IP source prefix\n"
	     "Access list name\n") {
...
DEFUN(bgp_damp_set, bgp_damp_set_cmd, "bgp dampening <1-45> <1-20000> <1-20000> <1-255>",
...
ALIAS(bgp_damp_set, bgp_damp_set2_cmd, "bgp dampening <1-45>",
      "BGP Specific commands\n"
      "Enable route-flap dampening\n"
      "Half-life time for the penalty\n")

ALIAS(bgp_damp_set, bgp_damp_set3_cmd, "bgp dampening",
      "BGP Specific commands\n"
      "Enable route-flap dampening\n")

DEFUN(bgp_damp_unset, bgp_damp_unset_cmd, "no bgp dampening",
...
ALIAS(bgp_damp_unset, bgp_damp_unset2_cmd, "no bgp dampening <1-45> <1-20000> <1-20000> <1-255>",
      NO_STR "BGP Specific commands\n"
	     "Enable route-flap dampening\n"
	     "Half-life time for the penalty\n"
	     "Value to start reusing a route\n"
...
DEFUN(show_ip_bgp_dampened_paths, show_ip_bgp_dampened_paths_cmd, "show ip bgp dampened-paths", SHOW_STR IP_STR BGP_STR "Display paths suppressed due to dampening\n") {
	return bgp_show(vty, NULL, AFI_IP, SAFI_UNICAST, bgp_show_type_dampend_paths, NULL);
}

ALIAS(show_ip_bgp_dampened_paths, show_ip_bgp_damp_dampened_paths_cmd, "show ip bgp dampening dampened-paths",
      SHOW_STR IP_STR BGP_STR "Display detailed information about dampening\n"
			      "Display paths suppressed due to dampening\n")
...
DEFUN(show_ip_bgp_flap_statistics, show_ip_bgp_flap_statistics_cmd, "show ip bgp flap-statistics", SHOW_STR IP_STR BGP_STR "Display flap statistics of routes\n") {
	return bgp_show(vty, NULL, AFI_IP, SAFI_UNICAST, bgp_show_type_flap_statistics, NULL);
}

ALIAS(show_ip_bgp_flap_statistics, show_ip_bgp_damp_flap_statistics_cmd, "show ip bgp dampening flap-statistics",
      SHOW_STR IP_STR BGP_STR "Display detailed information about dampening\n"
			      "Display flap statistics of routes\n")

DEFUN(show_bgp_ipv4_safi_dampened_paths, show_bgp_ipv4_safi_dampened_paths_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) dampened-paths",
      SHOW_STR BGP_STR IP_STR "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
...
ALIAS(show_bgp_ipv4_safi_dampened_paths, show_bgp_ipv4_safi_damp_dampened_paths_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) dampening dampened-paths",
      SHOW_STR BGP_STR IP_STR "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
			      "Address Family modifier\n"
...
DEFUN(show_bgp_ipv6_safi_dampened_paths, show_bgp_ipv6_safi_dampened_paths_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) dampened-paths",
      SHOW_STR BGP_STR IPV6_STR "Address Family modifier\n"
				"Address Family modifier\n"
				"Address Family modifier\n"
				"Address Family modifier\n"
...
ALIAS(show_bgp_ipv6_safi_dampened_paths, show_bgp_ipv6_safi_damp_dampened_paths_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) dampening dampened-paths",
      SHOW_STR BGP_STR IPV6_STR "Address Family modifier\n"
				"Address Family modifier\n"
				"Address Family modifier\n"
				"Address Family modifier\n"
...
DEFUN(show_bgp_ipv4_safi_flap_statistics, show_bgp_ipv4_safi_flap_statistics_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) flap-statistics",
      SHOW_STR BGP_STR "Address Family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
...
ALIAS(show_bgp_ipv4_safi_flap_statistics, show_bgp_ipv4_safi_damp_flap_statistics_cmd, "show bgp ipv4 (encap|multicast|unicast|vpn) dampening flap-statistics",
      SHOW_STR BGP_STR "Address Family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
...
DEFUN(show_bgp_ipv6_safi_flap_statistics, show_bgp_ipv6_safi_flap_statistics_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) flap-statistics",
      SHOW_STR BGP_STR "Address Family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
...
ALIAS(show_bgp_ipv6_safi_flap_statistics, show_bgp_ipv6_safi_damp_flap_statistics_cmd, "show bgp ipv6 (encap|multicast|unicast|vpn) dampening flap-statistics",
      SHOW_STR BGP_STR "Address Family\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
		       "Address Family modifier\n"
...
/* Display specified route of BGP table. */
static int bgp_clear_damp_route(struct vty *vty, const char *view_name, const char *ip_str, afi_t afi, safi_t safi, struct prefix_rd *prd, int prefix_check) {
	int ret;
	struct prefix match;
	struct bgp_node *rn;
	struct bgp_node *rm;
...
DEFUN(clear_ip_bgp_dampening, clear_ip_bgp_dampening_cmd, "clear ip bgp dampening", CLEAR_STR IP_STR BGP_STR "Clear route flap dampening information\n") {
	bgp_damp_info_clean();
	return CMD_SUCCESS;
}

DEFUN(clear_ip_bgp_dampening_prefix, clear_ip_bgp_dampening_prefix_cmd, "clear ip bgp dampening A.B.C.D/M",
      CLEAR_STR IP_STR BGP_STR "Clear route flap dampening information\n"
			       "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n") {
	return bgp_clear_damp_route(vty, NULL, argv[0], AFI_IP, SAFI_UNICAST, NULL, 1);
}

DEFUN(clear_ip_bgp_dampening_address, clear_ip_bgp_dampening_address_cmd, "clear ip bgp dampening A.B.C.D",
      CLEAR_STR IP_STR BGP_STR "Clear route flap dampening information\n"
			       "Network to clear damping information\n") {
	return bgp_clear_damp_route(vty, NULL, argv[0], AFI_IP, SAFI_UNICAST, NULL, 0);
}

DEFUN(clear_ip_bgp_dampening_address_mask, clear_ip_bgp_dampening_address_mask_cmd, "clear ip bgp dampening A.B.C.D A.B.C.D",
      CLEAR_STR IP_STR BGP_STR "Clear route flap dampening information\n"
			       "Network to clear damping information\n"
			       "Network mask\n") {
	int ret;
	...
	return bgp_clear_damp_route(vty, NULL, prefix_str, AFI_IP, SAFI_UNICAST, NULL, 0);
}

/* also used for encap safi */
static int bgp_config_write_network_vpnv4(struct vty *vty, struct bgp *bgp, afi_t afi, safi_t safi, int *write) {
	struct bgp_node *prn;
	struct bgp_node *rn;
	struct bgp_table *table;
	struct prefix *p;
		...
		if((table = prn->info) != NULL) {
			for(rn = bgp_table_top(table); rn; rn = bgp_route_next(rn)) {
				if((bgp_static = rn->info) != NULL) {
					...
					vty_out(vty, " network %s/%d rd %s tag %d", inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN), p->prefixlen, rdbuf, label);
					vty_out(vty, "%s", VTY_NEWLINE);
				}
			}
		}
	...
	for(rn = bgp_table_top(bgp->route[afi][safi]); rn; rn = bgp_route_next(rn)) {
		if((bgp_static = rn->info) != NULL) {
			...
			if(bgp_option_check(BGP_OPT_CONFIG_CISCO) && afi == AFI_IP) {
				...
				vty_out(vty, " network %s", inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN));
				...
				vty_out(vty, " network %s/%d", inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN), p->prefixlen);
			}
	...
	for(rn = bgp_table_top(bgp->aggregate[afi][safi]); rn; rn = bgp_route_next(rn)) {
		if((bgp_aggregate = rn->info) != NULL) {
			...
			if(bgp_option_check(BGP_OPT_CONFIG_CISCO) && afi == AFI_IP) {
				...
				masklen2ip(p->prefixlen, &netmask);
				vty_out(vty, " aggregate-address %s %s", inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN), inet_ntoa(netmask));
			} else {
				vty_out(vty, " aggregate-address %s/%d", inet_ntop(p->family, &p->u.prefix, buf, SU_ADDRSTRLEN), p->prefixlen);
			}
...
int bgp_config_write_distance(struct vty *vty, struct bgp *bgp, afi_t afi, safi_t safi, int *write) {
	struct bgp_node *rn;
	struct bgp_distance *bdistance;

	if(afi == AFI_IP && safi == SAFI_UNICAST) {
		/* Distance configuration. */
		if(bgp->distance_ebgp && bgp->distance_ibgp && bgp->distance_local
		   && (bgp->distance_ebgp != ZEBRA_EBGP_DISTANCE_DEFAULT || bgp->distance_ibgp != ZEBRA_IBGP_DISTANCE_DEFAULT || bgp->distance_local != ZEBRA_IBGP_DISTANCE_DEFAULT)) {
			vty_out(vty, " distance bgp %d %d %d%s", bgp->distance_ebgp, bgp->distance_ibgp, bgp->distance_local, VTY_NEWLINE);
		}

		for(rn = bgp_table_top(bgp_distance_table); rn; rn = bgp_route_next(rn)) {
			if((bdistance = rn->info) != NULL) {
				vty_out(vty, " distance %d %s/%d %s%s", bdistance->distance, inet_ntoa(rn->p.u.prefix4), rn->p.prefixlen, bdistance->access_list ? bdistance->access_list : "", VTY_NEWLINE);
			}
		}
	}
#ifdef HAVE_IPV6
		...
		bgp_config_write_family_header(vty, afi, safi, write);
		if(bgp->ipv6_distance_ebgp && bgp->ipv6_distance_ibgp && bgp->ipv6_distance_local
		   && (bgp->ipv6_distance_ebgp != ZEBRA_EBGP_DISTANCE_DEFAULT || bgp->ipv6_distance_ibgp != ZEBRA_IBGP_DISTANCE_DEFAULT || bgp->ipv6_distance_local != ZEBRA_IBGP_DISTANCE_DEFAULT)) {
			vty_out(vty, " distance bgp %d %d %d%s", bgp->ipv6_distance_ebgp, bgp->ipv6_distance_ibgp, bgp->ipv6_distance_local, VTY_NEWLINE);
		}

		for(rn = bgp_table_top(bgp_distance_table); rn; rn = bgp_route_next(rn)) {
			if((bdistance = rn->info) != NULL) {
				vty_out(vty, " distance %d %s/%d %s%s", bdistance->distance, inet6_ntoa(rn->p.u.prefix6), rn->p.prefixlen, bdistance->access_list ? bdistance->access_list : "", VTY_NEWLINE);
			}
		}
	}
#endif /* HAVE_IPV6 */
