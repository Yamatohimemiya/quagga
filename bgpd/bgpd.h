...
#define BGP_IS_VALID_STATE_FOR_NOTIF(S) (((S) == OpenSent) || ((S) == OpenConfirm) || ((S) == Established))
...
extern int bgp_nexthop_set(union sockunion *, union sockunion *, struct bgp_nexthop *, struct peer *);
...
extern struct peer *peer_lookup_with_open(union sockunion *, as_t, struct in_addr *, int *);
...
extern int peer_group_bind(struct bgp *, union sockunion *, struct peer_group *, afi_t, safi_t, as_t *);
extern int peer_group_unbind(struct bgp *, struct peer *, struct peer_group *, afi_t, safi_t);
