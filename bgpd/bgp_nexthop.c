...
int bgp_nexthop_onlink(afi_t afi, struct attr *attr) {
	struct bgp_node *rn;
	...
	if(afi == AFI_IP) {
		...
		if(attr->extra->mp_nexthop_len == 32) {
			...
			rn = bgp_node_match_ipv6(bgp_connected_table[AFI_IP6], &attr->extra->mp_nexthop_global);
			if(rn) {
				bgp_unlock_node(rn);
				return 1;
			}
...
void bgp_address_init(void) {
	bgp_address_hash = hash_create(bgp_address_hash_key_make, bgp_address_hash_cmp);
}

void bgp_address_destroy(void) {
...
static void bgp_address_add(struct prefix *p) {
	struct bgp_addr tmp;
	struct bgp_addr *addr;
...
void bgp_connected_add(struct connected *ifc) {
	struct prefix p;
	struct prefix *addr;
...
void bgp_connected_delete(struct connected *ifc) {
	struct prefix p;
	struct prefix *addr;
	struct interface *ifp;
	struct bgp_node *rn;
...
int bgp_nexthop_self(struct attr *attr) {
...
static int show_ip_bgp_nexthop_table(struct vty *vty, int detail) {
	struct bgp_node *rn;
	struct bgp_nexthop_cache *bnc;
		...
		for(rn = bgp_table_top(bgp_nexthop_cache_table[afi]); rn; rn = bgp_route_next(rn)) {
			if((bnc = rn->info) != NULL) {
				if(CHECK_FLAG(bnc->flags, BGP_NEXTHOP_VALID)) {
					vty_out(vty, " %s valid [IGP metric %d], #paths %d%s", inet_ntop(rn->p.family, &rn->p.u.prefix, buf, sizeof(buf)), bnc->metric, bnc->path_count, VTY_NEWLINE);
					if(detail) {
						for(nexthop = bnc->nexthop; nexthop; nexthop = nexthop->next) {
							switch(nexthop->type) {
								case NEXTHOP_TYPE_IPV6: vty_out(vty, "  gate %s%s", inet_ntop(AF_INET6, &nexthop->gate.ipv6, buf, INET6_ADDRSTRLEN), VTY_NEWLINE); break;
								case NEXTHOP_TYPE_IPV6_IFINDEX:
									vty_out(vty, "  gate %s, if %s%s", inet_ntop(AF_INET6, &nexthop->gate.ipv6, buf, INET6_ADDRSTRLEN), ifindex2ifname(nexthop->ifindex), VTY_NEWLINE);
									break;
								case NEXTHOP_TYPE_IPV4: vty_out(vty, "  gate %s%s", inet_ntop(AF_INET, &nexthop->gate.ipv4, buf, INET6_ADDRSTRLEN), VTY_NEWLINE); break;
								case NEXTHOP_TYPE_IFINDEX: vty_out(vty, "  if %s%s", ifindex2ifname(nexthop->ifindex), VTY_NEWLINE); break;
								case NEXTHOP_TYPE_IPV4_IFINDEX:
									vty_out(vty, "  gate %s, if %s%s", inet_ntop(AF_INET, &nexthop->gate.ipv4, buf, INET6_ADDRSTRLEN), ifindex2ifname(nexthop->ifindex), VTY_NEWLINE);
									break;
								default: vty_out(vty, "  invalid nexthop type %u%s", nexthop->type, VTY_NEWLINE);
							}
						}
					}
				} else {
					vty_out(vty, " %s invalid%s", inet_ntop(AF_INET, &rn->p.u.prefix, buf, sizeof(buf)), VTY_NEWLINE);
				}
...
DEFUN(show_ip_bgp_nexthop, show_ip_bgp_nexthop_cmd, "show ip bgp nexthop", SHOW_STR IP_STR BGP_STR "BGP nexthop table\n") {
	return show_ip_bgp_nexthop_table(vty, 0);
}

DEFUN(show_ip_bgp_nexthop_detail, show_ip_bgp_nexthop_detail_cmd, "show ip bgp nexthop detail", SHOW_STR IP_STR BGP_STR "BGP nexthop table\n") {
