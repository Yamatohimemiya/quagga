...
static struct community *community_new(void) {
	return (struct community *) XCALLOC(MTYPE_COMMUNITY, sizeof(struct community));
}
...
static char *community_com2str(struct community *com) {
	int i;
	char *str;
	char *pnt;
	int len;
...
struct community *community_merge(struct community *com1, struct community *com2) {
	if(com1->val) {
		com1->val = XREALLOC(MTYPE_COMMUNITY_VAL, com1->val, (com1->size + com2->size) * 4);
	} else {
...
static const char *community_gettoken(const char *buf, enum community_token *token, u_int32_t *val) {
	const char *p = buf;
...
void community_init(void) {
	comhash = hash_create((unsigned int (*)(void *)) community_hash_make, (int (*)(const void *, const void *)) community_cmp);
}
