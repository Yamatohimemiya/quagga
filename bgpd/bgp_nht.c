...
static void path_nh_map(struct bgp_info *path, struct bgp_nexthop_cache *bnc, int keep);

int bgp_nexthop_check(struct bgp_info *path, int connected) {
	...
	if(BGP_DEBUG(nht, NHT)) {
		...
		zlog_debug("%s: NHT checking %s", __FUNCTION__, bnc_str(bnc, buf, INET6_ADDRSTRLEN));
	}
	...
	return (bgp_zebra_num_connects() == 0 || CHECK_FLAG(bnc->flags, BGP_NEXTHOP_VALID));
}
 ...
 */
static struct bgp_node *bgp_get_nexthop_rn(struct bgp_info *path, struct peer *peer) {
	struct prefix p;
	afi_t afi;
...
static struct bgp_nexthop_cache *bgp_find_nexthop(struct bgp_info *path, struct peer *peer) {
...
static void bgp_unlink_nexthop_check(struct bgp_nexthop_cache *bnc) {
	if(LIST_EMPTY(&(bnc->paths)) && !bnc->nht_info) {
		if(BGP_DEBUG(nht, NHT)) {
			char buf[INET6_ADDRSTRLEN];
			zlog_debug("bgp_unlink_nexthop: freeing bnc %s", bnc_str(bnc, buf, INET6_ADDRSTRLEN));
		}
...
void bgp_unlink_nexthop(struct bgp_info *path) {
	...
	if(BGP_DEBUG(nht, NHT)) {
		char buf[INET6_ADDRSTRLEN];
		zlog_debug("%s: NHT unlinking %s", __FUNCTION__, bnc_str(bnc, buf, INET6_ADDRSTRLEN));
	}
...
void bgp_unlink_nexthop_by_peer(struct peer *peer) {
	...
	if(BGP_DEBUG(nht, NHT)) {
		zlog_debug("%s: NHT unlinking %s", __FUNCTION__, peer->host);
	}
...
int bgp_ensure_nexthop(struct bgp_info *ri, struct peer *peer, int connected) {
	struct bgp_node *rn;
	...
	if(!rn) {
		zlog_debug("%s: NHT could not ensure, failed to get rn!", __FUNCTION__);
		return 0;
	}
	...
	if(BGP_DEBUG(nht, NHT)) {
		char buf[INET6_ADDRSTRLEN];
		zlog_debug("%s: NHT ensured %s", __FUNCTION__, bnc_str(bnc, buf, INET6_ADDRSTRLEN));
	}
...
void bgp_parse_nexthop_update(void) {
	...
	switch(p.family) {
		case AF_INET: p.u.prefix4.s_addr = stream_get_ipv4(s); break;
		case AF_INET6: stream_get(&p.u.prefix6, s, 16); break;
		default: break;
	}
	...
	if(BGP_DEBUG(nht, NHT)) {
		char buf[INET6_ADDRSTRLEN];
		prefix2str(&p, buf, INET6_ADDRSTRLEN);
		zlog_debug("parse nexthop update(%s): metric=%d, #nexthop=%d", buf, metric, nexthop_num);
	}
	...
	if(nexthop_num) {
		...
		for(i = 0; i < nexthop_num; i++) {
			...
			switch(nexthop->type) {
				case ZEBRA_NEXTHOP_IPV4: nexthop->gate.ipv4.s_addr = stream_get_ipv4(s); break;
				case ZEBRA_NEXTHOP_IFINDEX:
				case ZEBRA_NEXTHOP_IFNAME: nexthop->ifindex = stream_getl(s); break;
				...
				case ZEBRA_NEXTHOP_IPV4_IFNAME:
					...
					break;
#ifdef HAVE_IPV6
				case ZEBRA_NEXTHOP_IPV6: stream_get(&nexthop->gate.ipv6, s, 16); break;
...
#ifdef HAVE_IPV6
		case AFI_IP6:
			if(ri->attr->extra->mp_nexthop_len == 16 && IN6_IS_ADDR_LINKLOCAL(&ri->attr->extra->mp_nexthop_global)) {
				return -1;
			}
 ...
 *   void.
 */
static void sendmsg_nexthop(struct bgp_nexthop_cache *bnc, int command) {
	struct stream *s;
	struct prefix *p;
	int ret;
	...
	if(!zclient || zclient->sock < 0) {
		zlog_debug("%s: Can't send NH register, Zebra client not established", __FUNCTION__);
		return;
	}
	...
	switch(PREFIX_FAMILY(p)) {
		case AF_INET: stream_put_in_addr(s, &p->u.prefix4); break;
		case AF_INET6: stream_put(s, &(p->u.prefix6), 16); break;
		default: break;
	}
...
static void evaluate_paths(struct bgp_nexthop_cache *bnc) {
	...
	LIST_FOREACH(path, &(bnc->paths), nh_thread) {
		if(!(path->type == ZEBRA_ROUTE_BGP && path->sub_type == BGP_ROUTE_NORMAL)) {
			continue;
		}
       ...
       */
		if((CHECK_FLAG(path->flags, BGP_INFO_VALID) ? 1 : 0) != (CHECK_FLAG(bnc->flags, BGP_NEXTHOP_VALID) ? 1 : 0)) {
			if(CHECK_FLAG(path->flags, BGP_INFO_VALID)) {
				bgp_aggregate_decrement(bgp, &rn->p, path, afi, SAFI_UNICAST);
				bgp_info_unset_flag(rn, path, BGP_INFO_VALID);
			} else {
				bgp_info_set_flag(rn, path, BGP_INFO_VALID);
				bgp_aggregate_increment(bgp, &rn->p, path, afi, SAFI_UNICAST);
			}
		}
		...
		if(CHECK_FLAG(bnc->flags, BGP_NEXTHOP_METRIC_CHANGED) || CHECK_FLAG(bnc->flags, BGP_NEXTHOP_CHANGED)) {
 ...
 *          association.
 */
static void path_nh_map(struct bgp_info *path, struct bgp_nexthop_cache *bnc, int make) {
	if(path->nexthop) {
