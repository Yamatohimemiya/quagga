...
#define ASSEGMENT_DATA_SIZE(N, S) ((N) * ((S) ? AS_VALUE_SIZE : AS16_VALUE_SIZE))
...
#define ASSEGMENT_TYPES_PACKABLE(X, Y) (((X)->type == (Y)->type) && ((X)->type == AS_SEQUENCE))
...
static struct assegment *assegment_prepend_asns(struct assegment *seg, as_t asnum, int num) {
	as_t *newas;
	int i;
...
static struct assegment *assegment_append_asns(struct assegment *seg, as_t *asnos, int num) {
	as_t *newas;
...
#define ASN_STR_LEN (10 + 1)
	str_size = MAX(assegment_count_asns(seg, 0) * ASN_STR_LEN + 2 + 1, ASPATH_STR_DEFAULT_LEN);
	...
	while(seg) {
		...
		switch(seg->type) {
			case AS_SET:
			case AS_CONFED_SET: seperator = ','; break;
			case AS_SEQUENCE:
			case AS_CONFED_SEQUENCE: seperator = ' '; break;
			default:
...
#undef SEGMENT_STR_LEN
		...
		if(seg->type != AS_SEQUENCE) {
			len += snprintf(str_buf + len, str_size - len, "%c", aspath_delimiter_char(seg->type, AS_SEG_START));
		}
		...
		if(seg->type != AS_SEQUENCE) {
			len += snprintf(str_buf + len, str_size - len, "%c", aspath_delimiter_char(seg->type, AS_SEG_END));
		}
		if(seg->next) {
...
static int assegments_parse(struct stream *s, size_t length, struct assegment **result, int use32bit) {
	...
	if(BGP_DEBUG(as4, AS4_SEGMENT)) {
		zlog_debug("[AS4SEG] Parse aspath segment: got total byte length %lu", (unsigned long) length);
	}
	/* basic checks */
	if((STREAM_READABLE(s) < length) || (STREAM_READABLE(s) < AS_HEADER_SIZE) || (length % AS16_VALUE_SIZE)) {
		return -1;
	}
	...
	while(bytes < length) {
		...
		if(BGP_DEBUG(as4, AS4_SEGMENT)) {
			zlog_debug("[AS4SEG] Parse aspath segment: Bytes now: %lu", (unsigned long) bytes);
		}
...
static void assegment_data_put(struct stream *s, as_t *as, int num, int use32bit) {
	int i;
...
static struct assegment *aspath_aggregate_as_set_add(struct aspath *aspath, struct assegment *asset, as_t as) {
	int i;
	...
	if(asset == NULL) {
		...
		asset->length++;
		asset->as = XREALLOC(MTYPE_AS_SEG_DATA, asset->as, asset->length * AS_VALUE_SIZE);
...
struct aspath *aspath_aggregate(struct aspath *as1, struct aspath *as2) {
	...
	while(seg1 && seg2) {
		...
		if(match != minlen || match != seg1->length || seg1->length != seg2->length) {
			break;
		}
 ...
 */
struct aspath *aspath_aggregate_mpath(struct aspath *as1, struct aspath *as2) {
	int i;
	int minlen;
	int match;
	int from1, from2;
...
int aspath_firstas_check(struct aspath *aspath, as_t asno) {
	...
	if(aspath->segments && (aspath->segments->type == AS_SEQUENCE) && (aspath->segments->as[0] == asno)) {
		return 1;
	}
 ...
 */
struct aspath *aspath_filter_exclude(struct aspath *source, struct aspath *exclude_list) {
...
static struct aspath *aspath_add_asns(struct aspath *aspath, as_t asno, u_char type, unsigned num) {
...
int aspath_cmp_left(const struct aspath *aspath1, const struct aspath *aspath2) {
	...
	while(seg1 && ((seg1->type == AS_CONFED_SEQUENCE) || (seg1->type == AS_CONFED_SET))) {
		seg1 = seg1->next;
	}

	while(seg2 && ((seg2->type == AS_CONFED_SEQUENCE) || (seg2->type == AS_CONFED_SET))) {
		seg2 = seg2->next;
	}

	/* Check as1's */
	if(!(seg1 && seg2 && (seg1->type == AS_SEQUENCE) && (seg2->type == AS_SEQUENCE))) {
		return 0;
	}
 ...
 */
struct aspath *aspath_reconcile_as4(struct aspath *aspath, struct aspath *as4path) {
	...
	hops = (aspath_count_hops(aspath) + aspath_count_confeds(aspath)) - aspath_count_hops(as4path);
	...
	if(BGP_DEBUG(as4, AS4)) {
		zlog_debug("[AS4] got AS_PATH %s and AS4_PATH %s synthesizing now", aspath->str, as4path->str);
	}
	...
	while(seg && hops > 0) {
	     ...
	     */
			...
			case AS_SEQUENCE: cpasns = MIN(seg->length, hops); hops -= seg->length;
		}
   ...
   */
	...
	if(BGP_DEBUG(as4, AS4)) {
		zlog_debug("[AS4] result of synthesizing is %s", mergedpath->str);
	}
...
/* Compare leftmost AS value for MED check.  If as1's leftmost AS and
   ...
   only).  */
int aspath_cmp_left_confed(const struct aspath *aspath1, const struct aspath *aspath2) {
	if(!(aspath1 && aspath2)) {
		return 0;
	}
	...
	if((aspath1->segments->type != AS_CONFED_SEQUENCE) || (aspath2->segments->type != AS_CONFED_SEQUENCE)) {
		return 0;
	}
...
static const char *aspath_gettoken(const char *buf, enum as_token *token, u_long *asno) {
	const char *p = buf;
...
struct aspath *aspath_str2aspath(const char *str) {
	...
	while((str = aspath_gettoken(str, &token, &asno)) != NULL) {
		switch(token) {
			...
			default: aspath_free(aspath); return NULL;
		}
	}
 ...
 */
void aspath_print_vty(struct vty *vty, const char *format, struct aspath *as, const char *suffix) {
	assert(format);
...
static void aspath_show_all_iterator(struct hash_backet *backet, struct vty *vty) {
	struct aspath *as;
...
void aspath_print_all_vty(struct vty *vty) {
	hash_iterate(ashash, (void (*)(struct hash_backet *, void *)) aspath_show_all_iterator, vty);
}
