...
static struct ecommunity *ecommunity_new(void) {
	return (struct ecommunity *) XCALLOC(MTYPE_ECOMMUNITY, sizeof(struct ecommunity));
}
...
static int ecommunity_add_val(struct ecommunity *ecom, struct ecommunity_val *eval) {
	u_int8_t *p;
	int ret;
	int c;
	...
	memmove(ecom->val + (c + 1) * ECOMMUNITY_SIZE, ecom->val + c * ECOMMUNITY_SIZE, (ecom->size - 1 - c) * ECOMMUNITY_SIZE);
...
struct ecommunity *ecommunity_merge(struct ecommunity *ecom1, struct ecommunity *ecom2) {
	if(ecom1->val) {
		ecom1->val = XREALLOC(MTYPE_ECOMMUNITY_VAL, ecom1->val, (ecom1->size + ecom2->size) * ECOMMUNITY_SIZE);
	} else {
		ecom1->val = XMALLOC(MTYPE_ECOMMUNITY_VAL, (ecom1->size + ecom2->size) * ECOMMUNITY_SIZE);
	}

	memcpy(ecom1->val + (ecom1->size * ECOMMUNITY_SIZE), ecom2->val, ecom2->size * ECOMMUNITY_SIZE);
...
int ecommunity_cmp(const void *arg1, const void *arg2) {
	...
	return (ecom1->size == ecom2->size && memcmp(ecom1->val, ecom2->val, ecom1->size * ECOMMUNITY_SIZE) == 0);
}
...
static const char *ecommunity_gettoken(const char *str, struct ecommunity_val *eval, enum ecommunity_token *token) {
	int ret;
	int dot = 0;
	int digit = 0;
	int separator = 0;
...
#define ECOMMUNITY_STR_DEFAULT_LEN 27
	...
	for(i = 0; i < ecom->size; i++) {
		...
		switch(encode) {
			...
			case ECOMMUNITY_ENCODE_AS4: break;
		...
		switch(format) {
			case ECOMMUNITY_FORMAT_COMMUNITY_LIST: prefix = (type == ECOMMUNITY_ROUTE_TARGET ? "rt " : "soo "); break;
			case ECOMMUNITY_FORMAT_DISPLAY: prefix = (type == ECOMMUNITY_ROUTE_TARGET ? "RT:" : "SoO:"); break;
			case ECOMMUNITY_FORMAT_ROUTE_MAP: prefix = ""; break;
		...
		if(encode == ECOMMUNITY_ENCODE_AS) {
			...
			len = sprintf(str_buf + str_pnt, "%s%s:%u", prefix, inet_ntoa(eip.ip), eip.val);
			str_pnt += len;
			first = 0;
		}
	}
...
int ecommunity_match(const struct ecommunity *ecom1, const struct ecommunity *ecom2) {
	int i = 0;
	int j = 0;
