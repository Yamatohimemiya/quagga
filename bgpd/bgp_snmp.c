...
/* Hook functions. */
static u_char *bgpVersion(struct variable *, oid[], size_t *, int, size_t *, WriteMethod **);
static u_char *bgpLocalAs(struct variable *, oid[], size_t *, int, size_t *, WriteMethod **);
static u_char *bgpPeerTable(struct variable *, oid[], size_t *, int, size_t *, WriteMethod **);
static u_char *bgpRcvdPathAttrTable(struct variable *, oid[], size_t *, int, size_t *, WriteMethod **);
static u_char *bgpIdentifier(struct variable *, oid[], size_t *, int, size_t *, WriteMethod **);
static u_char *bgp4PathAttrTable(struct variable *, oid[], size_t *, int, size_t *, WriteMethod **);
...
struct variable bgp_variables[] = {
 ...
 /* BGP peer table. */
	...
	{ BGPPEERMINROUTEADVERTISEMENTINTERVAL, INTEGER,	 RWRITE, bgpPeerTable,	       3, { 3, 1, 23 }},
...
static u_char *bgpVersion(struct variable *v, oid name[], size_t *length, int exact, size_t *var_len, WriteMethod **write_method) {
...
static u_char *bgpLocalAs(struct variable *v, oid name[], size_t *length, int exact, size_t *var_len, WriteMethod **write_method) {
	struct bgp *bgp;
...
static struct peer *bgpPeerTable_lookup(struct variable *v, oid name[], size_t *length, struct in_addr *addr, int exact) {
...
/* BGP write methods. */
static int write_bgpPeerTable(int action, u_char *var_val, u_char var_val_type, size_t var_val_len, u_char *statP, oid *name, size_t length) {
	struct in_addr addr;
	struct peer *peer;
	long intval;
	...
	zlog_info("%s: SNMP write .%ld = %ld", peer->host, (long) name[PEERTAB_NAMELEN - 1], intval);
	...
	#define BGP_PeerAdmin_start 2
		...
		case BGPPEERKEEPALIVECONFIGURED:
			...
			break;
		case BGPPEERMINROUTEADVERTISEMENTINTERVAL: peer->v_routeadv = intval; break;
	}
	return SNMP_ERR_NOERROR;
}

static u_char *bgpPeerTable(struct variable *v, oid name[], size_t *length, int exact, size_t *var_len, WriteMethod **write_method) {
	...
	switch(v->magic) {
		case BGPPEERIDENTIFIER: return SNMP_IPADDRESS(peer->remote_id); break;
		case BGPPEERSTATE: return SNMP_INTEGER(peer->status); break;
		case BGPPEERADMINSTATUS: *write_method = write_bgpPeerTable;
	#define BGP_PeerAdmin_stop 1
	#define BGP_PeerAdmin_start 2
			...
			break;
		case BGPPEERNEGOTIATEDVERSION: return SNMP_INTEGER(BGP_VERSION_4); break;
		...
		case BGPPEERREMOTEPORT:
			...
			break;
		case BGPPEERREMOTEAS: return SNMP_INTEGER(peer->as); break;
		case BGPPEERINUPDATES: return SNMP_INTEGER(peer->update_in); break;
		case BGPPEEROUTUPDATES: return SNMP_INTEGER(peer->update_out); break;
		case BGPPEERINTOTALMESSAGES: return SNMP_INTEGER(peer->open_in + peer->update_in + peer->keepalive_in + peer->notify_in + peer->refresh_in + peer->dynamic_cap_in); break;
		case BGPPEEROUTTOTALMESSAGES: return SNMP_INTEGER(peer->open_out + peer->update_out + peer->keepalive_out + peer->notify_out + peer->refresh_out + peer->dynamic_cap_out); break;
		case BGPPEERLASTERROR:
			...
			break;
		case BGPPEERFSMESTABLISHEDTRANSITIONS: return SNMP_INTEGER(peer->established); break;
		...
		case BGPPEERCONNECTRETRYINTERVAL:
			*write_method = write_bgpPeerTable;
			...
			break;
		case BGPPEERHOLDTIME: return SNMP_INTEGER(peer->v_holdtime); break;
		case BGPPEERKEEPALIVE: return SNMP_INTEGER(peer->v_keepalive); break;
		case BGPPEERHOLDTIMECONFIGURED:
			*write_method = write_bgpPeerTable;
...
static u_char *bgpIdentifier(struct variable *v, oid name[], size_t *length, int exact, size_t *var_len, WriteMethod **write_method) {
	struct bgp *bgp;
...
static u_char *bgpRcvdPathAttrTable(struct variable *v, oid name[], size_t *length, int exact, size_t *var_len, WriteMethod **write_method) {
...
static struct bgp_info *bgp4PathAttrLookup(struct variable *v, oid name[], size_t *length, struct bgp *bgp, struct prefix_ipv4 *addr, int exact) {
	oid *offset;
	int offsetlen;
	...
	if(exact) {
		...
		/* Lookup node. */
		rn = bgp_node_lookup(bgp->rib[AFI_IP][SAFI_UNICAST], (struct prefix *) addr);
		if(rn) {
			bgp_unlock_node(rn);
		...
		if(offsetlen == 0) {
			...
			rn = bgp_node_get(bgp->rib[AFI_IP][SAFI_UNICAST], (struct prefix *) addr);
		...
		do {
			...
			for(binfo = rn->info; binfo; binfo = binfo->next) {
				if(binfo->peer->su.sin.sin_family == AF_INET && ntohl(paddr.s_addr) < ntohl(binfo->peer->su.sin.sin_addr.s_addr)) {
					if(min) {
						if(ntohl(binfo->peer->su.sin.sin_addr.s_addr) < ntohl(min->peer->su.sin.sin_addr.s_addr)) {
							min = binfo;
						}
					} else {
						min = binfo;
			...
			if(min) {
				...
				*offset = rn->p.prefixlen;
				offset++;
				oid_copy_addr(offset, &min->peer->su.sin.sin_addr, IN_ADDR_SIZE);
...
static u_char *bgp4PathAttrTable(struct variable *v, oid name[], size_t *length, int exact, size_t *var_len, WriteMethod **write_method) {
	struct bgp *bgp;
	struct bgp_info *binfo;
	...
	switch(v->magic) {
		case BGP4PATHATTRPEER: /* 1 */ return SNMP_IPADDRESS(binfo->peer->su.sin.sin_addr); break;
		case BGP4PATHATTRIPADDRPREFIXLEN: /* 2 */ return SNMP_INTEGER(addr.prefixlen); break;
		case BGP4PATHATTRIPADDRPREFIX: /* 3 */ return SNMP_IPADDRESS(addr.prefix); break;
		case BGP4PATHATTRORIGIN: /* 4 */ return SNMP_INTEGER(binfo->attr->origin); break;
		case BGP4PATHATTRASPATHSEGMENT: /* 5 */ return aspath_snmp_pathseg(binfo->attr->aspath, var_len); break;
		case BGP4PATHATTRNEXTHOP: /* 6 */ return SNMP_IPADDRESS(binfo->attr->nexthop); break;
		case BGP4PATHATTRMULTIEXITDISC: /* 7 */ return SNMP_INTEGER(binfo->attr->med); break;
		case BGP4PATHATTRLOCALPREF: /* 8 */ return SNMP_INTEGER(binfo->attr->local_pref); break;
		case BGP4PATHATTRATOMICAGGREGATE: /* 9 */ return SNMP_INTEGER(1); break;
		...
		case BGP4PATHATTRAGGREGATORADDR: /* 11 */
			...
			break;
		case BGP4PATHATTRCALCLOCALPREF: /* 12 */ return SNMP_INTEGER(-1); break;
...
void bgpTrapEstablished(struct peer *peer) {
	int ret;
	struct in_addr addr;
	...
	smux_trap(
		bgp_variables, sizeof bgp_variables / sizeof(struct variable), bgp_trap_oid, sizeof bgp_trap_oid / sizeof(oid), bgp_oid, sizeof bgp_oid / sizeof(oid), index, IN_ADDR_SIZE, bgpTrapList,
		sizeof bgpTrapList / sizeof(struct trap_object), BGPESTABLISHED
	);
}

void bgpTrapBackwardTransition(struct peer *peer) {
	int ret;
	struct in_addr addr;
	oid index[sizeof(oid) * IN_ADDR_SIZE];
	...
	smux_trap(
		bgp_variables, sizeof bgp_variables / sizeof(struct variable), bgp_trap_oid, sizeof bgp_trap_oid / sizeof(oid), bgp_oid, sizeof bgp_oid / sizeof(oid), index, IN_ADDR_SIZE, bgpTrapList,
		sizeof bgpTrapList / sizeof(struct trap_object), BGPBACKWARDTRANSITION
	);
}
