...
static int bgp_start_jitter(int time) {
	return ((random() % (time + 1)) - (time / 2));
}

/* Check if suppress start/restart of sessions to peer. */
#define BGP_PEER_START_SUPPRESSED(P) (CHECK_FLAG((P)->flags, PEER_FLAG_SHUTDOWN) || CHECK_FLAG((P)->sflags, PEER_STATUS_PREFIX_OVERFLOW))
		...
		case OpenConfirm:
         ...
         timer and KeepAlive timers are not started. */
			if(peer->v_holdtime == 0) {
				...
				BGP_TIMER_ON(peer->t_holdtime, bgp_holdtime_timer, peer->v_holdtime);
				BGP_TIMER_ON(peer->t_keepalive, bgp_keepalive_timer, peer->v_keepalive);
			}
			BGP_TIMER_OFF(peer->t_routeadv);
			break;
...
static int bgp_start_timer(struct thread *thread) {
	...
	if(BGP_DEBUG(fsm, FSM)) {
		zlog(peer->log, LOG_DEBUG, "%s [FSM] Timer (start timer expire).", peer->host);
	}
...
static int bgp_connect_timer(struct thread *thread) {
	...
	if(BGP_DEBUG(fsm, FSM)) {
		zlog(peer->log, LOG_DEBUG, "%s [FSM] Timer (connect timer expire)", peer->host);
	}
...
static int bgp_holdtime_timer(struct thread *thread) {
	...
	if(BGP_DEBUG(fsm, FSM)) {
		zlog(peer->log, LOG_DEBUG, "%s [FSM] Timer (holdtime timer expire)", peer->host);
	}
...
static int bgp_keepalive_timer(struct thread *thread) {
	...
	if(BGP_DEBUG(fsm, FSM)) {
		zlog(peer->log, LOG_DEBUG, "%s [FSM] Timer (keepalive timer expire)", peer->host);
	}
...
static int bgp_routeadv_timer(struct thread *thread) {
	...
	if(BGP_DEBUG(fsm, FSM)) {
		zlog(peer->log, LOG_DEBUG, "%s [FSM] Timer (routeadv timer expire)", peer->host);
	}
...
void bgp_fsm_change_status(struct peer *peer, int status) {
   ...
   */
	...
	if(BGP_DEBUG(normal, NORMAL)) {
		zlog_debug("%s went from %s to %s", peer->host, LOOKUP(bgp_status_msg, peer->ostatus), LOOKUP(bgp_status_msg, peer->status));
	}
}
...
int bgp_stop(struct peer *peer) {
	...
	if(peer->status == Established) {
		...
		if(bgp_flag_check(peer->bgp, BGP_FLAG_LOG_NEIGHBOR_CHANGES)) {
			zlog_info("%%ADJCHANGE: neighbor %s Down %s", peer->host, peer_down_str[(int) peer->last_reset]);
		}
		...
		if(CHECK_FLAG(peer->sflags, PEER_STATUS_NSF_WAIT)) {
			if(BGP_DEBUG(events, EVENTS)) {
				zlog_debug("%s graceful restart timer started for %d sec", peer->host, peer->v_gr_restart);
				zlog_debug("%s graceful restart stalepath timer started for %d sec", peer->host, peer->bgp->stalepath_time);
			}
			BGP_TIMER_ON(peer->t_gr_restart, bgp_graceful_restart_timer_expire, peer->v_gr_restart);
			BGP_TIMER_ON(peer->t_gr_stale, bgp_graceful_stale_timer_expire, peer->bgp->stalepath_time);
		} else {
 ...
 * points of x.
 */
__attribute__((unused)) static int back_off_exp2_bias(const int first, int val, const int max) {
...
static int bgp_stop_with_error(struct peer *peer) {
	peer->v_start = back_off_exp2_bias(BGP_INIT_START_TIMER, peer->v_start, 60);
	bgp_stop(peer);
	return 0;
}
...
static int bgp_connect_success(struct peer *peer) {
	...
	if(peer->fd < 0) {
		zlog_err("bgp_connect_success peer's fd is negative value %d", peer->fd);
		return -1;
	}
...
int bgp_start(struct peer *peer) {
   ...
   */
	...
	if((peer_ttl(peer) == 1 || peer->gtsm_hops == 1) && !CHECK_FLAG(peer->flags, PEER_FLAG_DISABLE_CONNECTED_CHECK)) {
		connected = 1;
	}
	...
	switch(status) {
		...
		case connect_success:
			if(BGP_DEBUG(fsm, FSM)) {
				plog_debug(peer->log, "%s [FSM] Connect immediately success", peer->host);
			}
		...
		case connect_in_progress:
         ...
         readable or writable. */
			if(BGP_DEBUG(fsm, FSM)) {
				plog_debug(peer->log, "%s [FSM] Non blocking connect waiting result", peer->host);
			}
			if(peer->fd < 0) {
				zlog_err("bgp_start peer's fd is negative value %d", peer->fd);
				return -1;
			}
			BGP_READ_ON(peer->t_read, bgp_read, peer->fd);
			BGP_WRITE_ON(peer->t_write, bgp_write, peer->fd);
...
static int bgp_fsm_event_error(struct peer *peer) {
	plog_err(peer->log, "%s [FSM] unexpected packet received in state %s", peer->host, LOOKUP(bgp_status_msg, peer->status));
...
static int bgp_establish(struct peer *peer) {
	...
	for(afi = AFI_IP; afi < AFI_MAX; afi++) {
		for(safi = SAFI_UNICAST; safi < SAFI_RESERVED_3; safi++) {
			if(peer->afc_nego[afi][safi] && CHECK_FLAG(peer->cap, PEER_CAP_RESTART_ADV) && CHECK_FLAG(peer->af_cap[afi][safi], PEER_CAP_RESTART_AF_RCV)) {
				if(peer->nsf[afi][safi] && !CHECK_FLAG(peer->af_cap[afi][safi], PEER_CAP_RESTART_AF_PRESERVE_RCV)) {
					bgp_clear_stale_route(peer, afi, safi);
				}
		...
		for(safi = SAFI_UNICAST; safi < SAFI_MAX; safi++) {
			if(CHECK_FLAG(peer->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_SM_ADV)) {
				if(CHECK_FLAG(peer->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_RM_RCV)) {
					bgp_route_refresh_send(peer, afi, safi, ORF_TYPE_PREFIX, REFRESH_IMMEDIATE, 0);
				} else if(CHECK_FLAG(peer->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_RM_OLD_RCV)) {
					bgp_route_refresh_send(peer, afi, safi, ORF_TYPE_PREFIX_OLD, REFRESH_IMMEDIATE, 0);
				}
			}
		}
	}
	...
	for(afi = AFI_IP; afi < AFI_MAX; afi++) {
		for(safi = SAFI_UNICAST; safi < SAFI_MAX; safi++) {
			if(CHECK_FLAG(peer->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_RM_ADV)) {
				if(CHECK_FLAG(peer->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_SM_RCV) || CHECK_FLAG(peer->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_SM_OLD_RCV)) {
...
int bgp_event(struct thread *thread) {
	...
	if(BGP_DEBUG(fsm, FSM) && peer->status != next) {
		plog_debug(peer->log, "%s [FSM] %s (%s->%s)", peer->host, bgp_event_str[event], LOOKUP(bgp_status_msg, peer->status), LOOKUP(bgp_status_msg, next));
	}
