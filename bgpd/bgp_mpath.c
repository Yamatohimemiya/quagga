...
bool bgp_mpath_is_configured_sort(struct bgp *bgp, bgp_peer_sort_t sort, afi_t afi, safi_t safi) {
   ...
   */
	switch(sort) {
		case BGP_PEER_IBGP: return cfg->maxpaths_ibgp != BGP_DEFAULT_MAXPATHS;
		case BGP_PEER_EBGP: return cfg->maxpaths_ebgp != BGP_DEFAULT_MAXPATHS;
		default: return false;
	}
}

bool bgp_mpath_is_configured(struct bgp *bgp, afi_t afi, safi_t safi) {
	return bgp_mpath_is_configured_sort(bgp, BGP_PEER_IBGP, afi, safi) || bgp_mpath_is_configured_sort(bgp, BGP_PEER_EBGP, afi, safi);
}
 ...
 */
int bgp_maximum_paths_set(struct bgp *bgp, afi_t afi, safi_t safi, int peertype, u_int16_t maxpaths) {
	if(!bgp || (afi >= AFI_MAX) || (safi >= SAFI_MAX)) {
		return -1;
	}

	switch(peertype) {
		case BGP_PEER_IBGP: bgp->maxpaths[afi][safi].maxpaths_ibgp = maxpaths; break;
		case BGP_PEER_EBGP: bgp->maxpaths[afi][safi].maxpaths_ebgp = maxpaths; break;
		default: return -1;
	}
 ...
 */
int bgp_maximum_paths_unset(struct bgp *bgp, afi_t afi, safi_t safi, int peertype) {
	if(!bgp || (afi >= AFI_MAX) || (safi >= SAFI_MAX)) {
		return -1;
	}

	switch(peertype) {
		case BGP_PEER_IBGP: bgp->maxpaths[afi][safi].maxpaths_ibgp = BGP_DEFAULT_MAXPATHS; break;
		case BGP_PEER_EBGP: bgp->maxpaths[afi][safi].maxpaths_ebgp = BGP_DEFAULT_MAXPATHS; break;
		default: return -1;
	}
 ...
 * or greater than bi2.
 */
static int bgp_info_nexthop_cmp(struct bgp_info *bi1, struct bgp_info *bi2) {
	...
	if(!compare && ae1 && ae2) {
		if(ae1->mp_nexthop_len == ae2->mp_nexthop_len) {
			switch(ae1->mp_nexthop_len) {
				case 4:
				case 12: compare = IPV4_ADDR_CMP(&ae1->mp_nexthop_global_in, &ae2->mp_nexthop_global_in); break;
				case 16: compare = IPV6_ADDR_CMP(&ae1->mp_nexthop_global, &ae2->mp_nexthop_global); break;
				case 32:
					compare = IPV6_ADDR_CMP(&ae1->mp_nexthop_global, &ae2->mp_nexthop_global);
					if(!compare) {
						compare = IPV6_ADDR_CMP(&ae1->mp_nexthop_local, &ae2->mp_nexthop_local);
					}
					break;
			}
		}
 ...
 * list entry
 */
static void bgp_info_mpath_enqueue(struct bgp_info *prev_info, struct bgp_info *binfo) {
 ...
 * bgp_best_selection
 */
void bgp_info_mpath_update(struct bgp_node *rn, struct bgp_info *new_best, struct bgp_info *old_best, struct list *mp_list, afi_t afi, safi_t safi) {
	...
	if(new_best) {
		...
		maxpaths = (new_best->peer->sort == BGP_PEER_IBGP) ? mpath_cfg->maxpaths_ibgp : mpath_cfg->maxpaths_ebgp;
	}
       ...
       */
		if(mp_node && (listgetdata(mp_node) == cur_mpath)) {
			...
			if((mpath_count < maxpaths) && bgp_info_nexthop_cmp(prev_mpath, cur_mpath)) {
				...
				if(debug) {
					zlog_debug(
						"%s remove mpath nexthop %s peer %s", pfx_buf, inet_ntop(AF_INET, &cur_mpath->attr->nexthop, nh_buf[0], sizeof(nh_buf[0])),
						sockunion2str(cur_mpath->peer->su_remote, nh_buf[1], sizeof(nh_buf[1]))
					);
				}
			}
		...
		if(cur_mpath && (!mp_node || (bgp_info_mpath_cmp(cur_mpath, listgetdata(mp_node)) < 0))) {
           ...
           */
			...
			mpath_changed = 1;
			if(debug) {
				zlog_debug("%s remove mpath nexthop %s peer %s", pfx_buf, inet_ntop(AF_INET, &cur_mpath->attr->nexthop, nh_buf[0], sizeof(nh_buf[0])), sockunion2str(cur_mpath->peer->su_remote, nh_buf[1], sizeof(nh_buf[1])));
			}
			cur_mpath = next_mpath;
		} else {
			/*
           ...
           */
			...
			list_delete_node(mp_list, mp_node);
			if((mpath_count < maxpaths) && (new_mpath != new_best) && bgp_info_nexthop_cmp(prev_mpath, new_mpath)) {
				...
				if(debug) {
					zlog_debug(
						"%s add mpath nexthop %s peer %s", pfx_buf, inet_ntop(AF_INET, &new_mpath->attr->nexthop, nh_buf[0], sizeof(nh_buf[0])), sockunion2str(new_mpath->peer->su_remote, nh_buf[1], sizeof(nh_buf[1]))
					);
				}
			}
			mp_node = mp_next_node;
...
void bgp_info_mpath_aggregate_update(struct bgp_info *new_best, struct bgp_info *old_best) {
	...
	if(old_best && (old_best != new_best) && (old_attr = bgp_info_mpath_attr(old_best))) {
   ...
   */
	if(!CHECK_FLAG(new_best->flags, BGP_INFO_MULTIPATH_CHG) && (old_best == new_best)) {
		attr_chg = 0;

		if(CHECK_FLAG(new_best->flags, BGP_INFO_ATTR_CHANGED)) {
			attr_chg = 1;
		} else {
			for(mpinfo = bgp_info_mpath_first(new_best); mpinfo; mpinfo = bgp_info_mpath_next(mpinfo)) {
