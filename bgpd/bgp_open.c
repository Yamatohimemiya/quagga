	...
	while(pnt < end) {
		...
		if(hdr->code == CAPABILITY_CODE_MP) {
			...
			switch(ntohs(mpc.afi)) {
				case AFI_IP: vty_out(vty, "AFI IPv4, "); break;
				case AFI_IP6: vty_out(vty, "AFI IPv6, "); break;
				default: vty_out(vty, "AFI Unknown %d, ", ntohs(mpc.afi)); break;
			}
			switch(mpc.safi) {
				case SAFI_UNICAST: vty_out(vty, "SAFI Unicast"); break;
				case SAFI_MULTICAST: vty_out(vty, "SAFI Multicast"); break;
				case SAFI_MPLS_LABELED_VPN: vty_out(vty, "SAFI MPLS-labeled VPN"); break;
				case SAFI_ENCAP: vty_out(vty, "SAFI ENCAP"); break;
				default: vty_out(vty, "SAFI Unknown %d ", mpc.safi); break;
			}
			vty_out(vty, "%s", VTY_NEWLINE);
		} else if(hdr->code >= 128) {
			vty_out(vty, "  Capability error: vendor specific capability code %d", hdr->code);
		} else {
			vty_out(vty, "  Capability error: unknown capability code %d", hdr->code);
		}
...
int bgp_afi_safi_valid_indices(afi_t afi, safi_t *safi) {
	switch(afi) {
		case AFI_IP:
		case AFI_IP6:
			switch(*safi) {
					/* BGP MPLS-labeled VPN SAFI isn't contigious with others, remap */
				case SAFI_MPLS_LABELED_VPN: *safi = SAFI_MPLS_VPN;
				case SAFI_UNICAST:
				case SAFI_MULTICAST:
				case SAFI_MPLS_VPN:
				case SAFI_ENCAP: return 1;
			}
		case AFI_ETHER:
		default: break;
	}
...
static int bgp_capability_mp(struct peer *peer, struct capability_header *hdr) {
	...
	if(BGP_DEBUG(normal, NORMAL)) {
		zlog_debug("%s OPEN has MP_EXT CAP for afi/safi: %u/%u", peer->host, mpc.afi, mpc.safi);
	}
...
static void bgp_capability_orf_not_support(struct peer *peer, afi_t afi, safi_t safi, u_char type, u_char mode) {
	if(BGP_DEBUG(normal, NORMAL)) {
		zlog_debug("%s Addr-family %d/%d has ORF type/mode %d/%d not supported", peer->host, afi, safi, type, mode);
	}
}
...
static int bgp_capability_orf_entry(struct peer *peer, struct capability_header *hdr) {
	struct stream *s = BGP_INPUT(peer);
	...
	if(BGP_DEBUG(normal, NORMAL)) {
		zlog_debug("%s ORF Cap entry for afi/safi: %u/%u", peer->host, entry.mpc.afi, entry.mpc.safi);
	}
	...
	for(i = 0; i < entry.num; i++) {
		...
		switch(mode) {
			case ORF_MODE_BOTH:
			case ORF_MODE_SEND:
			case ORF_MODE_RECEIVE: break;
			default: bgp_capability_orf_not_support(peer, afi, safi, type, mode); continue;
		}
		...
		switch(hdr->code) {
			case CAPABILITY_CODE_ORF:
				switch(type) {
					case ORF_TYPE_PREFIX: break;
					default: bgp_capability_orf_not_support(peer, afi, safi, type, mode); continue;
				}
				break;
			case CAPABILITY_CODE_ORF_OLD:
				switch(type) {
					case ORF_TYPE_PREFIX_OLD: break;
					default: bgp_capability_orf_not_support(peer, afi, safi, type, mode); continue;
				}
				break;
			default: bgp_capability_orf_not_support(peer, afi, safi, type, mode); continue;
		...
		/* AFI vs SAFI */
		if(!((afi == AFI_IP && safi == SAFI_UNICAST) || (afi == AFI_IP && safi == SAFI_MULTICAST) || (afi == AFI_IP6 && safi == SAFI_UNICAST))) {
		...
		if(BGP_DEBUG(normal, NORMAL)) {
			zlog_debug(
				...
				peer->host, LOOKUP(orf_type_str, type), LOOKUP(orf_mode_str, mode), entry.mpc.afi, safi
			);
		}
		...
		switch(mode) {
			case ORF_MODE_BOTH:
				...
				break;
			case ORF_MODE_SEND: SET_FLAG(peer->af_cap[afi][safi], sm_cap); break;
			case ORF_MODE_RECEIVE: SET_FLAG(peer->af_cap[afi][safi], rm_cap); break;
		}
	}
	return 0;
}

static int bgp_capability_restart(struct peer *peer, struct capability_header *caphdr) {
	struct stream *s = BGP_INPUT(peer);
	...
	if(BGP_DEBUG(normal, NORMAL)) {
		...
		zlog_debug("%s Peer has%srestarted. Restart Time : %d", peer->host, CHECK_FLAG(peer->cap, PEER_CAP_RESTART_BIT_RCV) ? " " : " not ", peer->v_gr_restart);
	}
	...
	while(stream_get_getp(s) + 4 <= end) {
		...
		if(!bgp_afi_safi_valid_indices(afi, &safi)) {
			...
			if(BGP_DEBUG(normal, NORMAL)) {
				zlog_debug("%s Address family %s is%spreserved", peer->host, afi_safi_print(afi, safi), CHECK_FLAG(peer->af_cap[afi][safi], PEER_CAP_RESTART_AF_PRESERVE_RCV) ? " " : " not ");
			}
...
static as_t bgp_capability_as4(struct peer *peer, struct capability_header *hdr) {
	...
	if(hdr->length != CAPABILITY_CODE_AS4_LEN) {
		zlog_err("%s AS4 capability has incorrect data length %d", peer->host, hdr->length);
		return 0;
	}
	...
	if(BGP_DEBUG(as4, AS4)) {
		zlog_debug("%s [AS4] about to set cap PEER_CAP_AS4_RCV, got as4 %u", peer->host, as4);
	}
	return as4;
}
...
static const size_t cap_modsizes[] = {
	[CAPABILITY_CODE_MP] = 4,  [CAPABILITY_CODE_REFRESH] = 1, [CAPABILITY_CODE_ORF] = 1,	     [CAPABILITY_CODE_RESTART] = 1,
	[CAPABILITY_CODE_AS4] = 4, [CAPABILITY_CODE_DYNAMIC] = 1, [CAPABILITY_CODE_REFRESH_OLD] = 1, [CAPABILITY_CODE_ORF_OLD] = 1,
};
 ...
 */
static int bgp_capability_parse(struct peer *peer, size_t length, int *mp_capability, u_char **error) {
	int ret;
	struct stream *s = BGP_INPUT(peer);
	...
	while(stream_get_getp(s) < end) {
		...
		if(BGP_DEBUG(normal, NORMAL)) {
			zlog_debug("%s OPEN has %s capability (%u), length %u", peer->host, LOOKUP(capcode_str, caphdr.code), caphdr.code, caphdr.length);
		}
			...
			case CAPABILITY_CODE_DYNAMIC:
				...
				if(caphdr.length < cap_minsizes[caphdr.code]) {
					zlog_info(
						...
						" expected at least %u",
						peer->host, LOOKUP(capcode_str, caphdr.code), caphdr.length, (unsigned) cap_minsizes[caphdr.code]
					);
					bgp_notify_send(peer, BGP_NOTIFY_OPEN_ERR, BGP_NOTIFY_OPEN_UNSPECIFIC);
					return -1;
				}
				if(caphdr.length && caphdr.length % cap_modsizes[caphdr.code] != 0) {
					zlog_info(
						"%s %s Capability length error: got %u,"
						" expected a multiple of %u",
						peer->host, LOOKUP(capcode_str, caphdr.code), caphdr.length, (unsigned) cap_modsizes[caphdr.code]
					);
					bgp_notify_send(peer, BGP_NOTIFY_OPEN_ERR, BGP_NOTIFY_OPEN_UNSPECIFIC);
					return -1;
				}
		...
		switch(caphdr.code) {
			...
			case CAPABILITY_CODE_RESTART:
				...
				break;
			case CAPABILITY_CODE_DYNAMIC: SET_FLAG(peer->cap, PEER_CAP_DYNAMIC_RCV); break;
			case CAPABILITY_CODE_AS4:
			...
			default:
				if(caphdr.code > 128) {
                   ...
                   capabilities.  It seems reasonable for now...  */
					zlog_warn("%s Vendor specific capability %d", peer->host, caphdr.code);
				} else {
					zlog_warn("%s unrecognized capability code: %d - ignored", peer->host, caphdr.code);
					memcpy(*error, sp, caphdr.length + 2);
					*error += caphdr.length + 2;
				}
		}
		if(stream_get_getp(s) != (start + caphdr.length)) {
			if(stream_get_getp(s) > (start + caphdr.length)) {
				zlog_warn("%s Cap-parser for %s read past cap-length, %u!", peer->host, LOOKUP(capcode_str, caphdr.code), caphdr.length);
			}
...
static int bgp_auth_parse(struct peer *peer, size_t length) {
	bgp_notify_send(peer, BGP_NOTIFY_OPEN_ERR, BGP_NOTIFY_OPEN_AUTH_FAILURE);
	return -1;
}
...
int bgp_open_option_parse(struct peer *peer, u_char length, int *mp_capability) {
	...
	if(BGP_DEBUG(normal, NORMAL)) {
		zlog_debug("%s rcv OPEN w/ OPTION parameter len: %u", peer->host, length);
	}
	...
	while(stream_get_getp(s) < end) {
		...
		if(BGP_DEBUG(normal, NORMAL)) {
			zlog_debug("%s rcvd OPEN w/ optional parameter type %u (%s) len %u", peer->host, opt_type, opt_type == BGP_OPEN_OPT_AUTH ? "Authentication" : opt_type == BGP_OPEN_OPT_CAP ? "Capability" : "Unknown", opt_length);
		}

		switch(opt_type) {
			case BGP_OPEN_OPT_AUTH: ret = bgp_auth_parse(peer, opt_length); break;
			case BGP_OPEN_OPT_CAP: ret = bgp_capability_parse(peer, opt_length, mp_capability, &error); break;
			default:
				bgp_notify_send(peer, BGP_NOTIFY_OPEN_ERR, BGP_NOTIFY_OPEN_UNSUP_PARAM);
				ret = -1;
				break;
		}
	...
	if(CHECK_FLAG(peer->flags, PEER_FLAG_STRICT_CAP_MATCH)) {
		...
		if(error != error_data) {
			bgp_notify_send_with_data(peer, BGP_NOTIFY_OPEN_ERR, BGP_NOTIFY_OPEN_UNSUP_CAPBL, error_data, error - error_data);
			return -1;
		}
		...
		if(!strict_capability_same(peer)) {
			bgp_notify_send(peer, BGP_NOTIFY_OPEN_ERR, BGP_NOTIFY_OPEN_UNSUP_CAPBL);
			return -1;
		}
	}
...
static void bgp_open_capability_orf(struct stream *s, struct peer *peer, afi_t afi, safi_t safi, u_char code) {
	u_char cap_len;
	u_char orf_len;
	unsigned long capp;
	unsigned long orfp;
	...
	if(CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_ORF_PREFIX_SM) || CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_ORF_PREFIX_RM)) {
		stream_putc(s, (code == CAPABILITY_CODE_ORF ? ORF_TYPE_PREFIX : ORF_TYPE_PREFIX_OLD));
