...
static struct lcommunity *lcommunity_new(void) {
	return (struct lcommunity *) XCALLOC(MTYPE_LCOMMUNITY, sizeof(struct lcommunity));
}
...
static int lcommunity_add_val(struct lcommunity *lcom, struct lcommunity_val *lval) {
	u_int8_t *p;
	int ret;
	int c;
	...
	memmove(lcom->val + (c + 1) * LCOMMUNITY_SIZE, lcom->val + c * LCOMMUNITY_SIZE, (lcom->size - 1 - c) * LCOMMUNITY_SIZE);
...
struct lcommunity *lcommunity_merge(struct lcommunity *lcom1, struct lcommunity *lcom2) {
	if(lcom1->val) {
		lcom1->val = XREALLOC(MTYPE_LCOMMUNITY_VAL, lcom1->val, (lcom1->size + lcom2->size) * LCOMMUNITY_SIZE);
	} else {
		lcom1->val = XMALLOC(MTYPE_LCOMMUNITY_VAL, (lcom1->size + lcom2->size) * LCOMMUNITY_SIZE);
	}

	memcpy(lcom1->val + (lcom1->size * LCOMMUNITY_SIZE), lcom2->val, lcom2->size * LCOMMUNITY_SIZE);
...
int lcommunity_cmp(const void *arg1, const void *arg2) {
	...
	return (lcom1->size == lcom2->size && memcmp(lcom1->val, lcom2->val, lcom1->size * LCOMMUNITY_SIZE) == 0);
}
...
static const char *lcommunity_gettoken(const char *str, struct lcommunity_val *lval, enum lcommunity_token *token) {
	const char *p = str;
...
#define LCOMMUNITY_STR_DEFAULT_LEN 40
	...
	for(i = 0; i < lcom->size; i++) {
		...
		len = sprintf(str_buf + str_pnt, "%u:%u:%u", globaladmin, localdata1, localdata2);
		str_pnt += len;
		first = 0;
	}
	return str_buf;
...
int lcommunity_match(const struct lcommunity *lcom1, const struct lcommunity *lcom2) {
	int i = 0;
	int j = 0;
...
void lcommunity_del_val(struct lcommunity *lcom, u_char *ptr) {
	...
	while(i < lcom->size) {
		if(memcmp(lcom->val + i * LCOMMUNITY_SIZE, ptr, LCOMMUNITY_SIZE) == 0) {
			...
			if(lcom->size > 0) {
				lcom->val = XREALLOC(MTYPE_COMMUNITY_VAL, lcom->val, lcom_length(lcom));
			} else {
