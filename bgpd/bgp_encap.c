...
static u_int16_t decode_rd_type(u_char *pnt) {
	u_int16_t v;
...
static void decode_rd_as(u_char *pnt, struct rd_as *rd_as) {
...
static void decode_rd_as4(u_char *pnt, struct rd_as *rd_as) {
...
static void decode_rd_ip(u_char *pnt, struct rd_ip *rd_ip) {
...
static void ecom2prd(struct ecommunity *ecom, struct prefix_rd *prd) {
	int i;
...
int bgp_nlri_parse_encap(
	struct peer *peer, struct attr *attr, /* Need even for withdraw */
   ...
   */
	...
	for(; pnt < lim; pnt += psize) {
		...
		if(rdtype == RD_TYPE_AS) {
			zlog_info("rd-as %u:%u prefix %s/%d", rd_as.as, rd_as.val, inet_ntop(p.family, &p.u.prefix, buf, BUFSIZ), p.prefixlen);
		} else if(rdtype == RD_TYPE_IP) {
			zlog_info("rd-ip %s:%u prefix %s/%d", inet_ntoa(rd_ip.ip), rd_ip.val, inet_ntop(p.family, &p.u.prefix, buf, BUFSIZ), p.prefixlen);
		} else if(rdtype == RD_TYPE_AS4) {
			zlog_info("rd-as4 %u:%u prefix %s/%d", rd_as.as, rd_as.val, inet_ntop(p.family, &p.u.prefix, buf, BUFSIZ), p.prefixlen);
		} else {
			zlog_info("rd unknown, default to 0:0 prefix %s/%d", inet_ntop(p.family, &p.u.prefix, buf, BUFSIZ), p.prefixlen);
		}

		if(attr) {
			bgp_update(peer, &p, attr, afi, SAFI_ENCAP, ZEBRA_ROUTE_BGP, BGP_ROUTE_NORMAL, &prd, NULL, 0);
		} else {
			bgp_withdraw(peer, &p, attr, afi, SAFI_ENCAP, ZEBRA_ROUTE_BGP, BGP_ROUTE_NORMAL, &prd, NULL);
		}
	}
...
DEFUN(encap_network, encap_network_cmd, "network A.B.C.D/M rd ASN:nn_or_IP-address:nn tag WORD",
...
/* For testing purpose, static route of ENCAP. */
DEFUN(no_encap_network, no_encap_network_cmd, "no network A.B.C.D/M rd ASN:nn_or_IP-address:nn tag WORD",
...
static int show_adj_route_encap(struct vty *vty, struct peer *peer, struct prefix_rd *prd) {
	struct bgp *bgp;
	...
	for(rn = bgp_table_top(bgp->rib[AFI_IP][SAFI_ENCAP]); rn; rn = bgp_route_next(rn)) {
				...
				if((attr = rm->info) != NULL) {
					...
					if(rd_header) {
						...
						switch(type) {
							...
							default: vty_out(vty, "unknown RD type");
						}
...
static int bgp_show_encap(struct vty *vty, afi_t afi, struct prefix_rd *prd, enum bgp_show_type type, void *output_arg, int tags) {
	struct bgp *bgp;
	struct bgp_table *table;
	struct bgp_node *rn;
	struct bgp_node *rm;
				...
				for(ri = rm->info; ri; ri = ri->next) {
					...
					if(rd_header) {
						...
						switch(type) {
							...
							default: vty_out(vty, "Unknown RD type"); break;
						}
...
DEFUN(show_bgp_ipv4_encap, show_bgp_ipv4_encap_cmd, "show bgp ipv4 encap",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display ENCAP NLRI specific information\n") {
	return bgp_show_encap(vty, AFI_IP, NULL, bgp_show_type_normal, NULL, 0);
}

DEFUN(show_bgp_ipv6_encap, show_bgp_ipv6_encap_cmd, "show bgp ipv6 encap",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display ENCAP NLRI specific information\n") {
	return bgp_show_encap(vty, AFI_IP6, NULL, bgp_show_type_normal, NULL, 0);
}

DEFUN(show_bgp_ipv4_encap_rd, show_bgp_ipv4_encap_rd_cmd, "show bgp ipv4 encap rd ASN:nn_or_IP-address:nn",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display ENCAP NLRI specific information\n"
...
DEFUN(show_bgp_ipv6_encap_rd, show_bgp_ipv6_encap_rd_cmd, "show bgp ipv6 encap rd ASN:nn_or_IP-address:nn",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display ENCAP NLRI specific information\n"
		       "Display information for a route distinguisher\n"
		       "ENCAP Route Distinguisher\n"
...
DEFUN(show_bgp_ipv4_encap_tags, show_bgp_ipv4_encap_tags_cmd, "show bgp ipv4 encap tags",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display ENCAP NLRI specific information\n"
		       "Display BGP tags for prefixes\n") {
...
DEFUN(show_bgp_ipv6_encap_tags, show_bgp_ipv6_encap_tags_cmd, "show bgp ipv6 encap tags",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display ENCAP NLRI specific information\n"
		       "Display BGP tags for prefixes\n") {
...
DEFUN(show_bgp_ipv4_encap_rd_tags, show_bgp_ipv4_encap_rd_tags_cmd, "show bgp ipv4 encap rd ASN:nn_or_IP-address:nn tags",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display ENCAP NLRI specific information\n"
		       "Display information for a route distinguisher\n"
		       "ENCAP Route Distinguisher\n"
...
DEFUN(show_bgp_ipv6_encap_rd_tags, show_bgp_ipv6_encap_rd_tags_cmd, "show bgp ipv6 encap rd ASN:nn_or_IP-address:nn tags",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display ENCAP NLRI specific information\n"
		       "Display information for a route distinguisher\n"
		       "ENCAP Route Distinguisher\n"
...
DEFUN(show_bgp_ipv4_encap_neighbor_routes, show_bgp_ipv4_encap_neighbor_routes_cmd, "show bgp ipv4 encap neighbors A.B.C.D routes",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display ENCAP NLRI specific information\n"
...
DEFUN(show_bgp_ipv6_encap_neighbor_routes, show_bgp_ipv6_encap_neighbor_routes_cmd, "show bgp ipv6 encap neighbors A.B.C.D routes",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display ENCAP NLRI specific information\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
...
DEFUN(show_bgp_ipv4_encap_rd_neighbor_routes, show_bgp_ipv4_encap_rd_neighbor_routes_cmd, "show bgp ipv4 encap rd ASN:nn_or_IP-address:nn neighbors (A.B.C.D|X:X::X:X) routes",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display ENCAP NLRI specific information\n"
		       "Display information for a route distinguisher\n"
		       "ENCAP Route Distinguisher\n"
...
DEFUN(show_bgp_ipv6_encap_rd_neighbor_routes, show_bgp_ipv6_encap_rd_neighbor_routes_cmd, "show bgp ipv6 encap rd ASN:nn_or_IP-address:nn neighbors (A.B.C.D|X:X::X:X) routes",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display ENCAP NLRI specific information\n"
		       "Display information for a route distinguisher\n"
		       "ENCAP Route Distinguisher\n"
...
DEFUN(show_bgp_ipv4_encap_neighbor_advertised_routes, show_bgp_ipv4_encap_neighbor_advertised_routes_cmd, "show bgp ipv4 encap neighbors A.B.C.D advertised-routes",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display ENCAP NLRI specific information\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
...
DEFUN(show_bgp_ipv6_encap_neighbor_advertised_routes, show_bgp_ipv6_encap_neighbor_advertised_routes_cmd, "show bgp ipv6 encap neighbors A.B.C.D advertised-routes",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display ENCAP NLRI specific information\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
...
DEFUN(show_bgp_ipv4_encap_rd_neighbor_advertised_routes, show_bgp_ipv4_encap_rd_neighbor_advertised_routes_cmd, "show bgp ipv4 encap rd ASN:nn_or_IP-address:nn neighbors (A.B.C.D|X:X::X:X) advertised-routes",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display ENCAP NLRI specific information\n"
		       "Display information for a route distinguisher\n"
		       "ENCAP Route Distinguisher\n"
...
DEFUN(show_bgp_ipv6_encap_rd_neighbor_advertised_routes, show_bgp_ipv6_encap_rd_neighbor_advertised_routes_cmd, "show bgp ipv6 encap rd ASN:nn_or_IP-address:nn neighbors (A.B.C.D|X:X::X:X) advertised-routes",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display ENCAP NLRI specific information\n"
		       "Display information for a route distinguisher\n"
		       "ENCAP Route Distinguisher\n"
