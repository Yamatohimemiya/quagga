...
const char *bgp_type_str[] = {
	NULL, "OPEN", "UPDATE", "NOTIFICATION", "KEEPALIVE", "ROUTE-REFRESH", "CAPABILITY", NULL,
};
...
/* Dump attribute. */
int bgp_dump_attr(struct peer *peer, struct attr *attr, char *buf, size_t size) {
	if(!attr) {
		return 0;
	}
	...
	if(CHECK_FLAG(attr->flag, ATTR_FLAG_BIT(BGP_ATTR_ORIGIN))) {
		snprintf(buf + strlen(buf), size - strlen(buf), ", origin %s", bgp_origin_str[attr->origin]);
	}
	...
	if(attr->extra) {
		...
		/* Add MP case. */
		if(attr->extra->mp_nexthop_len == 16 || attr->extra->mp_nexthop_len == 32) {
			snprintf(buf + strlen(buf), size - strlen(buf), ", mp_nexthop %s", inet_ntop(AF_INET6, &attr->extra->mp_nexthop_global, addrbuf, BUFSIZ));
		}

		if(attr->extra->mp_nexthop_len == 32) {
			snprintf(buf + strlen(buf), size - strlen(buf), "(%s)", inet_ntop(AF_INET6, &attr->extra->mp_nexthop_local, addrbuf, BUFSIZ));
		}
	}

	if(CHECK_FLAG(attr->flag, ATTR_FLAG_BIT(BGP_ATTR_LOCAL_PREF))) {
		snprintf(buf + strlen(buf), size - strlen(buf), ", localpref %u", attr->local_pref);
	}

	if(CHECK_FLAG(attr->flag, ATTR_FLAG_BIT(BGP_ATTR_MULTI_EXIT_DISC))) {
		snprintf(buf + strlen(buf), size - strlen(buf), ", metric %u", attr->med);
	}

	if(CHECK_FLAG(attr->flag, ATTR_FLAG_BIT(BGP_ATTR_COMMUNITIES))) {
		snprintf(buf + strlen(buf), size - strlen(buf), ", community %s", community_str(attr->community));
	}
	...
	if(CHECK_FLAG(attr->flag, ATTR_FLAG_BIT(BGP_ATTR_AGGREGATOR))) {
		snprintf(buf + strlen(buf), size - strlen(buf), ", aggregated by %u %s", attr->extra->aggregator_as, inet_ntoa(attr->extra->aggregator_addr));
	}

	if(CHECK_FLAG(attr->flag, ATTR_FLAG_BIT(BGP_ATTR_ORIGINATOR_ID))) {
		snprintf(buf + strlen(buf), size - strlen(buf), ", originator %s", inet_ntoa(attr->extra->originator_id));
	}
	...
	if(CHECK_FLAG(attr->flag, ATTR_FLAG_BIT(BGP_ATTR_CLUSTER_LIST))) {
		...
		for(i = 0; i < attr->extra->cluster->length / 4; i++) {
			snprintf(buf + strlen(buf), size - strlen(buf), " %s", inet_ntoa(attr->extra->cluster->list[i]));
		}
	}

	if(CHECK_FLAG(attr->flag, ATTR_FLAG_BIT(BGP_ATTR_AS_PATH))) {
		snprintf(buf + strlen(buf), size - strlen(buf), ", path %s", aspath_print(attr->aspath));
	}
...
/* dump notify packet */
void bgp_notify_print(struct peer *peer, struct bgp_notify *bgp_notify, const char *direct) {
	const char *subcode_str;
	const char *code_str;

	subcode_str = "";
	code_str = LOOKUP_DEF(bgp_notify_msg, bgp_notify->code, "Unrecognized Error Code");

	switch(bgp_notify->code) {
		case BGP_NOTIFY_HEADER_ERR: subcode_str = LOOKUP_DEF(bgp_notify_head_msg, bgp_notify->subcode, "Unrecognized Error Subcode"); break;
		case BGP_NOTIFY_OPEN_ERR: subcode_str = LOOKUP_DEF(bgp_notify_open_msg, bgp_notify->subcode, "Unrecognized Error Subcode"); break;
		case BGP_NOTIFY_UPDATE_ERR: subcode_str = LOOKUP_DEF(bgp_notify_update_msg, bgp_notify->subcode, "Unrecognized Error Subcode"); break;
		case BGP_NOTIFY_HOLD_ERR: break;
		case BGP_NOTIFY_FSM_ERR: break;
		case BGP_NOTIFY_CEASE: subcode_str = LOOKUP_DEF(bgp_notify_cease_msg, bgp_notify->subcode, "Unrecognized Error Subcode"); break;
		case BGP_NOTIFY_CAPABILITY_ERR: subcode_str = LOOKUP_DEF(bgp_notify_capability_msg, bgp_notify->subcode, "Unrecognized Error Subcode"); break;
	}

	if(bgp_flag_check(peer->bgp, BGP_FLAG_LOG_NEIGHBOR_CHANGES)) {
		zlog_info(
			"%%NOTIFICATION: %s neighbor %s %d/%d (%s%s) %d bytes %s", strcmp(direct, "received") == 0 ? "received from" : "sent to", peer->host, bgp_notify->code, bgp_notify->subcode, code_str, subcode_str, bgp_notify->length,
			bgp_notify->data ? bgp_notify->data : ""
		);
	} else if(BGP_DEBUG(normal, NORMAL)) {
		plog_debug(
			peer->log, "%s %s NOTIFICATION %d/%d (%s%s) %d bytes %s", peer ? peer->host : "", direct, bgp_notify->code, bgp_notify->subcode, code_str, subcode_str, bgp_notify->length, bgp_notify->data ? bgp_notify->data : ""
		);
	}
}
...
DEFUN(debug_bgp_as4, debug_bgp_as4_cmd, "debug bgp as4", DEBUG_STR BGP_STR "BGP AS4 actions\n") {
...
ALIAS(no_debug_bgp_as4, undebug_bgp_as4_cmd, "undebug bgp as4", UNDEBUG_STR BGP_STR "BGP AS4 actions\n")

DEFUN(debug_bgp_as4_segment, debug_bgp_as4_segment_cmd, "debug bgp as4 segment",
...
DEFUN(no_debug_bgp_as4_segment, no_debug_bgp_as4_segment_cmd, "no debug bgp as4 segment",
      NO_STR DEBUG_STR BGP_STR "BGP AS4 actions\n"
			       "BGP AS4 aspath segment handling\n") {
	if(vty->node == CONFIG_NODE) {
...
ALIAS(no_debug_bgp_as4_segment, undebug_bgp_as4_segment_cmd, "undebug bgp as4 segment",
      UNDEBUG_STR BGP_STR "BGP AS4 actions\n"
			  "BGP AS4 aspath segment handling\n")
...
DEFUN(no_debug_bgp_fsm, no_debug_bgp_fsm_cmd, "no debug bgp fsm", NO_STR DEBUG_STR BGP_STR "Finite State Machine\n") {
	if(vty->node == CONFIG_NODE) {
		DEBUG_OFF(fsm, FSM);
	} else {
...
ALIAS(no_debug_bgp_fsm, undebug_bgp_fsm_cmd, "undebug bgp fsm", UNDEBUG_STR BGP_STR "Finite State Machine\n")
...
DEFUN(no_debug_bgp_events, no_debug_bgp_events_cmd, "no debug bgp events", NO_STR DEBUG_STR BGP_STR "BGP events\n") {
	if(vty->node == CONFIG_NODE) {
...
ALIAS(no_debug_bgp_events, undebug_bgp_events_cmd, "undebug bgp events", UNDEBUG_STR BGP_STR "BGP events\n")
...
DEFUN(no_debug_bgp_nht, no_debug_bgp_nht_cmd, "no debug bgp nht", NO_STR DEBUG_STR BGP_STR "BGP nexthop tracking events\n") {
	if(vty->node == CONFIG_NODE) {
		DEBUG_OFF(nht, NHT);
	} else {
...
ALIAS(no_debug_bgp_nht, undebug_bgp_nht_cmd, "undebug bgp nht", UNDEBUG_STR BGP_STR "BGP next-hop tracking updates\n")
...
DEFUN(no_debug_bgp_filter, no_debug_bgp_filter_cmd, "no debug bgp filters", NO_STR DEBUG_STR BGP_STR "BGP filters\n") {
	if(vty->node == CONFIG_NODE) {
...
ALIAS(no_debug_bgp_filter, undebug_bgp_filter_cmd, "undebug bgp filters", UNDEBUG_STR BGP_STR "BGP filters\n")
...
DEFUN(no_debug_bgp_keepalive, no_debug_bgp_keepalive_cmd, "no debug bgp keepalives", NO_STR DEBUG_STR BGP_STR "BGP keepalives\n") {
	if(vty->node == CONFIG_NODE) {
...
ALIAS(no_debug_bgp_keepalive, undebug_bgp_keepalive_cmd, "undebug bgp keepalives", UNDEBUG_STR BGP_STR "BGP keepalives\n")
...
DEFUN(debug_bgp_update_direct, debug_bgp_update_direct_cmd, "debug bgp updates (in|out)",
      DEBUG_STR BGP_STR "BGP updates\n"
			"Inbound updates\n"
			"Outbound updates\n") {
	if(vty->node == CONFIG_NODE) {
...
DEFUN(no_debug_bgp_update, no_debug_bgp_update_cmd, "no debug bgp updates", NO_STR DEBUG_STR BGP_STR "BGP updates\n") {
	if(vty->node == CONFIG_NODE) {
		DEBUG_OFF(update, UPDATE_IN);
		DEBUG_OFF(update, UPDATE_OUT);
	} else {
...
ALIAS(no_debug_bgp_update, undebug_bgp_update_cmd, "undebug bgp updates", UNDEBUG_STR BGP_STR "BGP updates\n")

DEFUN(debug_bgp_normal, debug_bgp_normal_cmd, "debug bgp", DEBUG_STR BGP_STR) {
	if(vty->node == CONFIG_NODE) {
...
DEFUN(no_debug_bgp_normal, no_debug_bgp_normal_cmd, "no debug bgp", NO_STR DEBUG_STR BGP_STR) {
	if(vty->node == CONFIG_NODE) {
...
ALIAS(no_debug_bgp_normal, undebug_bgp_normal_cmd, "undebug bgp", UNDEBUG_STR BGP_STR)

DEFUN(debug_bgp_zebra, debug_bgp_zebra_cmd, "debug bgp zebra", DEBUG_STR BGP_STR "BGP Zebra messages\n") {
	if(vty->node == CONFIG_NODE) {
...
DEFUN(no_debug_bgp_zebra, no_debug_bgp_zebra_cmd, "no debug bgp zebra", NO_STR DEBUG_STR BGP_STR "BGP Zebra messages\n") {
	if(vty->node == CONFIG_NODE) {
...
ALIAS(no_debug_bgp_zebra, undebug_bgp_zebra_cmd, "undebug bgp zebra", UNDEBUG_STR BGP_STR "BGP Zebra messages\n")
...
DEFUN(no_debug_bgp_allow_martians, no_debug_bgp_allow_martians_cmd, "no debug bgp allow-martians", NO_STR DEBUG_STR BGP_STR "BGP allow martian next hops\n") {
	if(vty->node == CONFIG_NODE) {
...
ALIAS(no_debug_bgp_allow_martians, undebug_bgp_allow_martians_cmd, "undebug bgp allow-martians", UNDEBUG_STR BGP_STR "BGP allow martian next hops\n")
...
ALIAS(no_debug_bgp_all, undebug_bgp_all_cmd, "undebug all bgp", UNDEBUG_STR "Enable all debugging\n" BGP_STR)

DEFUN(show_debugging_bgp, show_debugging_bgp_cmd, "show debugging bgp", SHOW_STR DEBUG_STR BGP_STR) {
