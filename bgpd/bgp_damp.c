...
static int bgp_reuse_timer(struct thread *t) {
	...
	damp->t_reuse = NULL;
	damp->t_reuse = thread_add_timer(bm->master, bgp_reuse_timer, NULL, DELTA_REUSE);
	...
	for(; bdi; bdi = next) {
		...
		if(bdi->penalty < damp->reuse_limit) {
			...
			if(bdi->lastrecord == BGP_RECORD_UPDATE) {
				...
				bgp_aggregate_increment(bgp, &bdi->rn->p, bdi->binfo, bdi->afi, bdi->safi);
...
int bgp_damp_withdraw(struct bgp_info *binfo, struct bgp_node *rn, afi_t afi, safi_t safi, int attr_change) {
	...
	if(bdi == NULL) {
	 ...
	 1. allocate a damping structure.
		...
		bdi->penalty = (bgp_damp_decay(t_now - bdi->t_updated, bdi->penalty) + (attr_change ? DEFAULT_PENALTY / 2 : DEFAULT_PENALTY));
...
int bgp_damp_update(struct bgp_info *binfo, struct bgp_node *rn, afi_t afi, safi_t safi) {
	...
	if(!CHECK_FLAG(bdi->binfo->flags, BGP_INFO_DAMPED) && (bdi->penalty < damp->suppress_value)) {
		status = BGP_DAMP_USED;
	} else if(CHECK_FLAG(bdi->binfo->flags, BGP_INFO_DAMPED) && (bdi->penalty < damp->reuse_limit)) {
...
static void bgp_damp_parameter_set(int hlife, int reuse, int sup, int maxsup) {
	...
	damp->decay_array = XMALLOC(MTYPE_BGP_DAMP_ARRAY, sizeof(double) * (damp->decay_array_size));
	...
	damp->reuse_list = XCALLOC(MTYPE_BGP_DAMP_ARRAY, damp->reuse_list_size * sizeof(struct bgp_reuse_node *));

	/* Reuse-array computations */
	damp->reuse_index = XCALLOC(MTYPE_BGP_DAMP_ARRAY, sizeof(int) * damp->reuse_index_size);
	...
	for(i = 0; i < damp->reuse_index_size; i++) {
		damp->reuse_index[i] = (int) (((double) damp->half_life / DELTA_REUSE) * log10(1.0 / (damp->reuse_limit * (1.0 + ((double) i / damp->scale_factor)))) / log10(0.5));
	}
}

int bgp_damp_enable(struct bgp *bgp, afi_t afi, safi_t safi, time_t half, unsigned int reuse, unsigned int suppress, time_t max) {
	if(CHECK_FLAG(bgp->af_flags[afi][safi], BGP_CONFIG_DAMPENING)) {
		if(damp->half_life == half && damp->reuse_limit == reuse && damp->suppress_value == suppress && damp->max_suppress_time == max) {
			return 0;
		}
...
void bgp_config_write_damp(struct vty *vty) {
	if(bgp_damp_cfg.half_life == DEFAULT_HALF_LIFE * 60 && bgp_damp_cfg.reuse_limit == DEFAULT_REUSE && bgp_damp_cfg.suppress_value == DEFAULT_SUPPRESS && bgp_damp_cfg.max_suppress_time == bgp_damp_cfg.half_life * 4) {
		vty_out(vty, " bgp dampening%s", VTY_NEWLINE);
	} else if(bgp_damp_cfg.half_life != DEFAULT_HALF_LIFE * 60 && bgp_damp_cfg.reuse_limit == DEFAULT_REUSE && bgp_damp_cfg.suppress_value == DEFAULT_SUPPRESS && bgp_damp_cfg.max_suppress_time == bgp_damp_cfg.half_life * 4) {
		vty_out(vty, " bgp dampening %lld%s", bgp_damp_cfg.half_life / 60LL, VTY_NEWLINE);
	} else {
		vty_out(vty, " bgp dampening %lld %d %d %lld%s", bgp_damp_cfg.half_life / 60LL, bgp_damp_cfg.reuse_limit, bgp_damp_cfg.suppress_value, bgp_damp_cfg.max_suppress_time / 60LL, VTY_NEWLINE);
	}
}

static const char *bgp_get_reuse_time(unsigned int penalty, char *buf, size_t len) {
	time_t reuse_time = 0;
	struct tm *tm = NULL;
...
void bgp_damp_info_vty(struct vty *vty, struct bgp_info *binfo) {
	...
	vty_out(vty, "      Dampinfo: penalty %d, flapped %d times in %s", penalty, bdi->flap, peer_uptime(bdi->start_time, timebuf, BGP_UPTIME_LEN));

	if(CHECK_FLAG(binfo->flags, BGP_INFO_DAMPED) && !CHECK_FLAG(binfo->flags, BGP_INFO_HISTORY)) {
		vty_out(vty, ", reuse in %s", bgp_get_reuse_time(penalty, timebuf, BGP_UPTIME_LEN));
	}

	vty_out(vty, "%s", VTY_NEWLINE);
}

const char *bgp_damp_reuse_time_vty(struct vty *vty, struct bgp_info *binfo, char *timebuf, size_t len) {
	struct bgp_damp_info *bdi;
	time_t t_now, t_diff;
	int penalty;
...
int bgp_show_dampening_parameters(struct vty *vty, afi_t afi, safi_t safi) {
	...
	if(CHECK_FLAG(bgp->af_flags[afi][safi], BGP_CONFIG_DAMPENING)) {
		vty_out(vty, "Half-life time: %ld min%s", damp->half_life / 60, VTY_NEWLINE);
		vty_out(vty, "Reuse penalty: %d%s", damp->reuse_limit, VTY_NEWLINE);
		vty_out(vty, "Suppress penalty: %d%s", damp->suppress_value, VTY_NEWLINE);
		vty_out(vty, "Max suppress time: %ld min%s", damp->max_suppress_time / 60, VTY_NEWLINE);
		vty_out(vty, "Max suppress penalty: %u%s", damp->ceiling, VTY_NEWLINE);
		vty_out(vty, "%s", VTY_NEWLINE);
	} else {
		vty_out(vty, "dampening not enabled for %s%s", afi == AFI_IP ? "IPv4" : "IPv6", VTY_NEWLINE);
	}
