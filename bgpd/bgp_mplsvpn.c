...
int bgp_nlri_parse_vpn(struct peer *peer, struct attr *attr, struct bgp_nlri *packet) {
	u_char *pnt;
	u_char *lim;
	struct prefix p;
	int psize = 0;
	...
	for(; pnt < lim; pnt += psize) {
		...
		if((psize - VPN_PREFIXLEN_MIN_BYTES) > (ssize_t) sizeof(p.u)) {
			plog_err(
				...
				peer->host, prefixlen - VPN_PREFIXLEN_MIN_BYTES * 8, sizeof(p.u)
			);
			return -1;
		}
		...
		if((psize - VPN_PREFIXLEN_MIN_BYTES) > prefix_blen(&p)) {
			plog_err(
				...
				peer->host, prefixlen - VPN_PREFIXLEN_MIN_BYTES * 8, p.family, prefix_blen(&p)
			);
			return -1;
		}
		...
		switch(type) {
			case RD_TYPE_AS: decode_rd_as(pnt + 5, &rd_as); break;

			case RD_TYPE_AS4: decode_rd_as4(pnt + 5, &rd_as); break;

			case RD_TYPE_IP: decode_rd_ip(pnt + 5, &rd_ip); break;

			default: zlog_err("Unknown RD type %d", type); break; /* just report */
		}

		p.prefixlen = prefixlen - VPN_PREFIXLEN_MIN_BYTES * 8;
		memcpy(&p.u.prefix, pnt + VPN_PREFIXLEN_MIN_BYTES, psize - VPN_PREFIXLEN_MIN_BYTES);

		if(attr) {
			bgp_update(peer, &p, attr, packet->afi, SAFI_MPLS_VPN, ZEBRA_ROUTE_BGP, BGP_ROUTE_NORMAL, &prd, tagpnt, 0);
		} else {
			bgp_withdraw(peer, &p, attr, packet->afi, SAFI_MPLS_VPN, ZEBRA_ROUTE_BGP, BGP_ROUTE_NORMAL, &prd, tagpnt);
		}
	}
...
int str2tag(const char *str, u_char *tag) {
	unsigned long l;
	char *endptr;
	u_int32_t t;
...
DEFUN(vpnv4_network, vpnv4_network_cmd, "network A.B.C.D/M rd ASN:nn_or_IP-address:nn tag WORD",
...
DEFUN(vpnv4_network_route_map, vpnv4_network_route_map_cmd, "network A.B.C.D/M rd ASN:nn_or_IP-address:nn tag WORD route-map WORD",
      "Specify a network to announce via BGP\n"
      "IP prefix <network>/<length>, e.g., 35.0.0.0/8\n"
      "Specify Route Distinguisher\n"
      "VPN Route Distinguisher\n"
...
/* For testing purpose, static route of MPLS-VPN. */
DEFUN(no_vpnv4_network, no_vpnv4_network_cmd, "no network A.B.C.D/M rd ASN:nn_or_IP-address:nn tag WORD",
...
static int show_adj_route_vpn(struct vty *vty, struct peer *peer, struct prefix_rd *prd) {
	struct bgp *bgp;
...
static int bgp_show_mpls_vpn(struct vty *vty, afi_t afi, struct prefix_rd *prd, enum bgp_show_type type, void *output_arg, int tags) {
	struct bgp *bgp;
	struct bgp_table *table;
	struct bgp_node *rn;
	struct bgp_node *rm;
...
DEFUN(show_bgp_ipv4_vpn, show_bgp_ipv4_vpn_cmd, "show bgp ipv4 vpn",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display VPN NLRI specific information\n") {
	return bgp_show_mpls_vpn(vty, AFI_IP, NULL, bgp_show_type_normal, NULL, 0);
}

DEFUN(show_bgp_ipv6_vpn, show_bgp_ipv6_vpn_cmd, "show bgp ipv6 vpn",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display VPN NLRI specific information\n") {
	return bgp_show_mpls_vpn(vty, AFI_IP6, NULL, bgp_show_type_normal, NULL, 0);
}

DEFUN(show_bgp_ipv4_vpn_rd, show_bgp_ipv4_vpn_rd_cmd, "show bgp ipv4 vpn rd ASN:nn_or_IP-address:nn",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display VPN NLRI specific information\n"
...
DEFUN(show_bgp_ipv6_vpn_rd, show_bgp_ipv6_vpn_rd_cmd, "show bgp ipv6 vpn rd ASN:nn_or_IP-address:nn",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display VPN NLRI specific information\n"
		       "Display information for a route distinguisher\n"
		       "VPN Route Distinguisher\n") {
...
DEFUN(show_bgp_ipv4_vpn_tags, show_bgp_ipv4_vpn_tags_cmd, "show bgp ipv4 vpn tags",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display VPN NLRI specific information\n"
...
DEFUN(show_bgp_ipv6_vpn_tags, show_bgp_ipv6_vpn_tags_cmd, "show bgp ipv6 vpn tags",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display VPN NLRI specific information\n"
		       "Display BGP tags for prefixes\n") {
...
DEFUN(show_bgp_ipv4_vpn_rd_tags, show_bgp_ipv4_vpn_rd_tags_cmd, "show bgp ipv4 vpn rd ASN:nn_or_IP-address:nn tags",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display VPN NLRI specific information\n"
		       "Display information for a route distinguisher\n"
		       "VPN Route Distinguisher\n"
...
DEFUN(show_bgp_ipv6_vpn_rd_tags, show_bgp_ipv6_vpn_rd_tags_cmd, "show bgp ipv6 vpn rd ASN:nn_or_IP-address:nn tags",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display VPN NLRI specific information\n"
		       "Display information for a route distinguisher\n"
		       "VPN Route Distinguisher\n"
...
DEFUN(show_bgp_ipv4_vpn_neighbor_routes, show_bgp_ipv4_vpn_neighbor_routes_cmd, "show bgp ipv4 vpn neighbors (A.B.C.D|X:X::X:X) routes",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display VPN NLRI specific information\n"
...
DEFUN(show_bgp_ipv6_vpn_neighbor_routes, show_bgp_ipv6_vpn_neighbor_routes_cmd, "show bgp ipv6 vpn neighbors (A.B.C.D|X:X::X:X) routes",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display VPN NLRI specific information\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
...
DEFUN(show_bgp_ipv4_vpn_neighbor_advertised_routes, show_bgp_ipv4_vpn_neighbor_advertised_routes_cmd, "show bgp ipv4 vpn neighbors (A.B.C.D|X:X::X:X) advertised-routes",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display VPN NLRI specific information\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
...
DEFUN(show_bgp_ipv6_vpn_neighbor_advertised_routes, show_bgp_ipv6_vpn_neighbor_advertised_routes_cmd, "show bgp ipv6 vpn neighbors (A.B.C.D|X:X::X:X) advertised-routes",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display VPN NLRI specific information\n"
		       "Detailed information on TCP and BGP neighbor connections\n"
		       "Neighbor to display information about\n"
...
DEFUN(show_ip_bgp_vpnv4_rd_neighbor_advertised_routes, show_bgp_ipv4_vpn_rd_neighbor_advertised_routes_cmd, "show bgp ipv4 vpn rd ASN:nn_or_IP-address:nn neighbors (A.B.C.D|X:X::X:X) advertised-routes",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display VPN NLRI specific information\n"
		       "Display information for a route distinguisher\n"
		       "VPN Route Distinguisher\n"
...
DEFUN(show_ip_bgp_vpnv6_rd_neighbor_advertised_routes, show_bgp_ipv6_vpn_rd_neighbor_advertised_routes_cmd, "show bgp ipv6 vpn rd ASN:nn_or_IP-address:nn neighbors (A.B.C.D|X:X::X:X) advertised-routes",
      SHOW_STR BGP_STR "Address Family\n"
		       "Display VPN NLRI specific information\n"
		       "Display information for a route distinguisher\n"
		       "VPN Route Distinguisher\n"
...
DEFUN(show_bgp_ipv4_vpn_rd_neighbor_routes, show_bgp_ipv4_vpn_rd_neighbor_routes_cmd, "show bgp ipv4 vpn rd ASN:nn_or_IP-address:nn neighbors (A.B.C.D|X:X::X:X) routes",
      SHOW_STR BGP_STR "Address Family\n"
...
DEFUN(show_bgp_ipv6_vpn_rd_neighbor_routes, show_bgp_ipv6_vpn_rd_neighbor_routes_cmd, "show bgp ipv6 vpn rd ASN:nn_or_IP-address:nn neighbors (A.B.C.D|X:X::X:X) routes",
      SHOW_STR BGP_STR "Address Family\n"
		       "Address Family modifier\n"
		       "Display information for a route distinguisher\n"
		       "VPN Route Distinguisher\n"
