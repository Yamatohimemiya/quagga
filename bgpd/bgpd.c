...
int bgp_option_set(int flag) {
	switch(flag) {
		...
		case BGP_OPT_NO_LISTEN: SET_FLAG(bm->options, flag); break;
		default: return BGP_ERR_INVALID_FLAG;
	}
	return 0;
}
...
int bgp_option_unset(int flag) {
	switch(flag) {
		case BGP_OPT_MULTIPLE_INSTANCE:
			...
			/* Fall through.  */
		case BGP_OPT_NO_FIB:
		case BGP_OPT_CONFIG_CISCO: UNSET_FLAG(bm->options, flag); break;
		default: return BGP_ERR_INVALID_FLAG;
	}
	return 0;
}
...
static int bgp_router_id_set(struct bgp *bgp, struct in_addr *id) {
	struct peer *peer;
	struct listnode *node, *nnode;

	if(bgp_config_check(bgp, BGP_CONFIG_ROUTER_ID) && IPV4_ADDR_SAME(&bgp->router_id, id)) {
		return 0;
	}
...
void bgp_router_id_zebra_bump(void) {
	struct listnode *node, *nnode;
	struct bgp *bgp;
...
int bgp_cluster_id_set(struct bgp *bgp, struct in_addr *cluster_id) {
	...
	if(bgp_config_check(bgp, BGP_CONFIG_CLUSTER_ID) && IPV4_ADDR_SAME(&bgp->cluster_id, cluster_id)) {
		return 0;
	}
...
int bgp_confederation_peers_add(struct bgp *bgp, as_t as) {
	...
	if(bgp->confed_peers) {
		bgp->confed_peers = XREALLOC(MTYPE_BGP_CONFED_LIST, bgp->confed_peers, (bgp->confed_peers_cnt + 1) * sizeof(as_t));
	} else {
		bgp->confed_peers = XMALLOC(MTYPE_BGP_CONFED_LIST, (bgp->confed_peers_cnt + 1) * sizeof(as_t));
	}
...
int bgp_confederation_peers_remove(struct bgp *bgp, as_t as) {
	...
	if(bgp->confed_peers_cnt == 0) {
		...
		bgp->confed_peers = XREALLOC(MTYPE_BGP_CONFED_LIST, bgp->confed_peers, bgp->confed_peers_cnt * sizeof(as_t));
	}
...
int peer_rsclient_active(struct peer *peer) {
	int i;
	int j;

	for(i = AFI_IP; i < AFI_MAX; i++) {
		for(j = SAFI_UNICAST; j < SAFI_MAX; j++) {
			if(CHECK_FLAG(peer->af_flags[i][j], PEER_FLAG_RSERVER_CLIENT) && !peer->af_group[i][j]) {
				return 1;
			}
		}
	}
...
static int peer_cmp(struct peer *p1, struct peer *p2) {
...
static bgp_peer_sort_t peer_calc_sort(struct peer *peer) {
	...
	if(CHECK_FLAG(peer->sflags, PEER_STATUS_GROUP)) {
		if(peer->as) {
			...
			if(peer1) {
				return (peer1->local_as == peer1->as ? BGP_PEER_IBGP : BGP_PEER_EBGP);
			}
		}
	...
	if(bgp && CHECK_FLAG(bgp->config, BGP_CONFIG_CONFEDERATION)) {
		...
		return (peer->local_as == 0 ? BGP_PEER_INTERNAL : peer->local_as == peer->as ? BGP_PEER_IBGP : BGP_PEER_EBGP);
	}
}
...
static struct peer *peer_new(struct bgp *bgp) {
	afi_t afi;
	safi_t safi;
	struct peer *peer;
	struct servent *sp;
...
/* Create new BGP peer.  */
static struct peer *peer_create(union sockunion *su, struct bgp *bgp, as_t local_as, as_t remote_as, afi_t afi, safi_t safi) {
	int active;
	struct peer *peer;
...
static void peer_as_change(struct peer *peer, as_t as) {
	struct peer *conf;
	...
	if(bgp_config_check(peer->bgp, BGP_CONFIG_CONFEDERATION) && !bgp_confederation_peers_check(peer->bgp, as) && peer->bgp->as != as) {
	...
	if(peer_sort(peer) != BGP_PEER_IBGP) {
		UNSET_FLAG(peer->af_flags[AFI_IP][SAFI_UNICAST], PEER_FLAG_REFLECTOR_CLIENT);
		UNSET_FLAG(peer->af_flags[AFI_IP][SAFI_MULTICAST], PEER_FLAG_REFLECTOR_CLIENT);
		UNSET_FLAG(peer->af_flags[AFI_IP][SAFI_MPLS_VPN], PEER_FLAG_REFLECTOR_CLIENT);
		UNSET_FLAG(peer->af_flags[AFI_IP][SAFI_ENCAP], PEER_FLAG_REFLECTOR_CLIENT);
		UNSET_FLAG(peer->af_flags[AFI_IP6][SAFI_UNICAST], PEER_FLAG_REFLECTOR_CLIENT);
		UNSET_FLAG(peer->af_flags[AFI_IP6][SAFI_MULTICAST], PEER_FLAG_REFLECTOR_CLIENT);
		UNSET_FLAG(peer->af_flags[AFI_IP6][SAFI_MPLS_VPN], PEER_FLAG_REFLECTOR_CLIENT);
		UNSET_FLAG(peer->af_flags[AFI_IP6][SAFI_ENCAP], PEER_FLAG_REFLECTOR_CLIENT);
	}
...
int peer_remote_as(struct bgp *bgp, union sockunion *su, as_t *as, afi_t afi, safi_t safi) {
	...
	if(peer) {
	 ...
	 iBGP peer then spoof the source AS */
		if(bgp_config_check(bgp, BGP_CONFIG_CONFEDERATION) && !bgp_confederation_peers_check(bgp, *as) && bgp->as != *as) {
		...
		if(!active && peer_active(peer)) {
			...
			if(peer->status == Established) {
				if(CHECK_FLAG(peer->cap, PEER_CAP_DYNAMIC_RCV)) {
					...
					bgp_capability_send(peer, afi, safi, CAPABILITY_CODE_MP, CAPABILITY_ACTION_SET);
		...
		if(peer->status == Established) {
			if(CHECK_FLAG(peer->cap, PEER_CAP_DYNAMIC_RCV)) {
				...
				if(peer_active_nego(peer)) {
					bgp_capability_send(peer, afi, safi, CAPABILITY_CODE_MP, CAPABILITY_ACTION_UNSET);
...
int peer_delete(struct peer *peer) {
   ...
   */
	...
	if(!CHECK_FLAG(peer->sflags, PEER_STATUS_GROUP) && (pn = listnode_lookup(bgp->peer, peer))) {
		peer_unlock(peer); /* bgp peer list reference */
		list_delete_node(bgp->peer, pn);
	}

	if(peer_rsclient_active(peer) && (pn = listnode_lookup(bgp->rsclient, peer))) {
...
static int peer_group_active(struct peer *peer) {
	if(peer->af_group[AFI_IP][SAFI_UNICAST] || peer->af_group[AFI_IP][SAFI_MULTICAST] || peer->af_group[AFI_IP][SAFI_MPLS_VPN] || peer->af_group[AFI_IP][SAFI_ENCAP] || peer->af_group[AFI_IP6][SAFI_UNICAST]
	   || peer->af_group[AFI_IP6][SAFI_MULTICAST] || peer->af_group[AFI_IP6][SAFI_MPLS_VPN] || peer->af_group[AFI_IP6][SAFI_ENCAP]) {
		return 1;
	}
	return 0;
}
...
static struct peer_group *peer_group_new(void) {
	return (struct peer_group *) XCALLOC(MTYPE_PEER_GROUP, sizeof(struct peer_group));
}
...
static void peer_group2peer_config_copy(struct peer_group *group, struct peer *peer, afi_t afi, safi_t safi) {
	int in = FILTER_IN;
	int out = FILTER_OUT;
	struct peer *conf;
...
int peer_group_bind(struct bgp *bgp, union sockunion *su, struct peer_group *group, afi_t afi, safi_t safi, as_t *as) {
	...
	if(!group->conf->as) {
		if(peer_sort(group->conf) != BGP_PEER_INTERNAL && peer_sort(group->conf) != peer_sort(peer)) {
			if(as) {
				*as = peer->as;
			}
			return BGP_ERR_PEER_GROUP_PEER_TYPE_DIFFERENT;
	...
	if(CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT)) {
		...
		/* Export policy. */
		if(!CHECK_FLAG(group->conf->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT) && peer->filter[afi][safi].map[RMAP_EXPORT].name) {
...
static struct bgp *bgp_create(as_t *as, const char *name) {
	...
	THREAD_TIMER_ON(bm->master, bgp->t_startup, bgp_startup_timer_expire, bgp, bgp->restart_time);
...
struct bgp *bgp_lookup(as_t as, const char *name) {
	...
	for(ALL_LIST_ELEMENTS(bm->bgp, node, nnode, bgp)) {
		if(bgp->as == as && ((bgp->name == NULL && name == NULL) || (bgp->name && name && strcmp(bgp->name, name) == 0))) {
			return bgp;
		}
	}
	return NULL;
...
struct bgp *bgp_lookup_by_name(const char *name) {
	...
	for(ALL_LIST_ELEMENTS(bm->bgp, node, nnode, bgp)) {
		if((bgp->name == NULL && name == NULL) || (bgp->name && name && strcmp(bgp->name, name) == 0)) {
			return bgp;
		}
	}
	return NULL;
...
int bgp_delete(struct bgp *bgp) {
	...
	for(ALL_LIST_ELEMENTS(bgp->peer, node, next, peer)) {
		if(peer->status == Established || peer->status == OpenSent || peer->status == OpenConfirm) {
...
struct peer *peer_lookup(struct bgp *bgp, union sockunion *su) {
	...
	if(bgp != NULL) {
		for(ALL_LIST_ELEMENTS(bgp->peer, node, nnode, peer)) {
			if(sockunion_same(&peer->su, su) && !CHECK_FLAG(peer->sflags, PEER_STATUS_ACCEPT_PEER)) {
				return peer;
			}
		}
	} else if(bm->bgp != NULL) {
...
int peer_active(struct peer *peer) {
	if(peer->afc[AFI_IP][SAFI_UNICAST] || peer->afc[AFI_IP][SAFI_MULTICAST] || peer->afc[AFI_IP][SAFI_MPLS_VPN] || peer->afc[AFI_IP][SAFI_ENCAP] || peer->afc[AFI_IP6][SAFI_UNICAST] || peer->afc[AFI_IP6][SAFI_MULTICAST]
...
int peer_active_nego(struct peer *peer) {
	if(peer->afc_nego[AFI_IP][SAFI_UNICAST] || peer->afc_nego[AFI_IP][SAFI_MULTICAST] || peer->afc_nego[AFI_IP][SAFI_MPLS_VPN] || peer->afc_nego[AFI_IP][SAFI_ENCAP] || peer->afc_nego[AFI_IP6][SAFI_UNICAST]
	   || peer->afc_nego[AFI_IP6][SAFI_MULTICAST] || peer->afc_nego[AFI_IP6][SAFI_MPLS_VPN] || peer->afc_nego[AFI_IP6][SAFI_ENCAP]) {
		return 1;
	}
	return 0;
}
...
static void peer_change_action(struct peer *peer, afi_t afi, safi_t safi, enum peer_change_type type) {
	if(CHECK_FLAG(peer->sflags, PEER_STATUS_GROUP)) {
		return;
	}
...
/* Proper action set. */
static int peer_flag_action_set(const struct peer_flag_action *action_list, int size, struct peer_flag_action *action, u_int32_t flag) {
	int i;
	int found = 0;
	int reset_in = 0;
	int reset_out = 0;
	...
	if(flag == PEER_FLAG_SHUTDOWN) {
		if(CHECK_FLAG(peer->flags, flag)) {
			...
			if(peer->t_pmax_restart) {
				...
				if(BGP_DEBUG(events, EVENTS)) {
					zlog_debug("%s Maximum-prefix restart timer canceled", peer->host);
				}
			}
...
static int peer_flag_modify(struct peer *peer, u_int32_t flag, int set) {
	...
	/* Flag conflict check.  */
	if(set && CHECK_FLAG(peer->flags | flag, PEER_FLAG_STRICT_CAP_MATCH) && CHECK_FLAG(peer->flags | flag, PEER_FLAG_OVERRIDE_CAPABILITY)) {
...
static int peer_is_group_member(struct peer *peer, afi_t afi, safi_t safi) {
	if(peer->af_group[afi][safi]) {
		return 1;
	}
	return 0;
...
static int peer_af_flag_modify(struct peer *peer, afi_t afi, safi_t safi, u_int32_t flag, int set) {
	int found;
	int size;
	struct listnode *node, *nnode;
	struct peer_group *group;
	...
	if(flag & PEER_FLAG_REMOVE_PRIVATE_AS && peer_sort(peer) == BGP_PEER_IBGP) {
...
int peer_update_source_if_set(struct peer *peer, const char *ifname) {
	...
	if(peer->update_if) {
		if(!CHECK_FLAG(peer->sflags, PEER_STATUS_GROUP) && strcmp(peer->update_if, ifname) == 0) {
			return 0;
		}
...
int peer_update_source_addr_set(struct peer *peer, const union sockunion *su) {
	...
	if(peer->update_source) {
		if(!CHECK_FLAG(peer->sflags, PEER_STATUS_GROUP) && sockunion_cmp(peer->update_source, su) == 0) {
			return 0;
		}
		sockunion_free(peer->update_source);
		peer->update_source = NULL;
...
int peer_update_source_unset(struct peer *peer) {
	...
	if(!CHECK_FLAG(peer->sflags, PEER_STATUS_GROUP) && !peer->update_source && !peer->update_if) {
		return 0;
	}
	...
	if(peer_group_active(peer)) {
		...
		if(group->conf->update_source) {
			...
			peer->update_if = XSTRDUP(MTYPE_PEER_UPDATE_SOURCE, group->conf->update_if);
		}
	}
...
int peer_default_originate_set(struct peer *peer, afi_t afi, safi_t safi, const char *rmap) {
	struct peer_group *group;
	struct listnode *node, *nnode;
	...
	if(!CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_DEFAULT_ORIGINATE) || (rmap && !peer->default_rmap[afi][safi].name) || (rmap && strcmp(rmap, peer->default_rmap[afi][safi].name) != 0)) {
...
/* neighbor interface */
int peer_interface_set(struct peer *peer, const char *str) {
	if(peer->ifname) {
		free(peer->ifname);
	}
...
int peer_local_as_set(struct peer *peer, as_t as, int no_prepend, int replace_as) {
	struct bgp *bgp = peer->bgp;
	struct peer_group *group;
	struct listnode *node, *nnode;

	if(peer_sort(peer) != BGP_PEER_EBGP && peer_sort(peer) != BGP_PEER_INTERNAL) {
	...
	if(peer->change_local_as == as && ((CHECK_FLAG(peer->flags, PEER_FLAG_LOCAL_AS_NO_PREPEND) && no_prepend) || (!CHECK_FLAG(peer->flags, PEER_FLAG_LOCAL_AS_NO_PREPEND) && !no_prepend))
	   && ((CHECK_FLAG(peer->flags, PEER_FLAG_LOCAL_AS_REPLACE_AS) && replace_as) || (!CHECK_FLAG(peer->flags, PEER_FLAG_LOCAL_AS_REPLACE_AS) && !replace_as))) {
		return 0;
	}
...
int peer_password_set(struct peer *peer, const char *password) {
	...
	if(peer->password && strcmp(peer->password, password) == 0 && !CHECK_FLAG(peer->sflags, PEER_STATUS_GROUP)) {
		return 0;
	}
...
int peer_password_unset(struct peer *peer) {
	...
	if(!CHECK_FLAG(peer->sflags, PEER_STATUS_GROUP)) {
		if(peer_group_active(peer) && peer->group->conf->password && strcmp(peer->group->conf->password, peer->password) == 0) {
			return BGP_ERR_PEER_GROUP_HAS_THE_FLAG;
		}
...
int peer_distribute_set(struct peer *peer, afi_t afi, safi_t safi, int direct, const char *name) {
	struct bgp_filter *filter;
	struct peer_group *group;
	struct listnode *node, *nnode;
...
static void peer_distribute_update(const char *name) {
	afi_t afi;
	safi_t safi;
	int direct;
			...
			for(afi = AFI_IP; afi < AFI_MAX; afi++) {
				for(safi = SAFI_UNICAST; safi < SAFI_MAX; safi++) {
					...
					for(direct = FILTER_IN; direct < FILTER_MAX; direct++) {
						if(filter->dlist[direct].name) {
							filter->dlist[direct].alist = access_list_lookup(afi, filter->dlist[direct].name);
						} else {
							filter->dlist[direct].alist = NULL;
						}
					}
...
int peer_prefix_list_set(struct peer *peer, afi_t afi, safi_t safi, int direct, const char *name) {
	struct bgp_filter *filter;
	struct peer_group *group;
	struct listnode *node, *nnode;
		...
		for(ALL_LIST_ELEMENTS(bgp->peer, node, nnode, peer)) {
			for(afi = AFI_IP; afi < AFI_MAX; afi++) {
				for(safi = SAFI_UNICAST; safi < SAFI_MAX; safi++) {
					...
					for(direct = FILTER_IN; direct < FILTER_MAX; direct++) {
						if(filter->plist[direct].name) {
							filter->plist[direct].plist = prefix_list_lookup(afi, filter->plist[direct].name);
						} else {
							filter->plist[direct].plist = NULL;
						}
					}
...
int peer_aslist_set(struct peer *peer, afi_t afi, safi_t safi, int direct, const char *name) {
	struct bgp_filter *filter;
	struct peer_group *group;
	struct listnode *node, *nnode;
...
static void peer_aslist_update(void) {
	afi_t afi;
	safi_t safi;
	int direct;
	struct listnode *mnode, *mnnode;
		...
		for(ALL_LIST_ELEMENTS(bgp->peer, node, nnode, peer)) {
			for(afi = AFI_IP; afi < AFI_MAX; afi++) {
				for(safi = SAFI_UNICAST; safi < SAFI_MAX; safi++) {
					...
					for(direct = FILTER_IN; direct < FILTER_MAX; direct++) {
						if(filter->aslist[direct].name) {
							filter->aslist[direct].aslist = as_list_lookup(filter->aslist[direct].name);
						} else {
							filter->aslist[direct].aslist = NULL;
						}
					}
...
int peer_route_map_set(struct peer *peer, afi_t afi, safi_t safi, int direct, const char *name) {
	struct bgp_filter *filter;
	struct peer_group *group;
	struct listnode *node, *nnode;
	...
	if(direct != RMAP_IN && direct != RMAP_OUT && direct != RMAP_IMPORT && direct != RMAP_EXPORT) {
		return BGP_ERR_INVALID_VALUE;
	}

	if((direct == RMAP_OUT || direct == RMAP_IMPORT) && peer_is_group_member(peer, afi, safi)) {
		return BGP_ERR_INVALID_FOR_PEER_GROUP_MEMBER;
	}
...
int peer_unsuppress_map_set(struct peer *peer, afi_t afi, safi_t safi, const char *name) {
	struct bgp_filter *filter;
	struct peer_group *group;
	struct listnode *node, *nnode;
...
int peer_maximum_prefix_set(struct peer *peer, afi_t afi, safi_t safi, u_int32_t max, u_char threshold, int warning, u_int16_t restart) {
	struct peer_group *group;
	struct listnode *node, *nnode;
...
int peer_maximum_prefix_unset(struct peer *peer, afi_t afi, safi_t safi) {
	...
	if(peer->af_group[afi][safi]) {
		if(CHECK_FLAG(peer->group->conf->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX)) {
			SET_FLAG(peer->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX);
		} else {
			UNSET_FLAG(peer->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX);
		}

		if(CHECK_FLAG(peer->group->conf->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX_WARNING)) {
			SET_FLAG(peer->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX_WARNING);
		} else {
			UNSET_FLAG(peer->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX_WARNING);
		}
...
int peer_clear(struct peer *peer) {
	if(!CHECK_FLAG(peer->flags, PEER_FLAG_SHUTDOWN)) {
		...
		peer->v_start = BGP_INIT_START_TIMER;
		if(BGP_IS_VALID_STATE_FOR_NOTIF(peer->status)) {
			bgp_notify_send(peer, BGP_NOTIFY_CEASE, BGP_NOTIFY_CEASE_ADMIN_RESET);
		} else {
			BGP_EVENT_ADD(peer, BGP_Stop);
		}
	}
...
int peer_clear_soft(struct peer *peer, afi_t afi, safi_t safi, enum bgp_clear_type stype) {
	...
	if(stype == BGP_CLEAR_SOFT_IN_ORF_PREFIX) {
		if(CHECK_FLAG(peer->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_SM_ADV) && (CHECK_FLAG(peer->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_RM_RCV) || CHECK_FLAG(peer->af_cap[afi][safi], PEER_CAP_ORF_PREFIX_RM_OLD_RCV))) {
			...
			if(filter->plist[FILTER_IN].plist) {
				if(CHECK_FLAG(peer->af_sflags[afi][safi], PEER_STATUS_ORF_PREFIX_SEND)) {
					bgp_route_refresh_send(peer, afi, safi, prefix_type, REFRESH_DEFER, 1);
				}
				bgp_route_refresh_send(peer, afi, safi, prefix_type, REFRESH_IMMEDIATE, 0);
			} else {
				if(CHECK_FLAG(peer->af_sflags[afi][safi], PEER_STATUS_ORF_PREFIX_SEND)) {
					bgp_route_refresh_send(peer, afi, safi, prefix_type, REFRESH_IMMEDIATE, 1);
				} else {
					bgp_route_refresh_send(peer, afi, safi, 0, 0, 0);
				}
			}
	...
	if(stype == BGP_CLEAR_SOFT_IN || stype == BGP_CLEAR_SOFT_BOTH || stype == BGP_CLEAR_SOFT_IN_ORF_PREFIX) {
...
#define ONE_YEAR_SECOND ONE_DAY_SECOND * 365
	...
	if(uptime1 < ONE_DAY_SECOND) {
		...
		snprintf(buf, len, "%02dy%02dw%dd", tm->tm_year - 70, tm->tm_yday / 7, tm->tm_yday - ((tm->tm_yday / 7) * 7));
	}
	return buf;
}

static void bgp_config_write_filter(struct vty *vty, struct peer *peer, afi_t afi, safi_t safi) {
	struct bgp_filter *filter;
	...
	if(filter->dlist[in].name) {
		if(!gfilter || !gfilter->dlist[in].name || strcmp(filter->dlist[in].name, gfilter->dlist[in].name) != 0) {
			vty_out(vty, " neighbor %s distribute-list %s in%s", addr, filter->dlist[in].name, VTY_NEWLINE);
		}
	}
	if(filter->dlist[out].name && !gfilter) {
		vty_out(vty, " neighbor %s distribute-list %s out%s", addr, filter->dlist[out].name, VTY_NEWLINE);
	}

	/* prefix-list. */
	if(filter->plist[in].name) {
		if(!gfilter || !gfilter->plist[in].name || strcmp(filter->plist[in].name, gfilter->plist[in].name) != 0) {
			vty_out(vty, " neighbor %s prefix-list %s in%s", addr, filter->plist[in].name, VTY_NEWLINE);
		}
	}
	if(filter->plist[out].name && !gfilter) {
		vty_out(vty, " neighbor %s prefix-list %s out%s", addr, filter->plist[out].name, VTY_NEWLINE);
	}

	/* route-map. */
	if(filter->map[RMAP_IN].name) {
		if(!gfilter || !gfilter->map[RMAP_IN].name || strcmp(filter->map[RMAP_IN].name, gfilter->map[RMAP_IN].name) != 0) {
			vty_out(vty, " neighbor %s route-map %s in%s", addr, filter->map[RMAP_IN].name, VTY_NEWLINE);
		}
	}
	if(filter->map[RMAP_OUT].name && !gfilter) {
		vty_out(vty, " neighbor %s route-map %s out%s", addr, filter->map[RMAP_OUT].name, VTY_NEWLINE);
	}
	if(filter->map[RMAP_IMPORT].name && !gfilter) {
		vty_out(vty, " neighbor %s route-map %s import%s", addr, filter->map[RMAP_IMPORT].name, VTY_NEWLINE);
	}
	if(filter->map[RMAP_EXPORT].name) {
		if(!gfilter || !gfilter->map[RMAP_EXPORT].name || strcmp(filter->map[RMAP_EXPORT].name, gfilter->map[RMAP_EXPORT].name) != 0) {
			vty_out(vty, " neighbor %s route-map %s export%s", addr, filter->map[RMAP_EXPORT].name, VTY_NEWLINE);
		}
	}

	/* unsuppress-map */
	if(filter->usmap.name && !gfilter) {
		vty_out(vty, " neighbor %s unsuppress-map %s%s", addr, filter->usmap.name, VTY_NEWLINE);
	}

	/* filter-list. */
	if(filter->aslist[in].name) {
		if(!gfilter || !gfilter->aslist[in].name || strcmp(filter->aslist[in].name, gfilter->aslist[in].name) != 0) {
			vty_out(vty, " neighbor %s filter-list %s in%s", addr, filter->aslist[in].name, VTY_NEWLINE);
		}
	}
	if(filter->aslist[out].name && !gfilter) {
		vty_out(vty, " neighbor %s filter-list %s out%s", addr, filter->aslist[out].name, VTY_NEWLINE);
	}
}

/* BGP peer configuration display function. */
static void bgp_config_write_peer(struct vty *vty, struct bgp *bgp, struct peer *peer, afi_t afi, safi_t safi) {
   ...
   ************************************/
	if(afi == AFI_IP && safi == SAFI_UNICAST) {
		...
		if(!peer_group_active(peer)) {
			...
			if(peer->as) {
				vty_out(vty, " neighbor %s remote-as %u%s", addr, peer->as, VTY_NEWLINE);
			}
		} else {
			if(!g_peer->as) {
				vty_out(vty, " neighbor %s remote-as %u%s", addr, peer->as, VTY_NEWLINE);
		...
		if(peer->change_local_as) {
			if(!peer_group_active(peer)) {
				vty_out(vty, " neighbor %s local-as %u%s%s%s", addr, peer->change_local_as, CHECK_FLAG(peer->flags, PEER_FLAG_LOCAL_AS_NO_PREPEND) ? " no-prepend" : "",
					CHECK_FLAG(peer->flags, PEER_FLAG_LOCAL_AS_REPLACE_AS) ? " replace-as" : "", VTY_NEWLINE);
			}
		}
		...
		/* Password. */
		if(peer->password) {
			if(!peer_group_active(peer) || !g_peer->password || strcmp(peer->password, g_peer->password) != 0) {
		...
		if(CHECK_FLAG(peer->flags, PEER_FLAG_PASSIVE)) {
			if(!peer_group_active(peer) || !CHECK_FLAG(g_peer->flags, PEER_FLAG_PASSIVE)) {
		...
		if(peer->gtsm_hops && !peer_group_active(peer)) {
			vty_out(vty, " neighbor %s ttl-security hops %d%s", addr, peer->gtsm_hops, VTY_NEWLINE);
		} else if(peer->ttl && !peer_group_active(peer)) {
			vty_out(vty, " neighbor %s ebgp-multihop %d%s", addr, peer->ttl, VTY_NEWLINE);
		}
		...
		/* Update-source. */
		if(peer->update_if) {
			if(!peer_group_active(peer) || !g_peer->update_if || strcmp(g_peer->update_if, peer->update_if) != 0) {
				vty_out(vty, " neighbor %s update-source %s%s", addr, peer->update_if, VTY_NEWLINE);
			}
		}
		if(peer->update_source) {
			if(!peer_group_active(peer) || !g_peer->update_source || sockunion_cmp(g_peer->update_source, peer->update_source) != 0) {
				vty_out(vty, " neighbor %s update-source %s%s", addr, sockunion2str(peer->update_source, buf, SU_ADDRSTRLEN), VTY_NEWLINE);
			}
		}

		/* advertisement-interval */
		if(CHECK_FLAG(peer->config, PEER_CONFIG_ROUTEADV) && !peer_group_active(peer)) {
			vty_out(vty, " neighbor %s advertisement-interval %d%s", addr, peer->v_routeadv, VTY_NEWLINE);
		}

		/* timers. */
		if(CHECK_FLAG(peer->config, PEER_CONFIG_TIMER) && !peer_group_active(peer)) {
			vty_out(vty, " neighbor %s timers %d %d%s", addr, peer->keepalive, peer->holdtime, VTY_NEWLINE);
		}

		if(CHECK_FLAG(peer->config, PEER_CONFIG_CONNECT) && !peer_group_active(peer)) {
			vty_out(vty, " neighbor %s timers connect %d%s", addr, peer->connect, VTY_NEWLINE);
		}

		/* Default weight. */
		if(CHECK_FLAG(peer->config, PEER_CONFIG_WEIGHT)) {
			if(!peer_group_active(peer) || g_peer->weight != peer->weight) {
				vty_out(vty, " neighbor %s weight %d%s", addr, peer->weight, VTY_NEWLINE);
			}
		}
		...
		if(CHECK_FLAG(peer->flags, PEER_FLAG_OVERRIDE_CAPABILITY)) {
			if(!peer_group_active(peer) || !CHECK_FLAG(g_peer->flags, PEER_FLAG_OVERRIDE_CAPABILITY)) {
   ...
   ************************************/
	...
	/* ORF capability.  */
	if(CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_ORF_PREFIX_SM) || CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_ORF_PREFIX_RM)) {
		if(!peer->af_group[afi][safi]) {
	...
	if(peer_af_flag_check(peer, afi, safi, PEER_FLAG_REFLECTOR_CLIENT) && !peer->af_group[afi][safi]) {
		vty_out(vty, " neighbor %s route-reflector-client%s", addr, VTY_NEWLINE);
	}

	/* Nexthop self. */
	if(peer_af_flag_check(peer, afi, safi, PEER_FLAG_NEXTHOP_SELF) && !peer->af_group[afi][safi]) {
		vty_out(vty, " neighbor %s next-hop-self%s%s", addr, peer_af_flag_check(peer, afi, safi, PEER_FLAG_NEXTHOP_SELF_ALL) ? " all" : "", VTY_NEWLINE);
	}
	...
	if(!peer->af_group[afi][safi]) {
		if(bgp_option_check(BGP_OPT_CONFIG_CISCO)) {
			if(peer_af_flag_check(peer, afi, safi, PEER_FLAG_SEND_COMMUNITY) && peer_af_flag_check(peer, afi, safi, PEER_FLAG_SEND_EXT_COMMUNITY) && peer_af_flag_check(peer, afi, safi, PEER_FLAG_SEND_LARGE_COMMUNITY)) {
				...
				vty_out(vty, " neighbor %s send-community extended%s", addr, VTY_NEWLINE);
			} else if(peer_af_flag_check(peer, afi, safi, PEER_FLAG_SEND_LARGE_COMMUNITY)) {
				vty_out(vty, " neighbor %s send-community large%s", addr, VTY_NEWLINE);
			...
			if(!peer_af_flag_check(peer, afi, safi, PEER_FLAG_SEND_COMMUNITY) && !peer_af_flag_check(peer, afi, safi, PEER_FLAG_SEND_EXT_COMMUNITY) && !peer_af_flag_check(peer, afi, safi, PEER_FLAG_SEND_LARGE_COMMUNITY)) {
				vty_out(vty, " no neighbor %s send-community all%s", addr, VTY_NEWLINE);
			} else if(!peer_af_flag_check(peer, afi, safi, PEER_FLAG_SEND_EXT_COMMUNITY)) {
				vty_out(vty, " no neighbor %s send-community extended%s", addr, VTY_NEWLINE);
			} else if(!peer_af_flag_check(peer, afi, safi, PEER_FLAG_SEND_LARGE_COMMUNITY)) {
				vty_out(vty, " no neighbor %s send-community large%s", addr, VTY_NEWLINE);
			} else if(!peer_af_flag_check(peer, afi, safi, PEER_FLAG_SEND_COMMUNITY)) {
	...
	/* Default information */
	if(peer_af_flag_check(peer, afi, safi, PEER_FLAG_DEFAULT_ORIGINATE) && !peer->af_group[afi][safi]) {
	...
	if(CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_SOFT_RECONFIG)) {
		if(!peer->af_group[afi][safi] || !CHECK_FLAG(g_peer->af_flags[afi][safi], PEER_FLAG_SOFT_RECONFIG)) {
			vty_out(vty, " neighbor %s soft-reconfiguration inbound%s", addr, VTY_NEWLINE);
		}
	}

	/* maximum-prefix. */
	if(CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX)) {
		if(!peer->af_group[afi][safi] || g_peer->pmax[afi][safi] != peer->pmax[afi][safi] || g_peer->pmax_threshold[afi][safi] != peer->pmax_threshold[afi][safi]
		   || CHECK_FLAG(g_peer->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX_WARNING) != CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_MAX_PREFIX_WARNING)) {
	...
	/* Route server client. */
	if(CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_RSERVER_CLIENT) && !peer->af_group[afi][safi]) {
		vty_out(vty, " neighbor %s route-server-client%s", addr, VTY_NEWLINE);
	}

	/* Nexthop-local unchanged. */
	if(CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_NEXTHOP_LOCAL_UNCHANGED) && !peer->af_group[afi][safi]) {
	...
	if(peer_af_flag_check(peer, afi, safi, PEER_FLAG_ALLOWAS_IN)) {
		if(!peer_group_active(peer) || !peer_af_flag_check(g_peer, afi, safi, PEER_FLAG_ALLOWAS_IN) || peer->allowas_in[afi][safi] != g_peer->allowas_in[afi][safi]) {
			if(peer->allowas_in[afi][safi] == 3) {
				vty_out(vty, " neighbor %s allowas-in%s", addr, VTY_NEWLINE);
			} else {
				vty_out(vty, " neighbor %s allowas-in %d%s", addr, peer->allowas_in[afi][safi], VTY_NEWLINE);
			}
		}
	}
	...
	/* atribute-unchanged. */
	if((CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_AS_PATH_UNCHANGED) || CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_NEXTHOP_UNCHANGED) || CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_MED_UNCHANGED))
	   && !peer->af_group[afi][safi]) {
		if(CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_AS_PATH_UNCHANGED) && CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_NEXTHOP_UNCHANGED) && CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_MED_UNCHANGED)) {
			vty_out(vty, " neighbor %s attribute-unchanged%s", addr, VTY_NEWLINE);
		} else {
			vty_out(vty, " neighbor %s attribute-unchanged%s%s%s%s", addr, (CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_AS_PATH_UNCHANGED)) ? " as-path" : "",
				(CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_NEXTHOP_UNCHANGED)) ? " next-hop" : "", (CHECK_FLAG(peer->af_flags[afi][safi], PEER_FLAG_MED_UNCHANGED)) ? " med" : "", VTY_NEWLINE);
		}
	}
}
...
static int bgp_config_write_family(struct vty *vty, struct bgp *bgp, afi_t afi, safi_t safi) {
	int write = 0;
	struct peer *peer;
	struct peer_group *group;
	struct listnode *node, *nnode;
...
int bgp_config_write(struct vty *vty) {
	...
	for(ALL_LIST_ELEMENTS(bm->bgp, mnode, mnnode, bgp)) {
		...
		if(CHECK_FLAG(bgp->config, BGP_CONFIG_ROUTER_ID)) {
			vty_out(vty, " bgp router-id %s%s", inet_ntoa(bgp->router_id), VTY_NEWLINE);
		}
		...
		if(CHECK_FLAG(bgp->config, BGP_CONFIG_CLUSTER_ID)) {
			vty_out(vty, " bgp cluster-id %s%s", inet_ntoa(bgp->cluster_id), VTY_NEWLINE);
		}

		/* Confederation identifier*/
		if(CHECK_FLAG(bgp->config, BGP_CONFIG_CONFEDERATION)) {
			vty_out(vty, " bgp confederation identifier %i%s", bgp->confed_id, VTY_NEWLINE);
		}
		...
		if(bgp->stalepath_time != BGP_DEFAULT_STALEPATH_TIME) {
			vty_out(vty, " bgp graceful-restart stalepath-time %d%s", bgp->stalepath_time, VTY_NEWLINE);
		}
		if(bgp->restart_time != BGP_DEFAULT_RESTART_TIME) {
			vty_out(vty, " bgp graceful-restart restart-time %d%s", bgp->restart_time, VTY_NEWLINE);
		}
		...
		if(bgp_flag_check(bgp, BGP_FLAG_RR_ALLOW_OUTBOUND_POLICY)) {
			vty_out(vty, " bgp route-reflector allow-outbound-policy%s", VTY_NEWLINE);
		}
		if(bgp_flag_check(bgp, BGP_FLAG_COMPARE_ROUTER_ID)) {
			vty_out(vty, " bgp bestpath compare-routerid%s", VTY_NEWLINE);
		}
		if(bgp_flag_check(bgp, BGP_FLAG_MED_CONFED) || bgp_flag_check(bgp, BGP_FLAG_MED_MISSING_AS_WORST)) {
		...
		if(bgp->default_keepalive != BGP_DEFAULT_KEEPALIVE && bgp->default_holdtime != BGP_DEFAULT_HOLDTIME) {
			vty_out(vty, " timers bgp %d %d%s", bgp->default_keepalive, bgp->default_holdtime, VTY_NEWLINE);
		}
