 ...
 */
static int bgp_md5_set_socket(int socket, union sockunion *su, const char *password) {
	int ret = -1;
	int en = ENOSYS;
	...
	if(ret < 0) {
		zlog(NULL, LOG_WARNING, "can't set TCP_MD5SIG option on socket %d: %s", socket, safe_strerror(en));
	}
...
static int bgp_md5_set_connect(int socket, union sockunion *su, const char *password) {
	int ret = -1;
...
static void bgp_update_sock_send_buffer_size(int fd) {
...
void bgp_set_socket_ttl(struct peer *peer, int bgp_sock) {
	...
	if(ret) {
		zlog_err("%s: Can't set TxTTL on peer (rtrid %s) socket, err = %d", __func__, inet_ntop(AF_INET, &peer->remote_id, buf, sizeof(buf)), errno);
	}

	ret = sockopt_minttl(peer->su.sa.sa_family, bgp_sock, minttl);
	if(ret && (errno != ENOTSUP || minttl)) {
		zlog_err("%s: Can't set MinTTL on peer (rtrid %s) socket, err = %d", __func__, inet_ntop(AF_INET, &peer->remote_id, buf, sizeof(buf)), errno);
	}
}
...
static int bgp_accept(struct thread *thread) {
	...
	if(BGP_DEBUG(events, EVENTS)) {
		zlog_debug("[Event] BGP connection from host %s:%d", inet_sutop(&su, buf), sockunion_get_port(&su));
	}
   ...
   */
	if(!peer1 || peer1->status == Idle || peer1->status > Established) {
		if(BGP_DEBUG(events, EVENTS)) {
			if(!peer1) {
				zlog_debug("[Event] BGP connection IP address %s is not configured", inet_sutop(&su, buf));
			} else {
				zlog_debug("[Event] BGP connection IP address %s is %s state", inet_sutop(&su, buf), LOOKUP(bgp_status_msg, peer1->status));
			}
		}
		close(bgp_sock);
		return -1;
...
#define PEER_ARRAY_COPY(D, S, A) memcpy((D)->A, (S)->A, sizeof(((D)->A)[0][0]) * AFI_MAX * SAFI_MAX);
...
static int bgp_bind(struct peer *peer) {
	...
	ret = setsockopt(peer->fd, SOL_SOCKET, SO_BINDTODEVICE, &ifreq, sizeof(ifreq));
	myerrno = errno;
	...
	if(ret < 0) {
		zlog(peer->log, LOG_INFO, "bind to interface %s failed, errno=%d", peer->ifname, myerrno);
		return ret;
	}
...
static int bgp_update_address(struct interface *ifp, const union sockunion *dst, union sockunion *addr) {
...
static void bgp_update_source(struct peer *peer) {
...
int bgp_getsockname(struct peer *peer) {
	...
	return bgp_nexthop_set(peer->su_local, peer->su_remote, &peer->nexthop, peer);
}
