...
static int bgp_router_id_update(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
...
static int bgp_read_nexthop_update(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	bgp_parse_nexthop_update();
	return 0;
}

/* Inteface addition message from zebra. */
static int bgp_interface_add(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
...
static int bgp_interface_delete(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct stream *s;
	struct interface *ifp;
...
static int bgp_interface_up(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct stream *s;
	struct interface *ifp;
	struct connected *c;
...
static int bgp_interface_down(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct stream *s;
	struct interface *ifp;
	struct connected *c;
	struct listnode *node, *nnode;
...
static int bgp_interface_address_add(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	...
	if(BGP_DEBUG(zebra, ZEBRA)) {
		...
		zlog_debug("Zebra rcvd: interface %s address add %s", ifc->ifp->name, buf);
	}
...
static int bgp_interface_address_delete(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct connected *ifc;
	...
	if(BGP_DEBUG(zebra, ZEBRA)) {
		char buf[128];
		prefix2str(ifc->address, buf, sizeof(buf));
		zlog_debug("Zebra rcvd: interface %s address delete %s", ifc->ifp->name, buf);
	}
...
static int zebra_read_ipv4(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct stream *s;
	...
	if(command == ZEBRA_IPV4_ROUTE_ADD) {
		if(BGP_DEBUG(zebra, ZEBRA)) {
			...
			zlog_debug(
				"Zebra rcvd: IPv4 route add %s %s/%d nexthop %s metric %u tag %d", zebra_route_string(api.type), inet_ntop(AF_INET, &p.prefix, buf[0], sizeof(buf[0])), p.prefixlen,
				inet_ntop(AF_INET, &nexthop, buf[1], sizeof(buf[1])), api.metric, api.tag
			);
		}
		bgp_redistribute_add((struct prefix *) &p, &nexthop, NULL, api.metric, api.type, api.tag);
	} else {
		if(BGP_DEBUG(zebra, ZEBRA)) {
			char buf[2][INET_ADDRSTRLEN];
			zlog_debug(
				"Zebra rcvd: IPv4 route delete %s %s/%d "
				"nexthop %s metric %u tag %d",
				zebra_route_string(api.type), inet_ntop(AF_INET, &p.prefix, buf[0], sizeof(buf[0])), p.prefixlen, inet_ntop(AF_INET, &nexthop, buf[1], sizeof(buf[1])), api.metric, api.tag
			);
		}
...
/* Zebra route add and delete treatment. */
static int zebra_read_ipv6(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct stream *s;
	...
	if(command == ZEBRA_IPV6_ROUTE_ADD) {
		if(BGP_DEBUG(zebra, ZEBRA)) {
			...
			zlog_debug(
				"Zebra rcvd: IPv6 route add %s %s/%d nexthop %s metric %u tag %d", zebra_route_string(api.type), inet_ntop(AF_INET6, &p.prefix, buf[0], sizeof(buf[0])), p.prefixlen,
				inet_ntop(AF_INET, &nexthop, buf[1], sizeof(buf[1])), api.metric, api.tag
			);
		}
		bgp_redistribute_add((struct prefix *) &p, NULL, &nexthop, api.metric, api.type, api.tag);
	} else {
		if(BGP_DEBUG(zebra, ZEBRA)) {
			char buf[2][INET6_ADDRSTRLEN];
			zlog_debug(
				"Zebra rcvd: IPv6 route delete %s %s/%d "
				"nexthop %s metric %u tag %d",
				zebra_route_string(api.type), inet_ntop(AF_INET6, &p.prefix, buf[0], sizeof(buf[0])), p.prefixlen, inet_ntop(AF_INET6, &nexthop, buf[1], sizeof(buf[1])), api.metric, api.tag
			);
		}
		bgp_redistribute_delete((struct prefix *) &p, api.type);
	}
...
int bgp_nexthop_set(union sockunion *local, union sockunion *remote, struct bgp_nexthop *nexthop, struct peer *peer) {
	int ret = 0;
...
#ifdef KAME
	if(IN6_IS_ADDR_LINKLOCAL(&nexthop->v6_global) && IN6_LINKLOCAL_IFINDEX(nexthop->v6_global)) {
		SET_IN6_LINKLOCAL_IFINDEX(nexthop->v6_global, 0);
	}
	if(IN6_IS_ADDR_LINKLOCAL(&nexthop->v6_local) && IN6_LINKLOCAL_IFINDEX(nexthop->v6_local)) {
...
void bgp_zebra_announce(struct prefix *p, struct bgp_info *info, struct bgp *bgp, safi_t safi) {
	int flags;
	u_char distance;
	struct peer *peer;
	...
	if(p->family == AF_INET) {
		...
		if((oldsize = stream_get_size(bgp_nexthop_buf)) < (sizeof(struct in_addr *) * nhcount)) {
		...
		if(BGP_DEBUG(zebra, ZEBRA)) {
			...
			char buf[2][INET_ADDRSTRLEN];
			zlog_debug(
				"Zebra send: IPv4 route add %s/%d nexthop %s metric %u"
				" tag %u count %d",
				inet_ntop(AF_INET, &p->u.prefix4, buf[0], sizeof(buf[0])), p->prefixlen, inet_ntop(AF_INET, api.nexthop[0], buf[1], sizeof(buf[1])), api.metric, api.tag, api.nexthop_num
			);
			for(i = 1; i < api.nexthop_num; i++) {
				zlog_debug("Zebra send: IPv4 route add [nexthop %d] %s", i, inet_ntop(AF_INET, api.nexthop[i], buf[1], sizeof(buf[1])));
			}
		}
	...
	if(p->family == AF_INET6) {
		...
		/* resize nexthop buffer size if necessary */
		if((oldsize = stream_get_size(bgp_nexthop_buf)) < (sizeof(struct in6_addr *) * nhcount)) {
		...
		if((oldsize = stream_get_size(bgp_ifindices_buf)) < (sizeof(unsigned int) * nhcount)) {
		...
		if(info->attr->extra->mp_nexthop_len == 32) {
			...
			if(IN6_IS_ADDR_UNSPECIFIED(&info->attr->extra->mp_nexthop_global) && peer->su_remote->sa.sa_family == AF_INET6) {
		...
		if(BGP_DEBUG(zebra, ZEBRA)) {
			...
			zlog_debug(
				...
				" tag %u",
				inet_ntop(AF_INET6, &p->u.prefix6, buf[0], sizeof(buf[0])), p->prefixlen, inet_ntop(AF_INET6, nexthop, buf[1], sizeof(buf[1])), api.metric, api.tag
			);
		}

		zapi_ipv6_route(ZEBRA_IPV6_ROUTE_ADD, zclient, (struct prefix_ipv6 *) p, &api);
	}
}
...
void bgp_zebra_withdraw(struct prefix *p, struct bgp_info *info, safi_t safi) {
	...
	if(p->family == AF_INET) {
		...
		if(BGP_DEBUG(zebra, ZEBRA)) {
			char buf[2][INET_ADDRSTRLEN];
			zlog_debug("Zebra send: IPv4 route delete %s/%d metric %u tag %d", inet_ntop(AF_INET, &p->u.prefix4, buf[0], sizeof(buf[0])), p->prefixlen, api.metric, api.tag);
		}
	...
	if(p->family == AF_INET6) {
		...
		if(BGP_DEBUG(zebra, ZEBRA)) {
			char buf[2][INET6_ADDRSTRLEN];
			zlog_debug("Zebra send: IPv6 route delete %s/%d metric %u tag %d", inet_ntop(AF_INET6, &p->u.prefix6, buf[0], sizeof(buf[0])), p->prefixlen, api.metric, api.tag);
		}

		zapi_ipv6_route(ZEBRA_IPV6_ROUTE_DELETE, zclient, (struct prefix_ipv6 *) p, &api);
	}
}
...
int bgp_redistribute_rmap_set(struct bgp *bgp, afi_t afi, int type, const char *name) {
...
int bgp_redistribute_metric_set(struct bgp *bgp, afi_t afi, int type, u_int32_t metric) {
...
int bgp_redistribute_unset(struct bgp *bgp, afi_t afi, int type) {
	...
	if(bgp->redist[AFI_IP][type] == 0 && bgp->redist[AFI_IP6][type] == 0 && zclient->sock >= 0) {
