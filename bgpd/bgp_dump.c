...
static int bgp_dump_interval_add(struct bgp_dump *bgp_dump, int interval) {
	...
	if(interval > 0) {
	   ...
	   */
		...
		bgp_dump->t_interval = thread_add_timer(bm->master, bgp_dump_interval_func, bgp_dump, interval);
	} else {
		/* One-off dump: execute immediately, don't affect any scheduled dumps */
		bgp_dump->t_interval = thread_add_event(bm->master, bgp_dump_interval_func, bgp_dump, 0);
	}
...
/* Dump common header. */
static void bgp_dump_header(struct stream *obuf, int type, int subtype, int dump_type) {
	struct timeval clock;
	long msecs;
	time_t secs;

	if((dump_type == BGP_DUMP_ALL_ET || dump_type == BGP_DUMP_UPDATES_ET) && type == MSG_PROTOCOL_BGP4MP) {
...
static void bgp_dump_routes_index_table(struct bgp *bgp) {
	...
	/* MRT header */
	bgp_dump_header(obuf, MSG_TABLE_DUMP_V2, TABLE_DUMP_V2_PEER_INDEX_TABLE, BGP_DUMP_ROUTES);
	...
	for(ALL_LIST_ELEMENTS_RO(bgp->peer, node, peer)) {
		...
		if(sockunion_family(&peer->su) == AF_INET) {
			...
			stream_write(obuf, (u_char *) &peer->su.sin6.sin6_addr, IPV6_MAX_BYTELEN);
		}
...
static struct bgp_info *bgp_dump_route_node_record(int afi, struct bgp_node *rn, struct bgp_info *info, unsigned int seq) {
	struct stream *obuf;
	size_t sizep;
	size_t endp;
	...
	for(; info; info = info->next) {
		...
		if(cur_endp > BGP_MAX_PACKET_SIZE + BGP_DUMP_MSG_HEADER + BGP_DUMP_HEADER_SIZE) {
...
static unsigned int bgp_dump_routes_func(int afi, int first_run, unsigned int seq) {
...
static void bgp_dump_common(struct stream *obuf, struct peer *peer, int forceas4) {
...
void bgp_dump_state(struct peer *peer, int status_old, int status_new) {
	...
	bgp_dump_header(obuf, MSG_PROTOCOL_BGP4MP, BGP4MP_STATE_CHANGE_AS4, bgp_dump_all.type);
...
static void bgp_dump_packet_func(struct bgp_dump *bgp_dump, struct peer *peer, struct stream *packet) {
	struct stream *obuf;
	...
	if(CHECK_FLAG(peer->cap, PEER_CAP_AS4_RCV)) {
		...
		bgp_dump_header(obuf, MSG_PROTOCOL_BGP4MP, BGP4MP_MESSAGE, bgp_dump->type);
	}
...
static int bgp_dump_set(struct vty *vty, struct bgp_dump *bgp_dump, enum bgp_dump_type type, const char *path, const char *interval_str) {
	unsigned int interval;

	/* Don't schedule duplicate dumps if the dump command is given twice */
	if(bgp_dump->filename && strcmp(path, bgp_dump->filename) == 0 && type == bgp_dump->type) {
		if(interval_str) {
			if(bgp_dump->interval_str && strcmp(bgp_dump->interval_str, interval_str) == 0) {
				return CMD_SUCCESS;
			}
		} else {
...
DEFUN(dump_bgp_all, dump_bgp_all_cmd, "dump bgp (all|all-et|updates|updates-et|routes-mrt) PATH [INTERVAL]",
      "Dump packet\n"
      "BGP packet dump\n"
	...
	switch(bgp_dump_type) {
		case BGP_DUMP_ALL:
		case BGP_DUMP_ALL_ET: bgp_dump_struct = &bgp_dump_all; break;
		case BGP_DUMP_UPDATES:
		case BGP_DUMP_UPDATES_ET: bgp_dump_struct = &bgp_dump_updates; break;
		case BGP_DUMP_ROUTES:
		default: bgp_dump_struct = &bgp_dump_routes; break;
	}
	...
	return bgp_dump_set(vty, bgp_dump_struct, bgp_dump_type, argv[1], interval);
}

DEFUN(no_dump_bgp_all, no_dump_bgp_all_cmd, "no dump bgp (all|updates|routes-mrt) [PATH] [INTERVAL]",
      NO_STR "Stop dump packet\n"
...
static int config_write_bgp_dump(struct vty *vty) {
	if(bgp_dump_all.filename) {
		...
		if(bgp_dump_all.interval_str) {
			vty_out(vty, "dump bgp %s %s %s%s", type_str, bgp_dump_all.filename, bgp_dump_all.interval_str, VTY_NEWLINE);
		} else {
			vty_out(vty, "dump bgp %s %s%s", type_str, bgp_dump_all.filename, VTY_NEWLINE);
		}
	}
	if(bgp_dump_updates.filename) {
		...
		if(bgp_dump_updates.interval_str) {
			vty_out(vty, "dump bgp %s %s %s%s", type_str, bgp_dump_updates.filename, bgp_dump_updates.interval_str, VTY_NEWLINE);
		} else {
			vty_out(vty, "dump bgp updates %s%s", bgp_dump_updates.filename, VTY_NEWLINE);
		}
	}
	if(bgp_dump_routes.filename) {
		if(bgp_dump_routes.interval_str) {
			vty_out(vty, "dump bgp routes-mrt %s %s%s", bgp_dump_routes.filename, bgp_dump_routes.interval_str, VTY_NEWLINE);
		}
	}
	return 0;
}
...
void bgp_dump_init(void) {
	...
	bgp_dump_obuf = stream_new((BGP_MAX_PACKET_SIZE << 1) + BGP_DUMP_MSG_HEADER + BGP_DUMP_HEADER_SIZE);
