	...
	while(s->getp < lim) {
		...
		switch(type) {
			...
			case BGP_ATTR_NEXT_HOP:
				...
				break;
			default: stream_getw_from(s, length); break;
		}
	}
...
int main(int argc, char **argv) {
	...
	while(1) {
		...
		if(type == MSG_TABLE_DUMP) {
			switch(subtype) {
				case AFI_IP: printf("/AFI_IP\n"); break;
				case AFI_IP6: printf("/AFI_IP6\n"); break;
				default: printf("/UNKNOWN %d", subtype); break;
			}
		} else {
			switch(subtype) {
				case BGP4MP_STATE_CHANGE: printf("/CHANGE\n"); break;
				case BGP4MP_MESSAGE: printf("/MESSAGE\n"); break;
				case BGP4MP_ENTRY: printf("/ENTRY\n"); break;
				case BGP4MP_SNAPSHOT: printf("/SNAPSHOT\n"); break;
				default: printf("/UNKNOWN %d", subtype); break;
			}
		}
