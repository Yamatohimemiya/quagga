...
static void bgp_connect_check(struct peer *peer) {
	...
	if(status == 0) {
		...
		if(BGP_DEBUG(events, EVENTS)) {
			plog_debug(peer->log, "%s [Event] Connect failed (%s)", peer->host, safe_strerror(errno));
		}
...
static struct stream *bgp_update_packet(struct peer *peer, afi_t afi, safi_t safi) {
	...
	while(adv) {
		...
		space_remaining = STREAM_CONCAT_REMAIN(s, snlri, STREAM_SIZE(s)) - BGP_MAX_PACKET_SIZE_OVERFLOW;
	   ...
	   */
			...
			total_attr_len = bgp_packet_attribute(NULL, peer, s, adv->baa->attr, ((afi == AFI_IP && safi == SAFI_UNICAST) ? &rn->p : NULL), afi, safi, from, prd, tag);
			space_remaining = STREAM_CONCAT_REMAIN(s, snlri, STREAM_SIZE(s)) - BGP_MAX_PACKET_SIZE_OVERFLOW;
			space_needed = BGP_NLRI_LENGTH + bgp_packet_mpattr_prefix_size(afi, safi, &rn->p);
			;
           ...
           * return */
			if(space_remaining < space_needed) {
				zlog_err(
					"%s cannot send UPDATE, the attributes do not leave "
					"room for NLRI",
					peer->host
				);
		...
		if(afi == AFI_IP && safi == SAFI_UNICAST) {
			...
			if(stream_empty(snlri)) {
				mpattrlen_pos = bgp_packet_mpattr_start(snlri, afi, safi, adv->baa->attr);
			}
		...
		if(BGP_DEBUG(update, UPDATE_OUT)) {
			...
			zlog(peer->log, LOG_DEBUG, "%s send UPDATE %s/%d", peer->host, inet_ntop(rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ), rn->p.prefixlen);
		}
...
static struct stream *bgp_update_packet_eor(struct peer *peer, afi_t afi, safi_t safi) {
	struct stream *s;
...
*/
static struct stream *bgp_withdraw_packet(struct peer *peer, afi_t afi, safi_t safi) {
	struct stream *s;
	struct stream *packet;
	struct bgp_adj_out *adj;
	struct bgp_advertise *adv;
	...
	while((adv = BGP_ADV_FIFO_HEAD(&peer->sync[afi][safi]->withdraw)) != NULL) {
		...
		space_remaining = STREAM_REMAIN(s) - BGP_MAX_PACKET_SIZE_OVERFLOW;
		space_needed = (BGP_NLRI_LENGTH + BGP_TOTAL_ATTR_LEN + bgp_packet_mpattr_prefix_size(afi, safi, &rn->p));
		...
		if(BGP_DEBUG(update, UPDATE_OUT)) {
			...
			zlog(peer->log, LOG_DEBUG, "%s send UPDATE %s/%d -- unreachable", peer->host, inet_ntop(rn->p.family, &(rn->p.u.prefix), buf, INET6_BUFSIZ), rn->p.prefixlen);
		}
...
void bgp_default_update_send(struct peer *peer, struct attr *attr, afi_t afi, safi_t safi, struct peer *from) {
	struct stream *s;
	struct prefix p;
	unsigned long pos;
	bgp_size_t total_attr_len;
	...
	if(BGP_DEBUG(update, UPDATE_OUT)) {
		...
		zlog(peer->log, LOG_DEBUG, "%s send UPDATE %s/%d %s", peer->host, inet_ntop(p.family, &(p.u.prefix), buf, INET6_BUFSIZ), p.prefixlen, attrstr);
	}
...
void bgp_default_withdraw_send(struct peer *peer, afi_t afi, safi_t safi) {
	...
	if(BGP_DEBUG(update, UPDATE_OUT)) {
		...
		zlog(peer->log, LOG_DEBUG, "%s send UPDATE %s/%d -- unreachable", peer->host, inet_ntop(p.family, &(p.u.prefix), buf, INET6_BUFSIZ), p.prefixlen);
	}
		...
		for(safi = SAFI_UNICAST; safi < SAFI_MAX; safi++) {
			...
			if(adv) {
				if(adv->binfo && adv->binfo->uptime < peer->synctime) {
					if(CHECK_FLAG(adv->binfo->peer->cap, PEER_CAP_RESTART_RCV) && CHECK_FLAG(adv->binfo->peer->cap, PEER_CAP_RESTART_ADV)
					   && !(CHECK_FLAG(adv->binfo->peer->cap, PEER_CAP_RESTART_BIT_RCV) && CHECK_FLAG(adv->binfo->peer->cap, PEER_CAP_RESTART_BIT_ADV)) && !CHECK_FLAG(adv->binfo->flags, BGP_INFO_STALE)
					   && safi != SAFI_MPLS_VPN) {
...
//							}
			...
			if(CHECK_FLAG(peer->cap, PEER_CAP_RESTART_RCV)) {
				if(peer->afc_nego[afi][safi] && peer->synctime && !CHECK_FLAG(peer->af_sflags[afi][safi], PEER_STATUS_EOR_SEND) && safi != SAFI_MPLS_VPN) {
...
int bgp_write(struct thread *thread) {
	...
	do {
		...
		switch(type) {
			case BGP_MSG_OPEN: peer->open_out++; break;
			case BGP_MSG_UPDATE: peer->update_out++; break;
			case BGP_MSG_NOTIFY:
				peer->notify_out++;
			...
			case BGP_MSG_KEEPALIVE: peer->keepalive_out++; break;
			case BGP_MSG_ROUTE_REFRESH_NEW:
			case BGP_MSG_ROUTE_REFRESH_OLD: peer->refresh_out++; break;
			case BGP_MSG_CAPABILITY: peer->dynamic_cap_out++; break;
		}
...
static int bgp_write_notify(struct peer *peer) {
   ...
   * to write the entire NOTIFY doesn't get different FSM treatment */
	...
	val = 1;
	(void) setsockopt(peer->fd, IPPROTO_TCP, TCP_NODELAY, (char *) &val, sizeof(val));
...
void bgp_keepalive_send(struct peer *peer) {
	...
	if(BGP_DEBUG(normal, NORMAL)) {
		zlog_debug("%s send message type %d, length (incl. header) %d", peer->host, BGP_MSG_KEEPALIVE, length);
	}
...
void bgp_open_send(struct peer *peer) {
	...
	stream_putw(s, (local_as <= BGP_AS_MAX) ? (u_int16_t) local_as : BGP_AS_TRANS);
	...
	if(BGP_DEBUG(normal, NORMAL)) {
		zlog_debug("%s sending OPEN, version %d, my as %u, holdtime %d, id %s", peer->host, BGP_VERSION_4, local_as, send_holdtime, inet_ntoa(peer->local_id));
	}

	if(BGP_DEBUG(normal, NORMAL)) {
		zlog_debug("%s send message type %d, length (incl. header) %d", peer->host, BGP_MSG_OPEN, length);
	}
...
void bgp_notify_send_with_data(struct peer *peer, u_char code, u_char sub_code, u_char *data, size_t datalen) {
	...
	if(BGP_DEBUG(normal, NORMAL)) {
		zlog_debug("%s send message type %d, length (incl. header) %d", peer->host, BGP_MSG_NOTIFY, length);
	}
	...
	if(sub_code != BGP_NOTIFY_CEASE_CONFIG_CHANGE) {
		if(sub_code == BGP_NOTIFY_CEASE_ADMIN_RESET) {
			...
			zlog_info("Notification sent to neighbor %s:%u: User reset", peer->host, sockunion_get_port(&peer->su));
		} else if(sub_code == BGP_NOTIFY_CEASE_ADMIN_SHUTDOWN) {
			peer->last_reset = PEER_DOWN_USER_SHUTDOWN;
			zlog_info("Notification sent to neighbor %s:%u shutdown", peer->host, sockunion_get_port(&peer->su));
		} else {
			peer->last_reset = PEER_DOWN_NOTIFY_SEND;
			zlog_info("Notification sent to neighbor %s:%u: type %u/%u", peer->host, sockunion_get_port(&peer->su), code, sub_code);
		}
	} else {
		zlog_info("Notification sent to neighbor %s:%u: configuration change", peer->host, sockunion_get_port(&peer->su));
	}
...
void bgp_route_refresh_send(struct peer *peer, afi_t afi, safi_t safi, u_char orf_type, u_char when_to_refresh, int remove) {
	...
	if(orf_type == ORF_TYPE_PREFIX || orf_type == ORF_TYPE_PREFIX_OLD) {
		if(remove || filter->plist[FILTER_IN].plist) {
			...
			if(remove) {
				...
				if(BGP_DEBUG(normal, NORMAL)) {
					zlog_debug("%s sending REFRESH_REQ to remove ORF(%d) (%s) for afi/safi: %d/%d", peer->host, orf_type, (when_to_refresh == REFRESH_DEFER ? "defer" : "immediate"), afi, safi);
				}
			} else {
				SET_FLAG(peer->af_sflags[afi][safi], PEER_STATUS_ORF_PREFIX_SEND);
				prefix_bgp_orf_entry(s, filter->plist[FILTER_IN].plist, ORF_COMMON_PART_ADD, ORF_COMMON_PART_PERMIT, ORF_COMMON_PART_DENY);
				if(BGP_DEBUG(normal, NORMAL)) {
					zlog_debug("%s sending REFRESH_REQ with pfxlist ORF(%d) (%s) for afi/safi: %d/%d", peer->host, orf_type, (when_to_refresh == REFRESH_DEFER ? "defer" : "immediate"), afi, safi);
				}
			}
	...
	if(BGP_DEBUG(normal, NORMAL)) {
		if(!orf_refresh) {
			zlog_debug("%s sending REFRESH_REQ for afi/safi: %d/%d", peer->host, afi, safi);
		}
		zlog_debug("%s send message type %d, length (incl. header) %d", peer->host, CHECK_FLAG(peer->cap, PEER_CAP_REFRESH_NEW_RCV) ? BGP_MSG_ROUTE_REFRESH_NEW : BGP_MSG_ROUTE_REFRESH_OLD, length);
	}
...
void bgp_capability_send(struct peer *peer, afi_t afi, safi_t safi, int capability_code, int action) {
	...
	if(capability_code == CAPABILITY_CODE_MP) {
		...
		if(BGP_DEBUG(normal, NORMAL)) {
			zlog_debug("%s sending CAPABILITY has %s MP_EXT CAP for afi/safi: %d/%d", peer->host, action == CAPABILITY_ACTION_SET ? "Advertising" : "Removing", afi, safi);
		}
	}
	...
	if(BGP_DEBUG(normal, NORMAL)) {
		zlog_debug("%s send message type %d, length (incl. header) %d", peer->host, BGP_MSG_CAPABILITY, length);
	}
	...
	for(ALL_LIST_ELEMENTS(bgp->peer, node, nnode, peer)) {
		...
		if(peer->status == Established) {
           ...
           */
			...
			if(new->fd >= 0) {
				if(BGP_DEBUG(events, EVENTS)) {
					zlog_debug("%s:%u Existing Established peer, sending NOTIFY", new->host, sockunion_get_port(&new->su));
				}
		...
		if(peer->status == OpenConfirm || peer->status == OpenSent) {
		 ...
		 connection initiated by the remote system. */
				...
				if(out->fd >= 0) {
					if(BGP_DEBUG(events, EVENTS)) {
						zlog_debug(
							"%s Collision resolution, remote ID higher,"
							" closing outbound",
							peer->host
						);
					}
					bgp_notify_send(out, BGP_NOTIFY_CEASE, BGP_NOTIFY_CEASE_COLLISION_RESOLUTION);
				}
		 ...
		 OpenConfirm state). */
				...
				if(in->fd >= 0) {
					if(BGP_DEBUG(events, EVENTS)) {
						zlog_debug(
							"%s Collision resolution, local ID higher,"
							" closing inbound",
							peer->host
						);
					}

					bgp_notify_send(in, BGP_NOTIFY_CEASE, BGP_NOTIFY_CEASE_COLLISION_RESOLUTION);
				}
				return ret_close_in;
			}
		}
...
static int bgp_open_receive(struct peer *peer, bgp_size_t size) {
	...
	if(BGP_DEBUG(normal, NORMAL)) {
		zlog_debug(
			...
			peer->host, version, remote_as, holdtime, inet_ntoa(remote_id), CHECK_FLAG(peer->sflags, PEER_STATUS_ACCEPT_PEER) ? "in" : "out"
		);
	}
	...
	if(CHECK_FLAG(peer->sflags, PEER_STATUS_ACCEPT_PEER)) {
		...
		if(!realpeer) {
	     ...
	     mismatch. */
			if(as) {
				if(BGP_DEBUG(normal, NORMAL)) {
					zlog_debug("%s bad OPEN, wrong router identifier %s", peer->host, inet_ntoa(remote_id));
				}
				bgp_notify_send_with_data(peer, BGP_NOTIFY_OPEN_ERR, BGP_NOTIFY_OPEN_BAD_BGP_IDENT, notify_data_remote_id, 4);
			} else {
				if(BGP_DEBUG(normal, NORMAL)) {
					zlog_debug("%s bad OPEN, remote AS is %u, expected %u", peer->host, remote_as, peer->as);
				}
	...
	if(CHECK_FLAG(peer->sflags, PEER_STATUS_ACCEPT_PEER)) {
       ...
       */
		if(realpeer->status == Idle || realpeer->status > Established) {
			if(BGP_DEBUG(events, EVENTS)) {
				zlog_debug("%s peer status is %s, closing the new connection", realpeer->host, LOOKUP(bgp_status_msg, realpeer->status));
			}
			return -1;
		}
       ...
       * having to special case this?) */
		if(realpeer->status == Established && CHECK_FLAG(realpeer->sflags, PEER_STATUS_NSF_MODE)) {
 	   ...
 	   */
			if(realpeer->status != Active && realpeer->status != Connect) {
				if(BGP_DEBUG(events, EVENTS)) {
					zlog_warn(
						"%s real peer status should be Active or Connect,"
						" but is %s",
						realpeer->host, LOOKUP(bgp_status_msg, realpeer->status)
					);
				}
				bgp_notify_send(realpeer, BGP_NOTIFY_CEASE, BGP_NOTIFY_CEASE_COLLISION_RESOLUTION);
			}
		}

		if(BGP_DEBUG(events, EVENTS)) {
			zlog_debug("%s:%u [Event] Transfer accept BGP peer to real (state %s)", peer->host, sockunion_get_port(&peer->su), LOOKUP(bgp_status_msg, realpeer->status));
		}
		...
		bool open_deferred = CHECK_FLAG(peer->sflags, PEER_STATUS_OPEN_DEFERRED);
		...
		if(peer->fd < 0) {
			zlog_err("bgp_open_receive peer's fd is negative value %d", peer->fd);
			return -1;
		}
	...
	if(remote_id.s_addr == 0 || IPV4_CLASS_DE(ntohl(remote_id.s_addr)) || ntohl(peer->local_id.s_addr) == ntohl(remote_id.s_addr)) {
		if(BGP_DEBUG(normal, NORMAL)) {
			zlog_debug("%s bad OPEN, wrong router identifier %s", peer->host, inet_ntoa(remote_id));
		}
		bgp_notify_send_with_data(peer, BGP_NOTIFY_OPEN_ERR, BGP_NOTIFY_OPEN_BAD_BGP_IDENT, notify_data_remote_id, 4);
	...
	if(version != BGP_VERSION_4) {
		...
		if(BGP_DEBUG(normal, NORMAL)) {
			zlog_debug("%s bad protocol version, remote requested %d, local request %d", peer->host, version, BGP_VERSION_4);
		}
	...
	if(holdtime < 3 && holdtime != 0) {
		...
		bgp_notify_send_with_data(peer, BGP_NOTIFY_OPEN_ERR, BGP_NOTIFY_OPEN_UNACEP_HOLDTIME, (u_int8_t *) &netholdtime, 2);
		return -1;
	}
...
int bgp_nlri_parse(struct peer *peer, struct attr *attr, struct bgp_nlri *packet) {
	switch(packet->safi) {
		case SAFI_UNICAST:
		case SAFI_MULTICAST: return bgp_nlri_parse_ip(peer, attr, packet);
		case SAFI_MPLS_VPN:
		case SAFI_MPLS_LABELED_VPN: return bgp_nlri_parse_vpn(peer, attr, packet);
		case SAFI_ENCAP: return bgp_nlri_parse_encap(peer, attr, packet);
	}
	return -1;
}
...
static int bgp_update_receive(struct peer *peer, bgp_size_t size) {
	...
	if(peer->status != Established) {
		zlog_err("%s [FSM] Update packet received under status %s", peer->host, LOOKUP(bgp_status_msg, peer->status));
...
#define NLRI_ATTR_ARG (attr_parse_ret != BGP_ATTR_PARSE_WITHDRAW ? &attr : NULL)
	...
	if(attribute_len) {
		attr_parse_ret = bgp_attr_parse(peer, &attr, attribute_len, &nlris[NLRI_MP_UPDATE], &nlris[NLRI_MP_WITHDRAW]);
	...
	/* Logging the attribute. */
	if(attr_parse_ret == BGP_ATTR_PARSE_WITHDRAW || BGP_DEBUG(update, UPDATE_IN)) {
		char attrstr[BUFSIZ];
		attrstr[0] = '\0';

		ret = bgp_dump_attr(peer, &attr, attrstr, BUFSIZ);
		int lvl = (attr_parse_ret == BGP_ATTR_PARSE_WITHDRAW) ? LOG_ERR : LOG_DEBUG;

		if(attr_parse_ret == BGP_ATTR_PARSE_WITHDRAW) {
			zlog(peer->log, LOG_ERR, "%s rcvd UPDATE with errors in attr(s)!! Withdrawing route.", peer->host);
		}

		if(ret) {
			zlog(peer->log, lvl, "%s rcvd UPDATE w/ attr: %s", peer->host, attrstr);
		}
	}
	...
	for(i = NLRI_UPDATE; i < NLRI_TYPE_MAX; i++) {
       ...
       */
		if(!bgp_afi_safi_valid_indices(nlris[i].afi, &nlris[i].safi)) {
			plog_info(peer->log, "%s [Info] UPDATE with unsupported AFI/SAFI %u/%u", peer->host, nlris[i].afi, nlris[i].safi);
			continue;
		}
		...
		if(!peer->afc[nlris[i].afi][nlris[i].safi]) {
			plog_info(peer->log, "%s [Info] UPDATE for non-enabled AFI/SAFI %u/%u", peer->host, nlris[i].afi, nlris[i].safi);
			continue;
		}
		...
		switch(i) {
			case NLRI_UPDATE:
			case NLRI_MP_UPDATE: nlri_ret = bgp_nlri_parse(peer, NLRI_ATTR_ARG, &nlris[i]); break;
			case NLRI_WITHDRAW:
			case NLRI_MP_WITHDRAW: nlri_ret = bgp_nlri_parse(peer, NULL, &nlris[i]);
		}

		if(nlri_ret < 0) {
			plog_err(peer->log, "%s [Error] Error parsing NLRI", peer->host);
			if(peer->status == Established) {
				bgp_notify_send(peer, BGP_NOTIFY_UPDATE_ERR, i <= NLRI_WITHDRAW ? BGP_NOTIFY_UPDATE_INVAL_NETWORK : BGP_NOTIFY_UPDATE_OPT_ATTR_ERR);
			}
			bgp_attr_unintern_sub(&attr);
			return -1;
		}
...
static void bgp_route_refresh_receive(struct peer *peer, bgp_size_t size) {
	...
	if(!CHECK_FLAG(peer->cap, PEER_CAP_REFRESH_ADV)) {
		plog_err(peer->log, "%s [Error] BGP route refresh is not enabled", peer->host);
		bgp_notify_send(peer, BGP_NOTIFY_HEADER_ERR, BGP_NOTIFY_HEADER_BAD_MESTYPE);
		return;
	}

	/* Status must be Established. */
	if(peer->status != Established) {
		plog_err(peer->log, "%s [Error] Route refresh packet received under status %s", peer->host, LOOKUP(bgp_status_msg, peer->status));
		bgp_notify_send(peer, BGP_NOTIFY_FSM_ERR, 0);
		return;
	}
	...
	if(BGP_DEBUG(normal, NORMAL)) {
		zlog_debug("%s rcvd REFRESH_REQ for afi/safi: %d/%d", peer->host, afi, safi);
	}

	/* Check AFI and SAFI. */
	if((afi != AFI_IP && afi != AFI_IP6) || (safi != SAFI_UNICAST && safi != SAFI_MULTICAST && safi != SAFI_MPLS_LABELED_VPN)) {
		if(BGP_DEBUG(normal, NORMAL)) {
			zlog_debug("%s REFRESH_REQ for unrecognized afi/safi: %d/%d - ignored", peer->host, afi, safi);
		}
		return;
	}
	...
	if(size != BGP_MSG_ROUTE_REFRESH_MIN_SIZE - BGP_HEADER_SIZE) {
		...
		while((stream_pnt(s) + 2) < end) {
			...
			if(orf_type == ORF_TYPE_PREFIX || orf_type == ORF_TYPE_PREFIX_OLD) {
				...
				if(BGP_DEBUG(normal, NORMAL)) {
					zlog_debug("%s rcvd Prefixlist ORF(%d) length %d", peer->host, orf_type, orf_len);
				}
				...
				while(p_pnt < p_end) {
                   ...
                   */
					...
					if(BGP_DEBUG(normal, NORMAL)) {
						...
						zlog_debug(
							"%s rcvd %s %s seq %u %s/%d ge %d le %d%s", peer->host, (common & ORF_COMMON_PART_REMOVE ? "Remove" : "Add"), (common & ORF_COMMON_PART_DENY ? "deny" : "permit"), orfp.seq,
							inet_ntop(orfp.p.family, &orfp.p.u.prefix, buf, INET6_BUFSIZ), orfp.p.prefixlen, orfp.ge, orfp.le, ok ? "" : " MALFORMED"
						);
					}

					if(ok) {
						ret = prefix_bgp_orf_set(name, afi, &orfp, (common & ORF_COMMON_PART_DENY ? 0 : 1), (common & ORF_COMMON_PART_REMOVE ? 0 : 1));
					}
		...
		if(BGP_DEBUG(normal, NORMAL)) {
			zlog_debug("%s rcvd Refresh %s ORF request", peer->host, when_to_refresh == REFRESH_DEFER ? "Defer" : "Immediate");
		}
...
static int bgp_capability_msg_parse(struct peer *peer, u_char *pnt, bgp_size_t length) {
	...
	for(; pnt < end; pnt += hdr->length + 3) {
		...
		/* Action value check.  */
		if(action != CAPABILITY_ACTION_SET && action != CAPABILITY_ACTION_UNSET) {
			zlog_info("%s Capability Action Value error %d", peer->host, action);
			bgp_notify_send(peer, BGP_NOTIFY_CEASE, 0);
			return -1;
		}

		if(BGP_DEBUG(normal, NORMAL)) {
			zlog_debug("%s CAPABILITY has action: %d, code: %u, length %u", peer->host, action, hdr->code, hdr->length);
		}
		...
		if(hdr->code == CAPABILITY_CODE_MP) {
			...
			if(BGP_DEBUG(normal, NORMAL)) {
				zlog_debug("%s CAPABILITY has %s MP_EXT CAP for afi/safi: %u/%u", peer->host, action == CAPABILITY_ACTION_SET ? "Advertising" : "Removing", ntohs(mpc.afi), mpc.safi);
			}
			...
			zlog_warn("%s unrecognized capability code: %d - ignored", peer->host, hdr->code);
		}
	}
	return 0;
}
...
int bgp_capability_receive(struct peer *peer, bgp_size_t size) {
	...
	if(!CHECK_FLAG(peer->cap, PEER_CAP_DYNAMIC_ADV)) {
		plog_err(peer->log, "%s [Error] BGP dynamic capability is not enabled", peer->host);
		bgp_notify_send(peer, BGP_NOTIFY_HEADER_ERR, BGP_NOTIFY_HEADER_BAD_MESTYPE);
		return -1;
	}
	...
	/* Status must be Established. */
	if(peer->status != Established) {
		plog_err(peer->log, "%s [Error] Dynamic capability packet received under status %s", peer->host, LOOKUP(bgp_status_msg, peer->status));
		bgp_notify_send(peer, BGP_NOTIFY_FSM_ERR, 0);
		return -1;
	}
...
static int bgp_read_packet(struct peer *peer) {
	...
	if(nbytes < 0) {
		...
		plog_err(peer->log, "%s [Error] bgp_read_packet error: %s", peer->host, safe_strerror(errno));
	...
	if(nbytes == 0) {
		if(BGP_DEBUG(events, EVENTS)) {
			plog_debug(peer->log, "%s [Event] BGP connection closed fd %d", peer->host, peer->fd);
		}
...
static time_t bgp_recent_clock(void) {
...
int bgp_read(struct thread *thread) {
	...
	if(stream_get_endp(peer->ibuf) < BGP_HEADER_SIZE) {
		...
		if(BGP_DEBUG(normal, NORMAL) && type != 2 && type != 0) {
			zlog_debug("%s rcv message type %d, length (excl. header) %d", peer->host, type, size - BGP_HEADER_SIZE);
		}

		/* Marker check */
		if(((type == BGP_MSG_OPEN) || (type == BGP_MSG_KEEPALIVE)) && !bgp_marker_all_one(peer->ibuf, BGP_MARKER_SIZE)) {
			bgp_notify_send(peer, BGP_NOTIFY_HEADER_ERR, BGP_NOTIFY_HEADER_NOT_SYNC);
			goto done;
		}

		/* BGP type check. */
		if(type != BGP_MSG_OPEN && type != BGP_MSG_UPDATE && type != BGP_MSG_NOTIFY && type != BGP_MSG_KEEPALIVE && type != BGP_MSG_ROUTE_REFRESH_NEW && type != BGP_MSG_ROUTE_REFRESH_OLD && type != BGP_MSG_CAPABILITY) {
			if(BGP_DEBUG(normal, NORMAL)) {
				plog_debug(peer->log, "%s unknown message type 0x%02x", peer->host, type);
			}
			bgp_notify_send_with_data(peer, BGP_NOTIFY_HEADER_ERR, BGP_NOTIFY_HEADER_BAD_MESTYPE, &type, 1);
			goto done;
		}
		/* Mimimum packet length check. */
		if((size < BGP_HEADER_SIZE) || (size > BGP_MAX_PACKET_SIZE) || (type == BGP_MSG_OPEN && size < BGP_MSG_OPEN_MIN_SIZE) || (type == BGP_MSG_UPDATE && size < BGP_MSG_UPDATE_MIN_SIZE)
		   || (type == BGP_MSG_NOTIFY && size < BGP_MSG_NOTIFY_MIN_SIZE) || (type == BGP_MSG_KEEPALIVE && size != BGP_MSG_KEEPALIVE_MIN_SIZE) || (type == BGP_MSG_ROUTE_REFRESH_NEW && size < BGP_MSG_ROUTE_REFRESH_MIN_SIZE)
		   || (type == BGP_MSG_ROUTE_REFRESH_OLD && size < BGP_MSG_ROUTE_REFRESH_MIN_SIZE) || (type == BGP_MSG_CAPABILITY && size < BGP_MSG_CAPABILITY_MIN_SIZE)) {
			if(BGP_DEBUG(normal, NORMAL)) {
				plog_debug(peer->log, "%s bad message length - %d for %s", peer->host, size, type == 128 ? "ROUTE-REFRESH" : bgp_type_str[(int) type]);
			}
	...
	switch(type) {
		...
		case BGP_MSG_UPDATE:
			...
			break;
		case BGP_MSG_NOTIFY: bgp_notify_receive(peer, size); break;
