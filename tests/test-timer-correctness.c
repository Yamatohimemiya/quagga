...
int main(int argc, char **argv) {
	...
	for(i = 0; i < SCHEDULE_TIMERS; i++) {
		...
		ret = snprintf(arg, TIMESTR_LEN + 1, "%lld.%06lld", (long long) timers[i]->u.sands.tv_sec, (long long) timers[i]->u.sands.tv_usec);
		assert(ret > 0);
   ...
   * are run. */
	...
	for(i = 0; i < j; i++) {
		...
		ret = snprintf(expected_buf + expected_buf_pos, expected_buf_len - expected_buf_pos, "%lld.%06lld\n", (long long) alarms[i]->tv_sec, (long long) alarms[i]->tv_usec);
		assert(ret > 0);
