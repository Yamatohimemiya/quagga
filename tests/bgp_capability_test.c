...
#define INVALID_AFI 0
	...
	/* 0 */
	{
		"caphdr", "capability header, and no more",
		{ CAPABILITY_CODE_REFRESH, 0x0 },
		2, SHOULD_PARSE,
	 },
 /* 1 */
	{
		"nodata", "header, no data but length says there is",
		{ 0x1, 0xa },
		2, SHOULD_ERR,
	 },
 /* 2 */
	{
		"padded", "valid, with padding",
		{ CAPABILITY_CODE_REFRESH, 0x2, 0x0, 0x0 },
		4, SHOULD_PARSE,
	 },
 /* 3 */
	{
		"minsize", "violates minsize requirement",
...
static struct test_segment mp_segments[] = {
 ...
 /* 6 */
	{
		"MP3", "MP IP6/MPLS-labeled VPN",
		{ CAPABILITY_CODE_MP, 0x4, 0x0, 0x2, 0x0, 0x80 },
		6, SHOULD_PARSE,
		0, 1,
		AFI_IP6, SAFI_MPLS_LABELED_VPN,
		VALID_AFI, },
 /* 7 */
	{
		"MP5", "MP IP6/MPLS-VPN",
 ...
 /* 8 */
	{
		"MP6", "MP IP4/MPLS-laveled VPN",
		{ CAPABILITY_CODE_MP, 0x4, 0x0, 0x1, 0x0, 0x80 },
		6, SHOULD_PARSE,
		0, 1,
		AFI_IP, SAFI_MPLS_LABELED_VPN,
		VALID_AFI, },
 /* 10 */
	{
		"MP8", "MP unknown AFI/SAFI", { CAPABILITY_CODE_MP, 0x4, 0x0, 0xa, 0x0, 0x81 }, 6, SHOULD_PARSE, 0, 1, 0xa, 0x81, INVALID_AFI, /* parses, but unknown */
	},
 /* 11 */
	{
		"MP-short", "MP IP4/Unicast, length too short (< minimum)",
		{ CAPABILITY_CODE_MP, 0x2, 0x0, 0x1, 0x0, 0x1 },
		6, SHOULD_ERR,
	 },
 /* 12 */
	{
		"MP-overflow", "MP IP4/Unicast, length too long",
...
static struct test_segment misc_segments[] = {
	/* 13 */
	{
		"ORF", "ORF, simple, single entry, single tuple",
 ...
 /* 14 */
	{
		"ORF-many", "ORF, multi entry/tuple",
		{
 ...
 /* 15 */
	{
		"ORFlo", "ORF, multi entry/tuple, hdr length too short",
		{
			/* hdr */ CAPABILITY_CODE_ORF,
			0x15,
			/* mpc */ 0x0,
		...
		SHOULD_ERR, /* It should error on invalid Route-Refresh.. */
	},
 /* 16 */
	{ "ORFlu",
 ...
 /* 17 */
	{
		"ORFnu", "ORF, multi entry/tuple, entry number too long",
		{
			/* hdr */ 0x3,	   0x21,
			/* mpc */ 0x0,	   0x1,	 0x0,  0x1,
			/* num */ 0x3,
		...
		SHOULD_PARSE, /* parses, but last few tuples should be gibberish */
	},
 /* 18 */
	{
		"ORFno", "ORF, multi entry/tuple, entry number too short",
		{
			/* hdr */ 0x3,	   0x21,
			/* mpc */ 0x0,	   0x1,	 0x0,  0x1,
			/* num */ 0x3,
		...
		SHOULD_PARSE, /* Parses, but should get gibberish afi/safis */
	},
 /* 17 */
	{
		"ORFpad", "ORF, multi entry/tuple, padded to align",
		{
			/* hdr */ 0x3,	   0x22,
			/* mpc */ 0x0,	   0x1,	 0x0,  0x1,
			/* num */ 0x3,
			...
			/* mpc */ 0x0,	   0x2,	 0x0,  0x2,
			/* num */ 0x3,
			/* tuples */ 0x40, 0x3,	 0x80, 0x1, 0x80, 0x2, 0x00,
		}, 36,
		SHOULD_PARSE, },
 /* 19 */
	{
		...
		2882400018, },
	{
		"AS4", "AS4 capability: short",
		{ 0x41, 0x4, 0xab, 0xcd, 0xef }, /* AS: 2882400018 */
		5, SHOULD_ERR,
	 },
	{
		"AS4", "AS4 capability: long",
		...
		2882400018, },
	{
		...
		SHOULD_PARSE, },
	{
		"GR-short", "GR capability, but header length too short",
		{
			/* hdr */ 0x40,
			0xa,
			/* R-bit, time */ 0xf1,
		...
		SHOULD_ERR, },
	{
		"GR-long", "GR capability, but header length too long",
		{
			/* hdr */ 0x40,
			0xf,
			/* R-bit, time */ 0xf1,
		...
		SHOULD_ERR, },
	{
		"GR-trunc", "GR capability, but truncated",
		{
			/* hdr */ 0x40,
			0xf,
			/* R-bit, time */ 0xf1,
		...
		SHOULD_ERR, },
	{
		"GR-empty", "GR capability, but empty.",
		{
			/* hdr */ 0x40,
			0x0,
		}, 2,
		SHOULD_ERR, },
	{
		"MP-empty", "MP capability, but empty.",
		{
			/* hdr */ 0x1,
			0x0,
		}, 2,
		SHOULD_ERR, },
	{
		"ORF-empty", "ORF capability, but empty.",
		{
			/* hdr */ 0x3,
			0x0,
		}, 2,
		SHOULD_ERR, },
	{
		"AS4-empty", "AS4 capability, but empty.",
		{
			/* hdr */ 0x41,
			0x0,
		}, 2,
		SHOULD_ERR, },
	{
		"dyn-empty", "Dynamic capability, but empty.",
		{
			/* hdr */ 0x42,
			0x0,
		}, 2,
		SHOULD_PARSE, },
	{
		"dyn-old", "Dynamic capability (deprecated version)",
...
struct test_segment dynamic_cap_msgs[] = {
	{
		"DynCap", "Dynamic Capability Message, IP/Multicast", { 0x0, 0x1, 0x4, 0x0, 0x1, 0x0, 0x2 }, 7, SHOULD_PARSE, /* horrible alignment, just as with ORF */
	},
	{
		"DynCapLong", "Dynamic Capability Message, IP/Multicast, truncated",
		{ 0x0, 0x1, 0x4, 0x0, 0x1, 0x0, 0x2 },
		5, SHOULD_ERR,
	 },
	{
		"DynCapPadded", "Dynamic Capability Message, IP/Multicast, padded", { 0x0, 0x1, 0x4, 0x0, 0x1, 0x0, 0x2, 0x0 }, 8, SHOULD_ERR, /* No way to tell padding from data.. */
	},
	{
		"DynCapMPCpadded", "Dynamic Capability Message, IP/Multicast, cap data padded", { 0x0, 0x1, 0x5, 0x0, 0x1, 0x0, 0x2, 0x0 }, 8, SHOULD_PARSE, /* You can though add padding to the capability data */
	},
	{
...
struct test_segment opt_params[] = {
	{
		"Cap-singlets", "One capability per Optional-Param",
		{
		...
		SHOULD_PARSE, },
	{
		"Cap-series", "Series of capability, one Optional-Param",
		{
			0x02, 0x10, 0x01, 0x04, 0x00, 0x01, 0x00, 0x01, /* MP IPv4/Uni */
		...
		SHOULD_PARSE, },
	{
		"AS4more", "AS4 capability after other caps (singlets)",
		{
			0x02, 0x06, 0x01, 0x04, 0x00, 0x01, 0x00, 0x01, /* MP IPv4/Uni */
			0x02, 0x06, 0x01, 0x04, 0x00, 0x02, 0x00, 0x01, /* MP IPv6/Uni */
			0x02, 0x02, 0x80, 0x00,				/* RR (old) */
	...
	{
		"AS4series", "AS4 capability, in series of capabilities",
		{
			0x02, 0x16, 0x01, 0x04, 0x00, 0x01, 0x00, 0x01, /* MP IPv4/Uni */
			0x01, 0x04, 0x00, 0x02, 0x00, 0x01,		/* MP IPv6/Uni */
			0x80, 0x00,					/* RR (old) */
			0x02, 0x00,					/* RR */
	...
	{
		"AS4real", "AS4 capability, in series of capabilities",
		{
			0x02, 0x06, 0x01, 0x04, 0x00, 0x01, 0x00, 0x01, /* MP IPv4/uni */
			0x02, 0x06, 0x01, 0x04, 0x00, 0x02, 0x00, 0x01, /* MP IPv6/uni */
			0x02, 0x02, 0x80, 0x00,				/* RR old */
	...
	{
		"AS4real2", "AS4 capability, in series of capabilities",
		{
			0x02, 0x06, 0x01, 0x04, 0x00, 0x01, 0x00, 0x01, 0x02, 0x06, 0x01, 0x04, 0x00, 0x02, 0x00, 0x01, 0x02, 0x02, 0x80, 0x00, 0x02, 0x02, 0x02, 0x00, 0x02, 0x06, 0x41, 0x04, 0x00,
			0x00, 0xfc, 0x03, 0x02, 0x09, 0x82, 0x07, 0x00, 0x01, 0x00, 0x01, 0x01, 0x80, 0x03, 0x02, 0x09, 0x03, 0x07, 0x00, 0x01, 0x00, 0x01, 0x01, 0x40, 0x03, 0x02, 0x02, 0x42, 0x00,
		}, 58,
		SHOULD_PARSE, 64515,
	 },
...
#define RANDOM_FUZZ 35
	...
	switch(type) {
		case CAPABILITY: len += 2; /* to cover the OPT-Param header */
		case OPT_PARAM:
			printf("len: %u\n", len);
			...
			break;
		case DYNCAP: ret = bgp_capability_receive(peer, t->len); break;
	...
	if(!ret && t->validate_afi) {
		...
		printf("MP: %u/%u (%u): recv %u, nego %u\n", t->afi, t->safi, safi, peer->afc_recv[t->afi][safi], peer->afc_nego[t->afi][safi]);
	...
	if(tty) {
		printf("%s", (failed > oldfailed) ? VT100_RED "failed!" VT100_RESET : VT100_GREEN "OK" VT100_RESET);
	} else {
