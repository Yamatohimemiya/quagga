...
static int test_callback(struct cmd_element *cmd, struct vty *vty, int argc, const char *argv[]) {
	int offset;
	int rv;
	int i;
	...
	for(i = 0; i < argc; i++) {
		rv = snprintf(test_buf + offset, sizeof(test_buf) - offset, "%s'%s'", (i == 0) ? ": " : ", ", argv[i]);
		if(rv < 0) {
			abort();
		}
		offset += rv;
...
static void test_run(struct prng *prng, struct vty *vty, const char *cmd, unsigned int edit_dist, unsigned int node_index, int verbose) {
	const char *test_str;
	vector vline;
	int ret;
	unsigned int i;
	...
	for(i = 0; i < vector_active(cmdvec); i++) {
		if((cnode = vector_slot(cmdvec, i)) != NULL) {
			...
			if(verbose || !no_match) {
				printf("execute relaxed '%s'@%d: rv==%d%s%s\n", test_str, cnode->node, ret, (test_buf[0] != '\0') ? ", " : "", test_buf);
			}
...
int main(int argc, char **argv) {
	...
	while((opt = getopt(argc, argv, "e:n:v")) != -1) {
		switch(opt) {
			case 'e': max_edit_distance = atoi(optarg); break;
			case 'n': node_index = atoi(optarg); break;
	...
	for(test_cmd = 0; test_cmd < vector_active(test_cmds); test_cmd++) {
		for(edit_distance = 0; edit_distance <= max_edit_distance; edit_distance++) {
