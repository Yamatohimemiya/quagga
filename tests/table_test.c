 ...
 * @see print_table
 */
static void print_subtree(struct route_node *rn, const char *legend, int indent_level) {
 ...
 * present in the tree.
 */
static void verify_next_by_iterating(struct route_table *table, struct prefix *target_pfx, struct prefix *next_pfx) {
	route_table_iter_t iter;
	struct route_node *rn;
...
static void verify_next(struct route_table *table, const char *target, const char *next) {
	...
	verify_next_by_iterating(table, (struct prefix *) &target_pfx, (struct prefix *) &next_pfx);
}
...
static void test_get_next(void) {
   ...
   */
	add_nodes(table, "1.0.0.0/16", "1.0.0.0/24", "1.0.1.0/24", "1.0.128.0/17", NULL);
   ...
   */
	...
	add_nodes(table, "1.0.0.0/16", "1.0.0.0/24", "1.0.1.0/25", "1.0.128.0/17", NULL);
 ...
 */
static void verify_prefix_iter_cmp(const char *p1, const char *p2, int exp_result) {
	...
	result = route_table_prefix_iter_cmp((struct prefix *) &p1_pfx, (struct prefix *) &p2_pfx);
   ...
   */
	result = route_table_prefix_iter_cmp((struct prefix *) &p2_pfx, (struct prefix *) &p1_pfx);
...
static void test_iter_pause(void) {
	struct route_table *table;
	int i, num_prefixes;
	const char *prefixes[] = { "1.0.1.0/24", "1.0.1.0/25", "1.0.1.128/25", "1.0.2.0/24", "2.0.0.0/8" };
