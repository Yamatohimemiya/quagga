 ...
 */
static u_int16_t reduce_ospfd(struct csum_vals *vals, testsz_t len, testoff_t off) {
#define x vals->x
#define y vals->y
#define c0 vals->a.c0
#define c1 vals->a.c1
...
static u_int16_t reduce_ospfd1(struct csum_vals *vals, testsz_t len, testoff_t off) {
#define x vals->x
#define y vals->y
#define c0 vals->a.c0
#define c1 vals->a.c1
...
static u_int16_t reduce_isisd(struct csum_vals *vals, testsz_t len, testoff_t off) {
#define x vals->x
#define y vals->y
#define c0 vals->a.c0
#define c1 vals->a.c1
...
static u_int16_t reduce_isisd_yfix(struct csum_vals *vals, testsz_t len, testoff_t off) {
#define x vals->x
#define y vals->y
#define c0 vals->a.c0
#define c1 vals->a.c1
...
/* Move the mods yp */
static u_int16_t reduce_isisd_mod(struct csum_vals *vals, testsz_t len, testoff_t off) {
#define x vals->x
#define y vals->y
#define c0 vals->a.c0
#define c1 vals->a.c1
...
static u_int16_t reduce_isisd_mody(struct csum_vals *vals, testsz_t len, testoff_t off) {
#define x vals->x
#define y vals->y
#define c0 vals->a.c0
#define c1 vals->a.c1
	...
	while(1) {
		...
		if(ospfd != lib) {
			printf("Mismatch in values at size %u\n"
			       ...
			       "lib: 0x%04x\n",
			       exercise, ospfd, ospfd_vals.a.c0, ospfd_vals.a.c1, ospfd_vals.x, ospfd_vals.y, isisd, isisd_vals.a.c0, isisd_vals.a.c1, isisd_vals.x, isisd_vals.y, lib);

			/* Investigate reduction phase discrepencies */
			if(ospfd_vals.a.c0 == isisd_vals.a.c0 && ospfd_vals.a.c1 == isisd_vals.a.c1) {
				printf("\n");
				for(i = 0; reducts[i].name != NULL; i++) {
					ospfd = reducts[i].f(&ospfd_vals, exercise + sizeof(u_int16_t), exercise);
			...
			for(i = 0; i < exercise; i++) {
				printf("0x%02x,%s", buffer[i], (i + 1) % 8 ? " " : "\n  ");
			}
			printf("\n}\n");
			exit(1);
		}
