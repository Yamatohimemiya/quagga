...
static int run_bgp_cfg_maximum_paths(testcase_t *t) {
	...
	for(afi = AFI_IP; afi < AFI_MAX; afi++) {
		for(safi = SAFI_UNICAST; safi < SAFI_MAX; safi++) {
			...
			EXPECT_TRUE(api_result == 0, test_result);
			EXPECT_TRUE((bgp->maxpaths[afi][safi].maxpaths_ebgp == BGP_DEFAULT_MAXPATHS), test_result);
			EXPECT_TRUE((bgp->maxpaths[afi][safi].maxpaths_ibgp == BGP_DEFAULT_MAXPATHS), test_result);
		}
	}
...
int test_mp_list_info_count = sizeof(test_mp_list_info) / sizeof(struct bgp_info);
...
static int run_bgp_mp_list(testcase_t *t) {
	...
	for(i = 0, mp_node = mp_list.head; i < test_mp_list_info_count; i++, mp_node = listnextnode(mp_node)) {
...
static int run_bgp_info_mpath_update(testcase_t *t) {
	...
	EXPECT_TRUE(mpath == &test_mp_list_info[1], test_result);
	EXPECT_TRUE(CHECK_FLAG(mpath->flags, BGP_INFO_MULTIPATH), test_result);
	EXPECT_TRUE(!CHECK_FLAG(test_mp_list_info[0].flags, BGP_INFO_MULTIPATH), test_result);
...
static void run_tests(testcase_t *tests[], int num_tests, int *pass_count, int *fail_count) {
