	...
	switch(c) {
		...
		case ' ':
			...
			if(config) {
				...
				} else if(strncmp(line, " address-family vpn6", strlen(" address-family vpn6")) == 0) {
					config = config_get(BGP_VPNV6_NODE, line);
				} else if(strncmp(line, " address-family encapv6", strlen(" address-family encapv6")) == 0) {
					config = config_get(BGP_ENCAPV6_NODE, line);
				} else if(strncmp(line, " address-family encap", strlen(" address-family encap")) == 0) {
				...
				} else if(config->index == LINK_PARAMS_NODE && strncmp(line, "  exit-link-params", strlen("  exit")) == 0) {
					config_add_line(config->line, line);
					config->index = INTERFACE_NODE;
				} else if(config->index == RMAP_NODE || config->index == INTERFACE_NODE || config->index == VTY_NODE) {
		...
		default:
			...
			} else if((strncmp(line, "ipv6 forwarding", strlen("ipv6 forwarding")) == 0) || (strncmp(line, "ip forwarding", strlen("ip forwarding")) == 0)) {
			...
			} else if(strncmp(line, "password", strlen("password")) == 0 || strncmp(line, "enable password", strlen("enable password")) == 0) {
				config = config_get(AAA_NODE, line);
			} else if(strncmp(line, "ip protocol", strlen("ip protocol")) == 0) {
				config = config_get(PROTOCOL_NODE, line);
			} else {
				if(strncmp(line, "log", strlen("log")) == 0 || strncmp(line, "hostname", strlen("hostname")) == 0) {
...
void vtysh_config_parse(char *line) {
	char *begin;
	char *pnt;
...
#define NO_DELIMITER(I) \
	((I) == ACCESS_NODE || (I) == PREFIX_NODE || (I) == IP_NODE || (I) == AS_LIST_NODE || (I) == COMMUNITY_LIST_NODE || (I) == ACCESS_IPV6_NODE || (I) == PREFIX_IPV6_NODE || (I) == SERVICE_NODE || (I) == FORWARDING_NODE \
	 || (I) == DEBUG_NODE || (I) == AAA_NODE)
...
static void vtysh_read_file(FILE *confp) {
	int ret;
	struct vty *vty;
	...
	if(ret != CMD_SUCCESS) {
		switch(ret) {
			case CMD_ERR_AMBIGUOUS: fprintf(stderr, "Ambiguous command.\n"); break;
			case CMD_ERR_NO_MATCH: fprintf(stderr, "There is no such command.\n"); break;
		}
...
int vtysh_read_config(char *config_default_dir) {
	FILE *confp = NULL;
