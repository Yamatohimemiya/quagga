...
static u_short pf_filter[] = { ENF_PUSHWORD + 0,       /* Get the SSAP/DSAP values */
...
static int dlpisend(int fd, const void *cbuf, size_t cbuflen, const void *dbuf, size_t dbuflen, int flags) {
...
static int dlpiok(int fd, t_uscalar_t oprim) {
	...
	retv = dlpirctl(fd);
	if(retv < (ssize_t) DL_OK_ACK_SIZE || doa->dl_primitive != DL_OK_ACK || doa->dl_correct_primitive != oprim) {
		return -1;
	} else {
		return 0;
	}
...
static int dlpiaddr(int fd, u_char *addr) {
	...
	retv = dlpirctl(fd);
	if(retv < (ssize_t) DL_PHYS_ADDR_ACK_SIZE || dpaa->dl_primitive != DL_PHYS_ADDR_ACK) {
		return -1;
	}

	if(dpaa->dl_addr_offset < DL_PHYS_ADDR_ACK_SIZE || dpaa->dl_addr_length != ETHERADDRL || dpaa->dl_addr_offset + dpaa->dl_addr_length > (size_t) retv) {
		return -1;
	}
...
static int open_dlpi_dev(struct isis_circuit *circuit) {
	...
	if(circuit->circ_type != CIRCUIT_T_BROADCAST) {
		zlog_warn("%s: non-broadcast interface %s", __func__, circuit->interface->name);
		return ISIS_WARNING;
	}

	/* Try the vanity node first, if permitted */
	if(getenv("DLPI_DEVONLY") == NULL) {
		(void) snprintf(devpath, sizeof(devpath), "/dev/net/%s", circuit->interface->name);
		fd = dlpiopen(devpath, &acklen);
	}

	/* Now try as an ordinary Style 1 node */
	if(fd == -1) {
		(void) snprintf(devpath, sizeof(devpath), "/dev/%s", circuit->interface->name);
		unit = -1;
		fd = dlpiopen(devpath, &acklen);
	}
	...
	if(fd == -1) {
		...
		*cp = '\0';
		...
		if(fd == -1) {
			zlog_warn("%s: unknown interface %s", __func__, circuit->interface->name);
			return ISIS_WARNING;
		}

		/* Double check the DLPI style */
		if(dia->dl_provider_style != DL_STYLE2) {
			zlog_warn("open_dlpi_dev(): interface %s: %s is not style 2", circuit->interface->name, devpath);
			close(fd);
			return ISIS_WARNING;
		}
		...
		if(dia->dl_provider_style != DL_STYLE1) {
			zlog_warn("open_dlpi_dev(): interface %s: %s is not style 1", circuit->interface->name, devpath);
			close(fd);
			return ISIS_WARNING;
		}
	}
	...
	if((dia->dl_sap_length != 2 && dia->dl_sap_length != -2) || dia->dl_service_mode != DL_CLDLS || dia->dl_addr_length != ETHERADDRL + 2 || dia->dl_brdcst_addr_length != ETHERADDRL) {
		zlog_warn("%s: unsupported interface type for %s", __func__, circuit->interface->name);
		close(fd);
		return ISIS_WARNING;
	}
	switch(dia->dl_mac_type) {
		...
		case DL_100BT: break;
		default:
			zlog_warn("%s: unexpected mac type on %s: %lld", __func__, circuit->interface->name, (long long) dia->dl_mac_type);
			close(fd);
			return ISIS_WARNING;
	}
   ...
   */
	if(dlpiaddr(fd, circuit->u.bc.snpa) == -1) {
		zlog_warn("open_dlpi_dev(): interface %s: unable to get MAC address", circuit->interface->name);
		close(fd);
		return ISIS_WARNING;
	}
	...
	if(dlpibind(fd) == -1) {
		zlog_warn("%s: cannot bind SAP 0 on %s", __func__, circuit->interface->name);
		close(fd);
		return ISIS_WARNING;
	}
   ...
   */
	...
	if(retval != 0) {
		zlog_warn("%s: unable to join multicast on %s", __func__, circuit->interface->name);
		close(fd);
		return ISIS_WARNING;
	}
	...
	if(ioctl(fd, I_PUSH, "pfmod") == 0) {
		...
		if(ioctl(fd, I_STR, &sioc) == -1) {
			zlog_warn("%s: could not perform PF_IOCSETF on %s", __func__, circuit->interface->name);
		}
	}
...
int isis_recv_pdu_bcast(struct isis_circuit *circuit, u_char *ssnpa) {
	...
	if(retv < 0) {
		zlog_warn("isis_recv_pdu_bcast: getmsg failed: %s", safe_strerror(errno));
		return ISIS_WARNING;
	}
	...
	if(ctlbuf.len < (ssize_t) DL_UNITDATA_IND_SIZE || dui->dl_primitive != DL_UNITDATA_IND) {
		return ISIS_WARNING;
	}

	if(dui->dl_src_addr_length != ETHERADDRL + 2 || dui->dl_src_addr_offset < DL_UNITDATA_IND_SIZE || dui->dl_src_addr_offset + dui->dl_src_addr_length > (size_t) ctlbuf.len) {
		return ISIS_WARNING;
	}

	memcpy(ssnpa, (char *) dui + dui->dl_src_addr_offset + (circuit->sap_length > 0 ? circuit->sap_length : 0), ETHERADDRL);

	if(databuf.len < LLC_LEN || sock_buff[0] != ISO_SAP || sock_buff[1] != ISO_SAP || sock_buff[2] != 3) {
		return ISIS_WARNING;
	}

	stream_write(circuit->rcv_stream, sock_buff + LLC_LEN, databuf.len - LLC_LEN);
...
int isis_send_pdu_bcast(struct isis_circuit *circuit, int level) {
	...
	*dstsap = buflen;
	...
	sock_buff[1] = ISO_SAP;
	sock_buff[2] = 0x03;
	memcpy(sock_buff + LLC_LEN, circuit->snd_stream->data, stream_get_endp(circuit->snd_stream));
	rv = dlpisend(circuit->fd, dur, sizeof(*dur) + dur->dl_dest_addr_length, sock_buff, buflen, 0);
	if(rv < 0) {
		zlog_warn("IS-IS dlpi: could not transmit packet on %s: %s", circuit->interface->name, safe_strerror(errno));
