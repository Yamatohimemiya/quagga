...
/* Utility mask array. */
static const u_char maskbit[] = { 0x00, 0x80, 0xc0, 0xe0, 0xf0, 0xf8, 0xfc, 0xfe, 0xff };
...
static int area_match(struct list *left, struct list *right) {
	...
	for(ALL_LIST_ELEMENTS_RO(left, node1, addr1)) {
		for(ALL_LIST_ELEMENTS_RO(right, node2, addr2)) {
			if(addr1->addr_len == addr2->addr_len && !memcmp(addr1->area_addr, addr2->area_addr, (int) addr1->addr_len)) {
				return 1; /* match */
			}
		}
	}
 ...
 */
static int authentication_check(struct isis_passwd *remote, struct isis_passwd *local, struct stream *stream, uint32_t auth_tlv_offset) {
	...
	switch(local->type) {
		...
		case ISIS_PASSWD_TYPE_UNUSED: break;
		...
		case ISIS_PASSWD_TYPE_HMAC_MD5:
			...
			memset(STREAM_DATA(stream) + auth_tlv_offset + 3, 0, ISIS_AUTH_MD5_SIZE);
			/* Compute the digest */
			hmac_md5(STREAM_DATA(stream), stream_get_endp(stream), (unsigned char *) &(local->passwd), local->len, (unsigned char *) &digest);
			/* Copy back the authentication value after the check */
			memcpy(STREAM_DATA(stream) + auth_tlv_offset + 3, remote->passwd, ISIS_AUTH_MD5_SIZE);
			return memcmp(digest, remote->passwd, ISIS_AUTH_MD5_SIZE);

		default: zlog_err("Unsupported authentication type"); return ISIS_ERROR;
	}
...
static int lsp_authentication_check(struct stream *stream, struct isis_area *area, int level, struct isis_passwd *passwd) {
	...
	retval = parse_tlvs(area->area_tag, STREAM_PNT(stream) + ISIS_LSP_HDR_LEN, pdu_len - ISIS_FIXED_HDR_LEN - ISIS_LSP_HDR_LEN, &expected, &found, &tlvs, &auth_tlv_offset);

	if(retval != ISIS_OK) {
		zlog_err(
			"ISIS-Upd (%s): Parse failed L%d LSP %s, seq 0x%08x, "
			"cksum 0x%04x, lifetime %us, len %u",
			area->area_tag, level, rawlspid_print(hdr->lsp_id), ntohl(hdr->seq_num), ntohs(hdr->checksum), ntohs(hdr->rem_lifetime), pdu_len
		);
		if((isis->debugs & DEBUG_UPDATE_PACKETS) && (isis->debugs & DEBUG_PACKET_DUMP)) {
	...
	if(tlvs.auth_info.type != ISIS_PASSWD_TYPE_CLEARTXT && tlvs.auth_info.type != ISIS_PASSWD_TYPE_HMAC_MD5) {
   ...
   */
	...
	hdr->checksum = 0;
	hdr->rem_lifetime = 0;
	retval = authentication_check(&tlvs.auth_info, passwd, stream, auth_tlv_offset);
...
static void tlvs_to_adj_area_addrs(struct tlvs *tlvs, struct isis_adjacency *adj) {
...
static void tlvs_to_adj_ipv4_addrs(struct tlvs *tlvs, struct isis_adjacency *adj) {
	struct listnode *node;
...
#ifdef HAVE_IPV6
static void tlvs_to_adj_ipv6_addrs(struct tlvs *tlvs, struct isis_adjacency *adj) {
	struct listnode *node;
...
static int process_p2p_hello(struct isis_circuit *circuit) {
	...
	if(isis->debugs & DEBUG_ADJ_PACKETS) {
		zlog_debug("ISIS-Adj (%s): Rcvd P2P IIH on %s, cirType %s, cirID %u", circuit->area->area_tag, circuit->interface->name, circuit_t2string(circuit->is_type), circuit->circuit_id);
		if(isis->debugs & DEBUG_PACKET_DUMP) {
			zlog_dump_data(STREAM_DATA(circuit->rcv_stream), stream_get_endp(circuit->rcv_stream));
		}
	}
	...
	if((stream_get_endp(circuit->rcv_stream) - stream_get_getp(circuit->rcv_stream)) < ISIS_P2PHELLO_HDRLEN) {
   ...
   */
	...
	if(pdu_len < (ISIS_FIXED_HDR_LEN + ISIS_P2PHELLO_HDRLEN) || pdu_len > ISO_MTU(circuit) || pdu_len > stream_get_endp(circuit->rcv_stream)) {
		zlog_warn(
   ...
   */
	...
	retval = parse_tlvs(circuit->area->area_tag, STREAM_PNT(circuit->rcv_stream), pdu_len - ISIS_P2PHELLO_HDRLEN - ISIS_FIXED_HDR_LEN, &expected, &found, &tlvs, &auth_tlv_offset);
	...
	if(circuit->passwd.type) {
		if(!(found & TLVFLAG_AUTH_INFO) || authentication_check(&tlvs.auth_info, &circuit->passwd, circuit->rcv_stream, auth_tlv_offset)) {
			isis_event_auth_failure(circuit->area->area_tag, "P2P hello authentication failure", hdr->source_id);
			free_tlvs(&tlvs);
			return ISIS_OK;
		}
	}
   ...
   */
	if(found & TLVFLAG_IPV4_ADDR) {
		...
		zlog_warn(
			"ISIS-Adj: no IPv4 in P2P IIH from %s "
   ...
   */
	if(!memcmp(hdr->source_id, isis->sysid, ISIS_SYS_ID_LEN)) {
		zlog_warn("ISIS-Adj (%s): it's own IIH PDU - discarded", circuit->area->area_tag);
		free_tlvs(&tlvs);
		return ISIS_WARNING;
	}
   ...
   * packets */
	...
	THREAD_TIMER_ON(master, adj->t_expire, isis_adj_expire, adj, (long) adj->hold_time);
		...
		if(circuit->area->is_type == IS_LEVEL_1_AND_2) {
			switch(hdr->circuit_t) {
				...
				case IS_LEVEL_2:
					...
					} else if((adj->adj_usage == ISIS_ADJ_LEVEL1) || (adj->adj_usage == ISIS_ADJ_LEVEL1AND2)) {
						/* (8) down - wrong system */
						isis_adj_state_change(adj, ISIS_ADJ_DOWN, "Wrong System");
   ...
   */
	switch(adj->adj_usage) {
		case ISIS_ADJ_LEVEL1: adj->sys_type = ISIS_SYSTYPE_L1_IS; break;
		case ISIS_ADJ_LEVEL2: adj->sys_type = ISIS_SYSTYPE_L2_IS; break;
		case ISIS_ADJ_LEVEL1AND2: adj->sys_type = ISIS_SYSTYPE_L2_IS; break;
		case ISIS_ADJ_NONE: adj->sys_type = ISIS_SYSTYPE_UNKNOWN; break;
	}
	...
	if(isis->debugs & DEBUG_ADJ_PACKETS) {
		zlog_debug(
			...
			" cir id %02d, length %d",
			circuit->area->area_tag, circuit->interface->name, circuit_t2string(circuit->is_type), circuit->circuit_id, pdu_len
		);
	}
 ...
 */
static int process_lan_hello(int level, struct isis_circuit *circuit, const u_char *ssnpa) {
	int retval = ISIS_OK;
	...
	if(isis->debugs & DEBUG_ADJ_PACKETS) {
		zlog_debug(
			...
			"cirID %u",
			circuit->area->area_tag, level, circuit->interface->name, circuit_t2string(circuit->is_type), circuit->circuit_id
		);
		if(isis->debugs & DEBUG_PACKET_DUMP) {
			zlog_dump_data(STREAM_DATA(circuit->rcv_stream), stream_get_endp(circuit->rcv_stream));
		}
	...
	if((stream_get_endp(circuit->rcv_stream) - stream_get_getp(circuit->rcv_stream)) < ISIS_LANHELLO_HDRLEN) {
		zlog_warn("Packet too short");
		return ISIS_WARNING;
	}
	...
	if(!accept_level(level, circuit->is_type)) {
		if(isis->debugs & DEBUG_ADJ_PACKETS) {
			zlog_debug("ISIS-Adj (%s): Interface level mismatch, %s", circuit->area->area_tag, circuit->interface->name);
		}
		return ISIS_WARNING;
	}
   ...
   */
	...
	if(hdr.pdu_len < (ISIS_FIXED_HDR_LEN + ISIS_LANHELLO_HDRLEN) || hdr.pdu_len > ISO_MTU(circuit) || hdr.pdu_len > stream_get_endp(circuit->rcv_stream)) {
		zlog_warn(
			"ISIS-Adj (%s): Rcvd LAN IIH from (%s) with "
			"invalid pdu length %d",
			circuit->area->area_tag, circuit->interface->name, hdr.pdu_len
		);
		return ISIS_WARNING;
	}
   ...
   */
	...
	if(hdr.circuit_t != IS_LEVEL_1 && hdr.circuit_t != IS_LEVEL_2 && hdr.circuit_t != IS_LEVEL_1_AND_2 && (level & hdr.circuit_t) == 0) {
   ...
   */
	...
	auth_tlv_offset = stream_get_getp(circuit->rcv_stream);
	retval = parse_tlvs(circuit->area->area_tag, STREAM_PNT(circuit->rcv_stream), hdr.pdu_len - ISIS_LANHELLO_HDRLEN - ISIS_FIXED_HDR_LEN, &expected, &found, &tlvs, &auth_tlv_offset);
	...
	if(!(found & TLVFLAG_NLPID)) {
		zlog_warn("No supported protocols TLV in Level %d LAN IS to IS hello", level);
		retval = ISIS_WARNING;
		goto out;
	}
	...
	if(circuit->passwd.type) {
		if(!(found & TLVFLAG_AUTH_INFO) || authentication_check(&tlvs.auth_info, &circuit->passwd, circuit->rcv_stream, auth_tlv_offset)) {
			isis_event_auth_failure(circuit->area->area_tag, "LAN hello authentication failure", hdr.source_id);
			retval = ISIS_WARNING;
			goto out;
		}
	}
	...
	if(!memcmp(hdr.source_id, isis->sysid, ISIS_SYS_ID_LEN)) {
		zlog_warn("ISIS-Adj (%s): duplicate system ID on interface %s", circuit->area->area_tag, circuit->interface->name);
		return ISIS_WARNING;
	}
   ...
   */
	if(listcount(circuit->area->area_addrs) == 0 || (level == IS_LEVEL_1 && area_match(circuit->area->area_addrs, tlvs.area_addrs) == 0)) {
		if(isis->debugs & DEBUG_ADJ_PACKETS) {
			zlog_debug("ISIS-Adj (%s): Area mismatch, level %d IIH on %s", circuit->area->area_tag, level, circuit->interface->name);
		}
		retval = ISIS_OK;
		goto out;
	}
   ...
   */
	if(!memcmp(circuit->u.bc.snpa, ssnpa, ETH_ALEN)) {
		zlog_debug("ISIS-Adj (%s): it's own IIH PDU - discarded", circuit->area->area_tag);
   ...
   */
	if(found & TLVFLAG_IPV4_ADDR) {
		...
		zlog_warn(
			"ISIS-Adj: no IPv4 in LAN IIH from %s "
			"(this isisd has no IPv6)\n",
			circuit->interface->name
		);
	...
	if((adj == NULL) || (memcmp(adj->snpa, ssnpa, ETH_ALEN)) || (adj->level != level)) {
		if(!adj) {
			/*
           * Do as in 8.4.2.5
           */
		...
		isis_adj_build_neigh_list(circuit->u.bc.adjdb[level - 1], circuit->u.bc.lan_neighs[level - 1]);
	}
   ...
   */
	...
	if(found & TLVFLAG_LAN_NEIGHS) {
		if(adj->adj_state != ISIS_ADJ_UP) {
			...
			if(found == 0) {
				isis_adj_state_change(adj, ISIS_ADJ_INITIALIZING, "own SNPA not found in LAN Neighbours TLV");
			}
		}
	} else if(adj->adj_state == ISIS_ADJ_UP) {
		isis_adj_state_change(adj, ISIS_ADJ_INITIALIZING, "no LAN Neighbours TLV found");
	}
...
out:
	if(isis->debugs & DEBUG_ADJ_PACKETS) {
		zlog_debug(
			...
			"cirID %u, length %zd",
			circuit->area->area_tag, level, snpa_print(ssnpa), circuit->interface->name, circuit_t2string(circuit->is_type), circuit->circuit_id, stream_get_endp(circuit->rcv_stream)
		);
	}
 ...
 */
static int process_lsp(int level, struct isis_circuit *circuit, const u_char *ssnpa) {
	struct isis_link_state_hdr *hdr;
	...
	if(isis->debugs & DEBUG_UPDATE_PACKETS) {
		zlog_debug("ISIS-Upd (%s): Rcvd L%d LSP on %s, cirType %s, cirID %u", circuit->area->area_tag, level, circuit->interface->name, circuit_t2string(circuit->is_type), circuit->circuit_id);
		if(isis->debugs & DEBUG_PACKET_DUMP) {
			zlog_dump_data(STREAM_DATA(circuit->rcv_stream), stream_get_endp(circuit->rcv_stream));
		}
	}

	if((stream_get_endp(circuit->rcv_stream) - stream_get_getp(circuit->rcv_stream)) < ISIS_LSP_HDR_LEN) {
		zlog_warn("Packet too short");
		return ISIS_WARNING;
	}
	...
	/* lsp length check */
	if(pdu_len < (ISIS_FIXED_HDR_LEN + ISIS_LSP_HDR_LEN) || pdu_len > ISO_MTU(circuit) || pdu_len > stream_get_endp(circuit->rcv_stream)) {
		zlog_debug("ISIS-Upd (%s): LSP %s invalid LSP length %d", circuit->area->area_tag, rawlspid_print(hdr->lsp_id), pdu_len);
   ...
   */
	...
	if(isis->debugs & DEBUG_UPDATE_PACKETS) {
		zlog_debug(
			...
			circuit->area->area_tag, level, rawlspid_print(hdr->lsp_id), ntohl(hdr->seq_num), ntohs(hdr->checksum), ntohs(hdr->rem_lifetime), pdu_len, circuit->interface->name
		);
	}

	/* lsp is_type check */
	if((hdr->lsp_bits & IS_LEVEL_1_AND_2) != IS_LEVEL_1 && (hdr->lsp_bits & IS_LEVEL_1_AND_2) != IS_LEVEL_1_AND_2) {
		zlog_debug("ISIS-Upd (%s): LSP %s invalid LSP is type %x", circuit->area->area_tag, rawlspid_print(hdr->lsp_id), hdr->lsp_bits);
	...
	if(iso_csum_verify(STREAM_PNT(circuit->rcv_stream) + 4, pdu_len - 12, &hdr->checksum)) {
		zlog_debug("ISIS-Upd (%s): LSP %s invalid LSP checksum 0x%04x", circuit->area->area_tag, rawlspid_print(hdr->lsp_id), ntohs(hdr->checksum));
	...
	if(circuit->ext_domain) {
		zlog_debug(
			...
			"externalDomain = true",
			circuit->area->area_tag, rawlspid_print(hdr->lsp_id), level
		);
	...
	if(!accept_level(level, circuit->is_type)) {
		zlog_debug(
			...
			" type %s",
			circuit->area->area_tag, rawlspid_print(hdr->lsp_id), level, circuit_t2string(circuit->is_type)
		);
	...
	(level == IS_LEVEL_1) ? (passwd = &circuit->area->area_passwd) : (passwd = &circuit->area->domain_passwd);
	if(passwd->type) {
		if(lsp_authentication_check(circuit->rcv_stream, circuit->area, level, passwd)) {
			isis_event_auth_failure(circuit->area->area_tag, "LSP authentication failure", hdr->lsp_id);
			return ISIS_WARNING;
		}
	}
	...
	if(lsp) {
		comp = lsp_compare(circuit->area->area_tag, lsp, hdr->seq_num, hdr->checksum, hdr->rem_lifetime);
	}
	if(lsp
	   && (lsp->own_lsp
	...
	if(circuit->circ_type == CIRCUIT_T_BROADCAST) {
		...
		if(!adj) {
			zlog_debug(
				...
				"lifetime %us on %s",
				circuit->area->area_tag, rawlspid_print(hdr->lsp_id), ntohl(hdr->seq_num), ntohs(hdr->checksum), ntohs(hdr->rem_lifetime), circuit->interface->name
			);
	...
	/* for non broadcast, we just need to find same level adj */
		...
		if(!circuit->u.p2p.neighbor) {
			...
			if(((level == IS_LEVEL_1) && (circuit->u.p2p.neighbor->adj_usage == ISIS_ADJ_LEVEL2)) || ((level == IS_LEVEL_2) && (circuit->u.p2p.neighbor->adj_usage == ISIS_ADJ_LEVEL1))) {
				return ISIS_WARNING; /* Silently discard */
			}
			adj = circuit->u.p2p.neighbor;
		}
...
dontcheckadj:
   ...
   *            wrong checksum, initiate a purge. */
	if(lsp && (lsp->lsp_header->seq_num == hdr->seq_num) && (lsp->lsp_header->checksum != hdr->checksum)) {
		zlog_warn("ISIS-Upd (%s): LSP %s seq 0x%08x with confused checksum received.", circuit->area->area_tag, rawlspid_print(hdr->lsp_id), ntohl(hdr->seq_num));
		hdr->rem_lifetime = 0;
		lsp_confusion = 1;
	} else {
		lsp_confusion = 0;
		   ...
		   * through incoming circuit as usual */
				...
				if(isis->debugs & DEBUG_UPDATE_PACKETS) {
					zlog_debug(
						...
						"seq 0x%08x",
						circuit->area->area_tag, rawlspid_print(hdr->lsp_id), ntohl(lsp->lsp_header->seq_num)
					);
				}
			}
		}
   ...
   * purge */
	if(memcmp(hdr->lsp_id, isis->sysid, ISIS_SYS_ID_LEN) == 0) {
	   ...
	   */
			...
			/* i */
			if(!lsp) {
				lsp = lsp_new_from_stream_ptr(circuit->rcv_stream, pdu_len, lsp0, circuit->area, level);
...
static int process_snp(int snp_type, int level, struct isis_circuit *circuit, const u_char *ssnpa) {
	int retval = ISIS_OK;
	int cmp, own_lsp;
	char typechar = ' ';
	uint16_t pdu_len;
	...
	if(snp_type == ISIS_SNP_CSNP_FLAG) {
		...
		if(pdu_len < (ISIS_FIXED_HDR_LEN + ISIS_CSNP_HDRLEN) || pdu_len > ISO_MTU(circuit) || pdu_len > stream_get_endp(circuit->rcv_stream)) {
		...
		if(pdu_len < (ISIS_FIXED_HDR_LEN + ISIS_PSNP_HDRLEN) || pdu_len > ISO_MTU(circuit) || pdu_len > stream_get_endp(circuit->rcv_stream)) {
   ...
   */
	...
	if(!accept_level(level, circuit->is_type)) {
		zlog_debug(
			...
			circuit->area->area_tag, level, typechar, circuit->interface->name, circuit_t2string(circuit->is_type), level
		);
	...
	if((snp_type == ISIS_SNP_PSNP_FLAG) && (circuit->circ_type == CIRCUIT_T_BROADCAST) && (!circuit->u.bc.is_dr[level - 1])) {
		zlog_debug(
			"ISIS-Snp (%s): Rcvd L%d %cSNP from %s on %s, "
			"skipping: we are not the DIS",
			circuit->area->area_tag, level, typechar, snpa_print(ssnpa), circuit->interface->name
		);
   ...
   * - already checked */
	...
	auth_tlv_offset = stream_get_getp(circuit->rcv_stream);
	retval = parse_tlvs(circuit->area->area_tag, STREAM_PNT(circuit->rcv_stream), pdu_len - stream_get_getp(circuit->rcv_stream), &expected, &found, &tlvs, &auth_tlv_offset);
	...
	if(CHECK_FLAG(passwd->snp_auth, SNP_AUTH_RECV)) {
		if(passwd->type) {
			if(!(found & TLVFLAG_AUTH_INFO) || authentication_check(&tlvs.auth_info, passwd, circuit->rcv_stream, auth_tlv_offset)) {
	...
	if(isis->debugs & DEBUG_SNP_PACKETS) {
		zlog_debug("ISIS-Snp (%s): Rcvd L%d %cSNP from %s on %s", circuit->area->area_tag, level, typechar, snpa_print(ssnpa), circuit->interface->name);
		if(tlvs.lsp_entries) {
			for(ALL_LIST_ELEMENTS_RO(tlvs.lsp_entries, node, entry)) {
				zlog_debug(
					"ISIS-Snp (%s):         %cSNP entry %s, seq 0x%08x,"
					" cksum 0x%04x, lifetime %us",
					circuit->area->area_tag, typechar, rawlspid_print(entry->lsp_id), ntohl(entry->seq_num), ntohs(entry->checksum), ntohs(entry->rem_lifetime)
				);
			}
		}
	}
	...
	if(tlvs.lsp_entries) {
		for(ALL_LIST_ELEMENTS_RO(tlvs.lsp_entries, node, entry)) {
			...
			if(lsp) {
				...
				cmp = lsp_compare(circuit->area->area_tag, lsp, entry->seq_num, entry->checksum, entry->rem_lifetime);
	     ...
	     * insert it and set SSN on it */
				if(entry->rem_lifetime && entry->checksum && entry->seq_num && memcmp(entry->lsp_id, isis->sysid, ISIS_SYS_ID_LEN)) {
					lsp = lsp_new(circuit->area, entry->lsp_id, ntohs(entry->rem_lifetime), 0, 0, entry->checksum, level);
					lsp_insert(lsp, circuit->area->lspdb[level - 1]);
	...
	if(snp_type == ISIS_SNP_CSNP_FLAG) {
       ...
       */
		lsp_list = list_new();
		lsp_build_list_nonzero_ht(chdr->start_lsp_id, chdr->stop_lsp_id, lsp_list, circuit->area->lspdb[level - 1]);
...
static int process_csnp(int level, struct isis_circuit *circuit, const u_char *ssnpa) {
	if(isis->debugs & DEBUG_SNP_PACKETS) {
		zlog_debug("ISIS-Snp (%s): Rcvd L%d CSNP on %s, cirType %s, cirID %u", circuit->area->area_tag, level, circuit->interface->name, circuit_t2string(circuit->is_type), circuit->circuit_id);
		if(isis->debugs & DEBUG_PACKET_DUMP) {
			zlog_dump_data(STREAM_DATA(circuit->rcv_stream), stream_get_endp(circuit->rcv_stream));
		}
	}
	...
	if((stream_get_endp(circuit->rcv_stream) - stream_get_getp(circuit->rcv_stream)) < ISIS_CSNP_HDRLEN) {
...
static int process_psnp(int level, struct isis_circuit *circuit, const u_char *ssnpa) {
	if(isis->debugs & DEBUG_SNP_PACKETS) {
		zlog_debug("ISIS-Snp (%s): Rcvd L%d PSNP on %s, cirType %s, cirID %u", circuit->area->area_tag, level, circuit->interface->name, circuit_t2string(circuit->is_type), circuit->circuit_id);
		if(isis->debugs & DEBUG_PACKET_DUMP) {
			zlog_dump_data(STREAM_DATA(circuit->rcv_stream), stream_get_endp(circuit->rcv_stream));
		}
	}

	if((stream_get_endp(circuit->rcv_stream) - stream_get_getp(circuit->rcv_stream)) < ISIS_PSNP_HDRLEN) {
...
static int isis_handle_pdu(struct isis_circuit *circuit, u_char *ssnpa) {
   ...
   */
	...
	if(circuit->is_passive) {
		zlog_warn("Received ISIS PDU on passive circuit %s", circuit->interface->name);
		return ISIS_WARNING;
	}
	...
	switch(hdr->pdu_type) {
		case L1_LAN_HELLO: retval = process_lan_hello(ISIS_LEVEL1, circuit, ssnpa); break;
		case L2_LAN_HELLO: retval = process_lan_hello(ISIS_LEVEL2, circuit, ssnpa); break;
		case P2P_HELLO: retval = process_p2p_hello(circuit); break;
		case L1_LINK_STATE: retval = process_lsp(ISIS_LEVEL1, circuit, ssnpa); break;
		case L2_LINK_STATE: retval = process_lsp(ISIS_LEVEL2, circuit, ssnpa); break;
		case L1_COMPLETE_SEQ_NUM: retval = process_csnp(ISIS_LEVEL1, circuit, ssnpa); break;
		case L2_COMPLETE_SEQ_NUM: retval = process_csnp(ISIS_LEVEL2, circuit, ssnpa); break;
		case L1_PARTIAL_SEQ_NUM: retval = process_psnp(ISIS_LEVEL1, circuit, ssnpa); break;
		case L2_PARTIAL_SEQ_NUM: retval = process_psnp(ISIS_LEVEL2, circuit, ssnpa); break;
		default: return ISIS_ERROR;
	}
...
int isis_receive(struct thread *thread) {
   ...
   */
	if(!circuit->is_passive) {
		circuit->t_read = thread_add_timer_msec(master, isis_receive, circuit, listcount(circuit->area->circuit_list) * 100);
	}
...
void fill_fixed_hdr(struct isis_fixed_hdr *hdr, u_char pdu_type) {
	...
	switch(pdu_type) {
		case L1_LAN_HELLO:
		case L2_LAN_HELLO: hdr->length = ISIS_LANHELLO_HDRLEN; break;
		case P2P_HELLO: hdr->length = ISIS_P2PHELLO_HDRLEN; break;
		case L1_LINK_STATE:
		case L2_LINK_STATE: hdr->length = ISIS_LSP_HDR_LEN; break;
		case L1_COMPLETE_SEQ_NUM:
		case L2_COMPLETE_SEQ_NUM: hdr->length = ISIS_CSNP_HDRLEN; break;
		case L1_PARTIAL_SEQ_NUM:
		case L2_PARTIAL_SEQ_NUM: hdr->length = ISIS_PSNP_HDRLEN; break;
		default: zlog_warn("fill_fixed_hdr(): unknown pdu type %d", pdu_type); return;
	}
 ...
 * SEND SIDE                             
 */
static void fill_fixed_hdr_andstream(struct isis_fixed_hdr *hdr, u_char pdu_type, struct stream *stream) {
...
int send_hello(struct isis_circuit *circuit, int level) {
   ...
   */
	...
	interval = circuit->hello_multiplier[level - 1] * circuit->hello_interval[level - 1];
   ...
   */
	...
	switch(circuit->passwd.type) {
		/* Cleartext */
		case ISIS_PASSWD_TYPE_CLEARTXT:
			if(tlv_add_authinfo(circuit->passwd.type, circuit->passwd.len, circuit->passwd.passwd, circuit->snd_stream)) {
				return ISIS_WARNING;
			}
			break;
		...
		case ISIS_PASSWD_TYPE_HMAC_MD5:
			...
			if(tlv_add_authinfo(circuit->passwd.type, ISIS_AUTH_MD5_SIZE, hmac_md5_hash, circuit->snd_stream)) {
				return ISIS_WARNING;
			}
			break;
	...
	/*  LAN Neighbors TLV */
	if(circuit->circ_type == CIRCUIT_T_BROADCAST) {
		if(level == IS_LEVEL_1 && circuit->u.bc.lan_neighs[0] && listcount(circuit->u.bc.lan_neighs[0]) > 0) {
			if(tlv_add_lan_neighs(circuit->u.bc.lan_neighs[0], circuit->snd_stream)) {
				return ISIS_WARNING;
			}
		}
		if(level == IS_LEVEL_2 && circuit->u.bc.lan_neighs[1] && listcount(circuit->u.bc.lan_neighs[1]) > 0) {
			if(tlv_add_lan_neighs(circuit->u.bc.lan_neighs[1], circuit->snd_stream)) {
				return ISIS_WARNING;
			}
		}
	}
	...
	if(circuit->ip_router && circuit->ip_addrs && listcount(circuit->ip_addrs) > 0) {
	...
	if(circuit->ipv6_router && circuit->ipv6_link && listcount(circuit->ipv6_link) > 0) {
	...
	if(circuit->passwd.type == ISIS_PASSWD_TYPE_HMAC_MD5) {
		hmac_md5(STREAM_DATA(circuit->snd_stream), stream_get_endp(circuit->snd_stream), (unsigned char *) &circuit->passwd.passwd, circuit->passwd.len, (unsigned char *) &hmac_md5_hash);
		/* Copy the hash into the stream */
		memcpy(STREAM_DATA(circuit->snd_stream) + auth_tlv_offset + 3, hmac_md5_hash, ISIS_AUTH_MD5_SIZE);
	}

	if(isis->debugs & DEBUG_ADJ_PACKETS) {
		if(circuit->circ_type == CIRCUIT_T_BROADCAST) {
			zlog_debug("ISIS-Adj (%s): Sending L%d LAN IIH on %s, length %zd", circuit->area->area_tag, level, circuit->interface->name, length);
		} else {
			zlog_debug("ISIS-Adj (%s): Sending P2P IIH on %s, length %zd", circuit->area->area_tag, circuit->interface->name, length);
		}
		if(isis->debugs & DEBUG_PACKET_DUMP) {
			zlog_dump_data(STREAM_DATA(circuit->snd_stream), stream_get_endp(circuit->snd_stream));
		}
	}

	retval = circuit->tx(circuit, level);
	if(retval != ISIS_OK) {
		zlog_err("ISIS-Adj (%s): Send L%d IIH on %s failed", circuit->area->area_tag, level, circuit->interface->name);
	}
...
int send_lan_l1_hello(struct thread *thread) {
	...
	if(!(circuit->area->is_type & IS_LEVEL_1)) {
		zlog_warn("ISIS-Hello (%s): Trying to send L1 IIH in L2-only area", circuit->area->area_tag);
		return 1;
	}
	...
	THREAD_TIMER_ON(master, circuit->u.bc.t_send_lan_hello[0], send_lan_l1_hello, circuit, isis_jitter(circuit->hello_interval[0], IIH_JITTER));
...
int send_lan_l2_hello(struct thread *thread) {
	...
	if(!(circuit->area->is_type & IS_LEVEL_2)) {
		zlog_warn("ISIS-Hello (%s): Trying to send L2 IIH in L1 area", circuit->area->area_tag);
		return 1;
	}
	...
	/* set next timer thread */
	THREAD_TIMER_ON(master, circuit->u.bc.t_send_lan_hello[1], send_lan_l2_hello, circuit, isis_jitter(circuit->hello_interval[1], IIH_JITTER));
...
int send_p2p_hello(struct thread *thread) {
	...
	/* set next timer thread */
	THREAD_TIMER_ON(master, circuit->u.p2p.t_send_p2p_hello, send_p2p_hello, circuit, isis_jitter(circuit->hello_interval[1], IIH_JITTER));

	return ISIS_OK;
}

static int build_csnp(int level, u_char *start, u_char *stop, struct list *lsps, struct isis_circuit *circuit) {
	struct isis_fixed_hdr fixed_hdr;
	struct isis_passwd *passwd;
	unsigned long lenp;
	u_int16_t length;
	...
	if(level == IS_LEVEL_1) {
		fill_fixed_hdr_andstream(&fixed_hdr, L1_COMPLETE_SEQ_NUM, circuit->snd_stream);
	} else {
		fill_fixed_hdr_andstream(&fixed_hdr, L2_COMPLETE_SEQ_NUM, circuit->snd_stream);
	}
   ...
   */
	...
	if(CHECK_FLAG(passwd->snp_auth, SNP_AUTH_SEND)) {
		switch(passwd->type) {
			/* Cleartext */
			case ISIS_PASSWD_TYPE_CLEARTXT:
				if(tlv_add_authinfo(ISIS_PASSWD_TYPE_CLEARTXT, passwd->len, passwd->passwd, circuit->snd_stream)) {
					return ISIS_WARNING;
				}
				break;
			...
			case ISIS_PASSWD_TYPE_HMAC_MD5:
				...
				auth_tlv_offset = stream_get_endp(circuit->snd_stream);
				memset(&hmac_md5_hash, 0, ISIS_AUTH_MD5_SIZE);
				if(tlv_add_authinfo(ISIS_PASSWD_TYPE_HMAC_MD5, ISIS_AUTH_MD5_SIZE, hmac_md5_hash, circuit->snd_stream)) {
					return ISIS_WARNING;
				}
				break;
	...
	/* For HMAC MD5 we need to compute the md5 hash and store it */
	if(CHECK_FLAG(passwd->snp_auth, SNP_AUTH_SEND) && passwd->type == ISIS_PASSWD_TYPE_HMAC_MD5) {
		hmac_md5(STREAM_DATA(circuit->snd_stream), stream_get_endp(circuit->snd_stream), (unsigned char *) &passwd->passwd, passwd->len, (unsigned char *) &hmac_md5_hash);
		/* Copy the hash into the stream */
		memcpy(STREAM_DATA(circuit->snd_stream) + auth_tlv_offset + 3, hmac_md5_hash, ISIS_AUTH_MD5_SIZE);
	}
	...
	if(CHECK_FLAG(passwd->snp_auth, SNP_AUTH_SEND)) {
		switch(passwd->type) {
			...
			case ISIS_PASSWD_TYPE_CLEARTXT:
				...
				/* HMAC MD5 */
			case ISIS_PASSWD_TYPE_HMAC_MD5: length += ISIS_AUTH_MD5_SIZE; break;
 ...
 */
static uint16_t max_lsps_per_snp(int snp_type, int level, struct isis_circuit *circuit) {
	int snp_hdr_len;
	int auth_tlv_len;
	uint16_t lsp_count;
	...
	lsp_count = get_max_lsp_count(stream_get_size(circuit->snd_stream) - snp_hdr_len - auth_tlv_len);
	return lsp_count;
}
...
int send_csnp(struct isis_circuit *circuit, int level) {
	...
	if(circuit->area->lspdb[level - 1] == NULL || dict_count(circuit->area->lspdb[level - 1]) == 0) {
		return retval;
	}
	...
	while(loop) {
		list = list_new();
		lsp_build_list(start, stop, num_lsps, list, circuit->area->lspdb[level - 1]);
		/*
       ...
       */
		...
		if(retval != ISIS_OK) {
			zlog_err("ISIS-Snp (%s): Build L%d CSNP on %s failed", circuit->area->area_tag, level, circuit->interface->name);
			list_delete(list);
			return retval;
		}

		if(isis->debugs & DEBUG_SNP_PACKETS) {
			zlog_debug("ISIS-Snp (%s): Sending L%d CSNP on %s, length %zd", circuit->area->area_tag, level, circuit->interface->name, stream_get_endp(circuit->snd_stream));
			for(ALL_LIST_ELEMENTS_RO(list, node, lsp)) {
				zlog_debug(
					"ISIS-Snp (%s):         CSNP entry %s, seq 0x%08x,"
					" cksum 0x%04x, lifetime %us",
					circuit->area->area_tag, rawlspid_print(lsp->lsp_header->lsp_id), ntohl(lsp->lsp_header->seq_num), ntohs(lsp->lsp_header->checksum), ntohs(lsp->lsp_header->rem_lifetime)
				);
			}
			if(isis->debugs & DEBUG_PACKET_DUMP) {
				zlog_dump_data(STREAM_DATA(circuit->snd_stream), stream_get_endp(circuit->snd_stream));
		...
		retval = circuit->tx(circuit, level);
		if(retval != ISIS_OK) {
			zlog_err("ISIS-Snp (%s): Send L%d CSNP on %s failed", circuit->area->area_tag, level, circuit->interface->name);
			list_delete(list);
			return retval;
		}
...
int send_l1_csnp(struct thread *thread) {
	...
	/* set next timer thread */
	THREAD_TIMER_ON(master, circuit->t_send_csnp[0], send_l1_csnp, circuit, isis_jitter(circuit->csnp_interval[0], CSNP_JITTER));
...
int send_l2_csnp(struct thread *thread) {
	...
	/* set next timer thread */
	THREAD_TIMER_ON(master, circuit->t_send_csnp[1], send_l2_csnp, circuit, isis_jitter(circuit->csnp_interval[1], CSNP_JITTER));

	return retval;
}

static int build_psnp(int level, struct isis_circuit *circuit, struct list *lsps) {
	struct isis_fixed_hdr fixed_hdr;
	unsigned long lenp;
	u_int16_t length;
	struct isis_lsp *lsp;
	...
	if(level == IS_LEVEL_1) {
		fill_fixed_hdr_andstream(&fixed_hdr, L1_PARTIAL_SEQ_NUM, circuit->snd_stream);
	} else {
		fill_fixed_hdr_andstream(&fixed_hdr, L2_PARTIAL_SEQ_NUM, circuit->snd_stream);
	}
...
static int send_psnp(int level, struct isis_circuit *circuit) {
	...
	if(circuit->circ_type == CIRCUIT_T_BROADCAST && circuit->u.bc.is_dr[level - 1]) {
		return ISIS_OK;
	}
	...
	while(1) {
		list = list_new();
		lsp_build_list_ssn(circuit, num_lsps, list, circuit->area->lspdb[level - 1]);
		...
		if(retval != ISIS_OK) {
			zlog_err("ISIS-Snp (%s): Build L%d PSNP on %s failed", circuit->area->area_tag, level, circuit->interface->name);
			list_delete(list);
			return retval;
		}

		if(isis->debugs & DEBUG_SNP_PACKETS) {
			zlog_debug("ISIS-Snp (%s): Sending L%d PSNP on %s, length %zd", circuit->area->area_tag, level, circuit->interface->name, stream_get_endp(circuit->snd_stream));
			if(isis->debugs & DEBUG_PACKET_DUMP) {
				zlog_dump_data(STREAM_DATA(circuit->snd_stream), stream_get_endp(circuit->snd_stream));
			}
		}
		...
		retval = circuit->tx(circuit, level);
		if(retval != ISIS_OK) {
			zlog_err("ISIS-Snp (%s): Send L%d PSNP on %s failed", circuit->area->area_tag, level, circuit->interface->name);
			list_delete(list);
			return retval;
		}
...
int send_l2_psnp(struct thread *thread) {
	...
	/* set next timer thread */
	THREAD_TIMER_ON(master, circuit->t_send_psnp[1], send_l2_psnp, circuit, isis_jitter(circuit->psnp_interval[1], PSNP_JITTER));
   ...
   * the circuit's MTU. So handle and log this case here. */
	if(stream_get_endp(lsp->pdu) > stream_get_size(circuit->snd_stream)) {
		zlog_err(
			...
			circuit->area->area_tag, lsp->level, rawlspid_print(lsp->lsp_header->lsp_id), ntohl(lsp->lsp_header->seq_num), ntohs(lsp->lsp_header->checksum), ntohs(lsp->lsp_header->rem_lifetime), circuit->interface->name,
			stream_get_endp(lsp->pdu), stream_get_size(circuit->snd_stream)
		);
		if(isis->debugs & DEBUG_PACKET_DUMP) {
	...
	if(retval != ISIS_OK) {
		zlog_err("ISIS-Upd (%s): Send L%d LSP on %s failed %s", circuit->area->area_tag, lsp->level, circuit->interface->name, (retval == ISIS_WARNING) ? "temporarily" : "permanently");
	}

out:
	if(clear_srm || (retval == ISIS_OK && circuit->circ_type == CIRCUIT_T_BROADCAST) || (retval != ISIS_OK && retval != ISIS_WARNING)) {
...
int ack_lsp(struct isis_link_state_hdr *hdr, struct isis_circuit *circuit, int level) {
	unsigned long lenp;
	int retval;
	u_int16_t length;
	struct isis_fixed_hdr fixed_hdr;
	...
	retval = circuit->tx(circuit, level);
	if(retval != ISIS_OK) {
		zlog_err("ISIS-Upd (%s): Send L%d LSP PSNP on %s failed", circuit->area->area_tag, level, circuit->interface->name);
	}
