...
const char *graph_type[] = { "double cycle", "cycle", "path" };
...
long x, y, yy1, yy2, yyp, dl, dx, xn, yyn, count, *mess;

double n;
long n0, source, i, i0, j, dij;
...
int i_f = 0, ip_f = 0, ix_f = 0, ih_f = 0, im_f = 0, il_f = 0, in_f = 0, is_f = 0;
...
void free_arc(void *val) {
	free(val);
}

void print_arc(struct vty *vty, struct list *topology, long i, long j, long length) {
	struct arc *myarc;
		...
		switch(args[1]) {
			case 'c': /* spanning graph in one layer */
				...
				switch(args[2]) {
					...
					default: /* unknown switch  value */ usage(vty); return 1;
				}
				break;
			...
			default: /* unknoun case */ usage(vty); return 1;
		}
	}
...
int gen_spgrid_topology(struct vty *vty, struct list *topology) {
	...
	switch(cw) {
		case PATH: mc = (double) Y - 1; break;
		case CYCLE: mc = (double) Y; break;
		case DOUBLE_CYCLE: mc = 2 * (double) Y;
	}
	...
	for(x = 0; x < X - 1; x++) {
		...
		for(count = 0, xn = x + 1; count < ix && xn < X; count++, xn += ih) {
			if(ip_f) {
				for(y = 0; y < Y; y++) {
					mess[y] = y;
				}
