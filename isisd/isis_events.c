...
void isis_event_circuit_state_change(struct isis_circuit *circuit, struct isis_area *area, int up) {
	area->circuit_state_changes++;

	if(isis->debugs & DEBUG_EVENTS) {
		zlog_debug("ISIS-Evt (%s) circuit %s", area->area_tag, up ? "up" : "down");
	}
...
void isis_circuit_is_type_set(struct isis_circuit *circuit, int newtype) {
	...
	if(isis->debugs & DEBUG_EVENTS) {
		zlog_debug("ISIS-Evt (%s) circuit type change %s -> %s", circuit->area->area_tag, circuit_t2string(circuit->is_type), circuit_t2string(newtype));
	}
	...
	if(!(newtype & circuit->area->is_type)) {
		zlog_err(
			...
			" area is %s",
			circuit->area->area_tag, circuit_t2string(newtype), circuit_t2string(circuit->area->is_type)
		);
		return;
	}
...
void isis_event_adjacency_state_change(struct isis_adjacency *adj, int newstate) {
   ...
   * - the only proto-type was supported */
	...
	if(isis->debugs & DEBUG_EVENTS) {
		zlog_debug("ISIS-Evt (%s) Adjacency State change", adj->circuit->area->area_tag);
	}
...
void isis_event_auth_failure(char *area_tag, const char *error_string, u_char *sysid) {
	if(isis->debugs & DEBUG_EVENTS) {
		zlog_debug("ISIS-Evt (%s) Authentication failure %s from %s", area_tag, error_string, sysid_print(sysid));
	}
