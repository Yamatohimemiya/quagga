...
static int isis_multicast_join(int fd, int registerto, int if_num) {
	...
	zlog_debug(
		...
		fd, registerto, if_num, mreq.mr_address[0], mreq.mr_address[1], mreq.mr_address[2], mreq.mr_address[3], mreq.mr_address[4], mreq.mr_address[5]
	);
	#endif /* EXTREME_DEBUG */
...
static int open_packet_socket(struct isis_circuit *circuit) {
	...
	if(fd < 0) {
		zlog_warn("open_packet_socket(): socket() failed %s", safe_strerror(errno));
		return ISIS_WARNING;
	}
   ...
   */
	...
	if(if_is_broadcast(circuit->interface)) {
       ...
       */
		...
		/* joining ALL_L2_ISS */
		retval |= isis_multicast_join(circuit->fd, 2, circuit->interface->ifindex);
		/* joining ALL_ISS (used in RFC 5309 p2p-over-lan as well) */
		retval |= isis_multicast_join(circuit->fd, 3, circuit->interface->ifindex);
	} else {
...
int isis_recv_pdu_bcast(struct isis_circuit *circuit, u_char *ssnpa) {
	...
	bytesread = recvfrom(circuit->fd, (void *) &llc, LLC_LEN, MSG_PEEK, (struct sockaddr *) &s_addr, (socklen_t *) &addr_len);
	...
	if((bytesread < 0) || (s_addr.sll_ifindex != (int) circuit->interface->ifindex)) {
		if(bytesread < 0) {
			zlog_warn(
				...
				circuit->interface->name, circuit->fd, bytesread, safe_strerror(errno)
			);
		}
		if(s_addr.sll_ifindex != (int) circuit->interface->ifindex) {
			zlog_warn(
				...
				"packet type %u",
				s_addr.sll_ifindex, circuit->interface->ifindex, s_addr.sll_pkttype
			);
		}

		/* get rid of the packet */
		bytesread = recvfrom(circuit->fd, discard_buff, sizeof(discard_buff), MSG_DONTWAIT, (struct sockaddr *) &s_addr, (socklen_t *) &addr_len);
		return ISIS_WARNING;
	}
	/*
   ...
   */
	...
	bytesread = recvfrom(circuit->fd, sock_buff, sizeof(sock_buff), MSG_DONTWAIT, (struct sockaddr *) &s_addr, (socklen_t *) &addr_len);
	if(bytesread < 0) {
		zlog_warn("isis_recv_pdu_bcast(): recvfrom() failed");
		return ISIS_WARNING;
	}
...
int isis_recv_pdu_p2p(struct isis_circuit *circuit, u_char *ssnpa) {
	...
	bytesread = stream_recvfrom(circuit->rcv_stream, circuit->fd, circuit->interface->mtu, 0, (struct sockaddr *) &s_addr, (socklen_t *) &addr_len);
   ...
   */
	if(ntohs(s_addr.sll_protocol) != 0x00FE) {
		zlog_warn("isis_recv_pdu_p2p(): protocol mismatch(): %X", ntohs(s_addr.sll_protocol));
		return ISIS_WARNING;
	}
...
int isis_send_pdu_bcast(struct isis_circuit *circuit, int level) {
   ...
   */
	...
	if(sendmsg(circuit->fd, &msg, 0) < 0) {
		zlog_warn("IS-IS pfpacket: could not transmit packet on %s: %s", circuit->interface->name, safe_strerror(errno));
...
int isis_send_pdu_p2p(struct isis_circuit *circuit, int level) {
	...
	rv = sendto(circuit->fd, circuit->snd_stream->data, stream_get_endp(circuit->snd_stream), 0, (struct sockaddr *) &sa, sizeof(struct sockaddr_ll));
	if(rv < 0) {
		zlog_warn("IS-IS pfpacket: could not transmit packet on %s: %s", circuit->interface->name, safe_strerror(errno));
		if(ERRNO_IO_RETRY(errno)) {
			return ISIS_WARNING;
