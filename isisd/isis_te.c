...
static void set_circuitparams_admin_grp(struct mpls_te_circuit *mtc, u_int32_t admingrp) {
...
static void __attribute__((unused)) set_circuitparams_llri(struct mpls_te_circuit *mtc, u_int32_t local, u_int32_t remote) {
...
void set_circuitparams_local_ipaddr(struct mpls_te_circuit *mtc, struct in_addr addr) {
...
void set_circuitparams_rmt_ipaddr(struct mpls_te_circuit *mtc, struct in_addr addr) {
...
static void set_circuitparams_unrsv_bw(struct mpls_te_circuit *mtc, int priority, float fp) {
...
static void set_circuitparams_te_metric(struct mpls_te_circuit *mtc, u_int32_t te_metric) {
...
static void set_circuitparams_inter_as(struct mpls_te_circuit *mtc, struct in_addr addr, u_int32_t as) {
...
static void set_circuitparams_av_delay(struct mpls_te_circuit *mtc, u_int32_t delay, u_char anormal) {
	u_int32_t tmp;
	/* Note that TLV-length field is the size of array. */
...
static void set_circuitparams_mm_delay(struct mpls_te_circuit *mtc, u_int32_t low, u_int32_t high, u_char anormal) {
	u_int32_t tmp;
	/* Note that TLV-length field is the size of array. */
...
static void set_circuitparams_delay_var(struct mpls_te_circuit *mtc, u_int32_t jitter) {
	/* Note that TLV-length field is the size of array. */
...
static void set_circuitparams_pkt_loss(struct mpls_te_circuit *mtc, u_int32_t loss, u_char anormal) {
	u_int32_t tmp;
	/* Note that TLV-length field is the size of array. */
...
static u_char show_vty_subtlv_admin_grp(struct vty *vty, struct te_subtlv_admin_grp *tlv) {
	if(vty != NULL) {
		vty_out(vty, "    Administrative Group: 0x%x%s", (u_int32_t) ntohl(tlv->value), VTY_NEWLINE);
	} else {
		zlog_debug("      Administrative Group: 0x%x", (u_int32_t) ntohl(tlv->value));
	}
...
static u_char show_vty_subtlv_llri(struct vty *vty, struct te_subtlv_llri *tlv) {
	if(vty != NULL) {
		vty_out(vty, "    Link Local  ID: %d%s", (u_int32_t) ntohl(tlv->local), VTY_NEWLINE);
		vty_out(vty, "    Link Remote ID: %d%s", (u_int32_t) ntohl(tlv->remote), VTY_NEWLINE);
	} else {
...
static u_char show_vty_subtlv_local_ipaddr(struct vty *vty, struct te_subtlv_local_ipaddr *tlv) {
	if(vty != NULL) {
...
static u_char show_vty_subtlv_rmt_ipaddr(struct vty *vty, struct te_subtlv_rmt_ipaddr *tlv) {
	if(vty != NULL) {
...
static u_char show_vty_subtlv_max_bw(struct vty *vty, struct te_subtlv_max_bw *tlv) {
	float fval;
...
static u_char show_vty_subtlv_max_rsv_bw(struct vty *vty, struct te_subtlv_max_rsv_bw *tlv) {
	float fval;
...
static u_char show_vty_subtlv_unrsv_bw(struct vty *vty, struct te_subtlv_unrsv_bw *tlv) {
	float fval1, fval2;
	int i;
...
static u_char show_vty_subtlv_te_metric(struct vty *vty, struct te_subtlv_te_metric *tlv) {
	u_int32_t te_metric;
...
static u_char show_vty_subtlv_av_delay(struct vty *vty, struct te_subtlv_av_delay *tlv) {
	u_int32_t delay;
	u_int32_t A;
...
static u_char show_vty_subtlv_mm_delay(struct vty *vty, struct te_subtlv_mm_delay *tlv) {
	u_int32_t low, high;
	u_int32_t A;
...
static u_char show_vty_subtlv_delay_var(struct vty *vty, struct te_subtlv_delay_var *tlv) {
	u_int32_t jitter;
...
static u_char show_vty_subtlv_pkt_loss(struct vty *vty, struct te_subtlv_pkt_loss *tlv) {
	u_int32_t loss;
	u_int32_t A;
	float fval;
...
static u_char show_vty_subtlv_res_bw(struct vty *vty, struct te_subtlv_res_bw *tlv) {
	float fval;
...
static u_char show_vty_subtlv_ava_bw(struct vty *vty, struct te_subtlv_ava_bw *tlv) {
	float fval;
...
static u_char show_vty_subtlv_use_bw(struct vty *vty, struct te_subtlv_use_bw *tlv) {
	float fval;
...
static u_char show_vty_unknown_tlv(struct vty *vty, struct subtlv_header *tlvh) {
	...
	if(vty != NULL) {
		if(tlvh->length != 0) {
			vty_out(vty, "    Unknown TLV: [type(%#.2x), length(%#.2x)]%s", tlvh->type, tlvh->length, VTY_NEWLINE);
			vty_out(vty, "       Dump: [00]");
		...
		zlog_debug("      Unknown TLV: [type(%#.2x), length(%#.2x)]", tlvh->type, tlvh->length);
	}
...
void mpls_te_print_detail(struct vty *vty, struct te_is_neigh *te) {
	...
	for(; sum < te->sub_tlvs_length; tlvh = (next ? next : SUBTLV_HDR_NEXT(tlvh))) {
		...
		switch(tlvh->type) {
			case TE_SUBTLV_ADMIN_GRP: sum += show_vty_subtlv_admin_grp(vty, (struct te_subtlv_admin_grp *) tlvh); break;
			case TE_SUBTLV_LLRI: sum += show_vty_subtlv_llri(vty, (struct te_subtlv_llri *) tlvh); break;
			case TE_SUBTLV_LOCAL_IPADDR: sum += show_vty_subtlv_local_ipaddr(vty, (struct te_subtlv_local_ipaddr *) tlvh); break;
			case TE_SUBTLV_RMT_IPADDR: sum += show_vty_subtlv_rmt_ipaddr(vty, (struct te_subtlv_rmt_ipaddr *) tlvh); break;
			case TE_SUBTLV_MAX_BW: sum += show_vty_subtlv_max_bw(vty, (struct te_subtlv_max_bw *) tlvh); break;
			case TE_SUBTLV_MAX_RSV_BW: sum += show_vty_subtlv_max_rsv_bw(vty, (struct te_subtlv_max_rsv_bw *) tlvh); break;
			case TE_SUBTLV_UNRSV_BW: sum += show_vty_subtlv_unrsv_bw(vty, (struct te_subtlv_unrsv_bw *) tlvh); break;
			case TE_SUBTLV_TE_METRIC: sum += show_vty_subtlv_te_metric(vty, (struct te_subtlv_te_metric *) tlvh); break;
			case TE_SUBTLV_RAS: sum += show_vty_subtlv_ras(vty, (struct te_subtlv_ras *) tlvh); break;
			case TE_SUBTLV_RIP: sum += show_vty_subtlv_rip(vty, (struct te_subtlv_rip *) tlvh); break;
			case TE_SUBTLV_AV_DELAY: sum += show_vty_subtlv_av_delay(vty, (struct te_subtlv_av_delay *) tlvh); break;
			case TE_SUBTLV_MM_DELAY: sum += show_vty_subtlv_mm_delay(vty, (struct te_subtlv_mm_delay *) tlvh); break;
			case TE_SUBTLV_DELAY_VAR: sum += show_vty_subtlv_delay_var(vty, (struct te_subtlv_delay_var *) tlvh); break;
			case TE_SUBTLV_PKT_LOSS: sum += show_vty_subtlv_pkt_loss(vty, (struct te_subtlv_pkt_loss *) tlvh); break;
			case TE_SUBTLV_RES_BW: sum += show_vty_subtlv_res_bw(vty, (struct te_subtlv_res_bw *) tlvh); break;
			case TE_SUBTLV_AVA_BW: sum += show_vty_subtlv_ava_bw(vty, (struct te_subtlv_ava_bw *) tlvh); break;
			case TE_SUBTLV_USE_BW: sum += show_vty_subtlv_use_bw(vty, (struct te_subtlv_use_bw *) tlvh); break;
...
void isis_mpls_te_config_write_router(struct vty *vty) {
	...
	if(IS_MPLS_TE(isisMplsTE)) {
		...
		vty_out(vty, "  mpls-te router-address %s%s", inet_ntoa(isisMplsTE.router_id), VTY_NEWLINE);
	}
...
DEFUN(isis_mpls_te_on, isis_mpls_te_on_cmd, "mpls-te on", MPLS_TE_STR "Enable MPLS-TE functionality\n") {
   ...
   */
	for(ALL_LIST_ELEMENTS_RO(isisMplsTE.cir_list, node, circuit)) {
		...
		if((circuit->mtc->status == disable) && HAS_LINK_PARAMS(circuit->interface)) {
...
DEFUN(no_isis_mpls_te_on, no_isis_mpls_te_on_cmd, "no mpls-te", NO_STR "Disable the MPLS-TE functionality\n") {
	struct listnode *node;
	struct isis_circuit *circuit;
...
DEFUN(isis_mpls_te_router_addr, isis_mpls_te_router_addr_cmd, "mpls-te router-address A.B.C.D",
...
DEFUN(isis_mpls_te_inter_as, isis_mpls_te_inter_as_cmd, "mpls-te inter-as (level-1|level-1-2|level-2-only)",
...
DEFUN(no_isis_mpls_te_inter_as, no_isis_mpls_te_inter_as_cmd, "no mpls-te inter-as",
...
DEFUN(show_isis_mpls_te_router, show_isis_mpls_te_router_cmd, "show isis mpls-te router", SHOW_STR ISIS_STR MPLS_TE_STR "Router information\n") {
	if(IS_MPLS_TE(isisMplsTE)) {
...
static void show_mpls_te_sub(struct vty *vty, struct interface *ifp) {
	...
	if((IS_MPLS_TE(isisMplsTE)) && ((mtc = lookup_mpls_params_by_ifp(ifp)) != NULL)) {
...
DEFUN(show_isis_mpls_te_interface, show_isis_mpls_te_interface_cmd, "show isis mpls-te interface [INTERFACE]",
      SHOW_STR ISIS_STR MPLS_TE_STR "Interface information\n"
				    "Interface name\n") {
