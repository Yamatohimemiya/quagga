...
#define DEFAULT_LSP_MTU 1497
   ...
   */
	...
	struct isis_redist redist_settings[REDIST_PROTOCOL_COUNT][ZEBRA_ROUTE_MAX + 1][ISIS_LEVELS];
...
#define lsp_debug(...) \
	do { \
		if(isis->debugs & DEBUG_LSP_GEN) zlog_debug(__VA_ARGS__); \
	} while(0)

#define sched_debug(...) \
	do { \
		if(isis->debugs & DEBUG_LSP_SCHED) zlog_debug(__VA_ARGS__); \
	} while(0)
