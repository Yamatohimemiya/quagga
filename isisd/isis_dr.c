...
const char *isis_disflag2string(int disflag) {
	switch(disflag) {
		case ISIS_IS_NOT_DIS: return "is not DIS";
		case ISIS_IS_DIS: return "is DIS";
		case ISIS_WAS_DIS: return "was DIS";
		default: return "unknown DIS state";
	}
...
static int isis_check_dr_change(struct isis_adjacency *adj, int level) {
	...
	if(adj->dis_record[level - 1].dis != adj->dis_record[(1 * ISIS_LEVELS) + level - 1].dis)
	/* was there a DIS state transition ? */
	{
		adj->dischanges[level - 1]++;
		/* ok rotate the history list through */
		for(i = DIS_RECORDS - 1; i > 0; i--) {
			adj->dis_record[(i * ISIS_LEVELS) + level - 1].dis = adj->dis_record[((i - 1) * ISIS_LEVELS) + level - 1].dis;
			adj->dis_record[(i * ISIS_LEVELS) + level - 1].last_dis_change = adj->dis_record[((i - 1) * ISIS_LEVELS) + level - 1].last_dis_change;
		}
	}
	return ISIS_OK;
}
...
int isis_dr_elect(struct isis_circuit *circuit, int level) {
   ...
   */
	if(adj_dr->prio[level - 1] < own_prio || (adj_dr->prio[level - 1] == own_prio && memcmp(adj_dr->snpa, circuit->u.bc.snpa, ETH_ALEN) < 0)) {
...
int isis_dr_resign(struct isis_circuit *circuit, int level) {
	...
	if(level == 1) {
		...
		THREAD_TIMER_ON(master, circuit->t_send_psnp[0], send_l1_psnp, circuit, isis_jitter(circuit->psnp_interval[level - 1], PSNP_JITTER));
	} else {
		...
		THREAD_TIMER_ON(master, circuit->t_send_psnp[1], send_l2_psnp, circuit, isis_jitter(circuit->psnp_interval[level - 1], PSNP_JITTER));
	}
...
int isis_dr_commence(struct isis_circuit *circuit, int level) {
	...
	if(level == 1) {
		...
		*(circuit->u.bc.l1_desig_is + ISIS_SYS_ID_LEN) = circuit->circuit_id;
		...
		THREAD_TIMER_ON(master, circuit->t_send_csnp[0], send_l1_csnp, circuit, isis_jitter(circuit->csnp_interval[level - 1], CSNP_JITTER));
		...
		*(circuit->u.bc.l2_desig_is + ISIS_SYS_ID_LEN) = circuit->circuit_id;
		...
		THREAD_TIMER_ON(master, circuit->t_send_csnp[1], send_l2_csnp, circuit, isis_jitter(circuit->csnp_interval[level - 1], CSNP_JITTER));
	}
