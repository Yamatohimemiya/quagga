...
static struct isis_redist *get_redist_settings(struct isis_area *area, int family, int type, int level) {
	int protocol = redist_protocol(family);
...
static struct route_node *isis_redist_route_node_create(route_table_delegate_t *delegate, struct route_table *table) {
	struct route_node *node;
	node = XCALLOC(MTYPE_ROUTE_NODE, sizeof(*node));
	return node;
}

static void isis_redist_route_node_destroy(route_table_delegate_t *delegate, struct route_table *table, struct route_node *node) {
	if(node->info) {
...
static route_table_delegate_t isis_redist_rt_delegate = { .create_node = isis_redist_route_node_create, .destroy_node = isis_redist_route_node_destroy };
...
static void isis_redist_install(struct isis_area *area, int level, struct prefix *p, struct isis_ext_info *info) {
	int family = p->family;
 ...
 * specific area for a specific level.
 * Schedule an lsp regenerate if necessary. */
static void isis_redist_uninstall(struct isis_area *area, int level, struct prefix *p) {
	int family = p->family;
	struct route_table *er_table = get_ext_reach(area, family, level);
	struct route_node *er_node;
...
static void isis_redist_update_ext_reach(struct isis_area *area, int level, struct isis_redist *redist, struct prefix *p, struct isis_ext_info *info) {
	...
	if(area_info.origin == DEFAULT_ROUTE && redist->redist != DEFAULT_ORIGINATE_ALWAYS) {
		area_info.distance = 255;
	}
...
void isis_redist_add(int type, struct prefix *p, u_char distance, uint32_t metric) {
	...
	zlog_debug("%s: New route %s from %s.", __func__, debug_buf, zebra_route_string(type));

	if(!ei_table) {
		zlog_warn("%s: External information table not initialized.", __func__);
		return;
	}
...
void isis_redist_delete(int type, struct prefix *p) {
	...
	zlog_debug("%s: Removing route %s from %s.", __func__, debug_buf, zebra_route_string(type));
...
static void isis_redist_routemap_set(struct isis_redist *redist, const char *routemap) {
	if(redist->map_name) {
...
static void isis_redist_set(struct isis_area *area, int level, int family, int type, uint32_t metric, const char *routemap, int originate_type) {
	int protocol = redist_protocol(family);
	...
	if(!area->ext_reach[protocol][level - 1]) {
		area->ext_reach[protocol][level - 1] = route_table_init_with_delegate(&isis_redist_rt_delegate);
	}

	for(i = 0; i < REDIST_PROTOCOL_COUNT; i++) {
		if(!area->isis->ext_info[i]) {
			area->isis->ext_info[i] = route_table_init_with_delegate(&isis_redist_rt_delegate);
		}
	}
...
static void isis_redist_unset(struct isis_area *area, int level, int family, int type) {
	struct isis_redist *redist = get_redist_settings(area, family, type, level);
	struct route_table *er_table = get_ext_reach(area, family, level);
	struct route_node *rn;
	struct isis_ext_info *info;
...
DEFUN(isis_redistribute, isis_redistribute_cmd, "redistribute (ipv4|ipv6) " QUAGGA_REDIST_STR_ISISD " (level-1|level-2) {metric <0-16777215>|route-map WORD}",
      REDIST_STR "Redistribute IPv4 routes\n"
...
DEFUN(isis_default_originate, isis_default_originate_cmd,
...
int isis_redist_config_write(struct vty *vty, struct isis_area *area, int family) {
	...
	for(type = 0; type < ZEBRA_ROUTE_MAX; type++) {
		...
		for(level = 1; level <= ISIS_LEVELS; level++) {
			...
			vty_out(vty, " redistribute %s %s level-%d", family_str, zebra_route_string(type), level);
			if(redist->metric != 0xffffffff) {
	...
	for(level = 1; level <= ISIS_LEVELS; level++) {
		...
		vty_out(vty, " default-information originate %s level-%d", family_str, level);
