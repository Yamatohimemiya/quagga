...
struct isis_area *isis_area_lookup(const char *area_tag) {
	...
	for(ALL_LIST_ELEMENTS_RO(isis->area_list, node, area)) {
		if((area->area_tag == NULL && area_tag == NULL) || (area->area_tag && area_tag && strcmp(area->area_tag, area_tag) == 0)) {
			return area;
		}
	}
...
int area_net_title(struct vty *vty, const char *net_title) {
	...
	if(listcount(area->area_addrs) >= isis->max_area_addrs) {
		vty_out(vty, "Maximum of area addresses (%d) already reached %s", isis->max_area_addrs, VTY_NEWLINE);
...
#ifdef EXTREME_DEBUG
	zlog_debug("added area address %s for area %s (address length %d)", net_title, area->area_tag, addr->addr_len);
#endif /* EXTREME_DEBUG */
	if(addr->addr_len < 8 || addr->addr_len > 20) {
		vty_out(vty, "area address must be at least 8..20 octets long (%d)%s", addr->addr_len, VTY_NEWLINE);
		XFREE(MTYPE_ISIS_AREA_ADDR, addr);
		return CMD_ERR_AMBIGUOUS;
	}

	if(addr->area_addr[addr->addr_len - 1] != 0) {
		vty_out(vty, "nsel byte (last byte) in area address must be 0%s", VTY_NEWLINE);
		XFREE(MTYPE_ISIS_AREA_ADDR, addr);
		return CMD_ERR_AMBIGUOUS;
	}
...
int area_clear_net_title(struct vty *vty, const char *net_title) {
	...
	addr.addr_len = dotformat2buff(buff, net_title);
	if(addr.addr_len < 8 || addr.addr_len > 20) {
		vty_out(vty, "Unsupported area address length %d, should be 8...20 %s", addr.addr_len, VTY_NEWLINE);
		return CMD_ERR_AMBIGUOUS;
	}
	...
	for(ALL_LIST_ELEMENTS_RO(area->area_addrs, node, addrp)) {
		if((addrp->addr_len + ISIS_SYS_ID_LEN + 1) == addr.addr_len && !memcmp(addrp->area_addr, addr.area_addr, addr.addr_len)) {
			break;
		}
	}

	if(!addrp) {
		vty_out(vty, "No area address %s for area %s %s", net_title, area->area_tag, VTY_NEWLINE);
		return CMD_ERR_NO_MATCH;
	}
...
int show_isis_interface_common(struct vty *vty, const char *ifname, char detail) {
	...
	for(ALL_LIST_ELEMENTS_RO(isis->area_list, anode, area)) {
		...
		if(detail == ISIS_UI_LEVEL_BRIEF) {
			vty_out(vty, "  Interface   CircId   State    Type     Level%s", VTY_NEWLINE);
		}
...
DEFUN(show_isis_interface, show_isis_interface_cmd, "show isis interface",
      SHOW_STR "ISIS network information\n"
	       "ISIS interface\n") {
	return show_isis_interface_common(vty, NULL, ISIS_UI_LEVEL_BRIEF);
}

DEFUN(show_isis_interface_detail, show_isis_interface_detail_cmd, "show isis interface detail",
      SHOW_STR "ISIS network information\n"
	       "ISIS interface\n"
...
DEFUN(show_isis_interface_arg, show_isis_interface_arg_cmd, "show isis interface WORD",
      SHOW_STR "ISIS network information\n"
	       "ISIS interface\n"
	       "ISIS interface name\n") {
...
DEFUN(show_isis_neighbor, show_isis_neighbor_cmd, "show isis neighbor",
      SHOW_STR "ISIS network information\n"
	       "ISIS neighbor adjacencies\n") {
	return show_isis_neighbor_common(vty, NULL, ISIS_UI_LEVEL_BRIEF);
}

DEFUN(show_isis_neighbor_detail, show_isis_neighbor_detail_cmd, "show isis neighbor detail",
      SHOW_STR "ISIS network information\n"
	       "ISIS neighbor adjacencies\n"
	       "show detailed information\n") {
...
DEFUN(show_isis_neighbor_arg, show_isis_neighbor_arg_cmd, "show isis neighbor WORD",
      SHOW_STR "ISIS network information\n"
	       "ISIS neighbor adjacencies\n"
	       "System id\n") {
...
DEFUN(clear_isis_neighbor, clear_isis_neighbor_cmd, "clear isis neighbor",
      CLEAR_STR "Reset ISIS network information\n"
		"Reset ISIS neighbor adjacencies\n") {
	return clear_isis_neighbor_common(vty, NULL);
}

DEFUN(clear_isis_neighbor_arg, clear_isis_neighbor_arg_cmd, "clear isis neighbor WORD",
      CLEAR_STR "ISIS network information\n"
		"ISIS neighbor adjacencies\n"
		"System id\n") {
...
void print_debug(struct vty *vty, int flags, int onoff) {
	...
	if(flags & DEBUG_ADJ_PACKETS) {
		vty_out(vty, "IS-IS Adjacency related packets debugging is %s%s", onoffs, VTY_NEWLINE);
	}
	if(flags & DEBUG_CHECKSUM_ERRORS) {
		vty_out(vty, "IS-IS checksum errors debugging is %s%s", onoffs, VTY_NEWLINE);
	}
	if(flags & DEBUG_LOCAL_UPDATES) {
		vty_out(vty, "IS-IS local updates debugging is %s%s", onoffs, VTY_NEWLINE);
	}
	if(flags & DEBUG_PROTOCOL_ERRORS) {
		vty_out(vty, "IS-IS protocol errors debugging is %s%s", onoffs, VTY_NEWLINE);
	}
	if(flags & DEBUG_SNP_PACKETS) {
		vty_out(vty, "IS-IS CSNP/PSNP packets debugging is %s%s", onoffs, VTY_NEWLINE);
	}
	if(flags & DEBUG_SPF_EVENTS) {
		vty_out(vty, "IS-IS SPF events debugging is %s%s", onoffs, VTY_NEWLINE);
	}
	if(flags & DEBUG_SPF_STATS) {
		vty_out(vty, "IS-IS SPF Timing and Statistics Data debugging is %s%s", onoffs, VTY_NEWLINE);
	}
	if(flags & DEBUG_SPF_TRIGGERS) {
		vty_out(vty, "IS-IS SPF triggering events debugging is %s%s", onoffs, VTY_NEWLINE);
	}
	if(flags & DEBUG_UPDATE_PACKETS) {
		vty_out(vty, "IS-IS Update related packet debugging is %s%s", onoffs, VTY_NEWLINE);
	}
	if(flags & DEBUG_RTE_EVENTS) {
		vty_out(vty, "IS-IS Route related debuggin is %s%s", onoffs, VTY_NEWLINE);
	}
...
DEFUN(show_debugging, show_debugging_isis_cmd, "show debugging isis", SHOW_STR "State of each debugging option\n") {
	if(isis->debugs) {
...
DEFUN(debug_isis_adj, debug_isis_adj_cmd, "debug isis adj-packets",
      DEBUG_STR "IS-IS information\n"
...
DEFUN(no_debug_isis_adj, no_debug_isis_adj_cmd, "no debug isis adj-packets",
      UNDEBUG_STR "IS-IS information\n"
		  "IS-IS Adjacency related packets\n") {
	isis->debugs &= ~DEBUG_ADJ_PACKETS;
...
DEFUN(debug_isis_csum, debug_isis_csum_cmd, "debug isis checksum-errors",
      DEBUG_STR "IS-IS information\n"
...
DEFUN(no_debug_isis_csum, no_debug_isis_csum_cmd, "no debug isis checksum-errors",
      UNDEBUG_STR "IS-IS information\n"
		  "IS-IS LSP checksum errors\n") {
	isis->debugs &= ~DEBUG_CHECKSUM_ERRORS;
...
DEFUN(debug_isis_lupd, debug_isis_lupd_cmd, "debug isis local-updates",
      DEBUG_STR "IS-IS information\n"
...
DEFUN(no_debug_isis_lupd, no_debug_isis_lupd_cmd, "no debug isis local-updates",
      UNDEBUG_STR "IS-IS information\n"
		  "IS-IS local update packets\n") {
	isis->debugs &= ~DEBUG_LOCAL_UPDATES;
...
DEFUN(debug_isis_err, debug_isis_err_cmd, "debug isis protocol-errors",
      DEBUG_STR "IS-IS information\n"
...
DEFUN(no_debug_isis_err, no_debug_isis_err_cmd, "no debug isis protocol-errors",
      UNDEBUG_STR "IS-IS information\n"
		  "IS-IS LSP protocol errors\n") {
	isis->debugs &= ~DEBUG_PROTOCOL_ERRORS;
...
DEFUN(debug_isis_snp, debug_isis_snp_cmd, "debug isis snp-packets",
      DEBUG_STR "IS-IS information\n"
...
DEFUN(no_debug_isis_snp, no_debug_isis_snp_cmd, "no debug isis snp-packets",
      UNDEBUG_STR "IS-IS information\n"
		  "IS-IS CSNP/PSNP packets\n") {
	isis->debugs &= ~DEBUG_SNP_PACKETS;
...
DEFUN(debug_isis_upd, debug_isis_upd_cmd, "debug isis update-packets",
      DEBUG_STR "IS-IS information\n"
...
DEFUN(no_debug_isis_upd, no_debug_isis_upd_cmd, "no debug isis update-packets",
      UNDEBUG_STR "IS-IS information\n"
		  "IS-IS Update related packets\n") {
	isis->debugs &= ~DEBUG_UPDATE_PACKETS;
...
DEFUN(debug_isis_spfevents, debug_isis_spfevents_cmd, "debug isis spf-events",
      DEBUG_STR "IS-IS information\n"
...
DEFUN(no_debug_isis_spfevents, no_debug_isis_spfevents_cmd, "no debug isis spf-events",
      UNDEBUG_STR "IS-IS information\n"
		  "IS-IS Shortest Path First Events\n") {
	isis->debugs &= ~DEBUG_SPF_EVENTS;
...
DEFUN(debug_isis_spfstats, debug_isis_spfstats_cmd, "debug isis spf-statistics ",
      DEBUG_STR "IS-IS information\n"
...
DEFUN(no_debug_isis_spfstats, no_debug_isis_spfstats_cmd, "no debug isis spf-statistics",
      UNDEBUG_STR "IS-IS information\n"
		  "IS-IS SPF Timing and Statistic Data\n") {
	isis->debugs &= ~DEBUG_SPF_STATS;
...
DEFUN(debug_isis_spftrigg, debug_isis_spftrigg_cmd, "debug isis spf-triggers",
      DEBUG_STR "IS-IS information\n"
...
DEFUN(no_debug_isis_spftrigg, no_debug_isis_spftrigg_cmd, "no debug isis spf-triggers",
      UNDEBUG_STR "IS-IS information\n"
		  "IS-IS SPF triggering events\n") {
	isis->debugs &= ~DEBUG_SPF_TRIGGERS;
...
DEFUN(debug_isis_rtevents, debug_isis_rtevents_cmd, "debug isis route-events",
      DEBUG_STR "IS-IS information\n"
...
DEFUN(no_debug_isis_rtevents, no_debug_isis_rtevents_cmd, "no debug isis route-events",
      UNDEBUG_STR "IS-IS information\n"
		  "IS-IS Route related events\n") {
	isis->debugs &= ~DEBUG_RTE_EVENTS;
...
DEFUN(debug_isis_events, debug_isis_events_cmd, "debug isis events",
      DEBUG_STR "IS-IS information\n"
		"IS-IS Events\n") {
...
DEFUN(no_debug_isis_events, no_debug_isis_events_cmd, "no debug isis events",
      UNDEBUG_STR "IS-IS information\n"
		  "IS-IS Events\n") {
	isis->debugs &= ~DEBUG_EVENTS;
...
DEFUN(debug_isis_packet_dump, debug_isis_packet_dump_cmd, "debug isis packet-dump",
      DEBUG_STR "IS-IS information\n"
		"IS-IS packet dump\n") {
...
DEFUN(no_debug_isis_packet_dump, no_debug_isis_packet_dump_cmd, "no debug isis packet-dump",
      UNDEBUG_STR "IS-IS information\n"
		  "IS-IS packet dump\n") {
	isis->debugs &= ~DEBUG_PACKET_DUMP;
...
DEFUN(debug_isis_lsp_gen, debug_isis_lsp_gen_cmd, "debug isis lsp-gen",
      DEBUG_STR "IS-IS information\n"
...
DEFUN(no_debug_isis_lsp_gen, no_debug_isis_lsp_gen_cmd, "no debug isis lsp-gen",
      UNDEBUG_STR "IS-IS information\n"
		  "IS-IS generation of own LSPs\n") {
	isis->debugs &= ~DEBUG_LSP_GEN;
...
DEFUN(debug_isis_lsp_sched, debug_isis_lsp_sched_cmd, "debug isis lsp-sched",
      DEBUG_STR "IS-IS information\n"
...
DEFUN(no_debug_isis_lsp_sched, no_debug_isis_lsp_sched_cmd, "no debug isis lsp-gen",
      UNDEBUG_STR "IS-IS information\n"
		  "IS-IS scheduling of LSP generation\n") {
	isis->debugs &= ~DEBUG_LSP_SCHED;
...
DEFUN(show_hostname, show_hostname_cmd, "show isis hostname",
...
DEFUN(show_isis_summary, show_isis_summary_cmd, "show isis summary",
      SHOW_STR "IS-IS information\n"
	       "IS-IS summary\n") {
	...
	vty_out(vty, "Process Id      : %ld%s", isis->process_id, VTY_NEWLINE);
	if(isis->sysid_set) {
		vty_out(vty, "System Id       : %s%s", sysid_print(isis->sysid), VTY_NEWLINE);
	}
	...
	if(isis->area_list) {
		vty_out(vty, "Number of areas : %d%s", isis->area_list->count, VTY_NEWLINE);
	}

	for(ALL_LIST_ELEMENTS_RO(isis->area_list, node, area)) {
		vty_out(vty, "Area %s:%s", area->area_tag ? area->area_tag : "null", VTY_NEWLINE);

		if(listcount(area->area_addrs) > 0) {
			struct area_addr *area_addr;
			for(ALL_LIST_ELEMENTS_RO(area->area_addrs, node2, area_addr)) {
				vty_out(vty, "  Net: %s%s", isonet_print(area_addr->area_addr, area_addr->addr_len + ISIS_SYS_ID_LEN + 1), VTY_NEWLINE);
			}
		}
		...
		for(level = ISIS_LEVEL1; level <= ISIS_LEVELS; level++) {
			...
			vty_out(vty, "      minimum interval  : %d%s", area->min_spf_interval[level - 1], VTY_NEWLINE);

			vty_out(vty, "      last run elapsed  : ");
			vty_out_timestr(vty, spftree->last_run_timestamp);
			vty_out(vty, "%s", VTY_NEWLINE);

			vty_out(vty, "      last run duration : %u usec%s", (u_int32_t) spftree->last_run_duration, VTY_NEWLINE);

			vty_out(vty, "      run count         : %d%s", spftree->runcount, VTY_NEWLINE);
			...
			vty_out(vty, "      last run duration : %llu msec%s", (unsigned long long) spftree->last_run_duration, VTY_NEWLINE);
		...
		for(level = 0; level < ISIS_LEVELS; level++) {
			if(area->lspdb[level] && dict_count(area->lspdb[level]) > 0) {
				...
				if(lsp) {
					...
					lsp_count = lsp_print_all(vty, area->lspdb[level], ui_level, area->dynhostname);

					vty_out(vty, "    %u LSPs%s%s", lsp_count, VTY_NEWLINE, VTY_NEWLINE);
				}
			}
		}
	}
...
DEFUN(show_database_brief, show_database_cmd, "show isis database",
      SHOW_STR "IS-IS information\n"
	       "IS-IS link state database\n") {
	return show_isis_database(vty, NULL, ISIS_UI_LEVEL_BRIEF);
}

DEFUN(show_database_lsp_brief, show_database_arg_cmd, "show isis database WORD",
      SHOW_STR "IS-IS information\n"
	       "IS-IS link state database\n"
	       "LSP ID\n") {
...
DEFUN(show_database_lsp_detail, show_database_arg_detail_cmd, "show isis database WORD detail",
      SHOW_STR "IS-IS information\n"
	       "IS-IS link state database\n"
	       "LSP ID\n"
...
DEFUN(show_database_detail, show_database_detail_cmd, "show isis database detail",
      SHOW_STR "IS-IS information\n"
	       "IS-IS link state database\n") {
	return show_isis_database(vty, NULL, ISIS_UI_LEVEL_DETAIL);
}

DEFUN(show_database_detail_lsp, show_database_detail_arg_cmd, "show isis database detail WORD",
      SHOW_STR "IS-IS information\n"
	       "IS-IS link state database\n"
	       "Detailed information\n"
	       "LSP ID\n") {
 ...
 * 'router isis' command 
 */
DEFUN(router_isis, router_isis_cmd, "router isis WORD",
 ...
 */
DEFUN(no_router_isis, no_router_isis_cmd, "no router isis WORD",
      "no\n" ROUTER_STR "ISO IS-IS\n"
      "ISO Routing area tag") {
 ...
 * 'no net' command
 */
DEFUN(no_net, no_net_cmd, "no net WORD",
      NO_STR "A Network Entity Title for this process (OSI only)\n"
	     "XX.XXXX. ... .XXX.XX  Network entity title (NET)\n") {
...
static int isis_area_passwd_set(struct isis_area *area, int level, u_char passwd_type, const char *passwd, u_char snp_auth) {
...
int isis_area_passwd_cleartext_set(struct isis_area *area, int level, const char *passwd, u_char snp_auth) {
	return isis_area_passwd_set(area, level, ISIS_PASSWD_TYPE_CLEARTXT, passwd, snp_auth);
}

int isis_area_passwd_hmac_md5_set(struct isis_area *area, int level, const char *passwd, u_char snp_auth) {
	return isis_area_passwd_set(area, level, ISIS_PASSWD_TYPE_HMAC_MD5, passwd, snp_auth);
}
...
static void area_resign_level(struct isis_area *area, int level) {
	...
	sched_debug("ISIS (%s): Resigned from L%d - canceling LSP regeneration timer.", area->area_tag, level);
...
void isis_area_is_type_set(struct isis_area *area, int is_type) {
	...
	if(isis->debugs & DEBUG_EVENTS) {
		zlog_debug("ISIS-Evt (%s) system type change %s -> %s", area->area_tag, circuit_t2string(area->is_type), circuit_t2string(is_type));
	}
...
void isis_area_max_lsp_lifetime_set(struct isis_area *area, int level, uint16_t max_lsp_lifetime) {
	assert((level == IS_LEVEL_1) || (level == IS_LEVEL_2));
...
void isis_area_lsp_refresh_set(struct isis_area *area, int level, uint16_t lsp_refresh) {
	assert((level == IS_LEVEL_1) || (level == IS_LEVEL_2));
...
DEFUN(topology_generate_grid, topology_generate_grid_cmd,
...
DEFUN(show_isis_generated_topology, show_isis_generated_topology_cmd, "show isis generated-topologies",
      SHOW_STR "ISIS network information\n"
	...
	for(ALL_LIST_ELEMENTS_RO(isis->area_list, node, area)) {
		...
		for(ALL_LIST_ELEMENTS_RO(area->topology, node2, arc)) {
			vty_out(vty, "%9ld %11ld %12ld%s", arc->from_node, arc->to_node, arc->distance, VTY_NEWLINE);
		}
	}
	return CMD_SUCCESS;
}
...
DEFUN(topology_baseis, topology_baseis_cmd, "topology base-is WORD",
      "Topology generation for IS-IS\n"
...
DEFUN(no_topology_baseis, no_topology_baseis_cmd, "no topology base-is WORD",
      NO_STR "Topology generation for IS-IS\n"
	     "A Network IS Base for this topology\n"
	     "XXXX.XXXX.XXXX Network entity title (NET)\n") {
	struct isis_area *area;
...
ALIAS(no_topology_baseis, no_topology_baseis_noid_cmd, "no topology base-is",
      NO_STR "Topology generation for IS-IS\n"
	     "A Network IS Base for this topology\n")

DEFUN(topology_basedynh, topology_basedynh_cmd, "topology base-dynh WORD",
      "Topology generation for IS-IS\n"
	 ...
	 * false. */
			...
			if(area->lsp_gen_interval[0] == area->lsp_gen_interval[1]) {
				if(area->lsp_gen_interval[0] != DEFAULT_MIN_LSP_GEN_INTERVAL) {
					vty_out(vty, " lsp-gen-interval %d%s", area->lsp_gen_interval[0], VTY_NEWLINE);
					write++;
				}
			} else {
				if(area->lsp_gen_interval[0] != DEFAULT_MIN_LSP_GEN_INTERVAL) {
					vty_out(vty, " lsp-gen-interval level-1 %d%s", area->lsp_gen_interval[0], VTY_NEWLINE);
					write++;
				}
				if(area->lsp_gen_interval[1] != DEFAULT_MIN_LSP_GEN_INTERVAL) {
					vty_out(vty, " lsp-gen-interval level-2 %d%s", area->lsp_gen_interval[1], VTY_NEWLINE);
					write++;
				}
			}
			/* ISIS - LSP lifetime */
			if(area->max_lsp_lifetime[0] == area->max_lsp_lifetime[1]) {
				if(area->max_lsp_lifetime[0] != DEFAULT_LSP_LIFETIME) {
					vty_out(vty, " max-lsp-lifetime %u%s", area->max_lsp_lifetime[0], VTY_NEWLINE);
					write++;
				}
			} else {
				if(area->max_lsp_lifetime[0] != DEFAULT_LSP_LIFETIME) {
					vty_out(vty, " max-lsp-lifetime level-1 %u%s", area->max_lsp_lifetime[0], VTY_NEWLINE);
					write++;
				}
				if(area->max_lsp_lifetime[1] != DEFAULT_LSP_LIFETIME) {
					vty_out(vty, " max-lsp-lifetime level-2 %u%s", area->max_lsp_lifetime[1], VTY_NEWLINE);
					write++;
				}
			}
			...
			if(area->lsp_refresh[0] == area->lsp_refresh[1]) {
				if(area->lsp_refresh[0] != DEFAULT_MAX_LSP_GEN_INTERVAL) {
					vty_out(vty, " lsp-refresh-interval %u%s", area->lsp_refresh[0], VTY_NEWLINE);
					write++;
				}
			} else {
				if(area->lsp_refresh[0] != DEFAULT_MAX_LSP_GEN_INTERVAL) {
					vty_out(vty, " lsp-refresh-interval level-1 %u%s", area->lsp_refresh[0], VTY_NEWLINE);
					write++;
				}
				if(area->lsp_refresh[1] != DEFAULT_MAX_LSP_GEN_INTERVAL) {
					vty_out(vty, " lsp-refresh-interval level-2 %u%s", area->lsp_refresh[1], VTY_NEWLINE);
					write++;
				}
			}
			...
			if(area->min_spf_interval[0] == area->min_spf_interval[1]) {
				if(area->min_spf_interval[0] != MINIMUM_SPF_INTERVAL) {
					vty_out(vty, " spf-interval %d%s", area->min_spf_interval[0], VTY_NEWLINE);
					write++;
				}
			} else {
				if(area->min_spf_interval[0] != MINIMUM_SPF_INTERVAL) {
					vty_out(vty, " spf-interval level-1 %d%s", area->min_spf_interval[0], VTY_NEWLINE);
					write++;
				}
				if(area->min_spf_interval[1] != MINIMUM_SPF_INTERVAL) {
					vty_out(vty, " spf-interval level-2 %d%s", area->min_spf_interval[1], VTY_NEWLINE);
					write++;
				}
			}
			...
			if(area->domain_passwd.type == ISIS_PASSWD_TYPE_HMAC_MD5) {
				vty_out(vty, " domain-password md5 %s", area->domain_passwd.passwd);
				...
				vty_out(vty, " domain-password clear %s", area->domain_passwd.passwd);
				if(CHECK_FLAG(area->domain_passwd.snp_auth, SNP_AUTH_SEND)) {
					vty_out(vty, " authenticate snp ");
					if(CHECK_FLAG(area->domain_passwd.snp_auth, SNP_AUTH_RECV)) {
						vty_out(vty, "validate");
...
#ifdef TOPOLOGY_GENERATE
			if(memcmp(area->topology_baseis, DEFAULT_TOPOLOGY_BASEIS, ISIS_SYS_ID_LEN)) {
				vty_out(vty, " topology base-is %s%s", sysid_print((u_char *) area->topology_baseis), VTY_NEWLINE);
				write++;
			}
			if(area->topology_basedynh) {
				vty_out(vty, " topology base-dynh %s%s", area->topology_basedynh, VTY_NEWLINE);
				write++;
			}
