...
struct isis_circuit *circuit_lookup_by_ifp(struct interface *ifp, struct list *list) {
...
void isis_circuit_add_addr(struct isis_circuit *circuit, struct connected *connected) {
	struct listnode *node;
	...
	if(connected->address->family == AF_INET) {
		...
		addr = ntohl(addr);
		if(IPV4_NET0(addr) || IPV4_NET127(addr) || IN_CLASSD(addr) || IPV4_LINKLOCAL(addr)) {
			return;
		}
		...
		zlog_debug("Added IP address %s to circuit %d", buf, circuit->circuit_id);
#endif /* EXTREME_DEBUG */
	}
#ifdef HAVE_IPV6
	...
	#ifdef EXTREME_DEBUG
		prefix2str(connected->address, buf, BUFSIZ);
		zlog_debug("Added IPv6 address %s to circuit %d", buf, circuit->circuit_id);
	#endif /* EXTREME_DEBUG */
	}
#endif /* HAVE_IPV6 */
	return;
...
void isis_circuit_del_addr(struct isis_circuit *circuit, struct connected *connected) {
...
static void isis_circuit_update_all_srmflags(struct isis_circuit *circuit, int is_set) {
	struct isis_area *area;
	struct isis_lsp *lsp;
	...
	for(level = ISIS_LEVEL1; level <= ISIS_LEVEL2; level++) {
		if(level & circuit->is_type) {
			if(area->lspdb[level - 1] && dict_count(area->lspdb[level - 1]) > 0) {
				for(dnode = dict_first(area->lspdb[level - 1]); dnode != NULL; dnode = dnode_next) {
...
int isis_circuit_up(struct isis_circuit *circuit) {
	...
	if(circuit->area->lsp_mtu > isis_circuit_pdu_size(circuit)) {
		zlog_err("Interface MTU %zu on %s is too low to support area lsp mtu %u!", isis_circuit_pdu_size(circuit), circuit->interface->name, circuit->area->lsp_mtu);
	...
	if(circuit->circ_type == CIRCUIT_T_BROADCAST) {
...
#ifdef EXTREME_DEGUG
		zlog_debug("isis_circuit_if_add: if_id %d, isomtu %d snpa %s", circuit->interface->ifindex, ISO_MTU(circuit), snpa_print(circuit->u.bc.snpa));
#endif /* EXTREME_DEBUG */
       ...
       */
		...
		if(circuit->is_type & IS_LEVEL_1) {
			THREAD_TIMER_ON(master, circuit->u.bc.t_run_dr[0], isis_run_dr_l1, circuit, 2 * circuit->hello_interval[0]);
		}
		if(circuit->is_type & IS_LEVEL_2) {
			THREAD_TIMER_ON(master, circuit->u.bc.t_run_dr[1], isis_run_dr_l2, circuit, 2 * circuit->hello_interval[1]);
		}
	} else {
...
void isis_circuit_print_vty(struct isis_circuit *circuit, struct vty *vty, char detail) {
	...
	if(detail == ISIS_UI_LEVEL_DETAIL) {
		...
		if(circuit->is_type & IS_LEVEL_1) {
			...
			if(circuit->area->newmetric) {
				...
				vty_out(vty, "      Metric: %d", circuit->metric[0]);
			}
			if(!circuit->is_passive) {
				vty_out(vty, ", Active neighbors: %u%s", circuit->upadjcount[0], VTY_NEWLINE);
				vty_out(vty,
					"      Hello interval: %u, "
					"Holddown count: %u %s%s",
					circuit->hello_interval[0], circuit->hello_multiplier[0], (circuit->pad_hellos ? "(pad)" : "(no-pad)"), VTY_NEWLINE);
				vty_out(vty,
					"      CNSP interval: %u, "
					"PSNP interval: %u%s",
					circuit->csnp_interval[0], circuit->psnp_interval[0], VTY_NEWLINE);
				if(circuit->circ_type == CIRCUIT_T_BROADCAST) {
					vty_out(vty, "      LAN Priority: %u, %s%s", circuit->priority[0], (circuit->u.bc.is_dr[0] ? "is DIS" : "is not DIS"), VTY_NEWLINE);
				}
			} else {
				vty_out(vty, "%s", VTY_NEWLINE);
			}
		...
		if(circuit->is_type & IS_LEVEL_2) {
			...
			if(circuit->area->newmetric) {
				...
				vty_out(vty, "      Metric: %d", circuit->metric[1]);
			}
			if(!circuit->is_passive) {
				vty_out(vty, ", Active neighbors: %u%s", circuit->upadjcount[1], VTY_NEWLINE);
				vty_out(vty,
					"      Hello interval: %u, "
					"Holddown count: %u %s%s",
					circuit->hello_interval[1], circuit->hello_multiplier[1], (circuit->pad_hellos ? "(pad)" : "(no-pad)"), VTY_NEWLINE);
				vty_out(vty,
					"      CNSP interval: %u, "
					"PSNP interval: %u%s",
					circuit->csnp_interval[1], circuit->psnp_interval[1], VTY_NEWLINE);
				if(circuit->circ_type == CIRCUIT_T_BROADCAST) {
					vty_out(vty, "      LAN Priority: %u, %s%s", circuit->priority[1], (circuit->u.bc.is_dr[1] ? "is DIS" : "is not DIS"), VTY_NEWLINE);
				}
			} else {
				vty_out(vty, "%s", VTY_NEWLINE);
			}
		...
		if(circuit->ip_addrs && listcount(circuit->ip_addrs) > 0) {
			...
			for(ALL_LIST_ELEMENTS_RO(circuit->ip_addrs, node, ip_addr)) {
				prefix2str(ip_addr, (char *) buf, BUFSIZ), vty_out(vty, "      %s%s", buf, VTY_NEWLINE);
			}
		}
		...
		for(ALL_LIST_ELEMENTS_RO(isis->area_list, node2, area)) {
			...
			if(circuit->csnp_interval[0] == circuit->csnp_interval[1]) {
				if(circuit->csnp_interval[0] != DEFAULT_CSNP_INTERVAL) {
					vty_out(vty, " isis csnp-interval %d%s", circuit->csnp_interval[0], VTY_NEWLINE);
					write++;
				}
			} else {
				for(i = 0; i < 2; i++) {
					if(circuit->csnp_interval[i] != DEFAULT_CSNP_INTERVAL) {
						vty_out(vty, " isis csnp-interval %d level-%d%s", circuit->csnp_interval[i], i + 1, VTY_NEWLINE);
						write++;
					}
				}
			}
			...
			if(circuit->psnp_interval[0] == circuit->psnp_interval[1]) {
				if(circuit->psnp_interval[0] != DEFAULT_PSNP_INTERVAL) {
					vty_out(vty, " isis psnp-interval %d%s", circuit->psnp_interval[0], VTY_NEWLINE);
					write++;
				}
			} else {
				for(i = 0; i < 2; i++) {
					if(circuit->psnp_interval[i] != DEFAULT_PSNP_INTERVAL) {
						vty_out(vty, " isis psnp-interval %d level-%d%s", circuit->psnp_interval[i], i + 1, VTY_NEWLINE);
						write++;
					}
				}
			}
			...
			if(circuit->hello_interval[0] == circuit->hello_interval[1]) {
				if(circuit->hello_interval[0] != DEFAULT_HELLO_INTERVAL) {
					vty_out(vty, " isis hello-interval %d%s", circuit->hello_interval[0], VTY_NEWLINE);
					write++;
				}
			} else {
				for(i = 0; i < 2; i++) {
					if(circuit->hello_interval[i] != DEFAULT_HELLO_INTERVAL) {
						vty_out(vty, " isis hello-interval %d level-%d%s", circuit->hello_interval[i], i + 1, VTY_NEWLINE);
						write++;
					}
				}
			}
			...
			if(circuit->hello_multiplier[0] == circuit->hello_multiplier[1]) {
				if(circuit->hello_multiplier[0] != DEFAULT_HELLO_MULTIPLIER) {
					vty_out(vty, " isis hello-multiplier %d%s", circuit->hello_multiplier[0], VTY_NEWLINE);
					write++;
				}
			} else {
				for(i = 0; i < 2; i++) {
					if(circuit->hello_multiplier[i] != DEFAULT_HELLO_MULTIPLIER) {
						vty_out(vty, " isis hello-multiplier %d level-%d%s", circuit->hello_multiplier[i], i + 1, VTY_NEWLINE);
						write++;
					}
				}
			}
			...
			if(circuit->priority[0] == circuit->priority[1]) {
				if(circuit->priority[0] != DEFAULT_PRIORITY) {
					vty_out(vty, " isis priority %d%s", circuit->priority[0], VTY_NEWLINE);
					write++;
				}
			} else {
				for(i = 0; i < 2; i++) {
					if(circuit->priority[i] != DEFAULT_PRIORITY) {
						vty_out(vty, " isis priority %d level-%d%s", circuit->priority[i], i + 1, VTY_NEWLINE);
						write++;
					}
				}
			}
			...
			if(circuit->te_metric[0] == circuit->te_metric[1]) {
				if(circuit->te_metric[0] != DEFAULT_CIRCUIT_METRIC) {
					vty_out(vty, " isis metric %d%s", circuit->te_metric[0], VTY_NEWLINE);
					write++;
				}
			} else {
				for(i = 0; i < 2; i++) {
					if(circuit->te_metric[i] != DEFAULT_CIRCUIT_METRIC) {
						vty_out(vty, " isis metric %d level-%d%s", circuit->te_metric[i], i + 1, VTY_NEWLINE);
						write++;
					}
				}
			}
			if(circuit->passwd.type == ISIS_PASSWD_TYPE_HMAC_MD5) {
				vty_out(vty, " isis password md5 %s%s", circuit->passwd.passwd, VTY_NEWLINE);
				write++;
			} else if(circuit->passwd.type == ISIS_PASSWD_TYPE_CLEARTXT) {
				vty_out(vty, " isis password clear %s%s", circuit->passwd.passwd, VTY_NEWLINE);
				write++;
			}
		}
		vty_out(vty, "!%s", VTY_NEWLINE);
...
int isis_circuit_metric_set(struct isis_circuit *circuit, int level, int metric) {
	...
	if(circuit->area && circuit->area->oldmetric && metric > MAX_NARROW_LINK_METRIC) {
		return -1;
	}
...
static int isis_circuit_passwd_set(struct isis_circuit *circuit, u_char passwd_type, const char *passwd) {
	int len;
...
int isis_circuit_circ_type_set(struct isis_circuit *circuit, int circ_type) {
   ...
   * is not supported. */
	if(circ_type == CIRCUIT_T_UNKNOWN || circ_type == CIRCUIT_T_LOOPBACK || circuit->circ_type == CIRCUIT_T_LOOPBACK) {
	...
	if(circuit->state != C_STATE_UP) {
		...
		struct isis_area *area = circuit->area;
		if(circ_type == CIRCUIT_T_BROADCAST && !if_is_broadcast(circuit->interface)) {
			return -1;
		}
