...
static struct isis_nexthop *isis_nexthop_create(struct in_addr *ip, ifindex_t ifindex) {
...
static int nexthoplookup(struct list *nexthops, struct in_addr *ip, ifindex_t ifindex) {
	struct listnode *node;
...
#ifdef HAVE_IPV6
static struct isis_nexthop6 *isis_nexthop6_new(struct in6_addr *ip6, ifindex_t ifindex) {
...
static struct isis_nexthop6 *isis_nexthop6_create(struct in6_addr *ip6, ifindex_t ifindex) {
	struct listnode *node;
	struct isis_nexthop6 *nexthop6;
...
static int nexthop6lookup(struct list *nexthops6, struct in6_addr *ip6, ifindex_t ifindex) {
	struct listnode *node;
	struct isis_nexthop6 *nh6;

	for(ALL_LIST_ELEMENTS_RO(nexthops6, node, nh6)) {
		if(!(memcmp(ip6, &nh6->ip6, sizeof(struct in6_addr))) && ifindex == nh6->ifindex) {
			return 1;
		}
	}
...
static void adjinfo2nexthop6(struct list *nexthops6, struct isis_adjacency *adj) {
	...
	for(ALL_LIST_ELEMENTS_RO(adj->ipv6_addrs, node, ipv6_addr)) {
		if(!nexthop6lookup(nexthops6, ipv6_addr, adj->circuit->interface->ifindex)) {
			nh6 = isis_nexthop6_create(ipv6_addr, adj->circuit->interface->ifindex);
...
static struct isis_route_info *isis_route_info_new(struct prefix *prefix, uint32_t cost, uint32_t depth, struct list *adjacencies) {
...
static int isis_route_info_same_attrib(struct isis_route_info *new, struct isis_route_info *old) {
...
static int isis_route_info_same(struct isis_route_info *new, struct isis_route_info *old, u_char family) {
	...
	if(family == AF_INET) {
		...
		for(ALL_LIST_ELEMENTS_RO(new->nexthops6, node, nexthop6)) {
			if(nexthop6lookup(old->nexthops6, &nexthop6->ip6, nexthop6->ifindex) == 0) {
				return 0;
			}
		}

		for(ALL_LIST_ELEMENTS_RO(old->nexthops6, node, nexthop6)) {
			if(nexthop6lookup(new->nexthops6, &nexthop6->ip6, nexthop6->ifindex) == 0) {
				return 0;
			}
		}
	}
...
struct isis_route_info *isis_route_create(struct prefix *prefix, u_int32_t cost, u_int32_t depth, struct list *adjacencies, struct isis_area *area, int level) {
	...
	if(!rinfo_old) {
		...
		if(isis->debugs & DEBUG_RTE_EVENTS) {
			zlog_debug("ISIS-Rte (%s) route already exists: %s", area->area_tag, buff);
		}
...
static void isis_route_validate_table(struct isis_area *area, struct route_table *table) {
	...
	for(rnode = route_top(table); rnode; rnode = route_next(rnode)) {
		...
		if(isis->debugs & DEBUG_RTE_EVENTS) {
			...
			zlog_debug(
				"ISIS-Rte (%s): route validate: %s %s %s %s", area->area_tag, (CHECK_FLAG(rinfo->flag, ISIS_ROUTE_FLAG_ZEBRA_SYNCED) ? "synced" : "not-synced"),
				(CHECK_FLAG(rinfo->flag, ISIS_ROUTE_FLAG_ZEBRA_RESYNC) ? "resync" : "not-resync"), (CHECK_FLAG(rinfo->flag, ISIS_ROUTE_FLAG_ACTIVE) ? "active" : "inactive"), buff
			);
		}
