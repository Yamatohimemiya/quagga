...
struct isis_lsp *lsp_search(u_char *id, dict_t *lspdb) {
	...
	for(dn = dict_first(lspdb); dn; dn = dict_next(lspdb, dn)) {
		zlog_debug("%s\t%pX", rawlspid_print((u_char *) dnode_getkey(dn)), dnode_get(dn));
	}
#endif /* EXTREME DEBUG */
 ...
 */
int lsp_compare(char *areatag, struct isis_lsp *lsp, u_int32_t seq_num, u_int16_t checksum, u_int16_t rem_lifetime) {
	/* no point in double ntohl on seqnum */
	if(lsp->lsp_header->seq_num == seq_num && lsp->lsp_header->checksum == checksum &&
	   /*comparing with 0, no need to do ntohl */
	   ((lsp->lsp_header->rem_lifetime == 0 && rem_lifetime == 0) || (lsp->lsp_header->rem_lifetime != 0 && rem_lifetime != 0))) {
		if(isis->debugs & DEBUG_SNP_PACKETS) {
			zlog_debug(
				"ISIS-Snp (%s): Compare LSP %s seq 0x%08x, cksum 0x%04x,"
				" lifetime %us",
				areatag, rawlspid_print(lsp->lsp_header->lsp_id), ntohl(lsp->lsp_header->seq_num), ntohs(lsp->lsp_header->checksum), ntohs(lsp->lsp_header->rem_lifetime)
			);
			zlog_debug(
				"ISIS-Snp (%s):         is equal to ours seq 0x%08x,"
				" cksum 0x%04x, lifetime %us",
				areatag, ntohl(seq_num), ntohs(checksum), ntohs(rem_lifetime)
			);
		}
		return LSP_EQUAL;
	}
	...
	if(ntohl(seq_num) > ntohl(lsp->lsp_header->seq_num) || (ntohl(seq_num) == ntohl(lsp->lsp_header->seq_num) && ((lsp->lsp_header->rem_lifetime != 0 && rem_lifetime == 0) || lsp->lsp_header->checksum != checksum))) {
		if(isis->debugs & DEBUG_SNP_PACKETS) {
			zlog_debug(
				"ISIS-Snp (%s): Compare LSP %s seq 0x%08x, cksum 0x%04x,"
				" lifetime %us",
				areatag, rawlspid_print(lsp->lsp_header->lsp_id), ntohl(seq_num), ntohs(checksum), ntohs(rem_lifetime)
			);
			zlog_debug(
				"ISIS-Snp (%s):       is newer than ours seq 0x%08x, "
				"cksum 0x%04x, lifetime %us",
				areatag, ntohl(lsp->lsp_header->seq_num), ntohs(lsp->lsp_header->checksum), ntohs(lsp->lsp_header->rem_lifetime)
			);
		}
		return LSP_NEWER;
	}
	if(isis->debugs & DEBUG_SNP_PACKETS) {
		zlog_debug("ISIS-Snp (%s): Compare LSP %s seq 0x%08x, cksum 0x%04x, lifetime %us", areatag, rawlspid_print(lsp->lsp_header->lsp_id), ntohl(seq_num), ntohs(checksum), ntohs(rem_lifetime));
		zlog_debug(
   ...
   */
	...
	switch(passwd->type) {
         ...
         * overwrite the MD5 hash */
			...
			memcpy(&lsp->tlv_data.auth_info.passwd, hmac_md5_hash, ISIS_AUTH_MD5_SIZE);
			tlv_add_authinfo(passwd->type, ISIS_AUTH_MD5_SIZE, hmac_md5_hash, lsp->pdu);
			break;
...
static void lsp_auth_update(struct isis_lsp *lsp) {
   ...
   */
	...
	memset(STREAM_DATA(lsp->pdu) + lsp->auth_tlv_offset + 3, 0, ISIS_AUTH_MD5_SIZE);
	/* Compute autentication value */
	hmac_md5(STREAM_DATA(lsp->pdu), stream_get_endp(lsp->pdu), (unsigned char *) &passwd->passwd, passwd->len, (unsigned char *) &hmac_md5_hash);
	/* Copy the hash into the stream */
	memcpy(STREAM_DATA(lsp->pdu) + lsp->auth_tlv_offset + 3, hmac_md5_hash, ISIS_AUTH_MD5_SIZE);
	memcpy(&lsp->tlv_data.auth_info.passwd, hmac_md5_hash, ISIS_AUTH_MD5_SIZE);
...
static void lsp_update_data(struct isis_lsp *lsp, struct stream *stream, struct isis_area *area, int level) {
   ...
   */
	...
	retval = parse_tlvs(area->area_tag, STREAM_DATA(lsp->pdu) + ISIS_FIXED_HDR_LEN + ISIS_LSP_HDR_LEN, ntohs(lsp->lsp_header->pdu_len) - ISIS_FIXED_HDR_LEN - ISIS_LSP_HDR_LEN, &expected, &found, &lsp->tlv_data, NULL);
	if(retval != ISIS_OK) {
	...
	if((found & TLVFLAG_DYN_HOSTNAME) && (area->dynhostname)) {
		isis_dynhn_insert(lsp->lsp_header->lsp_id, lsp->tlv_data.hostname, (lsp->lsp_header->lsp_bits & LSPBIT_IST) == IS_LEVEL_1_AND_2 ? IS_LEVEL_2 : IS_LEVEL_1);
	}

	return;
}

void lsp_update(struct isis_lsp *lsp, struct stream *stream, struct isis_area *area, int level) {
	dnode_t *dnode = NULL;
...
struct isis_lsp *lsp_new_from_stream_ptr(struct stream *stream, u_int16_t pdu_len, struct isis_lsp *lsp0, struct isis_area *area, int level) {
	struct isis_lsp *lsp;
...
struct isis_lsp *lsp_new(struct isis_area *area, u_char *lsp_id, u_int16_t rem_lifetime, u_int32_t seq_num, u_int8_t lsp_bits, u_int16_t checksum, int level) {
	struct isis_lsp *lsp;
	...
	if(isis->debugs & DEBUG_EVENTS) {
		zlog_debug(
			"New LSP with ID %s-%02x-%02x len %d seqnum %08x", sysid_print(lsp_id), LSP_PSEUDO_ID(lsp->lsp_header->lsp_id), LSP_FRAGMENT(lsp->lsp_header->lsp_id), ntohl(lsp->lsp_header->pdu_len), ntohl(lsp->lsp_header->seq_num)
		);
	}
 ...
 */
void lsp_build_list_nonzero_ht(u_char *start_id, u_char *stop_id, struct list *list, dict_t *lspdb) {
	...
	while(curr) {
		...
		if(curr && ((struct isis_lsp *) (curr->dict_data))->lsp_header->rem_lifetime) {
 ...
 */
void lsp_build_list(u_char *start_id, u_char *stop_id, u_char num_lsps, struct list *list, dict_t *lspdb) {
	u_char count;
	dnode_t *first, *last, *curr;
 ...
 */
void lsp_build_list_ssn(struct isis_circuit *circuit, u_char num_lsps, struct list *list, dict_t *lspdb) {
	dnode_t *dnode, *next;
	struct isis_lsp *lsp;
	u_char count = 0;
...
static void lsp_set_time(struct isis_lsp *lsp) {
	...
	lsp->lsp_header->rem_lifetime = htons(ntohs(lsp->lsp_header->rem_lifetime) - 1);
}
...
static void lspid_print(u_char *lsp_id, u_char *trg, char dynhost, char frag) {
	...
	if(frag) {
		sprintf((char *) trg, "%s.%02x-%02x", id, LSP_PSEUDO_ID(lsp_id), LSP_FRAGMENT(lsp_id));
	} else {
...
const char *lsp_bits2string(u_char *lsp_bits) {
	...
	pos += sprintf(pos, "%d/", ISIS_MASK_LSP_ATT_DEFAULT_BIT(*lsp_bits) ? 1 : 0);

	pos += sprintf(pos, "%d/", ISIS_MASK_LSP_PARTITION_BIT(*lsp_bits) ? 1 : 0);
...
void lsp_print_detail(struct isis_lsp *lsp, struct vty *vty, char dynhost) {
	...
	if(lsp->tlv_data.area_addrs) {
		for(ALL_LIST_ELEMENTS_RO(lsp->tlv_data.area_addrs, lnode, area_addr)) {
			vty_out(vty, "  Area Address: %s%s", isonet_print(area_addr->area_addr, area_addr->addr_len), VTY_NEWLINE);
		}
	}
	...
	if(lsp->tlv_data.nlpids) {
		for(i = 0; i < lsp->tlv_data.nlpids->count; i++) {
			switch(lsp->tlv_data.nlpids->nlpids[i]) {
				case NLPID_IP:
				case NLPID_IPV6: vty_out(vty, "  NLPID       : 0x%X%s", lsp->tlv_data.nlpids->nlpids[i], VTY_NEWLINE); break;
				default: vty_out(vty, "  NLPID       : %s%s", "unknown", VTY_NEWLINE); break;
			}
		}
	}
	...
	if(lsp->tlv_data.hostname) {
		...
		memcpy(hostname, lsp->tlv_data.hostname->name, lsp->tlv_data.hostname->namelen);
	...
	/* TE router id */
	if(lsp->tlv_data.router_id) {
		memcpy(ipv4_address, inet_ntoa(lsp->tlv_data.router_id->id), sizeof(ipv4_address));
	...
	if(lsp->tlv_data.is_neighs) {
		for(ALL_LIST_ELEMENTS_RO(lsp->tlv_data.is_neighs, lnode, is_neigh)) {
			...
			vty_out(vty, "  Metric      : %-8d IS            : %s%s", is_neigh->metrics.metric_default, LSPid, VTY_NEWLINE);
		}
	}

	/* for the internal reachable tlv */
	if(lsp->tlv_data.ipv4_int_reachs) {
		for(ALL_LIST_ELEMENTS_RO(lsp->tlv_data.ipv4_int_reachs, lnode, ipv4_reach)) {
			...
			vty_out(vty, "  Metric      : %-8d IPv4-Internal : %s %s%s", ipv4_reach->metrics.metric_default, ipv4_reach_prefix, ipv4_reach_mask, VTY_NEWLINE);
		}
	}

	/* for the external reachable tlv */
	if(lsp->tlv_data.ipv4_ext_reachs) {
		for(ALL_LIST_ELEMENTS_RO(lsp->tlv_data.ipv4_ext_reachs, lnode, ipv4_reach)) {
			memcpy(ipv4_reach_prefix, inet_ntoa(ipv4_reach->prefix), sizeof(ipv4_reach_prefix));
			memcpy(ipv4_reach_mask, inet_ntoa(ipv4_reach->mask), sizeof(ipv4_reach_mask));
			vty_out(vty, "  Metric      : %-8d IPv4-External : %s %s%s", ipv4_reach->metrics.metric_default, ipv4_reach_prefix, ipv4_reach_mask, VTY_NEWLINE);
		}
	}
	...
	if(lsp->tlv_data.ipv6_reachs) {
		for(ALL_LIST_ELEMENTS_RO(lsp->tlv_data.ipv6_reachs, lnode, ipv6_reach)) {
			...
			if((ipv6_reach->control_info & CTRL_INFO_DISTRIBUTION) == DISTRIBUTION_INTERNAL) {
				vty_out(vty, "  Metric      : %-8d IPv6-Internal : %s/%d%s", ntohl(ipv6_reach->metric), buff, ipv6_reach->prefix_len, VTY_NEWLINE);
			} else {
				vty_out(vty, "  Metric      : %-8d IPv6-External : %s/%d%s", ntohl(ipv6_reach->metric), buff, ipv6_reach->prefix_len, VTY_NEWLINE);
			}
		}
	}
#endif
	...
	if(lsp->tlv_data.te_is_neighs) {
		for(ALL_LIST_ELEMENTS_RO(lsp->tlv_data.te_is_neighs, lnode, te_is_neigh)) {
			...
			vty_out(vty, "  Metric      : %-8d IS-Extended   : %s%s", GET_TE_METRIC(te_is_neigh), LSPid, VTY_NEWLINE);
	...
	if(lsp->tlv_data.te_ipv4_reachs) {
		for(ALL_LIST_ELEMENTS_RO(lsp->tlv_data.te_ipv4_reachs, lnode, te_ipv4_reach)) {
			/* FIXME: There should be better way to output this stuff. */
			vty_out(vty, "  Metric      : %-8d IPv4-Extended : %s/%d%s", ntohl(te_ipv4_reach->te_metric), inet_ntoa(newprefix2inaddr(&te_ipv4_reach->prefix_start, te_ipv4_reach->control)), te_ipv4_reach->control & 0x3F,
				VTY_NEWLINE);
		}
	}
...
#define FRAG_NEEDED(S, T, I) (STREAM_SIZE(S) - STREAM_REMAIN(S) + (I) > FRAG_THOLD(S, T))

/* FIXME: It shouldn't be necessary to pass tlvsize here, TLVs can have
 * variable length (TE TLVs, sub TLVs). */
static void lsp_tlv_fit(struct isis_lsp *lsp, struct list **from, struct list **to, int tlvsize, int frag_thold, int tlv_build_func(struct list *, struct stream *)) {
	int count, i;
...
static u_int16_t lsp_rem_lifetime(struct isis_area *area, int level) {
	...
	rem_lifetime = isis_jitter(area->max_lsp_lifetime[level - 1], MAX_AGE_JITTER);
...
static u_int16_t lsp_refresh_time(struct isis_lsp *lsp, u_int16_t rem_lifetime) {
	...
	refresh_time = isis_jitter(area->lsp_refresh[level - 1], MAX_LSP_GEN_JITTER);

	/* RFC 4444 : make sure the refresh time is at least less than 300
   * of the remaining lifetime and more than gen interval */
	if(refresh_time <= area->lsp_gen_interval[level - 1] || refresh_time > (rem_lifetime - 300)) {
...
static struct isis_lsp *lsp_next_frag(u_char frag_num, struct isis_lsp *lsp0, struct isis_area *area, int level) {
	struct isis_lsp *lsp;
	...
	lsp = lsp_new(area, frag_id, ntohs(lsp0->lsp_header->rem_lifetime), 0, lsp_bits_generate(level, area->overload_bit, area->attached_bit), 0, level);
	lsp->area = area;
	lsp->own_lsp = 1;
	lsp_insert(lsp, area->lspdb[level - 1]);
	listnode_add(lsp0->lspu.frags, lsp);
...
static void lsp_build_ext_reach_ipv4(struct isis_lsp *lsp, struct isis_area *area, struct tlvs *tlv_data) {
...
static void lsp_build_ext_reach_ipv6(struct isis_lsp *lsp, struct isis_area *area, struct tlvs *tlv_data) {
	struct route_table *er_table;
	struct route_node *rn;
...
static void lsp_build_ext_reach(struct isis_lsp *lsp, struct isis_area *area, struct tlvs *tlv_data) {
...
static void lsp_build(struct isis_lsp *lsp, struct isis_area *area) {
   ...
   */
	...
	if(area->dynhostname) {
		...
		lsp->tlv_data.hostname = XMALLOC(MTYPE_ISIS_TLV, sizeof(struct hostname));

		strncpy((char *) lsp->tlv_data.hostname->name, hostname, sizeof(lsp->tlv_data.hostname->name));
		...
		lsp_debug("ISIS (%s): Adding dynamic hostname '%.*s'", area->area_tag, lsp->tlv_data.hostname->namelen, lsp->tlv_data.hostname->name);
	...
	if(isis->router_id != 0) {
       ...
       * TLV's are in use. */
		if(area->newmetric) {
			...
			lsp->tlv_data.router_id = XMALLOC(MTYPE_ISIS_TLV, sizeof(struct in_addr));
   ...
   */
	for(ALL_LIST_ELEMENTS_RO(area->circuit_list, node, circuit)) {
		if(!circuit->interface) {
			lsp_debug("ISIS (%s): Processing %s circuit %p with unknown interface", area->area_tag, circuit_type2string(circuit->circ_type), circuit);
		} else {
			lsp_debug("ISIS (%s): Processing %s circuit %s", area->area_tag, circuit_type2string(circuit->circ_type), circuit->interface->name);
		}
		...
		if(circuit->ip_router && circuit->ip_addrs && circuit->ip_addrs->count > 0) {
			...
			if(area->oldmetric) {
				...
				for(ALL_LIST_ELEMENTS_RO(circuit->ip_addrs, ipnode, ipv4)) {
					...
					lsp_debug("ISIS (%s): Adding old-style IP reachability for %s/%d", area->area_tag, buf, ipv4->prefixlen);
			...
			if(area->newmetric) {
				...
				for(ALL_LIST_ELEMENTS_RO(circuit->ip_addrs, ipnode, ipv4)) {
					...
					te_ipreach->control = (ipv4->prefixlen & 0x3F);
					memcpy(&te_ipreach->prefix_start, &ipv4->prefix.s_addr, (ipv4->prefixlen + 7) / 8);
       ...
       */
		if(circuit->ipv6_router && circuit->ipv6_non_link && circuit->ipv6_non_link->count > 0) {
			...
			for(ALL_LIST_ELEMENTS_RO(circuit->ipv6_non_link, ipnode, ipv6)) {
				...
				if(area->oldmetric) {
					ip6reach->metric = htonl(circuit->metric[level - 1]);
				} else {
				...
				if(level & circuit->is_type) {
					if(area->oldmetric) {
						...
						if(!memcmp(is_neigh->neigh_id, zero_id, ISIS_SYS_ID_LEN + 1)) {
							...
							lsp_debug("ISIS (%s): No DIS for circuit, not adding old-style IS neighbor.", area->area_tag);
						} else {
							listnode_add(tlv_data.is_neighs, is_neigh);
							lsp_debug("ISIS (%s): Adding DIS %s.%02x as old-style neighbor", area->area_tag, sysid_print(is_neigh->neigh_id), LSP_PSEUDO_ID(is_neigh->neigh_id));
						}
					}
					if(area->newmetric) {
						...
						if(!memcmp(te_is_neigh->neigh_id, zero_id, ISIS_SYS_ID_LEN + 1)) {
							...
							lsp_debug("ISIS (%s): No DIS for circuit, not adding te-style IS neighbor.", area->area_tag);
						} else {
							...
							lsp_debug("ISIS (%s): Adding DIS %s.%02x as te-style neighbor", area->area_tag, sysid_print(te_is_neigh->neigh_id), LSP_PSEUDO_ID(te_is_neigh->neigh_id));
						}
					}
				} else {
			...
			case CIRCUIT_T_P2P:
				...
				if(nei && (level & nei->circuit_t)) {
					if(area->oldmetric) {
						...
						is_neigh->metrics.metric_delay = METRICS_UNSUPPORTED;
						listnode_add(tlv_data.is_neighs, is_neigh);
						lsp_debug("ISIS (%s): Adding old-style is reach for %s", area->area_tag, sysid_print(is_neigh->neigh_id));
					}
					if(area->newmetric) {
						uint32_t metric;
						...
						listnode_add(tlv_data.te_is_neighs, te_is_neigh);
						lsp_debug("ISIS (%s): Adding te-style is reach for %s", area->area_tag, sysid_print(te_is_neigh->neigh_id));
					}
				} else {
					lsp_debug("ISIS (%s): No adjacency for given level on this circuit. Not adding IS neighbors", area->area_tag);
				}
				break;
			case CIRCUIT_T_LOOPBACK: break;
			default: zlog_warn("lsp_area_create: unknown circuit type");
		}
	}
	...
	while(tlv_data.ipv4_int_reachs && listcount(tlv_data.ipv4_int_reachs)) {
		...
		lsp_tlv_fit(lsp, &tlv_data.ipv4_int_reachs, &lsp->tlv_data.ipv4_int_reachs, IPV4_REACH_LEN, area->lsp_frag_threshold, tlv_add_ipv4_int_reachs);
	...
	while(tlv_data.ipv4_ext_reachs && listcount(tlv_data.ipv4_ext_reachs)) {
		...
		lsp_tlv_fit(lsp, &tlv_data.ipv4_ext_reachs, &lsp->tlv_data.ipv4_ext_reachs, IPV4_REACH_LEN, area->lsp_frag_threshold, tlv_add_ipv4_ext_reachs);
   ...
   * TLVs (sub TLVs!). */
	while(tlv_data.te_ipv4_reachs && listcount(tlv_data.te_ipv4_reachs)) {
		...
		lsp_tlv_fit(lsp, &tlv_data.te_ipv4_reachs, &lsp->tlv_data.te_ipv4_reachs, TE_IPV4_REACH_LEN, area->lsp_frag_threshold, tlv_add_te_ipv4_reachs);
	...
	while(tlv_data.ipv6_reachs && listcount(tlv_data.ipv6_reachs)) {
		...
		lsp_tlv_fit(lsp, &tlv_data.ipv6_reachs, &lsp->tlv_data.ipv6_reachs, IPV6_REACH_LEN, area->lsp_frag_threshold, tlv_add_ipv6_reachs);
	...
	while(tlv_data.is_neighs && listcount(tlv_data.is_neighs)) {
		...
		lsp_tlv_fit(lsp, &tlv_data.is_neighs, &lsp->tlv_data.is_neighs, IS_NEIGHBOURS_LEN, area->lsp_frag_threshold, tlv_add_is_neighs);
	...
	while(tlv_data.te_is_neighs && listcount(tlv_data.te_is_neighs)) {
		...
		lsp_tlv_fit(lsp, &tlv_data.te_is_neighs, &lsp->tlv_data.te_is_neighs, IS_NEIGHBOURS_LEN, area->lsp_frag_threshold, tlv_add_te_is_neighs);
	...
	/* Validate the LSP */
	retval = parse_tlvs(area->area_tag, STREAM_DATA(lsp->pdu) + ISIS_FIXED_HDR_LEN + ISIS_LSP_HDR_LEN, stream_get_endp(lsp->pdu) - ISIS_FIXED_HDR_LEN - ISIS_LSP_HDR_LEN, &expected, &found, &tlv_data, NULL);
...
int lsp_generate(struct isis_area *area, int level) {
	...
	if(oldlsp) {
		...
		lsp_search_and_destroy(oldlsp->lsp_header->lsp_id, area->lspdb[level - 1]);
	}
	rem_lifetime = lsp_rem_lifetime(area, level);
	newlsp = lsp_new(area, lspid, rem_lifetime, seq_num, area->is_type | area->overload_bit | area->attached_bit, 0, level);
	newlsp->area = area;
	newlsp->own_lsp = 1;
	...
	if(level == IS_LEVEL_1) {
		THREAD_TIMER_ON(master, area->t_lsp_refresh[level - 1], lsp_l1_refresh, area, refresh_time);
	} else if(level == IS_LEVEL_2) {
		THREAD_TIMER_ON(master, area->t_lsp_refresh[level - 1], lsp_l2_refresh, area, refresh_time);
	}
	...
	if(isis->debugs & DEBUG_UPDATE_PACKETS) {
		zlog_debug(
			...
			area->area_tag, level, rawlspid_print(newlsp->lsp_header->lsp_id), ntohl(newlsp->lsp_header->pdu_len), ntohl(newlsp->lsp_header->seq_num), ntohs(newlsp->lsp_header->checksum), ntohs(newlsp->lsp_header->rem_lifetime),
			refresh_time
		);
	}
	sched_debug("ISIS (%s): Built L%d LSP. Set triggered regenerate to non-pending.", area->area_tag, level);
...
static int lsp_regenerate(struct isis_area *area, int level) {
	...
	if(!lsp) {
		zlog_err("ISIS-Upd (%s): lsp_regenerate: no L%d LSP found!", area->area_tag, level);
		return ISIS_ERROR;
	}

	lsp_clear_data(lsp);
	lsp_build(lsp, area);
	lsp->lsp_header->lsp_bits = lsp_bits_generate(level, area->overload_bit, area->attached_bit);
	rem_lifetime = lsp_rem_lifetime(area, level);
	lsp->lsp_header->rem_lifetime = htons(rem_lifetime);
	...
	for(ALL_LIST_ELEMENTS_RO(lsp->lspu.frags, node, frag)) {
		frag->lsp_header->lsp_bits = lsp_bits_generate(level, area->overload_bit, area->attached_bit);
	...
	if(isis->debugs & DEBUG_UPDATE_PACKETS) {
		zlog_debug(
			...
			"seq 0x%08x, cksum 0x%04x, lifetime %us refresh %us",
			area->area_tag, level, rawlspid_print(lsp->lsp_header->lsp_id), ntohl(lsp->lsp_header->pdu_len), ntohl(lsp->lsp_header->seq_num), ntohs(lsp->lsp_header->checksum), ntohs(lsp->lsp_header->rem_lifetime), refresh_time
		);
	}
	sched_debug("ISIS (%s): Rebuilt L%d LSP. Set triggered regenerate to non-pending.", area->area_tag, level);
...
int lsp_regenerate_schedule(struct isis_area *area, int level, int all_pseudo) {
	...
	sched_debug("ISIS (%s): Scheduling regeneration of %s LSPs, %sincluding PSNs", area->area_tag, circuit_t2string(level), all_pseudo ? "" : "not ");
	...
	for(lvl = IS_LEVEL_1; lvl <= IS_LEVEL_2; lvl++) {
		...
		sched_debug("ISIS (%s): Checking whether L%d needs to be scheduled", area->area_tag, lvl);
		...
		if(area->lsp_regenerate_pending[lvl - 1]) {
			...
			sched_debug(
				...
				area->area_tag, (long long) remain.tv_sec, (long long) remain.tv_usec / 1000
			);
			continue;
		}
		...
		if(!lsp) {
			sched_debug("ISIS (%s): We do not have any LSPs to regenerate, nothing todo.", area->area_tag);
			continue;
		}
       ...
       * Throttle avoidance
       */
		sched_debug("ISIS (%s): Will schedule regen timer. Last run was: %lld, Now is: %lld", area->area_tag, (long long) lsp->last_generated, (long long) now);
		...
		if(diff < area->lsp_gen_interval[lvl - 1]) {
			...
			sched_debug("ISIS (%s): Scheduling in %ld ms to match configured lsp_gen_interval", area->area_tag, timeout);
		} else {
			/*
		...
		if(lvl == IS_LEVEL_1) {
			THREAD_TIMER_MSEC_ON(master, area->t_lsp_refresh[lvl - 1], lsp_l1_refresh, area, timeout);
		} else if(lvl == IS_LEVEL_2) {
			THREAD_TIMER_MSEC_ON(master, area->t_lsp_refresh[lvl - 1], lsp_l2_refresh, area, timeout);
		}
	}
...
static void lsp_build_pseudo(struct isis_lsp *lsp, struct isis_circuit *circuit, int level) {
	...
	lsp_debug("ISIS (%s): Constructing pseudo LSP %s for interface %s level %d", area->area_tag, rawlspid_print(lsp->lsp_header->lsp_id), circuit->interface->name, level);
   ...
   */
	if(circuit->area->oldmetric) {
		...
		lsp_debug("ISIS (%s): Adding %s.%02x as old-style neighbor (self)", area->area_tag, sysid_print(is_neigh->neigh_id), LSP_PSEUDO_ID(is_neigh->neigh_id));
	}
	if(circuit->area->newmetric) {
		...
		lsp_debug("ISIS (%s): Adding %s.%02x as te-style neighbor (self)", area->area_tag, sysid_print(te_is_neigh->neigh_id), LSP_PSEUDO_ID(te_is_neigh->neigh_id));
	}
	...
	for(ALL_LIST_ELEMENTS_RO(adj_list, node, adj)) {
		if(adj->level & level) {
			if((level == IS_LEVEL_1 && adj->sys_type == ISIS_SYSTYPE_L1_IS) || (level == IS_LEVEL_1 && adj->sys_type == ISIS_SYSTYPE_L2_IS && adj->adj_usage == ISIS_ADJ_LEVEL1AND2)
			   || (level == IS_LEVEL_2 && adj->sys_type == ISIS_SYSTYPE_L2_IS)) {
				...
				if(circuit->area->oldmetric) {
					...
					listnode_add(lsp->tlv_data.is_neighs, is_neigh);
					lsp_debug("ISIS (%s): Adding %s.%02x as old-style neighbor (peer)", area->area_tag, sysid_print(is_neigh->neigh_id), LSP_PSEUDO_ID(is_neigh->neigh_id));
				}
				if(circuit->area->newmetric) {
					te_is_neigh = XCALLOC(MTYPE_ISIS_TLV, sizeof(struct te_is_neigh));
					memcpy(&te_is_neigh->neigh_id, adj->sysid, ISIS_SYS_ID_LEN);
					listnode_add(lsp->tlv_data.te_is_neighs, te_is_neigh);
					lsp_debug("ISIS (%s): Adding %s.%02x as te-style neighbor (peer)", area->area_tag, sysid_print(te_is_neigh->neigh_id), LSP_PSEUDO_ID(te_is_neigh->neigh_id));
				}
				...
				lsp_debug("ISIS (%s): Adding %s as ES neighbor (peer)", area->area_tag, sysid_print(es_neigh->first_es_neigh));
			} else {
				lsp_debug("ISIS (%s): Ignoring neighbor %s, level does not match", area->area_tag, sysid_print(adj->sysid));
			}
		} else {
			lsp_debug("ISIS (%s): Ignoring neighbor %s, level does not intersect", area->area_tag, sysid_print(adj->sysid));
		}
	}
...
int lsp_generate_pseudo(struct isis_circuit *circuit, int level) {
	...
	if((circuit->is_type & level) != level || (circuit->state != C_STATE_UP) || (circuit->circ_type != CIRCUIT_T_BROADCAST) || (circuit->u.bc.is_dr[level - 1] == 0)) {
		return ISIS_ERROR;
	}
   ...
   */
	...
	/* RFC3787  section 4 SHOULD not set overload bit in pseudo LSPs */
	lsp = lsp_new(circuit->area, lsp_id, rem_lifetime, 1, circuit->area->is_type | circuit->area->attached_bit, 0, level);
	...
	if(level == IS_LEVEL_1) {
		THREAD_TIMER_ON(master, circuit->u.bc.t_refresh_pseudo_lsp[level - 1], lsp_l1_refresh_pseudo, circuit, refresh_time);
	} else if(level == IS_LEVEL_2) {
		THREAD_TIMER_ON(master, circuit->u.bc.t_refresh_pseudo_lsp[level - 1], lsp_l2_refresh_pseudo, circuit, refresh_time);
	}
	...
	if(isis->debugs & DEBUG_UPDATE_PACKETS) {
		zlog_debug(
			...
			"seq 0x%08x, cksum 0x%04x, lifetime %us, refresh %us",
			circuit->area->area_tag, level, rawlspid_print(lsp->lsp_header->lsp_id), ntohl(lsp->lsp_header->pdu_len), ntohl(lsp->lsp_header->seq_num), ntohs(lsp->lsp_header->checksum), ntohs(lsp->lsp_header->rem_lifetime),
			refresh_time
		);
	}
...
static int lsp_regenerate_pseudo(struct isis_circuit *circuit, int level) {
	...
	if(!lsp) {
		zlog_err("lsp_regenerate_pseudo: no l%d LSP %s found!", level, rawlspid_print(lsp_id));
		return ISIS_ERROR;
	}
	lsp_clear_data(lsp);
...
static int lsp_l1_refresh_pseudo(struct thread *thread) {
	...
	if((circuit->u.bc.is_dr[0] == 0) || (circuit->is_type & IS_LEVEL_1) == 0) {
		memcpy(id, isis->sysid, ISIS_SYS_ID_LEN);
...
static int lsp_l2_refresh_pseudo(struct thread *thread) {
	...
	if((circuit->u.bc.is_dr[1] == 0) || (circuit->is_type & IS_LEVEL_2) == 0) {
		memcpy(id, isis->sysid, ISIS_SYS_ID_LEN);
		LSP_PSEUDO_ID(id) = circuit->circuit_id;
		LSP_FRAGMENT(id) = 0;
...
int lsp_regenerate_schedule_pseudo(struct isis_circuit *circuit, int level) {
	...
	if(circuit == NULL || circuit->circ_type != CIRCUIT_T_BROADCAST || circuit->state != C_STATE_UP) {
		return ISIS_OK;
	}

	sched_debug("ISIS (%s): Scheduling regeneration of %s pseudo LSP for interface %s", area->area_tag, circuit_t2string(level), circuit->interface->name);
	...
	for(lvl = IS_LEVEL_1; lvl <= IS_LEVEL_2; lvl++) {
		sched_debug("ISIS (%s): Checking whether L%d pseudo LSP needs to be scheduled", area->area_tag, lvl);

		if(!((level & lvl) && (circuit->is_type & lvl))) {
			sched_debug("ISIS (%s): Level is not active on circuit", area->area_tag);
			continue;
		}

		if(circuit->u.bc.is_dr[lvl - 1] == 0) {
			sched_debug("ISIS (%s): This IS is not DR, nothing to do.", area->area_tag);
			continue;
		}

		if(circuit->lsp_regenerate_pending[lvl - 1]) {
			struct timeval remain = thread_timer_remain(circuit->u.bc.t_refresh_pseudo_lsp[lvl - 1]);
			sched_debug(
		...
		if(!lsp) {
			sched_debug("ISIS (%s): Pseudonode LSP does not exist yet, nothing to regenerate.", area->area_tag);
			continue;
		}
       ...
       * Throttle avoidance
       */
		sched_debug("ISIS (%s): Will schedule PSN regen timer. Last run was: %lld, Now is: %lld", area->area_tag, (long long) lsp->last_generated, (long long) now);
		THREAD_TIMER_OFF(circuit->u.bc.t_refresh_pseudo_lsp[lvl - 1]);
		diff = now - lsp->last_generated;
		if(diff < circuit->area->lsp_gen_interval[lvl - 1]) {
			timeout = 1000 * (circuit->area->lsp_gen_interval[lvl - 1] - diff);
			sched_debug("ISIS (%s): Sechduling in %ld ms to match configured lsp_gen_interval", area->area_tag, timeout);
		} else {
			timeout = 100;
			sched_debug(
				"ISIS (%s): Last generation was more than lsp_gen_interval ago."
		...
		if(lvl == IS_LEVEL_1) {
			THREAD_TIMER_MSEC_ON(master, circuit->u.bc.t_refresh_pseudo_lsp[lvl - 1], lsp_l1_refresh_pseudo, circuit, timeout);
		} else if(lvl == IS_LEVEL_2) {
			THREAD_TIMER_MSEC_ON(master, circuit->u.bc.t_refresh_pseudo_lsp[lvl - 1], lsp_l2_refresh_pseudo, circuit, timeout);
		}
	}
   ...
   */
	for(level = 0; level < ISIS_LEVELS; level++) {
		if(area->lspdb[level] && dict_count(area->lspdb[level]) > 0) {
			for(dnode = dict_first(area->lspdb[level]); dnode != NULL; dnode = dnode_next) {
               ...
               */
				...
				if(lsp->age_out == 0) {
					zlog_debug("ISIS-Upd (%s): L%u LSP %s seq 0x%08x aged out", area->area_tag, lsp->level, rawlspid_print(lsp->lsp_header->lsp_id), ntohl(lsp->lsp_header->seq_num));
#ifdef TOPOLOGY_GENERATE
           ...
           */
			if(listcount(lsp_list) > 0) {
				for(ALL_LIST_ELEMENTS_RO(area->circuit_list, cnode, circuit)) {
					...
					if(circuit->lsp_queue == NULL || diff < MIN_LSP_TRANS_INTERVAL) {
						continue;
					}
					for(ALL_LIST_ELEMENTS_RO(lsp_list, lspnode, lsp)) {
						if(circuit->upadjcount[lsp->level - 1] && ISIS_CHECK_FLAG(lsp->SRMflags, circuit)) {
...
void lsp_purge_non_exist(int level, struct isis_link_state_hdr *lsp_hdr, struct isis_area *area) {
   ...
   */
	...
	lsp->pdu = stream_new(LLC_LEN + area->lsp_mtu);
	lsp->isis_header = (struct isis_fixed_hdr *) STREAM_DATA(lsp->pdu);
	fill_fixed_hdr(lsp->isis_header, (lsp->level == IS_LEVEL_1) ? L1_LINK_STATE : L2_LINK_STATE);
	lsp->lsp_header = (struct isis_link_state_hdr *) (STREAM_DATA(lsp->pdu) + ISIS_FIXED_HDR_LEN);
...
#ifdef TOPOLOGY_GENERATE
static int top_lsp_refresh(struct thread *thread) {
	struct isis_lsp *lsp;
	u_int16_t rem_lifetime;
	...
	lsp_set_all_srmflags(lsp);
	if(isis->debugs & DEBUG_UPDATE_PACKETS) {
		zlog_debug("ISIS-Upd (): refreshing Topology L1 %s", rawlspid_print(lsp->lsp_header->lsp_id));
	}
	/* Refresh dynamic hostname in the cache. */
	isis_dynhn_insert(lsp->lsp_header->lsp_id, lsp->tlv_data.hostname, IS_LEVEL_1);

	lsp->lsp_header->lsp_bits = lsp_bits_generate(lsp->level, lsp->area->overload_bit, lsp->area->attached_bit);
	rem_lifetime = lsp_rem_lifetime(lsp->area, IS_LEVEL_1);
	lsp->lsp_header->rem_lifetime = htons(rem_lifetime);

	/* refresh_time = lsp_refresh_time (lsp, rem_lifetime); */
	THREAD_TIMER_ON(master, lsp->t_lsp_top_ref, top_lsp_refresh, lsp, lsp->area->lsp_refresh[0]);
...
void generate_topology_lsps(struct isis_area *area) {
	...
	for(i = 1; i < (max + 1); i++) {
		...
		lsp = lsp_new(area, lspid, rem_lifetime, 1, IS_LEVEL_1 | area->overload_bit | area->attached_bit, 0, 1);
		if(!lsp) {
			return;
		}
		lsp->from_topology = 1;
		...
		refresh_time = lsp_refresh_time(lsp, rem_lifetime);
		THREAD_TIMER_ON(master, lsp->t_lsp_top_ref, top_lsp_refresh, lsp, refresh_time);
		lsp_set_all_srmflags(lsp);
...
void build_topology_lsp_data(struct isis_lsp *lsp, struct isis_area *area, int lsp_top_num) {
	struct listnode *node;
	struct arc *arc;
	struct is_neigh *is_neigh;
	struct te_is_neigh *te_is_neigh;
	...
	for(ALL_LIST_ELEMENTS_RO(area->topology, node, arc)) {
		...
		if(area->newmetric) {
			...
			te_is_neigh = XCALLOC(MTYPE_ISIS_TLV, sizeof(struct te_is_neigh));
			memcpy(&te_is_neigh->neigh_id, area->topology_baseis, ISIS_SYS_ID_LEN);
	...
	while(tlv_data.is_neighs && listcount(tlv_data.is_neighs)) {
		...
		lsp_tlv_fit(lsp, &tlv_data.is_neighs, &lsp->tlv_data.is_neighs, IS_NEIGHBOURS_LEN, area->lsp_frag_threshold, tlv_add_is_neighs);
		if(tlv_data.is_neighs && listcount(tlv_data.is_neighs)) {
			lsp = lsp_next_frag(LSP_FRAGMENT(lsp->lsp_header->lsp_id) + 1, lsp0, area, IS_LEVEL_1);
		}
	}
