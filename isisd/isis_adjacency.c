...
struct isis_adjacency *isis_new_adj(const u_char *id, const u_char *snpa, int level, struct isis_circuit *circuit) {
	struct isis_adjacency *adj;
	int i;
	...
	if(circuit->circ_type == CIRCUIT_T_BROADCAST) {
		...
		{
			adj->dis_record[(i * ISIS_LEVELS) + level - 1].dis = ISIS_UNKNOWN_DIS;
			adj->dis_record[(i * ISIS_LEVELS) + level - 1].last_dis_change = time(NULL);
		}
	}
...
struct isis_adjacency *isis_adj_lookup_snpa(const u_char *ssnpa, struct list *adjdb) {
	struct listnode *node;
	struct isis_adjacency *adj;
...
static const char *adj_state2string(int state) {
	switch(state) {
		case ISIS_ADJ_INITIALIZING: return "Initializing";
		case ISIS_ADJ_UP: return "Up";
		case ISIS_ADJ_DOWN: return "Down";
...
void isis_adj_state_change(struct isis_adjacency *adj, enum isis_adj_state new_state, const char *reason) {
	int old_state;
	int level;
	...
	if(isis->debugs & DEBUG_ADJ_PACKETS) {
		zlog_debug("ISIS-Adj (%s): Adjacency state change %d->%d: %s", circuit->area->area_tag, old_state, new_state, reason ? reason : "unspecified");
	}
	...
	if(circuit->area->log_adj_changes) {
		...
		zlog_info("%%ADJCHANGE: Adjacency to %s (%s) changed from %s to %s, %s", adj_name, adj->circuit->interface->name, adj_state2string(old_state), adj_state2string(new_state), reason ? reason : "unspecified");
	}
...
void isis_adj_print(struct isis_adjacency *adj) {
	...
	zlog_debug("SystemId %20s SNPA %s, level %d\nHolding Time %d", sysid_print(adj->sysid), snpa_print(adj->snpa), adj->level, adj->hold_time);
...
void isis_adj_print_vty(struct isis_adjacency *adj, struct vty *vty, char detail) {
	...
	if(detail == ISIS_UI_LEVEL_BRIEF) {
		...
		now = time(NULL);
		if(adj->last_upd) {
			vty_out(vty, "%-9llu", (unsigned long long) adj->last_upd + adj->hold_time - now);
		} else {
			vty_out(vty, "-        ");
		}
	...
	if(detail == ISIS_UI_LEVEL_DETAIL) {
		...
		now = time(NULL);
		if(adj->last_upd) {
			vty_out(vty, ", Expires in %s", time2string(adj->last_upd + adj->hold_time - now));
		} else {
		...
		if(adj->circuit && (adj->circuit->circ_type == CIRCUIT_T_BROADCAST)) {
			...
			if(dyn) {
				vty_out(vty, ", LAN id: %s.%02x", dyn->name.name, adj->lanid[ISIS_SYS_ID_LEN]);
			} else {
				vty_out(vty, ", LAN id: %s.%02x", sysid_print(adj->lanid), adj->lanid[ISIS_SYS_ID_LEN]);
			}

			vty_out(vty, "%s", VTY_NEWLINE);
			vty_out(vty, "    LAN Priority: %u", adj->prio[adj->level - 1]);

			vty_out(vty, ", %s, DIS flaps: %u, Last: %s ago", isis_disflag2string(adj->dis_record[ISIS_LEVELS + level - 1].dis), adj->dischanges[level - 1],
		...
		if(adj->area_addrs && listcount(adj->area_addrs) > 0) {
			...
			for(ALL_LIST_ELEMENTS_RO(adj->area_addrs, node, area_addr)) {
				vty_out(vty, "      %s%s", isonet_print(area_addr->area_addr, area_addr->addr_len), VTY_NEWLINE);
			}
		}
		if(adj->ipv4_addrs && listcount(adj->ipv4_addrs) > 0) {
...
void isis_adj_build_neigh_list(struct list *adjdb, struct list *list) {
	...
	for(ALL_LIST_ELEMENTS_RO(adjdb, node, adj)) {
		...
		if((adj->adj_state == ISIS_ADJ_UP || adj->adj_state == ISIS_ADJ_INITIALIZING)) {
