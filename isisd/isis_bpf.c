...
struct bpf_insn llcfilter[] = { BPF_STMT(BPF_LD + BPF_B + BPF_ABS, ETHER_HDR_LEN), /* check first byte */
...
static int open_bpf_dev(struct isis_circuit *circuit) {
	...
	if(fd < 0) {
		zlog_warn("open_bpf_dev(): failed to create bpf socket: %s", safe_strerror(errno));
		return ISIS_WARNING;
	}
	...
	if(ioctl(fd, BIOCSETIF, (caddr_t) &ifr) < 0) {
		zlog_warn("open_bpf_dev(): failed to bind to interface: %s", safe_strerror(errno));
		return ISIS_WARNING;
	}
   ...
   */
	if(blen < circuit->interface->mtu) {
		if(ioctl(fd, BIOCSBLEN, &circuit->interface->mtu) < 0) {
			zlog_warn("failed to set BPF buffer len (%u to %u)", blen, circuit->interface->mtu);
		}
	}
   ...
   */
	...
	if(ioctl(fd, BIOCSETF, (caddr_t) &bpf_prog) < 0) {
		zlog_warn("open_bpf_dev(): failed to install filter: %s", safe_strerror(errno));
		return ISIS_WARNING;
	}
...
int isis_recv_pdu_bcast(struct isis_circuit *circuit, u_char *ssnpa) {
	...
	if(bytesread < 0) {
		zlog_warn("isis_recv_pdu_bcast(): read() failed: %s", safe_strerror(errno));
		return ISIS_WARNING;
	}
	...
	stream_write(circuit->rcv_stream, readbuff + offset, bpf_hdr->bh_caplen - LLC_LEN - ETHER_HDR_LEN);
	stream_set_getp(circuit->rcv_stream, 0);

	memcpy(ssnpa, readbuff + bpf_hdr->bh_hdrlen + ETHER_ADDR_LEN, ETHER_ADDR_LEN);
...
int isis_recv_pdu_p2p(struct isis_circuit *circuit, u_char *ssnpa) {
	...
	bytesread = stream_read(circuit->rcv_stream, circuit->fd, circuit->interface->mtu);
...
int isis_send_pdu_bcast(struct isis_circuit *circuit, int level) {
   ...
   */
	...
	memcpy(sock_buff + (LLC_LEN + ETHER_HDR_LEN), circuit->snd_stream->data, stream_get_endp(circuit->snd_stream));

	/* now we can send this */
	written = write(circuit->fd, sock_buff, buflen);
	if(written < 0) {
		zlog_warn("IS-IS bpf: could not transmit packet on %s: %s", circuit->interface->name, safe_strerror(errno));
