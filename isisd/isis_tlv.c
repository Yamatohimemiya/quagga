 ...
 */
int parse_tlvs(char *areatag, u_char *stream, int size, u_int32_t *expected, u_int32_t *found, struct tlvs *tlvs, u_int32_t *auth_tlv_offset) {
	u_char type, length;
	...
	*found = 0;
	...
	while(pnt < stream + size - 2) {
	   ...
	   */
			...
			case IS_NEIGHBOURS: *found |= TLVFLAG_IS_NEIGHS;
#ifdef EXTREME_TLV_DEBUG
				zlog_debug("ISIS-TLV (%s): IS Neighbours length %d", areatag, length);
#endif /* EXTREME_TLV_DEBUG */
...
#ifdef EXTREME_TLV_DEBUG
				zlog_debug("ISIS-TLV (%s): Extended IS Neighbours length %d", areatag, length);
#endif /* EXTREME_TLV_DEBUG */
	   ...
	   */
#ifdef EXTREME_TLV_DEBUG
				zlog_debug("ISIS-TLV (%s): ES Neighbours length %d", areatag, length);
#endif /* EXTREME_TLV_DEBUG */
...
#ifdef EXTREME_TLV_DEBUG
				zlog_debug("ISIS-TLV (%s): LAN Neigbours length %d", areatag, length);
#endif /* EXTREME_TLV_DEBUG */
	   ...
	   */
				*found |= TLVFLAG_NLPID;
#ifdef EXTREME_TLV_DEBUG
				zlog_debug("ISIS-TLV (%s): Protocols Supported length %d", areatag, length);
#endif /* EXTREME_TLV_DEBUG */
	   ...
	   */
...
#ifdef EXTREME_TLV_DEBUG
				zlog_debug("ISIS-TLV (%s): IPv4 Address length %d", areatag, length);
#endif /* EXTREME_TLV_DEBUG */
				if(*expected & TLVFLAG_IPV4_ADDR) {
					while(length > value_len) {
						ipv4_addr = (struct in_addr *) pnt;
#ifdef EXTREME_TLV_DEBUG
						zlog_debug("ISIS-TLV (%s) : IP ADDR %s, pnt %p", areatag, inet_ntoa(*ipv4_addr), pnt);
#endif /* EXTREME_TLV_DEBUG */
						if(!tlvs->ipv4_addrs) {
			...
			case AUTH_INFO: *found |= TLVFLAG_AUTH_INFO;
#ifdef EXTREME_TLV_DEBUG
				zlog_debug("ISIS-TLV (%s): IS-IS Authentication Information", areatag);
#endif
			...
			case DYNAMIC_HOSTNAME: *found |= TLVFLAG_DYN_HOSTNAME;
#ifdef EXTREME_TLV_DEBUG
				zlog_debug("ISIS-TLV (%s): Dynamic Hostname length %d", areatag, length);
#endif /* EXTREME_TLV_DEBUG */
...
#ifdef EXTREME_TLV_DEBUG
				zlog_debug("ISIS-TLV (%s): IPv4 internal Reachability length %d", areatag, length);
#endif /* EXTREME_TLV_DEBUG */
...
#ifdef EXTREME_TLV_DEBUG
				zlog_debug("ISIS-TLV (%s): IPv4 external Reachability length %d", areatag, length);
#endif /* EXTREME_TLV_DEBUG */
...
#ifdef EXTREME_TLV_DEBUG
				zlog_debug("ISIS-TLV (%s): IPv4 extended Reachability length %d", areatag, length);
#endif /* EXTREME_TLV_DEBUG */
				endpnt = pnt + length;
				if(*expected & TLVFLAG_TE_IPV4_REACHABILITY) {
					while(length > value_len) {
						...
						if((te_ipv4_reach->control & 0x3F) > IPV4_MAX_BITLEN) {
							zlog_warn(
								"ISIS-TLV (%s): invalid IPv4 extended reach"
								"ability prefix length %d",
								areatag, te_ipv4_reach->control & 0x3F
							);
							retval = ISIS_WARNING;
							break;
						}
						...
						value_len += 5 + ((te_ipv4_reach->control & 0x3F) ? ((((te_ipv4_reach->control & 0x3F) - 1) >> 3) + 1) : 0);
						pnt += 5 + ((te_ipv4_reach->control & 0x3F) ? ((((te_ipv4_reach->control & 0x3F) - 1) >> 3) + 1) : 0);
					}
				}
	...
	#ifdef EXTREME_TLV_DEBUG
				zlog_debug("ISIS-TLV (%s): IPv6 Address length %d", areatag, length);
	#endif /* EXTREME_TLV_DEBUG */
				...
				if(*expected & TLVFLAG_IPV6_REACHABILITY) {
					while(length > value_len) {
						...
						if(ipv6_reach->prefix_len > IPV6_MAX_BITLEN) {
							zlog_warn(
								"ISIS-TLV (%s): invalid IPv6 extended reach"
								"ability prefix length %d",
								areatag, ipv6_reach->prefix_len
							);
							retval = ISIS_WARNING;
							break;
						}
	   ...
	   */
			...
			default:
				zlog_warn("ISIS-TLV (%s): unsupported TLV type %d, length %d", areatag, type, length);
...
int add_tlv(u_char tag, u_char len, u_char *value, struct stream *stream) {
	if((stream_get_size(stream) - stream_get_endp(stream)) < (((unsigned) len) + 2)) {
		zlog_warn(
			"No room for TLV of type %d "
			"(total size %d available %d required %d)",
			tag, (int) stream_get_size(stream), (int) (stream_get_size(stream) - stream_get_endp(stream)), len + 2
		);
		return ISIS_WARNING;
	}
...
int tlv_add_authinfo(u_char auth_type, u_char auth_len, u_char *auth_value, struct stream *stream) {
	u_char value[255];
	u_char *pos = value;
	*pos++ = auth_type;
...
static int tlv_add_ipv4_reachs(u_char tag, struct list *ipv4_reachs, struct stream *stream) {
	struct listnode *node;
...
int tlv_add_te_ipv4_reachs(struct list *te_ipv4_reachs, struct stream *stream) {
	...
	for(ALL_LIST_ELEMENTS_RO(te_ipv4_reachs, node, te_reach)) {
		...
		if(pos - value + (5 + prefix_size) > 255) {
			retval = add_tlv(TE_IPV4_REACHABILITY, pos - value, value, stream);
			if(retval != ISIS_OK) {
				return retval;
			}
			pos = value;
