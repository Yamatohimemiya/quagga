...
void trickle_up(int index, struct pqueue *queue) {
	...
	while(index > 0 && (*queue->cmp)(tmp, queue->array[PARENT_OF(index)]) < 0) {
		/* actually trickle up */
...
void trickle_down(int index, struct pqueue *queue) {
	...
	while(HAVE_CHILD(index, queue)) {
		/* If right child exists, and if the right child is more proper
         to be moved upper.  */
		if(RIGHT_OF(index) < queue->size && (*queue->cmp)(queue->array[LEFT_OF(index)], queue->array[RIGHT_OF(index)]) > 0) {
...
struct pqueue *pqueue_create(void) {
	...
	queue->array = XCALLOC(MTYPE_PQUEUE_DATA, DATA_SIZE * PQUEUE_INIT_ARRAYSIZE);
...
void pqueue_remove_at(int index, struct pqueue *queue) {
	...
	if(index > 0 && (*queue->cmp)(queue->array[index], queue->array[PARENT_OF(index)]) < 0) {
