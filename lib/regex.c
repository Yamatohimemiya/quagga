...
static int debug;
	...
	#define DEBUG_PRINT2(x1, x2) \
		if(debug) printf(x1, x2)
	#define DEBUG_PRINT3(x1, x2, x3) \
		if(debug) printf(x1, x2, x3)
	#define DEBUG_PRINT4(x1, x2, x3, x4) \
		if(debug) printf(x1, x2, x3, x4)
...
unsigned char *end;
{
	...
	while(p < pend) {
		...
		switch((re_opcode_t) *p++) {
			...
			case duplicate: printf("/duplicate/%d", *p++); break;

			case anychar: printf("/anychar"); break;
			...
			case begline: printf("/begline"); break;

			case endline: printf("/endline"); break;
			...
			case push_dummy_failure: printf("/push_dummy_failure"); break;
			...
			case wordbound: printf("/wordbound"); break;

			case notwordbound: printf("/notwordbound"); break;

			case wordbeg: printf("/wordbeg"); break;

			case wordend: printf("/wordend");

	#ifdef emacs
			case before_dot: printf("/before_dot"); break;

			case at_dot: printf("/at_dot"); break;

			case after_dot: printf("/after_dot"); break;
			...
			case wordchar: printf("/wordchar"); break;

			case notwordchar: printf("/notwordchar"); break;

			case begbuf: printf("/begbuf"); break;

			case endbuf: printf("/endbuf"); break;

			default: printf("?%d", *(p - 1));
		}
...
static const size_t re_error_msgid_idx[] = { REG_NOERROR_IDX, REG_NOMATCH_IDX, REG_BADPAT_IDX, REG_ECOLLATE_IDX, REG_ECTYPE_IDX, REG_EESCAPE_IDX, REG_ESUBREG_IDX, REG_EBRACK_IDX, REG_EPAREN_IDX,
					     REG_EBRACE_IDX,  REG_BADBR_IDX,   REG_ERANGE_IDX, REG_ESPACE_IDX,	 REG_BADRPT_IDX, REG_EEND_IDX,	  REG_ESIZE_IDX,   REG_ERPAREN_IDX };
...
/* Define macros to initialize and free the failure stack.
   Do `return -2' if the alloc fails.  */
	...
	#define INIT_FAIL_STACK() \
		do { \
			fail_stack.stack = (fail_stack_elt_t *) REGEX_ALLOCATE_STACK(INIT_FAILURE_ALLOC * sizeof(fail_stack_elt_t)); \
\
...
#define DOUBLE_FAIL_STACK(fail_stack) \
	((fail_stack).size > (unsigned) (re_max_failures * MAX_FAILURE_ITEMS) \
		 ? 0 \
		 : ((fail_stack).stack = (fail_stack_elt_t *) REGEX_REALLOCATE_STACK((fail_stack).stack, (fail_stack).size * sizeof(fail_stack_elt_t), ((fail_stack).size << 1) * sizeof(fail_stack_elt_t)), \
\
...
/* Push pointer POINTER on FAIL_STACK.
   ...
   space to do so.  */
#define PUSH_PATTERN_OP(POINTER, FAIL_STACK) ((FAIL_STACK_FULL() && !DOUBLE_FAIL_STACK(FAIL_STACK)) ? 0 : ((FAIL_STACK).stack[(FAIL_STACK).avail++].pointer = POINTER, 1))
...
#define NUM_FAILURE_ITEMS (((0 ? 0 : highest_active_reg - lowest_active_reg + 1) * NUM_REG_ITEMS) + NUM_NONREG_ITEMS)
...
#define EXTEND_BUFFER() \
	do { \
		...
		if(old_buffer != bufp->buffer) { \
			...
			if(fixup_alt_jump) fixup_alt_jump = (fixup_alt_jump - old_buffer) + bufp->buffer; \
			if(laststart) laststart = (laststart - old_buffer) + bufp->buffer; \
			if(pending_exact) pending_exact = (pending_exact - old_buffer) + bufp->buffer; \
		} \
	} while(0)
...
#define SET_LIST_BIT(c) (b[((unsigned char) (c)) / BYTEWIDTH] |= 1 << (((unsigned char) c) % BYTEWIDTH))
...
#define GET_UNSIGNED_NUMBER(num) \
   ...
   problem.  Use a reasonable default value.  */
	...
	#define IS_CHAR_CLASS(string) \
		(STREQ(string, "alpha") || STREQ(string, "upper") || STREQ(string, "lower") || STREQ(string, "digit") || STREQ(string, "alnum") || STREQ(string, "xdigit") || STREQ(string, "space") || STREQ(string, "print") \
		 || STREQ(string, "punct") || STREQ(string, "graph") || STREQ(string, "cntrl") || STREQ(string, "blank"))
#endif
			...
			case '*':
               ...
               and also whether or not two or more matches is allowed.  */
                   ...
                   for null bytes, because of RE_DOT_NOT_NULL?  */
						if(TRANSLATE(*(p - 2)) == TRANSLATE('.') && zero_times_ok && p < pend && TRANSLATE(*p) == TRANSLATE('\n') && !(syntax & RE_DOT_NEWLINE)) { /* We have .*\n.  */
			...
			case '[':
               ...
               statement, so we only need one BUF_PUSH.  */
			       ...
			       avoid an arbitrary limit in some compiler.  */
									if((is_alnum && ISALNUM(ch)) || (is_alpha && ISALPHA(ch)) || (is_blank && ISBLANK(ch)) || (is_cntrl && ISCNTRL(ch))) {
										SET_LIST_BIT(ch);
									}
									if((is_digit && ISDIGIT(ch)) || (is_graph && ISGRAPH(ch)) || (is_lower && ISLOWER(ch)) || (is_print && ISPRINT(ch))) {
										SET_LIST_BIT(ch);
									}
									if((is_punct && ISPUNCT(ch)) || (is_space && ISSPACE(ch)) || (is_upper && ISUPPER(ch)) || (is_xdigit && ISXDIGIT(ch))) {
										SET_LIST_BIT(ch);
									}
									if(translate && (is_upper || is_lower) && (ISUPPER(ch) || ISLOWER(ch))) {
										SET_LIST_BIT(ch);
...
handle_close:
                 ...
                 ``can't happen''.  */
                   ...
                   as in `(ab)c(de)' -- the second group is #2.  */
							...
							fixup_alt_jump = COMPILE_STACK_TOP.fixup_alt_jump ? bufp->buffer + COMPILE_STACK_TOP.fixup_alt_jump - 1 : 0;
...
handle_alt:
                 ...
                 when know we're at the end of a series of alternatives.  */
					...
					case '{':
                        ...
                        operator.  */
						   || ((syntax & RE_INTERVALS) && (syntax & RE_NO_BK_BRACES)) || (p - 2 == pattern && p == pend)) {
							goto normal_backslash;
						}
...
normal_char:
				...
				if(!pending_exact
				   ...
				   || *p == '*' || *p == '^' || ((syntax & RE_BK_PLUS_QM) ? *p == '\\' && (p[1] == '+' || p[1] == '?') : (*p == '+' || *p == '?'))
				   || ((syntax & RE_INTERVALS) && ((syntax & RE_NO_BK_BRACES) ? *p == '{' : (p[0] == '\\' && p[1] == '{')))) {
	...
	#ifdef emacs
			if(!fail_stack.stack) {
				fail_stack.stack = (fail_stack_elt_t *) xmalloc(fail_stack.size * sizeof(fail_stack_elt_t));
			} else {
				fail_stack.stack = (fail_stack_elt_t *) xrealloc(fail_stack.stack, (fail_stack.size * sizeof(fail_stack_elt_t)));
			}
	#else  /* not emacs */
			if(!fail_stack.stack) {
				fail_stack.stack = (fail_stack_elt_t *) malloc(fail_stack.size * sizeof(fail_stack_elt_t));
			} else {
				fail_stack.stack = (fail_stack_elt_t *) realloc(fail_stack.stack, (fail_stack.size * sizeof(fail_stack_elt_t)));
			}
	#endif /* not emacs */
		}
	...
	while(1) {
		...
		switch(SWITCH_ENUM_CAST((re_opcode_t) *p++)) {
         ...
         with `break'.  */
			...
			case exactn: fastmap[p[1]] = 1; break;
         ...
         `continue'.  */
			...
			case wordbeg:
			case wordend:
			case push_dummy_failure: continue;
...
handle_on_failure_jump:
			...
			case set_number_at: p += 4; continue;

			case start_memory:
			case stop_memory: p += 2; continue;

			default: abort(); /* We have listed all the cases.  */
		}			  /* switch *p++ */
...
#define WORDCHAR_P(d) (SYNTAX((d) == end1 ? *string2 : (d) == string2 - 1 ? *(end1 - 1) : *(d)) == Sword)

/* Disabled due to a compiler bug -- see comment at case wordbound */
#if 0
/* Test if the character before D and the one at D differ with respect
   to being word-constituent.  */
	#define AT_WORD_BOUNDARY(d) (AT_STRINGS_BEG(d) || AT_STRINGS_END(d) || WORDCHAR_P(d - 1) != WORDCHAR_P(d))
#endif

/* Free everything we malloc.  */
#ifdef MATCH_MAY_ALLOCATE
	#define FREE_VAR(var) \
		if(var) REGEX_FREE(var); \
		var = NULL
...
static boolean common_op_match_null_string_p _RE_ARGS((unsigned char **p, unsigned char *end, register_info_type *reg_info));
...
succeed_label:
			...
			if(regs && !bufp->no_sub) {
		     ...
		     warning under GCC when assert expands to nothing.  */
				...
				if(regs->num_regs > 0) {
					regs->start[0] = pos;
					regs->end[0] = (MATCHING_IN_FIRST_STRING ? ((regoff_t) (d - string1)) : ((regoff_t) (d - string2 + size1)));
				}
		...
		switch(SWITCH_ENUM_CAST((re_opcode_t) *p++)) {
           ...
           currently have n == 0.  */
			case no_op: DEBUG_PRINT1("EXECUTING no_op.\n"); break;

			case succeed: DEBUG_PRINT1("EXECUTING succeed.\n"); goto succeed_label;
           ...
           registers data structure) under the register number.  */
			case start_memory:
             ...
             the string in case this attempt to match fails.  */
				old_regstart[*p] = REG_MATCH_NULL_STRING_P(reg_info[*p]) ? REG_UNSET(regstart[*p]) ? d : regstart[*p] : regstart[*p];
           ...
           followed by the numeric value of <digit> as the register number.  */
			case duplicate:
               ...
               the end of the first string.  */
					...
					dend2 = ((FIRST_STRING_P(regstart[regno]) == FIRST_STRING_P(regend[regno])) ? regend[regno] : end_match_1);
					for(;;) {
           ...
           case; that seems worse than this.  */
			case on_failure_keep_string_jump: DEBUG_PRINT1("EXECUTING on_failure_keep_string_jump"); EXTRACT_NUMBER_AND_INCR(mcnt, p);
#ifdef _LIBC
				DEBUG_PRINT3(" %d (to %p):\n", mcnt, p + mcnt);
#else
	   ...
	   We change it to either `pop_failure_jump' or `jump'.  */
		   ...
		   yet.  */
						...
						if((re_opcode_t) p1[3] == exactn && p1[5] != c) {
							...
							if(c < (unsigned char) (p1[4] * BYTEWIDTH) && p1[5 + c / BYTEWIDTH] & (1 << (c % BYTEWIDTH))) {
								not = !not ;
							}
		    ...
		    && ! ((int) p2[1] * BYTEWIDTH > (int) p1[5]
			  && (p2[2 + p1[5] / BYTEWIDTH]
			      & (1 << (p1[5] % BYTEWIDTH)))))
#else
						if((re_opcode_t) p1[3] == exactn && !((int) p2[1] * BYTEWIDTH > (int) p1[4] && (p2[2 + p1[4] / BYTEWIDTH] & (1 << (p1[4] % BYTEWIDTH)))))
#endif
						{
							p[-3] = (unsigned char) pop_failure_jump;
							DEBUG_PRINT3("  %c != %c => pop_failure_jump.\n", c, p1[5]);
...
unconditional_jump:
           ...
           in `group_match_null_string_p' et al.  */
			case jump_past_alt: DEBUG_PRINT1("EXECUTING jump_past_alt.\n"); goto unconditional_jump;
...
int eflags;
{
	...
	ret = re_search(
		&private_preg, string, len,
		/* start: */ 0, /* range: */ len, want_reg_info ? &regs : (struct re_registers *) 0
	);
...
weak_alias(__regexec, regexec)
   ...
   from either regcomp or regexec.   We don't use PREG here.  */
	...
	size_t regerror(err, preg, errbuf, errbuf_size)
int err;
const regex_t *preg;
char *errbuf;
size_t errbuf_size;
{
	...
	if(err < 0 || err >= (int) (sizeof(re_error_msgid_idx) / sizeof(re_error_msgid_idx[0]))) {
