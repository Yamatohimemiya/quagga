...
buffer_status_t buffer_flush_window(struct buffer *b, int fd, int width, int height, int erase_flag, int no_more_flag) {
	int nbytes;
	int iov_alloc;
	int iov_index;
	struct iovec *iov;
	...
	char more[] = " --More-- ";
	char erase[] = { 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08 };
	struct buffer_data *data;
	int column;
	...
	if(height < 1) {
		zlog_warn("%s called with non-positive window height %d, forcing to 1", __func__, height);
		height = 1;
	} else if(height >= 2) {
		height--;
	}
	if(width < 1) {
		zlog_warn("%s called with non-positive window width %d, forcing to 1", __func__, width);
		width = 1;
	}
		...
		/* This should not ordinarily happen. */
		{
			...
			if(iov != small_iov) {
				...
				zlog_err(
					...
					__func__, (void *) b->head, (void *) b->tail, (void *) b->head->next
				);
				iov = XMALLOC(MTYPE_TMP, iov_alloc * sizeof(*iov));
     ...
     example: Solaris2.6 are defined IOV_MAX size at 16.     */
	{
		...
		while(iov_index > 0) {
			...
			if((nbytes = writev(fd, c_iov, iov_size)) < 0) {
				zlog_warn("%s: writev to fd %d failed: %s", __func__, fd, safe_strerror(errno));
				break;
			}
...
#define MAX_FLUSH 131072
	...
	for(d = b->head; d && (iovcnt < MAX_CHUNKS) && (nbyte < MAX_FLUSH); d = d->next, iovcnt++) {
	...
	if((ssize_t) (written = writev(fd, iov, iovcnt)) < 0) {
		...
		zlog_warn("%s: write error on fd %d: %s", __func__, fd, safe_strerror(errno));
		return BUFFER_ERROR;
	}
