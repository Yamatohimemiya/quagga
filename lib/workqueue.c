...
struct work_queue *work_queue_new(struct thread_master *m, const char *queue_name) {
...
static int work_queue_schedule(struct work_queue *wq, unsigned int delay) {
	...
	if(CHECK_FLAG(wq->flags, WQ_UNPLUGGED) && (wq->thread == NULL) && (listcount(wq->items) > 0)) {
		wq->thread = thread_add_background(wq->master, work_queue_run, wq, delay);
		return 1;
	} else {
		return 0;
	}
...
DEFUN(show_work_queues, show_work_queues_cmd, "show work-queues", SHOW_STR "Work Queue information\n") {
	...
	vty_out(vty, "%c %8s %5s %8s %21s %6s %5s%s", ' ', "List", "(ms) ", "Q. Runs", "Cycle Counts   ", " ", "Worst", VTY_NEWLINE);
	vty_out(vty, "%c %8s %5s %8s %7s %6s %6s %6s %5s %s%s", 'P', "Items", "Hold", "Total", "Best", "Worst", "Gran.", "Avg.", "Lat.", "Name", VTY_NEWLINE);

	for(ALL_LIST_ELEMENTS_RO(work_queues, node, wq)) {
		vty_out(vty, "%c %8u %5u %8lu %7u %6u %6u %6u %5lu %s%s", (CHECK_FLAG(wq->flags, WQ_UNPLUGGED) ? ' ' : 'P'), listcount(wq->items), wq->spec.hold, wq->runs, wq->cycles.best, MIN(wq->cycles.best, wq->cycles.worst),
			wq->cycles.granularity, (wq->runs) ? (unsigned int) (wq->cycles.total / wq->runs) : 0, wq->worst_usec, wq->name, VTY_NEWLINE);
	}
   ...
   */
	...
	for(ALL_LIST_ELEMENTS(wq->items, node, nnode, item)) {
		...
		do {
			...
			item->ran++;
		} while((ret == WQ_RETRY_NOW) && (item->ran < wq->spec.max_retries));
		...
		if(!(cycles % wq->cycles.granularity) && (took = thread_should_yield(thread))) {
			yielded = 1;
			goto stats;
		}
	}
...
#define WQ_HYSTERESIS_FACTOR 4
	...
	if(yielded && (cycles < wq->cycles.granularity)) {
		wq->cycles.granularity = ((cycles > 0) ? cycles : WORK_QUEUE_MIN_GRANULARITY);
