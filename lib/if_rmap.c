...
static struct if_rmap *if_rmap_set(const char *ifname, enum if_rmap_type type, const char *routemap_name) {
	struct if_rmap *if_rmap;
	...
	if(type == IF_RMAP_IN) {
		...
		if_rmap->routemap[IF_RMAP_IN] = XSTRDUP(MTYPE_IF_RMAP_NAME, routemap_name);
	}
	if(type == IF_RMAP_OUT) {
		if(if_rmap->routemap[IF_RMAP_OUT]) {
			XFREE(MTYPE_IF_RMAP_NAME, if_rmap->routemap[IF_RMAP_OUT]);
		}
		if_rmap->routemap[IF_RMAP_OUT] = XSTRDUP(MTYPE_IF_RMAP_NAME, routemap_name);
	}
...
static int if_rmap_unset(const char *ifname, enum if_rmap_type type, const char *routemap_name) {
	struct if_rmap *if_rmap;
...
DEFUN(if_rmap, if_rmap_cmd, "route-map RMAP_NAME (in|out) IFNAME",
      "Route map set\n"
      "Route map name\n"
...
ALIAS(if_rmap, if_ipv6_rmap_cmd, "route-map RMAP_NAME (in|out) IFNAME",
      "Route map set\n"
      "Route map name\n"
      "Route map set for input filtering\n"
      "Route map set for output filtering\n"
...
ALIAS(no_if_rmap, no_if_ipv6_rmap_cmd, "no route-map ROUTEMAP_NAME (in|out) IFNAME",
      NO_STR "Route map unset\n"
	     "Route map name\n"
	     "Route map for input filtering\n"
	     "Route map for output filtering\n"
...
int config_write_if_rmap(struct vty *vty) {
	...
	for(i = 0; i < ifrmaphash->size; i++) {
		for(mp = ifrmaphash->index[i]; mp; mp = mp->next) {
			...
			if(if_rmap->routemap[IF_RMAP_IN]) {
				vty_out(vty, " route-map %s in %s%s", if_rmap->routemap[IF_RMAP_IN], if_rmap->ifname, VTY_NEWLINE);
				write++;
			}

			if(if_rmap->routemap[IF_RMAP_OUT]) {
				vty_out(vty, " route-map %s out %s%s", if_rmap->routemap[IF_RMAP_OUT], if_rmap->ifname, VTY_NEWLINE);
				write++;
			}
		}
	}
