 ...
 */
void *zzcalloc(int type, size_t size) {
	void *memory;
...
static void mtype_log(char *func, void *memory, const char *file, int line, int type) {
...
char *mtype_zstrdup(const char *file, int line, int type, const char *str) {
	char *memory;
...
static void log_memstats(int pri) {
	struct mlist *ml;
...
void log_memstats_stderr(const char *prefix) {
	...
	for(ml = mlists; ml->list; ml++) {
		...
		for(m = ml->list; m->index >= 0; m++) {
			if(m->index && mstat[m->index].alloc) {
				if(!i) {
					fprintf(stderr, "%s: memstats: Current memory utilization in module %s:\n", prefix, ml->name);
				}
				fprintf(stderr, "%s: memstats:  %-30s: %10ld%s\n", prefix, m->format, mstat[m->index].alloc, mstat[m->index].alloc < 0 ? " (REPORT THIS BUG!)" : "");
				i = j = 1;
			}
		}
	}
	...
	if(j) {
		...
		fprintf(stderr, "%s: memstats: No remaining tracked memory utilization.\n", prefix);
	}
}

static void show_separator(struct vty *vty) {
	vty_out(vty, "-----------------------------\r\n");
}

static int show_memory_vty(struct vty *vty, struct memory_list *list) {
	struct memory_list *m;
	int needsep = 0;
...
static int show_memory_mallinfo(struct vty *vty) {
	...
	vty_out(vty, "  Total heap allocated:  %s%s", mtype_memstr(buf, MTYPE_MEMSTR_LEN, minfo.arena), VTY_NEWLINE);
	vty_out(vty, "  Holding block headers: %s%s", mtype_memstr(buf, MTYPE_MEMSTR_LEN, minfo.hblkhd), VTY_NEWLINE);
	vty_out(vty, "  Used small blocks:     %s%s", mtype_memstr(buf, MTYPE_MEMSTR_LEN, minfo.usmblks), VTY_NEWLINE);
	vty_out(vty, "  Used ordinary blocks:  %s%s", mtype_memstr(buf, MTYPE_MEMSTR_LEN, minfo.uordblks), VTY_NEWLINE);
	vty_out(vty, "  Free small blocks:     %s%s", mtype_memstr(buf, MTYPE_MEMSTR_LEN, minfo.fsmblks), VTY_NEWLINE);
	vty_out(vty, "  Free ordinary blocks:  %s%s", mtype_memstr(buf, MTYPE_MEMSTR_LEN, minfo.fordblks), VTY_NEWLINE);
...
DEFUN(show_memory, show_memory_cmd, "show memory",
...
unsigned long mtype_stats_alloc(int type) {
