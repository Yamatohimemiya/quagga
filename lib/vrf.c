...
/* Build the table key */
static void vrf_build_key(vrf_id_t vrf_id, struct prefix *p) {
	p->family = AF_INET;
...
static struct vrf *vrf_get(vrf_id_t vrf_id) {
	struct prefix p;
...
static void vrf_delete(struct vrf *vrf) {
...
static struct vrf *vrf_lookup(vrf_id_t vrf_id) {
	struct prefix p;
	struct route_node *rn;
	struct vrf *vrf = NULL;
...
static int vrf_enable(struct vrf *vrf) {
	if(!vrf_is_enabled(vrf)) {
		...
		if(!vrf_is_enabled(vrf)) {
			zlog_err("Can not enable VRF %u: %s!", vrf->vrf_id, safe_strerror(errno));
			return 0;
		}

		if(have_netns()) {
			zlog_info("VRF %u is associated with NETNS %s.", vrf->vrf_id, vrf->name);
		}
...
void vrf_add_hook(int type, int (*func)(vrf_id_t, void **)) {
	switch(type) {
		case VRF_NEW_HOOK: vrf_master.vrf_new_hook = func; break;
		case VRF_DELETE_HOOK: vrf_master.vrf_delete_hook = func; break;
		case VRF_ENABLE_HOOK: vrf_master.vrf_enable_hook = func; break;
		case VRF_DISABLE_HOOK: vrf_master.vrf_disable_hook = func; break;
		default: break;
	}
}

/* Return the iterator of the first VRF. */
vrf_iter_t vrf_first(void) {
	struct route_node *rn;
...
vrf_iter_t vrf_next(vrf_iter_t iter) {
...
vrf_iter_t vrf_iterator(vrf_id_t vrf_id) {
	struct prefix p;
	struct route_node *rn;
...
vrf_id_t vrf_iter2id(vrf_iter_t iter) {
...
#define VRF_BITMAP_NUM_OF_BYTES_IN_GROUP (VRF_BITMAP_NUM_OF_BITS_IN_GROUP / CHAR_BIT + 1) /* +1 for ensure */
...
void vrf_bitmap_set(vrf_bitmap_t bmap, vrf_id_t vrf_id) {
	...
	if(bm->groups[group] == NULL) {
		bm->groups[group] = XCALLOC(MTYPE_VRF_BITMAP, VRF_BITMAP_NUM_OF_BYTES_IN_GROUP);
	}

	SET_FLAG(bm->groups[group][VRF_BITMAP_INDEX_IN_GROUP(offset)], VRF_BITMAP_FLAG(offset));
}
...
void vrf_bitmap_unset(vrf_bitmap_t bmap, vrf_id_t vrf_id) {
	...
	UNSET_FLAG(bm->groups[group][VRF_BITMAP_INDEX_IN_GROUP(offset)], VRF_BITMAP_FLAG(offset));
}
...
int vrf_bitmap_check(vrf_bitmap_t bmap, vrf_id_t vrf_id) {
	...
	return CHECK_FLAG(bm->groups[group][VRF_BITMAP_INDEX_IN_GROUP(offset)], VRF_BITMAP_FLAG(offset)) ? 1 : 0;
}
...
static char *vrf_netns_pathname(struct vty *vty, const char *name) {
	...
	if(!result) {
		vty_out(vty, "Invalid pathname: %s%s", safe_strerror(errno), VTY_NEWLINE);
		return NULL;
	}
	return pathname;
}

DEFUN(vrf_netns, vrf_netns_cmd, "vrf <1-65535> netns NAME",
      "Enable a VRF\n"
	...
	if(vrf->name && strcmp(vrf->name, pathname) != 0) {
		vty_out(vty, "VRF %u is already configured with NETNS %s%s", vrf->vrf_id, vrf->name, VTY_NEWLINE);
		return CMD_WARNING;
	}
	...
	if(!vrf_enable(vrf)) {
		vty_out(vty, "Can not associate VRF %u with NETNS %s%s", vrf->vrf_id, vrf->name, VTY_NEWLINE);
		return CMD_WARNING;
	}
...
DEFUN(no_vrf_netns, no_vrf_netns_cmd, "no vrf <1-65535> netns NAME",
      NO_STR "Enable a VRF\n"
	     "Specify the VRF identifier\n"
	     "Associate with a NETNS\n"
	     "The file name in " VRF_RUN_DIR ", or a full pathname\n") {
...
/* VRF node. */
static struct cmd_node vrf_node = { VRF_NODE, "", /* VRF node has no interface. */
				    1 };
...
static int vrf_config_write(struct vty *vty) {
	...
	for(rn = route_top(vrf_table); rn; rn = route_next(rn)) {
		if((vrf = rn->info) != NULL && vrf->vrf_id != VRF_DEFAULT && vrf->name) {
