 ...
 */
struct route_table *route_table_init_with_delegate(route_table_delegate_t *delegate) {
...
static struct route_node *route_node_set(struct route_table *table, const struct prefix *prefix) {
...
static void route_common(const struct prefix *n, const struct prefix *p, struct prefix *new) {
	int i;
	u_char diff;
	u_char mask;
...
/* Find matched prefix. */
struct route_node *route_node_match(const struct route_table *table, const struct prefix *p) {
	struct route_node *node;
...
struct route_node *route_node_match_ipv4(const struct route_table *table, const struct in_addr *addr) {
...
#ifdef HAVE_IPV6
struct route_node *route_node_match_ipv6(const struct route_table *table, const struct in6_addr *addr) {
...
struct route_node *route_node_lookup(const struct route_table *table, const struct prefix *p) {
	struct route_node *node;
...
struct route_node *route_node_get(struct route_table *const table, const struct prefix *p) {
...
struct route_node *route_next_until(struct route_node *node, struct route_node *limit) {
	struct route_node *next;
	struct route_node *start;
 ...
 */
static struct route_node *route_node_create(route_table_delegate_t *delegate, struct route_table *table) {
	struct route_node *node;
 ...
 */
static void route_node_destroy(route_table_delegate_t *delegate, struct route_table *table, struct route_node *node) {
 ...
 * Default delegate.
 */
static route_table_delegate_t default_delegate = { .create_node = route_node_create, .destroy_node = route_node_destroy };
 ...
 */
int route_table_prefix_iter_cmp(struct prefix *p1, struct prefix *p2) {
 ...
 */
static struct route_node *route_table_get_next_internal(const struct route_table *table, struct prefix *p) {
 ...
 * iteration.
 */
struct route_node *route_table_get_next(const struct route_table *table, struct prefix *p) {
	struct route_node *node;
...
void route_table_iter_pause(route_table_iter_t *iter) {
	switch(iter->state) {
		...
		case RT_ITER_STATE_DONE: return;
