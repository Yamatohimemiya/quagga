...
int zprivs_change_caps(zebra_privs_ops_t op) {
	...
	if(!cap_set_flag(zprivs_state.caps, CAP_EFFECTIVE, zprivs_state.syscaps_p->num, zprivs_state.syscaps_p->caps, cflag)) {
...
zebra_privs_current_t zprivs_state_caps(void) {
	...
	for(i = 0; i < zprivs_state.syscaps_p->num; i++) {
		if(cap_get_flag(zprivs_state.caps, zprivs_state.syscaps_p->caps[i], CAP_EFFECTIVE, &val)) {
			zlog_warn("zprivs_state_caps: could not cap_get_flag, %s", safe_strerror(errno));
...
static void zprivs_caps_init(struct zebra_privs_t *zprivs) {
	...
	/* set permitted caps */
	cap_set_flag(zprivs_state.caps, CAP_PERMITTED, zprivs_state.syscaps_p->num, zprivs_state.syscaps_p->caps, CAP_SET);

	/* set inheritable caps, if any */
	if(zprivs_state.syscaps_i && zprivs_state.syscaps_i->num) {
		cap_set_flag(zprivs_state.caps, CAP_INHERITABLE, zprivs_state.syscaps_i->num, zprivs_state.syscaps_i->caps, CAP_SET);
	}
...
static void zprivs_caps_terminate(void) {
	...
	if(cap_set_proc(zprivs_state.caps)) {
		fprintf(stderr, "privs_terminate: cap_set_proc failed, %s", safe_strerror(errno));
		exit(1);
	}
...
void zprivs_terminate(struct zebra_privs_t *zprivs) {
	...
	if(zprivs_state.zuid) {
		if(setreuid(zprivs_state.zuid, zprivs_state.zuid)) {
			fprintf(stderr, "privs_terminate: could not setreuid, %s", safe_strerror(errno));
			exit(1);
		}
	}
#endif /* HAVE_LCAPS */
...
void zprivs_get_ids(struct zprivs_ids_t *ids) {
	ids->uid_priv = getuid();
	(zprivs_state.zuid) ? (ids->uid_normal = zprivs_state.zuid) : (ids->uid_normal = -1);
	(zprivs_state.zgid) ? (ids->gid_normal = zprivs_state.zgid) : (ids->gid_normal = -1);
	(zprivs_state.vtygrp) ? (ids->gid_vty = zprivs_state.vtygrp) : (ids->gid_vty = -1);
