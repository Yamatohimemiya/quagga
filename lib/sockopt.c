...
int setsockopt_so_recvbuf(int sock, int size) {
	...
	if((ret = setsockopt(sock, SOL_SOCKET, SO_RCVBUF, (char *) &size, sizeof(int))) < 0) {
		zlog_err("fd %d: can't setsockopt SO_RCVBUF to %d: %s", sock, size, safe_strerror(errno));
	}
...
int setsockopt_so_sendbuf(const int sock, int size) {
	int ret = setsockopt(sock, SOL_SOCKET, SO_SNDBUF, (char *) &size, sizeof(int));

	if(ret < 0) {
		zlog_err("fd %d: can't setsockopt SO_SNDBUF to %d: %s", sock, size, safe_strerror(errno));
	}
...
int getsockopt_so_sendbuf(const int sock) {
	...
	int ret = getsockopt(sock, SOL_SOCKET, SO_SNDBUF, (char *) &optval, &optlen);
	if(ret < 0) {
		zlog_err("fd %d: can't getsockopt SO_SNDBUF: %d (%s)", sock, errno, safe_strerror(errno));
		return ret;
	}
	return optval;
}
...
static void *getsockopt_cmsg_data(struct msghdr *msgh, int level, int type) {
	...
	for(cmsg = ZCMSG_FIRSTHDR(msgh); cmsg != NULL; cmsg = CMSG_NXTHDR(msgh, cmsg)) {
...
int setsockopt_ipv6_tclass(int sock, int tclass) {
	...
	if(ret < 0) {
		zlog_warn("Can't set IPV6_TCLASS option for fd %d to %#x: %s", sock, tclass, safe_strerror(errno));
	}
	#endif
	return ret;
}
 ...
 */
int setsockopt_ipv4_multicast(int sock, int optname, unsigned int mcast_addr, ifindex_t ifindex) {
#ifdef HAVE_RFC3678
	struct group_req gr;
	...
	ret = setsockopt(sock, IPPROTO_IP, (optname == IP_ADD_MEMBERSHIP) ? MCAST_JOIN_GROUP : MCAST_LEAVE_GROUP, (void *) &gr, sizeof(gr));
	...
	ret = setsockopt(sock, IPPROTO_IP, optname, (void *) &mreqn, sizeof(mreqn));
	if((ret < 0) && (optname == IP_ADD_MEMBERSHIP) && (errno == EADDRINUSE)) {
		...
		zlog_info(
			...
			sock, inet_ntop(AF_INET, &mreqn.imr_multiaddr, buf[0], sizeof(buf[0])), ifindex
		);
	...
	if((ret < 0) && (optname == IP_ADD_MEMBERSHIP) && (errno == EADDRINUSE)) {
		...
		zlog_info(
			"setsockopt_ipv4_multicast attempting to drop and "
			"re-add (fd %d, mcast %s, ifindex %u)",
			sock, inet_ntop(AF_INET, &mreq.imr_multiaddr, buf[0], sizeof(buf[0])), ifindex
		);
 ...
 */
int setsockopt_ipv4_multicast_if(int sock, ifindex_t ifindex) {
...
static int setsockopt_ipv4_ifindex(int sock, ifindex_t val) {
	...
	if((ret = setsockopt(sock, IPPROTO_IP, IP_PKTINFO, &val, sizeof(val))) < 0) {
		zlog_warn("Can't set IP_PKTINFO option for fd %d to %d: %s", sock, val, safe_strerror(errno));
	}
#elif defined(IP_RECVIF)
	if((ret = setsockopt(sock, IPPROTO_IP, IP_RECVIF, &val, sizeof(val))) < 0) {
		zlog_warn("Can't set IP_RECVIF option for fd %d to %d: %s", sock, val, safe_strerror(errno));
	}
#else
...
int setsockopt_ipv4_tos(int sock, int tos) {
	...
	if(ret < 0) {
		zlog_warn("Can't set IP_TOS option for fd %d to %#x: %s", sock, tos, safe_strerror(errno));
	}
	return ret;
}
...
int setsockopt_ifindex(int af, int sock, ifindex_t val) {
	...
	switch(af) {
		case AF_INET: ret = setsockopt_ipv4_ifindex(sock, val); break;
#ifdef HAVE_IPV6
		case AF_INET6: ret = setsockopt_ipv6_pktinfo(sock, val); break;
#endif
		default: zlog_warn("setsockopt_ifindex: unknown address family %d", af);
	}
	return ret;
}
...
static ifindex_t getsockopt_ipv4_ifindex(struct msghdr *msgh) {
	...
	#ifndef SUNOS_5
	/* BSD */
	sdl = (struct sockaddr_dl *) getsockopt_cmsg_data(msgh, IPPROTO_IP, IP_RECVIF);
	if(sdl != NULL) {
...
ifindex_t getsockopt_ifindex(int af, struct msghdr *msgh) {
	switch(af) {
		case AF_INET: return (getsockopt_ipv4_ifindex(msgh)); break;
#ifdef HAVE_IPV6
		case AF_INET6: return (getsockopt_ipv6_ifindex(msgh)); break;
#endif
		default: zlog_warn("getsockopt_ifindex: unknown address family %d", af); return 0;
	}
}
...
int sockopt_tcp_signature(int sock, union sockunion *su, const char *password) {
   ...
   */
	...
	if(susock->sa.sa_family == su->sa.sa_family) {
       ...
       */
		if(su2->sa.sa_family == AF_INET6 && su->sa.sa_family == AF_INET) {
	...
	if((ret = setsockopt(sock, IPPROTO_TCP, TCP_MD5SIG, &md5sig, sizeof md5sig)) < 0) {
	 ...
	 one was not previously set. */
		if(ENOENT == errno) {
			...
			zlog_err("sockopt_tcp_signature: setsockopt(%d): %s", sock, safe_strerror(errno));
		}
	}
	return ret;
