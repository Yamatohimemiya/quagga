...
/* #define MEMORY_LOG */
#ifdef MEMORY_LOG
	#define XMALLOC(mtype, size) mtype_zmalloc(__FILE__, __LINE__, (mtype), (size))
	#define XCALLOC(mtype, size) mtype_zcalloc(__FILE__, __LINE__, (mtype), (size))
	#define XREALLOC(mtype, ptr, size) mtype_zrealloc(__FILE__, __LINE__, (mtype), (ptr), (size))
	#define XFREE(mtype, ptr) \
		do { \
			mtype_zfree(__FILE__, __LINE__, (mtype), (ptr)); \
			(ptr) = NULL; \
		} while(0)
	#define XSTRDUP(mtype, str) mtype_zstrdup(__FILE__, __LINE__, (mtype), (str))
#else
...
extern void *mtype_zrealloc(const char *file, int line, int type, void *ptr, size_t size);

extern void mtype_zfree(const char *file, int line, int type, void *ptr);

extern char *mtype_zstrdup(const char *file, int line, int type, const char *str);
