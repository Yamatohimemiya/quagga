...
static void smux_getresp_send(oid objid[], size_t objid_len, long reqid, long errstat, long errindex, u_char val_type, void *arg, size_t arg_len) {
	u_char buf[BUFSIZ];
	...
	ptr = asn_build_int(ptr, &len, (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_INTEGER), &reqid, sizeof(reqid));

	if(debug_smux) {
		zlog_debug("SMUX GETRSP errstat: %ld", errstat);
	}

	ptr = asn_build_int(ptr, &len, (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_INTEGER), &errstat, sizeof(errstat));
	if(debug_smux) {
		zlog_debug("SMUX GETRSP errindex: %ld", errindex);
	}

	ptr = asn_build_int(ptr, &len, (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_INTEGER), &errindex, sizeof(errindex));

	h2 = ptr;
	/* Place holder h2 for one variable */
	ptr = asn_build_sequence(ptr, &len, (u_char) (ASN_SEQUENCE | ASN_CONSTRUCTOR), 0);
	h2e = ptr;

	ptr = snmp_build_var_op(ptr, objid, &objid_len, val_type, arg_len, arg, &len);
...
static u_char *smux_var(u_char *ptr, size_t len, oid objid[], size_t *objid_len, size_t *var_val_len, u_char *var_val_type, void **var_value) {
	u_char type;
	u_char val_type;
	size_t val_len;
	u_char *val;
	...
	if(debug_smux) {
		...
		zlog_debug("SMUX var parse: type must be %d", (ASN_SEQUENCE | ASN_CONSTRUCTOR));
	}
	...
	if(debug_smux) {
		switch(val_type) {
         ...
         ASN_NULL. */
			...
			case ASN_INTEGER: zlog_debug("ASN_INTEGER"); break;
			case ASN_COUNTER:
			case ASN_GAUGE:
			case ASN_TIMETICKS:
			case ASN_UINTEGER: zlog_debug("ASN_COUNTER"); break;
			case ASN_COUNTER64: zlog_debug("ASN_COUNTER64"); break;
			case ASN_IPADDRESS: zlog_debug("ASN_IPADDRESS"); break;
			case ASN_OCTET_STR: zlog_debug("ASN_OCTET_STR"); break;
			case ASN_OPAQUE:
			case ASN_NSAP:
			case ASN_OBJECT_ID: zlog_debug("ASN_OPAQUE"); break;
			case SNMP_NOSUCHOBJECT: zlog_debug("SNMP_NOSUCHOBJECT"); break;
			case SNMP_NOSUCHINSTANCE: zlog_debug("SNMP_NOSUCHINSTANCE"); break;
			case SNMP_ENDOFMIBVIEW: zlog_debug("SNMP_ENDOFMIBVIEW"); break;
			case ASN_BIT_STR: zlog_debug("ASN_BIT_STR"); break;
			default: zlog_debug("Unknown type"); break;
		}
	}
	return ptr;
}
...
static int smux_set(oid *reqid, size_t *reqid_len, u_char val_type, void *val, size_t val_len, int action) {
	int j;
	...
	for(ALL_LIST_ELEMENTS(treelist, node, nnode, subtree)) {
		subresult = oid_compare_part(reqid, *reqid_len, subtree->name, subtree->name_len);
		...
		if(subresult == 0) {
			...
			for(j = 0; j < subtree->variables_num; j++) {
				...
				/* Always check suffix */
				result = oid_compare_part(suffix, suffix_len, v->name, v->namelen);
				...
				if(result == 0) {
					...
					statP = (*v->findVar)(v, suffix, &suffix_len, 1, &val_len, &write_method);
...
static int smux_get(oid *reqid, size_t *reqid_len, int exact, u_char *val_type, void **val, size_t *val_len) {
	int j;
	struct subtree *subtree;
	struct variable *v;
	int subresult;
		...
		if(subresult == 0) {
			...
			for(j = 0; j < subtree->variables_num; j++) {
				...
				if(result == 0) {
					...
					*val = (*v->findVar)(v, suffix, &suffix_len, exact, val_len, &write_method);
...
static int smux_getnext(oid *reqid, size_t *reqid_len, int exact, u_char *val_type, void **val, size_t *val_len) {
	int j;
...
static void smux_parse_set(u_char *ptr, size_t len, int action) {
	...
	if(debug_smux) {
		zlog_debug("SMUX SET(%s) message parse: len %zd", (RESERVE1 == action) ? "RESERVE1" : ((FREE == action) ? "FREE" : "COMMIT"), len);
	}
...
process_rest: /* see note below: YYY */
	...
	switch(type) {
		...
		case SMUX_SET:
			...
			break;
		default: zlog_info("Unknown type: %d", type); break;
	}
	return 0;
}
...
static int smux_open(int sock) {
	...
	/* SMUX Open. */
	version = 0;
	ptr = asn_build_int(ptr, &len, (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_INTEGER), &version, sizeof(version));

	/* SMUX connection oid. */
	ptr = asn_build_objid(ptr, &len, (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OBJECT_ID), smux_oid, smux_oid_len);

	/* SMUX connection description. */
	ptr = asn_build_string(ptr, &len, (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), (const u_char *) progname, strlen(progname));

	/* SMUX connection password. */
	ptr = asn_build_string(ptr, &len, (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OCTET_STR), (u_char *) smux_passwd, strlen(smux_passwd));
...
int smux_trap(struct variable *vp, size_t vp_len, const oid *ename, size_t enamelen, const oid *name, size_t namelen, const oid *iname, size_t inamelen, const struct trap_object *trapobj, size_t trapobjlen, u_char sptrap) {
	unsigned int i;
	u_char buf[BUFSIZ];
	u_char *ptr;
	size_t len, length;
	...
	/* IP address. */
	addr.s_addr = 0;
	ptr = asn_build_string(ptr, &len, (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_IPADDRESS), (u_char *) &addr, sizeof(addr));

	/* Generic trap integer. */
	val = SNMP_TRAP_ENTERPRISESPECIFIC;
	ptr = asn_build_int(ptr, &len, (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_INTEGER), (long *) &val, sizeof(val));
	...
	/* Timeticks timestamp. */
	val = 0;
	ptr = asn_build_unsigned_int(ptr, &len, (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_TIMETICKS), &val, sizeof(val));
	...
	for(i = 0; i < trapobjlen; i++) {
		...
		if(debug_smux) {
			...
			if(trapobj[i].namelen < 0) {
				smux_oid_dump("Trap", trapobj[i].name, (-1) * (trapobj[i].namelen));
			} else {
		...
		if(ret == 0) {
			ptr = snmp_build_var_op(ptr, oid, &oid_len, val_type, val_len, val, &len);
		}
	}

	/* Now variable size is known, fill in size */
	asn_build_sequence(h1, &length, (u_char) (ASN_SEQUENCE | ASN_CONSTRUCTOR), ptr - h1e);
...
static int smux_register(int sock) {
	...
	for(ALL_LIST_ELEMENTS(treelist, node, nnode, subtree)) {
		...
		/* Register MIB tree. */
		ptr = asn_build_objid(ptr, &len, (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_OBJECT_ID), subtree->name, subtree->name_len);

		/* Priority. */
		priority = -1;
		ptr = asn_build_int(ptr, &len, (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_INTEGER), &priority, sizeof(priority));

		/* Operation. */
		operation = 2; /* Register R/W */
		ptr = asn_build_int(ptr, &len, (u_char) (ASN_UNIVERSAL | ASN_PRIMITIVE | ASN_INTEGER), &operation, sizeof(operation));
...
void smux_event(enum smux_event event, int sock) {
	switch(event) {
		case SMUX_SCHEDULE: smux_connect_thread = thread_add_event(smux_master, smux_connect, NULL, 0); break;
		case SMUX_CONNECT: smux_connect_thread = thread_add_timer(smux_master, smux_connect, NULL, 10); break;
		case SMUX_READ: smux_read_thread = thread_add_read(smux_master, smux_read, NULL, sock); break;
		default: break;
	}
}
...
static int smux_peer_oid(struct vty *vty, const char *oid_str, const char *passwd_str) {
	int ret;
	oid oid[MAX_OID_LEN];
	size_t oid_len;
...
DEFUN(smux_peer, smux_peer_cmd, "smux peer OID",
...
DEFUN(smux_peer_password, smux_peer_password_cmd, "smux peer OID PASSWORD",
      "SNMP MUX protocol settings\n"
      "SNMP MUX peer settings\n"
...
DEFUN(no_smux_peer, no_smux_peer_cmd, "no smux peer",
...
ALIAS(no_smux_peer, no_smux_peer_oid_cmd, "no smux peer OID",
      NO_STR "SNMP MUX protocol settings\n"
	     "SNMP MUX peer settings\n"
	     "SMUX peering object ID\n")

ALIAS(no_smux_peer, no_smux_peer_oid_password_cmd, "no smux peer OID PASSWORD",
      NO_STR "SNMP MUX protocol settings\n"
	     "SNMP MUX peer settings\n"
	     "SMUX peering object ID\n"
	     "SMUX peering password\n")
...
void smux_register_mib(const char *descr, struct variable *var, size_t width, int num, oid name[], size_t namelen) {
	struct subtree *tree;
...
static int smux_tree_cmp(struct subtree *tree1, struct subtree *tree2) {
	return oid_compare(tree1->name, tree1->name_len, tree2->name, tree2->name_len);
}
