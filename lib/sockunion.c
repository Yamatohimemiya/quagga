...
const char *inet_sutop(const union sockunion *su, char *str) {
	switch(su->sa.sa_family) {
		case AF_INET: inet_ntop(AF_INET, &su->sin.sin_addr, str, INET_ADDRSTRLEN); break;
#ifdef HAVE_IPV6
		case AF_INET6: inet_ntop(AF_INET6, &su->sin6.sin6_addr, str, INET6_ADDRSTRLEN); break;
#endif /* HAVE_IPV6 */
	}
	return str;
}
...
const char *sockunion2str(const union sockunion *su, char *buf, size_t len) {
	switch(sockunion_family(su)) {
		case AF_UNSPEC: snprintf(buf, len, "(unspec)"); return buf;
		case AF_INET: return inet_ntop(AF_INET, &su->sin.sin_addr, buf, len);
#ifdef HAVE_IPV6
		case AF_INET6: return inet_ntop(AF_INET6, &su->sin6.sin6_addr, buf, len);
#endif /* HAVE_IPV6 */
	}
...
static int sockunion_sizeof(const union sockunion *su) {
	...
	ret = 0;
	switch(su->sa.sa_family) {
		case AF_INET: ret = sizeof(struct sockaddr_in); break;
#ifdef HAVE_IPV6
		case AF_INET6: ret = sizeof(struct sockaddr_in6); break;
#endif /* AF_INET6 */
	}
	return ret;
}
...
#ifdef HAVE_IPV6
		case AF_INET6: return inet_ntop(AF_INET6, &(su->sin6.sin6_addr), buf, len); break;
#endif /* HAVE_IPV6 */

		default: snprintf(buf, len, "af_unknown %d ", su->sa.sa_family); return buf;
	}
}
...
enum connect_result sockunion_connect(int fd, const union sockunion *peersu, unsigned short port, ifindex_t ifindex) {
	int ret;
	int val;
	union sockunion su;
	...
	switch(su.sa.sa_family) {
		case AF_INET: su.sin.sin_port = port; break;
#ifdef HAVE_IPV6
		case AF_INET6: su.sin6.sin6_port = port;
	#ifdef KAME
	...
	if(ret < 0) {
		if(errno != EINPROGRESS) {
			...
			zlog_info("can't connect to %s fd %d : %s", sockunion_log(&su, str, sizeof str), fd, safe_strerror(errno));
...
#ifdef HAVE_IPV6
	if(family == AF_INET6) {
		ret = setsockopt(sock, IPPROTO_IPV6, IPV6_UNICAST_HOPS, (void *) &ttl, sizeof(int));
		if(ret < 0) {
			zlog(NULL, LOG_WARNING, "can't set sockopt IPV6_UNICAST_HOPS %d to socket %d", ttl, sock);
			return -1;
		}
		return 0;
	}
...
int sockopt_minttl(int family, int sock, int minttl) {
	...
	if(family == AF_INET6) {
		...
		if(ret < 0) {
			zlog(NULL, LOG_WARNING, "can't set sockopt IPV6_MINHOPCNT to %d on socket %d: %s", minttl, sock, safe_strerror(errno));
		}
		return ret;
	}
#endif
...
int sockunion_same(const union sockunion *su1, const union sockunion *su2) {
	...
	switch(su1->sa.sa_family) {
		case AF_INET: ret = memcmp(&su1->sin.sin_addr, &su2->sin.sin_addr, sizeof(struct in_addr)); break;
#ifdef HAVE_IPV6
		case AF_INET6: ret = memcmp(&su1->sin6.sin6_addr, &su2->sin6.sin6_addr, sizeof(struct in6_addr)); break;
#endif /* HAVE_IPV6 */
	}
	if(ret == 0) {
		return 1;
...
unsigned int sockunion_hash(const union sockunion *su) {
	switch(sockunion_family(su)) {
		case AF_INET: return jhash_1word(su->sin.sin_addr.s_addr, 0);
#ifdef HAVE_IPV6
		case AF_INET6: return jhash2(su->sin6.sin6_addr.s6_addr32, ZEBRA_NUM_OF(su->sin6.sin6_addr.s6_addr32), 0);
#endif /* HAVE_IPV6 */
	}
	return 0;
}
...
const u_char *sockunion_get_addr(const union sockunion *su) {
	switch(sockunion_family(su)) {
		case AF_INET: return (const u_char *) &su->sin.sin_addr.s_addr;
#ifdef HAVE_IPV6
		case AF_INET6: return (const u_char *) &su->sin6.sin6_addr;
#endif /* HAVE_IPV6 */
	}
	return NULL;
}

unsigned short sockunion_get_port(const union sockunion *su) {
	switch(sockunion_family(su)) {
...
void sockunion_set(union sockunion *su, int family, const u_char *addr, size_t bytes) {
	...
	switch(family) {
		case AF_INET: memcpy(&su->sin.sin_addr.s_addr, addr, bytes); break;
#ifdef HAVE_IPV6
		case AF_INET6: memcpy(&su->sin6.sin6_addr, addr, bytes); break;
#endif /* HAVE_IPV6 */
	}
}
...
union sockunion *sockunion_getsockname(int fd) {
	...
	if(ret < 0) {
		zlog_warn("Can't get local address and port by getsockname: %s", safe_strerror(errno));
		return NULL;
	}
...
union sockunion *sockunion_getpeername(int fd) {
	...
	if(ret < 0) {
		zlog(NULL, LOG_WARNING, "Can't get remote address and port: %s", safe_strerror(errno));
		return NULL;
	}
...
static void __attribute__((unused)) sockunion_print(const union sockunion *su) {
	...
	switch(su->sa.sa_family) {
		case AF_INET: printf("%s\n", inet_ntoa(su->sin.sin_addr)); break;
#ifdef HAVE_IPV6
		case AF_INET6:
			{
		...
		case AF_LINK:
			...
			break;
#endif /* AF_LINK */
		default: printf("af_unknown %d\n", su->sa.sa_family); break;
	}
}

#ifdef HAVE_IPV6
static int in6addr_cmp(const struct in6_addr *addr1, const struct in6_addr *addr2) {
	unsigned int i;
	u_char *p1, *p2;
