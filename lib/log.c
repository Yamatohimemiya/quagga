...
const char *zlog_proto_names[] = {
	"NONE", "DEFAULT", "ZEBRA", "RIP", "BGP", "OSPF", "RIPNG", "BABEL", "OSPF6", "ISIS", "PIM", "MASC", "NHRP", NULL,
};

const char *zlog_priority[] = {
	"emergencies", "alerts", "critical", "errors", "warnings", "notifications", "informational", "debugging", NULL,
};
...
size_t quagga_timestamp(int timestamp_precision, char *buf, size_t buflen) {
	...
	if(buflen > cache.len) {
		...
		if((timestamp_precision > 0) && (buflen > cache.len + 1 + timestamp_precision)) {
...
static void vzlog(struct zlog *zl, int priority, const char *format, va_list args) {
	...
	if(priority <= zl->maxlvl[ZLOG_DEST_MONITOR]) {
		vty_log((zl->record_priority ? zlog_priority[priority] : NULL), zlog_proto_names[zl->protocol], format, &tsctl, args);
	}
...
#undef LOC
	...
	return open(CRASHLOG_PREFIX CRASHLOG_SUFFIX, O_WRONLY | O_CREAT | O_EXCL, LOGFILE_MASK);
...
void zlog_backtrace_sigsafe(int priority, void *program_counter) {
	...
	if(size <= 0 || (size_t) size > array_size(array)) {
		...
		#define DUMP(FD) \
			{ \
				if(program_counter) write((FD), pclabel, sizeof(pclabel) - 1); \
				write((FD), buf, s - buf); \
				printstack((FD)); \
			}
...
#define PLOG_FUNC(FUNCNAME, PRIORITY) \
	void FUNCNAME(struct zlog *zl, const char *format, ...) { \
		va_list args; \
		va_start(args, format); \
		vzlog(zl, PRIORITY, format, args); \
		va_end(args); \
...
void zlog_thread_info(int log_level) {
	if(thread_current) {
		zlog(NULL, log_level,
		     ...
		     "file %s, line %u",
		     thread_current->funcname, thread_current->schedfrom, thread_current->schedfrom_line);
	} else {
		zlog(NULL, log_level, "Current thread not known/applicable");
	}
}

void _zlog_assert_failed(const char *assertion, const char *file, unsigned int line, const char *function) {
	/* Force fallback file logging? */
	if(zlog_default && !zlog_default->fp && ((logfile_fd = open_crashlog()) >= 0) && ((zlog_default->fp = fdopen(logfile_fd, "w")) != NULL)) {
		zlog_default->maxlvl[ZLOG_DEST_FILE] = LOG_ERR;
	}
	zlog(NULL, LOG_CRIT, "Assertion `%s' failed in file %s, line %u, function %s", assertion, file, line, (function ? function : "?"));
...
/* Open log stream */
struct zlog *openzlog(const char *progname, zlog_proto_t protocol, int syslog_flags, int syslog_facility) {
	struct zlog *zl;
	u_int i;
...
int zlog_rotate(struct zlog *zl) {
	...
	if(zl->filename) {
		...
		umask(oldumask);
		if(zl->fp == NULL) {
			zlog_err("Log rotate failed: cannot open file %s for append: %s", zl->filename, safe_strerror(save_errno));
			return -1;
		}
 ...
 * provided otherwise.
 */
const char *mes_lookup(const struct message *meslist, int max, int index, const char *none, const char *mesname) {
	...
	/* fall back to linear search */
	{
		...
		for(i = 0; i < max; i++, meslist++) {
			if(meslist->key == index) {
				...
				zlog_debug("message index %d [%s] found in %s at position %d (max is %d)", index, str, mesname, i, max);
				return str;
			}
		}
	}
