...
int quagga_signal_timer(struct thread *t) {
	...
	sigm = THREAD_ARG(t);
	sigm->t = thread_add_timer(sigm->t->master, quagga_signal_timer, &sigmaster, QUAGGA_SIGNAL_TIMER_INTERVAL);
...
static void __attribute__((noreturn)) exit_handler(
	int signo
#ifdef SA_SIGINFO
	,
...
static void __attribute__((noreturn)) core_handler(
	int signo
#ifdef SA_SIGINFO
	,
	siginfo_t *siginfo, void *context
...
static void trap_default_signals(void) {
	...
	for(i = 0; i < array_size(sigmap); i++) {
		...
		for(j = 0; j < sigmap[i].nsigs; j++) {
			...
			if((sigaction(sigmap[i].sigs[j], NULL, &oact) == 0) && (oact.sa_handler == SIG_DFL)) {
				...
				if(sigaction(sigmap[i].sigs[j], &act, NULL) < 0) {
					zlog_warn("Unable to set signal handler for signal %d: %s", sigmap[i].sigs[j], safe_strerror(errno));
				}
			}
		}
	}
...
void signal_init(struct thread_master *m, int sigc, struct quagga_signal_t signals[]) {
	int i = 0;
	struct quagga_signal_t *sig;
...
#ifdef SIGEVENT_SCHEDULE_THREAD
	sigmaster.t = thread_add_timer(m, quagga_signal_timer, &sigmaster, QUAGGA_SIGNAL_TIMER_INTERVAL);
#endif /* SIGEVENT_SCHEDULE_THREAD */
}
