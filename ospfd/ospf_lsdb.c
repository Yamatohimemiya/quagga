...
void ospf_lsdb_delete(struct ospf_lsdb *lsdb, struct ospf_lsa *lsa) {
	...
	if(!lsdb) {
		...
		if(lsa) {
			zlog_warn("LSA[Type%d:%s]: LSA %p, lsa->lsdb %p", lsa->data->type, inet_ntoa(lsa->data->id), (void *) lsa, (void *) lsa->lsdb);
		}
		return;
	}
...
struct ospf_lsa *ospf_lsdb_lookup_by_id(struct ospf_lsdb *lsdb, u_char type, struct in_addr id, struct in_addr adv_router) {
	struct route_table *table;
	struct prefix_ls lp;
	struct route_node *rn;
	struct ospf_lsa *find;
...
struct ospf_lsa *ospf_lsdb_lookup_by_id_next(struct ospf_lsdb *lsdb, u_char type, struct in_addr id, struct in_addr adv_router, int first) {
	struct route_table *table;
	struct prefix_ls lp;
	struct route_node *rn;
	struct ospf_lsa *find;
