...
#define OSPF_LSA_UPDATE_TIMER_ON(T, F) \
	if(!(T)) (T) = thread_add_timer(master, (F), 0, 2)
...
extern void lsa_header_set(struct stream *, u_char, u_char, struct in_addr, struct in_addr);
...
extern struct ospf_lsa *ospf_summary_lsa_originate(struct prefix_ipv4 *, u_int32_t, struct ospf_area *);
extern struct ospf_lsa *ospf_summary_asbr_lsa_originate(struct prefix_ipv4 *, u_int32_t, struct ospf_area *);

extern struct ospf_lsa *ospf_lsa_install(struct ospf *, struct ospf_interface *, struct ospf_lsa *);

extern void ospf_nssa_lsa_flush(struct ospf *ospf, struct prefix_ipv4 *p);
extern void ospf_external_lsa_flush(struct ospf *, u_char, struct prefix_ipv4 *, ifindex_t /* , struct in_addr nexthop */);
...
extern struct ospf_lsa *ospf_lsa_lookup(struct ospf_area *, u_int32_t, struct in_addr, struct in_addr);
extern struct ospf_lsa *ospf_lsa_lookup_by_id(struct ospf_area *, u_int32_t, struct in_addr);
extern struct ospf_lsa *ospf_lsa_lookup_by_header(struct ospf_area *, struct lsa_header *);
...
extern struct ospf_lsa *ospf_lsa_lookup_by_prefix(struct ospf_lsdb *, u_char, struct prefix_ipv4 *, struct in_addr);
...
extern struct ospf_lsa *ospf_external_lsa_refresh(struct ospf *, struct ospf_lsa *, struct external_info *, int);
extern struct in_addr ospf_lsa_unique_id(struct ospf *, struct ospf_lsdb *, u_char, struct prefix_ipv4 *);
...
extern struct ospf_lsa *ospf_translated_nssa_refresh(struct ospf *, struct ospf_lsa *, struct ospf_lsa *);
