...
int ospf_mpls_te_init(void) {
	...
	rc = ospf_register_opaque_functab(
		OSPF_OPAQUE_AREA_LSA, OPAQUE_TYPE_TRAFFIC_ENGINEERING_LSA, ospf_mpls_te_new_if, ospf_mpls_te_del_if, ospf_mpls_te_ism_change, ospf_mpls_te_nsm_change, ospf_mpls_te_config_write_router, NULL, /*ospf_mpls_te_config_write_if */
		NULL,											 /* ospf_mpls_te_config_write_debug */
		ospf_mpls_te_show_info, ospf_mpls_te_lsa_originate_area, ospf_mpls_te_lsa_refresh, NULL, /* ospf_mpls_te_new_lsa_hook */
...
static int ospf_mpls_te_register(enum inter_as_mode mode) {
	...
	rc = ospf_register_opaque_functab(scope, OPAQUE_TYPE_INTER_AS_LSA, NULL, NULL, NULL, NULL, NULL, NULL, NULL, ospf_mpls_te_show_info, ospf_mpls_te_lsa_originate_as, ospf_mpls_te_lsa_refresh, NULL, NULL);
...
void ospf_mpls_te_term(void) {
	...
	ospf_delete_opaque_functab(OSPF_OPAQUE_AREA_LSA, OPAQUE_TYPE_TRAFFIC_ENGINEERING_LSA);
	OspfMplsTE.status = disabled;
...
static struct ospf_interface *lookup_oi_by_ifp(struct interface *ifp, struct ospf_area *area, enum oifstate oifstate) {
	...
	for(rn = route_top(IF_OIFS(ifp)); rn; rn = route_next(rn)) {
		...
		switch(oifstate) {
			...
			case OI_UP:
				...
				break;
			default: zlog_warn("lookup_oi_by_ifp: Unknown oifstate: %x", oifstate); goto out;
		}
...
static void ospf_mpls_te_foreach_area(void (*func)(struct mpls_te_link *lp, opcode_t sched_opcode), opcode_t sched_opcode) {
	struct listnode *node, *nnode;
...
static void set_linkparams_link_type(struct ospf_interface *oi, struct mpls_te_link *lp) {
	lp->link_type.header.type = htons(TE_LINK_SUBTLV_LINK_TYPE);
	lp->link_type.header.length = htons(TE_LINK_SUBTLV_TYPE_SIZE);

	switch(oi->type) {
		case OSPF_IFTYPE_POINTOPOINT: lp->link_type.link_type.value = LINK_TYPE_SUBTLV_VALUE_PTP; break;
		case OSPF_IFTYPE_BROADCAST:
		case OSPF_IFTYPE_NBMA: lp->link_type.link_type.value = LINK_TYPE_SUBTLV_VALUE_MA; break;
		default:
...
static void set_linkparams_link_id(struct ospf_interface *oi, struct mpls_te_link *lp) {
...
static void set_linkparams_lclif_ipaddr(struct mpls_te_link *lp, struct in_addr lclif) {
...
static void set_linkparams_rmtif_ipaddr(struct mpls_te_link *lp, struct in_addr rmtif) {
...
static void set_linkparams_te_metric(struct mpls_te_link *lp, u_int32_t te_metric) {
...
static void set_linkparams_unrsv_bw(struct mpls_te_link *lp, int priority, float fp) {
...
static void set_linkparams_rsc_clsclr(struct mpls_te_link *lp, u_int32_t classcolor) {
...
static void set_linkparams_inter_as(struct mpls_te_link *lp, struct in_addr addr, u_int32_t as) {
...
void set_linkparams_llri(struct mpls_te_link *lp, u_int32_t local, u_int32_t remote) {
...
void set_linkparams_lrrid(struct mpls_te_link *lp, struct in_addr local, struct in_addr remote) {
...
static void set_linkparams_av_delay(struct mpls_te_link *lp, u_int32_t delay, u_char anormal) {
	u_int32_t tmp;
	/* Note that TLV-length field is the size of array. */
...
static void set_linkparams_mm_delay(struct mpls_te_link *lp, u_int32_t low, u_int32_t high, u_char anormal) {
	u_int32_t tmp;
	/* Note that TLV-length field is the size of array. */
...
static void set_linkparams_pkt_loss(struct mpls_te_link *lp, u_int32_t loss, u_char anormal) {
	u_int32_t tmp;
	/* Note that TLV-length field is the size of array. */
...
static void update_linkparams(struct mpls_te_link *lp) {
	...
	if(IS_PARAM_SET(ifp->link_params, LP_RMT_AS)) {
		...
		if(IS_STD_TE(lp->type) && CHECK_FLAG(lp->flags, LPFLG_LSA_ENGAGED)) {
			if(IS_DEBUG_OSPF_TE) {
				zlog_debug("OSPF MPLS-TE Update IF: Switch from Standard LSA to INTER-AS for %s[%d/%d]", ifp->name, lp->flags, lp->type);
			}
		...
		if(IS_DEBUG_OSPF_TE) {
			zlog_debug("OSPF MPLS-TE Update IF: Switch from INTER-AS LSA to Standard for %s[%d/%d]", ifp->name, lp->flags, lp->type);
		}
...
static void initialize_linkparams(struct mpls_te_link *lp) {
	...
	if(IS_DEBUG_OSPF_TE) {
		zlog_debug("MPLS-TE(initialize_linkparams) Initialize Link Parameters for interface %s", ifp->name);
	}

	if((oi = lookup_oi_by_ifp(ifp, NULL, OI_ANY)) == NULL) {
		zlog_warn("MPLS-TE(initialize_linkparams) Could not find corresponding OSPF Interface for %s", ifp->name);
		return;
	}
...
static int ospf_mpls_te_new_if(struct interface *ifp) {
	...
	if(IS_DEBUG_OSPF_TE) {
		zlog_debug("MPLS-TE(ospf_mpls_te_new_if) Add new %s interface %s to MPLS-TE list", ifp->link_params ? "Active" : "Inactive", ifp->name);
	}
	...
	if(IS_DEBUG_OSPF_TE) {
		zlog_debug("OSPF MPLS-TE New IF: Add new LP context for %s[%d/%d]", ifp->name, new->flags, new->type);
	}
...
void ospf_mpls_te_update_if(struct interface *ifp) {
	...
	if(IS_DEBUG_OSPF_TE) {
		zlog_debug("OSPF MPLS-TE: Update LSA parameters for interface %s [%s]", ifp->name, HAS_LINK_PARAMS(ifp) ? "ON" : "OFF");
	}
...
static void ospf_mpls_te_ism_change(struct ospf_interface *oi, int old_state) {
...
#ifdef notyet
	if((lp->area != NULL && !IPV4_ADDR_SAME(&lp->area->area_id, &oi->area->area_id)) || (lp->area != NULL && oi->area == NULL)) {
	...
	switch(oi->state) {
		...
		case ISM_DR:
			...
			/* Try to Schedule LSA */
			if((ntohs(old_type.header.type) != ntohs(lp->link_type.header.type) || old_type.link_type.value != lp->link_type.link_type.value)
			   || (ntohs(old_id.header.type) != ntohs(lp->link_id.header.type) || ntohl(old_id.value.s_addr) != ntohl(lp->link_id.value.s_addr))) {
				if CHECK_FLAG(lp->flags, LPFLG_LSA_ENGAGED) {
					ospf_mpls_te_lsa_schedule(lp, REFRESH_THIS_LSA);
				} else {
					ospf_mpls_te_lsa_schedule(lp, REORIGINATE_THIS_LSA);
...
static struct ospf_lsa *ospf_mpls_te_lsa_new(struct ospf_area *area, struct mpls_te_link *lp) {
	struct stream *s;
...
static int ospf_mpls_te_lsa_originate1(struct ospf_area *area, struct mpls_te_link *lp) {
	struct ospf_lsa *new;
	int rc = -1;
...
static int ospf_mpls_te_lsa_originate_area(void *arg) {
	...
	for(ALL_LIST_ELEMENTS(OspfMplsTE.iflist, node, nnode, lp)) {
		...
		if(!is_mandated_params_set(lp)) {
			zlog_warn("ospf_mpls_te_lsa_originate_area: Link(%s) lacks some mandated MPLS-TE parameters.", lp->ifp ? lp->ifp->name : "?");
			continue;
		}

		/* Ok, let's try to originate an LSA for this area and Link. */
		if(IS_DEBUG_OSPF_TE) {
			zlog_debug("MPLS-TE(ospf_mpls_te_lsa_originate_area) Let's finally reoriginate the LSA %d through the Area %s for Link %s", lp->instance, inet_ntoa(area->area_id), lp->ifp ? lp->ifp->name : "?");
		}
...
static int ospf_mpls_te_lsa_originate2(struct ospf *top, struct mpls_te_link *lp) {
	struct ospf_lsa *new;
	int rc = -1;
	...
	if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("LSA[Type%d:%s]: Originate Opaque-LSA/MPLS-TE Inter-AS", new->data->type, inet_ntoa(new->data->id));
		ospf_lsa_header_dump(new->data);
	}
...
static int ospf_mpls_te_lsa_originate_as(void *arg) {
	...
	if((OspfMplsTE.status == disabled) || (OspfMplsTE.inter_as == Disable)) {
		zlog_info("ospf_mpls_te_lsa_originate_as: MPLS-TE Inter-AS is disabled for now.");
		rc = 0; /* This is not an error case. */
		goto out;
	}
	...
	for(ALL_LIST_ELEMENTS(OspfMplsTE.iflist, node, nnode, lp)) {
		...
		if(!is_mandated_params_set(lp)) {
			zlog_warn("ospf_mpls_te_lsa_originate_as: Link(%s) lacks some mandated MPLS-TE parameters.", lp->ifp ? lp->ifp->name : "?");
			continue;
		}
		...
		if(IS_DEBUG_OSPF_TE) {
			zlog_debug(
				"MPLS-TE(ospf_mpls_te_lsa_originate_as) Let's finally re-originate the Inter-AS LSA %d through the %s for Link %s", lp->instance, IS_FLOOD_AS(lp->type) ? "AS" : "Area", lp->ifp ? lp->ifp->name : "Unknown"
			);
		}
...
static struct ospf_lsa *ospf_mpls_te_lsa_refresh(struct ospf_lsa *lsa) {
	...
	/* Debug logging. */
	if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("LSA[Type%d:%s]: Refresh Opaque-LSA/MPLS-TE", new->data->type, inet_ntoa(new->data->id));
		ospf_lsa_header_dump(new->data);
	}
...
void ospf_mpls_te_lsa_schedule(struct mpls_te_link *lp, opcode_t opcode) {
	...
	if(!(CHECK_FLAG(lp->flags, LPFLG_LSA_ACTIVE)) || !(IS_FLOOD_AREA(lp->type) || IS_FLOOD_AS(lp->type))) {
		return;
	}
	...
	switch(opcode) {
		case REORIGINATE_THIS_LSA:
			if(IS_FLOOD_AS(lp->type)) {
				ospf_opaque_lsa_reoriginate_schedule((void *) top, OSPF_OPAQUE_AS_LSA, OPAQUE_TYPE_INTER_AS_LSA);
				break;
			}

			if(IS_FLOOD_AREA(lp->type)) {
				if(IS_INTER_AS(lp->type)) {
					ospf_opaque_lsa_reoriginate_schedule((void *) lp->area, OSPF_OPAQUE_AREA_LSA, OPAQUE_TYPE_INTER_AS_LSA);
				} else {
					ospf_opaque_lsa_reoriginate_schedule((void *) lp->area, OSPF_OPAQUE_AREA_LSA, OPAQUE_TYPE_TRAFFIC_ENGINEERING_LSA);
				}
				break;
			}
			break;
		case REFRESH_THIS_LSA: ospf_opaque_lsa_refresh_schedule(&lsa); break;
		case FLUSH_THIS_LSA:
			/* Reset Activity flag */
			lp->flags = LPFLG_LSA_INACTIVE;
			ospf_opaque_lsa_flush_schedule(&lsa);
			break;
		default: zlog_warn("ospf_mpls_te_lsa_schedule: Unknown opcode (%u)", opcode); break;
	}
...
static u_int16_t show_vty_router_addr(struct vty *vty, struct te_tlv_header *tlvh) {
...
static u_int16_t show_vty_link_header(struct vty *vty, struct te_tlv_header *tlvh) {
...
static u_int16_t show_vty_link_subtlv_link_type(struct vty *vty, struct te_tlv_header *tlvh) {
	struct te_link_subtlv_link_type *top;
	const char *cp = "Unknown";

	top = (struct te_link_subtlv_link_type *) tlvh;
	switch(top->link_type.value) {
		case LINK_TYPE_SUBTLV_VALUE_PTP: cp = "Point-to-point"; break;
		case LINK_TYPE_SUBTLV_VALUE_MA: cp = "Multiaccess"; break;
		default: break;
	}
...
static u_int16_t show_vty_link_subtlv_link_id(struct vty *vty, struct te_tlv_header *tlvh) {
...
static u_int16_t show_vty_link_subtlv_lclif_ipaddr(struct vty *vty, struct te_tlv_header *tlvh) {
...
static u_int16_t show_vty_link_subtlv_rmtif_ipaddr(struct vty *vty, struct te_tlv_header *tlvh) {
...
static u_int16_t show_vty_link_subtlv_te_metric(struct vty *vty, struct te_tlv_header *tlvh) {
...
static u_int16_t show_vty_link_subtlv_max_bw(struct vty *vty, struct te_tlv_header *tlvh) {
...
static u_int16_t show_vty_link_subtlv_max_rsv_bw(struct vty *vty, struct te_tlv_header *tlvh) {
...
static u_int16_t show_vty_link_subtlv_unrsv_bw(struct vty *vty, struct te_tlv_header *tlvh) {
...
static u_int16_t show_vty_link_subtlv_rsc_clsclr(struct vty *vty, struct te_tlv_header *tlvh) {
...
static u_int16_t show_vty_link_subtlv_lrrid(struct vty *vty, struct te_tlv_header *tlvh) {
	struct te_link_subtlv_lrrid *top;

	top = (struct te_link_subtlv_lrrid *) tlvh;

	if(vty != NULL) {
		vty_out(vty, "  Local  TE Router ID: %s%s", inet_ntoa(top->local), VTY_NEWLINE);
		vty_out(vty, "  Remote TE Router ID: %s%s", inet_ntoa(top->remote), VTY_NEWLINE);
	} else {
...
static u_int16_t show_vty_link_subtlv_llri(struct vty *vty, struct te_tlv_header *tlvh) {
	struct te_link_subtlv_llri *top;

	top = (struct te_link_subtlv_llri *) tlvh;

	if(vty != NULL) {
		vty_out(vty, "  Link Local  ID: %d%s", (u_int32_t) ntohl(top->local), VTY_NEWLINE);
		vty_out(vty, "  Link Remote ID: %d%s", (u_int32_t) ntohl(top->remote), VTY_NEWLINE);
	} else {
...
static u_int16_t show_vty_link_subtlv_rip(struct vty *vty, struct te_tlv_header *tlvh) {
	...
	if(vty != NULL) {
		...
		zlog_debug("    Inter-AS TE Remote ASBR IP address: %s", inet_ntoa(top->value));
	}

	return TLV_SIZE(tlvh);
}

static u_int16_t show_vty_link_subtlv_ras(struct vty *vty, struct te_tlv_header *tlvh) {
...
static u_int16_t show_vty_link_subtlv_av_delay(struct vty *vty, struct te_tlv_header *tlvh) {
	...
	if(vty != NULL) {
		vty_out(vty, "  %s Average Link Delay: %d (micro-sec)%s", anomalous ? "Anomalous" : "Normal", delay, VTY_NEWLINE);
	} else {
		zlog_debug("    %s Average Link Delay: %d (micro-sec)", anomalous ? "Anomalous" : "Normal", delay);
	}

	return TLV_SIZE(tlvh);
}

static u_int16_t show_vty_link_subtlv_mm_delay(struct vty *vty, struct te_tlv_header *tlvh) {
	...
	if(vty != NULL) {
		vty_out(vty, "  %s Min/Max Link Delay: %d/%d (micro-sec)%s", anomalous ? "Anomalous" : "Normal", low, high, VTY_NEWLINE);
	} else {
		zlog_debug("    %s Min/Max Link Delay: %d/%d (micro-sec)", anomalous ? "Anomalous" : "Normal", low, high);
	}

	return TLV_SIZE(tlvh);
}

static u_int16_t show_vty_link_subtlv_delay_var(struct vty *vty, struct te_tlv_header *tlvh) {
...
static u_int16_t show_vty_link_subtlv_pkt_loss(struct vty *vty, struct te_tlv_header *tlvh) {
	...
	if(vty != NULL) {
		vty_out(vty, "  %s Link Loss: %g (%%)%s", anomalous ? "Anomalous" : "Normal", fval, VTY_NEWLINE);
	} else {
		zlog_debug("    %s Link Loss: %g (%%)", anomalous ? "Anomalous" : "Normal", fval);
	}

	return TLV_SIZE(tlvh);
}

static u_int16_t show_vty_link_subtlv_res_bw(struct vty *vty, struct te_tlv_header *tlvh) {
...
static u_int16_t show_vty_link_subtlv_ava_bw(struct vty *vty, struct te_tlv_header *tlvh) {
...
static u_int16_t show_vty_link_subtlv_use_bw(struct vty *vty, struct te_tlv_header *tlvh) {
...
static u_int16_t show_vty_unknown_tlv(struct vty *vty, struct te_tlv_header *tlvh) {
	if(vty != NULL) {
...
static u_int16_t ospf_mpls_te_show_link_subtlv(struct vty *vty, struct te_tlv_header *tlvh0, u_int16_t subtotal, u_int16_t total) {
	...
	for(tlvh = tlvh0; sum < total; tlvh = (next ? next : TLV_HDR_NEXT(tlvh))) {
		...
		switch(ntohs(tlvh->type)) {
			case TE_LINK_SUBTLV_LINK_TYPE: sum += show_vty_link_subtlv_link_type(vty, tlvh); break;
			case TE_LINK_SUBTLV_LINK_ID: sum += show_vty_link_subtlv_link_id(vty, tlvh); break;
			case TE_LINK_SUBTLV_LCLIF_IPADDR: sum += show_vty_link_subtlv_lclif_ipaddr(vty, tlvh); break;
			case TE_LINK_SUBTLV_RMTIF_IPADDR: sum += show_vty_link_subtlv_rmtif_ipaddr(vty, tlvh); break;
			case TE_LINK_SUBTLV_TE_METRIC: sum += show_vty_link_subtlv_te_metric(vty, tlvh); break;
			case TE_LINK_SUBTLV_MAX_BW: sum += show_vty_link_subtlv_max_bw(vty, tlvh); break;
			case TE_LINK_SUBTLV_MAX_RSV_BW: sum += show_vty_link_subtlv_max_rsv_bw(vty, tlvh); break;
			case TE_LINK_SUBTLV_UNRSV_BW: sum += show_vty_link_subtlv_unrsv_bw(vty, tlvh); break;
			case TE_LINK_SUBTLV_RSC_CLSCLR: sum += show_vty_link_subtlv_rsc_clsclr(vty, tlvh); break;
			case TE_LINK_SUBTLV_LRRID: sum += show_vty_link_subtlv_lrrid(vty, tlvh); break;
			case TE_LINK_SUBTLV_LLRI: sum += show_vty_link_subtlv_llri(vty, tlvh); break;
			case TE_LINK_SUBTLV_RIP: sum += show_vty_link_subtlv_rip(vty, tlvh); break;
			case TE_LINK_SUBTLV_RAS: sum += show_vty_link_subtlv_ras(vty, tlvh); break;
			case TE_LINK_SUBTLV_AV_DELAY: sum += show_vty_link_subtlv_av_delay(vty, tlvh); break;
			case TE_LINK_SUBTLV_MM_DELAY: sum += show_vty_link_subtlv_mm_delay(vty, tlvh); break;
			case TE_LINK_SUBTLV_DELAY_VAR: sum += show_vty_link_subtlv_delay_var(vty, tlvh); break;
			case TE_LINK_SUBTLV_PKT_LOSS: sum += show_vty_link_subtlv_pkt_loss(vty, tlvh); break;
			case TE_LINK_SUBTLV_RES_BW: sum += show_vty_link_subtlv_res_bw(vty, tlvh); break;
			case TE_LINK_SUBTLV_AVA_BW: sum += show_vty_link_subtlv_ava_bw(vty, tlvh); break;
			case TE_LINK_SUBTLV_USE_BW: sum += show_vty_link_subtlv_use_bw(vty, tlvh); break;
...
static void ospf_mpls_te_show_info(struct vty *vty, struct ospf_lsa *lsa) {
	...
	struct te_tlv_header *tlvh, *next;
	u_int16_t sum, total;
	u_int16_t (*subfunc)(struct vty *vty, struct te_tlv_header *tlvh, u_int16_t subtotal, u_int16_t total) = NULL;

	sum = 0;
	total = ntohs(lsah->length) - OSPF_LSA_HEADER_SIZE;

	for(tlvh = TLV_HDR_TOP(lsah); sum < total; tlvh = (next ? next : TLV_HDR_NEXT(tlvh))) {
		if(subfunc != NULL) {
		...
		next = NULL;
		switch(ntohs(tlvh->type)) {
			case TE_TLV_ROUTER_ADDR: sum += show_vty_router_addr(vty, tlvh); break;
			case TE_TLV_LINK:
...
static void ospf_mpls_te_config_write_router(struct vty *vty) {
	if(OspfMplsTE.status == enabled) {
		...
		vty_out(vty, "  mpls-te router-address %s%s", inet_ntoa(OspfMplsTE.router_addr.value), VTY_NEWLINE);
	}
	...
	if(OspfMplsTE.inter_as == Area) {
		vty_out(vty, "  mpls-te inter-as area %s %s", inet_ntoa(OspfMplsTE.interas_areaid), VTY_NEWLINE);
	}
...
DEFUN(no_ospf_mpls_te, no_ospf_mpls_te_cmd, "no mpls-te", NO_STR "Disable the MPLS-TE functionality\n") {
	struct listnode *node, *nnode;
	struct mpls_te_link *lp;
...
DEFUN(ospf_mpls_te_router_addr, ospf_mpls_te_router_addr_cmd, "mpls-te router-address A.B.C.D",
...
static int set_inter_as_mode(struct vty *vty, const char *mode_name, const char *area_id) {
	...
	if(OspfMplsTE.status == enabled) {
		...
		if(strcmp(mode_name, "as") == 0) {
			...
			vty_out(vty, "Unknown mode. Please choose between as or area%s", VTY_NEWLINE);
			return CMD_WARNING;
		}

		if(IS_DEBUG_OSPF_EVENT) {
			zlog_debug("MPLS-TE: Inter-AS enable with %s flooding support", mode2text[mode]);
		}

		/* Register new callbacks regarding the flooding scope (AS or Area) */
		if(ospf_mpls_te_register(mode) < 0) {
			vty_out(vty, "Internal error: Unable to register Inter-AS functions%s", VTY_NEWLINE);
			return CMD_WARNING;
		}
		...
		if((OspfMplsTE.inter_as == Disable) && (mode != OspfMplsTE.inter_as)) {
			...
			vty_out(vty, "Please change Inter-AS support to disable first before going to mode %s%s", mode2text[mode], VTY_NEWLINE);
			return CMD_WARNING;
		}
	}
	return CMD_SUCCESS;
...
DEFUN(ospf_mpls_te_inter_as_as, ospf_mpls_te_inter_as_cmd, "mpls-te inter-as as",
      MPLS_TE_STR "Configure MPLS-TE Inter-AS support\n"
		  "AS native mode self originate INTER_AS LSA with Type 11 (as flooding scope)\n") {
	return set_inter_as_mode(vty, "as", "");
}

DEFUN(ospf_mpls_te_inter_as_area, ospf_mpls_te_inter_as_area_cmd, "mpls-te inter-as area (A.B.C.D|<0-4294967295>)",
      MPLS_TE_STR "Configure MPLS-TE Inter-AS support\n"
...
DEFUN(show_ip_ospf_mpls_te_router, show_ip_ospf_mpls_te_router_cmd, "show ip ospf mpls-te router",
      SHOW_STR IP_STR OSPF_STR "MPLS-TE information\n"
...
static void show_mpls_te_link_sub(struct vty *vty, struct interface *ifp) {
	...
	if((OspfMplsTE.status == enabled) && HAS_LINK_PARAMS(ifp) && !if_is_loopback(ifp) && if_is_up(ifp) && ((lp = lookup_linkparams_by_ifp(ifp)) != NULL)) {
		/* Continue only if interface is not passive or support Inter-AS TEv2 */
		if(!(ospf_oi_count(ifp) > 0)) {
			if(IS_INTER_AS(lp->type)) {
				vty_out(vty, "-- Inter-AS TEv2 link parameters for %s --%s", ifp->name, VTY_NEWLINE);
			} else {
				/* MPLS-TE is not activate on this interface */
				/* or this interface is passive and Inter-AS TEv2 is not activate */
				vty_out(vty, "  %s: MPLS-TE is disabled on this interface%s", ifp->name, VTY_NEWLINE);
				return;
			}
		} else {
			vty_out(vty, "-- MPLS-TE link parameters for %s --%s", ifp->name, VTY_NEWLINE);
		}
...
DEFUN(show_ip_ospf_mpls_te_link, show_ip_ospf_mpls_te_link_cmd, "show ip ospf mpls-te interface [INTERFACE]",
      SHOW_STR IP_STR OSPF_STR "MPLS-TE information\n"
