...
static void ospf_router_info_ism_change(struct ospf_interface *oi, int old_status);
static void ospf_router_info_nsm_change(struct ospf_neighbor *nbr, int old_status);
...
static int ospf_router_info_register(u_int8_t scope) {
	...
	zlog_info("Register Router Information with scope %s(%d)", scope == OSPF_OPAQUE_AREA_LSA ? "Area" : "AS", scope);
	rc = ospf_register_opaque_functab(
		scope, OPAQUE_TYPE_ROUTER_INFORMATION_LSA, NULL,						      /* new interface */
		NULL,												      /* del interface */
		ospf_router_info_ism_change, ospf_router_info_nsm_change, ospf_router_info_config_write_router, NULL, /* Config. write interface */
		NULL,												      /* Config. write debug */
		ospf_router_info_show_info, ospf_router_info_lsa_originate, ospf_router_info_lsa_refresh, NULL,	      /* new_lsa_hook */
		NULL
	); /* del_lsa_hook */
...
static int ospf_router_info_unregister() {
	if((OspfRI.scope != OSPF_OPAQUE_AS_LSA) && (OspfRI.scope != OSPF_OPAQUE_AREA_LSA)) {
		zlog_warn("Unable to unregister Router Info functions: Wrong scope!");
		return -1;
	}

	ospf_delete_opaque_functab(OspfRI.scope, OPAQUE_TYPE_ROUTER_INFORMATION_LSA);
...
static void set_router_info_capabilities(struct ri_tlv_router_cap *ric, u_int32_t cap) {
...
static void set_pce_domain(u_int16_t type, u_int32_t domain, struct ospf_pce_info *pce) {
	...
	new = XCALLOC(MTYPE_OSPF_PCE_PARAMS, sizeof(struct ri_pce_subtlv_domain));
...
static void unset_pce_domain(u_int16_t type, u_int32_t domain, struct ospf_pce_info *pce) {
	struct listnode *node;
...
static void set_pce_neighbor(u_int16_t type, u_int32_t domain, struct ospf_pce_info *pce) {
	...
	new = XCALLOC(MTYPE_OSPF_PCE_PARAMS, sizeof(struct ri_pce_subtlv_neighbor));
...
static void unset_pce_neighbor(u_int16_t type, u_int32_t domain, struct ospf_pce_info *pce) {
	struct listnode *node;
...
static void initialize_params(struct ospf_router_info *ori) {
   ...
   */
	...
	top = ospf_lookup();
	zlog_info("RI-> Initialize Router Info for %s scope within area %s", OspfRI.scope == OSPF_OPAQUE_AREA_LSA ? "Area" : "AS", inet_ntoa(OspfRI.area_id));
   ...
   */
	...
	/* PCE Capabilities */
	cap = PCE_CAP_BIDIRECTIONAL | PCE_CAP_DIVERSE_PATH | PCE_CAP_OBJECTIVES | PCE_CAP_ADDITIVE | PCE_CAP_MULTIPLE_REQ;
...
static int is_mandated_params_set(struct ospf_router_info ori) {
	...
	if((ntohs(ori.pce_info.pce_header.header.type) == RI_TLV_PCE) && (ntohs(ori.pce_info.pce_address.header.type) == 0) && (ntohs(ori.pce_info.pce_cap_flag.header.type) == 0)) {
		goto out;
	}
 ...
 *------------------------------------------------------------------------*/
static void ospf_router_info_ism_change(struct ospf_interface *oi, int old_state) {
	/* So far, nothing to do here. */
	return;
}

static void ospf_router_info_nsm_change(struct ospf_neighbor *nbr, int old_state) {
	/* So far, nothing to do here. */
	return;
}
...
static void ospf_router_info_lsa_body_set(struct stream *s) {
	...
	if((set_pce_header(&OspfRI.pce_info)) != 0) {
		...
		for(ALL_LIST_ELEMENTS_RO(OspfRI.pce_info.pce_neighbor, node, neighbor)) {
...
static struct ospf_lsa *ospf_router_info_lsa_new() {
	...
	if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("LSA[Type%d:%s]: Create an Opaque-LSA/ROUTER INFORMATION instance", lsa_type, inet_ntoa(lsa_id));
	}
...
static int ospf_router_info_lsa_originate1(void *arg) {
	...
	if(OspfRI.scope == OSPF_OPAQUE_AREA_LSA) {
		...
		if(area->area_id.s_addr != OspfRI.area_id.s_addr) {
			zlog_debug("RI -> This is not the Router Information Area. Stop processing");
			goto out;
		}
		OspfRI.area = area;
	}
	...
	if((new = ospf_router_info_lsa_new()) == NULL) {
		zlog_warn("ospf_router_info_lsa_originate1: ospf_router_info_lsa_new() ?");
		goto out;
	}
	...
	if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("LSA[Type%d:%s]: Originate Opaque-LSA/ROUTER INFORMATION", new->data->type, inet_ntoa(new->data->id));
...
static int ospf_router_info_lsa_originate(void *arg) {
	...
	if(OspfRI.status == disabled) {
		zlog_info("ospf_router_info_lsa_originate: ROUTER INFORMATION is disabled now.");
	...
	if(OspfRI.flags & RIFLG_LSA_ENGAGED) {
		...
		if(!is_mandated_params_set(OspfRI)) {
			zlog_warn("ospf_router_info_lsa_originate: lacks mandated ROUTER INFORMATION parameters");
		}
...
static struct ospf_lsa *ospf_router_info_lsa_refresh(struct ospf_lsa *lsa) {
	...
	if(OspfRI.status == disabled) {
       ...
       */
		zlog_info("ospf_router_info_lsa_refresh: ROUTER INFORMATION is disabled now.");
	...
	if(GET_OPAQUE_ID(ntohl(lsa->data->id.s_addr)) != 0) {
		zlog_warn("ospf_router_info_lsa_refresh: Unsupported Router Information ID");
		goto out;
	}
	...
	/* Create new Opaque-LSA/ROUTER INFORMATION instance. */
	if((new = ospf_router_info_lsa_new()) == NULL) {
		zlog_warn("ospf_router_info_lsa_refresh: ospf_router_info_lsa_new() ?");
		goto out;
	}
	...
	/* Debug logging. */
	if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("LSA[Type%d:%s]: Refresh Opaque-LSA/ROUTER INFORMATION", new->data->type, inet_ntoa(new->data->id));
		ospf_lsa_header_dump(new->data);
	}
...
static void ospf_router_info_lsa_schedule(opcode_t opcode) {
	...
	zlog_debug("RI-> LSA schedule %s%s%s", opcode == REORIGINATE_THIS_LSA ? "Re-Originate" : "", opcode == REFRESH_THIS_LSA ? "Refresh" : "", opcode == FLUSH_THIS_LSA ? "Flush" : "");

	top = ospf_lookup();
	if((OspfRI.scope == OSPF_OPAQUE_AREA_LSA) && (OspfRI.area == NULL)) {
		zlog_warn("ospf_router_info_lsa_schedule(): Router Info is Area scope flooding but area is not set");
		OspfRI.area = ospf_area_lookup_by_area_id(top, OspfRI.area_id);
	}
	...
	switch(opcode) {
		case REORIGINATE_THIS_LSA:
			if(OspfRI.scope == OSPF_OPAQUE_AREA_LSA) {
				ospf_opaque_lsa_reoriginate_schedule((void *) OspfRI.area, OSPF_OPAQUE_AREA_LSA, OPAQUE_TYPE_ROUTER_INFORMATION_LSA);
			} else {
				ospf_opaque_lsa_reoriginate_schedule((void *) top, OSPF_OPAQUE_AS_LSA, OPAQUE_TYPE_ROUTER_INFORMATION_LSA);
			}
			break;
		...
		case FLUSH_THIS_LSA:
			...
			ospf_opaque_lsa_flush_schedule(&lsa);
			break;
		default: zlog_warn("ospf_router_info_lsa_schedule: Unknown opcode (%u)", opcode); break;
	}
...
static u_int16_t show_vty_router_cap(struct vty *vty, struct ri_tlv_header *tlvh) {
...
static u_int16_t show_vty_pce_subtlv_address(struct vty *vty, struct ri_tlv_header *tlvh) {
	...
	if(ntohs(top->address.type) == PCE_ADDRESS_TYPE_IPV4) {
		...
		if(vty != NULL) {
			vty_out(vty, "  PCE Address: 0x%x%s", ntohl(top->address.value.s_addr), VTY_NEWLINE);
		} else {
			zlog_debug("    PCE Address: 0x%x", ntohl(top->address.value.s_addr));
		}
	}
...
static u_int16_t show_vty_pce_subtlv_path_scope(struct vty *vty, struct ri_tlv_header *tlvh) {
	struct ri_pce_subtlv_path_scope *top = (struct ri_pce_subtlv_path_scope *) tlvh;
...
static u_int16_t show_vty_pce_subtlv_domain(struct vty *vty, struct ri_tlv_header *tlvh) {
	...
	if(ntohs(top->type) == PCE_DOMAIN_TYPE_AREA) {
		...
		if(vty != NULL) {
			vty_out(vty, "  PCE domain Area: %s%s", inet_ntoa(tmp), VTY_NEWLINE);
		} else {
...
static u_int16_t show_vty_pce_subtlv_neighbor(struct vty *vty, struct ri_tlv_header *tlvh) {
	...
	if(ntohs(top->type) == PCE_DOMAIN_TYPE_AREA) {
		tmp.s_addr = top->value;
		if(vty != NULL) {
			vty_out(vty, "  PCE neighbor Area: %s%s", inet_ntoa(tmp), VTY_NEWLINE);
		} else {
...
static u_int16_t show_vty_pce_subtlv_cap_flag(struct vty *vty, struct ri_tlv_header *tlvh) {
...
static u_int16_t show_vty_unknown_tlv(struct vty *vty, struct ri_tlv_header *tlvh) {
	if(vty != NULL) {
...
static u_int16_t show_vty_pce_info(struct vty *vty, struct ri_tlv_header *ri, uint32_t total) {
	struct ri_tlv_header *tlvh;
	u_int16_t sum = 0;

	for(tlvh = ri; sum < total; tlvh = RI_TLV_HDR_NEXT(tlvh)) {
		switch(ntohs(tlvh->type)) {
			case RI_PCE_SUBTLV_ADDRESS: sum += show_vty_pce_subtlv_address(vty, tlvh); break;
			case RI_PCE_SUBTLV_PATH_SCOPE: sum += show_vty_pce_subtlv_path_scope(vty, tlvh); break;
			case RI_PCE_SUBTLV_DOMAIN: sum += show_vty_pce_subtlv_domain(vty, tlvh); break;
			case RI_PCE_SUBTLV_NEIGHBOR: sum += show_vty_pce_subtlv_neighbor(vty, tlvh); break;
			case RI_PCE_SUBTLV_CAP_FLAG: sum += show_vty_pce_subtlv_cap_flag(vty, tlvh); break;
...
static void ospf_router_info_show_info(struct vty *vty, struct ospf_lsa *lsa) {
	...
	for(tlvh = RI_TLV_HDR_TOP(lsah); sum < length; tlvh = RI_TLV_HDR_NEXT(tlvh)) {
		switch(ntohs(tlvh->type)) {
			case RI_TLV_CAPABILITIES: sum += show_vty_router_cap(vty, tlvh); break;
			case RI_TLV_PCE:
				tlvh++;
				sum += RI_TLV_HDR_SIZE;
...
static void ospf_router_info_config_write_router(struct vty *vty) {
	...
	if(OspfRI.status == enabled) {
		if(OspfRI.scope == OSPF_OPAQUE_AS_LSA) {
			...
			vty_out(vty, " router-info area %s%s", inet_ntoa(OspfRI.area_id), VTY_NEWLINE);
		}

		if(pce->pce_address.header.type != 0) {
			vty_out(vty, "  pce address %s%s", inet_ntoa(pce->pce_address.address.value), VTY_NEWLINE);
		}

		if(pce->pce_cap_flag.header.type != 0) {
			vty_out(vty, "  pce flag 0x%x%s", ntohl(pce->pce_cap_flag.value), VTY_NEWLINE);
		}
		...
		for(ALL_LIST_ELEMENTS_RO(pce->pce_domain, node, domain)) {
			if(domain->header.type != 0) {
				if(domain->type == PCE_DOMAIN_TYPE_AREA) {
					...
					vty_out(vty, "  pce domain as %d%s", ntohl(domain->value), VTY_NEWLINE);
				}
			}
		}
		...
		for(ALL_LIST_ELEMENTS_RO(pce->pce_neighbor, node, neighbor)) {
			if(neighbor->header.type != 0) {
				if(neighbor->type == PCE_DOMAIN_TYPE_AREA) {
					...
					vty_out(vty, "  pce neighbor as %d%s", ntohl(neighbor->value), VTY_NEWLINE);
				}
			}
		}

		if(pce->pce_scope.header.type != 0) {
			vty_out(vty, "  pce scope 0x%x%s", ntohl(OspfRI.pce_info.pce_scope.value), VTY_NEWLINE);
		}
	}
	return;
}
...
DEFUN(router_info, router_info_area_cmd, "router-info area A.B.C.D",
      OSPF_RI_STR "Enable the Router Information functionality with Area flooding scope\n"
	...
	if(argc == 1) {
		if(!inet_aton(argv[0], &OspfRI.area_id)) {
			vty_out(vty, "Please specify Router Info Area by A.B.C.D%s", VTY_NEWLINE);
			return CMD_WARNING;
		}
	...
	if(IS_DEBUG_OSPF_EVENT) {
		zlog_debug("RI-> Router Information (%s flooding): OFF -> ON", OspfRI.scope == OSPF_OPAQUE_AREA_LSA ? "Area" : "AS");
	}
...
ALIAS(router_info, router_info_as_cmd, "router-info as", OSPF_RI_STR "Enable the Router Information functionality with AS flooding scope\n")
...
DEFUN(pce_address, pce_address_cmd, "pce address A.B.C.D",
	...
	if(ntohs(pi->pce_address.header.type) == 0 || ntohl(pi->pce_address.address.value.s_addr) != ntohl(value.s_addr)) {
...
DEFUN(no_pce_address, no_pce_address_cmd, "no pce address", NO_STR PCE_STR "Disable PCE address\n") {
...
DEFUN(pce_path_scope, pce_path_scope_cmd, "pce scope BITPATTERN",
      PCE_STR "Path scope visibilities of the PCE for path computation\n"
	      "32-bit Hexadecimal value\n") {
	uint32_t scope;
	struct ospf_pce_info *pi = &OspfRI.pce_info;

	if(sscanf(argv[0], "0x%x", &scope) != 1) {
		vty_out(vty, "pce_path_scope: fscanf: %s%s", safe_strerror(errno), VTY_NEWLINE);
		return CMD_WARNING;
	}
...
DEFUN(no_pce_path_scope, no_pce_path_scope_cmd, "no pce scope", NO_STR PCE_STR "Disable PCE path scope\n") {
	unset_param(&OspfRI.pce_info.pce_address.header);
...
DEFUN(pce_domain, pce_domain_cmd, "pce domain as <0-65535>",
      PCE_STR "Configure PCE domain AS number\n"
	...
	if(sscanf(argv[0], "%d", &as) != 1) {
		vty_out(vty, "pce_domain: fscanf: %s%s", safe_strerror(errno), VTY_NEWLINE);
		return CMD_WARNING;
	}
...
DEFUN(no_pce_domain, no_pce_domain_cmd, "no pce domain as <0-65535>",
      NO_STR PCE_STR "Disable PCE domain AS number\n"
		     "AS number where the PCE as visibilities for path computation\n"
		     "AS number in decimal <0-65535>\n") {
	uint32_t as;
	struct ospf_pce_info *pce = &OspfRI.pce_info;
	...
	if(sscanf(argv[0], "%d", &as) != 1) {
		vty_out(vty, "no_pce_domain: fscanf: %s%s", safe_strerror(errno), VTY_NEWLINE);
		return CMD_WARNING;
	}
...
DEFUN(pce_neigbhor, pce_neighbor_cmd, "pce neighbor as <0-65535>",
      PCE_STR "Configure PCE neighbor domain AS number\n"
	...
	if(sscanf(argv[0], "%d", &as) != 1) {
		vty_out(vty, "pce_neighbor: fscanf: %s%s", safe_strerror(errno), VTY_NEWLINE);
		return CMD_WARNING;
	}
...
DEFUN(no_pce_neighbor, no_pce_neighbor_cmd, "no pce neighbor as <0-65535>",
      NO_STR PCE_STR "Disable PCE neighbor AS number\n"
	...
	if(sscanf(argv[0], "%d", &as) != 1) {
		vty_out(vty, "no_pce_neighbor: fscanf: %s%s", safe_strerror(errno), VTY_NEWLINE);
		return CMD_WARNING;
	}
...
DEFUN(pce_cap_flag, pce_cap_flag_cmd, "pce flag BITPATTERN",
      PCE_STR "Capabilities of the PCE for path computation\n"
	      "32-bit Hexadecimal value\n") {
	uint32_t cap;
	struct ospf_pce_info *pce = &OspfRI.pce_info;

	if(sscanf(argv[0], "0x%x", &cap) != 1) {
		vty_out(vty, "pce_cap_flag: fscanf: %s%s", safe_strerror(errno), VTY_NEWLINE);
		return CMD_WARNING;
	}

	if(ntohl(pce->pce_cap_flag.header.type) == 0 || cap != pce->pce_cap_flag.value) {
...
DEFUN(no_pce_cap_flag, no_pce_cap_flag_cmd, "no pce flag", NO_STR PCE_STR "Disable PCE capabilities\n") {
...
DEFUN(show_ip_ospf_router_info, show_ip_ospf_router_info_cmd, "show ip ospf router-info", SHOW_STR IP_STR OSPF_STR "Router Information\n") {
	if(OspfRI.status == enabled) {
...
DEFUN(show_ip_opsf_router_info_pce, show_ip_ospf_router_info_pce_cmd, "show ip ospf router-info pce",
