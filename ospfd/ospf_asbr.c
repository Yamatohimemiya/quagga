...
void ospf_external_route_remove(struct ospf *ospf, struct prefix_ipv4 *p) {
	...
	if(rn) {
		if((or = rn->info)) {
			zlog_info("Route[%s/%d]: external path deleted", inet_ntoa(p->prefix), p->prefixlen);
	...
	zlog_info("Route[%s/%d]: no such external path", inet_ntoa(p->prefix), p->prefixlen);
}
...
struct ospf_route *ospf_external_route_lookup(struct ospf *ospf, struct prefix_ipv4 *p) {
	...
	zlog_warn("Route[%s/%d]: lookup, no such prefix", inet_ntoa(p->prefix), p->prefixlen);

	return NULL;
}

/* Add an External info for AS-external-LSA. */
struct external_info *ospf_external_info_new(u_char type) {
...
int ospf_route_map_set_compare(struct route_map_set_values *values1, struct route_map_set_values *values2) {
	return values1->metric == values2->metric && values1->metric_type == values2->metric_type;
}

/* Add an External info for AS-external-LSA. */
struct external_info *ospf_external_info_add(u_char type, struct prefix_ipv4 p, ifindex_t ifindex, struct in_addr nexthop, route_tag_t tag) {
	struct external_info *new;
	struct route_node *rn;
	...
	if(rn) {
		if(rn->info) {
			route_unlock_node(rn);
			zlog_warn("Redistribute[%s]: %s/%d already exists, discard.", ospf_redist_string(type), inet_ntoa(p.prefix), p.prefixlen);
	...
	if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("Redistribute[%s]: %s/%d external info created.", ospf_redist_string(type), inet_ntoa(p.prefix), p.prefixlen);
	}
	return new;
}
...
struct external_info *ospf_external_info_lookup(u_char type, struct prefix_ipv4 *p) {
	struct route_node *rn;
	rn = route_node_lookup(EXTERNAL_INFO(type), (struct prefix *) p);
	if(rn) {
		route_unlock_node(rn);
...
struct ospf_lsa *ospf_external_info_find_lsa(struct ospf *ospf, struct prefix_ipv4 *p) {
	...
	lsa = ospf_lsdb_lookup_by_id(ospf->lsdb, OSPF_AS_EXTERNAL_LSA, p->prefix, ospf->router_id);
	...
	if(mask.s_addr != al->mask.s_addr) {
		...
		lsa = ospf_lsdb_lookup_by_id(ospf->lsdb, OSPF_AS_EXTERNAL_LSA, id, ospf->router_id);
		if(!lsa) {
			return NULL;
		}
	}
		...
		for(rn = route_top(EXTERNAL_INFO(type)); rn; rn = route_next(rn)) {
			if((ei = rn->info)) {
				if(ospf_external_info_find_lsa(ospf, &ei->p)) {
					if(is_prefix_default(&ei->p) && ospf->default_originate != DEFAULT_ORIGINATE_NONE) {
						continue;
					}
