...
#define IS_OPAQUE_LSA(type) ((type) == OSPF_OPAQUE_LINK_LSA || (type) == OSPF_OPAQUE_AREA_LSA || (type) == OSPF_OPAQUE_AS_LSA)
...
#define SET_OPAQUE_LSID(type, id) ((((type) << 24) & LSID_OPAQUE_TYPE_MASK) | ((id) & LSID_OPAQUE_ID_MASK))
...
#define OPAQUE_TYPE_RANGE_UNASSIGNED(type) (OPAQUE_TYPE_MAX <= (type) && (type) <= 127)

#define OPAQUE_TYPE_RANGE_RESERVED(type) (127 < (type) && (type) <= 255)

#define VALID_OPAQUE_INFO_LEN(lsahdr) ((ntohs((lsahdr)->length) >= sizeof(struct lsa_header)) && ((ntohs((lsahdr)->length) % sizeof(u_int32_t)) == 0))
...
extern struct ospf_lsa *ospf_opaque_lsa_install(struct ospf_lsa *, int rt_recalc);
...
extern void ospf_opaque_self_originated_lsa_received(struct ospf_neighbor *nbr, struct ospf_lsa *lsa);
