...
static const char *ospf_opaque_type_name(u_char opaque_type) {
	...
	switch(opaque_type) {
		case OPAQUE_TYPE_WILDCARD: /* This is a special assignment! */ name = "Wildcard"; break;
		case OPAQUE_TYPE_TRAFFIC_ENGINEERING_LSA: name = "Traffic Engineering LSA"; break;
		case OPAQUE_TYPE_SYCAMORE_OPTICAL_TOPOLOGY_DESC: name = "Sycamore optical topology description"; break;
		case OPAQUE_TYPE_GRACE_LSA: name = "Grace-LSA"; break;
		case OPAQUE_TYPE_INTER_AS_LSA: name = "Inter-AS TE-v2 LSA"; break;
		case OPAQUE_TYPE_ROUTER_INFORMATION_LSA: name = "Router Information LSA"; break;
		default:
...
static struct list *ospf_get_opaque_funclist(u_char lsa_type) {
	...
	switch(lsa_type) {
       ...
       */
			funclist = ospf_opaque_wildcard_funclist;
			break;
		case OSPF_OPAQUE_LINK_LSA: funclist = ospf_opaque_type9_funclist; break;
		case OSPF_OPAQUE_AREA_LSA: funclist = ospf_opaque_type10_funclist; break;
		case OSPF_OPAQUE_AS_LSA: funclist = ospf_opaque_type11_funclist; break;
		default: zlog_warn("ospf_get_opaque_funclist: Unexpected LSA-type(%u)", lsa_type); break;
	}
	return funclist;
}
...
int ospf_register_opaque_functab(
	u_char lsa_type, u_char opaque_type, int (*new_if_hook)(struct interface *ifp), int (*del_if_hook)(struct interface *ifp), void (*ism_change_hook)(struct ospf_interface *oi, int old_status),
	void (*nsm_change_hook)(struct ospf_neighbor *nbr, int old_status), void (*config_write_router)(struct vty *vty), void (*config_write_if)(struct vty *vty, struct interface *ifp), void (*config_write_debug)(struct vty *vty),
	void (*show_opaque_info)(struct vty *vty, struct ospf_lsa *lsa), int (*lsa_originator)(void *arg), struct ospf_lsa *(*lsa_refresher)(struct ospf_lsa *lsa), int (*new_lsa_hook)(struct ospf_lsa *lsa),
	int (*del_lsa_hook)(struct ospf_lsa *lsa)
) {
	struct list *funclist;
	...
	if((new = XCALLOC(MTYPE_OSPF_OPAQUE_FUNCTAB, sizeof(struct ospf_opaque_functab))) == NULL) {
		zlog_warn("ospf_register_opaque_functab: XMALLOC: %s", safe_strerror(errno));
		goto out;
	}
...
static struct ospf_opaque_functab *ospf_opaque_functab_lookup(struct ospf_lsa *lsa) {
	struct list *funclist;
...
static struct opaque_info_per_type *register_opaque_info_per_type(struct ospf_opaque_functab *functab, struct ospf_lsa *new) {
	...
	if((oipt = XCALLOC(MTYPE_OPAQUE_INFO_PER_TYPE, sizeof(struct opaque_info_per_type))) == NULL) {
...
static void free_opaque_info_per_type(void *val) {
	...
	switch(oipt->lsa_type) {
		...
		default: zlog_warn("free_opaque_info_per_type: Unexpected LSA-type(%u)", oipt->lsa_type); break; /* This case may not exist. */
	}
...
static struct opaque_info_per_type *lookup_opaque_info_by_type(struct ospf_lsa *lsa) {
	...
	switch(lsa->data->type) {
		...
		case OSPF_OPAQUE_AS_LSA:
			...
			break;
		default: zlog_warn("lookup_opaque_info_by_type: Unexpected LSA-type(%u)", lsa->data->type); break;
	}
...
static struct opaque_info_per_id *register_opaque_info_per_id(struct opaque_info_per_type *oipt, struct ospf_lsa *new) {
	...
	if((oipi = XCALLOC(MTYPE_OPAQUE_INFO_PER_ID, sizeof(struct opaque_info_per_id))) == NULL) {
...
static struct opaque_info_per_id *register_opaque_lsa(struct ospf_lsa *new) {
	...
	if((oipt = lookup_opaque_info_by_type(new)) == NULL && (oipt = register_opaque_info_per_type(functab, new)) == NULL) {
		goto out;
	}
...
DEFUN(capability_opaque, capability_opaque_cmd, "capability opaque",
...
ALIAS(capability_opaque, ospf_opaque_capable_cmd, "ospf opaque-lsa",
      "OSPF specific commands\n"
      "Enable the Opaque-LSA capability (rfc2370)\n")

DEFUN(no_capability_opaque, no_capability_opaque_cmd, "no capability opaque",
      NO_STR "Enable specific OSPF feature\n"
	     "Opaque LSA\n") {
	struct ospf *ospf = (struct ospf *) vty->index;
...
ALIAS(no_capability_opaque, no_ospf_opaque_capable_cmd, "no ospf opaque-lsa",
...
static int opaque_lsa_new_if_callback(struct list *funclist, struct interface *ifp) {
	struct listnode *node, *nnode;
	struct ospf_opaque_functab *functab;
	int rc = -1;
...
static int opaque_lsa_del_if_callback(struct list *funclist, struct interface *ifp) {
	struct listnode *node, *nnode;
	struct ospf_opaque_functab *functab;
	int rc = -1;
...
static void opaque_lsa_ism_change_callback(struct list *funclist, struct ospf_interface *oi, int old_status) {
	struct listnode *node, *nnode;
	struct ospf_opaque_functab *functab;
...
static void opaque_lsa_nsm_change_callback(struct list *funclist, struct ospf_neighbor *nbr, int old_status) {
	struct listnode *node, *nnode;
	struct ospf_opaque_functab *functab;
...
static void opaque_lsa_config_write_router_callback(struct list *funclist, struct vty *vty) {
	struct listnode *node, *nnode;
	struct ospf_opaque_functab *functab;
...
static void opaque_lsa_config_write_if_callback(struct list *funclist, struct vty *vty, struct interface *ifp) {
	struct listnode *node, *nnode;
	struct ospf_opaque_functab *functab;
...
static void opaque_lsa_config_write_debug_callback(struct list *funclist, struct vty *vty) {
	struct listnode *node, *nnode;
	struct ospf_opaque_functab *functab;
...
static int opaque_lsa_originate_callback(struct list *funclist, void *lsa_type_dependent) {
	struct listnode *node, *nnode;
	struct ospf_opaque_functab *functab;
	int rc = -1;
...
void show_opaque_info_detail(struct vty *vty, struct ospf_lsa *lsa) {
	...
	if(vty != NULL) {
		...
		vty_out(vty, "  Opaque-Info: %u octets of data%s%s", ntohs(lsah->length) - OSPF_LSA_HEADER_SIZE, VALID_OPAQUE_INFO_LEN(lsah) ? "" : "(Invalid length?)", VTY_NEWLINE);
	} else {
		zlog_debug("    Opaque-Type %u (%s)", opaque_type, ospf_opaque_type_name(opaque_type));
		zlog_debug("    Opaque-ID   0x%x", opaque_id);

		zlog_debug("    Opaque-Info: %u octets of data%s", ntohs(lsah->length) - OSPF_LSA_HEADER_SIZE, VALID_OPAQUE_INFO_LEN(lsah) ? "" : "(Invalid length?)");
	}
...
void ospf_opaque_lsa_originate_schedule(struct ospf_interface *oi, int *delay0) {
   ...
   */
	if(!list_isempty(ospf_opaque_type9_funclist) && list_isempty(oi->opaque_lsa_self) && oi->t_opaque_lsa_self == NULL) {
		if(IS_DEBUG_OSPF_EVENT) {
			zlog_debug("Schedule Type-9 Opaque-LSA origination in %d ms later.", delay);
		}
		oi->t_opaque_lsa_self = thread_add_timer_msec(master, ospf_opaque_type9_lsa_originate, oi, delay);
		delay += top->min_ls_interval;
	}

	if(!list_isempty(ospf_opaque_type10_funclist) && list_isempty(area->opaque_lsa_self) && area->t_opaque_lsa_self == NULL) {
		/*
       ...
       */
		...
		area->t_opaque_lsa_self = thread_add_timer_msec(master, ospf_opaque_type10_lsa_originate, area, delay);
		delay += top->min_ls_interval;
	}

	if(!list_isempty(ospf_opaque_type11_funclist) && list_isempty(top->opaque_lsa_self) && top->t_opaque_lsa_self == NULL) {
		/*
       ...
       */
		...
		top->t_opaque_lsa_self = thread_add_timer_msec(master, ospf_opaque_type11_lsa_originate, top, delay);
		delay += top->min_ls_interval;
	}
   ...
   */
	if(!list_isempty(ospf_opaque_type9_funclist) && !list_isempty(oi->opaque_lsa_self)) {
	   ...
	   */
			...
			ospf_opaque_lsa_reoriginate_schedule((void *) oi, OSPF_OPAQUE_LINK_LSA, oipt->opaque_type);
		}
	}

	if(!list_isempty(ospf_opaque_type10_funclist) && !list_isempty(area->opaque_lsa_self)) {
	   ...
	   */
			...
			ospf_opaque_lsa_reoriginate_schedule((void *) area, OSPF_OPAQUE_AREA_LSA, oipt->opaque_type);
		}
	}

	if(!list_isempty(ospf_opaque_type11_funclist) && !list_isempty(top->opaque_lsa_self)) {
	   ...
	   */
			...
			ospf_opaque_lsa_reoriginate_schedule((void *) top, OSPF_OPAQUE_AS_LSA, oipt->opaque_type);
		}
	}
...
static int ospf_opaque_type9_lsa_originate(struct thread *t) {
	...
	if(IS_DEBUG_OSPF_EVENT) {
		zlog_debug("Timer[Type9-LSA]: Originate Opaque-LSAs for OI %s", IF_NAME(oi));
	}
...
static int ospf_opaque_type10_lsa_originate(struct thread *t) {
	...
	if(IS_DEBUG_OSPF_EVENT) {
		zlog_debug("Timer[Type10-LSA]: Originate Opaque-LSAs for Area %s", inet_ntoa(area->area_id));
	}
...
static void ospf_opaque_lsa_reoriginate_resume(struct list *listtop, void *arg) {
   ...
   */
	for(ALL_LIST_ELEMENTS(listtop, node, nnode, oipt)) {
		...
		if((functab = oipt->functab) == NULL || functab->lsa_originator == NULL) {
			continue;
		}
...
struct ospf_lsa *ospf_opaque_lsa_install(struct ospf_lsa *lsa, int rt_recalc) {
	...
	if((oipt = lookup_opaque_info_by_type(lsa)) != NULL && (oipi = lookup_opaque_info_by_id(oipt, lsa)) != NULL) {
		ospf_lsa_unlock(&oipi->lsa);
   ...
   */
	switch(lsa->data->type) {
		...
		case OSPF_OPAQUE_AS_LSA:
			...
			break;
		default: zlog_warn("ospf_opaque_lsa_install: Unexpected LSA-type(%u)", lsa->data->type); goto out;
	}
...
struct ospf_lsa *ospf_opaque_lsa_refresh(struct ospf_lsa *lsa) {
	...
	if((functab = ospf_opaque_functab_lookup(lsa)) == NULL || functab->lsa_refresher == NULL) {
		/*
...
#define OSPF_OPAQUE_TIMER_ON(T, F, L, V) \
	if(!(T)) (T) = thread_add_timer_msec(master, (F), (L), (V))
...
void ospf_opaque_lsa_reoriginate_schedule(void *lsa_type_dependent, u_char lsa_type, u_char opaque_type) {
	struct ospf *top;
   ...
   */
	...
	if(IS_DEBUG_OSPF_EVENT) {
		zlog_debug(
			...
			lsa_type, delay, GET_OPAQUE_TYPE(ntohl(lsa->data->id.s_addr))
		);
	}
...
static int ospf_opaque_type9_lsa_reoriginate_timer(struct thread *t) {
	...
	if(!CHECK_FLAG(top->config, OSPF_OPAQUE_CAPABLE) || !ospf_if_is_enable(oi) || ospf_nbr_count_opaque_capable(oi) == 0) {
		if(IS_DEBUG_OSPF_EVENT) {
...
void ospf_opaque_lsa_refresh_schedule(struct ospf_lsa *lsa0) {
	...
	if((oipt = lookup_opaque_info_by_type(lsa0)) == NULL || (oipi = lookup_opaque_info_by_id(oipt, lsa0)) == NULL) {
	...
	switch(lsa->data->type) {
		case OSPF_OPAQUE_LINK_LSA:
		case OSPF_OPAQUE_AREA_LSA: ospf_ls_retransmit_delete_nbr_area(lsa->area, lsa); break;
		case OSPF_OPAQUE_AS_LSA:
			top = ospf_lookup();
			...
			break;
		default: zlog_warn("ospf_opaque_lsa_refresh_schedule: Unexpected LSA-type(%u)", lsa->data->type); goto out;
	}
	...
	OSPF_OPAQUE_TIMER_ON(oipi->t_opaque_lsa_self, ospf_opaque_lsa_refresh_timer, oipi, delay * 1000);
out:
	return;
}
...
void ospf_opaque_lsa_flush_schedule(struct ospf_lsa *lsa0) {
	...
	switch(lsa->data->type) {
		...
		case OSPF_OPAQUE_AS_LSA:
			...
			ospf_ls_retransmit_delete_nbr_as(top, lsa);
			break;
		default: zlog_warn("ospf_opaque_lsa_flush_schedule: Unexpected LSA-type(%u)", lsa->data->type); goto out;
	}
...
void ospf_opaque_self_originated_lsa_received(struct ospf_neighbor *nbr, struct ospf_lsa *lsa) {
	struct ospf *top;
   ...
   */
	lsa->data->ls_age = htons(OSPF_LSA_MAXAGE);
	switch(lsa->data->type) {
		case OSPF_OPAQUE_LINK_LSA: ospf_flood_through_area(nbr->oi->area, NULL /*inbr*/, lsa); break;
		case OSPF_OPAQUE_AREA_LSA: ospf_flood_through_area(nbr->oi->area, NULL /*inbr*/, lsa); break;
		case OSPF_OPAQUE_AS_LSA: ospf_flood_through_as(top, NULL /*inbr*/, lsa); break;
		default: zlog_warn("ospf_opaque_self_originated_lsa_received: Unexpected LSA-type(%u)", lsa->data->type); return;
	}
