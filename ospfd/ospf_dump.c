...
const size_t ospf_auth_type_str_max = sizeof(ospf_auth_type_str) / sizeof(ospf_auth_type_str[0]);
...
const char *ospf_area_name_string(struct ospf_area *area) {
	...
	snprintf(buf, OSPF_AREA_STRING_MAXLEN, "%d.%d.%d.%d", (area_id >> 24) & 0xff, (area_id >> 16) & 0xff, (area_id >> 8) & 0xff, area_id & 0xff);
	return buf;
}
...
const char *ospf_area_desc_string(struct ospf_area *area) {
	...
	switch(type) {
		case OSPF_AREA_NSSA: snprintf(buf, OSPF_AREA_DESC_STRING_MAXLEN, "%s [NSSA]", ospf_area_name_string(area)); break;
		case OSPF_AREA_STUB: snprintf(buf, OSPF_AREA_DESC_STRING_MAXLEN, "%s [Stub]", ospf_area_name_string(area)); break;
		default: return ospf_area_name_string(area);
	}
...
const char *ospf_if_name_string(struct ospf_interface *oi) {
	...
	snprintf(buf, OSPF_IF_STRING_MAXLEN, "%s:%d.%d.%d.%d", oi->ifp->name, (ifaddr >> 24) & 0xff, (ifaddr >> 16) & 0xff, (ifaddr >> 8) & 0xff, ifaddr & 0xff);
	return buf;
}
...
void ospf_nbr_state_message(struct ospf_neighbor *nbr, char *buf, size_t size) {
	...
	snprintf(buf, size, "%s/%s", LOOKUP(ospf_nsm_state_msg, nbr->state), LOOKUP(ospf_ism_state_msg, state));
}
...
char *ospf_options_dump(u_char options) {
	...
	snprintf(
		buf, OSPF_OPTION_STR_MAXLEN, "*|%s|%s|%s|%s|%s|%s|%s", (options & OSPF_OPTION_O) ? "O" : "-", (options & OSPF_OPTION_DC) ? "DC" : "-", (options & OSPF_OPTION_EA) ? "EA" : "-", (options & OSPF_OPTION_NP) ? "N/P" : "-",
		(options & OSPF_OPTION_MC) ? "MC" : "-", (options & OSPF_OPTION_E) ? "E" : "-", (options & OSPF_OPTION_MT) ? "M/T" : "-"
	);
...
static void ospf_packet_hello_dump(struct stream *s, u_int16_t length) {
	...
	zlog_debug("  Options %d (%s)", hello->options, ospf_options_dump(hello->options));
...
static char *ospf_dd_flags_dump(u_char flags, char *buf, size_t size) {
	...
	snprintf(buf, size, "%s|%s|%s", (flags & OSPF_DD_FLAG_I) ? "I" : "-", (flags & OSPF_DD_FLAG_M) ? "M" : "-", (flags & OSPF_DD_FLAG_MS) ? "MS" : "-");
...
void ospf_lsa_header_dump(struct lsa_header *lsah) {
	...
	zlog_debug("    Options %d (%s)", lsah->options, ospf_options_dump(lsah->options));
	zlog_debug("    LS type %d (%s)", lsah->type, (lsah->type ? lsah_type : "unknown type"));
...
static void ospf_router_lsa_dump(struct stream *s, u_int16_t length) {
	...
	zlog_debug("    flags %s", ospf_router_lsa_flags_dump(rl->flags, buf, BUFSIZ));
...
static void ospf_as_external_lsa_dump(struct stream *s, u_int16_t length) {
	...
	for(i = 0; size > 0; size -= 12, i++) {
		zlog_debug("    bit %s TOS=%d metric %d", IS_EXTERNAL_METRIC(al->e[i].tos) ? "E" : "-", al->e[i].tos & 0x7f, GET_METRIC(al->e[i].metric));
...
static void ospf_packet_db_desc_dump(struct stream *s, u_int16_t length) {
	...
	zlog_debug("  Options %d (%s)", dd->options, ospf_options_dump(dd->options));
	zlog_debug("  Flags %d (%s)", dd->flags, ospf_dd_flags_dump(dd->flags, dd_flags, sizeof dd_flags));
...
static void ospf_packet_ls_upd_dump(struct stream *s, u_int16_t length) {
	...
	while(length > 0 && count > 0) {
		...
		switch(lsa->type) {
			case OSPF_ROUTER_LSA: ospf_router_lsa_dump(s, length); break;
			case OSPF_NETWORK_LSA: ospf_network_lsa_dump(s, length); break;
			case OSPF_SUMMARY_LSA:
			case OSPF_ASBR_SUMMARY_LSA: ospf_summary_lsa_dump(s, length); break;
			case OSPF_AS_EXTERNAL_LSA: ospf_as_external_lsa_dump(s, length); break;
			case OSPF_AS_NSSA_LSA: ospf_as_external_lsa_dump(s, length); break;
			case OSPF_OPAQUE_LINK_LSA:
			case OSPF_OPAQUE_AREA_LSA:
			case OSPF_OPAQUE_AS_LSA: ospf_opaque_lsa_dump(s, length); break;
			default: break;
		}
...
static void ospf_header_dump(struct ospf_header *ospfh) {
	...
	zlog_debug("  Type %d (%s)", ospfh->type, LOOKUP(ospf_packet_type_str, ospfh->type));
	...
	switch(auth_type) {
		...
		case OSPF_AUTH_CRYPTOGRAPHIC:
			...
			zlog_debug("  Sequence number %ld", (u_long) ntohl(ospfh->u.crypt.crypt_seqnum));
			break;
		default: zlog_debug("* This is not supported authentication type"); break;
	}
}
...
void ospf_packet_dump(struct stream *s) {
	...
	switch(ospfh->type) {
		case OSPF_MSG_HELLO: ospf_packet_hello_dump(s, ntohs(ospfh->length)); break;
		case OSPF_MSG_DB_DESC: ospf_packet_db_desc_dump(s, ntohs(ospfh->length)); break;
		case OSPF_MSG_LS_REQ: ospf_packet_ls_req_dump(s, ntohs(ospfh->length)); break;
		case OSPF_MSG_LS_UPD: ospf_packet_ls_upd_dump(s, ntohs(ospfh->length)); break;
		case OSPF_MSG_LS_ACK: ospf_packet_ls_ack_dump(s, ntohs(ospfh->length)); break;
		default: break;
	}
...
/*
   [no] debug ospf packet (hello|dd|ls-request|ls-update|ls-ack|all)
                          [send|recv [detail]]
*/
DEFUN(debug_ospf_packet, debug_ospf_packet_all_cmd, "debug ospf packet (hello|dd|ls-request|ls-update|ls-ack|all)",
      DEBUG_STR OSPF_STR "OSPF packets\n"
			 "OSPF Hello\n"
...
ALIAS(debug_ospf_packet, debug_ospf_packet_send_recv_cmd, "debug ospf packet (hello|dd|ls-request|ls-update|ls-ack|all) (send|recv|detail)",
      "Debugging functions\n"
      "OSPF information\n"
      "OSPF packets\n"
      "OSPF Hello\n"
...
ALIAS(debug_ospf_packet, debug_ospf_packet_send_recv_detail_cmd, "debug ospf packet (hello|dd|ls-request|ls-update|ls-ack|all) (send|recv) (detail|)",
      "Debugging functions\n"
      "OSPF information\n"
      "OSPF packets\n"
      "OSPF Hello\n"
...
DEFUN(no_debug_ospf_packet, no_debug_ospf_packet_all_cmd, "no debug ospf packet (hello|dd|ls-request|ls-update|ls-ack|all)",
      NO_STR DEBUG_STR OSPF_STR "OSPF packets\n"
				"OSPF Hello\n"
				"OSPF Database Description\n"
				"OSPF Link State Request\n"
				"OSPF Link State Update\n"
...
ALIAS(no_debug_ospf_packet, no_debug_ospf_packet_send_recv_cmd, "no debug ospf packet (hello|dd|ls-request|ls-update|ls-ack|all) (send|recv|detail)",
...
ALIAS(no_debug_ospf_packet, no_debug_ospf_packet_send_recv_detail_cmd, "no debug ospf packet (hello|dd|ls-request|ls-update|ls-ack|all) (send|recv) (detail|)",
      NO_STR "Debugging functions\n"
	     "OSPF information\n"
	     "OSPF packets\n"
	     "OSPF Hello\n"
...
ALIAS(debug_ospf_ism, debug_ospf_ism_sub_cmd, "debug ospf ism (status|events|timers)",
      DEBUG_STR OSPF_STR "OSPF Interface State Machine\n"
			 "ISM Status Information\n"
			 "ISM Event Information\n"
			 "ISM TImer Information\n")

DEFUN(no_debug_ospf_ism, no_debug_ospf_ism_cmd, "no debug ospf ism", NO_STR DEBUG_STR OSPF_STR "OSPF Interface State Machine") {
	if(vty->node == CONFIG_NODE) {
		if(argc == 0) {
			DEBUG_OFF(ism, ISM);
		} else if(argc == 1) {
...
ALIAS(no_debug_ospf_ism, no_debug_ospf_ism_sub_cmd, "no debug ospf ism (status|events|timers)",
      NO_STR "Debugging functions\n"
	     "OSPF information\n"
...
ALIAS(debug_ospf_nsm, debug_ospf_nsm_sub_cmd, "debug ospf nsm (status|events|timers)",
      DEBUG_STR OSPF_STR "OSPF Neighbor State Machine\n"
			 "NSM Status Information\n"
			 "NSM Event Information\n"
			 "NSM Timer Information\n")

DEFUN(no_debug_ospf_nsm, no_debug_ospf_nsm_cmd, "no debug ospf nsm", NO_STR DEBUG_STR OSPF_STR "OSPF Neighbor State Machine") {
	if(vty->node == CONFIG_NODE) {
		if(argc == 0) {
			DEBUG_OFF(nsm, NSM);
		} else if(argc == 1) {
...
ALIAS(no_debug_ospf_nsm, no_debug_ospf_nsm_sub_cmd, "no debug ospf nsm (status|events|timers)",
      NO_STR "Debugging functions\n"
	     "OSPF information\n"
	     "OSPF Interface State Machine\n"
	     "NSM Status Information\n"
...
ALIAS(debug_ospf_lsa, debug_ospf_lsa_sub_cmd, "debug ospf lsa (generate|flooding|install|refresh)",
...
ALIAS(no_debug_ospf_lsa, no_debug_ospf_lsa_sub_cmd, "no debug ospf lsa (generate|flooding|install|refresh)",
      NO_STR DEBUG_STR OSPF_STR "OSPF Link State Advertisement\n"
				"LSA Generation\n"
				"LSA Flooding\n"
				"LSA Install/Delete\n"
				"LSA Refres\n")
...
ALIAS(debug_ospf_zebra, debug_ospf_zebra_sub_cmd, "debug ospf zebra (interface|redistribute)",
      DEBUG_STR OSPF_STR "OSPF Zebra information\n"
			 "Zebra interface\n"
			 "Zebra redistribute\n")
...
ALIAS(no_debug_ospf_zebra, no_debug_ospf_zebra_sub_cmd, "no debug ospf zebra (interface|redistribute)",
      NO_STR DEBUG_STR OSPF_STR "OSPF Zebra information\n"
				"Zebra interface\n"
				"Zebra redistribute\n")
...
DEFUN(no_debug_ospf_event, no_debug_ospf_event_cmd, "no debug ospf event", NO_STR DEBUG_STR OSPF_STR "OSPF event information\n") {
	if(vty->node == CONFIG_NODE) {
...
DEFUN(debug_ospf_nssa, debug_ospf_nssa_cmd, "debug ospf nssa", DEBUG_STR OSPF_STR "OSPF nssa information\n") {
	if(vty->node == CONFIG_NODE) {
...
DEFUN(no_debug_ospf_nssa, no_debug_ospf_nssa_cmd, "no debug ospf nssa", NO_STR DEBUG_STR OSPF_STR "OSPF nssa information\n") {
	if(vty->node == CONFIG_NODE) {
...
DEFUN(debug_ospf_te, debug_ospf_te_cmd, "debug ospf te", DEBUG_STR OSPF_STR "OSPF-TE information\n") {
	if(vty->node == CONFIG_NODE) {
		CONF_DEBUG_ON(te, TE);
	}
	TERM_DEBUG_ON(te, TE);
...
DEFUN(no_debug_ospf_te, no_debug_ospf_te_cmd, "no debug ospf te", NO_STR DEBUG_STR OSPF_STR "OSPF-TE information\n") {
	if(vty->node == CONFIG_NODE) {
...
DEFUN(show_debugging_ospf, show_debugging_ospf_cmd, "show debugging ospf", SHOW_STR DEBUG_STR OSPF_STR) {
	int i;
	...
	for(i = 0; i < 5; i++) {
		if(IS_DEBUG_OSPF_PACKET(i, SEND) && IS_DEBUG_OSPF_PACKET(i, RECV)) {
			vty_out(vty, "  OSPF packet %s%s debugging is on%s", LOOKUP(ospf_packet_type_str, i + 1), IS_DEBUG_OSPF_PACKET(i, DETAIL) ? " detail" : "", VTY_NEWLINE);
		} else {
			if(IS_DEBUG_OSPF_PACKET(i, SEND)) {
				vty_out(vty, "  OSPF packet %s send%s debugging is on%s", LOOKUP(ospf_packet_type_str, i + 1), IS_DEBUG_OSPF_PACKET(i, DETAIL) ? " detail" : "", VTY_NEWLINE);
			}
			if(IS_DEBUG_OSPF_PACKET(i, RECV)) {
				vty_out(vty, "  OSPF packet %s receive%s debugging is on%s", LOOKUP(ospf_packet_type_str, i + 1), IS_DEBUG_OSPF_PACKET(i, DETAIL) ? " detail" : "", VTY_NEWLINE);
			}
		}
	}
...
static int config_write_debug(struct vty *vty) {
	...
	const char *detail_str[] = { "", " send", " recv", "", " detail", " send detail", " recv detail", " detail" };
	...
	if(r == OSPF_DEBUG_SEND_RECV) {
		...
		for(i = 0; i < 5; i++) {
			if(conf_debug_ospf_packet[i] & OSPF_DEBUG_DETAIL) {
				vty_out(vty, "debug ospf packet %s detail%s", type_str[i], VTY_NEWLINE);
			}
		}
		return 1;
	}
	...
	for(i = 0; i < 5; i++) {
		...
		vty_out(vty, "debug ospf packet %s%s%s", type_str[i], detail_str[conf_debug_ospf_packet[i]], VTY_NEWLINE);
		write = 1;
	}
