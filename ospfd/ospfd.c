...
static void ospf_network_run_interface(struct ospf *, struct interface *, struct prefix *, struct ospf_area *);
static void ospf_network_run_subnet(struct ospf *, struct connected *, struct prefix *, struct ospf_area *);
static int ospf_network_match_iface(const struct connected *, const struct prefix *);
		...
		if(router_id_old.s_addr == 0 && ospf->external_origin) {
			...
			for(type = 0; type < ZEBRA_ROUTE_MAX; type++) {
				if(ospf->external_origin & (1 << type)) {
					thread_add_event(master, ospf_external_lsa_originate_timer, ospf, type);
				}
			}
			/* Originate Deafult. */
...
static struct ospf *ospf_new(void) {
	int i;
	...
	new->t_maxage_walker = thread_add_timer(master, ospf_lsa_maxage_walker, new, OSPF_LSA_MAXAGE_CHECK_INTERVAL);
	...
	new->t_lsa_refresher = thread_add_timer(master, ospf_lsa_refresh_walker, new, new->lsa_refresh_interval);
	...
	if(IS_DEBUG_OSPF(zebra, ZEBRA_INTERFACE)) {
		zlog_debug("%s: starting with OSPF send buffer size %u", __func__, new->maxsndbuflen);
	}
	if((new->ibuf = stream_new(OSPF_MAX_PACKET_SIZE + 1)) == NULL) {
		zlog_err("ospf_new: fatal error: stream_new(%u) failed allocating ibuf", OSPF_MAX_PACKET_SIZE + 1);
		exit(1);
	}
...
static void ospf_deferred_shutdown_finish(struct ospf *ospf) {
	...
	if(CHECK_FLAG(om->options, OSPF_MASTER_SHUTDOWN) && (listcount(om->ospf) == 0)) {
		exit(0);
	}
...
static void ospf_deferred_shutdown_check(struct ospf *ospf) {
	...
	OSPF_TIMER_ON(ospf->t_deferred_shutdown, ospf_deferred_shutdown_timer, timeout);
	return;
}
...
static struct ospf_area *ospf_area_new(struct ospf *ospf, struct in_addr area_id) {
...
void ospf_area_check_free(struct ospf *ospf, struct in_addr area_id) {
	...
	if(area && listcount(area->oiflist) == 0 && area->ranges->top == NULL && area->shortcut_configured == OSPF_SHORTCUT_DEFAULT && area->external_routing == OSPF_AREA_DEFAULT && area->no_summary == 0 && area->default_cost == 1
	   && EXPORT_NAME(area) == NULL && IMPORT_NAME(area) == NULL && area->auth_type == OSPF_AUTH_NULL) {
		listnode_delete(ospf->areas, area);
		ospf_area_free(area);
	}
}

struct ospf_area *ospf_area_get(struct ospf *ospf, struct in_addr area_id, int format) {
	struct ospf_area *area;
...
struct ospf_area *ospf_area_lookup_by_area_id(struct ospf *ospf, struct in_addr area_id) {
	struct ospf_area *area;
	struct listnode *node;
...
static struct ospf_network *ospf_network_new(struct in_addr area_id, int format) {
...
static void update_redistributed(struct ospf *ospf, int add_to_ospf) {
	...
	if(ospf_is_type_redistributed(ZEBRA_ROUTE_CONNECT)) {
		if(EXTERNAL_INFO(ZEBRA_ROUTE_CONNECT)) {
			for(rn = route_top(EXTERNAL_INFO(ZEBRA_ROUTE_CONNECT)); rn; rn = route_next(rn)) {
				if((ei = rn->info) != NULL) {
					if(add_to_ospf) {
...
int ospf_network_set(struct ospf *ospf, struct prefix_ipv4 *p, struct in_addr area_id) {
	struct ospf_network *network;
	struct ospf_area *area;
	struct route_node *rn;
...
int ospf_network_unset(struct ospf *ospf, struct prefix_ipv4 *p, struct in_addr area_id) {
	struct route_node *rn;
	struct ospf_network *network;
	struct listnode *node, *nnode;
	struct ospf_interface *oi;
 ...
 */
static int ospf_network_match_iface(const struct connected *co, const struct prefix *net) {
	/* new approach: more elegant and conceptually clean */
	return prefix_match(net, CONNECTED_PREFIX(co));
}

static void ospf_update_interface_area(struct connected *co, struct ospf_area *area) {
 ...
 */
static void ospf_network_run_subnet(struct ospf *ospf, struct connected *co, struct prefix *p, struct ospf_area *given_area) {
	struct ospf_interface *oi;
   ...
   */
	...
	if(OSPF_IF_PARAM_CONFIGURED(params, if_area)) {
		area = (ospf_area_get(ospf, params->if_area, OSPF_AREA_ID_FORMAT_ADDRESS));
	}
   ...
   */
	...
	if(p) {
       ...
       */
		if(p->family == co->address->family && ospf_network_match_iface(co, p)) {
...
static void ospf_network_run_interface(struct ospf *ospf, struct interface *ifp, struct prefix *p, struct ospf_area *given_area) {
...
static void ospf_area_type_set(struct ospf_area *area, int type) {
	...
	if(area->external_routing == type) {
		if(IS_DEBUG_OSPF_EVENT) {
			zlog_debug("Area[%s]: Types are the same, ignored.", inet_ntoa(area->area_id));
		}
		return;
	}
	...
	if(IS_DEBUG_OSPF_EVENT) {
		zlog_debug("Area[%s]: Configured as %s", inet_ntoa(area->area_id), LOOKUP(ospf_area_type_msg, type));
	}
...
int ospf_area_nssa_translator_role_set(struct ospf *ospf, struct in_addr area_id, int role) {
	struct ospf_area *area;
...
int ospf_area_export_list_set(struct ospf *ospf, struct ospf_area *area, const char *list_name) {
...
int ospf_area_import_list_set(struct ospf *ospf, struct ospf_area *area, const char *name) {
	struct access_list *list;
...
int ospf_timers_refresh_set(struct ospf *ospf, int interval) {
	...
	time_left = ospf->lsa_refresh_interval - (quagga_time(NULL) - ospf->lsa_refresher_started);

	if(time_left > interval) {
		OSPF_TIMER_OFF(ospf->t_lsa_refresher);
		ospf->t_lsa_refresher = thread_add_timer(master, ospf_lsa_refresh_walker, ospf, interval);
	}
...
int ospf_timers_refresh_unset(struct ospf *ospf) {
	...
	if(time_left > OSPF_LSA_REFRESH_INTERVAL_DEFAULT) {
		OSPF_TIMER_OFF(ospf->t_lsa_refresher);
		ospf->t_lsa_refresher = thread_add_timer(master, ospf_lsa_refresh_walker, ospf, OSPF_LSA_REFRESH_INTERVAL_DEFAULT);
	}
...
static void ospf_nbr_nbma_delete(struct ospf *ospf, struct ospf_nbr_nbma *nbr_nbma) {
	struct route_node *rn;
...
static void ospf_nbr_nbma_add(struct ospf_nbr_nbma *nbr_nbma, struct ospf_interface *oi) {
...
struct ospf_nbr_nbma *ospf_nbr_nbma_lookup(struct ospf *ospf, struct in_addr nbr_addr) {
	struct route_node *rn;
	struct prefix_ipv4 p;
...
struct ospf_nbr_nbma *ospf_nbr_nbma_lookup_next(struct ospf *ospf, struct in_addr *addr, int first) {
#if 0
  struct ospf_nbr_nbma *nbr_nbma;
  struct listnode *node;
#endif
...
int ospf_nbr_nbma_priority_set(struct ospf *ospf, struct in_addr nbr_addr, u_char priority) {
	struct ospf_nbr_nbma *nbr_nbma;
