 ...
 * for speed. */
static void ospf_flood_delayed_lsa_ack(struct ospf_neighbor *inbr, struct ospf_lsa *lsa) {
...
static void ospf_process_self_originated_lsa(struct ospf *ospf, struct ospf_lsa *new, struct ospf_area *area) {
	struct ospf_interface *oi;
	struct external_info *ei;
	struct listnode *node;

	if(IS_DEBUG_OSPF_EVENT) {
		zlog_debug("LSA[Type%d:%s]: Process self-originated LSA seq 0x%x", new->data->type, inet_ntoa(new->data->id), ntohl(new->data->ls_seqnum));
	}
	 ...
	 could be moved from one area to another. */
			for(ALL_LIST_ELEMENTS_RO(ospf->oiflist, node, oi)) {
				...
				if(IPV4_ADDR_SAME(&oi->address->u.prefix4, &new->data->id)) {
					if(oi->area != area || oi->type != OSPF_IFTYPE_BROADCAST || !IPV4_ADDR_SAME(&oi->address->u.prefix4, &DR(oi))) {
		...
		case OSPF_ASBR_SUMMARY_LSA: ospf_schedule_abr_task(ospf); break;
		case OSPF_AS_EXTERNAL_LSA:
		case OSPF_AS_NSSA_LSA:
			if((new->data->type == OSPF_AS_EXTERNAL_LSA) && CHECK_FLAG(new->flags, OSPF_LSA_LOCAL_XLT)) {
			...
			break;
		case OSPF_OPAQUE_AREA_LSA: ospf_opaque_lsa_refresh(new); break;
...
*/
int ospf_flood(struct ospf *ospf, struct ospf_neighbor *nbr, struct ospf_lsa *current, struct ospf_lsa *new) {
	struct ospf_interface *oi;
	int lsa_ack_flag;
	...
	if(IS_DEBUG_OSPF_EVENT) {
		zlog_debug("LSA[Flooding]: start, NBR %s (%s), cur(%p), New-LSA[%s]", inet_ntoa(nbr->router_id), LOOKUP(ospf_nsm_state_msg, nbr->state), (void *) current, dump_lsa_key(new));
	}
	...
	{
		if(IS_LSA_SELF(current) && (ntohs(current->data->ls_age) == 0 && ntohl(current->data->ls_seqnum) == OSPF_INITIAL_SEQUENCE_NUMBER)) {
			if(IS_DEBUG_OSPF_EVENT) {
		...
		} else if(tv_cmp(tv_sub(recent_relative_time(), current->tv_recv), msec2tv(ospf->min_ls_arrival)) < 0) {
			if(IS_DEBUG_OSPF_EVENT) {
	...
	if(current) {
		switch(current->data->type) {
			case OSPF_AS_EXTERNAL_LSA:
			case OSPF_OPAQUE_AS_LSA: ospf_ls_retransmit_delete_nbr_as(ospf, current); break;
			default: ospf_ls_retransmit_delete_nbr_area(nbr->oi->area, current); break;
		}
	}
...
static int ospf_flood_through_interface(struct ospf_interface *oi, struct ospf_neighbor *inbr, struct ospf_lsa *lsa) {
	...
	if(IS_DEBUG_OSPF_EVENT) {
		zlog_debug(
			...
			IF_NAME(oi), inbr ? inet_ntoa(inbr->router_id) : "NULL", dump_lsa_key(lsa)
		);
	}
	...
	for(rn = route_top(oi->nbrs); rn; rn = route_next(rn)) {
		...
		onbr = rn->info;
		if(IS_DEBUG_OSPF_EVENT) {
			zlog_debug("ospf_flood_through_interface(): considering nbr %s (%s)", inet_ntoa(onbr->router_id), LOOKUP(ospf_nsm_state_msg, onbr->state));
		}
...
int ospf_flood_through_area(struct ospf_area *area, struct ospf_neighbor *inbr, struct ospf_lsa *lsa) {
	...
	for(ALL_LIST_ELEMENTS(area->oiflist, node, nnode, oi)) {
		if(area->area_id.s_addr != OSPF_AREA_BACKBONE && oi->type == OSPF_IFTYPE_VIRTUALLINK) {
			continue;
		}
		...
		if((lsa->data->type == OSPF_OPAQUE_LINK_LSA) && (lsa->oi != oi)) {
           ...
           */
			if(IS_DEBUG_OSPF(lsa, LSA_FLOODING)) {
				zlog_debug("Type-9 Opaque-LSA: lsa->oi(%p) != oi(%p)", (void *) lsa->oi, (void *) oi);
			}
			continue;
		}
...
int ospf_flood_through_as(struct ospf *ospf, struct ospf_neighbor *inbr, struct ospf_lsa *lsa) {
	struct listnode *node;
	...
	for(ALL_LIST_ELEMENTS_RO(ospf->areas, node, area)) {
		...
		switch(area->external_routing) {
			...
			case OSPF_AREA_NSSA: /* Sending Type 5 or 7 into NSSA area */
				...
				if(lsa->data->type == OSPF_AS_NSSA_LSA && area == lsa->area) {
					/* We will send it. */
					continue_flag = 0;
				} else {
...
int ospf_flood_through(struct ospf *ospf, struct ospf_neighbor *inbr, struct ospf_lsa *lsa) {
	int lsa_ack_flag = 0;
	...
	switch(lsa->data->type) {
		...
		case OSPF_OPAQUE_AREA_LSA: lsa_ack_flag = ospf_flood_through_area(inbr->oi->area, inbr, lsa); break;
   ...
   */
	switch(lsa->data->type) {
		...
		case OSPF_AS_NSSA_LSA:
			...
			/* Fallthrough */
		default: lsa_ack_flag = ospf_flood_through_area(lsa->area, inbr, lsa); break;
	}
#endif /* ORIGINAL_CODING */
...
void ospf_ls_request_add(struct ospf_neighbor *nbr, struct ospf_lsa *lsa) {
   ...
   */
	if(IS_DEBUG_OSPF(lsa, LSA_FLOODING)) {
		zlog_debug("RqstL(%lu)++, NBR(%s), LSA[%s]", ospf_ls_request_count(nbr), inet_ntoa(nbr->router_id), dump_lsa_key(lsa));
	}
...
struct ospf_lsa *ospf_ls_request_lookup(struct ospf_neighbor *nbr, struct ospf_lsa *lsa) {
...
unsigned long ospf_ls_retransmit_count_self(struct ospf_neighbor *nbr, int lsa_type) {
...
void ospf_ls_retransmit_add(struct ospf_neighbor *nbr, struct ospf_lsa *lsa) {
	...
	if(ospf_lsa_more_recent(old, lsa) < 0) {
       ...
       */
		if(IS_DEBUG_OSPF(lsa, LSA_FLOODING)) {
			zlog_debug("RXmtL(%lu)++, NBR(%s), LSA[%s]", ospf_ls_retransmit_count(nbr), inet_ntoa(nbr->router_id), dump_lsa_key(lsa));
		}
...
struct ospf_lsa *ospf_ls_retransmit_lookup(struct ospf_neighbor *nbr, struct ospf_lsa *lsa) {
	return ospf_lsdb_lookup(&nbr->ls_rxmt, lsa);
}

static void ospf_ls_retransmit_delete_nbr_if(struct ospf_interface *oi, struct ospf_lsa *lsa) {
	struct route_node *rn;
	struct ospf_neighbor *nbr;
	struct ospf_lsa *lsr;
...
void ospf_ls_retransmit_delete_nbr_area(struct ospf_area *area, struct ospf_lsa *lsa) {
	struct listnode *node, *nnode;
	struct ospf_interface *oi;
...
void ospf_lsa_flush(struct ospf *ospf, struct ospf_lsa *lsa) {
	...
	switch(lsa->data->type) {
		...
		case OSPF_AS_NSSA_LSA:
		case OSPF_OPAQUE_LINK_LSA:
		case OSPF_OPAQUE_AREA_LSA: ospf_lsa_flush_area(lsa, lsa->area); break;
		case OSPF_AS_EXTERNAL_LSA:
		case OSPF_OPAQUE_AS_LSA: ospf_lsa_flush_as(ospf, lsa); break;
		default: zlog_info("%s: Unknown LSA type %u", __func__, lsa->data->type); break;
	}
}
