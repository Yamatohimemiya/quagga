...
static struct ospf_route *ospf_find_abr_route(struct route_table *rtrs, struct prefix_ipv4 *abr, struct ospf_area *area) {
...
static void ospf_ia_network_route(struct ospf *ospf, struct route_table *rt, struct prefix_ipv4 *p, struct ospf_route *new_or, struct ospf_route *abr_or) {
	struct route_node *rn1;
	struct ospf_route * or ;

	if(IS_DEBUG_OSPF_EVENT) {
		zlog_debug("ospf_ia_network_route(): processing summary route to %s/%d", inet_ntoa(p->prefix), p->prefixlen);
	}
	...
	if((rn1 = route_node_lookup(rt, (struct prefix *) p))) {
		...
		if(IS_DEBUG_OSPF_EVENT) {
			zlog_debug("ospf_ia_network_route(): add new route to %s/%d", inet_ntoa(p->prefix), p->prefixlen);
		}
		ospf_route_add(rt, p, new_or, abr_or);
	}
}

static void ospf_ia_router_route(struct ospf *ospf, struct route_table *rtrs, struct prefix_ipv4 *p, struct ospf_route *new_or, struct ospf_route *abr_or) {
	struct ospf_route * or = NULL;
	struct route_node *rn;
	int ret;

	if(IS_DEBUG_OSPF_EVENT) {
		zlog_debug("ospf_ia_router_route(): considering %s/%d", inet_ntoa(p->prefix), p->prefixlen);
	}
	/* Find a route to the same dest */
...
static int process_summary_lsa(struct ospf_area *area, struct route_table *rt, struct route_table *rtrs, struct ospf_lsa *lsa) {
	...
	if(sl->header.type == OSPF_SUMMARY_LSA && (range = ospf_area_range_match_any(ospf, &p)) && ospf_area_range_active(range)) {
		return 0;
	}
   ...
   * to exclude default? 
   */
	if(IS_OSPF_ABR(ospf) && ospf->abr_type != OSPF_ABR_STAND && area->external_routing != OSPF_AREA_DEFAULT && p.prefix.s_addr == OSPF_DEFAULT_DESTINATION && p.prefixlen == 0) {
...
static void ospf_examine_summaries(struct ospf_area *area, struct route_table *lsdb_rt, struct route_table *rt, struct route_table *rtrs) {
	struct ospf_lsa *lsa;
	struct route_node *rn;
...
int ospf_area_is_transit(struct ospf_area *area) {
	return (area->transit == OSPF_TRANSIT_TRUE) || ospf_full_virtual_nbrs(area); /* Cisco forgets to set the V-bit :( */
}

static void ospf_update_network_route(struct ospf *ospf, struct route_table *rt, struct route_table *rtrs, struct summary_lsa *lsa, struct prefix_ipv4 *p, struct ospf_area *area) {
	struct route_node *rn;
	...
	if(ospf->abr_type == OSPF_ABR_SHORTCUT) {
		if(or->path_type == OSPF_PATH_INTRA_AREA && !OSPF_IS_AREA_ID_BACKBONE(or->u.std.area_id)) {
			if(IS_DEBUG_OSPF_EVENT) {
	...
	if(or->cost > cost) {
		...
		if((ospf->abr_type == OSPF_ABR_SHORTCUT) && !OSPF_IS_AREA_ID_BACKBONE(or->u.std.area_id)) {
...
static void ospf_update_router_route(struct ospf *ospf, struct route_table *rtrs, struct summary_lsa *lsa, struct prefix_ipv4 *p, struct ospf_area *area) {
	struct ospf_route * or, *abr_or, *new_or;
	struct prefix_ipv4 abr;
	u_int32_t cost;
	...
	if((ospf->backbone == NULL) || !(or = ospf_find_asbr_route_through_area(rtrs, p, ospf->backbone))) {
		if(ospf->abr_type != OSPF_ABR_SHORTCUT) {
...
static int process_transit_summary_lsa(struct ospf_area *area, struct route_table *rt, struct route_table *rtrs, struct ospf_lsa *lsa) {
	struct ospf *ospf = area->ospf;
	struct summary_lsa *sl;
	...
	if(IS_DEBUG_OSPF_EVENT) {
		zlog_debug("process_transit_summaries(): LS ID: %s", inet_ntoa(lsa->data->id));
	}
	metric = GET_METRIC(sl->metric);
...
static void ospf_examine_transit_summaries(struct ospf_area *area, struct route_table *lsdb_rt, struct route_table *rt, struct route_table *rtrs) {
	struct ospf_lsa *lsa;
	struct route_node *rn;
...
void ospf_ia_routing(struct ospf *ospf, struct route_table *rt, struct route_table *rtrs) {
		...
		switch(ospf->abr_type) {
			...
			case OSPF_ABR_SHORTCUT:
				...
				for(ALL_LIST_ELEMENTS_RO(ospf->areas, node, area)) {
					if(area != ospf->backbone) {
						if(ospf_area_is_transit(area)
						   || ((area->shortcut_configured != OSPF_SHORTCUT_DISABLE) && ((ospf->backbone == NULL) || ((area->shortcut_configured == OSPF_SHORTCUT_ENABLE) && area->shortcut_capability)))) {
							OSPF_EXAMINE_TRANSIT_SUMMARIES_ALL(area, rt, rtrs);
						}
					}
				}
