...
const size_t ospf_packet_type_str_max = sizeof(ospf_packet_type_str) / sizeof(ospf_packet_type_str[0]);
...
static const u_int16_t ospf_packet_minlen[] = {
	0, OSPF_HELLO_MIN_SIZE, OSPF_DB_DESC_MIN_SIZE, OSPF_LS_REQ_MIN_SIZE, OSPF_LS_UPD_MIN_SIZE, OSPF_LS_ACK_MIN_SIZE,
};
...
static const u_int16_t ospf_lsa_minlen[] = {
	0, OSPF_ROUTER_LSA_MIN_SIZE, OSPF_NETWORK_LSA_MIN_SIZE, OSPF_SUMMARY_LSA_MIN_SIZE, OSPF_SUMMARY_LSA_MIN_SIZE, OSPF_AS_EXTERNAL_LSA_MIN_SIZE, 0, OSPF_AS_EXTERNAL_LSA_MIN_SIZE, 0, 0, 0, 0,
};
...
void ospf_packet_add(struct ospf_interface *oi, struct ospf_packet *op) {
	if(!oi->obuf) {
		zlog_err(
			...
			IF_NAME(oi), oi->state, LOOKUP(ospf_ism_state_msg, oi->state), LOOKUP(ospf_packet_type_str, stream_getc_from(op->s, 1)), inet_ntoa(op->dst)
		);
		return;
	}
...
static void ospf_packet_add_top(struct ospf_interface *oi, struct ospf_packet *op) {
	if(!oi->obuf) {
		zlog_err(
			"ospf_packet_add(interface %s in state %d [%s], packet type %s, "
			"destination %s) called with NULL obuf, ignoring "
...
struct ospf_packet *ospf_packet_dup(struct ospf_packet *op) {
	...
	if(stream_get_endp(op->s) != op->length) {
		/* XXX size_t */
		zlog_warn("ospf_packet_dup stream %lu ospf_packet %u size mismatch", (u_long) STREAM_SIZE(op->s), op->length);
	}
...
static int ospf_check_md5_digest(struct ospf_interface *oi, struct ospf_header *ospfh) {
	MD5_CTX ctx;
	...
	/* Get secret key. */
	ck = ospf_crypt_key_lookup(OSPF_IF_PARAM(oi, auth_crypt), ospfh->u.crypt.key_id);
	if(ck == NULL) {
		zlog_warn("interface %s: ospf_check_md5 no key %d", IF_NAME(oi), ospfh->u.crypt.key_id);
		return 0;
	}
	...
	if(nbr && ntohl(nbr->crypt_seqnum) > ntohl(ospfh->u.crypt.crypt_seqnum)) {
		zlog_warn("interface %s: ospf_check_md5 bad sequence %d (expect %d)", IF_NAME(oi), ntohl(ospfh->u.crypt.crypt_seqnum), ntohl(nbr->crypt_seqnum));
		return 0;
	}
	...
	if(memcmp((caddr_t) ospfh + length, digest, OSPF_AUTH_MD5_SIZE)) {
		zlog_warn("interface %s: ospf_check_md5 checksum mismatch", IF_NAME(oi));
		return 0;
	}
...
static int ospf_make_md5_digest(struct ospf_interface *oi, struct ospf_packet *op) {
	...
	if(stream_get_endp(op->s) != op->length) {
		/* XXX size_t */
		zlog_warn("ospf_make_md5_digest: length mismatch stream %lu ospf_packet %u", (u_long) stream_get_endp(op->s), op->length);
	}
...
static void ospf_write_frags(int fd, struct ospf_packet *op, struct ip *iph, struct msghdr *msg, unsigned int maxdatasize, unsigned int mtu, int flags, u_char type) {
   ...
   */
	...
	while((stream_get_endp(op->s) - stream_get_getp(op->s)) > maxdatasize) {
		...
		if(ret < 0) {
			zlog_warn(
				...
				inet_ntoa(iph->ip_dst), iph->ip_id, iph->ip_off, iph->ip_len, mtu, safe_strerror(errno)
			);
		}

		if(IS_DEBUG_OSPF_PACKET(type - 1, SEND)) {
			zlog_debug("ospf_write_frags: sent id %d, off %d, len %d to %s\n", iph->ip_id, iph->ip_off, iph->ip_len, inet_ntoa(iph->ip_dst));
...
#define OSPF_WRITE_IPHL_SHIFT 2
   ...
   */
	...
	if(sizeof(struct ip) > (unsigned int) (iph.ip_hl << OSPF_WRITE_IPHL_SHIFT)) {
   ...
   */
	...
	if(op->length > maxdatasize) {
		ospf_write_frags(ospf->fd, op, &iph, &msg, maxdatasize, oi->ifp->mtu, flags, type);
	}
#endif /* WANT_OSPF_WRITE_FRAGMENT */
	...
	if(ret < 0) {
		zlog_warn(
			...
			inet_ntoa(iph.ip_dst), iph.ip_id, iph.ip_off, iph.ip_len, oi->ifp->name, oi->ifp->mtu, safe_strerror(errno)
		);
	}
	...
	if(IS_DEBUG_OSPF_PACKET(type - 1, SEND)) {
		...
		zlog_debug("%s sent to [%s] via [%s].", LOOKUP(ospf_packet_type_str, type), inet_ntoa(op->dst), IF_NAME(oi));
	...
	if(!list_isempty(ospf->oi_write_q)) {
		ospf->t_write = thread_add_write(master, ospf_write, ospf, ospf->fd);
	}
...
static void ospf_hello(struct ip *iph, struct ospf_header *ospfh, struct stream *s, struct ospf_interface *oi, int size) {
	...
	if(IPV4_ADDR_SAME(&ospfh->router_id, &oi->ospf->router_id)) {
		if(IS_DEBUG_OSPF_PACKET(ospfh->type - 1, RECV)) {
			zlog_debug(
				...
				"dropping.",
				LOOKUP(ospf_packet_type_str, ospfh->type), inet_ntoa(iph->ip_src)
			);
		}
		return;
	}
	...
	if(oi->type != OSPF_IFTYPE_POINTOPOINT && oi->type != OSPF_IFTYPE_VIRTUALLINK) {
		if(oi->address->prefixlen != p.prefixlen) {
			zlog_warn(
				"Packet %s [Hello:RECV]: NetworkMask mismatch on %s (configured prefix length is %d, but hello packet indicates %d).", inet_ntoa(ospfh->router_id), IF_NAME(oi), (int) oi->address->prefixlen, (int) p.prefixlen
			);
			return;
		}
	}
	...
	if(OSPF_IF_PARAM(oi, v_wait) != ntohl(hello->dead_interval)) {
		zlog_warn(
			...
			inet_ntoa(ospfh->router_id), OSPF_IF_PARAM(oi, v_wait), ntohl(hello->dead_interval)
		);
		return;
	}
	...
	if(OSPF_IF_PARAM(oi, fast_hello) == 0) {
		if(OSPF_IF_PARAM(oi, v_hello) != ntohs(hello->hello_interval)) {
			zlog_warn(
				...
				"(expected %u, but received %u).",
				inet_ntoa(ospfh->router_id), OSPF_IF_PARAM(oi, v_hello), ntohs(hello->hello_interval)
			);
			return;
		}
	}
	...
	if(IS_DEBUG_OSPF_EVENT) {
		zlog_debug("Packet %s [Hello:RECV]: Options %s", inet_ntoa(ospfh->router_id), ospf_options_dump(hello->options));
	}
...
#define REJECT_IF_TBIT_ON 1 /* XXX */
	...
	if(CHECK_FLAG(hello->options, OSPF_OPTION_MT)) {
       ...
       */
		zlog_warn("Packet %s [Hello:RECV]: T-bit on, drop it.", inet_ntoa(ospfh->router_id));
		return;
	}
#endif /* REJECT_IF_TBIT_ON */

	if(CHECK_FLAG(oi->ospf->config, OSPF_OPAQUE_CAPABLE) && CHECK_FLAG(hello->options, OSPF_OPTION_O)) {
		/*
       * This router does know the correct usage of O-bit
       * the bit should be set in DD packet only.
       */
		zlog_warn("Packet %s [Hello:RECV]: O-bit abuse?", inet_ntoa(ospfh->router_id));
	...
	if(oi->area->external_routing == OSPF_AREA_NSSA) {
		if(!(CHECK_FLAG(OPTIONS(oi), OSPF_OPTION_NP) && CHECK_FLAG(hello->options, OSPF_OPTION_NP) && !CHECK_FLAG(OPTIONS(oi), OSPF_OPTION_E) && !CHECK_FLAG(hello->options, OSPF_OPTION_E))) {
       ...
       Packet's Options field should be ignored. */
		if(CHECK_FLAG(OPTIONS(oi), OSPF_OPTION_E) != CHECK_FLAG(hello->options, OSPF_OPTION_E)) {
			zlog_warn("Packet %s [Hello:RECV]: my options: %x, his options %x", inet_ntoa(ospfh->router_id), OPTIONS(oi), hello->options);
			return;
		}
	...
	if(oi->type == OSPF_IFTYPE_NBMA) {
		if(PRIORITY(oi) == 0 && hello->priority > 0 && IPV4_ADDR_CMP(&DR(oi), &iph->ip_src) && IPV4_ADDR_CMP(&BDR(oi), &iph->ip_src)) {
			OSPF_NSM_TIMER_ON(nbr->t_hello_reply, ospf_hello_reply_timer, OSPF_HELLO_REPLY_DELAY);
		}
	}
	...
	if(oi->type == OSPF_IFTYPE_NBMA && (old_state == NSM_Down || old_state == NSM_Attempt)) {
	...
	if(ospf_nbr_bidirectional(&oi->ospf->router_id, hello->neighbors, size - OSPF_HELLO_MIN_SIZE)) {
	...
	if(oi->state == ISM_Waiting && IPV4_ADDR_SAME(&nbr->address.u.prefix4, &hello->bd_router)) {
		OSPF_ISM_EVENT_SCHEDULE(oi, ISM_BackupSeen);
	}

	/* had not previously. */
	if((IPV4_ADDR_SAME(&nbr->address.u.prefix4, &hello->d_router) && IPV4_ADDR_CMP(&nbr->address.u.prefix4, &nbr->d_router))
	   || (IPV4_ADDR_CMP(&nbr->address.u.prefix4, &hello->d_router) && IPV4_ADDR_SAME(&nbr->address.u.prefix4, &nbr->d_router))) {
		OSPF_ISM_EVENT_SCHEDULE(oi, ISM_NeighborChange);
	}

	/* had not previously. */
	if((IPV4_ADDR_SAME(&nbr->address.u.prefix4, &hello->bd_router) && IPV4_ADDR_CMP(&nbr->address.u.prefix4, &nbr->bd_router))
	   || (IPV4_ADDR_CMP(&nbr->address.u.prefix4, &hello->bd_router) && IPV4_ADDR_SAME(&nbr->address.u.prefix4, &nbr->bd_router))) {
		OSPF_ISM_EVENT_SCHEDULE(oi, ISM_NeighborChange);
	}
...
static void ospf_db_desc_save_current(struct ospf_neighbor *nbr, struct ospf_db_desc *dd) {
...
static void ospf_db_desc_proc(struct stream *s, struct ospf_interface *oi, struct ospf_neighbor *nbr, struct ospf_db_desc *dd, u_int16_t size) {
	struct ospf_lsa *new, *find;
	struct lsa_header *lsah;

	stream_forward_getp(s, OSPF_DB_DESC_MIN_SIZE);
	for(size -= OSPF_DB_DESC_MIN_SIZE; size >= OSPF_LSA_HEADER_SIZE; size -= OSPF_LSA_HEADER_SIZE) {
		...
		if(IS_OPAQUE_LSA(lsah->type) && !CHECK_FLAG(nbr->options, OSPF_OPTION_O)) {
			...
			case OSPF_OPAQUE_AS_LSA:
             ...
             allow if from NSSA. */
				if(oi->area->external_routing == OSPF_AREA_STUB) {
					zlog_warn("Packet [DD:RECV]: LSA[Type%d:%s] from %s area.", lsah->type, inet_ntoa(lsah->id), (oi->area->external_routing == OSPF_AREA_STUB) ? "STUB" : "NSSA");
					OSPF_NSM_EVENT_SCHEDULE(nbr, NSM_SeqNumberMismatch);
					return;
				}
				break;
...
static int ospf_db_desc_is_dup(struct ospf_db_desc *dd, struct ospf_neighbor *nbr) {
	/* Is DD duplicated? */
	if(dd->options == nbr->last_recv.options && dd->flags == nbr->last_recv.flags && dd->dd_seqnum == htonl(nbr->last_recv.dd_seqnum)) {
		return 1;
	}
...
static void ospf_db_desc(struct ip *iph, struct ospf_header *ospfh, struct stream *s, struct ospf_interface *oi, u_int16_t size) {
	...
	if(nbr == NULL) {
		zlog_warn("Packet[DD]: Unknown Neighbor %s", inet_ntoa(ospfh->router_id));
		return;
	}

	/* Check MTU. */
	if((OSPF_IF_PARAM(oi, mtu_ignore) == 0) && (ntohs(dd->mtu) > oi->ifp->mtu)) {
		zlog_warn("Packet[DD]: Neighbor %s MTU %u is larger than [%s]'s MTU %u", inet_ntoa(nbr->router_id), ntohs(dd->mtu), IF_NAME(oi), oi->ifp->mtu);
		return;
	}
   ...
   */
	if((oi->area->external_routing == OSPF_AREA_NSSA) && (CHECK_FLAG(nbr->options, OSPF_OPTION_NP)) && (!CHECK_FLAG(dd->options, OSPF_OPTION_NP))) {
		if(IS_DEBUG_OSPF_EVENT) {
			zlog_debug("Packet[DD]: Neighbour %s: Has NSSA capability, sends with N bit clear in DD options", inet_ntoa(nbr->router_id));
		}
	...
	if(CHECK_FLAG(dd->options, OSPF_OPTION_O) && !CHECK_FLAG(oi->ospf->config, OSPF_OPAQUE_CAPABLE)) {
		/*
	...
	switch(nbr->state) {
		case NSM_Down:
		case NSM_Attempt:
		case NSM_TwoWay: zlog_warn("Packet[DD]: Neighbor %s state is %s, packet discarded.", inet_ntoa(nbr->router_id), LOOKUP(ospf_nsm_state_msg, nbr->state)); break;
		case NSM_Init:
	 ...
	 through to case ExStart below.  */
		...
		case NSM_ExStart:
			/* Initial DBD */
			if((IS_SET_DD_ALL(dd->flags) == OSPF_DD_FLAG_ALL) && (size == OSPF_DB_DESC_MIN_SIZE)) {
				if(IPV4_ADDR_CMP(&nbr->router_id, &oi->ospf->router_id) > 0) {
					/* We're Slave---obey */
					zlog_info("Packet[DD]: Neighbor %s Negotiation done (Slave).", inet_ntoa(nbr->router_id));
					nbr->dd_seqnum = ntohl(dd->dd_seqnum);
			...
			/* Ack from the Slave */
			else if(!IS_SET_DD_MS(dd->flags) && !IS_SET_DD_I(dd->flags) && ntohl(dd->dd_seqnum) == nbr->dd_seqnum && IPV4_ADDR_CMP(&nbr->router_id, &oi->ospf->router_id) < 0) {
				zlog_info("Packet[DD]: Neighbor %s Negotiation done (Master).", inet_ntoa(nbr->router_id));
				/* Reset I, leaving MS */
				UNSET_FLAG(nbr->dd_flags, OSPF_DD_FLAG_I);
			} else {
				zlog_warn("Packet[DD]: Neighbor %s Negotiation fails.", inet_ntoa(nbr->router_id));
				break;
			}
			...
			if(CHECK_FLAG(oi->ospf->config, OSPF_OPAQUE_CAPABLE)) {
				if(IS_DEBUG_OSPF_EVENT) {
					zlog_debug("Neighbor[%s] is %sOpaque-capable.", inet_ntoa(nbr->router_id), CHECK_FLAG(nbr->options, OSPF_OPTION_O) ? "" : "NOT ");
				}

				if(!CHECK_FLAG(nbr->options, OSPF_OPTION_O) && IPV4_ADDR_SAME(&DR(oi), &nbr->address.u.prefix4)) {
					zlog_warn(
		...
		case NSM_Exchange:
			if(ospf_db_desc_is_dup(dd, nbr)) {
				if(IS_SET_DD_MS(nbr->dd_flags)) {
					...
					zlog_info("Packet[DD] (Master): Neighbor %s packet duplicated.", inet_ntoa(nbr->router_id));
				} else
				/* Slave: cause to retransmit the last Database Description. */
				{
					zlog_info("Packet[DD] [Slave]: Neighbor %s packet duplicated.", inet_ntoa(nbr->router_id));
			...
			if(IS_SET_DD_MS(dd->flags) != IS_SET_DD_MS(nbr->last_recv.flags)) {
				zlog_warn("Packet[DD]: Neighbor %s MS-bit mismatch.", inet_ntoa(nbr->router_id));
				OSPF_NSM_EVENT_SCHEDULE(nbr, NSM_SeqNumberMismatch);
				if(IS_DEBUG_OSPF_EVENT) {
					zlog_debug("Packet[DD]: dd->flags=%d, nbr->dd_flags=%d", dd->flags, nbr->dd_flags);
				}
				break;
			}
			...
			if(IS_SET_DD_I(dd->flags)) {
				zlog_info("Packet[DD]: Neighbor %s I-bit set.", inet_ntoa(nbr->router_id));
				OSPF_NSM_EVENT_SCHEDULE(nbr, NSM_SeqNumberMismatch);
				break;
			}
			...
			if(dd->options != nbr->options) {
				...
				zlog_warn("Packet[DD]: Neighbor %s options mismatch.", inet_ntoa(nbr->router_id));
				OSPF_NSM_EVENT_SCHEDULE(nbr, NSM_SeqNumberMismatch);
				break;
			}

			/* Check DD sequence number. */
			if((IS_SET_DD_MS(nbr->dd_flags) && ntohl(dd->dd_seqnum) != nbr->dd_seqnum) || (!IS_SET_DD_MS(nbr->dd_flags) && ntohl(dd->dd_seqnum) != nbr->dd_seqnum + 1)) {
				zlog_warn("Packet[DD]: Neighbor %s sequence number mismatch.", inet_ntoa(nbr->router_id));
				OSPF_NSM_EVENT_SCHEDULE(nbr, NSM_SeqNumberMismatch);
				break;
			}
		...
		case NSM_Full:
			...
			OSPF_NSM_EVENT_SCHEDULE(nbr, NSM_SeqNumberMismatch);
			break;
		default: zlog_warn("Packet[DD]: Neighbor %s NSM illegal status %u.", inet_ntoa(nbr->router_id), nbr->state); break;
	}
}
...
static void ospf_ls_req(struct ip *iph, struct ospf_header *ospfh, struct stream *s, struct ospf_interface *oi, u_int16_t size) {
	struct ospf_neighbor *nbr;
	u_int32_t ls_type;
	...
	nbr = ospf_nbr_lookup(oi, iph, ospfh);
	if(nbr == NULL) {
		zlog_warn("Link State Request: Unknown Neighbor %s.", inet_ntoa(ospfh->router_id));
		return;
	}
	...
	if(nbr->state != NSM_Exchange && nbr->state != NSM_Loading && nbr->state != NSM_Full) {
		zlog_warn(
			"Link State Request received from %s: "
			"Neighbor state is %s, packet discarded.",
			inet_ntoa(ospfh->router_id), LOOKUP(ospf_nsm_state_msg, nbr->state)
		);
		return;
	}
...
static struct list *ospf_ls_upd_list_lsa(struct ospf_neighbor *nbr, struct stream *s, struct ospf_interface *oi, size_t size) {
	u_int16_t count, sum;
	u_int32_t length;
	struct lsa_header *lsah;
	struct ospf_lsa *lsa;
	...
	for(; size >= OSPF_LSA_HEADER_SIZE && count > 0; size -= length, stream_forward_getp(s, length), count--) {
		lsah = (struct lsa_header *) STREAM_PNT(s);
	   ...
	   * chance to compress repeated messages in syslog on the other */
			zlog_warn(
				"Link State Update: LSA checksum error %x/%x, ID=%s from: nbr %s, router ID %s, adv router %s", sum, lsah->checksum, inet_ntoa(lsah->id), inet_ntoa(nbr->src), inet_ntoa(nbr->router_id),
       ...
       */
		...
		if(CHECK_FLAG(nbr->options, OSPF_OPTION_O)) {
#ifdef STRICT_OBIT_USAGE_CHECK
			if((IS_OPAQUE_LSA(lsah->type) && !CHECK_FLAG(lsah->options, OSPF_OPTION_O)) || (!IS_OPAQUE_LSA(lsah->type) && CHECK_FLAG(lsah->options, OSPF_OPTION_O))) {
				/*
			...
			if(lsah->type == OSPF_OPAQUE_AS_LSA && nbr->oi->area->external_routing != OSPF_AREA_DEFAULT) {
				if(IS_DEBUG_OSPF_EVENT) {
		...
		switch(lsah->type) {
			case OSPF_AS_EXTERNAL_LSA:
			case OSPF_OPAQUE_AS_LSA: lsa->area = NULL; break;
			case OSPF_OPAQUE_LINK_LSA:
				lsa->oi = oi; /* Remember incoming interface for flooding control. */
				/* Fallthrough */
			default: lsa->area = oi->area; break;
		}
		...
		if(IS_DEBUG_OSPF_EVENT) {
			zlog_debug("LSA[Type%d:%s]: %p new LSA created with Link State Update", lsa->data->type, inet_ntoa(lsa->data->id), (void *) lsa);
		}
...
static void ospf_ls_upd(struct ospf *ospf, struct ip *iph, struct ospf_header *ospfh, struct stream *s, struct ospf_interface *oi, u_int16_t size) {
	struct ospf_neighbor *nbr;
	struct list *lsas;
	struct listnode *node, *nnode;
	...
	nbr = ospf_nbr_lookup(oi, iph, ospfh);
	if(nbr == NULL) {
		zlog_warn("Link State Update: Unknown Neighbor %s on int: %s", inet_ntoa(ospfh->router_id), IF_NAME(oi));
		return;
	}
   ...
   */
	for(ALL_LIST_ELEMENTS(lsas, node, nnode, lsa)) {
		...
		if(IS_DEBUG_OSPF_NSSA) {
			...
			zlog_debug(
				"LSA Type-%d from %s, ID: %s, ADV: %s", lsa->data->type, inet_ntop(AF_INET, &ospfh->router_id, buf1, INET_ADDRSTRLEN), inet_ntop(AF_INET, &lsa->data->id, buf2, INET_ADDRSTRLEN),
				inet_ntop(AF_INET, &lsa->data->adv_router, buf3, INET_ADDRSTRLEN)
			);
		}
		...
		if(lsa->data->type == OSPF_ROUTER_LSA) {
			if(!IPV4_ADDR_SAME(&lsa->data->id, &lsa->data->adv_router)) {
				...
				zlog_err(
					...
					"Adv-ID[%s] != LS-ID[%s]",
					inet_ntop(AF_INET, &ospfh->router_id, buf1, INET_ADDRSTRLEN), inet_ntop(AF_INET, &lsa->data->id, buf2, INET_ADDRSTRLEN), inet_ntop(AF_INET, &lsa->data->adv_router, buf3, INET_ADDRSTRLEN)
				);
	 ...
	 then take the following actions: */
		...
		if(IS_LSA_MAXAGE(lsa) && !current && ospf_check_nbr_status(oi->ospf)) {
			/* (4a) Response Link State Acknowledgment. */
			ospf_ls_ack_send(nbr, lsa);

			/* (4b) Discard LSA. */
			if(IS_DEBUG_OSPF(lsa, LSA)) {
				zlog_debug("Link State Update[%s]: LS age is equal to MaxAge.", dump_lsa_key(lsa));
			}
			DISCARD_LSA(lsa, 3);
		}

		if(IS_OPAQUE_LSA(lsa->data->type) && IPV4_ADDR_SAME(&lsa->data->adv_router, &oi->ospf->router_id)) {
			/*
       ...
       */
		...
		if(lsa->data->type == OSPF_NETWORK_LSA) {
			...
			for(ALL_LIST_ELEMENTS(oi->ospf->oiflist, oinode, oinnode, out_if)) {
				...
				if((IPV4_ADDR_SAME(&out_if->address->u.prefix4, &lsa->data->id)) && (!(IPV4_ADDR_SAME(&oi->ospf->router_id, &lsa->data->adv_router)))) {
					if(out_if->network_lsa_self) {
						ospf_lsa_flush_area(lsa, out_if->area);
						if(IS_DEBUG_OSPF_EVENT) {
							zlog_debug("ospf_lsa_discard() in ospf_ls_upd() point 9: lsa %p Type-%d", (void *) lsa, (int) lsa->data->type);
						}
						ospf_lsa_discard(lsa);
						Flag = 1;
					}
	 ...
	 the database copy the following steps must be performed.
		...
		if(current == NULL || (ret = ospf_lsa_more_recent(current, lsa)) < 0) {
	 ...
	 Link State Update packet. */
		...
		if(ospf_ls_request_lookup(nbr, lsa)) {
			OSPF_NSM_EVENT_SCHEDULE(nbr, NSM_BadLSReq);
			zlog_warn("LSA[%s] instance exists on Link state request list", dump_lsa_key(lsa));
	 ...
	 flushed before any new LSA instance can be introduced). */
		...
		{
			if(IS_LSA_MAXAGE(current) && current->data->ls_seqnum == htonl(OSPF_MAX_SEQUENCE_NUMBER)) {
				DISCARD_LSA(lsa, 7);
			}
	     ...
	     recent) LSA instance. */
				...
				if(tv_cmp(tv_sub(now, current->tv_orig), msec2tv(ospf->min_ls_arrival)) >= 0) {
...
static void ospf_ls_ack(struct ip *iph, struct ospf_header *ospfh, struct stream *s, struct ospf_interface *oi, u_int16_t size) {
	struct ospf_neighbor *nbr;
	...
	nbr = ospf_nbr_lookup(oi, iph, ospfh);
	if(nbr == NULL) {
		zlog_warn("Link State Acknowledgment: Unknown Neighbor %s.", inet_ntoa(ospfh->router_id));
		return;
	}
...
static struct stream *ospf_recv_packet(int fd, struct interface **ifp, struct stream *ibuf) {
	int ret;
	struct ip *iph;
	u_int16_t ip_len;
	ifindex_t ifindex = 0;
...
static struct ospf_interface *ospf_associate_packet_vl(struct ospf *ospf, struct interface *ifp, struct ip *iph, struct ospf_header *ospfh) {
	struct ospf_interface *rcv_oi;
	struct ospf_vl_data *vl_data;
	struct ospf_area *vl_area;
	struct listnode *node;

	if(IN_MULTICAST(ntohl(iph->ip_dst.s_addr)) || !OSPF_IS_AREA_BACKBONE(ospfh)) {
		return NULL;
	}
   ...
   */
	...
	for(ALL_LIST_ELEMENTS_RO(ospf->vlinks, node, vl_data)) {
		...
		if(OSPF_AREA_SAME(&vl_area, &rcv_oi->area) && IPV4_ADDR_SAME(&vl_data->vl_peer, &ospfh->router_id)) {
			if(IS_DEBUG_OSPF_EVENT) {
				zlog_debug("associating packet with %s", IF_NAME(vl_data->vl_oi));
			}
...
static int ospf_check_area_id(struct ospf_interface *oi, struct ospf_header *ospfh) {
...
/* Unbound socket will accept any Raw IP packets if proto is matched.
   ...
   i/f network mask. */
static int ospf_check_network_mask(struct ospf_interface *oi, struct in_addr ip_src) {
	...
	switch(pkt_auth_type) {
		case OSPF_AUTH_NULL: /* RFC2328 D.5.1 */
			if(OSPF_AUTH_NULL != (iface_auth_type = ospf_auth_type(oi))) {
				if(IS_DEBUG_OSPF_PACKET(ospfh->type - 1, RECV)) {
					zlog_warn("interface %s: auth-type mismatch, local %s, rcvd Null", IF_NAME(oi), LOOKUP(ospf_auth_type_str, iface_auth_type));
				}
				return 0;
			}
			if(!ospf_check_sum(ospfh)) {
				if(IS_DEBUG_OSPF_PACKET(ospfh->type - 1, RECV)) {
					zlog_warn("interface %s: Null auth OK, but checksum error, Router-ID %s", IF_NAME(oi), inet_ntoa(ospfh->router_id));
				}
				return 0;
			}
			return 1;
		case OSPF_AUTH_SIMPLE: /* RFC2328 D.5.2 */
			if(OSPF_AUTH_SIMPLE != (iface_auth_type = ospf_auth_type(oi))) {
				if(IS_DEBUG_OSPF_PACKET(ospfh->type - 1, RECV)) {
					zlog_warn("interface %s: auth-type mismatch, local %s, rcvd Simple", IF_NAME(oi), LOOKUP(ospf_auth_type_str, iface_auth_type));
				}
				return 0;
			}
			...
			if(!ospf_check_sum(ospfh)) {
				if(IS_DEBUG_OSPF_PACKET(ospfh->type - 1, RECV)) {
					zlog_warn("interface %s: Simple auth OK, checksum error, Router-ID %s", IF_NAME(oi), inet_ntoa(ospfh->router_id));
				}
				return 0;
			}
			return 1;
		case OSPF_AUTH_CRYPTOGRAPHIC: /* RFC2328 D.5.3 */
			if(OSPF_AUTH_CRYPTOGRAPHIC != (iface_auth_type = ospf_auth_type(oi))) {
				if(IS_DEBUG_OSPF_PACKET(ospfh->type - 1, RECV)) {
					zlog_warn("interface %s: auth-type mismatch, local %s, rcvd Cryptographic", IF_NAME(oi), LOOKUP(ospf_auth_type_str, iface_auth_type));
				}
				return 0;
			}
			if(ospfh->checksum) {
			...
			if(NULL == (ck = listgetdata(listtail(OSPF_IF_PARAM(oi, auth_crypt)))) || ospfh->u.crypt.key_id != ck->key_id ||
		...
		default:
			if(IS_DEBUG_OSPF_PACKET(ospfh->type - 1, RECV)) {
				zlog_warn("interface %s: invalid packet auth-type (%02x)", IF_NAME(oi), pkt_auth_type);
			}
			return 0;
	}
}
...
static int ospf_check_sum(struct ospf_header *ospfh) {
	...
	if(ret != sum) {
		zlog_info("ospf_check_sum(): checksum mismatch, my %X, his %X", ret, sum);
		return 0;
	}
...
static unsigned ospf_router_lsa_links_examin(struct router_lsa_link *link, u_int16_t linkbytes, const u_int16_t num_links) {
	...
	if(counted_links != num_links) {
		if(IS_DEBUG_OSPF_PACKET(0, RECV)) {
			zlog_debug("%s: %u link blocks declared, %u present", __func__, num_links, counted_links);
		}
		return MSG_NG;
	}
	return MSG_OK;
...
static unsigned ospf_lsa_examin(struct lsa_header *lsah, const u_int16_t lsalen, const u_char headeronly) {
	unsigned ret;
	struct router_lsa *rlsa;
	if(lsah->type < OSPF_MAX_LSA && ospf_lsa_minlen[lsah->type] && lsalen < OSPF_LSA_HEADER_SIZE + ospf_lsa_minlen[lsah->type]) {
		if(IS_DEBUG_OSPF_PACKET(0, RECV)) {
			zlog_debug("%s: undersized (%u B) %s", __func__, lsalen, LOOKUP(ospf_lsa_type_msg, lsah->type));
		}
		return MSG_NG;
	}
	switch(lsah->type) {
		case OSPF_ROUTER_LSA:
			...
			ret = ospf_router_lsa_links_examin(
				(struct router_lsa_link *) rlsa->link, lsalen - OSPF_LSA_HEADER_SIZE - 4, /* skip: basic header, "flags", 0, "# links" */
   ...
   * (basic LSA header size is already a multiple of 4). */
	...
	if(ret != MSG_OK && IS_DEBUG_OSPF_PACKET(0, RECV)) {
		zlog_debug("%s: alignment error in %s", __func__, LOOKUP(ospf_lsa_type_msg, lsah->type));
	}
	return ret;
}
...
static unsigned ospf_lsaseq_examin(
	...
	while(length) {
		...
		if(headeronly) {
			...
			if(lsalen > length) {
				if(IS_DEBUG_OSPF_PACKET(0, RECV)) {
					zlog_debug("%s: anomaly in LSA #%u: declared length is %u B, buffered length is %zu B", __func__, counted_lsas, lsalen, length);
				}
				return MSG_NG;
			}
...
static unsigned ospf_packet_examin(struct ospf_header *oh, const unsigned bytesonwire) {
   ...
   * in the OSPF header "length" field. */
	...
	if(ntohs(oh->auth_type) != OSPF_AUTH_CRYPTOGRAPHIC) {
		...
		if(oh->u.crypt.auth_data_len != OSPF_AUTH_MD5_SIZE) {
			if(IS_DEBUG_OSPF_PACKET(0, RECV)) {
				zlog_debug("%s: unsupported crypto auth length (%u B)", __func__, oh->u.crypt.auth_data_len);
			}
			return MSG_NG;
		}
	...
	if(bytesdeclared + bytesauth > bytesonwire) {
		if(IS_DEBUG_OSPF_PACKET(0, RECV)) {
			zlog_debug("%s: packet length error (%u real, %u+%u declared)", __func__, bytesonwire, bytesdeclared, bytesauth);
		}
		return MSG_NG;
	}
	/* Length, 2nd approximation. The type-specific constraint is checked
     against declared length, not amount of bytes on wire. */
	if(oh->type >= OSPF_MSG_HELLO && oh->type <= OSPF_MSG_LS_ACK && bytesdeclared < OSPF_HEADER_SIZE + ospf_packet_minlen[oh->type]) {
		if(IS_DEBUG_OSPF_PACKET(0, RECV)) {
			zlog_debug("%s: undersized (%u B) %s packet", __func__, bytesdeclared, LOOKUP(ospf_packet_type_str, oh->type));
		}
		return MSG_NG;
	}
	switch(oh->type) {
       ...
       by N>=0 header-only LSAs. */
			ret = ospf_lsaseq_examin(
				(struct lsa_header *) ((caddr_t) oh + OSPF_HEADER_SIZE + OSPF_DB_DESC_MIN_SIZE), bytesdeclared - OSPF_HEADER_SIZE - OSPF_DB_DESC_MIN_SIZE, 1, /* header-only LSAs */
				0
			);
			break;
		case OSPF_MSG_LS_REQ:
			/* RFC2328 A.3.4, packet header followed by N>=0 12-bytes request blocks. */
			ret = (bytesdeclared - OSPF_HEADER_SIZE - OSPF_LS_REQ_MIN_SIZE) % OSPF_LSA_KEY_SIZE ? MSG_NG : MSG_OK;
			break;
       ...
       by N>=0 full LSAs (with N declared beforehand). */
			...
			ret = ospf_lsaseq_examin(
				(struct lsa_header *) ((caddr_t) lsupd + OSPF_LS_UPD_MIN_SIZE), bytesdeclared - OSPF_HEADER_SIZE - OSPF_LS_UPD_MIN_SIZE, 0, /* full LSAs */
		...
		case OSPF_MSG_LS_ACK:
			...
			ret = ospf_lsaseq_examin(
				(struct lsa_header *) ((caddr_t) oh + OSPF_HEADER_SIZE + OSPF_LS_ACK_MIN_SIZE), bytesdeclared - OSPF_HEADER_SIZE - OSPF_LS_ACK_MIN_SIZE, 1, /* header-only LSAs */
				0
			);
			break;
		default:
...
static int ospf_verify_header(struct stream *ibuf, struct ospf_interface *oi, struct ip *iph, struct ospf_header *ospfh) {
	/* Check Area ID. */
	if(!ospf_check_area_id(oi, ospfh)) {
		zlog_warn("interface %s: ospf_read invalid Area ID %s.", IF_NAME(oi), inet_ntoa(ospfh->area_id));
		return -1;
	}

	/* Check network mask, Silently discarded. */
	if(!ospf_check_network_mask(oi, iph->ip_src)) {
		zlog_warn("interface %s: ospf_read network address is not same [%s]", IF_NAME(oi), inet_ntoa(iph->ip_src));
		return -1;
	}
...
int ospf_read(struct thread *thread) {
	...
	if(ospf_if_lookup_by_local_addr(ospf, NULL, iph->ip_src)) {
		if(IS_DEBUG_OSPF_PACKET(0, RECV)) {
			zlog_debug("ospf_read[%s]: Dropping self-originated packet", inet_ntoa(iph->ip_src));
		}
		return 0;
	}
	...
	if(oi && OSPF_IF_PASSIVE_STATUS(oi) == OSPF_IF_PASSIVE) {
		...
		if(IS_DEBUG_OSPF_EVENT) {
			zlog_debug(
				...
				inet_ntop(AF_INET, &ospfh->router_id, buf[0], sizeof(buf[0])), inet_ntop(AF_INET, &iph->ip_dst, buf[1], sizeof(buf[1])), inet_ntop(AF_INET, &oi->address->u.prefix4, buf[2], sizeof(buf[2]))
			);
		}
   ...
   */
	if((oi == NULL) || (OSPF_IS_AREA_ID_BACKBONE(ospfh->area_id) && !OSPF_IS_AREA_ID_BACKBONE(oi->area->area_id))) {
   ...
   */
		...
		if(IS_DEBUG_OSPF_EVENT) {
			zlog_warn("Packet from [%s] received on wrong link %s", inet_ntoa(iph->ip_src), ifp->name);
		}
		return 0;
		...
		zlog_warn(
			...
			inet_ntop(AF_INET, &iph->ip_src, buf[0], sizeof(buf[0])), inet_ntop(AF_INET, &iph->ip_dst, buf[1], sizeof(buf[1])), ifp->name, if_flag_dump(ifp->flags)
		);
   ...
   */
	if(iph->ip_dst.s_addr == htonl(OSPF_ALLDROUTERS) && (oi->state != ISM_DR && oi->state != ISM_Backup)) {
		zlog_warn("Dropping packet for AllDRouters from [%s] via [%s] (ISM: %s)", inet_ntoa(iph->ip_src), IF_NAME(oi), LOOKUP(ospf_ism_state_msg, oi->state));
		/* Try to fix multicast membership. */
	...
	if(IS_DEBUG_OSPF_PACKET(ospfh->type - 1, RECV)) {
		...
		zlog_debug("%s received from [%s] via [%s]", LOOKUP(ospf_packet_type_str, ospfh->type), inet_ntoa(ospfh->router_id), IF_NAME(oi));
	...
	switch(ospfh->type) {
		case OSPF_MSG_HELLO: ospf_hello(iph, ospfh, ibuf, oi, length); break;
		case OSPF_MSG_DB_DESC: ospf_db_desc(iph, ospfh, ibuf, oi, length); break;
		case OSPF_MSG_LS_REQ: ospf_ls_req(iph, ospfh, ibuf, oi, length); break;
		case OSPF_MSG_LS_UPD: ospf_ls_upd(ospf, iph, ospfh, ibuf, oi, length); break;
		case OSPF_MSG_LS_ACK: ospf_ls_ack(iph, ospfh, ibuf, oi, length); break;
		default: zlog(NULL, LOG_WARNING, "interface %s: OSPF packet header type %d is illegal", IF_NAME(oi), ospfh->type); break;
	}
...
/* Make OSPF header. */
static void ospf_make_header(int type, struct ospf_interface *oi, struct stream *s) {
	struct ospf_header *ospfh;
...
static int ospf_make_auth(struct ospf_interface *oi, struct ospf_header *ospfh) {
	...
	switch(ospf_auth_type(oi)) {
		case OSPF_AUTH_NULL:
			...
			break;
		case OSPF_AUTH_SIMPLE: memcpy(ospfh->u.auth_data, OSPF_IF_PARAM(oi, auth_simple), OSPF_AUTH_SIMPLE_SIZE); break;
...
static void ospf_fill_header(struct ospf_interface *oi, struct stream *s, u_int16_t length) {
	struct ospf_header *ospfh;
...
static int ospf_make_hello(struct ospf_interface *oi, struct stream *s) {
	...
	if(IS_DEBUG_OSPF_EVENT) {
		zlog_debug("make_hello: options: %x, int: %s", OPTIONS(oi), IF_NAME(oi));
	}
				...
				if(nbr->state != NSM_Attempt) {	     /* Ignore Down neighbor. */
					if(nbr->state != NSM_Down) { /* This is myself for DR election. */
						if(!IPV4_ADDR_SAME(&nbr->router_id, &oi->ospf->router_id)) {
							...
							if(nbr->d_router.s_addr != 0 && IPV4_ADDR_SAME(&nbr->d_router, &oi->address->u.prefix4) && IPV4_ADDR_SAME(&nbr->bd_router, &oi->address->u.prefix4)) {
								flag = 1;
							}
...
static int ospf_make_db_desc(struct ospf_interface *oi, struct ospf_neighbor *nbr, struct stream *s) {
	struct ospf_lsa *lsa;
	...
	for(i = OSPF_MIN_LSA; i < OSPF_MAX_LSA; i++) {
		...
		for(rn = route_top(table); rn; rn = route_next(rn)) {
			if((lsa = rn->info) != NULL) {
				if(IS_OPAQUE_LSA(lsa->data->type) && (!CHECK_FLAG(options, OSPF_OPTION_O))) {
...
static int ospf_make_ls_req_func(struct stream *s, u_int16_t *length, unsigned long delta, struct ospf_neighbor *nbr, struct ospf_lsa *lsa) {
	struct ospf_interface *oi;
...
static int ospf_make_ls_ack(struct ospf_interface *oi, struct list *ack, struct stream *s) {
	struct listnode *node, *nnode;
...
static void ospf_poll_send(struct ospf_nbr_nbma *nbr_nbma) {
	...
	if(nbr_nbma->priority == 0 && oi->state != ISM_DR && oi->state != ISM_Backup) {
		return;
	}
...
int ospf_poll_timer(struct thread *thread) {
	...
	if(IS_DEBUG_OSPF(nsm, NSM_TIMERS)) {
		zlog(NULL, LOG_DEBUG, "NSM[%s:%s]: Timer (Poll timer expire)", IF_NAME(nbr_nbma->oi), inet_ntoa(nbr_nbma->addr));
	}

	ospf_poll_send(nbr_nbma);

	if(nbr_nbma->v_poll > 0) {
		OSPF_POLL_TIMER_ON(nbr_nbma->t_poll, ospf_poll_timer, nbr_nbma->v_poll);
	}
...
int ospf_hello_reply_timer(struct thread *thread) {
	...
	if(IS_DEBUG_OSPF(nsm, NSM_TIMERS)) {
		zlog(NULL, LOG_DEBUG, "NSM[%s:%s]: Timer (hello-reply timer expire)", IF_NAME(nbr->oi), inet_ntoa(nbr->router_id));
	}
...
void ospf_hello_send(struct ospf_interface *oi) {
	...
	if(oi->type == OSPF_IFTYPE_NBMA) {
		...
		for(rn = route_top(oi->nbrs); rn; rn = route_next(rn)) {
		    ...
		    exist).  */
						if(PRIORITY(oi) == 0 && IPV4_ADDR_CMP(&DR(oi), &nbr->address.u.prefix4) && IPV4_ADDR_CMP(&BDR(oi), &nbr->address.u.prefix4)) {
							continue;
						}
...
void ospf_ls_upd_send_lsa(struct ospf_neighbor *nbr, struct ospf_lsa *lsa, int flag) {
	struct list *update;
...
static struct ospf_packet *ospf_ls_upd_packet_new(struct list *update, struct ospf_interface *oi) {
	...
	if((OSPF_LS_UPD_MIN_SIZE + ntohs(lsa->data->length)) > ospf_packet_max(oi)) {
		if(!warned) {
		...
		if(IS_DEBUG_OSPF_PACKET(0, SEND)) {
			zlog_debug(
				...
				inet_ntoa(lsa->data->id), ntohs(lsa->data->length), inet_ntoa(lsa->data->adv_router)
			);
		}
       ...
       */
		size = ntohs(lsa->data->length) + (oi->ifp->mtu - ospf_packet_max(oi)) + OSPF_LS_UPD_MIN_SIZE;
	} else {
		size = oi->ifp->mtu;
	}
	...
	if(size > OSPF_MAX_PACKET_SIZE) {
		zlog_warn(
			...
			inet_ntoa(lsa->data->id), ntohs(lsa->data->length), (long int) size
		);
...
static void ospf_ls_upd_queue_send(struct ospf_interface *oi, struct list *update, struct in_addr addr) {
	struct ospf_packet *op;
	u_int16_t length = OSPF_HEADER_SIZE;

	if(IS_DEBUG_OSPF_EVENT) {
		zlog_debug("listcount = %d, [%s]dst %s", listcount(update), IF_NAME(oi), inet_ntoa(addr));
	}
...
static int ospf_ls_upd_send_queue_event(struct thread *thread) {
	...
	if(again != 0) {
		...
		oi->t_ls_upd_event = thread_add_event(master, ospf_ls_upd_send_queue_event, oi, 0);
	}
...
static void ospf_ls_ack_send_list(struct ospf_interface *oi, struct list *ack, struct in_addr dst) {
	struct ospf_packet *op;
	u_int16_t length = OSPF_HEADER_SIZE;
...
static int ospf_ls_ack_send_event(struct thread *thread) {
	...
	while(listcount(oi->ls_ack_direct.ls_ack)) {
		ospf_ls_ack_send_list(oi, oi->ls_ack_direct.ls_ack, oi->ls_ack_direct.dst);
	}
...
void ospf_ls_ack_send(struct ospf_neighbor *nbr, struct ospf_lsa *lsa) {
	...
	if(oi->t_ls_ack_direct == NULL) {
		oi->t_ls_ack_direct = thread_add_event(master, ospf_ls_ack_send_event, oi, 0);
	}
}
