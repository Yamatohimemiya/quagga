...
#define OSPF_LSA_REFRESHER_SLOTS ((OSPF_LS_REFRESH_TIME + OSPF_LS_REFRESH_SHIFT) / 10 + 1)
...
#define LSA_OPTIONS_GET(area) (((area)->external_routing == OSPF_AREA_DEFAULT) ? OSPF_OPTION_E : 0)
#define LSA_OPTIONS_NSSA_GET(area) (((area)->external_routing == OSPF_AREA_NSSA) ? OSPF_OPTION_NP : 0)

#define OSPF_TIMER_ON(T, F, V) \
	do { \
		if(!(T)) (T) = thread_add_timer(master, (F), ospf, (V)); \
	} while(0)

#define OSPF_AREA_TIMER_ON(T, F, V) \
	do { \
		if(!(T)) (T) = thread_add_timer(master, (F), area, (V)); \
	} while(0)

#define OSPF_POLL_TIMER_ON(T, F, V) \
	do { \
		if(!(T)) (T) = thread_add_timer(master, (F), nbr_nbma, (V)); \
	} while(0)
...
extern int ospf_network_unset(struct ospf *, struct prefix_ipv4 *, struct in_addr);
...
extern int ospf_area_nssa_translator_role_set(struct ospf *, struct in_addr, int);
extern int ospf_area_export_list_set(struct ospf *, struct ospf_area *, const char *);
extern int ospf_area_export_list_unset(struct ospf *, struct ospf_area *);
extern int ospf_area_import_list_set(struct ospf *, struct ospf_area *, const char *);
...
extern int ospf_nbr_nbma_poll_interval_set(struct ospf *, struct in_addr, unsigned int);
...
extern struct ospf_nbr_nbma *ospf_nbr_nbma_lookup(struct ospf *, struct in_addr);
extern struct ospf_nbr_nbma *ospf_nbr_nbma_lookup_next(struct ospf *, struct in_addr *, int);
...
extern struct ospf_area *ospf_area_lookup_by_area_id(struct ospf *, struct in_addr);
