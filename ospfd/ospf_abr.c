...
static void ospf_area_range_add(struct ospf_area *area, struct ospf_area_range *range) {
...
static void ospf_area_range_delete(struct ospf_area *area, struct route_node *rn) {
	struct ospf_area_range *range = rn->info;

	if(range->specifics != 0) {
		ospf_delete_discard_route(area->ospf->new_table, (struct prefix_ipv4 *) &rn->p);
	}
...
struct ospf_area_range *ospf_area_range_lookup_next(struct ospf_area *area, struct in_addr *range_net, int first) {
	struct route_node *rn;
	struct prefix_ipv4 p;
...
int ospf_area_range_set(struct ospf *ospf, struct in_addr area_id, struct prefix_ipv4 *p, int advertise) {
	struct ospf_area *area;
	struct ospf_area_range *range;
	...
	if(range != NULL) {
		if((CHECK_FLAG(range->flags, OSPF_AREA_RANGE_ADVERTISE) && !CHECK_FLAG(advertise, OSPF_AREA_RANGE_ADVERTISE)) || (!CHECK_FLAG(range->flags, OSPF_AREA_RANGE_ADVERTISE) && CHECK_FLAG(advertise, OSPF_AREA_RANGE_ADVERTISE))) {
...
int ospf_area_range_cost_set(struct ospf *ospf, struct in_addr area_id, struct prefix_ipv4 *p, u_int32_t cost) {
	struct ospf_area *area;
	struct ospf_area_range *range;
	int ret = OSPF_AREA_ID_FORMAT_ADDRESS;
...
int ospf_area_range_substitute_set(struct ospf *ospf, struct in_addr area_id, struct prefix_ipv4 *p, struct prefix_ipv4 *s) {
	struct ospf_area *area;
	struct ospf_area_range *range;
	int ret = OSPF_AREA_ID_FORMAT_ADDRESS;
	...
	if(range != NULL) {
		if(!CHECK_FLAG(range->flags, OSPF_AREA_RANGE_ADVERTISE) || !CHECK_FLAG(range->flags, OSPF_AREA_RANGE_SUBSTITUTE)) {
			ospf_schedule_abr_task(ospf);
		}
	} else {
		range = ospf_area_range_new(p);
...
static int ospf_abr_nssa_am_elected(struct ospf_area *area) {
	...
	LSDB_LOOP(ROUTER_LSDB(area), rn, lsa) {
		/* sanity checks */
		if(!lsa || (lsa->data->type != OSPF_ROUTER_LSA) || IS_LSA_SELF(lsa)) {
			continue;
		}
	...
	if(IS_DEBUG_OSPF_NSSA) {
		zlog_debug("ospf_abr_nssa_am_elected: best electable ABR is: %s", (best) ? inet_ntoa(*best) : "<none>");
	}
...
void ospf_check_abr_status(struct ospf *ospf) {
	...
	if(IS_DEBUG_OSPF_EVENT) {
		...
		zlog_debug("ospf_check_abr_status(): bb_act_attached: %d", bb_act_attached);
		zlog_debug("ospf_check_abr_status(): areas_configured: %d", areas_configured);
		zlog_debug("ospf_check_abr_status(): areas_act_attached: %d", areas_act_attached);
	}
...
static void ospf_abr_update_aggregate(struct ospf_area_range *range, struct ospf_route * or, struct ospf_area *area) {
	if(IS_DEBUG_OSPF_EVENT) {
		zlog_debug("ospf_abr_update_aggregate(): Start");
	}

	if(CHECK_FLAG(area->stub_router_state, OSPF_AREA_IS_STUB_ROUTED) && (range->cost != OSPF_STUB_MAX_METRIC_SUMMARY_COST)) {
		range->cost = OSPF_STUB_MAX_METRIC_SUMMARY_COST;
		if(IS_DEBUG_OSPF_EVENT) {
			zlog_debug("ospf_abr_update_aggregate(): use summary max-metric 0x%08x", range->cost);
		}
	} else if(range->cost_config != OSPF_AREA_RANGE_COST_UNSPEC) {
		if(IS_DEBUG_OSPF_EVENT) {
			zlog_debug("ospf_abr_update_aggregate(): use configured cost %d", range->cost_config);
		}
		...
		if(range->specifics == 0) {
			if(IS_DEBUG_OSPF_EVENT) {
				zlog_debug("ospf_abr_update_aggregate(): use or->cost %d", or->cost);
			}
...
static int ospf_abr_translate_nssa(struct ospf_area *area, struct ospf_lsa *lsa) {
   ...
   */
	...
	if(!CHECK_FLAG(lsa->data->options, OSPF_OPTION_NP)) {
		if(IS_DEBUG_OSPF_NSSA) {
			zlog_debug("ospf_abr_translate_nssa(): LSA Id %s, P-bit off, NO Translation", inet_ntoa(lsa->data->id));
		}
		return 1;
	}

	if(IS_DEBUG_OSPF_NSSA) {
		zlog_debug("ospf_abr_translate_nssa(): LSA Id %s, TRANSLATING 7 to 5", inet_ntoa(lsa->data->id));
	}
...
void ospf_abr_announce_network_to_area(struct prefix_ipv4 *p, u_int32_t cost, struct ospf_area *area) {
	...
	old = ospf_lsa_lookup_by_prefix(area->lsdb, OSPF_SUMMARY_LSA, (struct prefix_ipv4 *) p, area->ospf->router_id);
	if(old) {
		if(IS_DEBUG_OSPF_EVENT) {
		...
		if((GET_METRIC(sl->metric) == full_cost) && ((old->flags & OSPF_LSA_IN_MAXAGE) == 0)) {
			...
			if(!lsa) {
				...
				zlog_warn("%s: Could not refresh %s to %s", __func__, buf, inet_ntoa(area->area_id));
				return;
			}
		...
		if(!lsa) {
			...
			prefix2str((struct prefix *) p, buf, sizeof(buf));
			zlog_warn("%s: Could not originate %s to %s", __func__, buf, inet_ntoa(area->area_id));
			return;
		}
...
static int ospf_abr_nexthops_belong_to_area(struct ospf_route * or, struct ospf_area *area) {
	struct listnode *node, *nnode;
...
static int ospf_abr_plist_in_check(struct ospf_area *area, struct ospf_route * or, struct prefix_ipv4 *p) {
	if(PREFIX_NAME_IN(area)) {
		if(PREFIX_LIST_IN(area) == NULL) {
			PREFIX_LIST_IN(area) = prefix_list_lookup(AFI_IP, PREFIX_NAME_IN(area));
		}
...
static int ospf_abr_plist_out_check(struct ospf_area *area, struct ospf_route * or, struct prefix_ipv4 *p) {
	if(PREFIX_NAME_OUT(area)) {
		if(PREFIX_LIST_OUT(area) == NULL) {
			PREFIX_LIST_OUT(area) = prefix_list_lookup(AFI_IP, PREFIX_NAME_OUT(area));
		}
...
static void ospf_abr_announce_network(struct ospf *ospf, struct prefix_ipv4 *p, struct ospf_route * or) {
	struct ospf_area_range *range;
	...
	for(ALL_LIST_ELEMENTS_RO(ospf->areas, node, area)) {
		if(IS_DEBUG_OSPF_EVENT) {
			zlog_debug("ospf_abr_announce_network(): looking at area %s", inet_ntoa(area->area_id));
		}
		...
		if(or->path_type == OSPF_PATH_INTRA_AREA) {
			...
			if((range = ospf_area_range_match(or_area, p)) && !ospf_area_is_transit(area)) {
...
static int ospf_abr_should_announce(struct ospf *ospf, struct prefix_ipv4 *p, struct ospf_route * or) {
	struct ospf_area *area;
...
static void ospf_abr_process_network_rt(struct ospf *ospf, struct route_table *rt) {
	struct ospf_area *area;
	...
	for(rn = route_top(rt); rn; rn = route_next(rn)) {
		...
		if(!(area = ospf_area_lookup_by_area_id(ospf, or->u.std.area_id))) {
			if(IS_DEBUG_OSPF_EVENT) {
				zlog_debug("ospf_abr_process_network_rt(): area %s no longer exists", inet_ntoa(or->u.std.area_id));
			}
			continue;
		}

		if(IS_DEBUG_OSPF_EVENT) {
			zlog_debug("ospf_abr_process_network_rt(): this is a route to %s/%d", inet_ntoa(rn->p.u.prefix4), rn->p.prefixlen);
		}
		...
		if(or->path_type == OSPF_PATH_INTRA_AREA && !ospf_abr_should_announce(ospf, (struct prefix_ipv4 *) &rn->p, or)) {
			if(IS_DEBUG_OSPF_EVENT) {
		...
		if(or->path_type == OSPF_PATH_INTRA_AREA && !ospf_abr_plist_out_check(area, or, (struct prefix_ipv4 *) &rn->p)) {
			if(IS_DEBUG_OSPF_EVENT) {
		...
		if((or->path_type == OSPF_PATH_INTER_AREA) && !OSPF_IS_AREA_ID_BACKBONE(or->u.std.area_id)) {
			if(IS_DEBUG_OSPF_EVENT) {
				zlog_debug("ospf_abr_process_network_rt():"
		...
		if((ospf->abr_type == OSPF_ABR_CISCO) || (ospf->abr_type == OSPF_ABR_IBM)) {
			if(!ospf_act_bb_connection(ospf) && or->path_type != OSPF_PATH_INTRA_AREA) {
				if(IS_DEBUG_OSPF_EVENT) {
...
static void ospf_abr_announce_rtr_to_area(struct prefix_ipv4 *p, u_int32_t cost, struct ospf_area *area) {
	struct ospf_lsa *lsa, *old = NULL;
	...
	old = ospf_lsa_lookup_by_prefix(area->lsdb, OSPF_ASBR_SUMMARY_LSA, p, area->ospf->router_id);
	if(old) {
		if(IS_DEBUG_OSPF_EVENT) {
	...
	if(old && (GET_METRIC(slsa->metric) == cost) && ((old->flags & OSPF_LSA_IN_MAXAGE) == 0)) {
		if(IS_DEBUG_OSPF_EVENT) {
		...
		if(!lsa) {
			...
			prefix2str((struct prefix *) p, buf, sizeof(buf));
			zlog_warn("%s: Could not refresh/originate %s to %s", __func__, buf, inet_ntoa(area->area_id));
			return;
		}
...
static void ospf_abr_announce_rtr(struct ospf *ospf, struct prefix_ipv4 *p, struct ospf_route * or) {
	struct listnode *node;
	struct ospf_area *area;
	...
	for(ALL_LIST_ELEMENTS_RO(ospf->areas, node, area)) {
		if(IS_DEBUG_OSPF_EVENT) {
			zlog_debug("ospf_abr_announce_rtr(): looking at area %s", inet_ntoa(area->area_id));
		}
...
static void ospf_abr_process_router_rt(struct ospf *ospf, struct route_table *rt) {
	struct ospf_route * or ;
	struct route_node *rn;
	struct list *l;
	...
	for(rn = route_top(rt); rn; rn = route_next(rn)) {
		...
		if(IS_DEBUG_OSPF_EVENT) {
			zlog_debug("ospf_abr_process_router_rt(): this is a route to %s", inet_ntoa(rn->p.u.prefix4));
		}

		for(ALL_LIST_ELEMENTS(l, node, nnode, or)) {
			if(!ospf_area_lookup_by_area_id(ospf, or->u.std.area_id)) {
				if(IS_DEBUG_OSPF_EVENT) {
					zlog_debug("ospf_abr_process_router_rt(): area %s no longer exists", inet_ntoa(or->u.std.area_id));
				}
				continue;
			}
			...
			if(!flag) {
				best = ospf_find_asbr_route(ospf, rt, (struct prefix_ipv4 *) &rn->p);
				flag = 1;
			}
...
static void ospf_abr_unapprove_translates(struct ospf *ospf) /* For NSSA Translations */
{
	struct ospf_lsa *lsa;
	struct route_node *rn;
...
static void ospf_abr_announce_aggregates(struct ospf *ospf) {
	...
	for(ALL_LIST_ELEMENTS_RO(ospf->areas, node, area)) {
		if(IS_DEBUG_OSPF_EVENT) {
			zlog_debug("ospf_abr_announce_aggregates(): looking at area %s", inet_ntoa(area->area_id));
		}
		...
		for(rn = route_top(area->ranges); rn; rn = route_next(rn)) {
			if((range = rn->info)) {
		       ...
		       when announced into transit areas */
						...
						if(ospf_area_is_transit(ar) && OSPF_IS_AREA_BACKBONE(area)) {
							if(IS_DEBUG_OSPF_EVENT) {
...
static void ospf_abr_send_nssa_aggregates(struct ospf *ospf) /* temporarily turned off */
{
	struct listnode *node;	/*, n; */
	struct ospf_area *area; /*, *ar; */
	struct route_node *rn;
	...
	for(ALL_LIST_ELEMENTS_RO(ospf->areas, node, area)) {
		...
		if(IS_DEBUG_OSPF_NSSA) {
			zlog_debug("ospf_abr_send_nssa_aggregates(): looking at area %s", inet_ntoa(area->area_id));
		}
...
static void ospf_abr_announce_stub_defaults(struct ospf *ospf) {
	...
	for(ALL_LIST_ELEMENTS_RO(ospf->areas, node, area)) {
		if(IS_DEBUG_OSPF_EVENT) {
			zlog_debug("ospf_abr_announce_stub_defaults(): looking at area %s", inet_ntoa(area->area_id));
		}
...
static int ospf_abr_remove_unapproved_translates_apply(struct ospf *ospf, struct ospf_lsa *lsa) {
	if(CHECK_FLAG(lsa->flags, OSPF_LSA_LOCAL_XLT) && !CHECK_FLAG(lsa->flags, OSPF_LSA_APPROVED)) {
		zlog_info(
	...
	for(ALL_LIST_ELEMENTS(ospf->areas, node, nnode, area)) {
		for(rn = route_top(area->ranges); rn; rn = route_next(rn)) {
			if((range = rn->info) != NULL) {
				if(CHECK_FLAG(range->flags, OSPF_AREA_RANGE_ADVERTISE)) {
					if(range->specifics) {
						ospf_add_discard_route(ospf->new_table, area, (struct prefix_ipv4 *) &rn->p);
					} else {
						ospf_delete_discard_route(ospf->new_table, (struct prefix_ipv4 *) &rn->p);
					}
				}
			}
		}
...
void ospf_schedule_abr_task(struct ospf *ospf) {
	...
	if(ospf->t_abr_task == NULL) {
		ospf->t_abr_task = thread_add_timer(master, ospf_abr_task_timer, ospf, OSPF_ABR_TASK_DELAY);
	}
}
