 ...
 */
#define OSPF_IF_PARAM_IS_SET(O, P) (OSPF_IF_PARAM_CONFIGURED((O)->params, P) || OSPF_IF_PARAM_CONFIGURED(IF_DEF_PARAMS((O)->ifp)->P))

#define OSPF_IF_PARAM(O, P) (OSPF_IF_PARAM_CONFIGURED((O)->params, P) ? (O)->params->P : IF_DEF_PARAMS((O)->ifp)->P)
...
#define OSPF_IF_PASSIVE_STATUS(O) \
	(OSPF_IF_PARAM_CONFIGURED((O)->params, passive_interface) ? (O)->params->passive_interface \
								  : (OSPF_IF_PARAM_CONFIGURED(IF_DEF_PARAMS((O)->ifp), passive_interface) ? IF_DEF_PARAMS((O)->ifp)->passive_interface : (O)->ospf->passive_interface_default))
...
#define OI_MEMBER_CHECK(O, M) (CHECK_FLAG((O)->multicast_memberships, OI_MEMBER_FLAG(M)))
...
extern struct ospf_interface *ospf_if_new(struct ospf *, struct interface *, struct prefix *);
...
extern struct ospf_interface *ospf_if_lookup_by_lsa_pos(struct ospf_area *, int);
extern struct ospf_interface *ospf_if_lookup_by_local_addr(struct ospf *, struct interface *, struct in_addr);
extern struct ospf_interface *ospf_if_lookup_by_prefix(struct ospf *, struct prefix_ipv4 *);
extern struct ospf_interface *ospf_if_table_lookup(struct interface *, struct prefix *);
extern struct ospf_interface *ospf_if_addr_local(struct in_addr);
extern struct ospf_interface *ospf_if_lookup_recv_if(struct ospf *, struct in_addr, struct interface *);
extern struct ospf_interface *ospf_if_is_configured(struct ospf *, struct in_addr *);

extern struct ospf_if_params *ospf_lookup_if_params(struct interface *, struct in_addr);
extern struct ospf_if_params *ospf_get_if_params(struct interface *, struct in_addr);
...
extern struct ospf_vl_data *ospf_vl_data_new(struct ospf_area *, struct in_addr);
extern struct ospf_vl_data *ospf_vl_lookup(struct ospf *, struct ospf_area *, struct in_addr);
...
extern void ospf_vl_up_check(struct ospf_area *, struct in_addr, struct vertex *);
