...
static struct ospf_neighbor *ospf_elect_dr(struct ospf_interface *oi, struct list *el_list) {
	struct list *dr_list;
	struct listnode *node;
...
static struct ospf_neighbor *ospf_elect_bdr(struct ospf_interface *oi, struct list *el_list) {
...
static void ospf_dr_eligible_routers(struct route_table *nbrs, struct list *el_list) {
...
static int ospf_dr_election(struct ospf_interface *oi) {
	...
	if(new_state != old_state && !(new_state == ISM_DROther && old_state < ISM_DROther)) {
		ospf_elect_bdr(oi, el_list);
		ospf_elect_dr(oi, el_list);
...
int ospf_hello_timer(struct thread *thread) {
	...
	if(IS_DEBUG_OSPF(ism, ISM_TIMERS)) {
		zlog(NULL, LOG_DEBUG, "ISM[%s]: Timer (Hello timer expire)", IF_NAME(oi));
	}
...
static int ospf_wait_timer(struct thread *thread) {
	...
	if(IS_DEBUG_OSPF(ism, ISM_TIMERS)) {
		zlog(NULL, LOG_DEBUG, "ISM[%s]: Timer (Wait timer expire)", IF_NAME(oi));
	}
...
static void ism_timer_set(struct ospf_interface *oi) {
	switch(oi->state) {
	 ...
	 BDRouter. The router begin to receive and send Hello Packets. */
			...
			OSPF_ISM_TIMER_ON(oi->t_wait, ospf_wait_timer, OSPF_IF_PARAM(oi, v_wait));
			OSPF_ISM_TIMER_OFF(oi->t_ls_ack);
			break;
...
static int ism_interface_up(struct ospf_interface *oi) {
	...
	if(oi->type == OSPF_IFTYPE_POINTOPOINT || oi->type == OSPF_IFTYPE_POINTOMULTIPOINT || oi->type == OSPF_IFTYPE_VIRTUALLINK) {
...
static const char *ospf_ism_event_str[] = {
	"NoEvent", "InterfaceUp", "WaitTimer", "BackupSeen", "NeighborChange", "LoopInd", "UnLoopInd", "InterfaceDown",
};
...
static void ism_change_state(struct ospf_interface *oi, int state) {
	...
	if(IS_DEBUG_OSPF(ism, ISM_STATUS)) {
		zlog(NULL, LOG_DEBUG, "ISM[%s]: State change %s -> %s", IF_NAME(oi), LOOKUP(ospf_ism_state_msg, oi->state), LOOKUP(ospf_ism_state_msg, state));
	}
	...
	if((state == ISM_DR) || (state == ISM_Backup) || (state == ISM_DROther) || (state == ISM_PointToPoint) || (state < old_state)) {
...
int ospf_ism_event(struct thread *thread) {
	...
	if(IS_DEBUG_OSPF(ism, ISM_EVENTS)) {
		zlog(NULL, LOG_DEBUG, "ISM[%s]: %s (%s)", IF_NAME(oi), LOOKUP(ospf_ism_state_msg, oi->state), ospf_ism_event_str[event]);
	}
