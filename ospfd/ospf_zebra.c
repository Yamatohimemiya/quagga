...
static int ospf_router_id_update_zebra(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct ospf *ospf;
...
static int ospf_interface_add(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct interface *ifp;

	ifp = zebra_interface_add_read(zclient->ibuf, vrf_id);

	if(IS_DEBUG_OSPF(zebra, ZEBRA_INTERFACE)) {
		zlog_debug("Zebra: interface add %s index %d flags %llx metric %d mtu %d", ifp->name, ifp->ifindex, (unsigned long long) ifp->flags, ifp->metric, ifp->mtu);
	}
...
static int ospf_interface_delete(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct interface *ifp;
	struct stream *s;
	...
	if(IS_DEBUG_OSPF(zebra, ZEBRA_INTERFACE)) {
		zlog_debug("Zebra: interface delete %s index %d flags %llx metric %d mtu %d", ifp->name, ifp->ifindex, (unsigned long long) ifp->flags, ifp->metric, ifp->mtu);
	}
...
static struct interface *zebra_interface_if_lookup(struct stream *s, vrf_id_t vrf_id) {
	...
	/* And look it up. */
	return if_lookup_by_name_len(ifname_tmp, strnlen(ifname_tmp, INTERFACE_NAMSIZ));
}

static int ospf_interface_state_up(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct interface *ifp;
	...
	if(if_is_operative(ifp)) {
		...
		if(if_tmp.bandwidth != ifp->bandwidth) {
			if(IS_DEBUG_OSPF(zebra, ZEBRA_INTERFACE)) {
				zlog_debug("Zebra: Interface[%s] bandwidth change %d -> %d.", ifp->name, if_tmp.bandwidth, ifp->bandwidth);
			}
		...
		if(if_tmp.mtu != ifp->mtu) {
			if(IS_DEBUG_OSPF(zebra, ZEBRA_INTERFACE)) {
				zlog_debug("Zebra: Interface[%s] MTU change %u -> %u.", ifp->name, if_tmp.mtu, ifp->mtu);
			}
...
static int ospf_interface_state_down(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct interface *ifp;
	struct ospf_interface *oi;
...
static int ospf_interface_address_add(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct connected *c;
...
static int ospf_interface_address_delete(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct connected *c;
	struct interface *ifp;
	struct ospf_interface *oi;
	struct route_node *rn;
...
static int ospf_interface_link_params(int command, struct zclient *zclient, zebra_size_t length) {
	struct interface *ifp;
...
void ospf_zebra_add(struct prefix_ipv4 *p, struct ospf_route * or) {
	...
	if(vrf_bitmap_check(zclient->redist[ZEBRA_ROUTE_OSPF], VRF_DEFAULT)) {
		...
		if(((or->path_type == OSPF_PATH_TYPE1_EXTERNAL) || (or->path_type == OSPF_PATH_TYPE2_EXTERNAL)) && (or->u.ext.tag > 0) && (or->u.ext.tag <= ROUTE_TAG_MAX)) {
		...
		for(ALL_LIST_ELEMENTS_RO(or->paths, node, path)) {
			...
			if(IS_DEBUG_OSPF(zebra, ZEBRA_REDISTRIBUTE)) {
				...
				zlog_debug("Zebra: Route add %s/%d nexthop %s", inet_ntop(AF_INET, &p->prefix, buf[0], sizeof(buf[0])), p->prefixlen, inet_ntop(AF_INET, &path->nexthop, buf[1], sizeof(buf[1])));
			}
		}
...
void ospf_zebra_delete(struct prefix_ipv4 *p, struct ospf_route * or) {
	...
	if(vrf_bitmap_check(zclient->redist[ZEBRA_ROUTE_OSPF], VRF_DEFAULT)) {
		...
		for(ALL_LIST_ELEMENTS_RO(or->paths, node, path)) {
			...
			if(IS_DEBUG_OSPF(zebra, ZEBRA_REDISTRIBUTE)) {
				char buf[2][INET_ADDRSTRLEN];
				zlog_debug("Zebra: Route delete %s/%d nexthop %s", inet_ntop(AF_INET, &p->prefix, buf[0], sizeof(buf[0])), p->prefixlen, inet_ntop(AF_INET, &path->nexthop, buf[1], sizeof(buf[1])));
			}
		}
...
void ospf_zebra_add_discard(struct prefix_ipv4 *p) {
	...
	if(vrf_bitmap_check(zclient->redist[ZEBRA_ROUTE_OSPF], VRF_DEFAULT)) {
		...
		if(IS_DEBUG_OSPF(zebra, ZEBRA_REDISTRIBUTE)) {
			zlog_debug("Zebra: Route add discard %s/%d", inet_ntoa(p->prefix), p->prefixlen);
		}
	}
}
...
void ospf_zebra_delete_discard(struct prefix_ipv4 *p) {
	...
	if(vrf_bitmap_check(zclient->redist[ZEBRA_ROUTE_OSPF], VRF_DEFAULT)) {
		...
		if(IS_DEBUG_OSPF(zebra, ZEBRA_REDISTRIBUTE)) {
			zlog_debug("Zebra: Route delete discard %s/%d", inet_ntoa(p->prefix), p->prefixlen);
		}
	}
}

int ospf_is_type_redistributed(int type) {
	return (DEFAULT_ROUTE_TYPE(type)) ? vrf_bitmap_check(zclient->default_information, VRF_DEFAULT) : vrf_bitmap_check(zclient->redist[type], VRF_DEFAULT);
}

int ospf_redistribute_set(struct ospf *ospf, int type, int mtype, int mvalue) {
	int force = 0;
	...
	if(ospf_is_type_redistributed(type)) {
		...
		if(IS_DEBUG_OSPF(zebra, ZEBRA_REDISTRIBUTE)) {
			zlog_debug("Redistribute[%s]: Refresh  Type[%d], Metric[%d]", ospf_redist_string(type), metric_type(ospf, type), metric_value(ospf, type));
		}
	...
	if(IS_DEBUG_OSPF(zebra, ZEBRA_REDISTRIBUTE)) {
		zlog_debug("Redistribute[%s]: Start  Type[%d], Metric[%d]", ospf_redist_string(type), metric_type(ospf, type), metric_value(ospf, type));
	}
...
int ospf_redistribute_unset(struct ospf *ospf, int type) {
	...
	if(IS_DEBUG_OSPF(zebra, ZEBRA_REDISTRIBUTE)) {
		zlog_debug("Redistribute[%s]: Stop", ospf_redist_string(type));
	}
...
int ospf_redistribute_default_set(struct ospf *ospf, int originate, int mtype, int mvalue) {
	...
	if(ospf_is_type_redistributed(DEFAULT_ROUTE)) {
	 ...
	 the change? */
		...
		if(IS_DEBUG_OSPF(zebra, ZEBRA_REDISTRIBUTE)) {
			zlog_debug("Redistribute[%s]: Refresh  Type[%d], Metric[%d]", ospf_redist_string(DEFAULT_ROUTE), metric_type(ospf, DEFAULT_ROUTE), metric_value(ospf, DEFAULT_ROUTE));
		}
		return CMD_SUCCESS;
	}
	...
	if(IS_DEBUG_OSPF(zebra, ZEBRA_REDISTRIBUTE)) {
		zlog_debug("Redistribute[DEFAULT]: Start  Type[%d], Metric[%d]", metric_type(ospf, DEFAULT_ROUTE), metric_value(ospf, DEFAULT_ROUTE));
	}
...
static int ospf_external_lsa_originate_check(struct ospf *ospf, struct external_info *ei) {
...
int ospf_redistribute_check(struct ospf *ospf, struct external_info *ei, int *changed) {
	...
	if(!DEFAULT_ROUTE_TYPE(type) && DISTRIBUTE_NAME(ospf, type)) {
		...
		if(DISTRIBUTE_LIST(ospf, type)) {
			if(access_list_apply(DISTRIBUTE_LIST(ospf, type), p) == FILTER_DENY) {
				if(IS_DEBUG_OSPF(zebra, ZEBRA_REDISTRIBUTE)) {
					zlog_debug("Redistribute[%s]: %s/%d filtered by ditribute-list.", ospf_redist_string(type), inet_ntoa(p->prefix), p->prefixlen);
				}
				return 0;
			}
		}
	...
	if(ROUTEMAP_NAME(ospf, type)) {
		...
		ret = route_map_apply(ROUTEMAP(ospf, type), (struct prefix *) p, RMAP_OSPF, ei);

		if(ret == RMAP_DENYMATCH) {
			ei->route_map_set = save_values;
			if(IS_DEBUG_OSPF(zebra, ZEBRA_REDISTRIBUTE)) {
				zlog_debug("Redistribute[%s]: %s/%d filtered by route-map.", ospf_redist_string(type), inet_ntoa(p->prefix), p->prefixlen);
			}
			return 0;
		}
		...
		if(changed) {
			*changed = !ospf_route_map_set_compare(&ei->route_map_set, &save_values);
		}
	}
...
void ospf_routemap_set(struct ospf *ospf, int type, const char *name) {
	if(ROUTEMAP_NAME(ospf, type)) {
...
void ospf_routemap_unset(struct ospf *ospf, int type) {
	if(ROUTEMAP_NAME(ospf, type)) {
		free(ROUTEMAP_NAME(ospf, type));
	}
...
static int ospf_zebra_read_ipv4(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct stream *s;
	struct zapi_ipv4 api;
			...
			if(ei) {
				if(is_prefix_default(&p)) {
					...
					if(!current) {
						...
						zlog_warn("ospf_zebra_read_ipv4() : %s already exists", inet_ntoa(p.prefix));
					}
				}
			}
		}
...
void ospf_distribute_list_update(struct ospf *ospf, uintptr_t type) {
	...
	/* Set timer. */
	ospf->t_distribute_update = thread_add_timer_msec(master, ospf_distribute_list_update_timer, (void *) type, ospf->min_ls_interval);
}

/* If access-list is updated, apply some check. */
static void ospf_filter_update(const char *name) {
	struct ospf *ospf;
	int type;
	int abr_inv = 0;
	...
	for(type = 0; type <= ZEBRA_ROUTE_MAX; type++) {
       ...
       * but no distribute list. */
		...
		if(DISTRIBUTE_NAME(ospf, type)) {
			...
			DISTRIBUTE_LIST(ospf, type) = access_list_lookup(AFI_IP, DISTRIBUTE_NAME(ospf, type));
			...
			if(DISTRIBUTE_LIST(ospf, type) == NULL || strcmp(DISTRIBUTE_NAME(ospf, type), name) == 0) {
				ospf_distribute_list_update(ospf, type);
			}
		}
	}
	...
	for(ALL_LIST_ELEMENTS_RO(ospf->areas, node, area)) {
		...
		if(PREFIX_NAME_OUT(area)) {
			if(strcmp(PREFIX_NAME_OUT(area), prefix_list_name(plist)) == 0) {
				PREFIX_LIST_IN(area) = prefix_list_lookup(AFI_IP, PREFIX_NAME_OUT(area));
				abr_inv++;
			}
		}
	}
...
int ospf_distance_set(struct vty *vty, struct ospf *ospf, const char *distance_str, const char *ip_str, const char *access_list_str) {
	int ret;
	struct prefix_ipv4 p;
	u_char distance;
	struct route_node *rn;
...
int ospf_distance_unset(struct vty *vty, struct ospf *ospf, const char *distance_str, const char *ip_str, const char *access_list_str) {
	int ret;
	struct prefix_ipv4 p;
	struct route_node *rn;
	struct ospf_distance *odistance;
...
u_char ospf_distance_apply(struct prefix_ipv4 *p, struct ospf_route * or) {
	...
	if(ospf->distance_external) {
		if(or->path_type == OSPF_PATH_TYPE1_EXTERNAL || or->path_type == OSPF_PATH_TYPE2_EXTERNAL) {
...
void ospf_zebra_init(struct thread_master *master) {
