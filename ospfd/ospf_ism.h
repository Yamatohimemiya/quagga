...
#define ISM_SNMP(x) (((x) == ISM_DROther) ? ISM_DR : ((x) == ISM_DR) ? ISM_DROther : (x))
...
#define OSPF_ISM_WRITE_ON(O) \
	do { \
		...
		if((O)->t_write == NULL) (O)->t_write = thread_add_write(master, ospf_write, (O), (O)->fd); \
	} while(0)
...
#define OSPF_ISM_TIMER_MSEC_ON(T, F, V) \
	do { \
		if(!(T)) (T) = thread_add_timer_msec(master, (F), oi, (V)); \
	} while(0)
...
#define OSPF_HELLO_TIMER_ON(O) \
	do { \
		if(OSPF_IF_PARAM((O), fast_hello)) OSPF_ISM_TIMER_MSEC_ON((O)->t_hello, ospf_hello_timer, 1000 / OSPF_IF_PARAM((O), fast_hello)); \
		else OSPF_ISM_TIMER_ON((O)->t_hello, ospf_hello_timer, OSPF_IF_PARAM((O), v_hello)); \
	} while(0)
...
#define OSPF_ISM_EVENT_SCHEDULE(I, E) thread_add_event(master, ospf_ism_event, (I), (E))

/* Macro for OSPF execute event. */
#define OSPF_ISM_EVENT_EXECUTE(I, E) thread_execute(master, ospf_ism_event, (I), (E))
