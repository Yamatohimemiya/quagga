...
int ospf_lsa_refresh_delay(struct ospf_lsa *lsa) {
	...
	if(tv_cmp(delta, msec2tv(OSPF_MIN_LS_INTERVAL)) < 0) {
		...
		if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
			zlog_debug("LSA[Type%d:%s]: Refresh timer delay %d seconds", lsa->data->type, inet_ntoa(lsa->data->id), delay);
		}
...
void ospf_lsa_data_free(struct lsa_header *lsah) {
	if(IS_DEBUG_OSPF(lsa, LSA)) {
		zlog_debug("LSA[Type%d:%s]: data freed %p", lsah->type, inet_ntoa(lsah->id), (void *) lsah);
	}
...
void lsa_header_set(struct stream *s, u_char options, u_char type, struct in_addr id, struct in_addr router_id) {
	struct lsa_header *lsah;
...
static u_char router_lsa_flags(struct ospf_area *area) {
	...
	if(area->ospf->abr_type == OSPF_ABR_SHORTCUT) {
		if(!OSPF_IS_AREA_BACKBONE(area)) {
			if((area->shortcut_configured == OSPF_SHORTCUT_DEFAULT && area->ospf->backbone == NULL) || area->shortcut_configured == OSPF_SHORTCUT_ENABLE) {
	...
	if(IS_OSPF_ABR(area->ospf)) {
       ...
       */
		if((area->external_routing == OSPF_AREA_NSSA) && (area->NSSATranslatorRole == OSPF_NSSA_ROLE_ALWAYS)) {
...
static char link_info_set(struct stream *s, struct in_addr id, struct in_addr data, u_char type, u_char tos, u_int16_t cost) {
   ...
   */
	if(STREAM_WRITEABLE(s) < OSPF_ROUTER_LSA_LINK_SIZE) {
		...
		if(ret == OSPF_MAX_LSA_SIZE) {
			zlog_warn("%s: Out of space in LSA stream, left %zd, size %zd", __func__, STREAM_REMAIN(s), STREAM_SIZE(s));
			return 0;
		}
	}
...
static int lsa_link_ptop_set(struct stream *s, struct ospf_interface *oi) {
	...
	if((nbr = ospf_nbr_lookup_ptop(oi))) {
	   ...
	   should specify the interface's MIB-II ifIndex value. */
			links += link_info_set(s, nbr->router_id, oi->address->u.prefix4, LSA_LINK_TYPE_POINTOPOINT, 0, cost);
		}
	}
	...
	links += link_info_set(s, id, mask, LSA_LINK_TYPE_STUB, 0, oi->output_cost);
	return links;
}
...
static int lsa_link_broadcast_set(struct stream *s, struct ospf_interface *oi) {
	...
	if(dr && (dr->state == NSM_Full || IPV4_ADDR_SAME(&oi->address->u.prefix4, &DR(oi))) && ospf_nbr_count(oi, NSM_Full) > 0) {
		if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
			zlog_debug(
		...
		return link_info_set(s, DR(oi), oi->address->u.prefix4, LSA_LINK_TYPE_TRANSIT, 0, cost);
	}
	/* Describe type 3 link. */
	else {
		if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
...
static int lsa_link_virtuallink_set(struct stream *s, struct ospf_interface *oi) {
	...
	if(oi->state == ISM_PointToPoint) {
		if((nbr = ospf_nbr_lookup_ptop(oi))) {
			if(nbr->state == NSM_Full) {
				return link_info_set(s, nbr->router_id, oi->address->u.prefix4, LSA_LINK_TYPE_VIRTUALLINK, 0, cost);
			}
		}
	}
		...
		if((nbr = rn->info) != NULL) {
			...
			if(!IPV4_ADDR_SAME(&nbr->router_id, &oi->ospf->router_id)) {
...

				{
					links += link_info_set(s, nbr->router_id, oi->address->u.prefix4, LSA_LINK_TYPE_POINTOPOINT, 0, cost);
					if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
						zlog_debug("PointToMultipoint: set link to %s", inet_ntoa(oi->address->u.prefix4));
					}
				}
			}
		}
	...
	for(ALL_LIST_ELEMENTS_RO(area->oiflist, node, oi)) {
		...
		if(if_is_operative(ifp)) {
			if(oi->state != ISM_Down) {
				...
				/* Describe each link. */
				switch(oi->type) {
					case OSPF_IFTYPE_POINTOPOINT: links += lsa_link_ptop_set(s, oi); break;
					case OSPF_IFTYPE_BROADCAST: links += lsa_link_broadcast_set(s, oi); break;
					case OSPF_IFTYPE_NBMA: links += lsa_link_nbma_set(s, oi); break;
					case OSPF_IFTYPE_POINTOMULTIPOINT: links += lsa_link_ptomp_set(s, oi); break;
					case OSPF_IFTYPE_VIRTUALLINK: links += lsa_link_virtuallink_set(s, oi); break;
					case OSPF_IFTYPE_LOOPBACK: links += lsa_link_loopback_set(s, oi);
				}
...
static void ospf_stub_router_check(struct ospf_area *area) {
   ...
   */
	...
	OSPF_AREA_TIMER_ON(area->t_stub_router, ospf_stub_router_timer, area->ospf->stub_router_startup_time);
}
...
static struct ospf_lsa *ospf_router_lsa_new(struct ospf_area *area) {
   ...
   */
	...
	lsa_header_set(s, LSA_OPTIONS_GET(area) | LSA_OPTIONS_NSSA_GET(area), OSPF_ROUTER_LSA, ospf->router_id, ospf->router_id);
...
static struct ospf_lsa *ospf_router_lsa_refresh(struct ospf_lsa *lsa) {
	...
	/* Debug logging. */
	if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("LSA[Type%d:%s]: router-LSA refresh", new->data->type, inet_ntoa(new->data->id));
		ospf_lsa_header_dump(new->data);
	}
...
int ospf_router_lsa_update(struct ospf *ospf) {
	...
	for(ALL_LIST_ELEMENTS(ospf->areas, node, nnode, area)) {
	 ...
	 First flush old LSA, then originate new. */
			...
			if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
				zlog_debug("LSA[Type%d:%s]: Refresh router-LSA for Area %s", lsa->data->type, inet_ntoa(lsa->data->id), area_str);
			}
...
static void ospf_network_lsa_body_set(struct stream *s, struct ospf_interface *oi) {
	struct in_addr mask;
	struct route_node *rn;
	struct ospf_neighbor *nbr;
...
static struct ospf_lsa *ospf_network_lsa_new(struct ospf_interface *oi) {
	...
	lsa_header_set(s, (OPTIONS(oi) | LSA_OPTIONS_GET(oi->area)), OSPF_NETWORK_LSA, DR(oi), oi->ospf->router_id);
...
void ospf_network_lsa_update(struct ospf_interface *oi) {
	...
	if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("LSA[Type%d:%s]: Originate network-LSA %p", new->data->type, inet_ntoa(new->data->id), (void *) new);
		ospf_lsa_header_dump(new->data);
	}
...
static struct ospf_lsa *ospf_network_lsa_refresh(struct ospf_lsa *lsa) {
	...
	if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("LSA[Type%d:%s]: network-LSA refresh", new->data->type, inet_ntoa(new->data->id));
		ospf_lsa_header_dump(new->data);
	}
...
static void ospf_summary_lsa_body_set(struct stream *s, struct prefix *p, u_int32_t metric) {
	struct in_addr mask;
...
static struct ospf_lsa *ospf_summary_lsa_new(struct ospf_area *area, struct prefix *p, u_int32_t metric, struct in_addr id) {
	struct stream *s;
	struct ospf_lsa *new;
	struct lsa_header *lsah;
	int length;
	...
	if(id.s_addr == 0xffffffff) {
		...
		if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
			zlog_debug("LSA[Type%d]: Link ID not available, can't originate", OSPF_SUMMARY_LSA);
		}
		return NULL;
	}
	...
	lsa_header_set(s, LSA_OPTIONS_GET(area), OSPF_SUMMARY_LSA, id, area->ospf->router_id);
...
struct ospf_lsa *ospf_summary_lsa_originate(struct prefix_ipv4 *p, u_int32_t metric, struct ospf_area *area) {
	struct ospf_lsa *new;
	struct in_addr id;
	...
	if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("LSA[Type%d:%s]: Originate summary-LSA %p", new->data->type, inet_ntoa(new->data->id), (void *) new);
		ospf_lsa_header_dump(new->data);
	}
...
static struct ospf_lsa *ospf_summary_lsa_refresh(struct ospf *ospf, struct ospf_lsa *lsa) {
	struct ospf_lsa *new;
	...
	new = ospf_summary_lsa_new(lsa->area, &p, GET_METRIC(sl->metric), sl->header.id);
	...
	/* Debug logging. */
	if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("LSA[Type%d:%s]: summary-LSA refresh", new->data->type, inet_ntoa(new->data->id));
		ospf_lsa_header_dump(new->data);
	}
...
static void ospf_summary_asbr_lsa_body_set(struct stream *s, struct prefix *p, u_int32_t metric) {
	/* Put Network Mask. */
...
static struct ospf_lsa *ospf_summary_asbr_lsa_new(struct ospf_area *area, struct prefix *p, u_int32_t metric, struct in_addr id) {
	struct stream *s;
	struct ospf_lsa *new;
	struct lsa_header *lsah;
	int length;
	...
	if(id.s_addr == 0xffffffff) {
		/* Maybe Link State ID not available. */
		if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
			zlog_debug("LSA[Type%d]: Link ID not available, can't originate", OSPF_ASBR_SUMMARY_LSA);
		}
		return NULL;
	}
	...
	lsa_header_set(s, LSA_OPTIONS_GET(area), OSPF_ASBR_SUMMARY_LSA, id, area->ospf->router_id);
...
struct ospf_lsa *ospf_summary_asbr_lsa_originate(struct prefix_ipv4 *p, u_int32_t metric, struct ospf_area *area) {
	struct ospf_lsa *new;
	struct in_addr id;
	...
	if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("LSA[Type%d:%s]: Originate summary-ASBR-LSA %p", new->data->type, inet_ntoa(new->data->id), (void *) new);
		ospf_lsa_header_dump(new->data);
	}
...
static struct ospf_lsa *ospf_summary_asbr_lsa_refresh(struct ospf *ospf, struct ospf_lsa *lsa) {
	struct ospf_lsa *new;
	struct summary_lsa *sl;
	struct prefix p;
	...
	sl = (struct summary_lsa *) lsa->data;
	p.prefixlen = ip_masklen(sl->mask);
	new = ospf_summary_asbr_lsa_new(lsa->area, &p, GET_METRIC(sl->metric), sl->header.id);
	if(!new) {
		return NULL;
	}
	...
	if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("LSA[Type%d:%s]: summary-ASBR-LSA refresh", new->data->type, inet_ntoa(new->data->id));
		ospf_lsa_header_dump(new->data);
	}
...
/* Get nexthop for AS-external-LSAs.  Return nexthop if its interface
   is connected, else 0*/
static struct in_addr ospf_external_lsa_nexthop_get(struct ospf *ospf, struct in_addr nexthop) {
	struct in_addr fwd;
	struct prefix nh;
	struct listnode *node;
	struct ospf_interface *oi;
...
int metric_type(struct ospf *ospf, u_char src) {
	return (ospf->dmetric[src].type < 0 ? DEFAULT_METRIC_TYPE : ospf->dmetric[src].type);
}

int metric_value(struct ospf *ospf, u_char src) {
...
static void ospf_external_lsa_body_set(struct stream *s, struct external_info *ei, struct ospf *ospf) {
	...
	mtype = (ROUTEMAP_METRIC_TYPE(ei) != -1) ? ROUTEMAP_METRIC_TYPE(ei) : metric_type(ospf, type);

	mvalue = (ROUTEMAP_METRIC(ei) != -1) ? ROUTEMAP_METRIC(ei) : metric_value(ospf, type);
...
static struct ospf_lsa *ospf_external_lsa_new(struct ospf *ospf, struct external_info *ei, struct in_addr *old_id) {
	struct stream *s;
	struct lsa_header *lsah;
	struct ospf_lsa *new;
	struct in_addr id;
	...
	/* Set LSA common header fields. */
	lsa_header_set(s, OSPF_OPTION_E, OSPF_AS_EXTERNAL_LSA, id, ospf->router_id);
...
/* As Type-7 */
static void ospf_install_flood_nssa(struct ospf *ospf, struct ospf_lsa *lsa, struct external_info *ei) {
	struct ospf_lsa *new;
...
static struct ospf_lsa *ospf_lsa_translated_nssa_new(struct ospf *ospf, struct ospf_lsa *type7) {
	struct ospf_lsa *new;
...
struct ospf_lsa *ospf_translated_nssa_originate(struct ospf *ospf, struct ospf_lsa *type7) {
	struct ospf_lsa *new;
...
struct ospf_lsa *ospf_translated_nssa_refresh(struct ospf *ospf, struct ospf_lsa *type7, struct ospf_lsa *type5) {
...
struct ospf_lsa *ospf_external_lsa_originate(struct ospf *ospf, struct external_info *ei) {
	struct ospf_lsa *new;
	...
	/* Debug logging. */
	if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("LSA[Type%d:%s]: Originate AS-external-LSA %p", new->data->type, inet_ntoa(new->data->id), (void *) new);
		ospf_lsa_header_dump(new->data);
	}
...
void ospf_nssa_lsa_flush(struct ospf *ospf, struct prefix_ipv4 *p) {
	...
	for(ALL_LIST_ELEMENTS(ospf->areas, node, nnode, area)) {
		if(area->external_routing == OSPF_AREA_NSSA) {
			if(!(lsa = ospf_lsa_lookup(area, OSPF_AS_NSSA_LSA, p->prefix, ospf->router_id))) {
				if(IS_DEBUG_OSPF(lsa, LSA_FLOODING)) {
					zlog_debug("LSA: There is no such AS-NSSA-LSA %s/%d in LSDB", inet_ntoa(p->prefix), p->prefixlen);
				}
				continue;
			}
			ospf_ls_retransmit_delete_nbr_area(area, lsa);
...
void ospf_external_lsa_flush(struct ospf *ospf, u_char type, struct prefix_ipv4 *p, ifindex_t ifindex /*, struct in_addr nexthop */) {
	struct ospf_lsa *lsa;

	if(IS_DEBUG_OSPF(lsa, LSA_FLOODING)) {
		zlog_debug("LSA: Flushing AS-external-LSA %s/%d", inet_ntoa(p->prefix), p->prefixlen);
	}
   ...
   */
	if(IS_LSA_SELF(lsa) && (ospf->anyNSSA) && !(CHECK_FLAG(lsa->flags, OSPF_LSA_LOCAL_XLT))) {
...
void ospf_external_lsa_refresh_default(struct ospf *ospf) {
	...
	if(ei) {
		if(lsa) {
			if(IS_DEBUG_OSPF_EVENT) {
				zlog_debug("LSA[Type5:0.0.0.0]: Refresh AS-external-LSA %p", (void *) lsa);
			}
...
void ospf_external_lsa_refresh_type(struct ospf *ospf, u_char type, int force) {
	struct route_node *rn;
	struct external_info *ei;
...
struct ospf_lsa *ospf_external_lsa_refresh(struct ospf *ospf, struct ospf_lsa *lsa, struct external_info *ei, int force) {
	struct ospf_lsa *new;
	int changed;
	...
	if(!changed && !force) {
		if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
			zlog_debug("LSA[Type%d:%s]: Not refreshed, not changed/forced", lsa->data->type, inet_ntoa(lsa->data->id));
		}
		return NULL;
	}
   ...
   */
	...
	/* Debug logging. */
	if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("LSA[Type%d:%s]: AS-external-LSA refresh", new->data->type, inet_ntoa(new->data->id));
		ospf_lsa_header_dump(new->data);
	}
...
static struct ospf_lsa *ospf_router_lsa_install(struct ospf *ospf, struct ospf_lsa *new, int rt_recalc) {
...
#define OSPF_INTERFACE_TIMER_ON(T, F, V) \
	if(!(T)) (T) = thread_add_timer(master, (F), oi, (V))

/* Install network-LSA to an area. */
static struct ospf_lsa *ospf_network_lsa_install(struct ospf *ospf, struct ospf_interface *oi, struct ospf_lsa *new, int rt_recalc) {
	/* RFC 2328 Section 13.2 Router-LSAs and network-LSAs
     The entire routing table must be recalculated, starting with
     the shortest path calculations for each area (not just the
     area whose link-state database has changed). 
...
static struct ospf_lsa *ospf_summary_lsa_install(struct ospf *ospf, struct ospf_lsa *new, int rt_recalc) {
...
static struct ospf_lsa *ospf_summary_asbr_lsa_install(struct ospf *ospf, struct ospf_lsa *new, int rt_recalc) {
	if(rt_recalc && !IS_LSA_SELF(new)) {
		/* RFC 2328 Section 13.2 Summary-LSAs
	 The best route to the destination described by the summary-
	 LSA must be recalculated (see Section 16.5).  If this
...
static struct ospf_lsa *ospf_external_lsa_install(struct ospf *ospf, struct ospf_lsa *new, int rt_recalc) {
...
void ospf_discard_from_db(struct ospf *ospf, struct ospf_lsdb *lsdb, struct ospf_lsa *lsa) {
	struct ospf_lsa *old;
	...
	if(!lsdb) {
		...
		if(!lsa) {
			...
			zlog_warn("LSA[Type%d:%s]: not associated with LSDB!", lsa->data->type, inet_ntoa(lsa->data->id));
		}
		return;
	}
	...
	switch(old->data->type) {
		case OSPF_AS_EXTERNAL_LSA:
			...
			break;
		case OSPF_OPAQUE_AS_LSA: ospf_ls_retransmit_delete_nbr_as(ospf, old); break;
		case OSPF_AS_NSSA_LSA:
			ospf_ls_retransmit_delete_nbr_area(old->area, old);
			ospf_ase_unregister_external_lsa(old, ospf);
			break;
		default: ospf_ls_retransmit_delete_nbr_area(old->area, old); break;
	}
...
struct ospf_lsa *ospf_lsa_install(struct ospf *ospf, struct ospf_interface *oi, struct ospf_lsa *lsa) {
	struct ospf_lsa *new = NULL;
	...
	switch(lsa->data->type) {
		...
		case OSPF_AS_NSSA_LSA:
			...
			break;
		case OSPF_AS_EXTERNAL_LSA:
		case OSPF_OPAQUE_AS_LSA: lsdb = ospf->lsdb; break;
		default: lsdb = lsa->area->lsdb; break;
	}
  ...
  */
   ...
   */
	...
	switch(lsa->data->type) {
		case OSPF_ROUTER_LSA: new = ospf_router_lsa_install(ospf, lsa, rt_recalc); break;
		case OSPF_NETWORK_LSA:
			assert(oi);
			new = ospf_network_lsa_install(ospf, oi, lsa, rt_recalc);
			break;
		case OSPF_SUMMARY_LSA: new = ospf_summary_lsa_install(ospf, lsa, rt_recalc); break;
		case OSPF_ASBR_SUMMARY_LSA: new = ospf_summary_asbr_lsa_install(ospf, lsa, rt_recalc); break;
		case OSPF_AS_EXTERNAL_LSA: new = ospf_external_lsa_install(ospf, lsa, rt_recalc); break;
		...
		case OSPF_OPAQUE_AS_LSA: new = ospf_opaque_lsa_install(lsa, rt_recalc); break;
		case OSPF_AS_NSSA_LSA: new = ospf_external_lsa_install(ospf, lsa, rt_recalc);
		default: /* type-6,8,9....nothing special */ break;
	}
	...
	if(IS_DEBUG_OSPF(lsa, LSA_INSTALL)) {
		...
		switch(lsa->data->type) {
			...
			case OSPF_AS_NSSA_LSA: zlog_debug("LSA[%s]: Install %s", dump_lsa_key(new), LOOKUP(ospf_lsa_type_msg, new->data->type)); break;
			default:
				strcpy(area_str, inet_ntoa(new->area->area_id));
				zlog_debug("LSA[%s]: Install %s to Area %s", dump_lsa_key(new), LOOKUP(ospf_lsa_type_msg, new->data->type), area_str);
				break;
		}
	}
   ...
   */
	if(IS_LSA_MAXAGE(new)) {
		if(IS_DEBUG_OSPF(lsa, LSA_INSTALL)) {
			zlog_debug("LSA[Type%d:%s]: Install LSA 0x%p, MaxAge", new->data->type, inet_ntoa(new->data->id), (void *) lsa);
		}
...
void ospf_lsa_maxage_delete(struct ospf *ospf, struct ospf_lsa *lsa) {
	...
	if((rn = route_node_lookup(ospf->maxage_lsa, (struct prefix *) &lsa_prefix))) {
		if(rn->info == lsa) {
...
void ospf_lsa_maxage(struct ospf *ospf, struct ospf_lsa *lsa) {
	...
	if(CHECK_FLAG(lsa->flags, OSPF_LSA_IN_MAXAGE)) {
		if(IS_DEBUG_OSPF(lsa, LSA_FLOODING)) {
			zlog_debug("LSA[Type%d:%s]: %p already exists on MaxAge LSA list", lsa->data->type, inet_ntoa(lsa->data->id), (void *) lsa);
		}
		return;
	}
	...
	if((rn = route_node_get(ospf->maxage_lsa, (struct prefix *) &lsa_prefix)) != NULL) {
		if(rn->info != NULL) {
			if(IS_DEBUG_OSPF(lsa, LSA_FLOODING)) {
				zlog_debug("LSA[%s]: found LSA (%p) in table for LSA %p %d", dump_lsa_key(lsa), rn->info, (void *) lsa, lsa_prefix.prefixlen);
			}
			route_unlock_node(rn);
		} else {
...
static int ospf_lsa_maxage_walker_remover(struct ospf *ospf, struct ospf_lsa *lsa) {
       ...
       they're flushed and submitted to the max_age list explicitly. */
		if(!ospf_lsa_is_self_originated(ospf, lsa)) {
             ...
             */
					break;
				case OSPF_AS_EXTERNAL_LSA:
				case OSPF_AS_NSSA_LSA: ospf_ase_incremental_update(ospf, lsa); break;
				default: ospf_spf_calculate_schedule(ospf, SPF_FLAG_MAXAGE); break;
			}
			ospf_lsa_maxage(ospf, lsa);
		}
	}
...
struct ospf_lsa *ospf_lsa_lookup_by_prefix(struct ospf_lsdb *lsdb, u_char type, struct prefix_ipv4 *p, struct in_addr router_id) {
	struct ospf_lsa *lsa;
...
struct ospf_lsa *ospf_lsa_lookup(struct ospf_area *area, u_int32_t type, struct in_addr id, struct in_addr adv_router) {
	...
	switch(type) {
		...
		case OSPF_AS_NSSA_LSA:
		case OSPF_OPAQUE_LINK_LSA:
		case OSPF_OPAQUE_AREA_LSA: return ospf_lsdb_lookup_by_id(area->lsdb, type, id, adv_router);
		case OSPF_AS_EXTERNAL_LSA:
		case OSPF_OPAQUE_AS_LSA: return ospf_lsdb_lookup_by_id(ospf->lsdb, type, id, adv_router);
		default: break;
	}
...
struct ospf_lsa *ospf_lsa_lookup_by_id(struct ospf_area *area, u_int32_t type, struct in_addr id) {
	struct ospf_lsa *lsa;
	struct route_node *rn;

	switch(type) {
		case OSPF_ROUTER_LSA: return ospf_lsdb_lookup_by_id(area->lsdb, type, id, id);
		case OSPF_NETWORK_LSA:
...
struct ospf_lsa *ospf_lsa_lookup_by_header(struct ospf_area *area, struct lsa_header *lsah) {
   ...
   */
	...
	if(match == NULL) {
		if(IS_DEBUG_OSPF(lsa, LSA) == OSPF_DEBUG_LSA) {
			zlog_debug("LSA[Type%d:%s]: Lookup by header, NO MATCH", lsah->type, inet_ntoa(lsah->id));
		}
	}
...
int ospf_lsa_different(struct ospf_lsa *l1, struct ospf_lsa *l2) {
	...
	if(memcmp(p1 + OSPF_LSA_HEADER_SIZE, p2 + OSPF_LSA_HEADER_SIZE, ntohs(l1->data->length) - OSPF_LSA_HEADER_SIZE) != 0) {
		return 1;
	}
...
void ospf_lsa_flush_self_originated(struct ospf_neighbor *nbr, struct ospf_lsa *self, struct ospf_lsa *new) {
	u_int32_t seqnum;
	...
	if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
		zlog_debug("LSA[Type%d:%s]: Flush self-originated LSA", self->data->type, inet_ntoa(self->data->id));
	}
}
#else  /* ORIGINAL_CODING */
static int ospf_lsa_flush_schedule(struct ospf *ospf, struct ospf_lsa *lsa) {
	...
	switch(lsa->data->type) {
		...
		case OSPF_OPAQUE_LINK_LSA:
		case OSPF_OPAQUE_AREA_LSA:
		case OSPF_OPAQUE_AS_LSA: ospf_opaque_lsa_refresh(lsa); break;
		default:
			ospf_refresher_unregister_lsa(ospf, lsa);
...
void ospf_flush_self_originated_lsas_now(struct ospf *ospf) {
	...
	for(ALL_LIST_ELEMENTS(ospf->areas, node, nnode, area)) {
		...
		for(ALL_LIST_ELEMENTS(area->oiflist, node2, nnode2, oi)) {
			if((lsa = oi->network_lsa_self) != NULL && oi->state == ISM_DR && oi->full_nbrs > 0) {
				if(IS_DEBUG_OSPF_EVENT) {
					zlog_debug("LSA[Type%d:%s]: Schedule self-originated LSA to FLUSH", lsa->data->type, inet_ntoa(lsa->data->id));
				}
			...
			if(oi->type != OSPF_IFTYPE_VIRTUALLINK && area->external_routing == OSPF_AREA_DEFAULT) {
				need_to_flush_ase = 1;
			}
		}
...
int ospf_lsa_is_self_originated(struct ospf *ospf, struct ospf_lsa *lsa) {
	...
	/* LSA is router-LSA. */
	else if(lsa->data->type == OSPF_ROUTER_LSA && IPV4_ADDR_SAME(&lsa->data->id, &ospf->router_id)) {
		SET_FLAG(lsa->flags, OSPF_LSA_SELF);
	}
...
struct in_addr ospf_lsa_unique_id(struct ospf *ospf, struct ospf_lsdb *lsdb, u_char type, struct prefix_ipv4 *p) {
	struct ospf_lsa *lsa;
	struct in_addr mask, id;
	...
	if(lsa) {
		...
		/* Masklen differs, then apply wildcard mask to Link State ID. */
			...
			id.s_addr = p->prefix.s_addr | (~mask.s_addr);
			lsa = ospf_lsdb_lookup_by_id(ospf->lsdb, type, id, ospf->router_id);
			if(lsa) {
				if(IS_DEBUG_OSPF(lsa, LSA_GENERATE)) {
					zlog_debug(
						"ospf_lsa_unique_id(): "
...
static int ospf_lsa_action(struct thread *t) {
	...
	if(IS_DEBUG_OSPF(lsa, LSA) == OSPF_DEBUG_LSA) {
		zlog_debug("LSA[Action]: Performing scheduled LSA action: %d", data->action);
	}

	switch(data->action) {
		case LSA_ACTION_FLOOD_AREA: ospf_flood_through_area(data->area, NULL, data->lsa); break;
		case LSA_ACTION_FLUSH_AREA: ospf_lsa_flush_area(data->lsa, data->area); break;
	}
...
struct ospf_lsa *ospf_lsa_refresh(struct ospf *ospf, struct ospf_lsa *lsa) {
	...
	switch(lsa->data->type) {
		...
		case OSPF_ROUTER_LSA: new = ospf_router_lsa_refresh(lsa); break;
		case OSPF_NETWORK_LSA: new = ospf_network_lsa_refresh(lsa); break;
		case OSPF_SUMMARY_LSA: new = ospf_summary_lsa_refresh(ospf, lsa); break;
		case OSPF_ASBR_SUMMARY_LSA: new = ospf_summary_asbr_lsa_refresh(ospf, lsa); break;
		case OSPF_AS_EXTERNAL_LSA:
       ...
       */
		...
		case OSPF_OPAQUE_LINK_LSA:
		case OSPF_OPAQUE_AREA_LSA:
		case OSPF_OPAQUE_AS_LSA: new = ospf_opaque_lsa_refresh(lsa); break;
		default: break;
	}
	return new;
}
...
void ospf_refresher_register_lsa(struct ospf *ospf, struct ospf_lsa *lsa) {
	...
	if(lsa->refresh_list < 0) {
		...
		if(LS_AGE(lsa) == 0 && ntohl(lsa->data->ls_seqnum) == OSPF_INITIAL_SEQUENCE_NUMBER) {
			...
			delay = OSPF_LS_REFRESH_TIME - LS_AGE(lsa) - OSPF_LS_REFRESH_JITTER + (random() % (2 * OSPF_LS_REFRESH_JITTER));
		}
		...
		index = (current_index + delay / OSPF_LSA_REFRESHER_GRANULARITY) % (OSPF_LSA_REFRESHER_SLOTS);

		if(IS_DEBUG_OSPF(lsa, LSA_REFRESH)) {
			zlog_debug("LSA[Refresh]: lsa %s with age %d added to index %d", inet_ntoa(lsa->data->id), LS_AGE(lsa), index);
		}
		...
		if(IS_DEBUG_OSPF(lsa, LSA_REFRESH)) {
			zlog_debug(
				"LSA[Refresh:%s]: ospf_refresher_register_lsa(): "
...
int ospf_lsa_refresh_walker(struct thread *t) {
	...
	/* Note: if clock has jumped backwards, then time change could be negative,
     ...
     modulus. */
	ospf->lsa_refresh_queue.index = ((unsigned long) (ospf->lsa_refresh_queue.index + (quagga_time(NULL) - ospf->lsa_refresher_started) / OSPF_LSA_REFRESHER_GRANULARITY)) % OSPF_LSA_REFRESHER_SLOTS;

	if(IS_DEBUG_OSPF(lsa, LSA_REFRESH)) {
		zlog_debug("LSA[Refresh]: ospf_lsa_refresh_walker(): next index %d", ospf->lsa_refresh_queue.index);
	}

	for(; i != ospf->lsa_refresh_queue.index; i = (i + 1) % OSPF_LSA_REFRESHER_SLOTS) {
		if(IS_DEBUG_OSPF(lsa, LSA_REFRESH)) {
			zlog_debug(
		...
		if(refresh_list) {
			for(ALL_LIST_ELEMENTS(refresh_list, node, nnode, lsa)) {
				if(IS_DEBUG_OSPF(lsa, LSA_REFRESH)) {
					zlog_debug(
						"LSA[Refresh:%s]: ospf_lsa_refresh_walker(): "
	...
	ospf->t_lsa_refresher = thread_add_timer(master, ospf_lsa_refresh_walker, ospf, ospf->lsa_refresh_interval);
