...
#define OSPF_NSM_TIMER_ON(T, F, V) \
	do { \
		if(!(T)) (T) = thread_add_timer(master, (F), nbr, (V)); \
	} while(0)
...
#define OSPF_NSM_EVENT_SCHEDULE(N, E) thread_add_event(master, ospf_nsm_event, (N), (E))

/* Macro for OSPF NSM execute event. */
#define OSPF_NSM_EVENT_EXECUTE(N, E) thread_execute(master, ospf_nsm_event, (N), (E))
