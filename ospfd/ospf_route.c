...
int ospf_route_match_same(struct route_table *rt, struct prefix_ipv4 *prefix, struct ospf_route *newor) {
	struct route_node *rn;
	struct ospf_route * or ;
	struct ospf_path *op;
	...
	if(or->type == newor->type && or->cost == newor->cost) {
		if(or->type == OSPF_DESTINATION_NETWORK) {
			...
			/* Check each path. */
			for(n1 = listhead(or->paths), n2 = listhead(newor->paths); n1 && n2; n1 = listnextnode(n1), n2 = listnextnode(n2)) {
				op = listgetdata(n1);
 ...
 * area route
 */
static void ospf_route_delete_same_ext(struct route_table *external_routes, struct route_table *routes) {
...
/* rt: Old, cmprt: New */
static void ospf_route_delete_uniq(struct route_table *rt, struct route_table *cmprt) {
	struct route_node *rn;
	struct ospf_route * or ;

	for(rn = route_top(rt); rn; rn = route_next(rn)) {
		if((or = rn->info) != NULL) {
			if(or->path_type == OSPF_PATH_INTRA_AREA || or->path_type == OSPF_PATH_INTER_AREA) {
				if(or->type == OSPF_DESTINATION_NETWORK) {
					if(!ospf_route_exist_new_table(cmprt, (struct prefix_ipv4 *) &rn->p)) {
						ospf_zebra_delete((struct prefix_ipv4 *) &rn->p, or);
					}
				} else if(or->type == OSPF_DESTINATION_DISCARD) {
					if(!ospf_route_exist_new_table(cmprt, (struct prefix_ipv4 *) &rn->p)) {
...
void ospf_route_install(struct ospf *ospf, struct route_table *rt) {
	...
	for(rn = route_top(rt); rn; rn = route_next(rn)) {
		if((or = rn->info) != NULL) {
			if(or->type == OSPF_DESTINATION_NETWORK) {
				if(!ospf_route_match_same(ospf->old_table, (struct prefix_ipv4 *) &rn->p, or)) {
...
void ospf_intra_add_router(struct route_table *rt, struct vertex *v, struct ospf_area *area) {
	...
	if(IS_DEBUG_OSPF_EVENT) {
		zlog_debug("ospf_intra_add_router: LS ID: %s", inet_ntoa(lsa->header.id));
	}
	...
	if(IS_DEBUG_OSPF_EVENT) {
		zlog_debug("ospf_intra_add_router: talking about %s/%d", inet_ntoa(p.prefix), p.prefixlen);
	}
...
void ospf_intra_add_transit(struct route_table *rt, struct vertex *v, struct ospf_area *area) {
	...
	if(rn->info) {
		...
		if(v->distance > cur_or->cost || IPV4_ADDR_CMP(&cur_or->u.std.origin->id, &lsa->header.id) > 0) {
			return;
		}
...
void ospf_intra_add_stub(struct route_table *rt, struct router_lsa_link *link, struct vertex *v, struct ospf_area *area, int parent_is_root, int lsa_pos) {
	u_int32_t cost;
	struct route_node *rn;
	struct ospf_route * or ;
	struct prefix_ipv4 p;
	...
	if(IS_DEBUG_OSPF_EVENT) {
		zlog_debug("ospf_intra_add_stub(): processing route to %s/%d", inet_ntoa(p.prefix), p.prefixlen);
	}
	...
	if(IS_DEBUG_OSPF_EVENT) {
		zlog_debug("ospf_intra_add_stub(): calculated cost is %d + %d = %d", v->distance, ntohs(link->m[0].metric), cost);
	}
   ...
   */
	if(parent_is_root && link->link_data.s_addr == 0xffffffff && ospf_if_lookup_by_local_addr(area->ospf, NULL, link->link_id)) {
		if(IS_DEBUG_OSPF_EVENT) {
	...
	if(v != area->spf) {
		...
		if((oi = ospf_if_lookup_by_lsa_pos(area, lsa_pos))) {
			if(IS_DEBUG_OSPF_EVENT) {
				zlog_debug("ospf_intra_add_stub(): the interface is %s", IF_NAME(oi));
			}
...
const char *ospf_path_type_str[] = { "unknown-type", "intra-area", "inter-area", "type1-external", "type2-external" };
...
void ospf_route_table_dump(struct route_table *rt) {
  ...
  zlog_debug ("					Hop(s)	 Router(s)");
	...
	for(rn = route_top(rt); rn; rn = route_next(rn)) {
		if((or = rn->info) != NULL) {
			if(or->type == OSPF_DESTINATION_NETWORK) {
				zlog_debug("N %s/%d\t%s\t%s\t%d", inet_ntop(AF_INET, &rn->p.u.prefix4, buf1, BUFSIZ), rn->p.prefixlen, inet_ntop(AF_INET, & or->u.std.area_id, buf2, BUFSIZ), ospf_path_type_str[or->path_type], or->cost);
				for(ALL_LIST_ELEMENTS_RO(or->paths, pnode, path)) {
					zlog_debug("  -> %s", inet_ntoa(path->nexthop));
				}
			} else {
				zlog_debug("R %s\t%s\t%s\t%d", inet_ntop(AF_INET, &rn->p.u.prefix4, buf1, BUFSIZ), inet_ntop(AF_INET, & or->u.std.area_id, buf2, BUFSIZ), ospf_path_type_str[or->path_type], or->cost);
			}
		}
	}
	zlog_debug("========================================");
...
static int ospf_asbr_route_cmp(struct ospf *ospf, struct ospf_route *r1, struct ospf_route *r2) {
...
/* Compare two routes.
 ...
 ret >  0 -- r2 is better. */
int ospf_route_cmp(struct ospf *ospf, struct ospf_route *r1, struct ospf_route *r2) {
	int ret = 0;
	...
	switch(r1->path_type) {
		...
		case OSPF_PATH_INTER_AREA: break;
...
static int ospf_path_exist(struct list *plist, struct in_addr nexthop, struct ospf_interface *oi) {
...
void ospf_route_copy_nexthops_from_vertex(struct ospf_route *to, struct vertex *v) {
...
void ospf_route_subst(struct route_node *rn, struct ospf_route *new_or, struct ospf_route *over) {
	route_lock_node(rn);
	ospf_route_free(rn->info);
...
void ospf_route_add(struct route_table *rt, struct prefix_ipv4 *p, struct ospf_route *new_or, struct ospf_route *over) {
	struct route_node *rn;
	...
	for(rn = route_top(rt); rn; rn = next) {
		...
		if(rn->info != NULL) {
			...
			if(listcount(or->paths) == 0) {
				if(IS_DEBUG_OSPF_EVENT) {
					zlog_debug("Pruning route to %s/%d", inet_ntoa(rn->p.u.prefix4), rn->p.prefixlen);
				}
...
void ospf_prune_unreachable_routers(struct route_table *rtrs) {
	...
	for(rn = route_top(rtrs); rn; rn = next) {
		...
		for(ALL_LIST_ELEMENTS(paths, node, nnode, or)) {
			if(listcount(or->paths) == 0) {
				if(IS_DEBUG_OSPF_EVENT) {
					zlog_debug("Pruning route to rtr %s", inet_ntoa(rn->p.u.prefix4));
					zlog_debug("               via area %s", inet_ntoa(or->u.std.area_id));
				}
