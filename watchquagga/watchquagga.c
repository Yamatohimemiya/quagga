...
static const char *mode_str[] = {
	"monitor", "global restart", "individual daemon restart", "phased zebra restart", "phased global restart for any failure",
};
...
static const char *phase_str[] = {
	"None", "Stop jobs running", "Waiting for other daemons to come down", "Zebra restart job running", "Waiting for zebra to come up", "Start jobs running",
};
...
#define IS_UP(DMN) (((DMN)->state == DAEMON_UP) || ((DMN)->state == DAEMON_UNRESPONSIVE))
...
static pid_t run_background(char *shell_cmd) {
	...
	switch(child = fork()) {
		case -1: zlog_err("fork failed, cannot run command [%s]: %s", shell_cmd, safe_strerror(errno)); return -1;
		case 0:
			/* Child process. */
			...
			}
			{
				...
				execv("/bin/sh", argv);
				zlog_err("execv(/bin/sh -c '%s') failed: %s", shell_cmd, safe_strerror(errno));
				_exit(127);
			}
		default:
...
static int restart_kill(struct thread *t_kill) {
	...
	zlog_warn(
		...
		restart->what, restart->name, (int) restart->pid, (long) delay.tv_sec, (restart->kills ? SIGKILL : SIGTERM)
	);
	kill(-restart->pid, (restart->kills ? SIGKILL : SIGTERM));
	restart->kills++;
	restart->t_kill = thread_add_timer(master, restart_kill, restart, gs.restart_timeout);
	return 0;
}
...
static void sigchild(void) {
	...
	switch(child = waitpid(-1, &status, WNOHANG)) {
		case -1: zlog_err("waitpid failed: %s", safe_strerror(errno)); return;
		case 0: zlog_warn("SIGCHLD received, but waitpid did not reap a child"); return;
	}
	...
	if((restart = find_child(child)) != NULL) {
		...
		zlog_err("waitpid returned status for an unknown child process %d", (int) child);
		name = "(unknown)";
		what = "background";
	}
	if(WIFSTOPPED(status)) {
		zlog_warn("warning: %s %s process %d is stopped", what, name, (int) child);
	} else if(WIFSIGNALED(status)) {
		zlog_warn("%s %s process %d terminated due to signal %d", what, name, (int) child, WTERMSIG(status));
	} else if(WIFEXITED(status)) {
		if(WEXITSTATUS(status) != 0) {
			zlog_warn("%s %s process %d exited with non-zero status %d", what, name, (int) child, WEXITSTATUS(status));
		} else {
			zlog_debug("%s %s process %d exited normally", what, name, (int) child);
		}
	} else {
		zlog_err("cannot interpret %s %s process %d wait status 0x%x", what, name, (int) child, status);
	}
	phase_check();
}

static int run_job(struct restart_info *restart, const char *cmdtype, const char *command, int force, int update_interval) {
	struct timeval delay;
	...
	if(restart->pid) {
		if(gs.loglevel > LOG_DEBUG + 1) {
			zlog_debug("cannot %s %s, previous pid %d still running", cmdtype, restart->name, (int) restart->pid);
		}
		return -1;
	}
	...
	/* Note: time_elapsed test must come before the force test, since we need
     ...
     restart interval. */
	if((time_elapsed(&delay, &restart->time)->tv_sec < restart->interval) && !force) {
		if(gs.loglevel > LOG_DEBUG + 1) {
			zlog_debug(
				"postponing %s %s: "
	...
	if(update_interval) {
		...
		if(gs.loglevel > LOG_DEBUG + 1) {
			zlog_debug("restart %s interval is now %ld", restart->name, restart->interval);
		}
	}
	return restart->pid;
}

#define SET_READ_HANDLER(DMN) (DMN)->t_read = thread_add_read(master, handle_read, (DMN), (DMN)->fd)

#define SET_WAKEUP_DOWN(DMN) (DMN)->t_wakeup = thread_add_timer_msec(master, wakeup_down, (DMN), FUZZY(gs.period))

#define SET_WAKEUP_UNRESPONSIVE(DMN) (DMN)->t_wakeup = thread_add_timer_msec(master, wakeup_unresponsive, (DMN), FUZZY(gs.period))

#define SET_WAKEUP_ECHO(DMN) (DMN)->t_wakeup = thread_add_timer_msec(master, wakeup_send_echo, (DMN), FUZZY(gs.period))
...
static int wakeup_init(struct thread *t_wakeup) {
	...
	if(try_connect(dmn) < 0) {
		SET_WAKEUP_DOWN(dmn);
		zlog_err("%s state -> down : initial connection attempt failed", dmn->name);
...
static int handle_read(struct thread *t_read) {
	...
	if((rc = read(dmn->fd, buf, sizeof(buf))) < 0) {
		...
		snprintf(why, sizeof(why), "unexpected read error: %s", safe_strerror(errno));
		daemon_down(dmn, why);
		return 0;
	}
	if(rc == 0) {
	...
	if(!dmn->echo_sent.tv_sec) {
		...
		snprintf(why, sizeof(why), "unexpected read returns %d bytes: %.*s", (int) rc, (int) rc, buf);
		daemon_down(dmn, why);
		return 0;
	}
	...
	if(dmn->state == DAEMON_UNRESPONSIVE) {
		...
		zlog_debug("%s: echo response received after %ld.%06ld seconds", dmn->name, (long) delay.tv_sec, (long) delay.tv_usec);
	}
...
static int check_connect(struct thread *t_write) {
	...
	if(getsockopt(dmn->fd, SOL_SOCKET, SO_ERROR, (char *) &sockerr, &reslen) < 0) {
		zlog_warn("%s: check_connect: getsockopt failed: %s", dmn->name, safe_strerror(errno));
		daemon_down(dmn, "getsockopt failed checking connection success");
		return 0;
	}
	if((reslen == sizeof(sockerr)) && sockerr) {
		char why[100];
		snprintf(why, sizeof(why), "getsockopt reports that connection attempt failed: %s", safe_strerror(sockerr));
		daemon_down(dmn, why);
		return 0;
	}
...
static int wakeup_connect_hanging(struct thread *t_wakeup) {
	...
	dmn->t_wakeup = NULL;
	snprintf(why, sizeof(why), "connection attempt timed out after %ld seconds", gs.timeout);
	daemon_down(dmn, why);
	return 0;
}
...
static int try_connect(struct daemon *dmn) {
	...
	snprintf(addr.sun_path, sizeof(addr.sun_path), "%s/%s.vty", gs.vtydir, dmn->name);
	...
	if(access(addr.sun_path, W_OK) < 0) {
		if(errno != ENOENT) {
			zlog_err("%s: access to socket %s denied: %s", dmn->name, addr.sun_path, safe_strerror(errno));
		}
		return -1;
	}

	if((sock = socket(AF_UNIX, SOCK_STREAM, 0)) < 0) {
		zlog_err("%s(%s): cannot make socket: %s", __func__, addr.sun_path, safe_strerror(errno));
		return -1;
	}

	if(set_nonblocking(sock) < 0) {
		zlog_err("%s(%s): set_nonblocking(%d) failed", __func__, addr.sun_path, sock);
		close(sock);
		return -1;
	}
	...
	if(connect(sock, (struct sockaddr *) &addr, len) < 0) {
		if((errno != EINPROGRESS) && (errno != EWOULDBLOCK)) {
			if(gs.loglevel > LOG_DEBUG) {
				zlog_debug("%s(%s): connect failed: %s", __func__, addr.sun_path, safe_strerror(errno));
			}
			close(sock);
			return -1;
		}
		...
		dmn->t_wakeup = thread_add_timer(master, wakeup_connect_hanging, dmn, gs.timeout);
		SET_READ_HANDLER(dmn);
		return 0;
	}
...
static int phase_hanging(struct thread *t_hanging) {
	...
	zlog_err("Phase [%s] hanging for %ld seconds, aborting phased restart", phase_str[gs.phase], PHASE_TIMEOUT);
	gs.phase = PHASE_NONE;
	return 0;
}
...
static void set_phase(restart_phase_t new_phase) {
	...
	gs.t_phase_hanging = thread_add_timer(master, phase_hanging, NULL, PHASE_TIMEOUT);
}

static void phase_check(void) {
	switch(gs.phase) {
		case PHASE_NONE: break;
...
static void try_restart(struct daemon *dmn) {
	switch(gs.mode) {
		case MODE_MONITOR: return;
		case MODE_GLOBAL_RESTART: run_job(&gs.restart, "restart", gs.restart_command, 0, 1); break;
		case MODE_SEPARATE_RESTART: run_job(&dmn->restart, "restart", gs.restart_command, 0, 1); break;
		...
		case MODE_PHASED_ALL_RESTART:
			...
			{
				struct timeval delay;
				if(time_elapsed(&delay, &gs.special->restart.time)->tv_sec < gs.special->restart.interval) {
					if(gs.loglevel > LOG_DEBUG + 1) {
						zlog_debug(
			...
			break;
		default: zlog_err("error: unknown restart mode %d", gs.mode); break;
	}
}
...
static int wakeup_send_echo(struct thread *t_wakeup) {
	...
	dmn->t_wakeup = NULL;
	if(((rc = write(dmn->fd, echocmd, sizeof(echocmd))) < 0) || ((size_t) rc != sizeof(echocmd))) {
		char why[100 + sizeof(echocmd)];
		snprintf(why, sizeof(why), "write '%s' returned %d instead of %u", echocmd, (int) rc, (u_int) sizeof(echocmd));
		daemon_down(dmn, why);
	} else {
...
int main(int argc, char **argv) {
	...
	while((opt = getopt_long(argc, argv, "aAb:dek:l:m:M:i:p:r:R:S:s:t:T:zvh", longopts, 0)) != EOF) {
		switch(opt) {
			...
			case 'A':
				...
				break;
			case 'b': blankstr = optarg; break;
			case 'd': daemon_mode = 1; break;
			case 'e': gs.do_ping = 0; break;
			case 'k':
			...
			case 'l':
				{
					char garbage[3];
					if((sscanf(optarg, "%d%1s", &gs.loglevel, garbage) != 1) || (gs.loglevel < LOG_EMERG)) {
			...
			case 'm':
				{
					char garbage[3];
					if((sscanf(optarg, "%ld%1s", &gs.min_restart_interval, garbage) != 1) || (gs.min_restart_interval < 0)) {
			...
			case 'M':
				{
					char garbage[3];
					if((sscanf(optarg, "%ld%1s", &gs.max_restart_interval, garbage) != 1) || (gs.max_restart_interval < 0)) {
			...
			case 'i':
				{
					char garbage[3];
					int period;
					if((sscanf(optarg, "%d%1s", &period, garbage) != 1) || (gs.period < 1)) {
			...
			case 'r':
				if((gs.mode == MODE_GLOBAL_RESTART) || (gs.mode == MODE_SEPARATE_RESTART)) {
					fputs("Ambiguous operating mode selected.\n", stderr);
					return usage(progname, 1);
				}
				if(!valid_command(optarg)) {
					fprintf(stderr, "Invalid restart command, must contain '%%s': %s\n", optarg);
					return usage(progname, 1);
				}
			...
			case 'R':
				...
				if(strchr(optarg, '%')) {
					fprintf(stderr, "Invalid restart-all arg, must not contain '%%s': %s\n", optarg);
					return usage(progname, 1);
				}
				gs.restart_command = optarg;
			...
			case 's':
				...
				break;
			case 'S': gs.vtydir = optarg; break;
			case 't':
				{
					char garbage[3];
					if((sscanf(optarg, "%ld%1s", &gs.timeout, garbage) != 1) || (gs.timeout < 1)) {
			...
			case 'T':
				{
					char garbage[3];
					if((sscanf(optarg, "%ld%1s", &gs.restart_timeout, garbage) != 1) || (gs.restart_timeout < 1)) {
						fprintf(stderr, "Invalid restart timeout argument: %s\n", optarg);
						return usage(progname, 1);
					}
				}
				break;
			case 'z': gs.unresponsive_restart = 1; break;
			case 'v':
				printf("%s version %s\n", progname, QUAGGA_VERSION);
				puts("Copyright 2004 Andrew J. Schorr");
				return 0;
			case 'h': return usage(progname, 0);
			default: fputs("Invalid option.\n", stderr); return usage(progname, 1);
		}
	}
	...
	switch(gs.mode) {
		...
		case MODE_PHASED_ALL_RESTART:
			if(!gs.restart_command || !gs.start_command || !gs.stop_command) {
				fprintf(stderr, "Need start, kill, and restart commands in [%s] mode.\n", mode_str[gs.mode]);
				return usage(progname, 1);
			}
			break;
	}
...

	{
		...
		for(i = optind; i < argc; i++) {
			...
			gs.numdown++;
			dmn->fd = -1;
			dmn->t_wakeup = thread_add_timer_msec(master, wakeup_init, dmn, 100 + (random() % 900));
			...
			if(((gs.mode == MODE_PHASED_ZEBRA_RESTART) || (gs.mode == MODE_PHASED_ALL_RESTART)) && !strcmp(dmn->name, special)) {
				gs.special = dmn;
			}
		}
	}
	...
	if(((gs.mode == MODE_PHASED_ZEBRA_RESTART) || (gs.mode == MODE_PHASED_ALL_RESTART)) && !gs.special) {
	...
	zlog_default = openzlog(progname, ZLOG_NONE, LOG_CONS | LOG_NDELAY | LOG_PID, LOG_DAEMON);
	...
	/* Announce which daemons are being monitored. */
	{
...

		{
			...
			zlog_notice("%s %s watching [%s], mode [%s]", progname, QUAGGA_VERSION, buf, mode_str[gs.mode]);
		}
	}
