...
static char *ospf6_route_table_name(struct ospf6_route_table *table) {
	...
	switch(table->scope_type) {
		case OSPF6_SCOPE_TYPE_GLOBAL:
			{
				switch(table->table_type) {
					case OSPF6_TABLE_TYPE_ROUTES: snprintf(name, sizeof(name), "global route table"); break;
					case OSPF6_TABLE_TYPE_BORDER_ROUTERS: snprintf(name, sizeof(name), "global brouter table"); break;
					case OSPF6_TABLE_TYPE_EXTERNAL_ROUTES: snprintf(name, sizeof(name), "global external table"); break;
					default: snprintf(name, sizeof(name), "global unknown table"); break;
				}
			}
			break;
		...
		case OSPF6_SCOPE_TYPE_AREA:
			{
				...
				switch(table->table_type) {
					case OSPF6_TABLE_TYPE_SPF_RESULTS: snprintf(name, sizeof(name), "area %s spf table", oa->name); break;
					case OSPF6_TABLE_TYPE_ROUTES: snprintf(name, sizeof(name), "area %s route table", oa->name); break;
					case OSPF6_TABLE_TYPE_PREFIX_RANGES: snprintf(name, sizeof(name), "area %s range table", oa->name); break;
					case OSPF6_TABLE_TYPE_SUMMARY_PREFIXES: snprintf(name, sizeof(name), "area %s summary prefix table", oa->name); break;
					case OSPF6_TABLE_TYPE_SUMMARY_ROUTERS: snprintf(name, sizeof(name), "area %s summary router table", oa->name); break;
					default: snprintf(name, sizeof(name), "area %s unknown table", oa->name); break;
				}
			}
			break;
		...
		case OSPF6_SCOPE_TYPE_INTERFACE:
			{
				...
				switch(table->table_type) {
					case OSPF6_TABLE_TYPE_CONNECTED_ROUTES: snprintf(name, sizeof(name), "interface %s connected table", oi->interface->name); break;
					default: snprintf(name, sizeof(name), "interface %s unknown table", oi->interface->name); break;
				}
			}
			break;
		...
		default:
			{
				switch(table->table_type) {
					case OSPF6_TABLE_TYPE_SPF_RESULTS: snprintf(name, sizeof(name), "temporary spf table"); break;
					default: snprintf(name, sizeof(name), "temporary unknown table"); break;
				}
			}
			break;
	}
...
void ospf6_linkstate_prefix(u_int32_t adv_router, u_int32_t id, struct prefix *prefix) {
...
static int ospf6_route_cmp(struct ospf6_route *ra, struct ospf6_route *rb) {
	assert(ospf6_route_is_same(ra, rb));
	assert(OSPF6_PATH_TYPE_NONE < ra->path.type && ra->path.type < OSPF6_PATH_TYPE_MAX);
	assert(OSPF6_PATH_TYPE_NONE < rb->path.type && rb->path.type < OSPF6_PATH_TYPE_MAX);
...
struct ospf6_route *ospf6_route_lookup(struct prefix *prefix, struct ospf6_route_table *table) {
...
struct ospf6_route *ospf6_route_lookup_identical(struct ospf6_route *route, struct ospf6_route_table *table) {
	struct ospf6_route *target;

	for(target = ospf6_route_lookup(&route->prefix, table); target; target = target->next) {
...
struct ospf6_route *ospf6_route_lookup_bestmatch(struct prefix *prefix, struct ospf6_route_table *table) {
	struct route_node *node;
	struct ospf6_route *route;
...
struct ospf6_route *ospf6_route_add(struct ospf6_route *route, struct ospf6_route_table *table) {
	...
	if(IS_OSPF6_DEBUG_ROUTE(MEMORY)) {
		zlog_debug("%s %p: route add %p: %s", ospf6_route_table_name(table), (void *) table, (void *) route, buf);
	...
	if(old) {
		...
		if(ospf6_route_is_identical(old, route)) {
			if(IS_OSPF6_DEBUG_ROUTE(MEMORY)) {
				zlog_debug("%s %p: route add %p: needless update of %p", ospf6_route_table_name(table), (void *) table, (void *) route, (void *) old);
			} else if(IS_OSPF6_DEBUG_ROUTE(TABLE)) {
				zlog_debug("%s: route add: needless update", ospf6_route_table_name(table));
			}
		...
		if(IS_OSPF6_DEBUG_ROUTE(MEMORY)) {
			zlog_debug("%s %p: route add %p: update of %p", ospf6_route_table_name(table), (void *) table, (void *) route, (void *) old);
		} else if(IS_OSPF6_DEBUG_ROUTE(TABLE)) {
			zlog_debug("%s: route add: update", ospf6_route_table_name(table));
		}
	...
	if(prev || next) {
		if(IS_OSPF6_DEBUG_ROUTE(MEMORY)) {
			zlog_debug("%s %p: route add %p: another path: prev %p, next %p", ospf6_route_table_name(table), (void *) table, (void *) route, (void *) prev, (void *) next);
		} else if(IS_OSPF6_DEBUG_ROUTE(TABLE)) {
			zlog_debug("%s: route add: another path found", ospf6_route_table_name(table));
		}
		...
		if(node->info == next) {
			...
			SET_FLAG(route->flag, OSPF6_ROUTE_BEST);
			if(IS_OSPF6_DEBUG_ROUTE(MEMORY)) {
				zlog_info("%s %p: route add %p: replacing previous best: %p", ospf6_route_table_name(table), (void *) table, (void *) route, (void *) next);
			}
		}
	...
	if(IS_OSPF6_DEBUG_ROUTE(MEMORY)) {
		zlog_debug("%s %p: route add %p: brand new route", ospf6_route_table_name(table), (void *) table, (void *) route);
	} else if(IS_OSPF6_DEBUG_ROUTE(TABLE)) {
		zlog_debug("%s: route add: brand new route", ospf6_route_table_name(table));
	}
...
void ospf6_route_remove(struct ospf6_route *route, struct ospf6_route_table *table) {
	...
	if(IS_OSPF6_DEBUG_ROUTE(MEMORY)) {
		zlog_debug("%s %p: route remove %p: %s", ospf6_route_table_name(table), (void *) table, (void *) route, buf);
	} else if(IS_OSPF6_DEBUG_ROUTE(TABLE)) {
...
struct ospf6_route *ospf6_route_head(struct ospf6_route_table *table) {
	...
	if(IS_OSPF6_DEBUG_ROUTE(MEMORY)) {
		zlog_info("%s %p: route head: %p<-[%p]->%p", ospf6_route_table_name(table), (void *) table, (void *) route->prev, (void *) route, (void *) route->next);
	}
...
struct ospf6_route *ospf6_route_next(struct ospf6_route *route) {
	...
	if(IS_OSPF6_DEBUG_ROUTE(MEMORY)) {
		zlog_info("%s %p: route next: %p<-[%p]->%p", ospf6_route_table_name(route->table), (void *) route->table, (void *) route->prev, (void *) route, (void *) route->next);
	}
...
struct ospf6_route *ospf6_route_match_head(struct prefix *prefix, struct ospf6_route_table *table) {
	struct route_node *node;
	struct ospf6_route *route;

	/* Walk down tree. */
	node = table->table->top;
	while(node && node->p.prefixlen < prefix->prefixlen && prefix_match(&node->p, prefix)) {
...
struct ospf6_route *ospf6_route_match_next(struct prefix *prefix, struct ospf6_route *route) {
	struct ospf6_route *next;
...
void ospf6_route_show(struct vty *vty, struct ospf6_route *route) {
	...
	/* destination */
	if(route->type == OSPF6_DEST_TYPE_LINKSTATE) {
		ospf6_linkstate_prefix2str(&route->prefix, destination, sizeof(destination));
	} else if(route->type == OSPF6_DEST_TYPE_ROUTER) {
		inet_ntop(route->prefix.family, &route->prefix.u.prefix, destination, sizeof(destination));
	} else {
		prefix2str(&route->prefix, destination, sizeof(destination));
	}

	/* nexthop */
	inet_ntop(AF_INET6, &route->nexthop[0].address, nexthop, sizeof(nexthop));
	ifname = ifindex2ifname(route->nexthop[0].ifindex);

	vty_out(vty, "%c%1s %2s %-30s %-25s %6.*s %s%s", (ospf6_route_is_best(route) ? '*' : ' '), OSPF6_DEST_TYPE_SUBSTR(route->type), OSPF6_PATH_TYPE_SUBSTR(route->path.type), destination, nexthop, IFNAMSIZ, ifname, duration, VNL);

	for(i = 1; i < OSPF6_MULTI_PATH_LIMIT && ospf6_nexthop_is_set(&route->nexthop[i]); i++) {
		/* nexthop */
		inet_ntop(AF_INET6, &route->nexthop[i].address, nexthop, sizeof(nexthop));
		ifname = ifindex2ifname(route->nexthop[i].ifindex);

		vty_out(vty, "%c%1s %2s %-30s %-25s %6.*s %s%s", ' ', "", "", "", nexthop, IFNAMSIZ, ifname, "", VNL);
	}
}
...
void ospf6_route_show_detail(struct vty *vty, struct ospf6_route *route) {
	...
	/* destination type */
	vty_out(vty, "Destination type: %s%s", OSPF6_DEST_TYPE_NAME(route->type), VNL);
	...
	/* Debugging info */
	vty_out(vty, "Lock: %d Flags: %s%s%s%s%s", route->lock, (CHECK_FLAG(route->flag, OSPF6_ROUTE_BEST) ? "B" : "-"), (CHECK_FLAG(route->flag, OSPF6_ROUTE_ADD) ? "A" : "-"), (CHECK_FLAG(route->flag, OSPF6_ROUTE_REMOVE) ? "R" : "-"),
		(CHECK_FLAG(route->flag, OSPF6_ROUTE_CHANGE) ? "C" : "-"), VNL);
	vty_out(vty, "Memory: prev: %p this: %p next: %p%s", (void *) route->prev, (void *) route, (void *) route->next, VNL);
	...
	/* Path type */
	vty_out(vty, "Path Type: %s%s", OSPF6_PATH_TYPE_NAME(route->path.type), VNL);

	/* LS Origin */
	inet_ntop(AF_INET, &route->path.origin.id, id, sizeof(id));
	inet_ntop(AF_INET, &route->path.origin.adv_router, adv_router, sizeof(adv_router));
	vty_out(vty, "LS Origin: %s Id: %s Adv: %s%s", ospf6_lstype_name(route->path.origin.type), id, adv_router, VNL);
	...
	vty_out(vty, "Metric: %d (%d)%s", route->path.cost, route->path.cost_e2, VNL);

	/* Nexthops */
	vty_out(vty, "Nexthop:%s", VNL);
	for(i = 0; i < OSPF6_MULTI_PATH_LIMIT && ospf6_nexthop_is_set(&route->nexthop[i]); i++) {
		/* nexthop */
		inet_ntop(AF_INET6, &route->nexthop[i].address, nexthop, sizeof(nexthop));
		ifname = ifindex2ifname(route->nexthop[i].ifindex);
...
static void ospf6_route_show_table_summary(struct vty *vty, struct ospf6_route_table *table) {
	...
	for(i = OSPF6_PATH_TYPE_INTRA; i <= OSPF6_PATH_TYPE_EXTERNAL2; i++) {
		vty_out(vty, "Number of %s routes: %d%s", OSPF6_PATH_TYPE_NAME(i), pathtype[i], VNL);
	}
}

static void ospf6_route_show_table_prefix(struct vty *vty, struct prefix *prefix, struct ospf6_route_table *table) {
	struct ospf6_route *route;
...
static void ospf6_route_show_table_address(struct vty *vty, struct prefix *prefix, struct ospf6_route_table *table) {
	struct ospf6_route *route;
...
static void ospf6_route_show_table_match(struct vty *vty, int detail, struct prefix *prefix, struct ospf6_route_table *table) {
	struct ospf6_route *route;
...
static void ospf6_route_show_table_type(struct vty *vty, int detail, u_char type, struct ospf6_route_table *table) {
	struct ospf6_route *route;
...
static void ospf6_route_show_table(struct vty *vty, int detail, struct ospf6_route_table *table) {
	struct ospf6_route *route;
...
int ospf6_route_table_show(struct vty *vty, int argc, const char *argv[], struct ospf6_route_table *table) {
	int summary = 0;
	int match = 0;
	int detail = 0;
	int slash = 0;
...
static void ospf6_linkstate_show_header(struct vty *vty) {
	vty_out(vty, "%-7s %-15s %-15s %-8s %-14s %s%s", "Type", "Router-ID", "Net-ID", "Rtr-Bits", "Options", "Cost", VNL);
}
...
static void ospf6_linkstate_show(struct vty *vty, struct ospf6_route *route) {
	...
	if(ntohl(id)) {
		vty_out(vty, "%-7s %-15s %-15s %-8s %-14s %lu%s", "Network", routername, idname, rbits, options, (unsigned long) route->path.cost, VNL);
	} else {
		vty_out(vty, "%-7s %-15s %-15s %-8s %-14s %lu%s", "Router", routername, idname, rbits, options, (unsigned long) route->path.cost, VNL);
	}
}

static void ospf6_linkstate_show_table_exact(struct vty *vty, struct prefix *prefix, struct ospf6_route_table *table) {
	struct ospf6_route *route;
...
static void ospf6_linkstate_show_table(struct vty *vty, int detail, struct ospf6_route_table *table) {
	struct ospf6_route *route;
...
int ospf6_linkstate_table_show(struct vty *vty, int argc, const char *argv[], struct ospf6_route_table *table) {
	int detail = 0;
	int is_id = 0;
	int is_router = 0;
	int i, ret;
	...
	if(is_router) {
		ospf6_linkstate_prefix(router.u.prefix4.s_addr, id.u.prefix4.s_addr, &prefix);
	}
...
void ospf6_brouter_show_header(struct vty *vty) {
	vty_out(vty, "%-15s %-8s %-14s %-10s %-15s%s", "Router-ID", "Rtr-Bits", "Options", "Path-Type", "Area", VNL);
}
...
DEFUN(debug_ospf6_route, debug_ospf6_route_cmd, "debug ospf6 route (table|intra-area|inter-area|memory)",
      DEBUG_STR OSPF6_STR "Debug route table calculation\n"
			  "Debug detail\n"
...
DEFUN(no_debug_ospf6_route, no_debug_ospf6_route_cmd, "no debug ospf6 route (table|intra-area|inter-area|memory)",
      NO_STR DEBUG_STR OSPF6_STR "Debug route table calculation\n"
				 "Debug intra-area route calculation\n"
				 "Debug route memory use\n") {
	unsigned char level = 0;
