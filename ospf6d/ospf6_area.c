	...
	switch(ntohs(lsa->header->type)) {
		...
		case OSPF6_LSTYPE_NETWORK:
			if(IS_OSPF6_DEBUG_EXAMIN_TYPE(lsa->header->type)) {
				...
				zlog_debug("Schedule SPF Calculation for %s", OSPF6_AREA(lsa->lsdb->data)->name);
			}
			ospf6_spf_schedule(OSPF6_PROCESS(OSPF6_AREA(lsa->lsdb->data)->ospf6), ospf6_lsadd_to_spf_reason(lsa));
			break;

		case OSPF6_LSTYPE_INTRA_PREFIX: ospf6_intra_prefix_lsa_add(lsa); break;

		case OSPF6_LSTYPE_INTER_PREFIX:
		case OSPF6_LSTYPE_INTER_ROUTER: ospf6_abr_examin_summary(lsa, (struct ospf6_area *) lsa->lsdb->data); break;
...
static void ospf6_area_lsdb_hook_remove(struct ospf6_lsa *lsa) {
	switch(ntohs(lsa->header->type)) {
		...
		case OSPF6_LSTYPE_NETWORK:
			...
			ospf6_spf_schedule(OSPF6_PROCESS(OSPF6_AREA(lsa->lsdb->data)->ospf6), ospf6_lsremove_to_spf_reason(lsa));
			break;

		case OSPF6_LSTYPE_INTRA_PREFIX: ospf6_intra_prefix_lsa_remove(lsa); break;
...
struct ospf6_area *ospf6_area_create(u_int32_t area_id, struct ospf6 *o) {
	...
	for(route = ospf6_route_head(o->route_table); route; route = ospf6_route_next(route)) {
...
static struct ospf6_area *ospf6_area_get(u_int32_t area_id, struct ospf6 *o) {
	struct ospf6_area *oa;
...
void ospf6_area_show(struct vty *vty, struct ospf6_area *oa) {
	...
	vty_out(vty, "     Number of Area scoped LSAs is %u%s", oa->lsdb->count, VNL);
...
DEFUN(area_range, area_range_cmd, "area A.B.C.D range X:X::X:X/M",
...
ALIAS(area_range, area_range_advertise_cmd, "area A.B.C.D range X:X::X:X/M (advertise|not-advertise)",
      "OSPF area parameters\n" OSPF6_AREA_ID_STR "Configured address range\n"
      "Specify IPv6 prefix\n")

DEFUN(no_area_range, no_area_range_cmd, "no area A.B.C.D range X:X::X:X/M",
      "OSPF area parameters\n" OSPF6_AREA_ID_STR "Configured address range\n"
      "Specify IPv6 prefix\n") {
	int ret;
	struct ospf6_area *oa;
...
void ospf6_area_config_write(struct vty *vty) {
	...
	for(ALL_LIST_ELEMENTS_RO(ospf6->area_list, node, oa)) {
		...
		if(PREFIX_NAME_IN(oa)) {
			vty_out(vty, " area %s filter-list prefix %s in%s", oa->name, PREFIX_NAME_IN(oa), VNL);
		}
		if(PREFIX_NAME_OUT(oa)) {
			vty_out(vty, " area %s filter-list prefix %s out%s", oa->name, PREFIX_NAME_OUT(oa), VNL);
		}
		if(IMPORT_NAME(oa)) {
			vty_out(vty, " area %s import-list %s%s", oa->name, IMPORT_NAME(oa), VNL);
		}
		if(EXPORT_NAME(oa)) {
			vty_out(vty, " area %s export-list %s%s", oa->name, EXPORT_NAME(oa), VNL);
		}
	}
}

DEFUN(area_filter_list, area_filter_list_cmd, "area A.B.C.D filter-list prefix WORD (in|out)",
...
DEFUN(no_area_filter_list, no_area_filter_list_cmd, "no area A.B.C.D filter-list prefix WORD (in|out)",
      NO_STR "OSPFv6 area parameters\n"
	     "OSPFv6 area ID in IP address format\n"
	     "Filter networks between OSPFv6 areas\n"
	     "Filter prefixes between OSPFv6 areas\n"
	     "Name of an IPv6 prefix-list\n"
...
DEFUN(area_import_list, area_import_list_cmd, "area A.B.C.D import-list NAME",
      "OSPFv6 area parameters\n"
      "OSPFv6 area ID in IP address format\n"
...
DEFUN(no_area_import_list, no_area_import_list_cmd, "no area A.B.C.D import-list NAME",
      "OSPFv6 area parameters\n"
      "OSPFv6 area ID in IP address format\n"
...
DEFUN(area_export_list, area_export_list_cmd, "area A.B.C.D export-list NAME",
      "OSPFv6 area parameters\n"
      "OSPFv6 area ID in IP address format\n"
...
DEFUN(no_area_export_list, no_area_export_list_cmd, "no area A.B.C.D export-list NAME",
      "OSPFv6 area parameters\n"
      "OSPFv6 area ID in IP address format\n"
      "Unset the filter for networks announced to other areas\n"
      "Name of the access-list\n") {
...
DEFUN(show_ipv6_ospf6_spf_tree, show_ipv6_ospf6_spf_tree_cmd, "show ipv6 ospf6 spf tree",
	...
	for(ALL_LIST_ELEMENTS_RO(ospf6->area_list, node, oa)) {
		...
		if(route == NULL) {
			vty_out(vty, "LS entry for root not found in area %s%s", oa->name, VNL);
			continue;
		}
...
DEFUN(show_ipv6_ospf6_area_spf_tree, show_ipv6_ospf6_area_spf_tree_cmd, "show ipv6 ospf6 area A.B.C.D spf tree",
      SHOW_STR IP6_STR OSPF6_STR OSPF6_AREA_STR OSPF6_AREA_ID_STR "Shortest Path First calculation\n"
								  "Show SPF tree\n") {
	u_int32_t area_id;
	struct ospf6_area *oa;
	struct ospf6_vertex *root;
...
DEFUN(show_ipv6_ospf6_simulate_spf_tree_root, show_ipv6_ospf6_simulate_spf_tree_root_cmd, "show ipv6 ospf6 simulate spf-tree A.B.C.D area A.B.C.D",
      SHOW_STR IP6_STR OSPF6_STR "Shortest Path First calculation\n"
				 "Show SPF tree\n"
