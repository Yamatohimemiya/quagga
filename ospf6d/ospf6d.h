...
#define timerstring(tv, buf, size) \
	do { \
		if((tv)->tv_sec / 60 / 60 / 24) snprintf(buf, size, "%lldd%02lld:%02lld:%02lld", (tv)->tv_sec / 60LL / 60 / 24, (tv)->tv_sec / 60LL / 60 % 24, (tv)->tv_sec / 60LL % 60, (tv)->tv_sec % 60LL); \
		else snprintf(buf, size, "%02lld:%02lld:%02lld", (tv)->tv_sec / 60LL / 60 % 24, (tv)->tv_sec / 60LL % 60, (tv)->tv_sec % 60LL); \
	} while(0)
#define timerstring_local(tv, buf, size) \
	do { \
		...
		if(ret == 0) zlog_warn("strftime error"); \
	} while(0)

#define threadtimer_string(now, t, buf, size) \
	do { \
		struct timeval result; \
		if(!t) snprintf(buf, size, "inactive"); \
		else { \
