...
#define OSPF6_DEBUG_BROUTER_ON() (conf_debug_ospf6_brouter |= OSPF6_DEBUG_BROUTER_SUMMARY)
#define OSPF6_DEBUG_BROUTER_OFF() (conf_debug_ospf6_brouter &= ~OSPF6_DEBUG_BROUTER_SUMMARY)
#define IS_OSPF6_DEBUG_BROUTER (conf_debug_ospf6_brouter & OSPF6_DEBUG_BROUTER_SUMMARY)
...
#define IS_OSPF6_DEBUG_BROUTER_SPECIFIC_ROUTER (conf_debug_ospf6_brouter & OSPF6_DEBUG_BROUTER_SPECIFIC_ROUTER)
#define IS_OSPF6_DEBUG_BROUTER_SPECIFIC_ROUTER_ID(router_id) (IS_OSPF6_DEBUG_BROUTER_SPECIFIC_ROUTER && conf_debug_ospf6_brouter_specific_router_id == (router_id))
...
#define IS_OSPF6_DEBUG_BROUTER_SPECIFIC_AREA (conf_debug_ospf6_brouter & OSPF6_DEBUG_BROUTER_SPECIFIC_AREA)
#define IS_OSPF6_DEBUG_BROUTER_SPECIFIC_AREA_ID(area_id) (IS_OSPF6_DEBUG_BROUTER_SPECIFIC_AREA && conf_debug_ospf6_brouter_specific_area_id == (area_id))
...
#define ROUTER_LSDESC_IS_TYPE(t, x) ((((struct ospf6_router_lsdesc *) (x))->type == OSPF6_ROUTER_LSDESC_##t) ? 1 : 0)
#define ROUTER_LSDESC_GET_METRIC(x) (ntohs(((struct ospf6_router_lsdesc *) (x))->metric))
#define ROUTER_LSDESC_GET_IFID(x) (ntohl(((struct ospf6_router_lsdesc *) (x))->interface_id))
#define ROUTER_LSDESC_GET_NBR_IFID(x) (ntohl(((struct ospf6_router_lsdesc *) (x))->neighbor_interface_id))
#define ROUTER_LSDESC_GET_NBR_ROUTERID(x) (((struct ospf6_router_lsdesc *) (x))->neighbor_router_id)
...
#define NETWORK_LSDESC_GET_NBR_ROUTERID(x) (((struct ospf6_network_lsdesc *) (x))->router_id)
...
#define OSPF6_ROUTER_LSA_SCHEDULE(oa) \
	do { \
		if(!(oa)->thread_router_lsa && CHECK_FLAG((oa)->flag, OSPF6_AREA_ENABLE)) (oa)->thread_router_lsa = thread_add_event(master, ospf6_router_lsa_originate, oa, 0); \
	} while(0)
#define OSPF6_NETWORK_LSA_SCHEDULE(oi) \
	do { \
		if(!(oi)->thread_network_lsa && !CHECK_FLAG((oi)->flag, OSPF6_INTERFACE_DISABLE)) (oi)->thread_network_lsa = thread_add_event(master, ospf6_network_lsa_originate, oi, 0); \
	} while(0)
#define OSPF6_LINK_LSA_SCHEDULE(oi) \
	do { \
		if(!(oi)->thread_link_lsa && !CHECK_FLAG((oi)->flag, OSPF6_INTERFACE_DISABLE)) (oi)->thread_link_lsa = thread_add_event(master, ospf6_link_lsa_originate, oi, 0); \
	} while(0)
#define OSPF6_INTRA_PREFIX_LSA_SCHEDULE_STUB(oa) \
	do { \
		if(!(oa)->thread_intra_prefix_lsa && CHECK_FLAG((oa)->flag, OSPF6_AREA_ENABLE)) (oa)->thread_intra_prefix_lsa = thread_add_event(master, ospf6_intra_prefix_lsa_originate_stub, oa, 0); \
	} while(0)
#define OSPF6_INTRA_PREFIX_LSA_SCHEDULE_TRANSIT(oi) \
	do { \
		if(!(oi)->thread_intra_prefix_lsa && !CHECK_FLAG((oi)->flag, OSPF6_INTERFACE_DISABLE)) (oi)->thread_intra_prefix_lsa = thread_add_event(master, ospf6_intra_prefix_lsa_originate_transit, oi, 0); \
	} while(0)
...
/* Function Prototypes */
extern char *ospf6_router_lsdesc_lookup(u_char type, u_int32_t interface_id, u_int32_t neighbor_interface_id, u_int32_t neighbor_router_id, struct ospf6_lsa *lsa);
extern char *ospf6_network_lsdesc_lookup(u_int32_t router_id, struct ospf6_lsa *lsa);
