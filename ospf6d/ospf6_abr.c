...
void ospf6_abr_enable_area(struct ospf6_area *area) {
	...
	for(ALL_LIST_ELEMENTS(area->ospf6->area_list, node, nnode, oa)) {
		...
		if(oa != area) {
			for(ro = ospf6_route_head(oa->range_table); ro; ro = ospf6_route_next(ro)) {
	...
	for(ro = ospf6_route_head(area->ospf6->brouter_table); ro; ro = ospf6_route_next(ro)) {
		ospf6_abr_originate_summary_to_area(ro, area);
	}

	/* install calculated routes to network (may be rejected by ranges) */
	for(ro = ospf6_route_head(area->ospf6->route_table); ro; ro = ospf6_route_next(ro)) {
		ospf6_abr_originate_summary_to_area(ro, area);
	}
}
...
void ospf6_abr_disable_area(struct ospf6_area *area) {
	...
	for(ro = ospf6_route_head(area->summary_prefix); ro; ro = nro) {
		...
		old = ospf6_lsdb_lookup(ro->path.origin.type, ro->path.origin.id, area->ospf6->router_id, area->lsdb);
		if(old) {
			ospf6_lsa_purge(old);
		}
...
void ospf6_abr_originate_summary_to_area(struct ospf6_route *route, struct ospf6_area *area) {
	...
	if(route->type == OSPF6_DEST_TYPE_ROUTER) {
		if(IS_OSPF6_DEBUG_ABR || IS_OSPF6_DEBUG_ORIGINATE(INTER_ROUTER)) {
			is_debug++;
			inet_ntop(AF_INET, &(ADV_ROUTER_IN_PREFIX(&route->prefix)), buf, sizeof(buf));
			zlog_debug("Originating summary in area %s for ASBR %s", area->name, buf);
		}
		...
		if(IS_OSPF6_DEBUG_ABR || IS_OSPF6_DEBUG_ORIGINATE(INTER_PREFIX)) {
			...
			zlog_debug("Originating summary in area %s for %s", area->name, buf);
		}
	...
	if(summary) {
		old = ospf6_lsdb_lookup(summary->path.origin.type, summary->path.origin.id, area->ospf6->router_id, area->lsdb);
	}
	...
	if(route->type != OSPF6_DEST_TYPE_NETWORK && route->type != OSPF6_DEST_TYPE_RANGE && (route->type != OSPF6_DEST_TYPE_ROUTER || !CHECK_FLAG(route->path.router_bits, OSPF6_ROUTER_BIT_E))) {
		if(is_debug) {
	...
	if(route->path.type == OSPF6_PATH_TYPE_EXTERNAL1 || route->path.type == OSPF6_PATH_TYPE_EXTERNAL2) {
		if(is_debug) {
	...
	if(route->type == OSPF6_DEST_TYPE_NETWORK && route->path.type == OSPF6_PATH_TYPE_INTRA) {
		/* search for configured address range for the route's area */
		route_area = ospf6_area_lookup(route->path.area_id, area->ospf6);
		assert(route_area);
		range = ospf6_route_lookup_bestmatch(&route->prefix, route_area->range_table);
		...
		if(range && !CHECK_FLAG(range->flag, OSPF6_ROUTE_REMOVE) && (route->path.area_id != OSPF_AREA_BACKBONE || !IS_AREA_TRANSIT(area))) {
			if(is_debug) {
				prefix2str(&range->prefix, buf, sizeof(buf));
				zlog_debug("Suppressed by range %s of area %s", buf, route_area->name);
			}
	...
	if(EXPORT_NAME(area)) {
		if(EXPORT_LIST(area) == NULL) {
			EXPORT_LIST(area) = access_list_lookup(AFI_IP6, EXPORT_NAME(area));
		}

		if(EXPORT_LIST(area)) {
			if(access_list_apply(EXPORT_LIST(area), &route->prefix) == FILTER_DENY) {
				if(is_debug) {
					inet_ntop(AF_INET, &(ADV_ROUTER_IN_PREFIX(&route->prefix)), buf, sizeof(buf));
	...
	if(PREFIX_NAME_OUT(area)) {
		if(PREFIX_LIST_OUT(area) == NULL) {
			PREFIX_LIST_OUT(area) = prefix_list_lookup(AFI_IP6, PREFIX_NAME_OUT(area));
		}

		if(PREFIX_LIST_OUT(area)) {
			if(prefix_list_apply(PREFIX_LIST_OUT(area), &route->prefix) != PREFIX_PERMIT) {
				if(is_debug) {
					inet_ntop(AF_INET, &(ADV_ROUTER_IN_PREFIX(&route->prefix)), buf, sizeof(buf));
	...
	if(summary == NULL) {
		...
		if(route->type == OSPF6_DEST_TYPE_NETWORK || route->type == OSPF6_DEST_TYPE_RANGE) {
		...
		summary->path.origin.id = ospf6_new_ls_id(summary->path.origin.type, summary->path.origin.adv_router, area->lsdb);
	...
	if(route->type == OSPF6_DEST_TYPE_ROUTER) {
		router_lsa = (struct ospf6_inter_router_lsa *) ((caddr_t) lsa_header + sizeof(struct ospf6_lsa_header));
		...
		prefix_lsa = (struct ospf6_inter_prefix_lsa *) ((caddr_t) lsa_header + sizeof(struct ospf6_lsa_header));
	...
	lsa_header->seqnum = ospf6_new_ls_seqnum(lsa_header->type, lsa_header->id, lsa_header->adv_router, area->lsdb);
...
static void ospf6_abr_range_update(struct ospf6_route *range) {
	...
	for(ro = ospf6_route_match_head(&range->prefix, ospf6->route_table); ro; ro = ospf6_route_match_next(&range->prefix, ro)) {
		if(ro->path.area_id == range->path.area_id && !CHECK_FLAG(ro->flag, OSPF6_ROUTE_REMOVE)) {
...
void ospf6_abr_examin_summary(struct ospf6_lsa *lsa, struct ospf6_area *oa) {
	...
	if(lsa->header->type == htons(OSPF6_LSTYPE_INTER_PREFIX)) {
		...
		prefix_lsa = (struct ospf6_inter_prefix_lsa *) OSPF6_LSA_HEADER_END(lsa->header);
		...
		router_lsa = (struct ospf6_inter_router_lsa *) OSPF6_LSA_HEADER_END(lsa->header);
	...
	while(route && ospf6_route_is_prefix(&prefix, route)) {
		if(route->path.area_id == oa->area_id && route->path.origin.type == lsa->header->type && route->path.origin.id == lsa->header->id && route->path.origin.adv_router == lsa->header->adv_router
	...
	if(lsa->header->type == htons(OSPF6_LSTYPE_INTER_PREFIX)) {
		...
		if(CHECK_FLAG(prefix_lsa->prefix.prefix_options, OSPF6_PREFIX_OPTION_NU) || CHECK_FLAG(prefix_lsa->prefix.prefix_options, OSPF6_PREFIX_OPTION_LA)) {
			if(is_debug) {
	...
	if(lsa->header->type == htons(OSPF6_LSTYPE_INTER_ROUTER)) {
		/* To pass test suites */
		if(!OSPF6_OPT_ISSET(router_lsa->options, OSPF6_OPT_R) || !OSPF6_OPT_ISSET(router_lsa->options, OSPF6_OPT_V6)) {
			if(is_debug) {
				zlog_debug("Prefix has NU/LA bit set, ignore");
			}
			if(old) {
	...
	if(abr_entry == NULL || abr_entry->path.area_id != oa->area_id || CHECK_FLAG(abr_entry->flag, OSPF6_ROUTE_REMOVE) || !CHECK_FLAG(abr_entry->path.router_bits, OSPF6_ROUTER_BIT_B)) {
		if(is_debug) {
...
void ospf6_abr_examin_brouter(u_int32_t router_id) {
	...
	for(ALL_LIST_ELEMENTS(ospf6->area_list, node, nnode, oa)) {
		type = htons(OSPF6_LSTYPE_INTER_ROUTER);
		for(lsa = ospf6_lsdb_type_router_head(type, router_id, oa->lsdb); lsa; lsa = ospf6_lsdb_type_router_next(type, router_id, lsa)) {
...
/* Display functions */
static char *ospf6_inter_area_prefix_lsa_get_prefix_str(struct ospf6_lsa *lsa, char *buf, int buflen, int pos) {
	struct ospf6_inter_prefix_lsa *prefix_lsa;
	struct in6_addr in6;
...
static int ospf6_inter_area_prefix_lsa_show(struct vty *vty, struct ospf6_lsa *lsa) {
	struct ospf6_inter_prefix_lsa *prefix_lsa;
	char buf[INET6_ADDRSTRLEN];

	prefix_lsa = (struct ospf6_inter_prefix_lsa *) OSPF6_LSA_HEADER_END(lsa->header);

	vty_out(vty, "     Metric: %lu%s", (u_long) OSPF6_ABR_SUMMARY_METRIC(prefix_lsa), VNL);

	ospf6_prefix_options_printbuf(prefix_lsa->prefix.prefix_options, buf, sizeof(buf));
	vty_out(vty, "     Prefix Options: %s%s", buf, VNL);

	vty_out(vty, "     Prefix: %s%s", ospf6_inter_area_prefix_lsa_get_prefix_str(lsa, buf, sizeof(buf), 0), VNL);

	return 0;
}

static char *ospf6_inter_area_router_lsa_get_prefix_str(struct ospf6_lsa *lsa, char *buf, int buflen, int pos) {
	struct ospf6_inter_router_lsa *router_lsa;
...
static int ospf6_inter_area_router_lsa_show(struct vty *vty, struct ospf6_lsa *lsa) {
	struct ospf6_inter_router_lsa *router_lsa;
	char buf[64];
	...
	vty_out(vty, "     Metric: %lu%s", (u_long) OSPF6_ABR_SUMMARY_METRIC(router_lsa), VNL);
...
/* Debug commands */
DEFUN(debug_ospf6_abr, debug_ospf6_abr_cmd, "debug ospf6 abr", DEBUG_STR OSPF6_STR "Debug OSPFv3 ABR function\n") {
	OSPF6_DEBUG_ABR_ON();
	return CMD_SUCCESS;
}

DEFUN(no_debug_ospf6_abr, no_debug_ospf6_abr_cmd, "no debug ospf6 abr", NO_STR DEBUG_STR OSPF6_STR "Debug OSPFv3 ABR function\n") {
	OSPF6_DEBUG_ABR_OFF();
	return CMD_SUCCESS;
}
...
struct ospf6_lsa_handler inter_prefix_handler = {
	OSPF6_LSTYPE_INTER_PREFIX, "Inter-Prefix", "IAP", ospf6_inter_area_prefix_lsa_show, ospf6_inter_area_prefix_lsa_get_prefix_str,
};

struct ospf6_lsa_handler inter_router_handler = {
	OSPF6_LSTYPE_INTER_ROUTER, "Inter-Router", "IAR", ospf6_inter_area_router_lsa_show, ospf6_inter_area_router_lsa_get_prefix_str,
};
