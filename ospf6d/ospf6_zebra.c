...
static int ospf6_router_id_update_zebra(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
...
static int ospf6_zebra_if_add(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct interface *ifp;

	ifp = zebra_interface_add_read(zclient->ibuf, vrf_id);
	if(IS_OSPF6_DEBUG_ZEBRA(RECV)) {
		zlog_debug("Zebra Interface add: %s index %d mtu %d", ifp->name, ifp->ifindex, ifp->mtu6);
	}
	ospf6_interface_if_add(ifp);
	return 0;
}

static int ospf6_zebra_if_del(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct interface *ifp;
	...
	if(IS_OSPF6_DEBUG_ZEBRA(RECV)) {
		zlog_debug("Zebra Interface delete: %s index %d mtu %d", ifp->name, ifp->ifindex, ifp->mtu6);
	}
...
static int ospf6_zebra_if_state_update(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct interface *ifp;
	...
	if(IS_OSPF6_DEBUG_ZEBRA(RECV)) {
		zlog_debug(
			...
			ifp->name, ifp->ifindex, (unsigned long long) ifp->flags, ifp->metric, ifp->mtu6, ifp->bandwidth
		);
	}
...
static int ospf6_zebra_if_address_update_add(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct connected *c;
	char buf[128];

	c = zebra_interface_address_read(ZEBRA_INTERFACE_ADDRESS_ADD, zclient->ibuf, vrf_id);
	if(c == NULL) {
		return 0;
	}

	if(IS_OSPF6_DEBUG_ZEBRA(RECV)) {
		zlog_debug("Zebra Interface address add: %s %5s %s/%d", c->ifp->name, prefix_family_str(c->address), inet_ntop(c->address->family, &c->address->u.prefix, buf, sizeof(buf)), c->address->prefixlen);
	}
...
static int ospf6_zebra_if_address_update_delete(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct connected *c;
	char buf[128];

	c = zebra_interface_address_read(ZEBRA_INTERFACE_ADDRESS_DELETE, zclient->ibuf, vrf_id);
	if(c == NULL) {
		return 0;
	}

	if(IS_OSPF6_DEBUG_ZEBRA(RECV)) {
		zlog_debug("Zebra Interface address delete: %s %5s %s/%d", c->ifp->name, prefix_family_str(c->address), inet_ntop(c->address->family, &c->address->u.prefix, buf, sizeof(buf)), c->address->prefixlen);
	}
...
static int ospf6_zebra_read_ipv6(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct stream *s;
	...
	if(CHECK_FLAG(api.message, ZAPI_MESSAGE_NEXTHOP)) {
		...
		nexthop = (struct in6_addr *) malloc(api.nexthop_num * sizeof(struct in6_addr));
	...
	if(IS_OSPF6_DEBUG_ZEBRA(RECV)) {
		...
		zlog_debug("Zebra Receive route %s: %s %s nexthop %s ifindex %ld tag %u", (command == ZEBRA_IPV6_ROUTE_ADD ? "add" : "delete"), zebra_route_string(api.type), prefixstr, nexthopstr, ifindex, api.tag);
	}

	if(command == ZEBRA_IPV6_ROUTE_ADD) {
		ospf6_asbr_redistribute_add(api.type, ifindex, (struct prefix *) &p, api.nexthop_num, nexthop, api.tag);
	} else {
...
DEFUN(show_zebra, show_zebra_cmd, "show zebra", SHOW_STR "Zebra information\n") {
	...
	vty_out(vty, "  enable: %d fail: %d%s", zclient->enable, zclient->fail, VNL);
	vty_out(vty, "  redistribute default: %d%s", vrf_bitmap_check(zclient->default_information, VRF_DEFAULT), VNL);
...
DEFUN(router_zebra, router_zebra_cmd, "router zebra",
...
DEFUN(no_router_zebra, no_router_zebra_cmd, "no router zebra",
      NO_STR "Configure routing process\n"
...
static void ospf6_zebra_route_update(int type, struct ospf6_route *request) {
	...
	if(IS_OSPF6_DEBUG_ZEBRA(SEND)) {
		...
		zlog_debug("Send %s route: %s", (type == REM ? "remove" : "add"), buf);
	}
	...
	if(request->path.origin.adv_router == ospf6->router_id && (request->path.type == OSPF6_PATH_TYPE_EXTERNAL1 || request->path.type == OSPF6_PATH_TYPE_EXTERNAL2)) {
		if(IS_OSPF6_DEBUG_ZEBRA(SEND)) {
	...
	if(type == REM && ospf6_route_is_best(request) && request->next && ospf6_route_is_same(request, request->next)) {
		if(IS_OSPF6_DEBUG_ZEBRA(SEND)) {
	...
	nexthops = XCALLOC(MTYPE_OSPF6_OTHER, nhcount * sizeof(struct in6_addr *));
	if(nexthops == NULL) {
	...
	ifindexes = XCALLOC(MTYPE_OSPF6_OTHER, nhcount * sizeof(unsigned int));
	if(ifindexes == NULL) {
		zlog_warn("Can't send route to zebra: malloc failed");
	...
	for(i = 0; i < nhcount; i++) {
		if(IS_OSPF6_DEBUG_ZEBRA(SEND)) {
			const char *ifname;
			inet_ntop(AF_INET6, &request->nexthop[i].address, buf, sizeof(buf));
			ifname = ifindex2ifname(request->nexthop[i].ifindex);
			zlog_debug("  nexthop: %s%%%.*s(%d)", buf, IFNAMSIZ, ifname, request->nexthop[i].ifindex);
		}
	...
	if(ret < 0) {
		zlog_err("zapi_ipv6_route() %s failed: %s", (type == REM ? "delete" : "add"), safe_strerror(errno));
	}
...
DEFUN(redistribute_ospf6, redistribute_ospf6_cmd, "redistribute ospf6",
...
DEFUN(no_redistribute_ospf6, no_redistribute_ospf6_cmd, "no redistribute ospf6",
...
int ospf6_distance_set(struct vty *vty, struct ospf6 *o, const char *distance_str, const char *ip_str, const char *access_list_str) {
	int ret;
	struct prefix_ipv6 p;
	u_char distance;
...
int ospf6_distance_unset(struct vty *vty, struct ospf6 *o, const char *ip_str, const char *access_list_str) {
	int ret;
	struct prefix_ipv6 p;
	struct route_node *rn;
	struct ospf6_distance *odistance;
...
u_char ospf6_distance_apply(struct ospf6_route * or, struct ospf6 *o) {
	if(o == NULL) {
		return 0;
	}
	...
	if(o->distance_external) {
		if(or->path.type == OSPF6_PATH_TYPE_EXTERNAL1 || or->path.type == OSPF6_PATH_TYPE_EXTERNAL2) {
...
DEFUN(debug_ospf6_zebra_sendrecv, debug_ospf6_zebra_sendrecv_cmd, "debug ospf6 zebra (send|recv)",
...
ALIAS(debug_ospf6_zebra_sendrecv, debug_ospf6_zebra_cmd, "debug ospf6 zebra", DEBUG_STR OSPF6_STR "Debug connection between zebra\n")

DEFUN(no_debug_ospf6_zebra_sendrecv, no_debug_ospf6_zebra_sendrecv_cmd, "no debug ospf6 zebra (send|recv)",
...
ALIAS(no_debug_ospf6_zebra_sendrecv, no_debug_ospf6_zebra_cmd, "no debug ospf6 zebra", NO_STR DEBUG_STR OSPF6_STR "Debug connection between zebra\n")
