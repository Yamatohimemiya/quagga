...
#define IS_OSPF6_DEBUG_LSA(name) (ospf6_lstype_debug(htons(OSPF6_LSTYPE_##name)) & OSPF6_LSA_DEBUG)
#define IS_OSPF6_DEBUG_ORIGINATE(name) (ospf6_lstype_debug(htons(OSPF6_LSTYPE_##name)) & OSPF6_LSA_DEBUG_ORIGINATE)
#define IS_OSPF6_DEBUG_EXAMIN(name) (ospf6_lstype_debug(htons(OSPF6_LSTYPE_##name)) & OSPF6_LSA_DEBUG_EXAMIN)
#define IS_OSPF6_DEBUG_LSA_TYPE(type) (ospf6_lstype_debug(type) & OSPF6_LSA_DEBUG)
#define IS_OSPF6_DEBUG_ORIGINATE_TYPE(type) (ospf6_lstype_debug(type) & OSPF6_LSA_DEBUG_ORIGINATE)
#define IS_OSPF6_DEBUG_EXAMIN_TYPE(type) (ospf6_lstype_debug(type) & OSPF6_LSA_DEBUG_EXAMIN)
#define IS_OSPF6_DEBUG_FLOOD_TYPE(type) (ospf6_lstype_debug(type) & OSPF6_LSA_DEBUG_FLOOD)
...
#define OSPF6_LSA_END(h) ((caddr_t) (h) + ntohs(((struct ospf6_lsa_header *) (h))->length))
#define OSPF6_LSA_IS_TYPE(t, L) ((L)->header->type == htons(OSPF6_LSTYPE_##t) ? 1 : 0)
#define OSPF6_LSA_IS_SAME(L1, L2) ((L1)->header->adv_router == (L2)->header->adv_router && (L1)->header->id == (L2)->header->id && (L1)->header->type == (L2)->header->type)
#define OSPF6_LSA_IS_MATCH(t, i, a, L) ((L)->header->adv_router == (a) && (L)->header->id == (i) && (L)->header->type == (t))
...
#define OSPF6_LSA_IS_KNOWN(type) (ospf6_get_lsa_handler(type) != &unknown_handler ? 1 : 0)
...
#define CONTINUE_IF_ADDRESS_LINKLOCAL(debug, addr) \
	if(IN6_IS_ADDR_LINKLOCAL(&(addr)->u.prefix6)) { \
		...
		if(debug) zlog_debug("Filter out Linklocal: %s", buf); \
		continue; \
	}
...
#define CONTINUE_IF_ADDRESS_UNSPECIFIED(debug, addr) \
	if(IN6_IS_ADDR_UNSPECIFIED(&(addr)->u.prefix6)) { \
		char buf[64]; \
		prefix2str(addr, buf, sizeof(buf)); \
		if(debug) zlog_debug("Filter out Unspecified: %s", buf); \
		continue; \
	}
...
#define CONTINUE_IF_ADDRESS_LOOPBACK(debug, addr) \
	if(IN6_IS_ADDR_LOOPBACK(&(addr)->u.prefix6)) { \
		char buf[64]; \
		prefix2str(addr, buf, sizeof(buf)); \
		if(debug) zlog_debug("Filter out Loopback: %s", buf); \
		continue; \
	}
...
#define CONTINUE_IF_ADDRESS_V4COMPAT(debug, addr) \
	if(IN6_IS_ADDR_V4COMPAT(&(addr)->u.prefix6)) { \
		char buf[64]; \
		prefix2str(addr, buf, sizeof(buf)); \
		if(debug) zlog_debug("Filter out V4Compat: %s", buf); \
		continue; \
	}
...
#define CONTINUE_IF_ADDRESS_V4MAPPED(debug, addr) \
	if(IN6_IS_ADDR_V4MAPPED(&(addr)->u.prefix6)) { \
		char buf[64]; \
		prefix2str(addr, buf, sizeof(buf)); \
		if(debug) zlog_debug("Filter out V4Mapped: %s", buf); \
		continue; \
	}
...
extern int ospf6_lsa_prohibited_duration(u_int16_t type, u_int32_t id, u_int32_t adv_router, void *scope);
