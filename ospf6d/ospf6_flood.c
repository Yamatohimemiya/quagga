...
struct ospf6_lsdb *ospf6_get_scoped_lsdb(struct ospf6_lsa *lsa) {
	...
	switch(OSPF6_LSA_SCOPE(lsa->header->type)) {
		case OSPF6_SCOPE_LINKLOCAL: lsdb = OSPF6_INTERFACE(lsa->lsdb->data)->lsdb; break;
		case OSPF6_SCOPE_AREA: lsdb = OSPF6_AREA(lsa->lsdb->data)->lsdb; break;
		case OSPF6_SCOPE_AS: lsdb = OSPF6_PROCESS(lsa->lsdb->data)->lsdb; break;
...
struct ospf6_lsdb *ospf6_get_scoped_lsdb_self(struct ospf6_lsa *lsa) {
	...
	switch(OSPF6_LSA_SCOPE(lsa->header->type)) {
		case OSPF6_SCOPE_LINKLOCAL: lsdb_self = OSPF6_INTERFACE(lsa->lsdb->data)->lsdb_self; break;
		case OSPF6_SCOPE_AREA: lsdb_self = OSPF6_AREA(lsa->lsdb->data)->lsdb_self; break;
		case OSPF6_SCOPE_AS: lsdb_self = OSPF6_PROCESS(lsa->lsdb->data)->lsdb_self; break;
		default: assert(0); break;
	}
	return lsdb_self;
}
...
void ospf6_lsa_originate(struct ospf6_lsa *lsa) {
	...
	lsa->refresh = thread_add_timer(master, ospf6_lsa_refresh, lsa, OSPF_LS_REFRESH_TIME);
...
void ospf6_install_lsa(struct ospf6_lsa *lsa) {
	...
	if(!OSPF6_LSA_IS_MAXAGE(lsa)) {
		lsa->expire = thread_add_timer(master, ospf6_lsa_expire, lsa, OSPF_LSA_MAXAGE + lsa->birth.tv_sec - now.tv_sec);
	} else {
		lsa->expire = NULL;
	}

	if(OSPF6_LSA_IS_SEQWRAP(lsa) && !(CHECK_FLAG(lsa->flag, OSPF6_LSA_SEQWRAPPED) && lsa->header->seqnum == htonl(OSPF_MAX_SEQUENCE_NUMBER))) {
		if(IS_OSPF6_DEBUG_EXAMIN_TYPE(lsa->header->type)) {
			zlog_debug("lsa install wrapping: sequence 0x%x", ntohl(lsa->header->seqnum));
		}
...
void ospf6_receive_lsa(struct ospf6_neighbor *from, struct ospf6_lsa_header *lsa_header) {
	...
	switch(OSPF6_LSA_SCOPE(new->header->type)) {
		case OSPF6_SCOPE_LINKLOCAL: new->lsdb = from->ospf6_if->lsdb; break;
		case OSPF6_SCOPE_AREA: new->lsdb = from->ospf6_if->area->lsdb; break;
		case OSPF6_SCOPE_AS: new->lsdb = from->ospf6_if->area->ospf6->lsdb; break;
		default:
			if(is_debug) {
	...
	/* (8) previous database copy is more recent */
	{
		...
		if(OSPF6_LSA_IS_MAXAGE(old) && old->header->seqnum == htonl(OSPF_MAX_SEQUENCE_NUMBER)) {
			if(is_debug) {
