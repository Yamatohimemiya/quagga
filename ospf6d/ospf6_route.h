...
#define ospf6_nexthop_is_set(x) ((x)->ifindex || !IN6_IS_ADDR_UNSPECIFIED(&(x)->address))
#define ospf6_nexthop_is_same(a, b) ((a)->ifindex == (b)->ifindex && IN6_ARE_ADDR_EQUAL(&(a)->address, &(b)->address))
...
#define OSPF6_ROUTE_TABLE_CREATE(s, t) ospf6_route_table_create(OSPF6_SCOPE_TYPE_##s, OSPF6_TABLE_TYPE_##t)

extern const char *ospf6_dest_type_str[OSPF6_DEST_TYPE_MAX];
extern const char *ospf6_dest_type_substr[OSPF6_DEST_TYPE_MAX];
#define OSPF6_DEST_TYPE_NAME(x) (0 < (x) && (x) < OSPF6_DEST_TYPE_MAX ? ospf6_dest_type_str[(x)] : ospf6_dest_type_str[0])
#define OSPF6_DEST_TYPE_SUBSTR(x) (0 < (x) && (x) < OSPF6_DEST_TYPE_MAX ? ospf6_dest_type_substr[(x)] : ospf6_dest_type_substr[0])

extern const char *ospf6_path_type_str[OSPF6_PATH_TYPE_MAX];
extern const char *ospf6_path_type_substr[OSPF6_PATH_TYPE_MAX];
#define OSPF6_PATH_TYPE_NAME(x) (0 < (x) && (x) < OSPF6_PATH_TYPE_MAX ? ospf6_path_type_str[(x)] : ospf6_path_type_str[0])
#define OSPF6_PATH_TYPE_SUBSTR(x) (0 < (x) && (x) < OSPF6_PATH_TYPE_MAX ? ospf6_path_type_substr[(x)] : ospf6_path_type_substr[0])
...
#define ospf6_route_is_prefix(p, r) (memcmp(p, &(r)->prefix, sizeof(struct prefix)) == 0)
#define ospf6_route_is_same(ra, rb) (prefix_same(&(ra)->prefix, &(rb)->prefix))
#define ospf6_route_is_same_origin(ra, rb) ((ra)->path.area_id == (rb)->path.area_id && memcmp(&(ra)->path.origin, &(rb)->path.origin, sizeof(struct ospf6_ls_origin)) == 0)
#define ospf6_route_is_identical(ra, rb) \
	((ra)->type == (rb)->type && memcmp(&(ra)->prefix, &(rb)->prefix, sizeof(struct prefix)) == 0 && memcmp(&(ra)->path, &(rb)->path, sizeof(struct ospf6_path)) == 0 \
	 && memcmp(&(ra)->nexthop, &(rb)->nexthop, sizeof(struct ospf6_nexthop) * OSPF6_MULTI_PATH_LIMIT) == 0)
...
#define ID_IN_PREFIX(x) ((x)->u.lp.adv_router.s_addr)

/* Function prototype */
extern void ospf6_linkstate_prefix(u_int32_t adv_router, u_int32_t id, struct prefix *prefix);
extern void ospf6_linkstate_prefix2str(struct prefix *prefix, char *buf, int size);
...
extern struct ospf6_route *ospf6_route_lookup(struct prefix *prefix, struct ospf6_route_table *table);
extern struct ospf6_route *ospf6_route_lookup_identical(struct ospf6_route *route, struct ospf6_route_table *table);
extern struct ospf6_route *ospf6_route_lookup_bestmatch(struct prefix *prefix, struct ospf6_route_table *table);

extern struct ospf6_route *ospf6_route_add(struct ospf6_route *route, struct ospf6_route_table *table);
extern void ospf6_route_remove(struct ospf6_route *route, struct ospf6_route_table *table);
...
extern struct ospf6_route *ospf6_route_match_head(struct prefix *prefix, struct ospf6_route_table *table);
extern struct ospf6_route *ospf6_route_match_next(struct prefix *prefix, struct ospf6_route *route);
...
extern int ospf6_route_table_show(struct vty *, int, const char *[], struct ospf6_route_table *);
extern int ospf6_linkstate_table_show(struct vty *vty, int argc, const char *argv[], struct ospf6_route_table *table);
