...
const u_int16_t ospf6_packet_minlen[OSPF6_MESSAGE_TYPE_ALL] = { 0, OSPF6_HELLO_MIN_SIZE, OSPF6_DB_DESC_MIN_SIZE, OSPF6_LS_REQ_MIN_SIZE, OSPF6_LS_UPD_MIN_SIZE, OSPF6_LS_ACK_MIN_SIZE };
...
const u_int16_t ospf6_lsa_minlen[OSPF6_LSTYPE_SIZE] = { 0,
...
void ospf6_dbdesc_print(struct ospf6_header *oh) {
	...
	zlog_debug(
		"    MBZ: %#x Bits: %s%s%s SeqNum: %#lx", dbdesc->reserved2, (CHECK_FLAG(dbdesc->bits, OSPF6_DBDESC_IBIT) ? "I" : "-"), (CHECK_FLAG(dbdesc->bits, OSPF6_DBDESC_MBIT) ? "M" : "-"),
		(CHECK_FLAG(dbdesc->bits, OSPF6_DBDESC_MSBIT) ? "m" : "s"), (u_long) ntohl(dbdesc->seqnum)
	);
...
static void ospf6_dbdesc_recv_master(struct ospf6_header *oh, struct ospf6_neighbor *on) {
	...
	for(p = (char *) ((caddr_t) dbdesc + sizeof(struct ospf6_dbdesc)); p + sizeof(struct ospf6_lsa_header) <= OSPF6_MESSAGE_END(oh); p += sizeof(struct ospf6_lsa_header)) {
		...
		switch(OSPF6_LSA_SCOPE(his->header->type)) {
			case OSPF6_SCOPE_LINKLOCAL: lsdb = on->ospf6_if->lsdb; break;
			case OSPF6_SCOPE_AREA: lsdb = on->ospf6_if->area->lsdb; break;
			case OSPF6_SCOPE_AS: lsdb = on->ospf6_if->area->ospf6->lsdb; break;
...
static void ospf6_dbdesc_recv_slave(struct ospf6_header *oh, struct ospf6_neighbor *on) {
	...
	switch(on->state) {
		...
		case OSPF6_NEIGHBOR_EXSTART:
         ...
         and process LSA Headers. Otherwise, ignore this message */
			if(CHECK_FLAG(dbdesc->bits, OSPF6_DBDESC_IBIT) && CHECK_FLAG(dbdesc->bits, OSPF6_DBDESC_MBIT) && CHECK_FLAG(dbdesc->bits, OSPF6_DBDESC_MSBIT)
			   && ntohs(oh->length) == sizeof(struct ospf6_header) + sizeof(struct ospf6_dbdesc)) {
	...
	/* schedule send lsreq */
	if((on->thread_send_lsreq == NULL) && (on->request_list->count)) {
		on->thread_send_lsreq = thread_add_event(master, ospf6_lsreq_send, on, 0);
	}
...
static unsigned ospf6_lsa_examin(struct ospf6_lsa_header *lsah, const u_int16_t lsalen, const u_char headeronly) {
	...
	switch(lsatype) {
       ...
       followed by 3-4 fields of a single IPv6 prefix. */
			...
			return ospf6_prefixes_examin((struct ospf6_prefix *) ((caddr_t) lsah + OSPF6_LSA_HEADER_SIZE + OSPF6_INTER_PREFIX_LSA_MIN_SIZE), lsalen - OSPF6_LSA_HEADER_SIZE - OSPF6_INTER_PREFIX_LSA_MIN_SIZE, 1);
       ...
       by N>=0 IPv6 prefix blocks (with N declared beforehand). */
			...
			return ospf6_prefixes_examin(
				(struct ospf6_prefix *) ((caddr_t) link_lsa + OSPF6_LINK_LSA_MIN_SIZE), lsalen - OSPF6_LSA_HEADER_SIZE - OSPF6_LINK_LSA_MIN_SIZE, ntohl(link_lsa->prefix_num) /* 32 bits */
			);
     ...
     followed by N>=0 IPv6 prefixes (with N declared beforehand). */
			...
			return ospf6_prefixes_examin(
				(struct ospf6_prefix *) ((caddr_t) intra_prefix_lsa + OSPF6_INTRA_PREFIX_LSA_MIN_SIZE), lsalen - OSPF6_LSA_HEADER_SIZE - OSPF6_INTRA_PREFIX_LSA_MIN_SIZE, ntohs(intra_prefix_lsa->prefix_num) /* 16 bits */
			);
	}
	...
	switch(oh->type) {
       ...
       by N>=0 full LSAs (with N declared beforehand). */
			...
			test = ospf6_lsaseq_examin(
				(struct ospf6_lsa_header *) ((caddr_t) lsupd + OSPF6_LS_UPD_MIN_SIZE), bytesonwire - OSPF6_HEADER_SIZE - OSPF6_LS_UPD_MIN_SIZE, 0, ntohl(lsupd->lsa_number) /* 32 bits */
			);
			break;
	...
	if(oh->area_id != oi->area->area_id) {
		if(IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV)) {
			if(oh->area_id == OSPF_AREA_BACKBONE) {
				...
				zlog_debug("%s: Area-ID mismatch (my %s, rcvd %s)", __func__, inet_ntop(AF_INET, &oi->area->area_id, buf[0], INET_ADDRSTRLEN), inet_ntop(AF_INET, &oh->area_id, buf[1], INET_ADDRSTRLEN));
			}
		}
		return MSG_NG;
	}
...
int ospf6_receive(struct thread *thread) {
	...
	if(IS_OSPF6_DEBUG_MESSAGE(oh->type, RECV)) {
		...
		switch(oh->type) {
			case OSPF6_MESSAGE_TYPE_HELLO: ospf6_hello_print(oh); break;
			case OSPF6_MESSAGE_TYPE_DBDESC: ospf6_dbdesc_print(oh); break;
			case OSPF6_MESSAGE_TYPE_LSREQ: ospf6_lsreq_print(oh); break;
			case OSPF6_MESSAGE_TYPE_LSUPDATE: ospf6_lsupdate_print(oh); break;
			case OSPF6_MESSAGE_TYPE_LSACK: ospf6_lsack_print(oh); break;
			default: assert(0);
		}
	}

	switch(oh->type) {
		case OSPF6_MESSAGE_TYPE_HELLO: ospf6_hello_recv(&src, &dst, oi, oh); break;

		case OSPF6_MESSAGE_TYPE_DBDESC: ospf6_dbdesc_recv(&src, &dst, oi, oh); break;

		case OSPF6_MESSAGE_TYPE_LSREQ: ospf6_lsreq_recv(&src, &dst, oi, oh); break;

		case OSPF6_MESSAGE_TYPE_LSUPDATE: ospf6_lsupdate_recv(&src, &dst, oi, oh); break;

		case OSPF6_MESSAGE_TYPE_LSACK: ospf6_lsack_recv(&src, &dst, oi, oh); break;
...
int ospf6_hello_send(struct thread *thread) {
	...
	if(iobuflen == 0) {
		zlog_debug("Unable to send Hello on interface %s iobuflen is 0", oi->interface->name);
		return 0;
	}
...
int ospf6_dbdesc_send(struct thread *thread) {
	...
	if(CHECK_FLAG(on->dbdesc_bits, OSPF6_DBDESC_IBIT) && (on->dbdesc_seqnum == 0)) {
		struct timeval tv;
...
int ospf6_lsupdate_send_interface(struct thread *thread) {
	...
	if(lsa_cnt) {
		...
		if((oi->state == OSPF6_INTERFACE_POINTTOPOINT) || (oi->state == OSPF6_INTERFACE_DR) || (oi->state == OSPF6_INTERFACE_BDR)) {
			ospf6_send(oi->linklocal_addr, &allspfrouters6, oi, oh);
		} else {
...
/* Commands */
DEFUN(debug_ospf6_message, debug_ospf6_message_cmd, "debug ospf6 message (unknown|hello|dbdesc|lsreq|lsupdate|lsack|all)",
      DEBUG_STR OSPF6_STR "Debug OSPFv3 message\n"
...
ALIAS(debug_ospf6_message, debug_ospf6_message_sendrecv_cmd, "debug ospf6 message (unknown|hello|dbdesc|lsreq|lsupdate|lsack|all) (send|recv)",
      DEBUG_STR OSPF6_STR "Debug OSPFv3 message\n"
			  "Debug Unknown message\n"
			  "Debug Hello message\n"
			  "Debug Database Description message\n"
...
DEFUN(no_debug_ospf6_message, no_debug_ospf6_message_cmd, "no debug ospf6 message (unknown|hello|dbdesc|lsreq|lsupdate|lsack|all)",
      NO_STR DEBUG_STR OSPF6_STR "Debug OSPFv3 message\n"
				 "Debug Unknown message\n"
				 "Debug Hello message\n"
				 "Debug Database Description message\n"
				 "Debug Link State Request message\n"
...
ALIAS(no_debug_ospf6_message, no_debug_ospf6_message_sendrecv_cmd,
...
int config_write_ospf6_debug_message(struct vty *vty) {
	const char *type_str[] = { "unknown", "hello", "dbdesc", "lsreq", "lsupdate", "lsack" };
