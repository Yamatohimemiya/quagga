...
#define IS_OSPF6_DEBUG_SPF(level) (conf_debug_ospf6_spf & OSPF6_DEBUG_SPF_##level)
...
static inline unsigned int ospf6_lsadd_to_spf_reason(struct ospf6_lsa *lsa) {
	...
	switch(ntohs(lsa->header->type)) {
		case OSPF6_LSTYPE_ROUTER: reason = OSPF6_SPF_FLAGS_ROUTER_LSA_ADDED; break;
		case OSPF6_LSTYPE_NETWORK: reason = OSPF6_SPF_FLAGS_NETWORK_LSA_ADDED; break;
		case OSPF6_LSTYPE_LINK: reason = OSPF6_SPF_FLAGS_LINK_LSA_ADDED; break;
		default: break;
	}
	return (reason);
}
...
static inline unsigned int ospf6_lsremove_to_spf_reason(struct ospf6_lsa *lsa) {
	...
	switch(ntohs(lsa->header->type)) {
		case OSPF6_LSTYPE_ROUTER: reason = OSPF6_SPF_FLAGS_ROUTER_LSA_REMOVED; break;
		case OSPF6_LSTYPE_NETWORK: reason = OSPF6_SPF_FLAGS_NETWORK_LSA_REMOVED; break;
		case OSPF6_LSTYPE_LINK: reason = OSPF6_SPF_FLAGS_LINK_LSA_REMOVED; break;
		default: break;
	}
	return (reason);
}
...
extern void ospf6_spf_calculation(u_int32_t router_id, struct ospf6_route_table *result_table, struct ospf6_area *oa);
extern void ospf6_spf_schedule(struct ospf6 *ospf, unsigned int reason);

extern void ospf6_spf_display_subtree(struct vty *vty, const char *prefix, int rest, struct ospf6_vertex *v);
