...
static void _lsdb_count_assert(struct ospf6_lsdb *lsdb) {
	...
	unsigned int num = 0;
	for(debug = ospf6_lsdb_head(lsdb); debug; debug = ospf6_lsdb_next(debug)) {
		num++;
	}
	...
	zlog_debug("PANIC !! lsdb[%p]->count = %d, real = %d", lsdb, lsdb->count, num);
	for(debug = ospf6_lsdb_head(lsdb); debug; debug = ospf6_lsdb_next(debug)) {
		zlog_debug("%p %p %s lsdb[%p]", debug->prev, debug->next, debug->name, debug->lsdb);
	}
	zlog_debug("DUMP END");
...
void ospf6_lsdb_add(struct ospf6_lsa *lsa, struct ospf6_lsdb *lsdb) {
	...
	ospf6_lsdb_set_key(&key, &lsa->header->adv_router, sizeof(lsa->header->adv_router));
...
struct ospf6_lsa *ospf6_lsdb_lookup_next(u_int16_t type, u_int32_t id, u_int32_t adv_router, struct ospf6_lsdb *lsdb) {
	struct route_node *node;
...
struct ospf6_lsa *ospf6_lsdb_type_router_head(u_int16_t type, u_int32_t adv_router, struct ospf6_lsdb *lsdb) {
	struct route_node *node;
	struct prefix_ipv6 key;
	...
	/* Walk down tree. */
	while(node && node->p.prefixlen <= key.prefixlen && prefix_match(&node->p, (struct prefix *) &key)) {
...
struct ospf6_lsa *ospf6_lsdb_type_router_next(u_int16_t type, u_int32_t adv_router, struct ospf6_lsa *lsa) {
	struct ospf6_lsa *next = ospf6_lsdb_next(lsa);

	if(next) {
		if(next->header->type != type || next->header->adv_router != adv_router) {
...
void ospf6_lsdb_show(struct vty *vty, enum ospf_lsdb_show_level level, u_int16_t *type, u_int32_t *id, u_int32_t *adv_router, struct ospf6_lsdb *lsdb) {
	struct ospf6_lsa *lsa;
	void (*showfunc)(struct vty *, struct ospf6_lsa *) = NULL;

	switch(level) {
		case OSPF6_LSDB_SHOW_LEVEL_DETAIL: showfunc = ospf6_lsa_show; break;
		case OSPF6_LSDB_SHOW_LEVEL_INTERNAL: showfunc = ospf6_lsa_show_internal; break;
		case OSPF6_LSDB_SHOW_LEVEL_DUMP: showfunc = ospf6_lsa_show_dump; break;
		case OSPF6_LSDB_SHOW_LEVEL_NORMAL:
		default: showfunc = ospf6_lsa_show_summary;
	}
	...
	while(lsa) {
		if((!adv_router || lsa->header->adv_router == *adv_router) && (!id || lsa->header->id == *id)) {
			(*showfunc)(vty, lsa);
		}
...
u_int32_t ospf6_new_ls_id(u_int16_t type, u_int32_t adv_router, struct ospf6_lsdb *lsdb) {
	struct ospf6_lsa *lsa;
	u_int32_t id = 1;

	for(lsa = ospf6_lsdb_type_router_head(type, adv_router, lsdb); lsa; lsa = ospf6_lsdb_type_router_next(type, adv_router, lsa)) {
...
/* Decide new LS sequence number to originate.
   note return value is network byte order */
u_int32_t ospf6_new_ls_seqnum(u_int16_t type, u_int32_t id, u_int32_t adv_router, struct ospf6_lsdb *lsdb) {
	struct ospf6_lsa *lsa;
