...
struct ospf6_lsa_handler unknown_handler = {
	OSPF6_LSTYPE_UNKNOWN, "Unknown", "Unk", ospf6_unknown_lsa_show, NULL, OSPF6_LSA_DEBUG,
};
...
int ospf6_lsa_is_differ(struct ospf6_lsa *lsa1, struct ospf6_lsa *lsa2) {
	...
	if(ntohs(lsa1->header->age) == OSPF_LSA_MAXAGE && ntohs(lsa2->header->age) != OSPF_LSA_MAXAGE) {
		return 1;
	}
	if(ntohs(lsa1->header->age) != OSPF_LSA_MAXAGE && ntohs(lsa2->header->age) == OSPF_LSA_MAXAGE) {
		return 1;
...
int ospf6_lsa_is_changed(struct ospf6_lsa *lsa1, struct ospf6_lsa *lsa2) {
	...
	return memcmp(OSPF6_LSA_HEADER_END(lsa1->header), OSPF6_LSA_HEADER_END(lsa2->header), length);
}
...
static void ospf6_lsa_age_set(struct ospf6_lsa *lsa) {
	...
	if(quagga_gettime(QUAGGA_CLK_MONOTONIC, &now) < 0) {
		zlog_warn("LSA: quagga_gettime failed, may fail LSA AGEs: %s", safe_strerror(errno));
	}
...
char *ospf6_lsa_printbuf(struct ospf6_lsa *lsa, char *buf, int size) {
	...
	snprintf(buf, size, "[%s Id:%s Adv:%s]", ospf6_lstype_name(lsa->header->type), id, adv_router);
	return buf;
}
...
void ospf6_lsa_header_print_raw(struct ospf6_lsa_header *header) {
	...
	zlog_debug("    [%s Id:%s Adv:%s]", ospf6_lstype_name(header->type), id, adv_router);
	zlog_debug("    Age: %4hu SeqNum: %#08lx Cksum: %04hx Len: %d", ntohs(header->age), (u_long) ntohl(header->seqnum), ntohs(header->checksum), ntohs(header->length));
}
...
void ospf6_lsa_show_summary_header(struct vty *vty) {
	vty_out(vty, "%-4s %-15s%-15s%4s %8s %30s%s", "Type", "LSId", "AdvRouter", "Age", "SeqNum", "Payload", VNL);
}
...
void ospf6_lsa_show_summary(struct vty *vty, struct ospf6_lsa *lsa) {
	...
	if((type == OSPF6_LSTYPE_INTER_PREFIX) || (type == OSPF6_LSTYPE_INTER_ROUTER) || (type == OSPF6_LSTYPE_AS_EXTERNAL)) {
		vty_out(vty, "%-4s %-15s%-15s%4hu %8lx %30s%s", ospf6_lstype_short_name(lsa->header->type), id, adv_router, ospf6_lsa_age_current(lsa), (u_long) ntohl(lsa->header->seqnum), handler->get_prefix_str(lsa, buf, sizeof(buf), 0),
			VNL);
	} else if(type != OSPF6_LSTYPE_UNKNOWN) {
		sprintf(tmpbuf, "%-4s %-15s%-15s%4hu %8lx", ospf6_lstype_short_name(lsa->header->type), id, adv_router, ospf6_lsa_age_current(lsa), (u_long) ntohl(lsa->header->seqnum));
		...
		vty_out(vty, "%-4s %-15s%-15s%4hu %8lx%s", ospf6_lstype_short_name(lsa->header->type), id, adv_router, ospf6_lsa_age_current(lsa), (u_long) ntohl(lsa->header->seqnum), VNL);
	}
}
...
void ospf6_lsa_show_internal(struct vty *vty, struct ospf6_lsa *lsa) {
	...
	vty_out(vty, "%s", VNL);
	vty_out(vty, "Age: %4hu Type: %s%s", ospf6_lsa_age_current(lsa), ospf6_lstype_name(lsa->header->type), VNL);
	vty_out(vty, "Link State ID: %s%s", id, VNL);
	vty_out(vty, "Advertising Router: %s%s", adv_router, VNL);
	vty_out(vty, "LS Sequence Number: %#010lx%s", (u_long) ntohl(lsa->header->seqnum), VNL);
	vty_out(vty, "CheckSum: %#06hx Length: %hu%s", ntohs(lsa->header->checksum), ntohs(lsa->header->length), VNL);
	vty_out(vty, "Flag: %x %s", lsa->flag, VNL);
	vty_out(vty, "Lock: %d %s", lsa->lock, VNL);
	vty_out(vty, "ReTx Count: %d%s", lsa->retrans_count, VNL);
	vty_out(vty, "Threads: Expire: 0x%p, Refresh: 0x%p %s", (void *) lsa->expire, (void *) lsa->refresh, VNL);
	vty_out(vty, "%s", VNL);
	return;
}
...
struct ospf6_lsa *ospf6_lsa_create(struct ospf6_lsa_header *header) {
	...
	new_header = (struct ospf6_lsa_header *) XMALLOC(MTYPE_OSPF6_LSA, lsa_size);
	...
	/* allocate memory */
	lsa = (struct ospf6_lsa *) XCALLOC(MTYPE_OSPF6_LSA, sizeof(struct ospf6_lsa));
...
struct ospf6_lsa *ospf6_lsa_create_headeronly(struct ospf6_lsa_header *header) {
	...
	/* allocate memory for this LSA */
	new_header = (struct ospf6_lsa_header *) XMALLOC(MTYPE_OSPF6_LSA, sizeof(struct ospf6_lsa_header));
...
int ospf6_lsa_refresh(struct thread *thread) {
	...
	self = ospf6_lsdb_lookup(old->header->type, old->header->id, old->header->adv_router, lsdb_self);
	if(self == NULL) {
	...
	self->header->seqnum = ospf6_new_ls_seqnum(self->header->type, self->header->id, self->header->adv_router, old->lsdb);
	ospf6_lsa_checksum(self->header);

	new = ospf6_lsa_create(self->header);
	new->lsdb = old->lsdb;
	new->refresh = thread_add_timer(master, ospf6_lsa_refresh, new, OSPF_LS_REFRESH_TIME);
...
DEFUN(debug_ospf6_lsa_type, debug_ospf6_lsa_hex_cmd, "debug ospf6 lsa (router|network|inter-prefix|inter-router|as-ext|grp-mbr|type7|link|intra-prefix|unknown)",
...
ALIAS(debug_ospf6_lsa_type, debug_ospf6_lsa_hex_detail_cmd, "debug ospf6 lsa (router|network|inter-prefix|inter-router|as-ext|grp-mbr|type7|link|intra-prefix|unknown) (originate|examine|flooding)",
      DEBUG_STR OSPF6_STR "Debug Link State Advertisements (LSAs)\n"
			  "Specify LS type as Hexadecimal\n")

DEFUN(no_debug_ospf6_lsa_type, no_debug_ospf6_lsa_hex_cmd, "no debug ospf6 lsa (router|network|inter-prefix|inter-router|as-ext|grp-mbr|type7|link|intra-prefix|unknown)",
      NO_STR DEBUG_STR OSPF6_STR "Debug Link State Advertisements (LSAs)\n"
				 "Specify LS type as Hexadecimal\n") {
	u_int i;
	struct ospf6_lsa_handler *handler = NULL;
...
ALIAS(no_debug_ospf6_lsa_type, no_debug_ospf6_lsa_hex_detail_cmd, "no debug ospf6 lsa (router|network|inter-prefix|inter-router|as-ext|grp-mbr|type7|link|intra-prefix) (originate|examine|flooding)",
      NO_STR DEBUG_STR OSPF6_STR "Debug Link State Advertisements (LSAs)\n"
				 "Specify LS type as Hexadecimal\n")
...
int config_write_ospf6_debug_lsa(struct vty *vty) {
	...
	for(i = 0; i < vector_active(ospf6_lsa_handler_vector); i++) {
		...
		if(CHECK_FLAG(handler->debug, OSPF6_LSA_DEBUG)) {
			vty_out(vty, "debug ospf6 lsa %s%s", ospf6_lsa_handler_name(handler), VNL);
		}
		if(CHECK_FLAG(handler->debug, OSPF6_LSA_DEBUG_ORIGINATE)) {
			vty_out(vty, "debug ospf6 lsa %s originate%s", ospf6_lsa_handler_name(handler), VNL);
		}
		if(CHECK_FLAG(handler->debug, OSPF6_LSA_DEBUG_EXAMIN)) {
			vty_out(vty, "debug ospf6 lsa %s examine%s", ospf6_lsa_handler_name(handler), VNL);
		}
		if(CHECK_FLAG(handler->debug, OSPF6_LSA_DEBUG_FLOOD)) {
			vty_out(vty, "debug ospf6 lsa %s flooding%s", ospf6_lsa_handler_name(handler), VNL);
		}
	}
