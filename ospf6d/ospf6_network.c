...
static void ospf6_reset_mcastloop(void) {
	u_int off = 0;
	if(setsockopt(ospf6_sock, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, &off, sizeof(u_int)) < 0) {
		zlog_warn("Network: reset IPV6_MULTICAST_LOOP failed: %s", safe_strerror(errno));
	}
}
...
int ospf6_sso(ifindex_t ifindex, struct in6_addr *group, int option) {
	...
	ret = setsockopt(ospf6_sock, IPPROTO_IPV6, option, &mreq6, sizeof(mreq6));
	if(ret < 0) {
		zlog_err("Network: setsockopt (%d) on ifindex %d failed: %s", option, ifindex, safe_strerror(errno));
	}
...
int ospf6_sendmsg(struct in6_addr *src, struct in6_addr *dst, ifindex_t *ifindex, struct iovec *message) {
	int retval;
	...
	if(retval != iov_totallen(message)) {
		zlog_warn("sendmsg failed: ifindex: %d: %s (%d)", *ifindex, safe_strerror(errno), errno);
	}

	return retval;
}

int ospf6_recvmsg(struct in6_addr *src, struct in6_addr *dst, ifindex_t *ifindex, struct iovec *message) {
	int retval;
