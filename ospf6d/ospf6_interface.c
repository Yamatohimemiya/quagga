...
const char *ospf6_interface_state_str[] = { "None", "Down", "Loopback", "Waiting", "PointToPoint", "DROther", "BDR", "DR", NULL };
...
struct ospf6_interface *ospf6_interface_create(struct interface *ifp) {
	...
	oi = (struct ospf6_interface *) XCALLOC(MTYPE_OSPF6_IF, sizeof(struct ospf6_interface));
	...
	if(oi->ifmtu > iobuflen) {
		if(IS_OSPF6_DEBUG_INTERFACE) {
			zlog_debug("Interface %s: IfMtu is adjusted to I/O buffer size: %d.", ifp->name, iobuflen);
		}
		oi->ifmtu = iobuflen;
	}
...
static struct in6_addr *ospf6_interface_get_linklocal_address(struct interface *ifp) {
	struct listnode *n;
	struct connected *c;
...
void ospf6_interface_state_update(struct interface *ifp) {
	...
	if(if_is_operative(ifp) && (ospf6_interface_get_linklocal_address(oi->interface) || if_is_loopback(oi->interface))) {
...
void ospf6_interface_connected_route_update(struct interface *ifp) {
	...
	for(ALL_LIST_ELEMENTS(oi->interface->connected, node, nnode, c)) {
		...
		if(oi->plist_name) {
			...
			if(ret == PREFIX_DENY) {
				if(IS_OSPF6_DEBUG_INTERFACE) {
					zlog_debug("%s on %s filtered by prefix-list %s ", buf, oi->interface->name, oi->plist_name);
				}
				continue;
			}
		}
...
static void ospf6_interface_state_change(u_char next_state, struct ospf6_interface *oi) {
	...
	if((prev_state == OSPF6_INTERFACE_DR || prev_state == OSPF6_INTERFACE_BDR) && (next_state != OSPF6_INTERFACE_DR && next_state != OSPF6_INTERFACE_BDR)) {
	...
	if(next_state == OSPF6_INTERFACE_DOWN) {
		...
		OSPF6_INTRA_PREFIX_LSA_SCHEDULE_STUB(oi->area);
	} else if(prev_state == OSPF6_INTERFACE_DR || next_state == OSPF6_INTERFACE_DR) {
		OSPF6_NETWORK_LSA_SCHEDULE(oi);
		OSPF6_INTRA_PREFIX_LSA_SCHEDULE_TRANSIT(oi);
		OSPF6_INTRA_PREFIX_LSA_SCHEDULE_STUB(oi->area);
	}
	...
	if((next_state == OSPF6_INTERFACE_POINTTOPOINT) || (next_state == OSPF6_INTERFACE_DROTHER) || (next_state == OSPF6_INTERFACE_BDR) || (next_state == OSPF6_INTERFACE_DR) || (next_state < prev_state)) {
...
#define IS_ELIGIBLE(n) ((n)->state >= OSPF6_NEIGHBOR_TWOWAY && (n)->priority != 0)

static struct ospf6_neighbor *better_bdrouter(struct ospf6_neighbor *a, struct ospf6_neighbor *b) {
	if((a == NULL || !IS_ELIGIBLE(a) || a->drouter == a->router_id) && (b == NULL || !IS_ELIGIBLE(b) || b->drouter == b->router_id)) {
		return NULL;
...
static u_char dr_election(struct ospf6_interface *oi) {
	...
	inet_ntop(AF_INET, &oi->area->ospf6->router_id, myself.name, sizeof(myself.name));
	...
	if((drouter == &myself && myself.drouter != myself.router_id) || (drouter != &myself && myself.drouter == myself.router_id) || (bdrouter == &myself && myself.bdrouter != myself.router_id)
	...
	if(oi->drouter != (drouter ? drouter->router_id : htonl(0)) || oi->bdrouter != (bdrouter ? bdrouter->router_id : htonl(0))) {
		if(IS_OSPF6_DEBUG_INTERFACE) {
			zlog_debug("DR Election on %s: DR: %s BDR: %s", oi->interface->name, (drouter ? drouter->name : "0.0.0.0"), (bdrouter ? bdrouter->name : "0.0.0.0"));
		}
...
int interface_up(struct thread *thread) {
	...
	if(!if_is_operative(oi->interface)) {
		if(IS_OSPF6_DEBUG_INTERFACE) {
			zlog_debug("Interface %s is down, can't execute [InterfaceUp]", oi->interface->name);
		}
		return 0;
	}

	/* check interface has a link-local address */
	if(!(ospf6_interface_get_linklocal_address(oi->interface) || if_is_loopback(oi->interface))) {
		if(IS_OSPF6_DEBUG_INTERFACE) {
	...
	if(oi->area == NULL) {
		zlog_debug("%s: Not scheduleing Hello for %s as there is no area assigned yet", __func__, oi->interface->name);
		return 0;
	}
	...
	if(ospf6_sso(oi->interface->ifindex, &allspfrouters6, IPV6_JOIN_GROUP) < 0) {
		if(oi->sso_try_cnt++ < OSPF6_INTERFACE_SSO_RETRY_MAX) {
			zlog_info("Scheduling %s for sso retry, trial count: %d", oi->interface->name, oi->sso_try_cnt);
			thread_add_timer(master, interface_up, oi, OSPF6_INTERFACE_SSO_RETRY_INT);
		}
		return 0;
	}
	...
	if((if_is_pointopoint(oi->interface)) || (oi->type == OSPF_IFTYPE_POINTOPOINT)) {
...
static int ospf6_interface_show(struct vty *vty, struct interface *ifp) {
	...
	vty_out(vty, "%s is %s, type %s%s", ifp->name, updown[if_is_operative(ifp)], type, VNL);
	...
	for(ALL_LIST_ELEMENTS_RO(ifp->connected, i, c)) {
		...
		switch(p->family) {
			case AF_INET: vty_out(vty, "    inet : %s%s", strbuf, VNL); break;
			case AF_INET6: vty_out(vty, "    inet6: %s%s", strbuf, VNL); break;
			default: vty_out(vty, "    ???  : %s%s", strbuf, VNL); break;
		}
	}

	if(oi->area) {
		vty_out(vty, "  Instance ID %d, Interface MTU %d (autodetect: %d)%s", oi->instance_id, oi->ifmtu, ifp->mtu6, VNL);
		vty_out(vty, "  MTU mismatch detection: %s%s", oi->mtu_ignore ? "disabled" : "enabled", VNL);
		inet_ntop(AF_INET, &oi->area->area_id, strbuf, sizeof(strbuf));
		vty_out(vty, "  Area ID %s, Cost %u%s", strbuf, oi->cost, VNL);
	} else {
		vty_out(vty, "  Not Attached to Area%s", VNL);
	}

	vty_out(vty, "  State %s, Transmit Delay %d sec, Priority %d%s", ospf6_interface_state_str[oi->state], oi->transdelay, oi->priority, VNL);
	vty_out(vty, "  Timer intervals configured:%s", VNL);
	vty_out(vty, "   Hello %d, Dead %d, Retransmit %d%s", oi->hello_interval, oi->dead_interval, oi->rxmt_interval, VNL);
	...
	vty_out(vty, "  Number of I/F scoped LSAs is %u%s", oi->lsdb->count, VNL);
	...
	vty_out(vty, "    %d Pending LSAs for LSUpdate in Time %s [thread %s]%s", oi->lsupdate_list->count, duration, (oi->thread_send_lsupdate ? "on" : "off"), VNL);
	...
	timerstring(&res, duration, sizeof(duration));
	vty_out(vty, "    %d Pending LSAs for LSAck in Time %s [thread %s]%s", oi->lsack_list->count, duration, (oi->thread_send_lsack ? "on" : "off"), VNL);
...
/* show interface */
DEFUN(show_ipv6_ospf6_interface, show_ipv6_ospf6_interface_ifname_cmd, "show ipv6 ospf6 interface IFNAME", SHOW_STR IP6_STR OSPF6_STR INTERFACE_STR IFNAME_STR) {
	struct interface *ifp;
	struct listnode *i;
...
ALIAS(show_ipv6_ospf6_interface, show_ipv6_ospf6_interface_cmd, "show ipv6 ospf6 interface", SHOW_STR IP6_STR OSPF6_STR INTERFACE_STR)

DEFUN(show_ipv6_ospf6_interface_ifname_prefix, show_ipv6_ospf6_interface_ifname_prefix_cmd, "show ipv6 ospf6 interface IFNAME prefix", SHOW_STR IP6_STR OSPF6_STR INTERFACE_STR IFNAME_STR "Display connected prefixes to advertise\n") {
	struct interface *ifp;
	struct ospf6_interface *oi;
...
ALIAS(show_ipv6_ospf6_interface_ifname_prefix, show_ipv6_ospf6_interface_ifname_prefix_detail_cmd, "show ipv6 ospf6 interface IFNAME prefix (X:X::X:X|X:X::X:X/M|detail)",
      SHOW_STR IP6_STR OSPF6_STR INTERFACE_STR IFNAME_STR "Display connected prefixes to advertise\n" OSPF6_ROUTE_ADDRESS_STR OSPF6_ROUTE_PREFIX_STR "Display details of the prefixes\n")

ALIAS(show_ipv6_ospf6_interface_ifname_prefix, show_ipv6_ospf6_interface_ifname_prefix_match_cmd, "show ipv6 ospf6 interface IFNAME prefix X:X::X:X/M (match|detail)",
...
ALIAS(show_ipv6_ospf6_interface_prefix, show_ipv6_ospf6_interface_prefix_detail_cmd, "show ipv6 ospf6 interface prefix (X:X::X:X|X:X::X:X/M|detail)",
      SHOW_STR IP6_STR OSPF6_STR INTERFACE_STR "Display connected prefixes to advertise\n" OSPF6_ROUTE_ADDRESS_STR OSPF6_ROUTE_PREFIX_STR "Display details of the prefixes\n")

ALIAS(show_ipv6_ospf6_interface_prefix, show_ipv6_ospf6_interface_prefix_match_cmd, "show ipv6 ospf6 interface prefix X:X::X:X/M (match|detail)",
      SHOW_STR IP6_STR OSPF6_STR INTERFACE_STR "Display connected prefixes to advertise\n" OSPF6_ROUTE_PREFIX_STR OSPF6_ROUTE_MATCH_STR "Display details of the prefixes\n")
...
DEFUN(ipv6_ospf6_ifmtu, ipv6_ospf6_ifmtu_cmd, "ipv6 ospf6 ifmtu <1-65535>",
	...
	if(ifp->mtu6 != 0 && ifp->mtu6 < ifmtu) {
		vty_out(vty, "%s's ospf6 ifmtu cannot go beyond physical mtu (%d)%s", ifp->name, ifp->mtu6, VNL);
		return CMD_WARNING;
	}
	...
	if(oi->ifmtu < ifmtu) {
		...
		if(iobuflen < ifmtu) {
			vty_out(vty, "%s's ifmtu is adjusted to I/O buffer size (%d).%s", ifp->name, iobuflen, VNL);
			oi->ifmtu = iobuflen;
		} else {
			oi->ifmtu = ifmtu;
		}
...
DEFUN(no_ipv6_ospf6_ifmtu, no_ipv6_ospf6_ifmtu_cmd, "no ipv6 ospf6 ifmtu", NO_STR IP6_STR OSPF6_STR "Interface MTU\n") {
	struct ospf6_interface *oi;
	struct interface *ifp;
	unsigned int iobuflen;
	struct listnode *node, *nnode;
...
DEFUN(no_ipv6_ospf6_cost, no_ipv6_ospf6_cost_cmd, "no ipv6 ospf6 cost", NO_STR IP6_STR OSPF6_STR "Calculate interface cost from bandwidth\n") {
	struct ospf6_interface *oi;
	struct interface *ifp;
...
DEFUN(auto_cost_reference_bandwidth, auto_cost_reference_bandwidth_cmd, "auto-cost reference-bandwidth <1-4294967>",
...
DEFUN(no_auto_cost_reference_bandwidth, no_auto_cost_reference_bandwidth_cmd, "no auto-cost reference-bandwidth",
...
DEFUN(ipv6_ospf6_hellointerval, ipv6_ospf6_hellointerval_cmd, "ipv6 ospf6 hello-interval <1-65535>", IP6_STR OSPF6_STR "Interval time of Hello packets\n" SECONDS_STR) {
	struct ospf6_interface *oi;
	struct interface *ifp;
...
/* interface variable set command */
DEFUN(ipv6_ospf6_deadinterval, ipv6_ospf6_deadinterval_cmd, "ipv6 ospf6 dead-interval <1-65535>", IP6_STR OSPF6_STR "Interval time after which a neighbor is declared down\n" SECONDS_STR) {
	struct ospf6_interface *oi;
	struct interface *ifp;
...
/* interface variable set command */
DEFUN(ipv6_ospf6_transmitdelay, ipv6_ospf6_transmitdelay_cmd, "ipv6 ospf6 transmit-delay <1-3600>", IP6_STR OSPF6_STR "Transmit delay of this interface\n" SECONDS_STR) {
	struct ospf6_interface *oi;
	struct interface *ifp;
...
/* interface variable set command */
DEFUN(ipv6_ospf6_retransmitinterval, ipv6_ospf6_retransmitinterval_cmd, "ipv6 ospf6 retransmit-interval <1-65535>", IP6_STR OSPF6_STR "Time between retransmitting lost link state advertisements\n" SECONDS_STR) {
	struct ospf6_interface *oi;
	struct interface *ifp;
...
/* interface variable set command */
DEFUN(ipv6_ospf6_priority, ipv6_ospf6_priority_cmd, "ipv6 ospf6 priority <0-255>",
...
DEFUN(ipv6_ospf6_instance, ipv6_ospf6_instance_cmd, "ipv6 ospf6 instance-id <0-255>",
...
DEFUN(ipv6_ospf6_mtu_ignore, ipv6_ospf6_mtu_ignore_cmd, "ipv6 ospf6 mtu-ignore", IP6_STR OSPF6_STR "Ignore MTU mismatch on this interface\n") {
	struct ospf6_interface *oi;
	struct interface *ifp;
...
DEFUN(no_ipv6_ospf6_mtu_ignore, no_ipv6_ospf6_mtu_ignore_cmd, "no ipv6 ospf6 mtu-ignore", NO_STR IP6_STR OSPF6_STR "Ignore MTU mismatch on this interface\n") {
	struct ospf6_interface *oi;
	struct interface *ifp;
...
DEFUN(ipv6_ospf6_advertise_prefix_list, ipv6_ospf6_advertise_prefix_list_cmd, "ipv6 ospf6 advertise prefix-list WORD",
...
DEFUN(no_ipv6_ospf6_advertise_prefix_list, no_ipv6_ospf6_advertise_prefix_list_cmd, "no ipv6 ospf6 advertise prefix-list",
...
DEFUN(ipv6_ospf6_network, ipv6_ospf6_network_cmd, "ipv6 ospf6 network (broadcast|point-to-point)",
...
DEFUN(no_ipv6_ospf6_network, no_ipv6_ospf6_network_cmd, "no ipv6 ospf6 network",
...
static int config_write_ospf6_interface(struct vty *vty) {
	...
	for(ALL_LIST_ELEMENTS_RO(iflist, i, ifp)) {
		...
		vty_out(vty, "interface %s%s", oi->interface->name, VNL);
		...
		if(oi->hello_interval != OSPF6_INTERFACE_HELLO_INTERVAL) {
			vty_out(vty, " ipv6 ospf6 hello-interval %d%s", oi->hello_interval, VNL);
		}

		if(oi->dead_interval != OSPF6_INTERFACE_DEAD_INTERVAL) {
			vty_out(vty, " ipv6 ospf6 dead-interval %d%s", oi->dead_interval, VNL);
		}

		if(oi->rxmt_interval != OSPF6_INTERFACE_RXMT_INTERVAL) {
			vty_out(vty, " ipv6 ospf6 retransmit-interval %d%s", oi->rxmt_interval, VNL);
		}

		if(oi->priority != OSPF6_INTERFACE_PRIORITY) {
			vty_out(vty, " ipv6 ospf6 priority %d%s", oi->priority, VNL);
		}

		if(oi->transdelay != OSPF6_INTERFACE_TRANSDELAY) {
			vty_out(vty, " ipv6 ospf6 transmit-delay %d%s", oi->transdelay, VNL);
		}

		if(oi->instance_id != OSPF6_INTERFACE_INSTANCE_ID) {
			vty_out(vty, " ipv6 ospf6 instance-id %d%s", oi->instance_id, VNL);
		}

		if(oi->plist_name) {
			vty_out(vty, " ipv6 ospf6 advertise prefix-list %s%s", oi->plist_name, VNL);
		}
...
/* Clear interface */
DEFUN(clear_ipv6_ospf6_interface, clear_ipv6_ospf6_interface_cmd, "clear ipv6 ospf6 interface [IFNAME]", CLEAR_STR IP6_STR OSPF6_STR INTERFACE_STR IFNAME_STR) {
	struct interface *ifp;
