...
static int ospf6_vertex_id_cmp(void *a, void *b) {
	...
	ret = ntohl(ospf6_linkstate_prefix_adv_router(&va->vertex_id)) - ntohl(ospf6_linkstate_prefix_adv_router(&vb->vertex_id));
	if(ret) {
		return ret;
	}

	ret = ntohl(ospf6_linkstate_prefix_id(&va->vertex_id)) - ntohl(ospf6_linkstate_prefix_id(&vb->vertex_id));
	return ret;
}
...
static struct ospf6_vertex *ospf6_vertex_create(struct ospf6_lsa *lsa) {
	...
	v = (struct ospf6_vertex *) XMALLOC(MTYPE_OSPF6_VERTEX, sizeof(struct ospf6_vertex));
	...
	/* vertex_id */
	ospf6_linkstate_prefix(lsa->header->adv_router, lsa->header->id, &v->vertex_id);
...
static struct ospf6_lsa *ospf6_lsdesc_lsa(caddr_t lsdesc, struct ospf6_vertex *v) {
	...
	if(IS_OSPF6_DEBUG_SPF(PROCESS)) {
		...
		if(lsa) {
			...
			zlog_debug("  Link to: [%s Id:%s Adv:%s] No LSA", ospf6_lstype_name(type), ibuf, abuf);
		}
	}
...
static char *ospf6_lsdesc_backlink(struct ospf6_lsa *lsa, caddr_t lsdesc, struct ospf6_vertex *v) {
	caddr_t backlink, found = NULL;
	int size;

	size = (OSPF6_LSA_IS_TYPE(ROUTER, lsa) ? sizeof(struct ospf6_router_lsdesc) : sizeof(struct ospf6_network_lsdesc));
	for(backlink = OSPF6_LSA_HEADER_END(lsa->header) + 4; backlink + size <= OSPF6_LSA_END(lsa->header); backlink += size) {
		assert(!(OSPF6_LSA_IS_TYPE(NETWORK, lsa) && VERTEX_IS_TYPE(NETWORK, v)));

		if(OSPF6_LSA_IS_TYPE(NETWORK, lsa) && NETWORK_LSDESC_GET_NBR_ROUTERID(backlink) == v->lsa->header->adv_router) {
			found = backlink;
		}
          ...
          ROUTER_LSDESC_GET_NBR_IFID (backlink)
			...
			if(!ROUTER_LSDESC_IS_TYPE(POINTTOPOINT, backlink) || !ROUTER_LSDESC_IS_TYPE(POINTTOPOINT, lsdesc)) {
				continue;
			}
			if(ROUTER_LSDESC_GET_NBR_IFID(backlink) != ROUTER_LSDESC_GET_IFID(lsdesc) || ROUTER_LSDESC_GET_NBR_IFID(lsdesc) != ROUTER_LSDESC_GET_IFID(backlink)) {
				continue;
			}
			if(ROUTER_LSDESC_GET_NBR_ROUTERID(backlink) != v->lsa->header->adv_router || ROUTER_LSDESC_GET_NBR_ROUTERID(lsdesc) != lsa->header->adv_router) {
				continue;
			}
			found = backlink;
		}
...
static void ospf6_nexthop_calc(struct ospf6_vertex *w, struct ospf6_vertex *v, caddr_t lsdesc) {
	int i;
	ifindex_t ifindex;
	...
	ifindex
		= (VERTEX_IS_TYPE(NETWORK, v) ? v->nexthop[0].ifindex :
	...
	adv_router = (VERTEX_IS_TYPE(NETWORK, v) ? NETWORK_LSDESC_GET_NBR_ROUTERID(lsdesc) : ROUTER_LSDESC_GET_NBR_ROUTERID(lsdesc));

	i = 0;
	for(lsa = ospf6_lsdb_type_router_head(type, adv_router, oi->lsdb); lsa; lsa = ospf6_lsdb_type_router_next(type, adv_router, lsa)) {
		if(VERTEX_IS_TYPE(ROUTER, v) && htonl(ROUTER_LSDESC_GET_NBR_IFID(lsdesc)) != lsa->header->id) {
			continue;
		}
		...
		if(i < OSPF6_MULTI_PATH_LIMIT) {
			memcpy(&w->nexthop[i].address, &link_lsa->linklocal_addr, sizeof(struct in6_addr));
...
static int ospf6_spf_install(struct ospf6_vertex *v, struct ospf6_route_table *result_table) {
	struct ospf6_route *route;
	int i, j;
	struct ospf6_vertex *prev;

	if(IS_OSPF6_DEBUG_SPF(PROCESS)) {
		zlog_debug("SPF install %s hops %d cost %d", v->name, v->hops, v->cost);
	}

	route = ospf6_route_lookup(&v->vertex_id, result_table);
	if(route && route->path.cost < v->cost) {
		if(IS_OSPF6_DEBUG_SPF(PROCESS)) {
			zlog_debug("  already installed with lower cost (%d), ignore", route->path.cost);
		}
		...
		for(i = 0; i < OSPF6_MULTI_PATH_LIMIT && ospf6_nexthop_is_set(&v->nexthop[i]); i++) {
...
void ospf6_spf_reason_string(unsigned int reason, char *buf, int size) {
	...
	for(bit = 0; bit < array_size(ospf6_spf_reason_str); bit++) {
		if((reason & (1 << bit)) && (len < size)) {
			len += snprintf((buf + len), (size - len), "%s%s", (len > 0) ? ", " : "", ospf6_spf_reason_str[bit]);
		}
	}
}
...
void ospf6_spf_calculation(u_int32_t router_id, struct ospf6_route_table *result_table, struct ospf6_area *oa) {
	...
	lsa = ospf6_lsdb_lookup(htons(OSPF6_LSTYPE_ROUTER), htonl(0), router_id, oa->lsdb);
	if(lsa == NULL) {
		return;
	}
	...
	while(candidate_list->size) {
		...
		size = (VERTEX_IS_TYPE(ROUTER, v) ? sizeof(struct ospf6_router_lsdesc) : sizeof(struct ospf6_network_lsdesc));
		for(lsdesc = OSPF6_LSA_HEADER_END(v->lsa->header) + 4; lsdesc + size <= OSPF6_LSA_END(v->lsa->header); lsdesc += size) {
...
static void ospf6_spf_log_database(struct ospf6_area *oa) {
	...
	for(ALL_LIST_ELEMENTS_RO(oa->if_list, node, oi)) {
		snprintf(p, end - p, " I/F %s: %d", oi->interface->name, oi->lsdb->count);
		p = (buffer + strlen(buffer) < end ? buffer + strlen(buffer) : end);
	}
...
static int ospf6_spf_calculation_thread(struct thread *t) {
	...
	if(ospf6->backbone) {
		if(IS_OSPF6_DEBUG_SPF(PROCESS)) {
			zlog_debug("SPF calculation for Backbone area %s", ospf6->backbone->name);
		}
		if(IS_OSPF6_DEBUG_SPF(DATABASE)) {
			ospf6_spf_log_database(ospf6->backbone);
		}

		ospf6_spf_calculation(ospf6->router_id, ospf6->backbone->spf_table, ospf6->backbone);
	...
	for(route = ospf6_route_head(ospf6->route_table); route; route = ospf6_route_next(route)) {
	...
	if(IS_OSPF6_DEBUG_SPF(PROCESS) || IS_OSPF6_DEBUG_SPF(TIME)) {
		zlog_debug("SPF runtime: %lld sec %lld usec", (long long) runtime.tv_sec, (long long) runtime.tv_usec);
	}

	zlog_info(
		"SPF processing: # Areas: %d, SPF runtime: %lld sec %lld usec, "
		"Reason: %s\n",
		areas_processed, (long long) runtime.tv_sec, (long long) runtime.tv_usec, rbuf
	);
...
void ospf6_spf_schedule(struct ospf6 *ospf6, unsigned int reason) {
	...
	if(ospf6->t_spf_calc) {
		if(IS_OSPF6_DEBUG_SPF(PROCESS) || IS_OSPF6_DEBUG_SPF(TIME)) {
			zlog_debug("SPF: calculation timer is already scheduled: %p", (void *) ospf6->t_spf_calc);
		}
		return;
	}
	...
	ospf6->t_spf_calc = thread_add_timer_msec(master, ospf6_spf_calculation_thread, ospf6, delay);
}

void ospf6_spf_display_subtree(struct vty *vty, const char *prefix, int rest, struct ospf6_vertex *v) {
...
DEFUN(debug_ospf6_spf_process, debug_ospf6_spf_process_cmd, "debug ospf6 spf process",
      DEBUG_STR OSPF6_STR "Debug SPF Calculation\n"
...
DEFUN(debug_ospf6_spf_time, debug_ospf6_spf_time_cmd, "debug ospf6 spf time",
      DEBUG_STR OSPF6_STR "Debug SPF Calculation\n"
...
DEFUN(debug_ospf6_spf_database, debug_ospf6_spf_database_cmd, "debug ospf6 spf database",
      DEBUG_STR OSPF6_STR "Debug SPF Calculation\n"
...
DEFUN(no_debug_ospf6_spf_process, no_debug_ospf6_spf_process_cmd, "no debug ospf6 spf process",
      NO_STR DEBUG_STR OSPF6_STR "Quit Debugging SPF Calculation\n"
...
DEFUN(no_debug_ospf6_spf_time, no_debug_ospf6_spf_time_cmd, "no debug ospf6 spf time",
      NO_STR DEBUG_STR OSPF6_STR "Quit Debugging SPF Calculation\n"
...
DEFUN(no_debug_ospf6_spf_database, no_debug_ospf6_spf_database_cmd, "no debug ospf6 spf database",
      NO_STR DEBUG_STR OSPF6_STR "Debug SPF Calculation\n"
...
static int ospf6_timers_spf_set(struct vty *vty, unsigned int delay, unsigned int hold, unsigned int max) {
...
DEFUN(ospf6_timers_throttle_spf, ospf6_timers_throttle_spf_cmd, "timers throttle spf <0-600000> <0-600000> <0-600000>",
...
DEFUN(no_ospf6_timers_throttle_spf, no_ospf6_timers_throttle_spf_cmd, "no timers throttle spf",
      NO_STR "Adjust routing timers\n"
	     "Throttling adaptive timer\n"
	     "OSPF6 SPF timers\n") {
	return ospf6_timers_spf_set(vty, OSPF_SPF_DELAY_DEFAULT, OSPF_SPF_HOLDTIME_DEFAULT, OSPF_SPF_MAX_HOLDTIME_DEFAULT);
}
...
void ospf6_spf_config_write(struct vty *vty) {
	if(ospf6->spf_delay != OSPF_SPF_DELAY_DEFAULT || ospf6->spf_holdtime != OSPF_SPF_HOLDTIME_DEFAULT || ospf6->spf_max_holdtime != OSPF_SPF_MAX_HOLDTIME_DEFAULT) {
		vty_out(vty, " timers throttle spf %d %d %d%s", ospf6->spf_delay, ospf6->spf_holdtime, ospf6->spf_max_holdtime, VTY_NEWLINE);
	}
}
