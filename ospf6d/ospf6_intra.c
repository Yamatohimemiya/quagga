...
static char *ospf6_router_lsa_get_nbr_id(struct ospf6_lsa *lsa, char *buf, int buflen, int pos) {
	...
	if(lsa) {
		...
		lsdesc = (struct ospf6_router_lsdesc *) (start + pos * (sizeof(struct ospf6_router_lsdesc)));
		if((char *) lsdesc < end) {
...
int ospf6_router_is_stub_router(struct ospf6_lsa *lsa) {
	...
	if(lsa != NULL && OSPF6_LSA_IS_TYPE(ROUTER, lsa)) {
		rtr_lsa = (struct ospf6_router_lsa *) ((caddr_t) lsa->header + sizeof(struct ospf6_lsa_header));
...
int ospf6_router_lsa_originate(struct thread *thread) {
	...
	for(ALL_LIST_ELEMENTS(oa->if_list, node, nnode, oi)) {
		...
		if((oa->router_lsa_size_limit != 0) && ((size_t) ((char *) lsdesc - buffer) + sizeof(struct ospf6_router_lsdesc) > oa->router_lsa_size_limit)) {
...
static char *ospf6_network_lsa_get_ar_id(struct ospf6_lsa *lsa, char *buf, int buflen, int pos) {
	char *start, *end, *current;
...
static char *ospf6_link_lsa_get_prefix_str(struct ospf6_lsa *lsa, char *buf, int buflen, int pos) {
	char *start, *end, *current;
	struct ospf6_link_lsa *link_lsa;
	struct in6_addr in6;
...
/*****************************************/
static char *ospf6_intra_prefix_lsa_get_prefix_str(struct ospf6_lsa *lsa, char *buf, int buflen, int pos) {
	char *start, *end, *current;
...
void ospf6_intra_prefix_lsa_add(struct ospf6_lsa *lsa) {
	...
	for(current = start; current < end; current += OSPF6_PREFIX_SIZE(op)) {
		...
		if(CHECK_FLAG(op->prefix_options, OSPF6_PREFIX_OPTION_NU)) {
			if(IS_OSPF6_DEBUG_EXAMIN(INTRA_PREFIX)) {
				ospf6_linkstate_prefix2str((struct prefix *) OSPF6_PREFIX_BODY(op), buf, sizeof(buf));
				zlog_debug("%s: Skipping Prefix %s has NU option set", __func__, buf);
			}
			continue;
		}
		...
		if(direct_connect) {
			...
			for(i = 0; i < OSPF6_MULTI_PATH_LIMIT && ospf6_nexthop_is_set(&ls_entry->nexthop[i]); i++) {
...
static void ospf6_brouter_debug_print(struct ospf6_route *brouter) {
	...
	zlog_info("  memory: prev: %p this: %p next: %p parent rnode: %p", (void *) brouter->prev, (void *) brouter, (void *) brouter->next, (void *) brouter->rnode);
	zlog_info("  type: %d prefix: %s installed: %s changed: %s", brouter->type, destination, installed, changed);
	zlog_info(
		"  lock: %d flags: %s%s%s%s", brouter->lock, (CHECK_FLAG(brouter->flag, OSPF6_ROUTE_BEST) ? "B" : "-"), (CHECK_FLAG(brouter->flag, OSPF6_ROUTE_ADD) ? "A" : "-"), (CHECK_FLAG(brouter->flag, OSPF6_ROUTE_REMOVE) ? "R" : "-"),
...
void ospf6_intra_brouter_calculation(struct ospf6_area *oa) {
	...
	for(brouter = ospf6_route_head(oa->ospf6->brouter_table); brouter; brouter = ospf6_route_next(brouter)) {
		...
		if(IS_OSPF6_DEBUG_BROUTER_SPECIFIC_ROUTER_ID(brouter_id) || IS_OSPF6_DEBUG_ROUTE(MEMORY)) {
			zlog_info("%p: mark as removing: area %s brouter %s", (void *) brouter, oa->name, brouter_name);
	...
	for(brouter = ospf6_route_head(oa->spf_table); brouter; brouter = ospf6_route_next(brouter)) {
		...
		if(!OSPF6_OPT_ISSET(brouter->path.options, OSPF6_OPT_V6) || !OSPF6_OPT_ISSET(brouter->path.options, OSPF6_OPT_R)) {
			continue;
		}
		...
		if(IS_OSPF6_DEBUG_BROUTER_SPECIFIC_ROUTER_ID(brouter_id) || IS_OSPF6_DEBUG_ROUTE(MEMORY)) {
			zlog_info("%p: transfer: area %s brouter %s", (void *) brouter, oa->name, brouter_name);
			ospf6_brouter_debug_print(brouter);
		}
	}
	...
	for(brouter = ospf6_route_head(oa->ospf6->brouter_table); brouter; brouter = nbrouter) {
...
struct ospf6_lsa_handler router_handler = { OSPF6_LSTYPE_ROUTER, "Router", "Rtr", ospf6_router_lsa_show, ospf6_router_lsa_get_nbr_id };

struct ospf6_lsa_handler network_handler = { OSPF6_LSTYPE_NETWORK, "Network", "Net", ospf6_network_lsa_show, ospf6_network_lsa_get_ar_id };

struct ospf6_lsa_handler link_handler = { OSPF6_LSTYPE_LINK, "Link", "Lnk", ospf6_link_lsa_show, ospf6_link_lsa_get_prefix_str };

struct ospf6_lsa_handler intra_prefix_handler = { OSPF6_LSTYPE_INTRA_PREFIX, "Intra-Prefix", "INP", ospf6_intra_prefix_lsa_show, ospf6_intra_prefix_lsa_get_prefix_str };
...
DEFUN(debug_ospf6_brouter_router, debug_ospf6_brouter_router_cmd, "debug ospf6 border-routers router-id A.B.C.D",
...
DEFUN(no_debug_ospf6_brouter_router, no_debug_ospf6_brouter_router_cmd, "no debug ospf6 border-routers router-id",
...
DEFUN(debug_ospf6_brouter_area, debug_ospf6_brouter_area_cmd, "debug ospf6 border-routers area-id A.B.C.D",
      DEBUG_STR OSPF6_STR "Debug border router\n"
...
DEFUN(no_debug_ospf6_brouter_area, no_debug_ospf6_brouter_area_cmd, "no debug ospf6 border-routers area-id",
      NO_STR DEBUG_STR OSPF6_STR "Debug border router\n"
				 "Debug border routers in specific Area\n") {
