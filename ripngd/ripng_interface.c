...
static int ripng_multicast_join(struct interface *ifp) {
     ...
     */
		...
		ret = setsockopt(ripng->sock, IPPROTO_IPV6, IPV6_JOIN_GROUP, (char *) &mreq, sizeof(mreq));
		save_errno = errno;
	 ...
	 */
		...
		if(ret < 0) {
			zlog_warn("can't setsockopt IPV6_JOIN_GROUP: %s", safe_strerror(save_errno));
		}
...
static int ripng_multicast_leave(struct interface *ifp) {
	...
	if(if_is_up(ifp) && if_is_multicast(ifp)) {
		...
		ret = setsockopt(ripng->sock, IPPROTO_IPV6, IPV6_LEAVE_GROUP, (char *) &mreq, sizeof(mreq));
		if(ret < 0) {
			zlog_warn("can't setsockopt IPV6_LEAVE_GROUP: %s\n", safe_strerror(errno));
		}

		if(IS_RIPNG_DEBUG_EVENT) {
			zlog_debug("RIPng %s leave from all-rip-routers multicast group", ifp->name);
		}
...
int ripng_interface_up(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct stream *s;
	...
	if(IS_RIPNG_DEBUG_ZEBRA) {
		zlog_debug("interface up %s index %d flags %llx metric %d mtu %d", ifp->name, ifp->ifindex, (unsigned long long) ifp->flags, ifp->metric, ifp->mtu6);
	}
...
int ripng_interface_down(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct stream *s;
	struct interface *ifp;
	...
	if(IS_RIPNG_DEBUG_ZEBRA) {
		zlog_debug("interface down %s index %d flags %#llx metric %d mtu %d", ifp->name, ifp->ifindex, (unsigned long long) ifp->flags, ifp->metric, ifp->mtu6);
	}
...
int ripng_interface_add(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct interface *ifp;

	ifp = zebra_interface_add_read(zclient->ibuf, vrf_id);

	if(IS_RIPNG_DEBUG_ZEBRA) {
		zlog_debug("RIPng interface add %s index %d flags %#llx metric %d mtu %d", ifp->name, ifp->ifindex, (unsigned long long) ifp->flags, ifp->metric, ifp->mtu6);
	}
...
int ripng_interface_delete(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct interface *ifp;
	struct stream *s;
	...
	zlog_info("interface delete %s index %d flags %#llx metric %d mtu %d", ifp->name, ifp->ifindex, (unsigned long long) ifp->flags, ifp->metric, ifp->mtu6);
...
static void ripng_apply_address_add(struct connected *ifc) {
	...
	if((ripng_enable_if_lookup(ifc->ifp->name) >= 0) || (ripng_enable_network_lookup2(ifc) >= 0)) {
		ripng_redistribute_add(ZEBRA_ROUTE_CONNECT, RIPNG_ROUTE_INTERFACE, &address, ifc->ifp->ifindex, NULL, 0);
	}
}

int ripng_interface_address_add(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct connected *c;
	struct prefix *p;
	...
	if(p->family == AF_INET6) {
		...
		if(IS_RIPNG_DEBUG_ZEBRA) {
			zlog_debug("RIPng connected address %s/%d add", inet6_ntoa(p->u.prefix6), p->prefixlen);
		}
...
int ripng_interface_address_delete(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	...
	if(ifc) {
		...
		if(p->family == AF_INET6) {
			if(IS_RIPNG_DEBUG_ZEBRA) {
				zlog_debug("RIPng connected address %s/%d delete", inet_ntop(AF_INET6, &p->u.prefix6, buf, INET6_ADDRSTRLEN), p->prefixlen);
			}
...
static int ripng_enable_network_lookup_if(struct interface *ifp) {
	...
	for(ALL_LIST_ELEMENTS_RO(ifp->connected, node, connected)) {
		...
		if(p->family == AF_INET6) {
			...
			node = route_node_match(ripng_enable_network, (struct prefix *) &address);
			if(node) {
...
static void ripng_connect_set(struct interface *ifp, int set) {
	...
	for(ALL_LIST_ELEMENTS(ifp->connected, node, nnode, connected)) {
		...
		if(set) {
			...
			if((ripng_enable_if_lookup(connected->ifp->name) >= 0) || (ripng_enable_network_lookup2(connected) >= 0)) {
				ripng_redistribute_add(ZEBRA_ROUTE_CONNECT, RIPNG_ROUTE_INTERFACE, &address, connected->ifp->ifindex, NULL, 0);
			}
		} else {
			ripng_redistribute_delete(ZEBRA_ROUTE_CONNECT, RIPNG_ROUTE_INTERFACE, &address, connected->ifp->ifindex);
			if(ripng_redistribute_check(ZEBRA_ROUTE_CONNECT)) {
				ripng_redistribute_add(ZEBRA_ROUTE_CONNECT, RIPNG_ROUTE_REDISTRIBUTE, &address, connected->ifp->ifindex, NULL, 0);
			}
		}
	}
}
...
void ripng_enable_apply(struct interface *ifp) {
	...
	if((!ripng_if_ipv6_lladdress_check(ifp)) && (ri->enable_network || ri->enable_interface)) {
		ri->enable_network = 0;
		ri->enable_interface = 0;
		zlog_warn("Interface %s does not have any link-local address", ifp->name);
	}
	...
	if(ri->enable_network || ri->enable_interface) {
		{
			...
			if(!ri->t_wakeup) {
				ri->t_wakeup = thread_add_timer(master, ripng_interface_wakeup, ifp, 1);
			}
...
int ripng_network_write(struct vty *vty, int config_mode) {
	...
	for(node = route_top(ripng_enable_network); node; node = route_next(node)) {
		if(node->info) {
			...
			vty_out(vty, "%s%s/%d%s", config_mode ? " network " : "    ", inet_ntop(p->family, &p->u.prefix, buf, BUFSIZ), p->prefixlen, VTY_NEWLINE);
		}
	}
...
DEFUN(ipv6_ripng_split_horizon, ipv6_ripng_split_horizon_cmd, "ipv6 ripng split-horizon",
      IPV6_STR "Routing Information Protocol\n"
...
DEFUN(ipv6_ripng_split_horizon_poisoned_reverse, ipv6_ripng_split_horizon_poisoned_reverse_cmd, "ipv6 ripng split-horizon poisoned-reverse",
      IPV6_STR "Routing Information Protocol\n"
	       "Perform split horizon\n"
...
DEFUN(no_ipv6_ripng_split_horizon, no_ipv6_ripng_split_horizon_cmd, "no ipv6 ripng split-horizon",
      NO_STR IPV6_STR "Routing Information Protocol\n"
		      "Perform split horizon\n") {
	struct interface *ifp;
	struct ripng_interface *ri;
...
ALIAS(no_ipv6_ripng_split_horizon, no_ipv6_ripng_split_horizon_poisoned_reverse_cmd, "no ipv6 ripng split-horizon poisoned-reverse",
      NO_STR IPV6_STR "Routing Information Protocol\n"
		      "Perform split horizon\n"
		      "With poisoned-reverse\n")

DEFUN(ripng_passive_interface, ripng_passive_interface_cmd, "passive-interface IFNAME",
      "Suppress routing updates on an interface\n"
      "Interface name\n") {
	return ripng_passive_interface_set(vty, argv[0]);
}

DEFUN(no_ripng_passive_interface, no_ripng_passive_interface_cmd, "no passive-interface IFNAME",
...
static int interface_config_write(struct vty *vty) {
	...
	for(ALL_LIST_ELEMENTS_RO(iflist, node, ifp)) {
       ...
       **/
		if((!ifp->desc) && (ri->split_horizon == ri->split_horizon_default)) {
			continue;
		}
		...
		if(ri->split_horizon != ri->split_horizon_default) {
			switch(ri->split_horizon) {
				case RIPNG_SPLIT_HORIZON: vty_out(vty, " ipv6 ripng split-horizon%s", VTY_NEWLINE); break;
				case RIPNG_SPLIT_HORIZON_POISONED_REVERSE: vty_out(vty, " ipv6 ripng split-horizon poisoned-reverse%s", VTY_NEWLINE); break;
				case RIPNG_NO_SPLIT_HORIZON:
				default: vty_out(vty, " no ipv6 ripng split-horizon%s", VTY_NEWLINE); break;
			}
		}
