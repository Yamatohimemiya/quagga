...
/* Send RIPng packet. */
int ripng_send_packet(caddr_t buf, int bufsize, struct sockaddr_in6 *to, struct interface *ifp) {
	int ret;
	struct msghdr msg;
	struct iovec iov;
	...
	if(ret < 0) {
		if(to) {
			zlog_err("RIPng send fail on %s to %s: %s", ifp->name, inet6_ntoa(to->sin6_addr), safe_strerror(errno));
		} else {
...
static int ripng_recv_packet(int sock, u_char *buf, int bufsize, struct sockaddr_in6 *from, ifindex_t *ifindex, int *hoplimit) {
	int ret;
	struct msghdr msg;
	struct iovec iov;
	struct cmsghdr *cmsgptr;
	...
	for(cmsgptr = ZCMSG_FIRSTHDR(&msg); cmsgptr != NULL; cmsgptr = CMSG_NXTHDR(&msg, cmsgptr)) {
...
void ripng_packet_dump(struct ripng_packet *packet, int size, const char *sndrcv) {
	...
	for(lim = (caddr_t) packet + size; (caddr_t) rte < lim; rte++) {
		if(rte->metric == RIPNG_METRIC_NEXTHOP) {
			...
			zlog_debug("  %s/%d metric %d tag %d", inet6_ntoa(rte->addr), rte->prefixlen, rte->metric, ntohs(rte->tag));
		}
	}
}

/* RIPng next hop address RTE (Route Table Entry). */
static void ripng_nexthop_rte(struct rte *rte, struct sockaddr_in6 *from, struct ripng_nexthop *nexthop) {
   ...
   set to zero on sending and ignored on receiption.  */
	if(ntohs(rte->tag) != 0) {
		zlog_warn("RIPng nexthop RTE with non zero tag value %d from %s", ntohs(rte->tag), inet6_ntoa(from->sin6_addr));
	}

	if(rte->prefixlen != 0) {
		zlog_warn("RIPng nexthop RTE with non zero prefixlen value %d from %s", rte->prefixlen, inet6_ntoa(from->sin6_addr));
	}
   ...
   a link-local address, it should be treated as 0:0:0:0:0:0:0:0.  */
	zlog_warn("RIPng nexthop RTE with non link-local address %s from %s", inet6_ntoa(rte->addr), inet_ntop(AF_INET6, &from->sin6_addr, buf, INET6_BUFSIZ));
...
static int ripng_lladdr_check(struct interface *ifp, struct in6_addr *addr) {
	...
	for(ALL_LIST_ELEMENTS_RO(ifp->connected, node, connected)) {
		...
		if(p->family == AF_INET6 && IN6_IS_ADDR_LINKLOCAL(&p->u.prefix6) && IN6_ARE_ADDR_EQUAL(&p->u.prefix6, addr)) {
			return 1;
		}
	}
	return 0;
...
struct ripng_info *ripng_ecmp_delete(struct ripng_info *rinfo) {
	...
	if(listcount(list) > 1) {
       ...
       * the list for garbage collection time, with INFINITY metric. */
		...
		RIPNG_TIMER_ON(rinfo->t_garbage_collect, ripng_garbage_collect, ripng->garbage_time);
...
static int ripng_filter(int ripng_distribute, struct prefix_ipv6 *p, struct ripng_interface *ri) {
	struct distribute *dist;
	struct access_list *alist;
	struct prefix_list *plist;
	int distribute = ripng_distribute == RIPNG_FILTER_OUT ? DISTRIBUTE_V6_OUT : DISTRIBUTE_V6_IN;
	const char *inout = ripng_distribute == RIPNG_FILTER_OUT ? "out" : "in";

	/* Input distribute-list filtering. */
	if(ri->list[ripng_distribute]) {
		if(access_list_apply(ri->list[ripng_distribute], (struct prefix *) p) == FILTER_DENY) {
			if(IS_RIPNG_DEBUG_PACKET) {
				zlog_debug("%s/%d filtered by distribute %s", inet6_ntoa(p->prefix), p->prefixlen, inout);
			}
			return -1;
		}
	}
	if(ri->prefix[ripng_distribute]) {
		if(prefix_list_apply(ri->prefix[ripng_distribute], (struct prefix *) p) == PREFIX_DENY) {
			if(IS_RIPNG_DEBUG_PACKET) {
				zlog_debug("%s/%d filtered by prefix-list %s", inet6_ntoa(p->prefix), p->prefixlen, inout);
			}
			return -1;
		}
	}
...
static void ripng_route_process(struct rte *rte, struct sockaddr_in6 *from, struct ripng_nexthop *ripng_nexthop, struct interface *ifp) {
	int ret;
	...
	if(ri->routemap[RIPNG_FILTER_IN]) {
		...
		ret = route_map_apply(ri->routemap[RIPNG_FILTER_IN], (struct prefix *) &p, RMAP_RIPNG, &newinfo);

		if(ret == RMAP_DENYMATCH) {
			if(IS_RIPNG_DEBUG_PACKET) {
				zlog_debug("RIPng %s/%d is filtered by route-map in", inet6_ntoa(p.prefix), p.prefixlen);
			}
			return;
		}
   ...
   * one. */
	...
	if((list = rp->info) != NULL) {
		for(ALL_LIST_ELEMENTS_RO(list, node, rinfo)) {
			...
			if(IPV6_ADDR_SAME(&rinfo->from, &from->sin6_addr) && IPV6_ADDR_SAME(&rinfo->nexthop, nexthop)) {
				break;
			}
	...
	if(rinfo) {
		...
		if(rinfo->type != ZEBRA_ROUTE_RIPNG && rinfo->metric != RIPNG_METRIC_INFINITY) {
			route_unlock_node(rp);
			return;
		}

		/* Local static route. */
		if(rinfo->type == ZEBRA_ROUTE_RIPNG && ((rinfo->sub_type == RIPNG_ROUTE_STATIC) || (rinfo->sub_type == RIPNG_ROUTE_DEFAULT)) && rinfo->metric != RIPNG_METRIC_INFINITY) {
			route_unlock_node(rp);
			return;
		}
	}
	...
	if(!rinfo) {
	 ...
	 route, reinitialize the timeout.  */
		same = (IN6_ARE_ADDR_EQUAL(&rinfo->from, &from->sin6_addr) && (rinfo->ifindex == ifp->ifindex));
...
void ripng_redistribute_add(int type, int sub_type, struct prefix_ipv6 *p, ifindex_t ifindex, struct in6_addr *nexthop, route_tag_t tag) {
	...
	if((list = rp->info) != NULL && listcount(list) != 0) {
		...
		if(rinfo->type == ZEBRA_ROUTE_CONNECT && rinfo->sub_type == RIPNG_ROUTE_INTERFACE && rinfo->metric != RIPNG_METRIC_INFINITY) {
			route_unlock_node(rp);
			return;
		}
       ...
       **/
		if(rinfo->type == ZEBRA_ROUTE_RIPNG && ((rinfo->sub_type == RIPNG_ROUTE_STATIC) || (rinfo->sub_type == RIPNG_ROUTE_DEFAULT))) {
			if(type != ZEBRA_ROUTE_RIPNG || ((sub_type != RIPNG_ROUTE_STATIC) && (sub_type != RIPNG_ROUTE_DEFAULT))) {
				route_unlock_node(rp);
				return;
			}
		}
	...
	if(IS_RIPNG_DEBUG_EVENT) {
		if(!nexthop) {
			zlog_debug("Redistribute new prefix %s/%d on the interface %s", inet6_ntoa(p->prefix), p->prefixlen, ifindex2ifname(ifindex));
		} else {
			zlog_debug("Redistribute new prefix %s/%d with nexthop %s on the interface %s", inet6_ntoa(p->prefix), p->prefixlen, inet6_ntoa(*nexthop), ifindex2ifname(ifindex));
		}
	}
			...
			if(rinfo != NULL && rinfo->type == type && rinfo->sub_type == sub_type && rinfo->ifindex == ifindex) {
				...
				if(IS_RIPNG_DEBUG_EVENT) {
					zlog_debug(
						...
						inet6_ntoa(p->prefix), p->prefixlen, ifindex2ifname(ifindex)
					);
				}
...
void ripng_redistribute_withdraw(int type) {
	...
	for(rp = route_top(ripng->table); rp; rp = route_next(rp)) {
		if((list = rp->info) != NULL) {
			rinfo = listgetdata(listhead(list));
			if((rinfo->type == type) && (rinfo->sub_type != RIPNG_ROUTE_INTERFACE)) {
				/* Perform poisoned reverse. */
				rinfo->metric = RIPNG_METRIC_INFINITY;
				RIPNG_TIMER_ON(rinfo->t_garbage_collect, ripng_garbage_collect, ripng->garbage_time);
				RIPNG_TIMER_OFF(rinfo->t_timeout);
				...
				if(IS_RIPNG_DEBUG_EVENT) {
					...
					zlog_debug("Poisone %s/%d on the interface %s [withdraw]", inet6_ntoa(p->prefix), p->prefixlen, ifindex2ifname(rinfo->ifindex));
				}
...
static void ripng_response_process(struct ripng_packet *packet, int size, struct sockaddr_in6 *from, struct interface *ifp, int hoplimit) {
   ...
   The Response must be ignored if it is not from the RIPng port.  */
	if(ntohs(from->sin6_port) != RIPNG_PORT_DEFAULT) {
		zlog_warn("RIPng packet comes from non RIPng port %d from %s", ntohs(from->sin6_port), inet6_ntoa(from->sin6_addr));
   ...
   datagram must be a link-local address.  */
	if(!IN6_IS_ADDR_LINKLOCAL(&from->sin6_addr)) {
		zlog_warn("RIPng packet comes from non link local address %s", inet6_ntoa(from->sin6_addr));
		ripng_peer_bad_packet(from);
		return;
	}
   ...
   and such datagrams must be ignored. */
	if(ripng_lladdr_check(ifp, &from->sin6_addr)) {
		zlog_warn("RIPng packet comes from my own link local address %s", inet6_ntoa(from->sin6_addr));
		ripng_peer_bad_packet(from);
		return;
	}
   ...
   packets) must be examined to ensure that the hop count is 255. */
	if(hoplimit >= 0 && hoplimit != 255) {
		zlog_warn("RIPng packet comes with non 255 hop count %d from %s", hoplimit, inet6_ntoa(from->sin6_addr));
		ripng_peer_bad_packet(from);
		return;
	}
	...
	for(lim = ((caddr_t) packet) + size; (caddr_t) rte < lim; rte++) {
		...
		if(IN6_IS_ADDR_MULTICAST(&rte->addr)) {
			zlog_warn("Destination prefix is a multicast address %s/%d [%d]", inet6_ntoa(rte->addr), rte->prefixlen, rte->metric);
			ripng_peer_bad_route(from);
			continue;
		}
		if(IN6_IS_ADDR_LINKLOCAL(&rte->addr)) {
			zlog_warn("Destination prefix is a link-local address %s/%d [%d]", inet6_ntoa(rte->addr), rte->prefixlen, rte->metric);
			ripng_peer_bad_route(from);
			continue;
		}
		if(IN6_IS_ADDR_LOOPBACK(&rte->addr)) {
			zlog_warn("Destination prefix is a loopback address %s/%d [%d]", inet6_ntoa(rte->addr), rte->prefixlen, rte->metric);
			ripng_peer_bad_route(from);
			continue;
		}
		...
		if(rte->prefixlen > 128) {
			zlog_warn("Invalid prefix length %s/%d from %s%%%s", inet6_ntoa(rte->addr), rte->prefixlen, inet6_ntoa(from->sin6_addr), ifp->name);
			ripng_peer_bad_route(from);
			continue;
		}
...
static void ripng_request_process(struct ripng_packet *packet, int size, struct sockaddr_in6 *from, struct interface *ifp) {
	...
	if(lim == ((caddr_t) (rte + 1)) && IN6_IS_ADDR_UNSPECIFIED(&rte->addr) && rte->prefixlen == 0 && rte->metric == RIPNG_METRIC_INFINITY) {
...
static int ripng_read(struct thread *thread) {
	...
	/* Read RIPng packet. */
	len = ripng_recv_packet(sock, STREAM_DATA(ripng->ibuf), STREAM_SIZE(ripng->ibuf), &from, &ifindex, &hoplimit);
	if(len < 0) {
	...
	if(((len - 4) % 20) != 0) {
		zlog_warn("RIPng invalid packet size %d from %s", len, inet6_ntoa(from.sin6_addr));
		ripng_peer_bad_packet(&from);
		return 0;
	}
	...
	if(IS_RIPNG_DEBUG_EVENT) {
		zlog_debug("RIPng packet received from %s port %d on %s", inet6_ntoa(from.sin6_addr), ntohs(from.sin6_port), ifp ? ifp->name : "unknown");
	}
	...
	if(packet->version != ripng->version) {
		zlog_warn("RIPng packet version %d doesn't fit to my version %d", packet->version, ripng->version);
		ripng_peer_bad_packet(&from);
		return 0;
	}
	...
	switch(packet->command) {
		case RIPNG_REQUEST: ripng_request_process(packet, len, &from, ifp); break;
		case RIPNG_RESPONSE: ripng_response_process(packet, len, &from, ifp, hoplimit); break;
		default:
...
static int ripng_update(struct thread *t) {
	...
	for(ALL_LIST_ELEMENTS_RO(iflist, node, ifp)) {
		...
		if(ri->ri_send == RIPNG_SEND_OFF) {
			if(IS_RIPNG_DEBUG_EVENT) {
				zlog(NULL, LOG_DEBUG, "[Event] RIPng send to if %d is suppressed by config", ifp->ifindex);
			}
			continue;
		}
...
int ripng_triggered_update(struct thread *t) {
	...
	ripng->t_triggered_interval = thread_add_timer(master, ripng_triggered_interval, NULL, interval);
...
int ripng_write_rte(int num, struct stream *s, struct prefix_ipv6 *p, struct in6_addr *nexthop, u_int16_t tag, u_char metric) {
	/* RIPng packet header. */
	if(num == 0) {
...
void ripng_output_process(struct interface *ifp, struct sockaddr_in6 *to, int route_type) {
	int ret;
	struct route_node *rp;
	struct ripng_info *rinfo;
	struct ripng_interface *ri;
	...
	if(IS_RIPNG_DEBUG_EVENT) {
		if(to) {
			zlog_debug("RIPng update routes to neighbor %s", inet6_ntoa(to->sin6_addr));
		} else {
	...
	for(rp = route_top(ripng->table); rp; rp = route_next(rp)) {
		if((list = rp->info) != NULL && (rinfo = listgetdata(listhead(list))) != NULL && rinfo->suppress == 0) {
	   ...
	   */
			...
			if(route_type == ripng_changed_route && (!(rinfo->flags & RIPNG_RTF_CHANGED))) {
				continue;
			}
			...
			if(ri->split_horizon == RIPNG_SPLIT_HORIZON) {
				...
				for(ALL_LIST_ELEMENTS_RO(list, listnode, tmp_rinfo)) {
					if(tmp_rinfo->type == ZEBRA_ROUTE_RIPNG && tmp_rinfo->ifindex == ifp->ifindex) {
						suppress = 1;
						break;
					}
				}
	   ...
	   */
			...
			if(ri->routemap[RIPNG_FILTER_OUT]) {
				...
				ret = route_map_apply(ri->routemap[RIPNG_FILTER_OUT], (struct prefix *) p, RMAP_RIPNG, rinfo);

				if(ret == RMAP_DENYMATCH) {
					if(IS_RIPNG_DEBUG_PACKET) {
						zlog_debug("RIPng %s/%d is filtered by route-map out", inet6_ntoa(p->prefix), p->prefixlen);
					}
					continue;
				}
			}
			...
			if(ripng->route_map[rinfo->type].name) {
				...
				ret = route_map_apply(ripng->route_map[rinfo->type].map, (struct prefix *) p, RMAP_RIPNG, rinfo);

				if(ret == RMAP_DENYMATCH) {
					if(IS_RIPNG_DEBUG_PACKET) {
						zlog_debug("RIPng %s/%d is filtered by route-map", inet6_ntoa(p->prefix), p->prefixlen);
					}
					continue;
				}
			}
			...
			if(!rinfo->metric_set) {
				...
				if(ripng->route_map[rinfo->type].metric_config && rinfo->metric != RIPNG_METRIC_INFINITY) {
		     ...
		     one, use default-metric value */
					if(rinfo->type != ZEBRA_ROUTE_RIPNG && rinfo->type != ZEBRA_ROUTE_CONNECT && rinfo->metric != RIPNG_METRIC_INFINITY) {
	   ...
	   */
			...
			if(ri->routemap[RIPNG_FILTER_OUT]) {
				...
				ret = route_map_apply(ri->routemap[RIPNG_FILTER_OUT], (struct prefix *) p, RMAP_RIPNG, &newinfo);
...
int ripng_request(struct interface *ifp) {
	...
	return ripng_send_packet((caddr_t) &ripng_packet, sizeof(ripng_packet), NULL, ifp);
}
...
void ripng_event(enum ripng_event event, int sock) {
	...
	switch(event) {
		...
		case RIPNG_UPDATE_EVENT:
			...
			ripng->t_update = thread_add_timer(master, ripng_update, NULL, sock ? 2 : ripng->update_time + jitter);
			break;
		case RIPNG_TRIGGERED_UPDATE:
			if(ripng->t_triggered_interval) {
				ripng->trigger = 1;
			} else if(!ripng->t_triggered_update) {
				ripng->t_triggered_update = thread_add_event(master, ripng_triggered_update, NULL, 0);
			}
			break;
		default: break;
	}
...
static char *ripng_route_subtype_print(struct ripng_info *rinfo) {
	...
	switch(rinfo->sub_type) {
		case RIPNG_ROUTE_RTE: strcat(str, "n"); break;
		case RIPNG_ROUTE_STATIC: strcat(str, "s"); break;
		case RIPNG_ROUTE_DEFAULT: strcat(str, "d"); break;
		case RIPNG_ROUTE_REDISTRIBUTE: strcat(str, "r"); break;
		case RIPNG_ROUTE_INTERFACE: strcat(str, "i"); break;
	...
	for(rp = route_top(ripng->table); rp; rp = route_next(rp)) {
...
#ifdef DEBUG
			len = vty_out(vty, "R(a) %d/%d %s/%d ", aggregate->count, aggregate->suppress, inet6_ntoa(p->prefix), p->prefixlen);
#else
			len = vty_out(vty, "R(a) %s/%d ", inet6_ntoa(p->prefix), p->prefixlen);
#endif /* DEBUG */
			vty_out(vty, "%s", VTY_NEWLINE);
			vty_out(vty, "%*s", 18, " ");

			vty_out(vty, "%*s", 28, " ");
			vty_out(vty, "self      %2d  %3d%s", aggregate->metric, aggregate->tag, VTY_NEWLINE);
		}
		...
		if((list = rp->info) != NULL) {
			for(ALL_LIST_ELEMENTS_RO(list, listnode, rinfo)) {
...
#ifdef DEBUG
				len = vty_out(vty, "%c(%s) 0/%d %s/%d ", zebra_route_char(rinfo->type), ripng_route_subtype_print(rinfo), rinfo->suppress, inet6_ntoa(p->prefix), p->prefixlen);
#else
				len = vty_out(vty, "%c(%s) %s/%d ", zebra_route_char(rinfo->type), ripng_route_subtype_print(rinfo), inet6_ntoa(p->prefix), p->prefixlen);
#endif /* DEBUG */
				vty_out(vty, "%s", VTY_NEWLINE);
				vty_out(vty, "%*s", 18, " ");
				...
				/* from */
				if((rinfo->type == ZEBRA_ROUTE_RIPNG) && (rinfo->sub_type == RIPNG_ROUTE_RTE)) {
				...
				vty_out(vty, " %2d  %3d  ", rinfo->metric, rinfo->tag);
...
DEFUN(show_ipv6_ripng_status, show_ipv6_ripng_status_cmd, "show ipv6 ripng status",
      SHOW_STR IPV6_STR "Show RIPng routes\n"
	...
	vty_out(vty, "  Sending updates every %ld seconds with +/-50%%,", ripng->update_time);
	vty_out(vty, " next due in %lu seconds%s", thread_timer_remain_second(ripng->t_update), VTY_NEWLINE);
	vty_out(vty, "  Timeout after %ld seconds,", ripng->timeout_time);
	vty_out(vty, " garbage collect after %ld seconds%s", ripng->garbage_time, VTY_NEWLINE);

	/* Filtering status show. */
	config_show_distribute(vty);

	/* Default metric information. */
	vty_out(vty, "  Default redistribution metric is %d%s", ripng->default_metric, VTY_NEWLINE);
	...
	for(ALL_LIST_ELEMENTS_RO(iflist, node, ifp)) {
		...
		if(ri->enable_network || ri->enable_interface) {
			vty_out(vty, "    %-17s%-3d   %-3d%s", ifp->name, ripng->version, ripng->version, VTY_NEWLINE);
		}
	}
...
DEFUN(router_ripng, router_ripng_cmd, "router ripng",
...
DEFUN(no_router_ripng, no_router_ripng_cmd, "no router ripng",
...
DEFUN(ripng_aggregate_address, ripng_aggregate_address_cmd, "aggregate-address X:X::X:X/M",
...
DEFUN(no_ripng_aggregate_address, no_ripng_aggregate_address_cmd, "no aggregate-address X:X::X:X/M",
      NO_STR "Delete aggregate RIPng route announcement\n"
	     "Aggregate network") {
	int ret;
	struct prefix p;
...
DEFUN(ripng_default_metric, ripng_default_metric_cmd, "default-metric <1-16>",
...
DEFUN(no_ripng_default_metric, no_ripng_default_metric_cmd, "no default-metric",
...
ALIAS(no_ripng_default_metric, no_ripng_default_metric_val_cmd, "no default-metric <1-16>",
      NO_STR "Set a metric of redistribute routes\n"
	     "Default metric\n")
...
DEFUN(ripng_timers, ripng_timers_cmd, "timers basic <0-65535> <0-65535> <0-65535>",
      "RIPng timers setup\n"
      "Basic timer\n"
...
DEFUN(no_ripng_timers, no_ripng_timers_cmd, "no timers basic",
      NO_STR "RIPng timers setup\n"
	     "Basic timer\n") {
...
ALIAS(no_ripng_timers, no_ripng_timers_val_cmd, "no timers basic <0-65535> <0-65535> <0-65535>",
      NO_STR "RIPng timers setup\n"
	     "Basic timer\n"
	     "Routing table update timer value in second. Default is 30.\n"
	     "Routing information timeout timer. Default is 180.\n"
...
DEFUN(show_ipv6_protocols, show_ipv6_protocols_cmd, "show ipv6 protocols", SHOW_STR IPV6_STR "Routing protocol information") {
	if(!ripng) {
		return CMD_SUCCESS;
	}
	...
	vty_out(vty, "Sending updates every %ld seconds, next due in %d seconds%s", ripng->update_time, 0, VTY_NEWLINE);

	vty_out(vty, "Timerout after %ld seconds, garbage correct %ld%s", ripng->timeout_time, ripng->garbage_time, VTY_NEWLINE);
...
DEFUN(ripng_default_information_originate, ripng_default_information_originate_cmd, "default-information originate",
...
DEFUN(no_ripng_default_information_originate, no_ripng_default_information_originate_cmd, "no default-information originate",
      NO_STR "Default route information\n"
	     "Distribute default route\n") {
	struct prefix_ipv6 p;
...
DEFUN(ripng_allow_ecmp, ripng_allow_ecmp_cmd, "allow-ecmp", "Allow Equal Cost MultiPath\n") {
	if(ripng->ecmp) {
...
DEFUN(no_ripng_allow_ecmp, no_ripng_allow_ecmp_cmd, "no allow-ecmp", NO_STR "Allow Equal Cost MultiPath\n") {
	if(!ripng->ecmp) {
...
static int ripng_config_write(struct vty *vty) {
	...
	if(ripng) {
		...
		if(ripng->default_metric != RIPNG_DEFAULT_METRIC_DEFAULT) {
			vty_out(vty, " default-metric %d%s", ripng->default_metric, VTY_NEWLINE);
		}
		...
		for(rp = route_top(ripng->aggregate); rp; rp = route_next(rp)) {
			if(rp->info != NULL) {
				vty_out(vty, " aggregate-address %s/%d%s", inet6_ntoa(rp->p.u.prefix6), rp->p.prefixlen,
		...
		for(rp = route_top(ripng->route); rp; rp = route_next(rp)) {
			if(rp->info != NULL) {
				vty_out(vty, " route %s/%d%s", inet6_ntoa(rp->p.u.prefix6), rp->p.prefixlen, VTY_NEWLINE);
			}
		}

		/* RIPng timers configuration. */
		if(ripng->update_time != RIPNG_UPDATE_TIMER_DEFAULT || ripng->timeout_time != RIPNG_TIMEOUT_TIMER_DEFAULT || ripng->garbage_time != RIPNG_GARBAGE_TIMER_DEFAULT) {
			vty_out(vty, " timers basic %ld %ld %ld%s", ripng->update_time, ripng->timeout_time, ripng->garbage_time, VTY_NEWLINE);
		}
#if 0
...
static void ripng_distribute_update_all_wrapper(const char *notused) {
...
static void ripng_routemap_update_redistribute(void) {
	...
	if(ripng) {
		for(i = 0; i < ZEBRA_ROUTE_MAX; i++) {
			if(ripng->route_map[i].name) {
				ripng->route_map[i].map = route_map_lookup_by_name(ripng->route_map[i].name);
			}
		}
	}
}
