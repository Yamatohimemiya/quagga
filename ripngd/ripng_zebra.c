...
static void ripng_zebra_ipv6_send(struct route_node *rp, u_char cmd) {
	...
	if(vrf_bitmap_check(zclient->redist[ZEBRA_ROUTE_RIPNG], VRF_DEFAULT)) {
		...
		zapi_ipv6_route(cmd, zclient, (struct prefix_ipv6 *) &rp->p, &api);

		if(IS_RIPNG_DEBUG_ZEBRA) {
			if(ripng->ecmp) {
				zlog_debug("%s: %s/%d nexthops %d", (cmd == ZEBRA_IPV6_ROUTE_ADD) ? "Install into zebra" : "Delete from zebra", inet6_ntoa(rp->p.u.prefix6), rp->p.prefixlen, count);
			} else {
				zlog_debug("%s: %s/%d", (cmd == ZEBRA_IPV6_ROUTE_ADD) ? "Install into zebra" : "Delete from zebra", inet6_ntoa(rp->p.u.prefix6), rp->p.prefixlen);
			}
		}
	}
}
...
static int ripng_zebra_read_ipv6(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct stream *s;
	struct zapi_ipv6 api;
...
DEFUN(ripng_redistribute_ripng, ripng_redistribute_ripng_cmd, "redistribute ripng",
...
DEFUN(no_ripng_redistribute_ripng, no_ripng_redistribute_ripng_cmd, "no redistribute ripng",
...
DEFUN(ripng_redistribute_type, ripng_redistribute_type_cmd, "redistribute " QUAGGA_REDIST_STR_RIPNGD, "Redistribute\n" QUAGGA_REDIST_HELP_STR_RIPNGD) {
	int type;
...
DEFUN(no_ripng_redistribute_type, no_ripng_redistribute_type_cmd, "no redistribute " QUAGGA_REDIST_STR_RIPNGD, NO_STR "Redistribute\n" QUAGGA_REDIST_HELP_STR_RIPNGD) {
	int type;
...
DEFUN(ripng_redistribute_type_metric, ripng_redistribute_type_metric_cmd, "redistribute " QUAGGA_REDIST_STR_RIPNGD " metric <0-16>",
...
ALIAS(no_ripng_redistribute_type, no_ripng_redistribute_type_metric_cmd, "no redistribute " QUAGGA_REDIST_STR_RIPNGD " metric <0-16>",
      NO_STR "Redistribute\n" QUAGGA_REDIST_HELP_STR_RIPNGD "Metric\n"
	     "Metric value\n")

DEFUN(ripng_redistribute_type_routemap, ripng_redistribute_type_routemap_cmd, "redistribute " QUAGGA_REDIST_STR_RIPNGD " route-map WORD",
...
ALIAS(no_ripng_redistribute_type, no_ripng_redistribute_type_routemap_cmd, "no redistribute " QUAGGA_REDIST_STR_RIPNGD " route-map WORD",
      NO_STR "Redistribute\n" QUAGGA_REDIST_HELP_STR_RIPNGD "Route map reference\n"
	     "Pointer to route-map entries\n")

DEFUN(ripng_redistribute_type_metric_routemap, ripng_redistribute_type_metric_routemap_cmd, "redistribute " QUAGGA_REDIST_STR_RIPNGD " metric <0-16> route-map WORD",
      "Redistribute\n" QUAGGA_REDIST_HELP_STR_RIPNGD "Metric\n"
      "Metric value\n"
      "Route map reference\n"
      "Pointer to route-map entries\n") {
...
ALIAS(no_ripng_redistribute_type, no_ripng_redistribute_type_metric_routemap_cmd, "no redistribute " QUAGGA_REDIST_STR_RIPNGD " metric <0-16> route-map WORD",
      NO_STR "Redistribute\n" QUAGGA_REDIST_HELP_STR_RIPNGD "Route map reference\n"
	     "Pointer to route-map entries\n")
			...
			if(config_mode) {
				if(ripng->route_map[i].metric_config) {
					if(ripng->route_map[i].name) {
						vty_out(vty, " redistribute %s metric %d route-map %s%s", zebra_route_string(i), ripng->route_map[i].metric, ripng->route_map[i].name, VTY_NEWLINE);
					} else {
						vty_out(vty, " redistribute %s metric %d%s", zebra_route_string(i), ripng->route_map[i].metric, VTY_NEWLINE);
					}
				} else {
					if(ripng->route_map[i].name) {
						vty_out(vty, " redistribute %s route-map %s%s", zebra_route_string(i), ripng->route_map[i].name, VTY_NEWLINE);
					} else {
