...
static struct ripng_peer *ripng_peer_get(struct in6_addr *addr) {
	...
	peer->t_timeout = thread_add_timer(master, ripng_peer_timeout, peer, RIPNG_PEER_TIMER_DEFAULT);
...
#define RIPNG_UPTIME_LEN 25
	...
	for(ALL_LIST_ELEMENTS(peer_list, node, nnode, peer)) {
		vty_out(vty, "    %s %s%14s %10d %10d %10d      %s%s", inet6_ntoa(peer->addr), VTY_NEWLINE, " ", peer->recv_badpackets, peer->recv_badroutes, ZEBRA_RIPNG_DISTANCE_DEFAULT, ripng_peer_uptime(peer, timebuf, RIPNG_UPTIME_LEN),
			VTY_NEWLINE);
	}
}
