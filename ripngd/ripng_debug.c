...
DEFUN(show_debugging_ripng, show_debugging_ripng_cmd, "show debugging ripng", SHOW_STR DEBUG_STR "RIPng configuration\n") {
	...
	if(IS_RIPNG_DEBUG_PACKET) {
		if(IS_RIPNG_DEBUG_SEND && IS_RIPNG_DEBUG_RECV) {
			vty_out(vty, "  RIPng packet debugging is on%s", VTY_NEWLINE);
		} else {
			if(IS_RIPNG_DEBUG_SEND) {
				vty_out(vty, "  RIPng packet send debugging is on%s", VTY_NEWLINE);
			} else {
				vty_out(vty, "  RIPng packet receive debugging is on%s", VTY_NEWLINE);
			}
		}
	}
...
DEFUN(debug_ripng_events, debug_ripng_events_cmd, "debug ripng events",
...
DEFUN(debug_ripng_packet, debug_ripng_packet_cmd, "debug ripng packet",
      DEBUG_STR "RIPng configuration\n"
...
DEFUN(debug_ripng_packet_direct, debug_ripng_packet_direct_cmd, "debug ripng packet (recv|send)",
      DEBUG_STR "RIPng configuration\n"
		"Debug option set for ripng packet\n"
...
DEFUN_DEPRECATED(
	debug_ripng_packet_detail, debug_ripng_packet_detail_cmd, "debug ripng packet (recv|send) detail",
	DEBUG_STR "RIPng configuration\n"
		  "Debug option set for ripng packet\n"
		  "Debug option set for receive packet\n"
		  "Debug option set for send packet\n"
...
DEFUN(debug_ripng_zebra, debug_ripng_zebra_cmd, "debug ripng zebra",
      DEBUG_STR "RIPng configuration\n"
...
DEFUN(no_debug_ripng_events, no_debug_ripng_events_cmd, "no debug ripng events",
      NO_STR DEBUG_STR "RIPng configuration\n"
		       "Debug option set for ripng events\n") {
	ripng_debug_event = 0;
	return CMD_SUCCESS;
}

DEFUN(no_debug_ripng_packet, no_debug_ripng_packet_cmd, "no debug ripng packet",
      NO_STR DEBUG_STR "RIPng configuration\n"
		       "Debug option set for ripng packet\n") {
...
DEFUN(no_debug_ripng_packet_direct, no_debug_ripng_packet_direct_cmd, "no debug ripng packet (recv|send)",
      NO_STR DEBUG_STR "RIPng configuration\n"
		       "Debug option set for ripng packet\n"
		       "Debug option set for receive packet\n"
		       "Debug option set for send packet\n") {
...
DEFUN(no_debug_ripng_zebra, no_debug_ripng_zebra_cmd, "no debug ripng zebra",
      NO_STR DEBUG_STR "RIPng configuration\n"
		       "Debug option set for ripng and zebra communication\n") {
	ripng_debug_zebra = 0;
	return CMD_WARNING;
...
static int config_write_debug(struct vty *vty) {
	...
	if(IS_RIPNG_DEBUG_PACKET) {
		if(IS_RIPNG_DEBUG_SEND && IS_RIPNG_DEBUG_RECV) {
			...
			if(IS_RIPNG_DEBUG_SEND) {
				vty_out(vty, "debug ripng packet send%s", VTY_NEWLINE);
			} else {
				vty_out(vty, "debug ripng packet recv%s", VTY_NEWLINE);
			}
			write++;
		}
	}
