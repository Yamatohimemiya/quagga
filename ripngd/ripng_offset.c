...
static int ripng_offset_list_unset(struct vty *vty, const char *alist, const char *direct_str, const char *metric_str, const char *ifname) {
	int direct;
	int metric;
	struct ripng_offset_list *offset;
	...
	if(offset) {
		...
		if(offset->direct[RIPNG_OFFSET_LIST_IN].alist_name == NULL && offset->direct[RIPNG_OFFSET_LIST_OUT].alist_name == NULL) {
...
int ripng_offset_list_apply_in(struct prefix_ipv6 *p, struct interface *ifp, u_char *metric) {
	struct ripng_offset_list *offset;
	struct access_list *alist;
...
/* If metric is modifed return 1. */
int ripng_offset_list_apply_out(struct prefix_ipv6 *p, struct interface *ifp, u_char *metric) {
	struct ripng_offset_list *offset;
	struct access_list *alist;
...
DEFUN(ripng_offset_list, ripng_offset_list_cmd, "offset-list WORD (in|out) <0-16>",
      "Modify RIPng metric\n"
      "Access-list name\n"
...
DEFUN(no_ripng_offset_list_ifname, no_ripng_offset_list_ifname_cmd, "no offset-list WORD (in|out) <0-16> IFNAME",
      NO_STR "Modify RIPng metric\n"
	     "Access-list name\n"
	     "For incoming updates\n"
	     "For outgoing updates\n"
...
static int offset_list_cmp(struct ripng_offset_list *o1, struct ripng_offset_list *o2) {
	...
	for(ALL_LIST_ELEMENTS(ripng_offset_list_master, node, nnode, offset)) {
		if(!offset->ifname) {
			if(offset->direct[RIPNG_OFFSET_LIST_IN].alist_name) {
				vty_out(vty, " offset-list %s in %d%s", offset->direct[RIPNG_OFFSET_LIST_IN].alist_name, offset->direct[RIPNG_OFFSET_LIST_IN].metric, VTY_NEWLINE);
			}
			if(offset->direct[RIPNG_OFFSET_LIST_OUT].alist_name) {
				vty_out(vty, " offset-list %s out %d%s", offset->direct[RIPNG_OFFSET_LIST_OUT].alist_name, offset->direct[RIPNG_OFFSET_LIST_OUT].metric, VTY_NEWLINE);
			}
		} else {
			if(offset->direct[RIPNG_OFFSET_LIST_IN].alist_name) {
				vty_out(vty, " offset-list %s in %d %s%s", offset->direct[RIPNG_OFFSET_LIST_IN].alist_name, offset->direct[RIPNG_OFFSET_LIST_IN].metric, offset->ifname, VTY_NEWLINE);
			}
			if(offset->direct[RIPNG_OFFSET_LIST_OUT].alist_name) {
				vty_out(vty, " offset-list %s out %d %s%s", offset->direct[RIPNG_OFFSET_LIST_OUT].alist_name, offset->direct[RIPNG_OFFSET_LIST_OUT].metric, offset->ifname, VTY_NEWLINE);
			}
		}
	}
