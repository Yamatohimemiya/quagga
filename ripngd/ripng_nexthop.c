...
#define METRIC_OUT(a) ((a)->rinfo ? (a)->rinfo->metric_out : (a)->aggregate->metric_out)
#define NEXTHOP_OUT_PTR(a) ((a)->rinfo ? &((a)->rinfo->nexthop_out) : &((a)->aggregate->nexthop_out))
...
void ripng_rte_add(struct list *ripng_rte_list, struct prefix_ipv6 *p, struct ripng_info *rinfo, struct ripng_aggregate *aggregate) {
 ...
 */
void ripng_rte_send(struct list *ripng_rte_list, struct interface *ifp, struct sockaddr_in6 *to) {
	struct ripng_rte_data *data;
   ...
   */
	...
	rtemax = (min(mtu, RIPNG_MAX_PACKET_SIZE) - IPV6_HDRLEN - sizeof(struct udphdr) - sizeof(struct ripng_packet) + sizeof(struct rte)) / sizeof(struct rte);
	...
	for(ALL_LIST_ELEMENTS(ripng_rte_list, node, nnode, data)) {
		...
		if(!IPV6_ADDR_SAME(&last_nexthop, NEXTHOP_OUT_PTR(data))) {
			...
			if(num == (rtemax - 1)) {
				ret = ripng_send_packet((caddr_t) STREAM_DATA(s), stream_get_endp(s), to, ifp);

				if(ret >= 0 && IS_RIPNG_DEBUG_SEND) {
					ripng_packet_dump((struct ripng_packet *) STREAM_DATA(s), stream_get_endp(s), "SEND");
				}
				num = 0;
				stream_reset(s);
			}
       ...
       */
		...
		num = ripng_write_rte(num, s, data->p, NULL, TAG_OUT(data), METRIC_OUT(data));
