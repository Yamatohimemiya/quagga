	...
	if(((oifp = if_lookup_by_index(ifi_index)) != NULL) && (oifp != ifp)) {
		if(ifi_index == IFINDEX_INTERNAL) {
			...
			if(IS_ZEBRA_DEBUG_KERNEL) {
				zlog_debug("interface index %d was renamed from %s to %s", ifi_index, oifp->name, ifp->name);
			}
			if(if_is_up(oifp)) {
				zlog_err(
...
static int netlink_recvbuf(struct nlsock *nl, uint32_t newsize) {
	...
	if(ret < 0) {
		zlog(NULL, LOG_ERR, "Can't get %s receive buffer size: %s", nl->name, safe_strerror(errno));
		return -1;
	}
	...
	ret = setsockopt(nl->sock, SOL_SOCKET, SO_RCVBUFFORCE, &nl_rcvbufsize, sizeof(nl_rcvbufsize));
	if(zserv_privs.change(ZPRIVS_LOWER)) {
		zlog_err("routing_socket: Can't lower privileges");
	}
	if(ret < 0) {
		ret = setsockopt(nl->sock, SOL_SOCKET, SO_RCVBUF, &nl_rcvbufsize, sizeof(nl_rcvbufsize));
	}
	if(ret < 0) {
		zlog(NULL, LOG_ERR, "Can't set %s receive buffer size: %s", nl->name, safe_strerror(errno));
		return -1;
	}
	...
	zlog(NULL, LOG_INFO, "Setting netlink socket receive buffer size: %u -> %u", oldsize, newsize);
	return 0;
}

/* Make socket for Linux netlink interface. */
static int netlink_socket(struct nlsock *nl, unsigned long groups, vrf_id_t vrf_id) {
	int ret;
	...
	if(sock < 0) {
		zlog(NULL, LOG_ERR, "Can't open %s socket: %s", nl->name, safe_strerror(errno));
		return -1;
	}
	...
	if(ret < 0) {
		zlog(NULL, LOG_ERR, "Can't bind %s socket to group 0x%x: %s", nl->name, snl.nl_groups, safe_strerror(save_errno));
		close(sock);
		return -1;
	}
	...
	if(ret < 0 || namelen != sizeof snl) {
		zlog(NULL, LOG_ERR, "Can't get %s socket name: %s", nl->name, safe_strerror(errno));
		close(sock);
		return -1;
	}
...
static int netlink_request(int family, int type, struct nlsock *nl) {
   ...
   */
	...
	if(ret < 0) {
		zlog(NULL, LOG_ERR, "%s sendto failed: %s", nl->name, safe_strerror(save_errno));
		return -1;
	}
...
static int netlink_parse_info(int (*filter)(struct sockaddr_nl *, struct nlmsghdr *, vrf_id_t), struct nlsock *nl, struct zebra_vrf *zvrf) {
	int status;
	int ret = 0;
	int error;
	...
	while(1) {
		...
		struct sockaddr_nl snl;
		struct msghdr msg = { .msg_name = (void *) &snl, .msg_namelen = sizeof snl, .msg_iov = &iov, .msg_iovlen = 1 };
		struct nlmsghdr *h;
		...
		if(status < 0) {
			...
			zlog(NULL, LOG_ERR, "%s recvmsg overrun: %s", nl->name, safe_strerror(errno));
			continue;
		}
		...
		if(msg.msg_namelen != sizeof snl) {
			zlog(NULL, LOG_ERR, "%s sender address length error: length %d", nl->name, msg.msg_namelen);
			return -1;
		}

		for(h = (struct nlmsghdr *) nl_rcvbuf.p; NLMSG_OK(h, (unsigned int) status); h = NLMSG_NEXT(h, status)) {
			/* Finish of reading. */
			...
			if(h->nlmsg_type == NLMSG_ERROR) {
				...
				if(err->error == 0) {
					if(IS_ZEBRA_DEBUG_KERNEL) {
						zlog_debug("%s: %s ACK: type=%s(%u), seq=%u, pid=%u", __FUNCTION__, nl->name, lookup(nlmsg_str, err->msg.nlmsg_type), err->msg.nlmsg_type, err->msg.nlmsg_seq, err->msg.nlmsg_pid);
					}
				...
				if(nl == &zvrf->netlink_cmd && ((msg_type == RTM_DELROUTE && (-errnum == ENODEV || -errnum == ESRCH)) || (msg_type == RTM_NEWROUTE && -errnum == EEXIST))) {
					if(IS_ZEBRA_DEBUG_KERNEL) {
						zlog_debug("%s: error: %s type=%s(%u), seq=%u, pid=%u", nl->name, safe_strerror(-errnum), lookup(nlmsg_str, msg_type), msg_type, err->msg.nlmsg_seq, err->msg.nlmsg_pid);
					}
					return 0;
				}

				zlog_err("%s error: %s, type=%s(%u), seq=%u, pid=%u", nl->name, safe_strerror(-errnum), lookup(nlmsg_str, msg_type), msg_type, err->msg.nlmsg_seq, err->msg.nlmsg_pid);
				return -1;
			}

			/* OK we got netlink message. */
			if(IS_ZEBRA_DEBUG_KERNEL) {
				zlog_debug("netlink_parse_info: %s type %s(%u), seq=%u, pid=%u", nl->name, lookup(nlmsg_str, h->nlmsg_type), h->nlmsg_type, h->nlmsg_seq, h->nlmsg_pid);
			}

			/* skip unsolicited messages originating from command socket
           * linux sets the originators port-id for {NEW|DEL}ADDR messages,
           * so this has to be checked here. */
			if(nl != &zvrf->netlink_cmd && h->nlmsg_pid == zvrf->netlink_cmd.snl.nl_pid && (h->nlmsg_type != RTM_NEWADDR && h->nlmsg_type != RTM_DELADDR)) {
				if(IS_ZEBRA_DEBUG_KERNEL) {
					zlog_debug("netlink_parse_info: %s packet comes from %s", zvrf->netlink_cmd.name, nl->name);
				}
				continue;
			}
		...
		if(msg.msg_flags & MSG_TRUNC) {
			zlog(NULL, LOG_ERR, "%s error: message truncated!", nl->name);
			zlog(NULL, LOG_ERR, "Must restart with larger --nl-bufsize value!");
			continue;
		}
		if(status) {
			zlog(NULL, LOG_ERR, "%s error: data remnant size %d", nl->name, status);
			return -1;
		}
	}
	return ret;
...
static void netlink_interface_update_hw_addr(struct rtattr **tb, struct interface *ifp) {
	int i;
...
/* Called from interface_lookup_netlink().  This function is only used
   during bootstrap. */
static int netlink_interface(struct sockaddr_nl *snl, struct nlmsghdr *h, vrf_id_t vrf_id) {
	int len;
...
static int netlink_interface_addr(struct sockaddr_nl *snl, struct nlmsghdr *h, vrf_id_t vrf_id) {
	int len;
	...
	if(ifp == NULL) {
		zlog_err("netlink_interface_addr can't find interface by index %d vrf %u", ifa->ifa_index, vrf_id);
		return -1;
	}
	...
	{
		char buf[BUFSIZ];
		zlog_debug("netlink_interface_addr %s %s vrf %u:", lookup(nlmsg_str, h->nlmsg_type), ifp->name, vrf_id);
		if(tb[IFA_LOCAL]) {
			zlog_debug("  IFA_LOCAL     %s/%d", inet_ntop(ifa->ifa_family, RTA_DATA(tb[IFA_LOCAL]), buf, BUFSIZ), ifa->ifa_prefixlen);
		}
		if(tb[IFA_ADDRESS]) {
			zlog_debug("  IFA_ADDRESS   %s/%d", inet_ntop(ifa->ifa_family, RTA_DATA(tb[IFA_ADDRESS]), buf, BUFSIZ), ifa->ifa_prefixlen);
		}
		if(tb[IFA_BROADCAST]) {
			zlog_debug("  IFA_BROADCAST %s/%d", inet_ntop(ifa->ifa_family, RTA_DATA(tb[IFA_BROADCAST]), buf, BUFSIZ), ifa->ifa_prefixlen);
		}
		...
		if(tb[IFA_CACHEINFO]) {
			...
			zlog_debug("  IFA_CACHEINFO pref %d, valid %d", ci->ifa_prefered, ci->ifa_valid);
		}
	}
	...
	if(tb[IFA_ADDRESS] && memcmp(RTA_DATA(tb[IFA_ADDRESS]), RTA_DATA(tb[IFA_LOCAL]), RTA_PAYLOAD(tb[IFA_ADDRESS]))) {
	...
	if(ifa->ifa_family == AF_INET) {
		if(h->nlmsg_type == RTM_NEWADDR) {
			connected_add_ipv4(ifp, flags, (struct in_addr *) addr, ifa->ifa_prefixlen, (struct in_addr *) broad, label);
		} else {
			connected_delete_ipv4(ifp, flags, (struct in_addr *) addr, ifa->ifa_prefixlen, (struct in_addr *) broad);
		}
	}
#ifdef HAVE_IPV6
	if(ifa->ifa_family == AF_INET6) {
		if(h->nlmsg_type == RTM_NEWADDR) {
			connected_add_ipv6(ifp, flags, (struct in6_addr *) addr, ifa->ifa_prefixlen, (struct in6_addr *) broad, label);
		} else {
			connected_delete_ipv6(ifp, (struct in6_addr *) addr, ifa->ifa_prefixlen, (struct in6_addr *) broad);
		}
	}
#endif /* HAVE_IPV6 */
...
static int netlink_routing_table(struct sockaddr_nl *snl, struct nlmsghdr *h, vrf_id_t vrf_id) {
	int len;
	struct rtmsg *rtm;
  ...
  if (table != RT_TABLE_MAIN && table != zebrad.rtm_table_default)
	...
	if(tb[RTA_METRICS]) {
		...
		netlink_parse_rtattr(mxrta, RTAX_MAX, RTA_DATA(tb[RTA_METRICS]), RTA_PAYLOAD(tb[RTA_METRICS]));
	...
	if(rtm->rtm_family == AF_INET) {
		...
		if(!tb[RTA_MULTIPATH]) {
			rib_add_ipv4(ZEBRA_ROUTE_KERNEL, flags, &p, gate, src, index, vrf_id, table, 0, mtu, 0, SAFI_UNICAST);
		} else {
			...
			for(;;) {
				...
				if(rtnh->rtnh_len > sizeof(*rtnh)) {
					memset(tb, 0, sizeof(tb));
					netlink_parse_rtattr(tb, RTA_MAX, RTNH_DATA(rtnh), rtnh->rtnh_len - sizeof(*rtnh));
					if(tb[RTA_GATEWAY]) {
						gate = RTA_DATA(tb[RTA_GATEWAY]);
					}
				}
	...
	if(rtm->rtm_family == AF_INET6) {
		...
		rib_add_ipv6(ZEBRA_ROUTE_KERNEL, flags, &p, gate, index, vrf_id, table, 0, mtu, 0, SAFI_UNICAST);
	}
#endif /* HAVE_IPV6 */
...
static int netlink_route_change(struct sockaddr_nl *snl, struct nlmsghdr *h, vrf_id_t vrf_id) {
	int len;
	struct rtmsg *rtm;
	struct rtattr *tb[RTA_MAX + 1];
	...
	if(IS_ZEBRA_DEBUG_KERNEL) {
		zlog_debug(
			"%s %s %s proto %s vrf %u", h->nlmsg_type == RTM_NEWROUTE ? "RTM_NEWROUTE" : "RTM_DELROUTE", rtm->rtm_family == AF_INET ? "ipv4" : "ipv6", rtm->rtm_type == RTN_UNICAST ? "unicast" : "multicast",
			lookup(rtproto_str, rtm->rtm_protocol), vrf_id
		);
	}
	...
	if(rtm->rtm_family == AF_INET) {
		...
		if(IS_ZEBRA_DEBUG_KERNEL) {
			...
			zlog_debug("%s %s vrf %u", h->nlmsg_type == RTM_NEWROUTE ? "RTM_NEWROUTE" : "RTM_DELROUTE", prefix2str(&p, buf, sizeof(buf)), vrf_id);
		}

		if(h->nlmsg_type == RTM_NEWROUTE) {
			if(!tb[RTA_MULTIPATH]) {
				rib_add_ipv4(ZEBRA_ROUTE_KERNEL, 0, &p, gate, src, index, vrf_id, table, 0, mtu, 0, SAFI_UNICAST);
			} else {
				/* This is a multipath route */
			...
			rib_delete_ipv4(ZEBRA_ROUTE_KERNEL, zebra_flags, &p, gate, index, vrf_id, SAFI_UNICAST);
		}
	}
	...
	if(rtm->rtm_family == AF_INET6) {
		...
		if(h->nlmsg_type == RTM_NEWROUTE) {
			rib_add_ipv6(ZEBRA_ROUTE_KERNEL, 0, &p, gate, index, vrf_id, table, 0, mtu, 0, SAFI_UNICAST);
		} else {
			rib_delete_ipv6(ZEBRA_ROUTE_KERNEL, zebra_flags, &p, gate, index, vrf_id, SAFI_UNICAST);
		}
	}
#endif /* HAVE_IPV6 */
...
static int netlink_link_change(struct sockaddr_nl *snl, struct nlmsghdr *h, vrf_id_t vrf_id) {
	int len;
	struct ifinfomsg *ifi;
	struct rtattr *tb[IFLA_MAX + 1];
	struct interface *ifp;
	...
	if((tb[IFLA_WIRELESS] != NULL) && (ifi->ifi_change == 0)) {
		if(IS_ZEBRA_DEBUG_KERNEL) {
			zlog_debug("%s: ignoring IFLA_WIRELESS message, vrf %u", __func__, vrf_id);
		}
		return 0;
	}
#endif /* IFLA_WIRELESS */
	...
	if(h->nlmsg_type == RTM_NEWLINK) {
		...
		if(ifp == NULL) {
			zlog_warn("interface %s vrf %u is deleted but can't find", name, vrf_id);
			return 0;
		}
...
static int netlink_information_fetch(struct sockaddr_nl *snl, struct nlmsghdr *h, vrf_id_t vrf_id) {
	...
	switch(h->nlmsg_type) {
		case RTM_NEWROUTE: return netlink_route_change(snl, h, vrf_id); break;
		case RTM_DELROUTE: return netlink_route_change(snl, h, vrf_id); break;
		case RTM_NEWLINK: return netlink_link_change(snl, h, vrf_id); break;
		case RTM_DELLINK: return netlink_link_change(snl, h, vrf_id); break;
		case RTM_NEWADDR: return netlink_interface_addr(snl, h, vrf_id); break;
		case RTM_DELADDR: return netlink_interface_addr(snl, h, vrf_id); break;
		default: zlog_warn("Unknown netlink nlmsg_type %d vrf %u\n", h->nlmsg_type, vrf_id); break;
	}
	return 0;
}

/* Interface lookup by netlink socket. */
int interface_lookup_netlink(struct zebra_vrf *zvrf) {
	int ret;
...
/* Routing table read function using netlink interface.  Only called
   bootstrap time. */
int netlink_route_read(struct zebra_vrf *zvrf) {
	int ret;
...
static int netlink_talk_filter(struct sockaddr_nl *snl, struct nlmsghdr *h, vrf_id_t vrf_id) {
	zlog_warn("netlink_talk: ignoring message type 0x%04x vrf %u", h->nlmsg_type, vrf_id);
	return 0;
}

/* sendmsg() to netlink socket then recvmsg(). */
static int netlink_talk(struct nlmsghdr *n, struct nlsock *nl, struct zebra_vrf *zvrf) {
	int status;
	struct sockaddr_nl snl;
	...
	if(IS_ZEBRA_DEBUG_KERNEL) {
		zlog_debug("netlink_talk: %s type %s(%u), seq=%u", nl->name, lookup(nlmsg_str, n->nlmsg_type), n->nlmsg_type, n->nlmsg_seq);
	}
	...
	if(status < 0) {
		zlog(NULL, LOG_ERR, "netlink_talk sendmsg() error: %s", safe_strerror(save_errno));
		return -1;
	}
 ...
 */
static void _netlink_route_build_singlepath(const char *routedesc, int bytelen, struct nexthop *nexthop, struct nlmsghdr *nlmsg, struct rtmsg *rtmsg, size_t req_size) {
	...
	if(nexthop->type == NEXTHOP_TYPE_IPV4 || nexthop->type == NEXTHOP_TYPE_IPV4_IFINDEX) {
		...
		if(nexthop->src.ipv4.s_addr) {
			addattr_l(nlmsg, req_size, RTA_PREFSRC, &nexthop->src.ipv4, bytelen);
		}
		...
		if(IS_ZEBRA_DEBUG_KERNEL) {
			zlog_debug(
				...
				"nexthop via %s if %u",
				routedesc, inet_ntoa(nexthop->gate.ipv4), nexthop->ifindex
			);
		}
	}
#ifdef HAVE_IPV6
	if(nexthop->type == NEXTHOP_TYPE_IPV6 || nexthop->type == NEXTHOP_TYPE_IPV6_IFNAME || nexthop->type == NEXTHOP_TYPE_IPV6_IFINDEX) {
		...
		if(IS_ZEBRA_DEBUG_KERNEL) {
			zlog_debug(
				"netlink_route_multipath() (%s): "
				"nexthop via %s if %u",
				routedesc, inet6_ntoa(nexthop->gate.ipv6), nexthop->ifindex
			);
		}
	}
#endif /* HAVE_IPV6 */
	if(nexthop->type == NEXTHOP_TYPE_IFINDEX || nexthop->type == NEXTHOP_TYPE_IFNAME || nexthop->type == NEXTHOP_TYPE_IPV4_IFINDEX) {
	...
	if(nexthop->type == NEXTHOP_TYPE_IPV6_IFINDEX || nexthop->type == NEXTHOP_TYPE_IPV6_IFNAME) {
		addattr32(nlmsg, req_size, RTA_OIF, nexthop->ifindex);
 ...
 */
static void _netlink_route_build_multipath(const char *routedesc, int bytelen, struct nexthop *nexthop, struct rtattr *rta, struct rtnexthop *rtnh, union g_addr **src) {
	...
	if(nexthop->type == NEXTHOP_TYPE_IPV4 || nexthop->type == NEXTHOP_TYPE_IPV4_IFINDEX) {
		rta_addattr_l(rta, NL_PKT_BUF_SIZE, RTA_GATEWAY, &nexthop->gate.ipv4, bytelen);
...
#ifdef HAVE_IPV6
	if(nexthop->type == NEXTHOP_TYPE_IPV6 || nexthop->type == NEXTHOP_TYPE_IPV6_IFNAME || nexthop->type == NEXTHOP_TYPE_IPV6_IFINDEX) {
		rta_addattr_l(rta, NL_PKT_BUF_SIZE, RTA_GATEWAY, &nexthop->gate.ipv6, bytelen);
		rtnh->rtnh_len += sizeof(struct rtattr) + bytelen;
...
#endif /* HAVE_IPV6 */
	/* ifindex */
	if(nexthop->type == NEXTHOP_TYPE_IPV4_IFINDEX || nexthop->type == NEXTHOP_TYPE_IFINDEX || nexthop->type == NEXTHOP_TYPE_IFNAME) {
	...
	} else if(nexthop->type == NEXTHOP_TYPE_IPV6_IFNAME || nexthop->type == NEXTHOP_TYPE_IPV6_IFINDEX) {
		rtnh->rtnh_ifindex = nexthop->ifindex;
 ...
 */
static void _netlink_route_debug(int cmd, struct prefix *p, struct nexthop *nexthop, const char *routedesc, int family, struct zebra_vrf *zvrf) {
	if(IS_ZEBRA_DEBUG_KERNEL) {
		char buf[PREFIX_STRLEN];
		zlog_debug("netlink_route_multipath() (%s): %s %s vrf %u type %s", routedesc, lookup(nlmsg_str, cmd), prefix2str(p, buf, sizeof(buf)), zvrf->vrf_id, nexthop_type_to_str(nexthop->type));
	}
}
...
static int netlink_route_multipath(int cmd, struct prefix *p, struct rib *rib) {
	...
	if(rib->mtu || rib->nexthop_mtu) {
		...
		addattr_l(&req.n, NL_PKT_BUF_SIZE, RTA_METRICS, RTA_DATA(rta), RTA_PAYLOAD(rta));
	}
   ...
   * or multipath case. */
	...
	if(nexthop_num == 1 || MULTIPATH_NUM == 1) {
		...
		for(ALL_NEXTHOPS_RO(rib->nexthop, nexthop, tnexthop, recursing)) {
			...
			if((cmd == RTM_NEWROUTE && CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_ACTIVE)) || (cmd == RTM_DELROUTE && CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_FIB))) {
...
static int netlink_address(int cmd, int family, struct interface *ifp, struct connected *ifc) {
	int bytelen;
	struct prefix *p;
	...
	if(ifc->label) {
		addattr_l(&req.n, sizeof req, IFA_LABEL, ifc->label, strlen(ifc->label) + 1);
	}
...
static int kernel_read(struct thread *thread) {
	...
	zvrf->t_netlink = thread_add_read(zebrad.master, kernel_read, zvrf, zvrf->netlink.sock);
...
void kernel_init(struct zebra_vrf *zvrf) {
	...
	if(zvrf->netlink.sock > 0) {
		...
		if(fcntl(zvrf->netlink.sock, F_SETFL, O_NONBLOCK) < 0) {
			zlog_err("Can't set %s socket flags: %s", zvrf->netlink.name, safe_strerror(errno));
		}
