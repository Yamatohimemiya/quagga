 ...
 */
#define RIB_DEST_FOREACH_ROUTE(dest, rib) for((rib) = (dest) ? (dest)->routes : NULL; (rib); (rib) = (rib)->next)

/*
 * Same as above, but allows the current node to be unlinked.
 */
#define RIB_DEST_FOREACH_ROUTE_SAFE(dest, rib, next) for((rib) = (dest) ? (dest)->routes : NULL; (rib) && ((next) = (rib)->next, 1); (rib) = (next))

#define RNODE_FOREACH_RIB(rn, rib) RIB_DEST_FOREACH_ROUTE(rib_dest_from_rnode(rn), rib)

#define RNODE_FOREACH_RIB_SAFE(rn, rib, next) RIB_DEST_FOREACH_ROUTE_SAFE(rib_dest_from_rnode(rn), rib, next)
...
#define ALL_NEXTHOPS_RO(head, nexthop, tnexthop, recursing) \
	...
	(nexthop); \
	(nexthop) = CHECK_FLAG((nexthop)->flags, NEXTHOP_FLAG_RECURSIVE) ? (((recursing) = 1), (nexthop)->resolved) \
									 : ((nexthop)->next ? ((recursing) ? (nexthop)->next : ((tnexthop) = (nexthop)->next)) : (((recursing) = 0), ((tnexthop) = (tnexthop)->next)))
...
extern struct nexthop *rib_nexthop_ipv4_add(struct rib *, struct in_addr *, struct in_addr *);
extern struct nexthop *rib_nexthop_ipv4_ifindex_add(struct rib *, struct in_addr *, struct in_addr *, ifindex_t);
...
extern void _rib_dump(const char *, union prefix46constptr, const struct rib *);
extern int rib_lookup_ipv4_route(struct prefix_ipv4 *, union sockunion *, vrf_id_t);
...
extern struct nexthop *rib_nexthop_ipv6_ifindex_add(struct rib *, struct in6_addr *, ifindex_t);
...
extern int rib_add_ipv4(int type, int flags, struct prefix_ipv4 *p, struct in_addr *gate, struct in_addr *src, ifindex_t ifindex, vrf_id_t vrf_id, int table_id, u_int32_t, u_int32_t, u_char, safi_t);

extern int rib_add_ipv4_multipath(struct prefix_ipv4 *, struct rib *, safi_t);

extern int rib_delete_ipv4(int type, int flags, struct prefix_ipv4 *p, struct in_addr *gate, ifindex_t ifindex, vrf_id_t, safi_t safi);

extern struct rib *rib_match_ipv4_safi(struct in_addr addr, safi_t safi, int skip_bgp, struct route_node **rn_out, vrf_id_t);
extern struct rib *rib_match_ipv4_multicast(struct in_addr addr, struct route_node **rn_out, vrf_id_t);
...
extern int static_add_ipv4_safi(safi_t safi, struct prefix *p, struct in_addr *gate, const char *ifname, u_char flags, route_tag_t, u_char distance, vrf_id_t vrf_id);
extern int static_delete_ipv4_safi(safi_t safi, struct prefix *p, struct in_addr *gate, const char *ifname, route_tag_t tag, u_char distance, vrf_id_t vrf_id);

extern int rib_add_ipv6(int type, int flags, struct prefix_ipv6 *p, struct in6_addr *gate, ifindex_t ifindex, vrf_id_t vrf_id, int table_id, u_int32_t metric, u_int32_t mtu, u_char distance, safi_t safi);

extern int rib_delete_ipv6(int type, int flags, struct prefix_ipv6 *p, struct in6_addr *gate, ifindex_t ifindex, vrf_id_t vrf_id, safi_t safi);
...
extern int static_add_ipv6(struct prefix *p, u_char type, struct in6_addr *gate, const char *ifname, u_char flags, route_tag_t, u_char distance, vrf_id_t vrf_id);

extern int rib_add_ipv6_multipath(struct prefix_ipv6 *, struct rib *, safi_t);

extern int static_delete_ipv6(struct prefix *p, u_char type, struct in6_addr *gate, const char *ifname, route_tag_t, u_char distance, vrf_id_t vrf_id);
