...
static const char *inet_2a(u_int32_t a, char *b) {
	sprintf(b, "%u.%u.%u.%u", (a) & 0xFF, (a >> 8) & 0xFF, (a >> 16) & 0xFF, (a >> 24) & 0xFF);
	return b;
}
...
static int if_group(struct interface *ifp, int sock, u_int32_t group, int add_leave) {
	struct ip_mreq m;
	struct prefix *p;
	int ret;
	...
	ret = setsockopt(sock, IPPROTO_IP, add_leave, (char *) &m, sizeof(struct ip_mreq));
	if(ret < 0) {
		zlog_warn("IRDP: %s can't setsockopt %s: %s", add_leave == IP_ADD_MEMBERSHIP ? "join group" : "leave group", inet_2a(group, b1), safe_strerror(errno));
	}
...
static int if_add_group(struct interface *ifp) {
	...
	if(irdp->flags & IF_DEBUG_MISC) {
		zlog_debug("IRDP: Adding group %s for %s", inet_2a(htonl(INADDR_ALLRTRS_GROUP), b1), ifp->name);
	}
	return 0;
}
...
static int if_drop_group(struct interface *ifp) {
	...
	if(irdp->flags & IF_DEBUG_MISC) {
		zlog_debug("IRDP: Leaving group %s for %s", inet_2a(htonl(INADDR_ALLRTRS_GROUP), b1), ifp->name);
	}
	return 0;
}
...
static void irdp_if_start(struct interface *ifp, int multicast, int set_defaults) {
	...
	if(irdp->irdp_sent < MAX_INITIAL_ADVERTISEMENTS && timer > MAX_INITIAL_ADVERT_INTERVAL) {
...
void irdp_config_write(struct vty *vty, struct interface *ifp) {
	...
	if(irdp->flags & IF_ACTIVE || irdp->flags & IF_SHUTDOWN) {
		...
		vty_out(vty, " ip irdp preference %ld%s", irdp->Preference, VTY_NEWLINE);

		for(ALL_LIST_ELEMENTS_RO(irdp->AdvPrefList, node, adv)) {
			vty_out(vty, " ip irdp address %s preference %d%s", inet_2a(adv->ip.s_addr, b1), adv->pref, VTY_NEWLINE);
		}

		vty_out(vty, " ip irdp holdtime %d%s", irdp->Lifetime, VTY_NEWLINE);

		vty_out(vty, " ip irdp minadvertinterval %ld%s", irdp->MinAdvertInterval, VTY_NEWLINE);

		vty_out(vty, " ip irdp maxadvertinterval %ld%s", irdp->MaxAdvertInterval, VTY_NEWLINE);
	}
}

DEFUN(ip_irdp_multicast, ip_irdp_multicast_cmd, "ip irdp multicast", IP_STR "ICMP Router discovery on this interface using multicast\n") {
...
DEFUN(no_ip_irdp, no_ip_irdp_cmd, "no ip irdp", NO_STR IP_STR "Disable ICMP Router discovery on this interface\n") {
	struct interface *ifp;
...
DEFUN(ip_irdp_shutdown, ip_irdp_shutdown_cmd, "ip irdp shutdown", IP_STR "ICMP Router discovery shutdown on this interface\n") {
	struct interface *ifp;
...
DEFUN(no_ip_irdp_shutdown, no_ip_irdp_shutdown_cmd, "no ip irdp shutdown", NO_STR IP_STR "ICMP Router discovery no shutdown on this interface\n") {
	struct interface *ifp;
...
DEFUN(ip_irdp_minadvertinterval, ip_irdp_minadvertinterval_cmd, "ip irdp minadvertinterval <3-1800>",
      IP_STR "ICMP Router discovery on this interface\n"
	...
	vty_out(vty, "ICMP warning maxadvertinterval is greater or equal than minadvertinterval%s", VTY_NEWLINE);

	vty_out(vty, "Please correct!%s", VTY_NEWLINE);
	return CMD_WARNING;
}

DEFUN(ip_irdp_maxadvertinterval, ip_irdp_maxadvertinterval_cmd, "ip irdp maxadvertinterval <4-1800>",
      IP_STR "ICMP Router discovery on this interface\n"
...
DEFUN(ip_irdp_preference, ip_irdp_preference_cmd, "ip irdp preference <0-2147483647>",
      IP_STR "ICMP Router discovery on this interface\n"
...
DEFUN(ip_irdp_address_preference, ip_irdp_address_preference_cmd, "ip irdp address A.B.C.D preference <0-2147483647>",
      IP_STR "Alter ICMP Router discovery preference this interface\n"
...
DEFUN(no_ip_irdp_address_preference, no_ip_irdp_address_preference_cmd, "no ip irdp address A.B.C.D preference <0-2147483647>",
      NO_STR IP_STR "Alter ICMP Router discovery preference this interface\n"
...
DEFUN(ip_irdp_debug_messages, ip_irdp_debug_messages_cmd, "ip irdp debug messages", IP_STR "ICMP Router discovery debug Averts. and Solicits (short)\n") {
	struct interface *ifp;
	struct zebra_if *zi;
	struct irdp_interface *irdp;
	ifp = (struct interface *) vty->index;
...
DEFUN(ip_irdp_debug_misc, ip_irdp_debug_misc_cmd, "ip irdp debug misc", IP_STR "ICMP Router discovery debug Averts. and Solicits (short)\n") {
	struct interface *ifp;
	struct zebra_if *zi;
	struct irdp_interface *irdp;
	ifp = (struct interface *) vty->index;
...
DEFUN(ip_irdp_debug_packet, ip_irdp_debug_packet_cmd, "ip irdp debug packet", IP_STR "ICMP Router discovery debug Averts. and Solicits (short)\n") {
	struct interface *ifp;
	struct zebra_if *zi;
	struct irdp_interface *irdp;
	ifp = (struct interface *) vty->index;
...
DEFUN(ip_irdp_debug_disable, ip_irdp_debug_disable_cmd, "ip irdp debug disable", IP_STR "ICMP Router discovery debug Averts. and Solicits (short)\n") {
	struct interface *ifp;
	struct zebra_if *zi;
	struct irdp_interface *irdp;
	ifp = (struct interface *) vty->index;
