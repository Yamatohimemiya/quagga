 ...
 */
		...
		#define ROUNDUP(a) ((a) > 0 ? (1 + (((a) -1) | (sizeof(ROUNDUP_TYPE) - 1))) : sizeof(ROUNDUP_TYPE))
 ...
 */
		#define SAROUNDUP(X) \
			(((struct sockaddr *) (X))->sa_family == AF_INET \
				 ...
				 : (((struct sockaddr *) (X))->sa_family == AF_INET6 ? ROUNDUP(sizeof(struct sockaddr_in6)) \
										     : (((struct sockaddr *) (X))->sa_family == AF_LINK ? ROUNDUP(sizeof(struct sockaddr_dl)) : sizeof(struct sockaddr))))
	#else /* HAVE_IPV6 */
		#define SAROUNDUP(X) (((struct sockaddr *) (X))->sa_family == AF_INET ? ROUNDUP(sizeof(struct sockaddr_in)) : (((struct sockaddr *) (X))->sa_family == AF_LINK ? ROUNDUP(sizeof(struct sockaddr_dl)) : sizeof(struct sockaddr)))
...
static inline void rta_copy(union sockunion *dest, caddr_t src) {
	...
	len = (((struct sockaddr *) src)->sa_len > sizeof(*dest)) ? sizeof(*dest) : ((struct sockaddr *) src)->sa_len;
#else
...
#define RTA_ADDR_GET(DEST, RTA, RTMADDRS, PNT) \
	if((RTMADDRS) & (RTA)) { \
		...
		if(af_check(((struct sockaddr *) (PNT))->sa_family)) rta_copy((DEST), (PNT)); \
		(PNT) += len; \
	}
...
#define RTA_NAME_GET(DEST, RTA, RTMADDRS, PNT, LEN) \
	if((RTMADDRS) & (RTA)) { \
		...
		if(IS_ZEBRA_DEBUG_KERNEL) zlog_debug("%s: RTA_SDL_GET nlen %d, alen %d", __func__, sdl->sdl_nlen, sdl->sdl_alen); \
		if(((DEST) != NULL) && (sdl->sdl_family == AF_LINK) && (sdl->sdl_nlen < IFNAMSIZ) && (sdl->sdl_nlen <= len)) { \
...
static int ifan_read(struct if_announcemsghdr *ifan) {
	...
	if(ifp) {
		assert((ifp->ifindex == ifan->ifan_index) || (ifp->ifindex == IFINDEX_INTERNAL));
	}

	if((ifp == NULL) || ((ifp->ifindex == IFINDEX_INTERNAL) && (ifan->ifan_what == IFAN_ARRIVAL))) {
		if(IS_ZEBRA_DEBUG_KERNEL) {
			zlog_debug("%s: creating interface for ifindex %d, name %s", __func__, ifan->ifan_index, ifan->ifan_name);
		}

		/* Create Interface */
		ifp = if_get_by_name_len(ifan->ifan_name, strnlen(ifan->ifan_name, sizeof(ifan->ifan_name)));
	...
	if(IS_ZEBRA_DEBUG_KERNEL) {
		zlog_debug("%s: interface %s index %d", __func__, ifan->ifan_name, ifan->ifan_index);
	}
...
static void bsd_linkdetect_translate(struct if_msghdr *ifm) {
	if((ifm->ifm_data.ifi_link_state >= LINK_STATE_UP) || (ifm->ifm_data.ifi_link_state == LINK_STATE_UNKNOWN)) {
...
int ifm_read(struct if_msghdr *ifm) {
	...
	if((ifp = if_lookup_by_index(ifm->ifm_index)) != NULL) {
       ...
       */
		if(ifnlen && (strncmp(ifp->name, ifname, IFNAMSIZ) != 0)) {
			if(IS_ZEBRA_DEBUG_KERNEL) {
				zlog_debug("%s: ifp name %s doesn't match sdl name %s", __func__, ifp->name, ifname);
			}
			ifp = NULL;
		}
	}
   ...
   */
	if((ifp == NULL) || (ifp->ifindex == IFINDEX_INTERNAL)) {
       ...
       */
		...
		if(ifp == NULL) {
			...
			if(IS_ZEBRA_DEBUG_KERNEL) {
				zlog_debug("%s: creating ifp for ifindex %d", __func__, ifm->ifm_index);
			}
		}
	...
	if(IS_ZEBRA_DEBUG_KERNEL) {
		zlog_debug("%s: interface %s index %d", __func__, ifp->name, ifp->ifindex);
	}
...
static void ifam_read_mesg(struct ifa_msghdr *ifm, union sockunion *addr, union sockunion *mask, union sockunion *brd, char *ifname, short *ifnlen) {
	caddr_t pnt, end;
	union sockunion dst;
			...
			case AF_INET6:
#endif
				{
					...
					zlog_debug(
						...
						"gateway %s",
						__func__, ifm->ifam_index, (ifnlen ? ifname : "(nil)"), ifm->ifam_addrs, ifm->ifam_flags, inet_ntop(family, &addr->sin.sin_addr, buf[0], sizeof(buf[0])), ip_masklen(mask->sin.sin_addr),
						inet_ntop(family, &brd->sin.sin_addr, buf[1], sizeof(buf[1])), inet_ntop(family, &dst.sin.sin_addr, buf[2], sizeof(buf[2])), inet_ntop(family, &gateway.sin.sin_addr, buf[3], sizeof(buf[3]))
					);
				}
				break;
			default: zlog_debug("%s: ifindex %d, ifname %s, ifam_addrs 0x%x", __func__, ifm->ifam_index, (ifnlen ? ifname : "(nil)"), ifm->ifam_addrs); break;
		}
	}
...
int ifam_read(struct ifa_msghdr *ifam) {
	...
	if((ifp = if_lookup_by_index(ifam->ifam_index)) == NULL) {
		zlog_warn("%s: no interface for ifname %s, index %d", __func__, ifname, ifam->ifam_index);
		return -1;
	}
	...
	switch(sockunion_family(&addr)) {
		case AF_INET:
			if(ifam->ifam_type == RTM_NEWADDR) {
				connected_add_ipv4(ifp, flags, &addr.sin.sin_addr, ip_masklen(mask.sin.sin_addr), &brd.sin.sin_addr, (isalias ? ifname : NULL));
			} else {
				connected_delete_ipv4(ifp, flags, &addr.sin.sin_addr, ip_masklen(mask.sin.sin_addr), &brd.sin.sin_addr);
			}
			break;
#ifdef HAVE_IPV6
		case AF_INET6:
	 ...
	 is KAME. */
			...
			if(ifam->ifam_type == RTM_NEWADDR) {
				connected_add_ipv6(ifp, flags, &addr.sin6.sin6_addr, ip6_masklen(mask.sin6.sin6_addr), &brd.sin6.sin6_addr, (isalias ? ifname : NULL));
			} else {
				connected_delete_ipv6(ifp, &addr.sin6.sin6_addr, ip6_masklen(mask.sin6.sin6_addr), &brd.sin6.sin6_addr);
			}
			break;
#endif /* HAVE_IPV6 */
		default:
...
static int rtm_read_mesg(struct rt_msghdr *rtm, union sockunion *dest, union sockunion *mask, union sockunion *gate, char *ifname, short *ifnlen) {
	caddr_t pnt, end;
...
void rtm_read(struct rt_msghdr *rtm) {
	...
	if(IS_ZEBRA_DEBUG_KERNEL) {
		zlog_debug("%s: got rtm of type %d (%s)", __func__, rtm->rtm_type, lookup(rtm_type_str, rtm->rtm_type));
	}
	...
	if(dest.sa.sa_family == AF_INET) {
		...
		if(rtm->rtm_type != RTM_GET && rtm->rtm_pid == pid) {
			...
			switch(rtm->rtm_type) {
               ...
               Do we have a correspondent entry in our RIB? */
					switch(ret) {
						case ZEBRA_RIB_NOTFOUND: zlog_debug("%s: %s %s: desync: RR isn't yet in RIB, while already in FIB", __func__, lookup(rtm_type_str, rtm->rtm_type), buf); break;
						case ZEBRA_RIB_FOUND_CONNECTED:
						case ZEBRA_RIB_FOUND_NOGATE:
							inet_ntop(AF_INET, &gate.sin.sin_addr, gate_buf, INET_ADDRSTRLEN);
							zlog_debug("%s: %s %s: desync: RR is in RIB, but gate differs (ours is %s)", __func__, lookup(rtm_type_str, rtm->rtm_type), buf, gate_buf);
							break;
						case ZEBRA_RIB_FOUND_EXACT: /* RIB RR == FIB RR */
							zlog_debug("%s: %s %s: done Ok", __func__, lookup(rtm_type_str, rtm->rtm_type), buf);
               ...
               have it in the RIB? Do we have anything instead? */
					switch(ret) {
						case ZEBRA_RIB_FOUND_EXACT:
							zlog_debug("%s: %s %s: desync: RR is still in RIB, while already not in FIB", __func__, lookup(rtm_type_str, rtm->rtm_type), buf);
							rib_lookup_and_dump(&p);
							break;
						case ZEBRA_RIB_FOUND_CONNECTED:
						case ZEBRA_RIB_FOUND_NOGATE:
							zlog_debug("%s: %s %s: desync: RR is still in RIB, plus gate differs", __func__, lookup(rtm_type_str, rtm->rtm_type), buf);
							rib_lookup_and_dump(&p);
							break;
					...
					break;
				default: zlog_debug("%s: %s: warning: loopback RTM of type %s received", __func__, buf, lookup(rtm_type_str, rtm->rtm_type));
			}
			return;
		}
       ...
       */
		if(rtm->rtm_type == RTM_CHANGE) {
			rib_delete_ipv4(ZEBRA_ROUTE_KERNEL, zebra_flags, &p, NULL, 0, VRF_DEFAULT, SAFI_UNICAST);
		}

		if(rtm->rtm_type == RTM_GET || rtm->rtm_type == RTM_ADD || rtm->rtm_type == RTM_CHANGE) {
			rib_add_ipv4(ZEBRA_ROUTE_KERNEL, zebra_flags, &p, &gate.sin.sin_addr, NULL, 0, VRF_DEFAULT, 0, 0, 0, 0, SAFI_UNICAST);
		} else {
			rib_delete_ipv4(ZEBRA_ROUTE_KERNEL, zebra_flags, &p, &gate.sin.sin_addr, 0, VRF_DEFAULT, SAFI_UNICAST);
		}
	}
#ifdef HAVE_IPV6
	if(dest.sa.sa_family == AF_INET6) {
       ...
       */
		if(rtm->rtm_type == RTM_CHANGE) {
			rib_delete_ipv6(ZEBRA_ROUTE_KERNEL, zebra_flags, &p, NULL, 0, VRF_DEFAULT, SAFI_UNICAST);
		}

		if(rtm->rtm_type == RTM_GET || rtm->rtm_type == RTM_ADD || rtm->rtm_type == RTM_CHANGE) {
			rib_add_ipv6(ZEBRA_ROUTE_KERNEL, zebra_flags, &p, &gate.sin6.sin6_addr, ifindex, VRF_DEFAULT, RT_TABLE_MAIN, 0, 0, 0, SAFI_UNICAST);
		} else {
			rib_delete_ipv6(ZEBRA_ROUTE_KERNEL, zebra_flags, &p, &gate.sin6.sin6_addr, ifindex, VRF_DEFAULT, SAFI_UNICAST);
		}
	}
#endif /* HAVE_IPV6 */
}
 ...
 */
int rtm_write(int message, union sockunion *dest, union sockunion *mask, union sockunion *gate, unsigned int index, int zebra_flags, int metric) {
	int ret;
	caddr_t pnt;
	struct interface *ifp;
   ...
   */
#ifdef RTF_CLONING
	if(!gate && (message == RTM_ADD || message == RTM_CHANGE) && ifp && (ifp->flags & IFF_POINTOPOINT) == 0) {
	...
	if(!gate) {
		if(!ifp) {
			...
			zlog_warn("%s: %s/%s: gate == NULL and no gateway found for ifindex %d", __func__, dest_buf, mask_buf, index);
			return -1;
		}
...
static void rtmsg_debug(struct rt_msghdr *rtm) {
	...
	zlog_debug("Kernel: pid %lld, rtm_addrs 0x%x", (long long) rtm->rtm_pid, rtm->rtm_addrs);
}
...
static int kernel_read(struct thread *thread) {
   ...
   */
	...
	switch(rtm->rtm_type) {
		case RTM_ADD:
		case RTM_DELETE:
		case RTM_CHANGE: rtm_read(rtm); break;
		case RTM_IFINFO: ifm_read(&buf.im.ifm); break;
		case RTM_NEWADDR:
		case RTM_DELADDR: ifam_read(&buf.ia.ifa); break;
#ifdef RTM_IFANNOUNCE
		case RTM_IFANNOUNCE: ifan_read(&buf.ian.ifan); break;
#endif /* RTM_IFANNOUNCE */
		default:
			if(IS_ZEBRA_DEBUG_KERNEL) {
...
/* Make routing socket. */
static void routing_socket(struct zebra_vrf *zvrf) {
