...
extern int rtm_write(int message, union sockunion *dest, union sockunion *mask, union sockunion *gate, unsigned int index, int zebra_flags, int metric);
...
static int kernel_rtm_ipv4(int cmd, struct prefix *p, struct rib *rib)

{
	...
	for(ALL_NEXTHOPS_RO(rib->nexthop, nexthop, tnexthop, recursing)) {
       ...
       */
		if((cmd == RTM_ADD && CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_ACTIVE)) || (cmd == RTM_DELETE && CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_FIB))) {
			...
			error = rtm_write(cmd, (union sockunion *) &sin_dest, (union sockunion *) mask, gate ? (union sockunion *) &sin_gate : NULL, ifindex, rib->flags, rib->metric);

			if(IS_ZEBRA_DEBUG_RIB) {
				if(!gate) {
					zlog_debug("%s: %s: attention! gate not found for rib %p", __func__, prefix_buf, rib);
					rib_dump(p, rib);
				} else {
			...
			switch(error) {
				...
				case ZEBRA_ERR_NOERROR:
					nexthop_num++;
					if(IS_ZEBRA_DEBUG_RIB) {
						zlog_debug("%s: %s: successfully did NH %s", __func__, prefix_buf, gate_buf);
					}
					if(cmd == RTM_ADD) {
              ...
              */
				case ZEBRA_ERR_RTEXIST:
					if(cmd != RTM_ADD) {
						zlog_err("%s: rtm_write() returned %d for command %d", __func__, error, cmd);
					}
					continue;
					break;
              ...
              */
				...
				default: zlog_err("%s: %s: rtm_write() unexpectedly returned %d for command %s", __func__, prefix2str(p, prefix_buf, sizeof(prefix_buf)), error, lookup(rtm_type_str, cmd)); break;
			}
		} /* if (cmd and flags make sense) */
		else if(IS_ZEBRA_DEBUG_RIB) {
			zlog_debug("%s: odd command %s for flags %d", __func__, lookup(rtm_type_str, cmd), nexthop->flags);
		}
	} /* for (ALL_NEXTHOPS_RO(...))*/
...
/* Interface between zebra message and rtm message. */
static int kernel_rtm_ipv6(int cmd, struct prefix *p, struct rib *rib) {
	...
	for(ALL_NEXTHOPS_RO(rib->nexthop, nexthop, tnexthop, recursing)) {
		...
		if((cmd == RTM_ADD && CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_ACTIVE))
		   || (cmd == RTM_DELETE
	#if 0
	      && CHECK_FLAG (nexthop->flags, NEXTHOP_FLAG_FIB)
			...
			if(nexthop->type == NEXTHOP_TYPE_IFINDEX || nexthop->type == NEXTHOP_TYPE_IFNAME || nexthop->type == NEXTHOP_TYPE_IPV6_IFNAME || nexthop->type == NEXTHOP_TYPE_IPV6_IFINDEX) {
				ifindex = nexthop->ifindex;
			}
...
static int kernel_rtm(int cmd, struct prefix *p, struct rib *rib) {
	switch(PREFIX_FAMILY(p)) {
		case AF_INET: return kernel_rtm_ipv4(cmd, p, rib);
		case AF_INET6: return kernel_rtm_ipv6(cmd, p, rib);
	}
	return 0;
}
