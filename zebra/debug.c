...
DEFUN(show_debugging_zebra, show_debugging_zebra_cmd, "show debugging zebra",
	...
	if(IS_ZEBRA_DEBUG_PACKET) {
		if(IS_ZEBRA_DEBUG_SEND && IS_ZEBRA_DEBUG_RECV) {
			vty_out(vty, "  Zebra packet%s debugging is on%s", IS_ZEBRA_DEBUG_DETAIL ? " detail" : "", VTY_NEWLINE);
		} else {
			if(IS_ZEBRA_DEBUG_SEND) {
				vty_out(vty, "  Zebra packet send%s debugging is on%s", IS_ZEBRA_DEBUG_DETAIL ? " detail" : "", VTY_NEWLINE);
			} else {
				vty_out(vty, "  Zebra packet receive%s debugging is on%s", IS_ZEBRA_DEBUG_DETAIL ? " detail" : "", VTY_NEWLINE);
			}
		}
	}
...
DEFUN(debug_zebra_nht, debug_zebra_nht_cmd, "debug zebra nht",
      DEBUG_STR "Zebra configuration\n"
...
DEFUN(debug_zebra_packet, debug_zebra_packet_cmd, "debug zebra packet",
      DEBUG_STR "Zebra configuration\n"
...
DEFUN(debug_zebra_packet_direct, debug_zebra_packet_direct_cmd, "debug zebra packet (recv|send|detail)",
      DEBUG_STR "Zebra configuration\n"
		"Debug option set for zebra packet\n"
...
DEFUN(debug_zebra_packet_detail, debug_zebra_packet_detail_cmd, "debug zebra packet (recv|send) detail",
      DEBUG_STR "Zebra configuration\n"
		"Debug option set for zebra packet\n"
		"Debug option set for receive packet\n"
		"Debug option set for send packet\n"
...
DEFUN(debug_zebra_rib, debug_zebra_rib_cmd, "debug zebra rib",
      DEBUG_STR "Zebra configuration\n"
		"Debug RIB events\n") {
...
DEFUN(debug_zebra_rib_q, debug_zebra_rib_q_cmd, "debug zebra rib queue",
      DEBUG_STR "Zebra configuration\n"
		"Debug RIB events\n"
		"Debug RIB queueing\n") {
...
DEFUN(no_debug_zebra_nht, no_debug_zebra_nht_cmd, "no debug zebra nht",
      NO_STR DEBUG_STR "Zebra configuration\n"
		       "Debug option set for zebra next hop tracking\n") {
	zebra_debug_nht = 0;
	return CMD_SUCCESS;
...
DEFUN(no_debug_zebra_packet, no_debug_zebra_packet_cmd, "no debug zebra packet",
      NO_STR DEBUG_STR "Zebra configuration\n"
		       "Debug option set for zebra packet\n") {
...
DEFUN(no_debug_zebra_packet_direct, no_debug_zebra_packet_direct_cmd, "no debug zebra packet (recv|send)",
      NO_STR DEBUG_STR "Zebra configuration\n"
		       "Debug option set for zebra packet\n"
		       "Debug option set for receive packet\n"
		       "Debug option set for send packet\n") {
...
DEFUN(no_debug_zebra_rib, no_debug_zebra_rib_cmd, "no debug zebra rib",
      NO_STR DEBUG_STR "Zebra configuration\n"
		       "Debug zebra RIB\n") {
	zebra_debug_rib = 0;
	return CMD_SUCCESS;
...
DEFUN(no_debug_zebra_rib_q, no_debug_zebra_rib_q_cmd, "no debug zebra rib queue",
      NO_STR DEBUG_STR "Zebra configuration\n"
		       "Debug zebra RIB\n"
		       "Debug RIB queueing\n") {
...
static int config_write_debug(struct vty *vty) {
	...
	if(IS_ZEBRA_DEBUG_PACKET) {
		if(IS_ZEBRA_DEBUG_SEND && IS_ZEBRA_DEBUG_RECV) {
			vty_out(vty, "debug zebra packet%s%s", IS_ZEBRA_DEBUG_DETAIL ? " detail" : "", VTY_NEWLINE);
			write++;
		} else {
			if(IS_ZEBRA_DEBUG_SEND) {
				vty_out(vty, "debug zebra packet send%s%s", IS_ZEBRA_DEBUG_DETAIL ? " detail" : "", VTY_NEWLINE);
			} else {
				vty_out(vty, "debug zebra packet recv%s%s", IS_ZEBRA_DEBUG_DETAIL ? " detail" : "", VTY_NEWLINE);
			}
			write++;
		}
	}
