...
static struct connected *router_id_find_node(struct list *l, struct connected *ifc) {
...
void router_id_add_address(struct connected *ifc) {
	...
	if(!strncmp(ifc->ifp->name, "lo", 2) || !strncmp(ifc->ifp->name, "dummy", 5)) {
		...
		if((zvrf = vrf_iter2info(iter)) != NULL) {
			if(zvrf->rid_user_assigned.u.prefix4.s_addr) {
				if(zvrf->vrf_id == VRF_DEFAULT) {
					vty_out(vty, "router-id %s%s", inet_ntoa(zvrf->rid_user_assigned.u.prefix4), VTY_NEWLINE);
				} else {
					vty_out(vty, "router-id %s vrf %u%s", inet_ntoa(zvrf->rid_user_assigned.u.prefix4), zvrf->vrf_id, VTY_NEWLINE);
				}
			}
		}
	}
...
DEFUN(router_id, router_id_cmd, "router-id A.B.C.D",
...
ALIAS(router_id, router_id_vrf_cmd, "router-id A.B.C.D " VRF_CMD_STR,
      "Manually set the router-id\n"
      "IP address to use for router-id\n" VRF_CMD_HELP_STR)

DEFUN(no_router_id, no_router_id_cmd, "no router-id", NO_STR "Remove the manually configured router-id\n") {
	struct prefix rid;
	vrf_id_t vrf_id = VRF_DEFAULT;
...
ALIAS(no_router_id, no_router_id_vrf_cmd, "no router-id " VRF_CMD_STR, NO_STR "Remove the manually configured router-id\n" VRF_CMD_HELP_STR)
