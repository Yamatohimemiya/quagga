...
static void zfpm_dt_log_fpm_message(Fpm__Message *msg) {
	...
	zfpm_debug("Vrf id: %d, Prefix: %s/%d, Metric: %d", add_route->vrf_id, inet_ntop(family, &prefix.u.prefix, buf, sizeof(buf)), prefix.prefixlen, add_route->metric);
   ...
   */
	for(i = 0; i < add_route->n_nexthops; i++) {
		...
		zfpm_debug("Nexthop - if_index: %d (%s), gateway: %s, ", if_index, if_name ? if_name : "name not specified", nexthop->address ? inet_ntoa(nh_addr.ipv4) : "None");
	}
}
