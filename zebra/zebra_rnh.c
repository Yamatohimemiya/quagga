...
#define lookup_rnh_table(v, f) \
	({ \
		...
		if(zvrf) t = zvrf->rnh_table[family2afi(f)]; \
		t; \
	})
...
struct rnh *zebra_add_rnh(struct prefix *p, vrf_id_t vrfid) {
...
void zebra_delete_rnh(struct rnh *rnh) {
	struct route_node *rn;
...
void zebra_add_rnh_client(struct rnh *rnh, struct zserv *client, vrf_id_t vrf_id) {
	if(IS_ZEBRA_DEBUG_NHT) {
		char buf[INET6_ADDRSTRLEN];
		zlog_debug("client %s registers rnh %s", zebra_route_string(client->proto), rnh_str(rnh, buf, INET6_ADDRSTRLEN));
	}
...
void zebra_remove_rnh_client(struct rnh *rnh, struct zserv *client) {
	if(IS_ZEBRA_DEBUG_NHT) {
		char buf[INET6_ADDRSTRLEN];
		zlog_debug("client %s unregisters rnh %s", zebra_route_string(client->proto), rnh_str(rnh, buf, INET6_ADDRSTRLEN));
	}
	...
	for(nrn = route_top(ntable); nrn; nrn = route_next(nrn)) {
		...
		if(compare_state(rib, rnh->state)) {
			if(IS_ZEBRA_DEBUG_NHT) {
				...
				zlog_debug(
					"rnh %s resolved through route %s - sending "
					"nexthop %s event to clients",
					bufn, bufp, rib ? "reachable" : "unreachable"
				);
			}
			copy_state(rnh, rib);
...
int zebra_dispatch_rnh_table(vrf_id_t vrfid, int family, struct zserv *client) {
	struct route_table *ntable;
	struct route_node *nrn;
	struct rnh *rnh;
	...
	for(nrn = route_top(ntable); nrn; nrn = route_next(nrn)) {
		...
		if(IS_ZEBRA_DEBUG_NHT) {
			char bufn[INET6_ADDRSTRLEN];
			prefix2str(&nrn->p, bufn, INET6_ADDRSTRLEN);
			zlog_debug("rnh %s - sending nexthop %s event to client %s", bufn, rnh->state ? "reachable" : "unreachable", zebra_route_string(client->proto));
		}
		send_client(rnh, client, vrfid);
	}
	return 1;
...
int zebra_cleanup_rnh_client(vrf_id_t vrfid, int family, struct zserv *client) {
	struct route_table *ntable;
	struct route_node *nrn;
	struct rnh *rnh;
	...
	for(nrn = route_top(ntable); nrn; nrn = route_next(nrn)) {
		...
		if(IS_ZEBRA_DEBUG_NHT) {
			char bufn[INET6_ADDRSTRLEN];
			prefix2str(&nrn->p, bufn, INET6_ADDRSTRLEN);
			zlog_debug("rnh %s - cleaning state for client %s", bufn, zebra_route_string(client->proto));
		}
 ...
 */
static void rib_copy_nexthop(struct rib *state, struct nexthop *nh) {
	struct nexthop *nexthop;
...
static int send_client(struct rnh *rnh, struct zserv *client, vrf_id_t vrf_id) {
	...
	if(rib) {
		...
		stream_putc(s, 0);
		for(nexthop = rib->nexthop; nexthop; nexthop = nexthop->next) {
			if(CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_FIB) && !CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_RECURSIVE) && CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_ACTIVE)) {
...
static void print_nh(struct nexthop *nexthop, struct vty *vty) {
	...
	switch(nexthop->type) {
		...
		case NEXTHOP_TYPE_IPV6_IFNAME:
			...
			break;
		case NEXTHOP_TYPE_IFINDEX: vty_out(vty, " is directly connected, %s", ifindex2ifname(nexthop->ifindex)); break;
		case NEXTHOP_TYPE_IFNAME: vty_out(vty, " is directly connected, %s", nexthop->ifname); break;
		case NEXTHOP_TYPE_BLACKHOLE: vty_out(vty, " is directly connected, Null0"); break;
		default: break;
	}
...
static void print_rnh(struct route_node *rn, struct vty *vty) {
	...
	rnh = rn->info;
	vty_out(vty, "%s%s", inet_ntop(rn->p.family, &rn->p.u.prefix, buf, BUFSIZ), VTY_NEWLINE);
	if(rnh->state) {
		vty_out(vty, " resolved via %s%s", zebra_route_string(rnh->state->type), VTY_NEWLINE);
		for(nexthop = rnh->state->nexthop; nexthop; nexthop = nexthop->next) {
			print_nh(nexthop, vty);
		}
	} else {
		vty_out(vty, " unresolved%s%s", CHECK_FLAG(rnh->flags, ZEBRA_NHT_CONNECTED) ? "(Connected)" : "", VTY_NEWLINE);
	}

	vty_out(vty, " Client list:");
	for(ALL_LIST_ELEMENTS_RO(rnh->client_list, node, client)) {
		vty_out(vty, " %s(fd %d)", zebra_route_string(client->proto), client->sock);
	}
	vty_out(vty, "%s", VTY_NEWLINE);
}
