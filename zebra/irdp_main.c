...
int irdp_sock_init(void) {
	...
	if(zserv_privs.change(ZPRIVS_RAISE)) {
		zlog_err("irdp_sock_init: could not raise privs, %s", safe_strerror(errno));
	}
	...
	if(zserv_privs.change(ZPRIVS_LOWER)) {
		zlog_err("irdp_sock_init: could not lower privs, %s", safe_strerror(errno));
	}
...
static int make_advertisement_packet(struct interface *ifp, struct prefix *p, struct stream *s) {
...
static void irdp_send(struct interface *ifp, struct prefix *p, struct stream *s) {
	...
	if(irdp->flags & IF_DEBUG_MESSAGES) {
		zlog_debug("IRDP: TX Advert on %s %s Holdtime=%d Preference=%d", ifp->name, prefix2str(p, buf, sizeof buf), irdp->flags & IF_SHUTDOWN ? 0 : irdp->Lifetime, get_pref(irdp, p));
	}
