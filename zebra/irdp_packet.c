...
static void parse_irdp_packet(char *p, int len, struct interface *ifp) {
   ...
   len of IP-header) 14+20 */
	...
	if(in_cksum(icmp, datalen) != icmp->checksum) {
		zlog_warn("IRDP: RX ICMP packet from %s. Bad checksum, silently ignored", inet_ntoa(src));
		return;
	}
	...
	if(!((ntohl(ip->ip_dst.s_addr) == INADDR_BROADCAST) && (irdp->flags & IF_BROADCAST)) || (ntohl(ip->ip_dst.s_addr) == INADDR_ALLRTRS_GROUP && !(irdp->flags & IF_BROADCAST))) {
		zlog_warn(
			"IRDP: RX illegal from %s to %s while %s operates in %s\n", inet_ntoa(src), ntohl(ip->ip_dst.s_addr) == INADDR_ALLRTRS_GROUP ? "multicast" : inet_ntoa(ip->ip_dst), ifp->name,
	...
	switch(icmp->type) {
		...
		default: zlog_warn("IRDP: RX type %d from %s. Bad ICMP type, silently ignored", icmp->type, inet_ntoa(src));
	}
}
...
void send_packet(struct interface *ifp, struct stream *s, u_int32_t dst, struct prefix *p, u_int32_t ttl) {
	...
	on = 1;
	if(setsockopt(irdp_sock, IPPROTO_IP, IP_HDRINCL, (char *) &on, sizeof(on)) < 0) {
		zlog_warn("sendto %s", safe_strerror(errno));
	}

	if(dst == INADDR_BROADCAST) {
		on = 1;
		if(setsockopt(irdp_sock, SOL_SOCKET, SO_BROADCAST, (char *) &on, sizeof(on)) < 0) {
			zlog_warn("sendto %s", safe_strerror(errno));
		}
	}
	...
	if(dst != INADDR_BROADCAST) {
		on = 0;
		if(setsockopt(irdp_sock, IPPROTO_IP, IP_MULTICAST_LOOP, (char *) &on, sizeof(on)) < 0) {
			zlog_warn("sendto %s", safe_strerror(errno));
		}
	}
