...
static int zserv_flush_data(struct thread *thread) {
	...
	switch(buffer_flush_available(client->wb, client->sock)) {
		case BUFFER_ERROR:
			...
			zebra_client_close(client);
			break;
		case BUFFER_PENDING: client->t_write = thread_add_write(zebrad.master, zserv_flush_data, client, client->sock); break;
...
int zebra_server_send_message(struct zserv *client) {
	...
	switch(buffer_write(client->wb, client->sock, STREAM_DATA(client->obuf), stream_get_endp(client->obuf))) {
		case BUFFER_ERROR:
			zlog_warn("%s: buffer_write failed to zserv client fd %d, closing", __func__, client->sock);
	 ...
	 deleted. */
			client->t_suicide = thread_add_event(zebrad.master, zserv_delayed_close, client, 0);
			return -1;
		case BUFFER_EMPTY: THREAD_OFF(client->t_write); break;
		case BUFFER_PENDING: THREAD_WRITE_ON(zebrad.master, client->t_write, zserv_flush_data, client, client->sock); break;
	}
 ...
 *
 */
int zsend_interface_address(int cmd, struct zserv *client, struct interface *ifp, struct connected *ifc) {
	int blen;
	struct stream *s;
	struct prefix *p;
 ...
 * duplication.
 */
int zsend_route_multipath(int cmd, struct zserv *client, struct prefix *p, struct rib *rib) {
	int psize;
	struct stream *s;
	...
	if(!vrf_bitmap_check(client->redist[rib->type], rib->vrf_id) && !(is_default(p) && vrf_bitmap_check(client->redist_default, rib->vrf_id))) {
		return 0;
	}
	...
	for(nexthop = rib->nexthop; nexthop; nexthop = nexthop->next) {
		if(CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_ACTIVE)) {
			...
			switch(nexthop->type) {
				...
				case NEXTHOP_TYPE_IPV4_IFINDEX: stream_put_in_addr(s, &nexthop->gate.ipv4); break;
#ifdef HAVE_IPV6
				case NEXTHOP_TYPE_IPV6:
				case NEXTHOP_TYPE_IPV6_IFINDEX:
				case NEXTHOP_TYPE_IPV6_IFNAME: stream_write(s, (u_char *) &nexthop->gate.ipv6, 16); break;
#endif
				default:
...
#ifdef HAVE_IPV6
static int zsend_ipv6_nexthop_lookup(struct zserv *client, struct in6_addr *addr, vrf_id_t vrf_id) {
	struct stream *s;
	struct rib *rib;
	unsigned long nump;
	u_char num;
       ...
       * chain of nexthops. */
		for(nexthop = rib->nexthop; nexthop; nexthop = nexthop->next) {
			if(CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_ACTIVE)) {
				...
				switch(nexthop->type) {
					case ZEBRA_NEXTHOP_IPV6: stream_put(s, &nexthop->gate.ipv6, 16); break;
					...
					case ZEBRA_NEXTHOP_IFNAME: stream_putl(s, nexthop->ifindex); break;
					default:
						/* do nothing */
						break;
				}
...
*/
static int zsend_ipv4_nexthop_lookup(struct zserv *client, struct in_addr addr, int cmd, vrf_id_t vrf_id) {
	struct stream *s;
	struct rib *rib;
	unsigned long nump;
	u_char num;
       ...
       * chain of nexthops. */
		for(nexthop = rib->nexthop; nexthop; nexthop = nexthop->next) {
			if(CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_ACTIVE)) {
				stream_putc(s, nexthop->type);
				switch(nexthop->type) {
					case ZEBRA_NEXTHOP_IPV4: stream_put_in_addr(s, &nexthop->gate.ipv4); break;
...
/* Nexthop register */
static int zserv_nexthop_register(struct zserv *client, int sock, u_short length, vrf_id_t vrf_id) {
	struct rnh *rnh;
	struct stream *s;
	struct prefix p;
	u_short l = 0;
...
/* Nexthop register */
static int zserv_nexthop_unregister(struct zserv *client, int sock, u_short length) {
	struct rnh *rnh;
	struct stream *s;
	struct prefix p;
	u_short l = 0;
...
static int zsend_ipv4_import_lookup(struct zserv *client, struct prefix_ipv4 *p, vrf_id_t vrf_id) {
	struct stream *s;
	struct rib *rib;
	unsigned long nump;
	u_char num;
...
int zsend_router_id_update(struct zserv *client, struct prefix *p, vrf_id_t vrf_id) {
	struct stream *s;
	int blen;
...
static int zread_interface_add(struct zserv *client, u_short length, vrf_id_t vrf_id) {
	...
	for(ALL_LIST_ELEMENTS(vrf_iflist(vrf_id), ifnode, ifnnode, ifp)) {
		...
		for(ALL_LIST_ELEMENTS(ifp->connected, cnode, cnnode, c)) {
			if(CHECK_FLAG(c->conf, ZEBRA_IFC_REAL) && (zsend_interface_address(ZEBRA_INTERFACE_ADDRESS_ADD, client, ifp, c) < 0)) {
				return -1;
			}
		}
	}
...
static int zread_interface_delete(struct zserv *client, u_short length, vrf_id_t vrf_id) {
		...
		for(i = 0; i < nexthop_num; i++) {
			...
			switch(nexthop_type) {
				...
				case ZEBRA_NEXTHOP_IPV4_IFINDEX:
					...
					break;
				case ZEBRA_NEXTHOP_IPV6: stream_forward_getp(s, IPV6_MAX_BYTELEN); break;
				case ZEBRA_NEXTHOP_BLACKHOLE: rib_nexthop_blackhole_add(rib); break;
			}
		}
	}
...
static int zread_ipv4_delete(struct zserv *client, u_short length, vrf_id_t vrf_id) {
	int i;
	struct stream *s;
	...
	if(CHECK_FLAG(api.message, ZAPI_MESSAGE_NEXTHOP)) {
		...
		for(i = 0; i < nexthop_num; i++) {
			...
			switch(nexthop_type) {
				case ZEBRA_NEXTHOP_IFINDEX: ifindex = stream_getl(s); break;
				case ZEBRA_NEXTHOP_IFNAME:
					ifname_len = stream_getc(s);
					stream_forward_getp(s, ifname_len);
					break;
	...
	rib_delete_ipv4(api.type, api.flags, &p, nexthop_p, ifindex, vrf_id, api.safi);
	client->v4_route_del_cnt++;
	return 0;
}

/* Nexthop lookup for IPv4. */
static int zread_ipv4_nexthop_lookup(int cmd, struct zserv *client, u_short length, vrf_id_t vrf_id) {
	struct in_addr addr;
	char buf[BUFSIZ];

	addr.s_addr = stream_get_ipv4(client->ibuf);
	if(IS_ZEBRA_DEBUG_PACKET && IS_ZEBRA_DEBUG_RECV) {
		zlog_debug("%s: looking up %s", __func__, inet_ntop(AF_INET, &addr, buf, BUFSIZ));
	}
...
/* Nexthop lookup for IPv4. */
static int zread_ipv4_import_lookup(struct zserv *client, u_short length, vrf_id_t vrf_id) {
	struct prefix_ipv4 p;
...
static int zread_ipv6_add(struct zserv *client, u_short length, vrf_id_t vrf_id) {
	int i;
	struct stream *s;
...
static int zread_ipv6_delete(struct zserv *client, u_short length, vrf_id_t vrf_id) {
	int i;
	struct stream *s;
	...
	if(CHECK_FLAG(api.message, ZAPI_MESSAGE_NEXTHOP)) {
		...
		for(i = 0; i < api.nexthop_num; i++) {
			...
			switch(nexthop_type) {
				case ZEBRA_NEXTHOP_IPV6: stream_get(&nexthop, s, 16); break;
				case ZEBRA_NEXTHOP_IFINDEX: ifindex = stream_getl(s); break;
			}
		}
	}
	...
	if(IN6_IS_ADDR_UNSPECIFIED(&nexthop)) {
		rib_delete_ipv6(api.type, api.flags, &p, NULL, ifindex, vrf_id, api.safi);
	} else {
		rib_delete_ipv6(api.type, api.flags, &p, &nexthop, ifindex, vrf_id, api.safi);
	}
...
static int zread_ipv6_nexthop_lookup(struct zserv *client, u_short length, vrf_id_t vrf_id) {
	struct in6_addr addr;
	char buf[BUFSIZ];

	stream_get(&addr, client->ibuf, 16);
	if(IS_ZEBRA_DEBUG_PACKET && IS_ZEBRA_DEBUG_RECV) {
		zlog_debug("%s: looking up %s", __func__, inet_ntop(AF_INET6, &addr, buf, BUFSIZ));
	}
...
static int zread_router_id_add(struct zserv *client, u_short length, vrf_id_t vrf_id) {
	struct prefix p;
...
static int zread_router_id_delete(struct zserv *client, u_short length, vrf_id_t vrf_id) {
...
static void zread_hello(struct zserv *client) {
	...
	if((proto < ZEBRA_ROUTE_MAX) && (proto > ZEBRA_ROUTE_STATIC)) {
		zlog_notice("client %d says hello and bids fair to announce only %s routes", client->sock, zebra_route_string(proto));

		/* if route-type was binded by other client */
		if(route_type_oaths[proto]) {
			zlog_warn("sender of %s routes changed %c->%c", zebra_route_string(proto), route_type_oaths[proto], client->sock);
		}
...
static int zread_vrf_unregister(struct zserv *client, u_short length, vrf_id_t vrf_id) {
	int i;
 ...
 */
static void zebra_score_rib(int client_sock) {
	int i;

	for(i = ZEBRA_ROUTE_RIP; i < ZEBRA_ROUTE_MAX; i++) {
		if(client_sock == route_type_oaths[i]) {
			zlog_notice("client %d disconnected. %lu %s routes removed from the rib", client_sock, rib_score_proto(i), zebra_route_string(i));
...
static int zebra_client_read(struct thread *thread) {
	...
	if((already = stream_get_endp(client->ibuf)) < ZEBRA_HEADER_SIZE) {
		ssize_t nbyte;
		if(((nbyte = stream_read_try(client->ibuf, sock, ZEBRA_HEADER_SIZE - already)) == 0) || (nbyte == -1)) {
	...
	if(length < ZEBRA_HEADER_SIZE) {
		zlog_warn("%s: socket %d message length %u is less than header size %d", __func__, sock, length, ZEBRA_HEADER_SIZE);
		zebra_client_close(client);
		return -1;
	}
	if(length > STREAM_SIZE(client->ibuf)) {
		zlog_warn("%s: socket %d message length %u exceeds buffer size %lu", __func__, sock, length, (u_long) STREAM_SIZE(client->ibuf));
		zebra_client_close(client);
		return -1;
	}
	...
	if(already < length) {
		ssize_t nbyte;
		if(((nbyte = stream_read_try(client->ibuf, sock, length - already)) == 0) || (nbyte == -1)) {
			if(IS_ZEBRA_DEBUG_EVENT) {
	...
	if(IS_ZEBRA_DEBUG_PACKET && IS_ZEBRA_DEBUG_RECV) {
		zlog_debug("zebra message received [%s] %d in VRF %u", zserv_command_string(command), length, vrf_id);
	}
	...
	switch(command) {
		case ZEBRA_ROUTER_ID_ADD: zread_router_id_add(client, length, vrf_id); break;
		case ZEBRA_ROUTER_ID_DELETE: zread_router_id_delete(client, length, vrf_id); break;
		case ZEBRA_INTERFACE_ADD: zread_interface_add(client, length, vrf_id); break;
		case ZEBRA_INTERFACE_DELETE: zread_interface_delete(client, length, vrf_id); break;
		case ZEBRA_IPV4_ROUTE_ADD: zread_ipv4_add(client, length, vrf_id); break;
		case ZEBRA_IPV4_ROUTE_DELETE: zread_ipv4_delete(client, length, vrf_id); break;
#ifdef HAVE_IPV6
		case ZEBRA_IPV6_ROUTE_ADD: zread_ipv6_add(client, length, vrf_id); break;
		case ZEBRA_IPV6_ROUTE_DELETE: zread_ipv6_delete(client, length, vrf_id); break;
#endif /* HAVE_IPV6 */
		case ZEBRA_REDISTRIBUTE_ADD: zebra_redistribute_add(command, client, length, vrf_id); break;
		case ZEBRA_REDISTRIBUTE_DELETE: zebra_redistribute_delete(command, client, length, vrf_id); break;
		case ZEBRA_REDISTRIBUTE_DEFAULT_ADD: zebra_redistribute_default_add(command, client, length, vrf_id); break;
		case ZEBRA_REDISTRIBUTE_DEFAULT_DELETE: zebra_redistribute_default_delete(command, client, length, vrf_id); break;
		case ZEBRA_IPV4_NEXTHOP_LOOKUP:
		case ZEBRA_IPV4_NEXTHOP_LOOKUP_MRIB: zread_ipv4_nexthop_lookup(command, client, length, vrf_id); break;
#ifdef HAVE_IPV6
		case ZEBRA_IPV6_NEXTHOP_LOOKUP: zread_ipv6_nexthop_lookup(client, length, vrf_id); break;
#endif /* HAVE_IPV6 */
		case ZEBRA_IPV4_IMPORT_LOOKUP: zread_ipv4_import_lookup(client, length, vrf_id); break;
		case ZEBRA_HELLO: zread_hello(client); break;
		case ZEBRA_VRF_UNREGISTER: zread_vrf_unregister(client, length, vrf_id);
		case ZEBRA_NEXTHOP_REGISTER: zserv_nexthop_register(client, sock, length, vrf_id); break;
		case ZEBRA_NEXTHOP_UNREGISTER: zserv_nexthop_unregister(client, sock, length); break;
		default: zlog_info("Zebra received unknown command %d", command); break;
	}
...
static void zebra_serv_un(const char *path) {
	...
	if(ret < 0) {
		zlog_warn("Can't bind to unix socket %s: %s", path, safe_strerror(errno));
		zlog_warn("zebra can't provide full functionality due to above error");
		close(sock);
		return;
	}
	...
	ret = listen(sock, 5);
	if(ret < 0) {
		zlog_warn("Can't listen to unix socket %s: %s", path, safe_strerror(errno));
		zlog_warn("zebra can't provide full functionality due to above error");
		close(sock);
		return;
	}
...
static void zebra_event(enum event event, int sock, struct zserv *client) {
	switch(event) {
		case ZEBRA_SERV: thread_add_read(zebrad.master, zebra_accept, client, sock); break;
		case ZEBRA_READ: client->t_read = thread_add_read(zebrad.master, zebra_client_read, client, sock); break;
		case ZEBRA_WRITE:
			/**/
			break;
	}
...
#define ONE_WEEK_SECOND 60 * 60 * 24 * 7
	...
	if(now < ONE_DAY_SECOND) {
		...
		snprintf(buf, buflen, "%dd%02dh%02dm", tm->tm_yday, tm->tm_hour, tm->tm_min);
	} else {
		snprintf(buf, buflen, "%02dw%dd%02dh", tm->tm_yday / 7, tm->tm_yday - ((tm->tm_yday / 7) * 7), tm->tm_hour);
	}
	return buf;
}
...
static void zebra_show_client_detail(struct vty *vty, struct zserv *client) {
	...
	vty_out(vty, "Client: %s %s", zebra_route_string(client->proto), VTY_NEWLINE);
	vty_out(vty, "------------------------ %s", VTY_NEWLINE);
	vty_out(vty, "FD: %d %s", client->sock, VTY_NEWLINE);
	vty_out(vty, "Route Table ID: %d %s", client->rtm_table, VTY_NEWLINE);

	vty_out(vty, "Connect Time: %s %s", zserv_time_buf(&client->connect_time, cbuf, ZEBRA_TIME_BUF), VTY_NEWLINE);
	if(client->nh_reg_time) {
		vty_out(vty, "Nexthop Registry Time: %s %s", zserv_time_buf(&client->nh_reg_time, nhbuf, ZEBRA_TIME_BUF), VTY_NEWLINE);
		if(client->nh_last_upd_time) {
			vty_out(vty, "Nexthop Last Update Time: %s %s", zserv_time_buf(&client->nh_last_upd_time, mbuf, ZEBRA_TIME_BUF), VTY_NEWLINE);
		} else {
	...
	vty_out(vty, "Last Msg Rx Time: %s %s", zserv_time_buf(&client->last_read_time, rbuf, ZEBRA_TIME_BUF), VTY_NEWLINE);
	vty_out(vty, "Last Msg Tx Time: %s %s", zserv_time_buf(&client->last_write_time, wbuf, ZEBRA_TIME_BUF), VTY_NEWLINE);
	if(client->last_read_time) {
		vty_out(vty, "Last Rcvd Cmd: %s %s", zserv_command_string(client->last_read_cmd), VTY_NEWLINE);
	}
	if(client->last_write_time) {
		vty_out(vty, "Last Sent Cmd: %s %s", zserv_command_string(client->last_write_cmd), VTY_NEWLINE);
	}
	vty_out(vty, "%s", VTY_NEWLINE);

	vty_out(vty, "Type        Add        Update     Del %s", VTY_NEWLINE);
	vty_out(vty, "================================================== %s", VTY_NEWLINE);
	vty_out(vty, "IPv4        %-12d%-12d%-12d%s", client->v4_route_add_cnt, client->v4_route_upd8_cnt, client->v4_route_del_cnt, VTY_NEWLINE);
	vty_out(vty, "IPv6        %-12d%-12d%-12d%s", client->v6_route_add_cnt, client->v6_route_upd8_cnt, client->v6_route_del_cnt, VTY_NEWLINE);
	vty_out(vty, "Redist:v4   %-12d%-12d%-12d%s", client->redist_v4_add_cnt, 0, client->redist_v4_del_cnt, VTY_NEWLINE);
	vty_out(vty, "Redist:v6   %-12d%-12d%-12d%s", client->redist_v6_add_cnt, 0, client->redist_v6_del_cnt, VTY_NEWLINE);
	vty_out(vty, "Connected   %-12d%-12d%-12d%s", client->ifadd_cnt, 0, client->ifdel_cnt, VTY_NEWLINE);
	vty_out(vty, "Interface Up Notifications: %d%s", client->ifup_cnt, VTY_NEWLINE);
	vty_out(vty, "Interface Down Notifications: %d%s", client->ifdown_cnt, VTY_NEWLINE);
...
static void zebra_show_client_brief(struct vty *vty, struct zserv *client) {
	...
	vty_out(vty, "%-8s%12s %12s%12s%8d/%-8d%8d/%-8d%s", zebra_route_string(client->proto), zserv_time_buf(&client->connect_time, cbuf, ZEBRA_TIME_BUF), zserv_time_buf(&client->last_read_time, rbuf, ZEBRA_TIME_BUF),
		zserv_time_buf(&client->last_write_time, wbuf, ZEBRA_TIME_BUF), client->v4_route_add_cnt + client->v4_route_upd8_cnt, client->v4_route_del_cnt, client->v6_route_add_cnt + client->v6_route_upd8_cnt, client->v6_route_del_cnt,
		VTY_NEWLINE);
}

/* Display default rtm_table for all clients. */
DEFUN(show_table, show_table_cmd, "show table", SHOW_STR "default routing table to use for all clients\n") {
	vty_out(vty, "table %d%s", zebrad.rtm_table_default, VTY_NEWLINE);
	return CMD_SUCCESS;
}

DEFUN(config_table, config_table_cmd, "table TABLENO",
...
/* This command is for debugging purpose. */
DEFUN(show_zebra_client_summary, show_zebra_client_summary_cmd, "show zebra client summary",
      SHOW_STR "Zebra information brief"
	       "Client information brief") {
	struct listnode *node;
	struct zserv *client;

	vty_out(vty, "Name    Connect Time    Last Read  Last Write  IPv4 Routes       IPv6 Routes    %s", VTY_NEWLINE);
	vty_out(vty, "--------------------------------------------------------------------------------%s", VTY_NEWLINE);
...
static struct cmd_node table_node = { TABLE_NODE, "", /* This node has no interface. */
				      1 };

/* Only display ip forwarding is enabled or not. */
DEFUN(show_ip_forwarding, show_ip_forwarding_cmd, "show ip forwarding", SHOW_STR IP_STR "IP forwarding status\n") {
	int ret;
...
DEFUN(show_ipv6_forwarding, show_ipv6_forwarding_cmd, "show ipv6 forwarding",
      SHOW_STR "IPv6 information\n"
	       "Forwarding status\n") {
	int ret;
	...
	switch(ret) {
		case -1: vty_out(vty, "ipv6 forwarding is unknown%s", VTY_NEWLINE); break;
		case 0: vty_out(vty, "ipv6 forwarding is %s%s", "off", VTY_NEWLINE); break;
		case 1: vty_out(vty, "ipv6 forwarding is %s%s", "on", VTY_NEWLINE); break;
		default: vty_out(vty, "ipv6 forwarding is %s%s", "off", VTY_NEWLINE); break;
	}
	return CMD_SUCCESS;
}
...
/* table node for routing tables. */
static struct cmd_node forwarding_node = { FORWARDING_NODE, "", /* This node has no interface. */
					   1 };
