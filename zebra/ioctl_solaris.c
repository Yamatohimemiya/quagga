...
int if_ioctl_ipv6(u_long request, caddr_t buffer) {
	...
	if(sock < 0) {
		...
		zlog_err("Cannot create IPv6 datagram socket: %s", safe_strerror(save_errno));
		exit(1);
	}
...
void if_get_mtu(struct interface *ifp) {
	...
	if(ifp->flags & IFF_IPV4) {
		...
		if(ret < 0) {
			zlog_info("Can't lookup mtu on %s by ioctl(SIOCGLIFMTU)", ifp->name);
			ifp->mtu = -1;
		} else {
...
int if_get_flags_direct(const char *ifname, uint64_t *flags, unsigned int af) {
	...
	if(ret) {
		zlog_debug("%s: ifname %s, error %s (%d)", __func__, ifname, safe_strerror(errno), errno);
	}
...
int if_set_flags(struct interface *ifp, uint64_t flags) {
	...
	if(ret < 0) {
		zlog_info("can't set interface flags on %s: %s", ifp->name, safe_strerror(errno));
	} else {
		ret = 0;
	}
...
int if_prefix_add_ipv6(struct interface *ifp, struct connected *ifc) {
	...
	zlog_warn("Can't set %s on interface %s", prefix2str(ifc->address, addrbuf, sizeof(addrbuf)), ifp->name);
...
int if_prefix_delete_ipv6(struct interface *ifp, struct connected *ifc) {
	...
	zlog_warn("Can't delete %s on interface %s", prefix2str(ifc->address, addrbuf, sizeof(addrbuf)), ifp->name);
