...
/* Hook functions. */
static u_char *ipFwNumber(struct variable *, oid[], size_t *, int, size_t *, WriteMethod **);
static u_char *ipFwTable(struct variable *, oid[], size_t *, int, size_t *, WriteMethod **);
static u_char *ipCidrNumber(struct variable *, oid[], size_t *, int, size_t *, WriteMethod **);
static u_char *ipCidrTable(struct variable *, oid[], size_t *, int, size_t *, WriteMethod **);
...
static u_char *ipFwNumber(struct variable *v, oid objid[], size_t *objid_len, int exact, size_t *val_len, WriteMethod **write_method) {
	static int result;
...
static u_char *ipCidrNumber(struct variable *v, oid objid[], size_t *objid_len, int exact, size_t *val_len, WriteMethod **write_method) {
	static int result;
	struct route_table *table;
	struct route_node *rn;
	struct rib *rib;
...
static int proto_trans(int type) {
	switch(type) {
		case ZEBRA_ROUTE_SYSTEM: return 1;  /* other */
		case ZEBRA_ROUTE_KERNEL: return 1;  /* other */
		case ZEBRA_ROUTE_CONNECT: return 2; /* local interface */
		case ZEBRA_ROUTE_STATIC: return 3;  /* static route */
		case ZEBRA_ROUTE_RIP: return 8;	    /* rip */
		case ZEBRA_ROUTE_RIPNG: return 1;   /* shouldn't happen */
		case ZEBRA_ROUTE_OSPF: return 13;   /* ospf */
		case ZEBRA_ROUTE_OSPF6: return 1;   /* shouldn't happen */
		case ZEBRA_ROUTE_BGP: return 14;    /* bgp */
		default: return 1;		    /* other */
	}
}

static void check_replace(struct route_node *np2, struct rib *rib2, struct route_node **np, struct rib **rib) {
	int proto, proto2;
	...
	if(in_addr_cmp((u_char *) &(*rib)->nexthop->gate.ipv4, (u_char *) &rib2->nexthop->gate.ipv4) <= 0) {
		return;
	}
...
static void get_fwtable_route_node(struct variable *v, oid objid[], size_t *objid_len, int exact, struct route_node **np, struct rib **rib) {
	struct in_addr dest;
	struct route_table *table;
   ...
   */
	...
	if(*objid_len > (unsigned) v->namelen + 6) {
		oid2in_addr(objid + v->namelen + 6, MIN(4, *objid_len - v->namelen - 6), &nexthop);
	}
		...
		for(*np = route_top(table); *np; *np = route_next(*np)) {
			if(!in_addr_cmp(&(*np)->p.u.prefix, (u_char *) &dest)) {
				RNODE_FOREACH_RIB(*np, *rib) {
					if(!in_addr_cmp((u_char *) &(*rib)->nexthop->gate.ipv4, (u_char *) &nexthop)) {
	...
	for(np2 = route_top(table); np2; np2 = route_next(np2)) {
		...
		if(in_addr_cmp(&np2->p.u.prefix, (u_char *) &dest) == 0) { /* have to look at each rib individually */
			RNODE_FOREACH_RIB(np2, rib2) {
				int proto2, policy2;
				...
				if((policy < policy2) || ((policy == policy2) && (proto < proto2)) || ((policy == policy2) && (proto == proto2) && (in_addr_cmp((u_char *) &rib2->nexthop->gate.ipv4, (u_char *) &nexthop) >= 0))) {
					check_replace(np2, rib2, np, rib);
				}
			}
		}
...
static u_char *ipFwTable(struct variable *v, oid objid[], size_t *objid_len, int exact, size_t *val_len, WriteMethod **write_method) {
	...
	if(smux_header_table(v, objid, objid_len, exact, val_len, write_method) == MATCH_FAILED) {
		return NULL;
	}
...
static u_char *ipCidrTable(struct variable *v, oid objid[], size_t *objid_len, int exact, size_t *val_len, WriteMethod **write_method) {
	if(smux_header_table(v, objid, objid_len, exact, val_len, write_method) == MATCH_FAILED) {
		return NULL;
	}

	switch(v->magic) {
		case IPCIDRROUTEDEST: break;
		default: return NULL; break;
	}
	return NULL;
}
