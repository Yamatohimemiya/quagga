...
static inline const char *addr_to_a(u_char af, void *addr) {
	...
	switch(af) {
		case AF_INET: return inet_ntoa(*((struct in_addr *) addr));

#ifdef HAVE_IPV6
		case AF_INET6: return inet6_ntoa(*((struct in6_addr *) addr));
#endif

		default: return "<Addr in unknown AF>";
	}
}
 ...
 */
static int netlink_route_info_add_nh(netlink_route_info_t *ri, struct nexthop *nexthop, int recursive) {
...
static u_char netlink_proto_from_route_type(int type) {
	switch(type) {
		...
		case ZEBRA_ROUTE_CONNECT: return RTPROT_KERNEL;

		default: return RTPROT_ZEBRA;
	}
}
 ...
 */
static int netlink_route_info_fill(netlink_route_info_t *ri, int cmd, rib_dest_t *dest, struct rib *rib) {
 ...
 */
static int netlink_route_info_encode(netlink_route_info_t *ri, char *in_buf, size_t in_buf_len) {
	size_t bytelen;
	int nexthop_num = 0;
	size_t buf_offset;
	netlink_nh_info_t *nhi;
	...
	if(ri->num_nhs == 1) {
		...
		if(nhi->gateway) {
			addattr_l(&req->n, in_buf_len, RTA_GATEWAY, nhi->gateway, bytelen);
		}
   ...
   */
	...
	addattr_l(&req->n, in_buf_len, RTA_MULTIPATH, RTA_DATA(rta), RTA_PAYLOAD(rta));
...
static void zfpm_log_route_info(netlink_route_info_t *ri, const char *label) {
	...
	zfpm_debug("%s : %s %s/%d, Proto: %s, Metric: %u", label, nl_msg_type_to_str(ri->nlmsg_type), prefix_addr_to_a(ri->prefix), ri->prefix->prefixlen, nl_rtproto_to_str(ri->rtm_protocol), ri->metric ? *ri->metric : 0);

	for(i = 0; i < ri->num_nhs; i++) {
		nhi = &ri->nhs[i];
		zfpm_debug("  Intf: %u, Gateway: %s, Recursive: %s, Type: %s", nhi->if_index, addr_to_a(ri->af, nhi->gateway), nhi->recursive ? "yes" : "no", nexthop_type_to_str(nhi->type));
	}
}
 ...
 * value indicates an error.
 */
int zfpm_netlink_encode_route(int cmd, rib_dest_t *dest, struct rib *rib, char *in_buf, size_t in_buf_len) {
