...
void connected_up_ipv4(struct interface *ifp, struct connected *ifc) {
	...
	rib_add_ipv4(ZEBRA_ROUTE_CONNECT, 0, &p, NULL, NULL, ifp->ifindex, ifp->vrf_id, RT_TABLE_MAIN, ifp->metric, 0, 0, SAFI_UNICAST);

	rib_add_ipv4(ZEBRA_ROUTE_CONNECT, 0, &p, NULL, NULL, ifp->ifindex, ifp->vrf_id, RT_TABLE_MAIN, ifp->metric, 0, 0, SAFI_MULTICAST);

	rib_update(ifp->vrf_id);
}

/* Add connected IPv4 route to the interface. */
void connected_add_ipv4(struct interface *ifp, int flags, struct in_addr *addr, u_char prefixlen, struct in_addr *broad, const char *label) {
	struct prefix_ipv4 *p;
	struct connected *ifc;
		...
		if(CONNECTED_PEER(ifc)) {
			...
			if(broad->s_addr != ipv4_broadcast_addr(addr->s_addr, prefixlen)) {
				...
				zlog_warn(
					...
					ifp->name, inet_ntop(AF_INET, broad, buf[0], sizeof(buf[0])), prefixlen, inet_ntop(AF_INET, &bcalc, buf[1], sizeof(buf[1]))
				);
			}
		}
...
void connected_down_ipv4(struct interface *ifp, struct connected *ifc) {
	...
	rib_delete_ipv4(ZEBRA_ROUTE_CONNECT, 0, &p, NULL, ifp->ifindex, ifp->vrf_id, SAFI_UNICAST);

	rib_delete_ipv4(ZEBRA_ROUTE_CONNECT, 0, &p, NULL, ifp->ifindex, ifp->vrf_id, SAFI_MULTICAST);

	rib_update(ifp->vrf_id);
}

/* Delete connected IPv4 route to the interface. */
void connected_delete_ipv4(struct interface *ifp, int flags, struct in_addr *addr, u_char prefixlen, struct in_addr *broad) {
	struct prefix_ipv4 p;
	struct connected *ifc;
...
void connected_up_ipv6(struct interface *ifp, struct connected *ifc) {
	...
	rib_add_ipv6(ZEBRA_ROUTE_CONNECT, 0, &p, NULL, ifp->ifindex, ifp->vrf_id, RT_TABLE_MAIN, ifp->metric, 0, 0, SAFI_UNICAST);

	rib_update(ifp->vrf_id);
}

/* Add connected IPv6 route to the interface. */
void connected_add_ipv6(struct interface *ifp, int flags, struct in6_addr *addr, u_char prefixlen, struct in6_addr *broad, const char *label) {
	struct prefix_ipv6 *p;
	struct connected *ifc;
...
void connected_down_ipv6(struct interface *ifp, struct connected *ifc) {
	...
	rib_delete_ipv6(ZEBRA_ROUTE_CONNECT, 0, &p, NULL, ifp->ifindex, ifp->vrf_id, SAFI_UNICAST);

	rib_update(ifp->vrf_id);
}

void connected_delete_ipv6(struct interface *ifp, struct in6_addr *address, u_char prefixlen, struct in6_addr *broad) {
	struct prefix_ipv6 p;
	struct connected *ifc;
