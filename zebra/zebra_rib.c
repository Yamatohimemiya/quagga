...
static void __attribute__((format(printf, 4, 5))) _rnode_zlog(const char *_func, struct route_node *rn, int priority, const char *msgfmt, ...) {
	...
	if(rn) {
		...
		snprintf(buf, sizeof(buf), "%s%s vrf %u", prefix2str(&rn->p, prefix, sizeof(prefix)), info->safi == SAFI_MULTICAST ? " (MRIB)" : "", info->zvrf->vrf_id);
	} else {
...
#define rnode_debug(node, ...) _rnode_zlog(__func__, node, LOG_DEBUG, __VA_ARGS__)
#define rnode_info(node, ...) _rnode_zlog(__func__, node, LOG_INFO, __VA_ARGS__)
...
struct nexthop *rib_nexthop_ifindex_add(struct rib *rib, ifindex_t ifindex) {
...
struct nexthop *rib_nexthop_ifname_add(struct rib *rib, char *ifname) {
	struct nexthop *nexthop;
...
struct nexthop *rib_nexthop_ipv4_add(struct rib *rib, struct in_addr *ipv4, struct in_addr *src) {
	struct nexthop *nexthop;
...
struct nexthop *rib_nexthop_ipv4_ifindex_add(struct rib *rib, struct in_addr *ipv4, struct in_addr *src, ifindex_t ifindex) {
	struct nexthop *nexthop;
...
static struct nexthop *rib_nexthop_ipv6_ifname_add(struct rib *rib, struct in6_addr *ipv6, char *ifname) {
	struct nexthop *nexthop;
...
struct nexthop *rib_nexthop_ipv6_ifindex_add(struct rib *rib, struct in6_addr *ipv6, ifindex_t ifindex) {
	struct nexthop *nexthop;
...
struct nexthop *rib_nexthop_blackhole_add(struct rib *rib) {
	struct nexthop *nexthop;
...
static int nexthop_active_ipv4(struct rib *rib, struct nexthop *nexthop, int set, struct route_node *top) {
	...
	while(rn) {
	   ...
	   * a blackhole, mark it as inactive. */
			if(CHECK_FLAG(match->flags, ZEBRA_FLAG_BLACKHOLE) || CHECK_FLAG(match->flags, ZEBRA_FLAG_REJECT)) {
				return 0;
			}
				...
				for(newhop = match->nexthop; newhop; newhop = newhop->next) {
					if(CHECK_FLAG(newhop->flags, NEXTHOP_FLAG_FIB) && !CHECK_FLAG(newhop->flags, NEXTHOP_FLAG_RECURSIVE)) {
						if(set) {
							...
							if(newhop->type == NEXTHOP_TYPE_IPV4 || newhop->type == NEXTHOP_TYPE_IPV4_IFINDEX || newhop->type == NEXTHOP_TYPE_IPV4_IFNAME) {
			 ...
			 * is directly connected. */
							if(newhop->type == NEXTHOP_TYPE_IFINDEX || newhop->type == NEXTHOP_TYPE_IFNAME) {
...
struct rib *rib_match_ipv4_safi(struct in_addr addr, safi_t safi, int skip_bgp, struct route_node **rn_out, vrf_id_t vrf_id) {
	struct route_table *table;
	struct route_node *rn;
	struct rib *match;
...
struct rib *rib_match_ipv4_multicast(struct in_addr addr, struct route_node **rn_out, vrf_id_t vrf_id) {
	struct rib *rib = NULL, *mrib = NULL, *urib = NULL;
	struct route_node *m_rn = NULL, *u_rn = NULL;
	int skip_bgp = 0; /* bool */

	switch(ipv4_multicast_mode) {
		case MCAST_MRIB_ONLY: return rib_match_ipv4_safi(addr, SAFI_MULTICAST, skip_bgp, rn_out, vrf_id);
		case MCAST_URIB_ONLY: return rib_match_ipv4_safi(addr, SAFI_UNICAST, skip_bgp, rn_out, vrf_id);
		case MCAST_NO_CONFIG:
		case MCAST_MIX_MRIB_FIRST:
			rib = mrib = rib_match_ipv4_safi(addr, SAFI_MULTICAST, skip_bgp, &m_rn, vrf_id);
			if(!mrib) {
				rib = urib = rib_match_ipv4_safi(addr, SAFI_UNICAST, skip_bgp, &u_rn, vrf_id);
			}
			break;
		case MCAST_MIX_DISTANCE:
			mrib = rib_match_ipv4_safi(addr, SAFI_MULTICAST, skip_bgp, &m_rn, vrf_id);
	...
	if(IS_ZEBRA_DEBUG_RIB) {
		...
		zlog_debug("%s: %s vrf %u: found %s, using %s", __func__, buf, vrf_id, mrib ? (urib ? "MRIB+URIB" : "MRIB") : urib ? "URIB" : "nothing", rib == urib ? "URIB" : rib == mrib ? "MRIB" : "none");
	}
	return rib;
}
...
struct rib *rib_lookup_ipv4(struct prefix_ipv4 *p, vrf_id_t vrf_id) {
	struct route_table *table;
	struct route_node *rn;
	struct rib *match;
 ...
 * 3: no matches found
 */
int rib_lookup_ipv4_route(struct prefix_ipv4 *p, union sockunion *qgate, vrf_id_t vrf_id) {
	struct route_table *table;
	struct route_node *rn;
	struct rib *match;
	struct nexthop *nexthop, *tnexthop;
	...
	for(ALL_NEXTHOPS_RO(match->nexthop, nexthop, tnexthop, recursing)) {
		if(CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_FIB)) {
			...
			if(IS_ZEBRA_DEBUG_RIB) {
				...
				zlog_debug("%s: qgate == %s, %s == %s", __func__, qgate_buf, recursing ? "rgate" : "gate", gate_buf);
			}
		}
	}
...
struct rib *rib_match_ipv6(struct in6_addr *addr, vrf_id_t vrf_id) {
	struct prefix_ipv6 p;
	struct route_table *table;
	struct route_node *rn;
	struct rib *match;
...
#define RIB_SYSTEM_ROUTE(R) ((R)->type == ZEBRA_ROUTE_KERNEL || (R)->type == ZEBRA_ROUTE_CONNECT)
...
static unsigned nexthop_active_check(struct route_node *rn, struct rib *rib, struct nexthop *nexthop, int set) {
	...
	switch(nexthop->type) {
		case NEXTHOP_TYPE_IFINDEX:
			...
			break;
		case NEXTHOP_TYPE_IPV6_IFNAME: family = AFI_IP6;
		...
		case NEXTHOP_TYPE_IPV6_IFINDEX:
			...
			break;
		case NEXTHOP_TYPE_BLACKHOLE: SET_FLAG(nexthop->flags, NEXTHOP_FLAG_ACTIVE); break;
		default: break;
	}
...
static int rib_update_kernel(struct route_node *rn, struct rib *old, struct rib *new) {
	int ret = 0;
	struct nexthop *nexthop, *tnexthop;
	rib_table_info_t *info = rn->table->info;
	int recursing;
...
static int rib_can_delete_dest(rib_dest_t *dest) {
   ...
   * prefix.
   */
	if(CHECK_FLAG(dest->flags, RIB_DEST_UPDATE_FPM) || CHECK_FLAG(dest->flags, RIB_DEST_SENT_TO_FPM)) {
		return 0;
	}
...
static struct rib *rib_choose_best(struct rib *current, struct rib *alternate) {
	if(current == NULL) {
		return alternate;
	}
   ...
   */
	if(alternate->type == ZEBRA_ROUTE_CONNECT) {
		if(current->type != ZEBRA_ROUTE_CONNECT || alternate->metric <= current->metric) {
			return alternate;
		}
...
static void rib_process(struct route_node *rn) {
   ...
   */
	...
	if(old_fib != new_fib || (new_fib && CHECK_FLAG(new_fib->status, RIB_ENTRY_CHANGED))) {
	...
	if(old_selected != new_selected || (new_selected && CHECK_FLAG(new_selected->status, RIB_ENTRY_CHANGED))) {
		if(old_selected) {
			if(!new_selected) {
	...
	RNODE_FOREACH_RIB_SAFE(rn, rib, next) {
		if(CHECK_FLAG(rib->status, RIB_ENTRY_REMOVED)) {
			if(IS_ZEBRA_DEBUG_RIB) {
				rnode_debug(rn, "rn %p, removing rib %p", (void *) rn, (void *) rib);
			}
			rib_unlink(rn, rib);
		}
	}
...
static const u_char meta_queue_map[ZEBRA_ROUTE_MAX] = {
	[ZEBRA_ROUTE_SYSTEM] = 4, [ZEBRA_ROUTE_KERNEL] = 0, [ZEBRA_ROUTE_CONNECT] = 0, [ZEBRA_ROUTE_STATIC] = 1, [ZEBRA_ROUTE_RIP] = 2,	  [ZEBRA_ROUTE_RIPNG] = 2, [ZEBRA_ROUTE_OSPF] = 2,
	[ZEBRA_ROUTE_OSPF6] = 2,  [ZEBRA_ROUTE_ISIS] = 2,   [ZEBRA_ROUTE_BGP] = 3,     [ZEBRA_ROUTE_HSLS] = 4,	 [ZEBRA_ROUTE_BABEL] = 2, [ZEBRA_ROUTE_NHRP] = 2,
};
...
static void rib_meta_queue_add(struct meta_queue *mq, struct route_node *rn) {
	...
	RNODE_FOREACH_RIB(rn, rib) {
		...
		if(CHECK_FLAG(rib_dest_from_rnode(rn)->flags, RIB_ROUTE_QUEUED(qindex))) {
			if(IS_ZEBRA_DEBUG_RIB_Q) {
...
static void rib_queue_add(struct zebra_t *zebra, struct route_node *rn) {
	...
	if(!rnode_to_ribs(rn)) {
		zlog_debug("%s: called for route_node (%p, %d) with no ribs", __func__, (void *) rn, rn->lock);
		zlog_backtrace(LOG_DEBUG);
		return;
	}
...
int rib_add_ipv4(int type, int flags, struct prefix_ipv4 *p, struct in_addr *gate, struct in_addr *src, ifindex_t ifindex, vrf_id_t vrf_id, int table_id, u_int32_t metric, u_int32_t mtu, u_char distance, safi_t safi) {
	struct rib *rib;
	...
	if(IS_ZEBRA_DEBUG_RIB) {
		zlog_debug("%s: calling rib_addnode (%p, %p)", __func__, (void *) rn, (void *) rib);
	}
	rib_addnode(rn, rib);
	...
	if(same) {
		if(IS_ZEBRA_DEBUG_RIB) {
			zlog_debug("%s: calling rib_delnode (%p, %p)", __func__, (void *) rn, (void *) rib);
		}
		rib_delnode(rn, same);
	}
...
void _rib_dump(const char *func, union prefix46constptr pp, const struct rib *rib) {
	...
	zlog_debug("%s: dumping RIB entry %p for %s vrf %u", func, (void *) rib, prefix2str(p, straddr, sizeof(straddr)), rib->vrf_id);
	...
	for(ALL_NEXTHOPS_RO(rib->nexthop, nexthop, tnexthop, recursing)) {
		...
		zlog_debug(
			"%s: %s %s with flags %s%s%s", func, (recursing ? "  NH" : "NH"), straddr, (CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_ACTIVE) ? "ACTIVE " : ""), (CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_FIB) ? "FIB " : ""),
...
void rib_lookup_and_dump(struct prefix_ipv4 *p) {
	...
	/* No route for this prefix. */
	if(!rn) {
		zlog_debug("%s: lookup failed for %s", __func__, prefix2str((struct prefix *) p, prefix_buf, sizeof(prefix_buf)));
		return;
	}
	...
	RNODE_FOREACH_RIB(rn, rib) {
		zlog_debug(
			"%s: rn %p, rib %p: %s, %s", __func__, (void *) rn, (void *) rib, (CHECK_FLAG(rib->status, RIB_ENTRY_REMOVED) ? "removed" : "NOT removed"), (CHECK_FLAG(rib->flags, ZEBRA_FLAG_SELECTED) ? "selected" : "NOT selected")
		);
		rib_dump(p, rib);
	}
}
...
int rib_add_ipv4_multipath(struct prefix_ipv4 *p, struct rib *rib, safi_t safi) {
	...
	ret = 1;
	if(IS_ZEBRA_DEBUG_RIB) {
		zlog_debug("%s: called rib_addnode (%p, %p) on new RIB entry", __func__, (void *) rn, (void *) rib);
		rib_dump(p, rib);
	}
	...
	if(same) {
		if(IS_ZEBRA_DEBUG_RIB) {
			zlog_debug("%s: calling rib_delnode (%p, %p) on existing RIB entry", __func__, (void *) rn, (void *) same);
			rib_dump(p, same);
		}
		rib_delnode(rn, same);
		ret = -1;
...
int rib_delete_ipv4(int type, int flags, struct prefix_ipv4 *p, struct in_addr *gate, ifindex_t ifindex, vrf_id_t vrf_id, safi_t safi) {
	struct route_table *table;
	struct route_node *rn;
	struct rib *rib;
	struct rib *fib = NULL;
	...
	if(IS_ZEBRA_DEBUG_KERNEL) {
		if(gate) {
			zlog_debug("rib_delete_ipv4(): route delete %s vrf %u via %s ifindex %d", prefix2str(p, buf1, sizeof(buf1)), vrf_id, inet_ntoa(*gate), ifindex);
		} else {
			zlog_debug("rib_delete_ipv4(): route delete %s vrf %u ifindex %d", prefix2str(p, buf1, sizeof(buf1)), vrf_id, ifindex);
		}
	}
	...
	if(!rn) {
		if(IS_ZEBRA_DEBUG_KERNEL) {
			if(gate) {
				zlog_debug("route %s vrf %u via %s ifindex %d doesn't exist in rib", prefix2str(p, buf1, sizeof(buf1)), vrf_id, inet_ntop(AF_INET, gate, buf2, INET_ADDRSTRLEN), ifindex);
			} else {
				zlog_debug("route %s vrf %u ifindex %d doesn't exist in rib", prefix2str(p, buf1, sizeof(buf1)), vrf_id, ifindex);
			}
		}
	...
	if(!same) {
		if(fib && type == ZEBRA_ROUTE_KERNEL && CHECK_FLAG(flags, ZEBRA_FLAG_SELFROUTE)) {
			if(IS_ZEBRA_DEBUG_KERNEL) {
				zlog_debug("Zebra route %s/%d was deleted by others from kernel", inet_ntop(AF_INET, &p->prefix, buf1, INET_ADDRSTRLEN), p->prefixlen);
			}
			...
			if(IS_ZEBRA_DEBUG_KERNEL) {
				if(gate) {
					zlog_debug(
						...
						prefix2str(p, buf1, sizeof(buf1)), vrf_id, inet_ntop(AF_INET, gate, buf2, INET_ADDRSTRLEN), ifindex, type
					);
				} else {
					zlog_debug("route %s vrf %u ifindex %d type %d doesn't exist in rib", prefix2str(p, buf1, sizeof(buf1)), vrf_id, ifindex, type);
				}
			}
			route_unlock_node(rn);
			return ZEBRA_ERR_RTNOEXIST;
...
static void static_install_route(afi_t afi, safi_t safi, struct prefix *p, struct static_route *si) {
	struct rib *rib;
	struct route_node *rn;
	struct route_table *table;
	...
	if(rib) {
		...
		route_unlock_node(rn);
		switch(si->type) {
			case STATIC_IPV4_GATEWAY: rib_nexthop_ipv4_add(rib, &si->addr.ipv4, NULL); break;
			case STATIC_IPV4_IFNAME: rib_nexthop_ifname_add(rib, si->ifname); break;
			case STATIC_IPV4_BLACKHOLE: rib_nexthop_blackhole_add(rib); break;
			case STATIC_IPV6_GATEWAY: rib_nexthop_ipv6_add(rib, &si->addr.ipv6); break;
			case STATIC_IPV6_IFNAME: rib_nexthop_ifname_add(rib, si->ifname); break;
			case STATIC_IPV6_GATEWAY_IFNAME: rib_nexthop_ipv6_ifname_add(rib, &si->addr.ipv6, si->ifname); break;
		}
		rib_queue_add(&zebrad, rn);
	} else {
...
static int static_nexthop_same(struct nexthop *nexthop, struct static_route *si) {
	if(nexthop->type == NEXTHOP_TYPE_IPV4 && si->type == STATIC_IPV4_GATEWAY && IPV4_ADDR_SAME(&nexthop->gate.ipv4, &si->addr.ipv4)) {
		return 1;
	}
	if(nexthop->type == NEXTHOP_TYPE_IFNAME && si->type == STATIC_IPV4_IFNAME && strcmp(nexthop->ifname, si->ifname) == 0) {
		return 1;
	}
	if(nexthop->type == NEXTHOP_TYPE_BLACKHOLE && si->type == STATIC_IPV4_BLACKHOLE) {
		return 1;
	}
	if(nexthop->type == NEXTHOP_TYPE_IPV6 && si->type == STATIC_IPV6_GATEWAY && IPV6_ADDR_SAME(&nexthop->gate.ipv6, &si->addr.ipv6)) {
		return 1;
	}
	if(nexthop->type == NEXTHOP_TYPE_IFNAME && si->type == STATIC_IPV6_IFNAME && strcmp(nexthop->ifname, si->ifname) == 0) {
		return 1;
	}
	if(nexthop->type == NEXTHOP_TYPE_IPV6_IFNAME && si->type == STATIC_IPV6_GATEWAY_IFNAME && IPV6_ADDR_SAME(&nexthop->gate.ipv6, &si->addr.ipv6) && strcmp(nexthop->ifname, si->ifname) == 0) {
		return 1;
	}
	return 0;
}
...
static void static_uninstall_route(afi_t afi, safi_t safi, struct prefix *p, struct static_route *si) {
	struct route_node *rn;
	struct rib *rib;
	struct nexthop *nexthop;
	struct route_table *table;
	...
	RNODE_FOREACH_RIB(rn, rib) {
		...
		if(rib->type == ZEBRA_ROUTE_STATIC && rib->distance == si->distance && rib->tag == si->tag) {
			break;
		}
	}
...
int static_add_ipv4_safi(safi_t safi, struct prefix *p, struct in_addr *gate, const char *ifname, u_char flags, route_tag_t tag, u_char distance, vrf_id_t vrf_id) {
	u_char type = 0;
	struct route_node *rn;
	...
	for(si = rn->info; si; si = si->next) {
		if(type == si->type && (!gate || IPV4_ADDR_SAME(gate, &si->addr.ipv4)) && (!ifname || strcmp(ifname, si->ifname) == 0)) {
...
int static_delete_ipv4_safi(safi_t safi, struct prefix *p, struct in_addr *gate, const char *ifname, route_tag_t tag, u_char distance, vrf_id_t vrf_id) {
	u_char type = 0;
	struct route_node *rn;
	struct static_route *si;
	...
	for(si = rn->info; si; si = si->next) {
		if(type == si->type && (!gate || IPV4_ADDR_SAME(gate, &si->addr.ipv4)) && (!ifname || strcmp(ifname, si->ifname) == 0) && (!tag || (tag == si->tag))) {
			break;
		}
	}
...
int rib_add_ipv6(int type, int flags, struct prefix_ipv6 *p, struct in6_addr *gate, ifindex_t ifindex, vrf_id_t vrf_id, int table_id, u_int32_t metric, u_int32_t mtu, u_char distance, safi_t safi) {
	struct rib *rib;
	struct rib *same = NULL;
	struct route_table *table;
	struct route_node *rn;
...
int rib_add_ipv6_multipath(struct prefix_ipv6 *p, struct rib *rib, safi_t safi) {
	struct route_table *table;
	struct route_node *rn;
	struct rib *same = NULL;
	struct nexthop *nexthop;
...
/* XXX factor with rib_delete_ipv6 */
int rib_delete_ipv6(int type, int flags, struct prefix_ipv6 *p, struct in6_addr *gate, ifindex_t ifindex, vrf_id_t vrf_id, safi_t safi) {
	struct route_table *table;
	struct route_node *rn;
	struct rib *rib;
	struct rib *fib = NULL;
	...
	if(!rn) {
		if(IS_ZEBRA_DEBUG_KERNEL) {
			if(gate) {
				zlog_debug("route %s vrf %u via %s ifindex %d doesn't exist in rib", prefix2str(p, buf1, sizeof(buf1)), vrf_id, inet_ntop(AF_INET6, gate, buf2, INET6_ADDRSTRLEN), ifindex);
			} else {
				zlog_debug("route %s vrf %u ifindex %d doesn't exist in rib", prefix2str(p, buf1, sizeof(buf1)), vrf_id, ifindex);
			}
		}
			...
			if(IS_ZEBRA_DEBUG_KERNEL) {
				if(gate) {
					zlog_debug(
						"route %s vrf %u via %s ifindex %d type %d "
						"doesn't exist in rib",
						prefix2str(p, buf1, sizeof(buf1)), vrf_id, inet_ntop(AF_INET6, gate, buf2, INET6_ADDRSTRLEN), ifindex, type
					);
				} else {
					zlog_debug("route %s vrf %u ifindex %d type %d doesn't exist in rib", prefix2str(p, buf1, sizeof(buf1)), vrf_id, ifindex, type);
				}
...
int static_add_ipv6(struct prefix *p, u_char type, struct in6_addr *gate, const char *ifname, u_char flags, route_tag_t tag, u_char distance, vrf_id_t vrf_id) {
	struct route_node *rn;
	struct static_route *si;
	struct static_route *pp;
	struct static_route *cp;
	...
	/* Do nothing if there is a same static route.  */
	for(si = rn->info; si; si = si->next) {
		if(type == si->type && tag == si->tag && (!gate || IPV6_ADDR_SAME(gate, &si->addr.ipv6)) && (!ifname || strcmp(ifname, si->ifname) == 0)) {
	...
	switch(type) {
		case STATIC_IPV6_GATEWAY: si->addr.ipv6 = *gate; break;
		case STATIC_IPV6_IFNAME: si->ifname = XSTRDUP(MTYPE_TMP, ifname); break;
...
int static_delete_ipv6(struct prefix *p, u_char type, struct in6_addr *gate, const char *ifname, route_tag_t tag, u_char distance, vrf_id_t vrf_id) {
	struct route_node *rn;
	struct static_route *si;
	struct route_table *stable;
	...
	/* Find same static route is the tree */
	for(si = rn->info; si; si = si->next) {
		if(distance == si->distance && type == si->type && (!gate || IPV6_ADDR_SAME(gate, &si->addr.ipv6)) && (!ifname || strcmp(ifname, si->ifname) == 0) && (!tag || (tag == si->tag))) {
			break;
		}
	}
...
unsigned long rib_score_proto(u_char proto) {
	...
	for(iter = vrf_first(); iter != VRF_ITER_INVALID; iter = vrf_next(iter)) {
		if((zvrf = vrf_iter2info(iter)) != NULL) {
			cnt += rib_score_proto_table(proto, zvrf->table[AFI_IP][SAFI_UNICAST]) + rib_score_proto_table(proto, zvrf->table[AFI_IP6][SAFI_UNICAST]);
		}
	}
   ...
   */
	...
	switch(iter->state) {
		...
		case RIB_TABLES_ITER_S_ITERATING:
			...
			while(1) {
				while(iter->afi_safi_ix < (int) ZEBRA_NUM_OF(afi_safis)) {
					table = zebra_vrf_table(afi_safis[iter->afi_safi_ix].afi, afi_safis[iter->afi_safi_ix].safi, iter->vrf_id);
					if(table) {
						break;
					}
	   ...
	   */
		...
		case RIB_TABLES_ITER_S_DONE: return NULL;
	}
...
struct zebra_vrf *zebra_vrf_lookup(vrf_id_t vrf_id) {
...
struct zebra_vrf *zebra_vrf_alloc(vrf_id_t vrf_id) {
	struct zebra_vrf *zvrf;
#ifdef HAVE_NETLINK
	char nl_name[64];
#endif
...
struct route_table *zebra_vrf_static_table(afi_t afi, safi_t safi, vrf_id_t vrf_id) {
	struct zebra_vrf *zvrf = vrf_info_lookup(vrf_id);
