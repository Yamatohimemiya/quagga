...
static void handle_route_entry(mib2_ipRouteEntry_t *routeEntry) {
	...
	rib_add_ipv4(ZEBRA_ROUTE_KERNEL, zebra_flags, &prefix, &gateway, NULL, 0, VRF_DEFAULT, RT_TABLE_MAIN, 0, 0, 0, SAFI_UNICAST);
}
...
void route_read(struct zebra_vrf *zvrf) {
	...
	if((dev = open(_PATH_GETMSG_ROUTE, O_RDWR)) == -1) {
		zlog_warn("can't open %s: %s", _PATH_GETMSG_ROUTE, safe_strerror(errno));
		return;
	}
	...
	while(1) {
		...
		if(retval == 0 && (size_t) msgdata.len >= sizeof(struct T_optmgmt_ack) && TLIack->PRIM_type == T_OPTMGMT_ACK && TLIack->MGMT_flags == T_SUCCESS && MIB2hdr->len == 0) {
			break;
		}

		if((size_t) msgdata.len >= sizeof(struct T_error_ack) && TLIerr->PRIM_type == T_ERROR_ACK) {
			zlog_warn("getmsg(ctl) returned T_ERROR_ACK: %s", safe_strerror((TLIerr->TLI_error == TSYSERR) ? TLIerr->UNIX_error : EPROTO));
			break;
		}
		...
		/* should dump more debugging info to the log statement,
		   ...
		   critical yet. */
		if(retval != MOREDATA || (size_t) msgdata.len < sizeof(struct T_optmgmt_ack) || TLIack->PRIM_type != T_OPTMGMT_ACK || TLIack->MGMT_flags != T_SUCCESS) {
			errno = ENOMSG;
		...
		/* MIB2_IP_21 is the the pseudo-MIB2 ipRouteTable
		   ...
		   you're looking for." */
		process = (MIB2hdr->level == MIB2_IP && MIB2hdr->name == MIB2_IP_21) ? 1 : 0;
		...
		/* getmsg writes the data buffer out completely, not
		   ...
		   retrieving. */
		msgdata.maxlen = sizeof(storage) - (sizeof(storage) % sizeof(mib2_ipRouteEntry_t));
		...
		do {
			...
			if(retval == -1) {
				zlog_warn("getmsg(data) failed: %s", safe_strerror(errno));
				goto exit;
			}
			...
			if(process) {
				...
				lastRouteEntry = (mib2_ipRouteEntry_t *) (msgdata.buf + msgdata.len);
				do {
