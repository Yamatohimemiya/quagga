...
#define IFNUM_BASE 32
	...
	if(sock < 0) {
		zlog_warn("Can't make %s socket stream: %s", (af == AF_INET ? "AF_INET" : "AF_INET6"), safe_strerror(errno));
...
calculate_lifc_len: /* must hold privileges to enter here */
	...
	if(ret < 0) {
		zlog_warn("interface_list_ioctl: SIOCGLIFNUM failed %s", safe_strerror(save_errno));
		close(sock);
		return -1;
	}
   ...
   */
	...
	for(n = 0; n < lifconf.lifc_len; n += sizeof(struct lifreq)) {
       ...
       */
		if(if_get_flags_direct(lifreq->lifr_name, &lifflags, lifreq->lifr_addr.ss_family) || !CHECK_FLAG(lifflags, IFF_UP)) {
			lifreq++;
			continue;
		}

		/* Find the normalised name */
		while((normallen < sizeof(lifreq->lifr_name)) && (*(lifreq->lifr_name + normallen) != '\0') && (*(lifreq->lifr_name + normallen) != ':')) {
			normallen++;
		}
       ...
       */
		if(*(lifreq->lifr_name + normallen) != '\0') {
			if_get_addr(ifp, (struct sockaddr *) &lifreq->lifr_addr, lifreq->lifr_name);
		} else {
...
/* Interface address lookup by ioctl.  This function only looks up
   IPv4 address. */
#define ADDRLEN(sa) (((sa)->sa_family == AF_INET ? sizeof(struct sockaddr_in) : sizeof(struct sockaddr_in6)))

#define SIN(s) ((struct sockaddr_in *) (s))
#define SIN6(s) ((struct sockaddr_in6 *) (s))

/* Retrieve address information for the given ifp */
static int if_get_addr(struct interface *ifp, struct sockaddr *addr, const char *label) {
	int ret;
	struct lifreq lifreq;
   ...
   */
	...
	if(af == AF_INET) {
		...
		if(ret < 0) {
			if(errno != EADDRNOTAVAIL) {
				zlog_warn("SIOCGLIFNETMASK (%s) fail: %s", ifp->name, safe_strerror(errno));
				return ret;
			}
			return 0;
		}
		...
		if(if_ioctl_ipv6(SIOCGLIFSUBNET, (caddr_t) &lifreq) < 0) {
			if(ifp->flags & IFF_POINTOPOINT) {
				...
				zlog_warn("SIOCGLIFSUBNET (%s) fail: %s", ifp->name, safe_strerror(errno));
			}
		} else {
	...
	if(af == AF_INET) {
		connected_add_ipv4(ifp, flags, &SIN(addr)->sin_addr, prefixlen, (struct in_addr *) dest_pnt, label);
	}
#ifdef HAVE_IPV6
	else if(af == AF_INET6) {
		connected_add_ipv6(ifp, flags, &SIN6(addr)->sin6_addr, prefixlen, (struct in6_addr *) dest_pnt, label);
	}
#endif /* HAVE_IPV6 */
...
struct connected *if_lookup_linklocal(struct interface *ifp) {
	...
	for(ALL_LIST_ELEMENTS_RO(ifp->connected, node, ifc)) {
		if((ifc->address->family == AF_INET6) && (IN6_IS_ADDR_LINKLOCAL(&ifc->address->u.prefix6))) {
			return ifc;
		}
	}
#endif /* HAVE_IPV6 */
