...
static void if_zebra_linkdetect_set_val(struct interface *ifp, zebra_if_linkdetect val) {
	switch(val) {
		case IF_LINKDETECT_ON: SET_FLAG(ifp->status, ZEBRA_INTERFACE_LINKDETECTION); break;
		case IF_LINKDETECT_OFF: UNSET_FLAG(ifp->status, ZEBRA_INTERFACE_LINKDETECTION); break;
		default: break;
	}
}

static void if_zebra_linkdetect_set(struct interface *ifp) {
...
static int if_zebra_new_hook(struct interface *ifp) {
	...
	switch(zif_defaults.linkdetect) {
		...
		default: SET_FLAG(ifp->status, ZEBRA_INTERFACE_LINKDETECTION); break;
	}
...
static void if_flags_mangle(struct interface *ifp, uint64_t *newflags) {
	...
	if(CHECK_FLAG(zif->primary_state, IFF_UP) || listcount(ifp->connected) > 0) {
...
static void if_addr_wakeup(struct interface *ifp) {
	...
	for(ALL_LIST_ELEMENTS(ifp->connected, node, nnode, ifc)) {
		...
		if(CHECK_FLAG(ifc->conf, ZEBRA_IFC_CONFIGURED) && !CHECK_FLAG(ifc->conf, ZEBRA_IFC_QUEUED)) {
			/* Address check. */
			if(p->family == AF_INET) {
				if(!if_is_up(ifp)) {
		  ...
		  */
				...
				if(ret < 0) {
					zlog_warn("Can't set interface's address: %s", safe_strerror(errno));
					continue;
				}
...
void if_delete_update(struct interface *ifp) {
	...
	if(ifp->connected) {
		...
		while((node = (last ? last->next : listhead(ifp->connected)))) {
			...
			if(p->family == AF_INET && (rn = route_node_lookup(zebra_if->ipv4_subnets, p))) {
...
static void nd_dump_vty(struct vty *vty, struct interface *ifp) {
	...
	if(rtadv->AdvSendAdvertisements) {
		vty_out(vty, "  ND advertised reachable time is %d milliseconds%s", rtadv->AdvReachableTime, VTY_NEWLINE);
		vty_out(vty, "  ND advertised retransmit interval is %d milliseconds%s", rtadv->AdvRetransTimer, VTY_NEWLINE);
		...
		if(rtadv->AdvDefaultLifetime != -1) {
			vty_out(vty, "  ND router advertisements live for %d seconds%s", rtadv->AdvDefaultLifetime, VTY_NEWLINE);
		} else {
		...
		if(rtadv->AdvHomeAgentFlag) {
			...
			if(rtadv->HomeAgentLifetime != -1) {
				vty_out(vty, "  Home Agent lifetime is %u seconds%s", rtadv->HomeAgentLifetime, VTY_NEWLINE);
			} else {
				vty_out(vty, "  Home Agent lifetime tracks ra-lifetime%s", VTY_NEWLINE);
			}
			vty_out(vty, "  Home Agent preference is %u%s", rtadv->HomeAgentPreference, VTY_NEWLINE);
		}
...
static void if_dump_vty(struct vty *vty, struct interface *ifp) {
	...
	vty_out(vty, "  Link ups:   %s%s", event_counter_format(&zebra_if->up_events), VTY_NEWLINE);
	vty_out(vty, "  Link downs: %s%s", event_counter_format(&zebra_if->down_events), VTY_NEWLINE);
	...
	vty_out(vty, "  index %d metric %d mtu %d ", ifp->ifindex, ifp->metric, ifp->mtu);
#ifdef HAVE_IPV6
	if(ifp->mtu6 != ifp->mtu) {
		vty_out(vty, "mtu6 %d ", ifp->mtu6);
	}
#endif
	vty_out(vty, "%s  flags: %s%s", VTY_NEWLINE, if_flag_dump(ifp->flags), VTY_NEWLINE);
	...
	if(HAS_LINK_PARAMS(ifp)) {
		...
		if(IS_PARAM_SET(iflp, LP_UNRSV_BW)) {
			...
			for(i = 0; i < MAX_CLASS_TYPE; i += 2) {
				vty_out(vty, "      [%d]: %g (Bytes/sec),\t[%d]: %g (Bytes/sec)%s", i, iflp->unrsv_bw[i], i + 1, iflp->unrsv_bw[i + 1], VTY_NEWLINE);
			}
		}
	...
	vty_out(vty,
		...
		"%lu dropped%s",
		ifp->stats.rx_packets, ifp->stats.rx_multicast, ifp->stats.rx_bytes, ifp->stats.rx_dropped, VTY_NEWLINE);

	vty_out(vty,
		"    %lu input errors, %lu length, %lu overrun,"
		" %lu CRC, %lu frame%s",
		ifp->stats.rx_errors, ifp->stats.rx_length_errors, ifp->stats.rx_over_errors, ifp->stats.rx_crc_errors, ifp->stats.rx_frame_errors, VTY_NEWLINE);

	vty_out(vty, "    %lu fifo, %lu missed%s", ifp->stats.rx_fifo_errors, ifp->stats.rx_missed_errors, VTY_NEWLINE);

	vty_out(vty, "    %lu output packets, %lu bytes, %lu dropped%s", ifp->stats.tx_packets, ifp->stats.tx_bytes, ifp->stats.tx_dropped, VTY_NEWLINE);

	vty_out(vty,
		"    %lu output errors, %lu aborted, %lu carrier,"
		" %lu fifo, %lu heartbeat%s",
		ifp->stats.tx_errors, ifp->stats.tx_aborted_errors, ifp->stats.tx_carrier_errors, ifp->stats.tx_fifo_errors, ifp->stats.tx_heartbeat_errors, VTY_NEWLINE);

	vty_out(vty, "    %lu window, %lu collisions%s", ifp->stats.tx_window_errors, ifp->stats.collisions, VTY_NEWLINE);
	...
	vty_out(vty,
		...
		(unsigned long long) ifp->stats.ifi_ipackets, (unsigned long long) ifp->stats.ifi_ibytes, (unsigned long long) ifp->stats.ifi_iqdrops, (unsigned long long) ifp->stats.ifi_imcasts, VTY_NEWLINE);

	vty_out(vty, "    input errors %llu%s", (unsigned long long) ifp->stats.ifi_ierrors, VTY_NEWLINE);

	vty_out(vty,
		"    output packets %llu, bytes %llu,"
		" multicast packets %llu%s",
		(unsigned long long) ifp->stats.ifi_opackets, (unsigned long long) ifp->stats.ifi_obytes, (unsigned long long) ifp->stats.ifi_omcasts, VTY_NEWLINE);

	vty_out(vty, "    output errors %llu%s", (unsigned long long) ifp->stats.ifi_oerrors, VTY_NEWLINE);

	vty_out(vty, "    collisions %llu%s", (unsigned long long) ifp->stats.ifi_collisions, VTY_NEWLINE);
	#else
	/* Statistics print out using sysctl (). */
	vty_out(vty,
		"    input packets %lu, bytes %lu, dropped %lu,"
		" multicast packets %lu%s",
		ifp->stats.ifi_ipackets, ifp->stats.ifi_ibytes, ifp->stats.ifi_iqdrops, ifp->stats.ifi_imcasts, VTY_NEWLINE);

	vty_out(vty, "    input errors %lu%s", ifp->stats.ifi_ierrors, VTY_NEWLINE);

	vty_out(vty, "    output packets %lu, bytes %lu, multicast packets %lu%s", ifp->stats.ifi_opackets, ifp->stats.ifi_obytes, ifp->stats.ifi_omcasts, VTY_NEWLINE);

	vty_out(vty, "    output errors %lu%s", ifp->stats.ifi_oerrors, VTY_NEWLINE);

	vty_out(vty, "    collisions %lu%s", ifp->stats.ifi_collisions, VTY_NEWLINE);
 ...
 */
DEFUN_NOSH(
	zebra_interface, zebra_interface_cmd, "interface IFNAME",
...
ALIAS(zebra_interface, zebra_interface_vrf_cmd, "interface IFNAME " VRF_CMD_STR,
      "Select an interface to configure\n"
      "Interface's name\n" VRF_CMD_HELP_STR)
...
DEFUN(show_interface, show_interface_cmd, "show interface", SHOW_STR "Interface status and configuration\n") {
	struct listnode *node;
	struct interface *ifp;
...
ALIAS(show_interface, show_interface_vrf_cmd, "show interface " VRF_CMD_STR, SHOW_STR "Interface status and configuration\n" VRF_CMD_HELP_STR)

/* Show all interfaces to vty. */
DEFUN(show_interface_vrf_all, show_interface_vrf_all_cmd, "show interface " VRF_ALL_CMD_STR, SHOW_STR "Interface status and configuration\n" VRF_ALL_CMD_HELP_STR) {
	struct listnode *node;
	struct interface *ifp;
	vrf_iter_t iter;
...
DEFUN(show_interface_name, show_interface_name_cmd, "show interface IFNAME",
...
ALIAS(show_interface_name, show_interface_name_vrf_cmd, "show interface IFNAME " VRF_CMD_STR,
      SHOW_STR "Interface status and configuration\n"
	       "Interface name\n" VRF_CMD_HELP_STR)

/* Show specified interface to vty. */
DEFUN(show_interface_name_vrf_all, show_interface_name_vrf_all_cmd, "show interface IFNAME " VRF_ALL_CMD_STR,
      SHOW_STR "Interface status and configuration\n"
	       "Interface name\n" VRF_ALL_CMD_HELP_STR) {
	struct interface *ifp;
	vrf_iter_t iter;
...
DEFUN(show_interface_desc, show_interface_desc_cmd, "show interface description",
      SHOW_STR "Interface status and configuration\n"
...
ALIAS(show_interface_desc, show_interface_desc_vrf_cmd, "show interface description " VRF_CMD_STR,
      SHOW_STR "Interface status and configuration\n"
	       "Interface description\n" VRF_CMD_HELP_STR)

DEFUN(show_interface_desc_vrf_all, show_interface_desc_vrf_all_cmd, "show interface description " VRF_ALL_CMD_STR,
      SHOW_STR "Interface status and configuration\n"
	       "Interface description\n" VRF_ALL_CMD_HELP_STR) {
	vrf_iter_t iter;
	...
	for(iter = vrf_first(); iter != VRF_ITER_INVALID; iter = vrf_next(iter)) {
		if(!list_isempty(vrf_iter2iflist(iter))) {
			vty_out(vty, "%s\tVRF %u%s%s", VTY_NEWLINE, vrf_iter2id(iter), VTY_NEWLINE, VTY_NEWLINE);
...
DEFUN(multicast, multicast_cmd, "multicast", "Set multicast flag to interface\n") {
	int ret;
	struct interface *ifp;
	struct zebra_if *if_data;
...
static int config_write_zebra_if_defaults(struct vty *vty) {
	if(zif_defaults.linkdetect != IF_LINKDETECT_UNSPEC) {
		vty_out(vty, "default link-detect %s%s", zif_defaults.linkdetect == IF_LINKDETECT_ON ? "on" : "off", VTY_NEWLINE);
	}
	return 0;
}

DEFUN(default_linkdetect, default_linkdetect_cmd, "default link-detect (on|off)",
...
DEFUN(linkdetect, linkdetect_cmd, "link-detect [default]",
...
DEFUN(no_linkdetect, no_linkdetect_cmd, "no link-detect", NO_STR "Disable link detection on interface\n") {
	struct interface *ifp;
	struct zebra_if *zif;
...
DEFUN(bandwidth_if, bandwidth_if_cmd, "bandwidth <1-10000000>",
...
DEFUN(no_bandwidth_if, no_bandwidth_if_cmd, "no bandwidth", NO_STR "Set bandwidth informational parameter\n") {
	struct interface *ifp;
...
ALIAS(no_bandwidth_if, no_bandwidth_if_val_cmd, "no bandwidth <1-10000000>",
      NO_STR "Set bandwidth informational parameter\n"
	     "Bandwidth in kilobits\n")
...
static void link_param_cmd_set_uint32(struct interface *ifp, uint32_t *field, uint32_t type, uint32_t value) {
...
static void link_param_cmd_set_float(struct interface *ifp, float *field, uint32_t type, float value) {
	/* Update field as needed */
	if(IS_PARAM_UNSET(ifp->link_params, type) || *field != value) {
		*field = value;
		SET_PARAM(ifp->link_params, type);
...
DEFUN(link_params, link_params_cmd, "link-params", LINK_PARAMS_STR) {
	vty->node = LINK_PARAMS_NODE;

	return CMD_SUCCESS;
}

DEFUN(exit_link_params, exit_link_params_cmd, "exit-link-params", "Exit from Link Params configuration mode\n") {
...
DEFUN(link_params_enable, link_params_enable_cmd, "enable", "Activate link parameters on this interface\n") {
...
DEFUN(no_link_params_enable, no_link_params_enable_cmd, "no enable", NO_STR "Disable link parameters on this interface\n") {
	struct interface *ifp = (struct interface *) vty->index;
...
/* STANDARD TE metrics */
DEFUN(link_params_metric, link_params_metric_cmd, "metric <0-4294967295>",
...
DEFUN(no_link_params_metric, no_link_params_metric_cmd, "no metric", NO_STR "Disable Link Metric on this interface\n") {
	struct interface *ifp = (struct interface *) vty->index;
...
DEFUN(link_params_maxbw, link_params_maxbw_cmd, "max-bw BANDWIDTH",
	...
	if(sscanf(argv[0], "%g", &bw) != 1) {
		vty_out(vty, "link_params_maxbw: fscanf: %s%s", safe_strerror(errno), VTY_NEWLINE);
		return CMD_WARNING;
	}

	/* Check that Maximum bandwidth is not lower than other bandwidth parameters */
	if((bw <= iflp->max_rsv_bw) || (bw <= iflp->unrsv_bw[0]) || (bw <= iflp->unrsv_bw[1]) || (bw <= iflp->unrsv_bw[2]) || (bw <= iflp->unrsv_bw[3]) || (bw <= iflp->unrsv_bw[4]) || (bw <= iflp->unrsv_bw[5]) || (bw <= iflp->unrsv_bw[6])
	   || (bw <= iflp->unrsv_bw[7]) || (bw <= iflp->ava_bw) || (bw <= iflp->res_bw) || (bw <= iflp->use_bw)) {
		vty_out(vty, "Maximum Bandwidth could not be lower than others bandwidth%s", VTY_NEWLINE);
		return CMD_WARNING;
	}
...
DEFUN(link_params_max_rsv_bw, link_params_max_rsv_bw_cmd, "max-rsv-bw BANDWIDTH",
	...
	if(sscanf(argv[0], "%g", &bw) != 1) {
		vty_out(vty, "link_params_max_rsv_bw: fscanf: %s%s", safe_strerror(errno), VTY_NEWLINE);
		return CMD_WARNING;
	}

	/* Check that bandwidth is not greater than maximum bandwidth parameter */
	if(bw > iflp->max_bw) {
		vty_out(vty, "Maximum Reservable Bandwidth could not be greater than Maximum Bandwidth (%g)%s", iflp->max_bw, VTY_NEWLINE);
		return CMD_WARNING;
	}
...
DEFUN(link_params_unrsv_bw, link_params_unrsv_bw_cmd, "unrsv-bw <0-7> BANDWIDTH",
	...
	if(sscanf(argv[0], "%d", &priority) != 1) {
		vty_out(vty, "link_params_unrsv_bw: fscanf: %s%s", safe_strerror(errno), VTY_NEWLINE);
		return CMD_WARNING;
	}
	...
	/* Check that bandwidth is not greater than maximum bandwidth parameter */
	if(bw > iflp->max_bw) {
		vty_out(vty, "UnReserved Bandwidth could not be greater than Maximum Bandwidth (%g)%s", iflp->max_bw, VTY_NEWLINE);
		return CMD_WARNING;
	}
...
DEFUN(link_params_admin_grp, link_params_admin_grp_cmd, "admin-grp BITPATTERN",
	...
	if(sscanf(argv[0], "0x%lx", &value) != 1) {
		vty_out(vty, "link_params_admin_grp: fscanf: %s%s", safe_strerror(errno), VTY_NEWLINE);
		return CMD_WARNING;
	}
...
DEFUN(no_link_params_admin_grp, no_link_params_admin_grp_cmd, "no admin-grp", NO_STR "Disable Administrative group membership on this interface\n") {
	struct interface *ifp = (struct interface *) vty->index;
...
DEFUN(link_params_inter_as, link_params_inter_as_cmd, "neighbor A.B.C.D as <1-4294967295>",
	...
	if(IS_PARAM_UNSET(iflp, LP_RMT_AS) || iflp->rmt_as != as || iflp->rmt_ip.s_addr != addr.s_addr) {
		iflp->rmt_as = as;
...
DEFUN(no_link_params_inter_as, no_link_params_inter_as_cmd, "no neighbor", NO_STR "Remove Neighbor IP address and AS number for Inter-AS TE\n") {
	struct interface *ifp = (struct interface *) vty->index;
	struct if_link_params *iflp = if_link_params_get(ifp);
...
DEFUN(link_params_delay, link_params_delay_cmd, "delay <0-16777215>",
	...
	switch(argc) {
		case 1:
			...
			if(IS_PARAM_SET(iflp, LP_MM_DELAY) && (delay <= iflp->min_delay || delay >= iflp->max_delay)) {
				vty_out(vty, "Average delay should be comprise between Min (%d) and Max (%d) delay%s", iflp->min_delay, iflp->max_delay, VTY_NEWLINE);
				return CMD_WARNING;
			}
			...
			break;
		case 2:
			vty_out(vty, "You should specify both Minimum and Maximum delay with Average delay%s", VTY_NEWLINE);
			return CMD_WARNING;
			break;
		case 3:
			...
			if(delay <= low || delay >= high) {
				vty_out(vty, "Average delay should be comprise between Min (%d) and Max (%d) delay%s", low, high, VTY_NEWLINE);
				return CMD_WARNING;
			}
			/* Update Delays if needed */
			if(IS_PARAM_UNSET(iflp, LP_DELAY) || IS_PARAM_UNSET(iflp, LP_MM_DELAY) || iflp->av_delay != delay || iflp->min_delay != low || iflp->max_delay != high) {
				iflp->av_delay = delay;
				SET_PARAM(iflp, LP_DELAY);
				iflp->min_delay = low;
				iflp->max_delay = high;
			...
			break;
		default: return CMD_WARNING; break;
	}
...
ALIAS(link_params_delay, link_params_delay_mm_cmd, "delay <0-16777215> min <0-16777215> max <0-16777215>",
...
DEFUN(link_params_delay_var, link_params_delay_var_cmd, "delay-variation <0-16777215>",
...
DEFUN(no_link_params_delay_var, no_link_params_delay_var_cmd, "no delay-variation", NO_STR "Disable Unidirectional Delay Variation on this interface\n") {
	struct interface *ifp = (struct interface *) vty->index;
...
DEFUN(link_params_pkt_loss, link_params_pkt_loss_cmd, "packet-loss PERCENTAGE",
	...
	if(sscanf(argv[0], "%g", &fval) != 1) {
		vty_out(vty, "link_params_pkt_loss: fscanf: %s%s", safe_strerror(errno), VTY_NEWLINE);
		return CMD_WARNING;
	}
...
DEFUN(no_link_params_pkt_loss, no_link_params_pkt_loss_cmd, "no packet-loss", NO_STR "Disable Unidirectional Link Packet Loss on this interface\n") {
	struct interface *ifp = (struct interface *) vty->index;
...
DEFUN(link_params_res_bw, link_params_res_bw_cmd, "res-bw BANDWIDTH",
	...
	if(sscanf(argv[0], "%g", &bw) != 1) {
		vty_out(vty, "link_params_res_bw: fscanf: %s%s", safe_strerror(errno), VTY_NEWLINE);
		return CMD_WARNING;
	}

	/* Check that bandwidth is not greater than maximum bandwidth parameter */
	if(bw > iflp->max_bw) {
		vty_out(vty, "Residual Bandwidth could not be greater than Maximum Bandwidth (%g)%s", iflp->max_bw, VTY_NEWLINE);
		return CMD_WARNING;
	}
...
DEFUN(no_link_params_res_bw, no_link_params_res_bw_cmd, "no res-bw", NO_STR "Disable Unidirectional Residual Bandwidth on this interface\n") {
	struct interface *ifp = (struct interface *) vty->index;
...
DEFUN(link_params_ava_bw, link_params_ava_bw_cmd, "ava-bw BANDWIDTH",
	...
	if(sscanf(argv[0], "%g", &bw) != 1) {
		vty_out(vty, "link_params_ava_bw: fscanf: %s%s", safe_strerror(errno), VTY_NEWLINE);
		return CMD_WARNING;
	}

	/* Check that bandwidth is not greater than maximum bandwidth parameter */
	if(bw > iflp->max_bw) {
		vty_out(vty, "Available Bandwidth could not be greater than Maximum Bandwidth (%g)%s", iflp->max_bw, VTY_NEWLINE);
		return CMD_WARNING;
	}
...
DEFUN(no_link_params_ava_bw, no_link_params_ava_bw_cmd, "no ava-bw", NO_STR "Disable Unidirectional Available Bandwidth on this interface\n") {
	struct interface *ifp = (struct interface *) vty->index;
...
DEFUN(link_params_use_bw, link_params_use_bw_cmd, "use-bw BANDWIDTH",
	...
	if(sscanf(argv[0], "%g", &bw) != 1) {
		vty_out(vty, "link_params_use_bw: fscanf: %s%s", safe_strerror(errno), VTY_NEWLINE);
		return CMD_WARNING;
	}

	/* Check that bandwidth is not greater than maximum bandwidth parameter */
	if(bw > iflp->max_bw) {
		vty_out(vty, "Utilised Bandwidth could not be greater than Maximum Bandwidth (%g)%s", iflp->max_bw, VTY_NEWLINE);
		return CMD_WARNING;
	}
...
DEFUN(no_link_params_use_bw, no_link_params_use_bw_cmd, "no use-bw", NO_STR "Disable Unidirectional Utilised Bandwidth on this interface\n") {
	struct interface *ifp = (struct interface *) vty->index;
...
static int ip_address_install(struct vty *vty, struct interface *ifp, const char *addr_str, const char *peer_str, const char *label) {
	struct zebra_if *if_data;
	...
	if(!CHECK_FLAG(ifc->conf, ZEBRA_IFC_QUEUED) && CHECK_FLAG(ifp->status, ZEBRA_INTERFACE_ACTIVE) && !(if_data && if_data->shutdown == IF_ZEBRA_SHUTDOWN_ON)) {
...
static int ip_address_uninstall(struct vty *vty, struct interface *ifp, const char *addr_str, const char *peer_str, const char *label) {
	struct prefix_ipv4 cp;
	struct connected *ifc;
	int ret;
...
DEFUN(ip_address, ip_address_cmd, "ip address A.B.C.D/M",
...
DEFUN(no_ip_address, no_ip_address_cmd, "no ip address A.B.C.D/M",
...
#ifdef HAVE_NETLINK
DEFUN(ip_address_label, ip_address_label_cmd, "ip address A.B.C.D/M label LINE",
      "Interface Internet Protocol config commands\n"
      "Set the IP address of an interface\n"
      "IP address (e.g. 10.0.0.1/8)\n"
...
DEFUN(no_ip_address_label, no_ip_address_label_cmd, "no ip address A.B.C.D/M label LINE",
      NO_STR "Interface Internet Protocol config commands\n"
	     "Set the IP address of an interface\n"
	     "IP address (e.g. 10.0.0.1/8)\n"
	     "Label of this address\n"
...
#ifdef HAVE_IPV6
static int ipv6_address_install(struct vty *vty, struct interface *ifp, const char *addr_str, const char *peer_str, const char *label, int secondary) {
	struct zebra_if *if_data;
...
static int ipv6_address_uninstall(struct vty *vty, struct interface *ifp, const char *addr_str, const char *peer_str, const char *label, int secondry) {
	struct prefix_ipv6 cp;
	struct connected *ifc;
	int ret;
...
DEFUN(ipv6_address, ipv6_address_cmd, "ipv6 address X:X::X:X/M",
...
DEFUN(no_ipv6_address, no_ipv6_address_cmd, "no ipv6 address X:X::X:X/M",
...
static int link_params_config_write(struct vty *vty, struct interface *ifp) {
	...
	if(IS_PARAM_SET(iflp, LP_UNRSV_BW)) {
		for(i = 0; i < 8; i++) {
			vty_out(vty, "  unrsv-bw %d %g%s", i, iflp->unrsv_bw[i], VTY_NEWLINE);
		}
	}
	if(IS_PARAM_SET(iflp, LP_ADM_GRP)) {
	...
	if(IS_PARAM_SET(iflp, LP_RMT_AS)) {
		vty_out(vty, "  neighbor %s as %u%s", inet_ntoa(iflp->rmt_ip), iflp->rmt_as, VTY_NEWLINE);
	}
	...
	for(iter = vrf_first(); iter != VRF_ITER_INVALID; iter = vrf_next(iter)) {
		for(ALL_LIST_ELEMENTS_RO(vrf_iter2iflist(iter), node, ifp)) {
			...
			if(ifp->vrf_id == VRF_DEFAULT) {
				...
				vty_out(vty, "interface %s vrf %u%s", ifp->name, ifp->vrf_id, VTY_NEWLINE);
			}
	 ...
	 while processing config script */
			...
			switch(if_data->linkdetect) {
				case IF_LINKDETECT_ON: vty_out(vty, " link-detect%s", VTY_NEWLINE); break;
				case IF_LINKDETECT_OFF: vty_out(vty, " no link-detect%s", VTY_NEWLINE); break;
				default: break;
			}
			...
			for(ALL_LIST_ELEMENTS_RO(ifp->connected, addrnode, ifc)) {
				if(CHECK_FLAG(ifc->conf, ZEBRA_IFC_CONFIGURED)) {
					...
					p = ifc->address;
					vty_out(vty, " ip%s address %s", p->family == AF_INET ? "" : "v6", prefix2str(p, buf, sizeof(buf)));
			...
			if(if_data) {
				if(if_data->multicast != IF_ZEBRA_MULTICAST_UNSPEC) {
					vty_out(vty, " %smulticast%s", if_data->multicast == IF_ZEBRA_MULTICAST_ON ? "" : "no ", VTY_NEWLINE);
				}
			}
