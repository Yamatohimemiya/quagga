...
void ospf_apiclient_handle_reply(struct ospf_apiclient *oclient, struct msg *msg);
void ospf_apiclient_handle_update_notify(struct ospf_apiclient *oclient, struct msg *msg);
void ospf_apiclient_handle_delete_notify(struct ospf_apiclient *oclient, struct msg *msg);
...
struct ospf_apiclient *ospf_apiclient_connect(char *host, int syncport) {
	...
	if(async_server_sock < 0) {
		fprintf(stderr, "ospf_apiclient_connect: creating async socket failed\n");
		return NULL;
	}
	...
	ret = setsockopt(async_server_sock, SOL_SOCKET, SO_REUSEADDR, (void *) &on, sizeof(on));
	if(ret < 0) {
...
#ifdef SO_REUSEPORT
	ret = setsockopt(async_server_sock, SOL_SOCKET, SO_REUSEPORT, (void *) &on, sizeof(on));
	if(ret < 0) {
	...
	if(fd1 < 0) {
		fprintf(stderr, "ospf_apiclient_connect: creating sync socket failed\n");
		return NULL;
	}

	/* Reuse addr and port */
	ret = setsockopt(fd1, SOL_SOCKET, SO_REUSEADDR, (void *) &on, sizeof(on));
	if(ret < 0) {
		fprintf(stderr, "ospf_apiclient_connect: SO_REUSEADDR failed\n");
		close(fd1);
		return NULL;
...
#ifdef SO_REUSEPORT
	ret = setsockopt(fd1, SOL_SOCKET, SO_REUSEPORT, (void *) &on, sizeof(on));
	if(ret < 0) {
		fprintf(stderr, "ospf_apiclient_connect: SO_REUSEPORT failed\n");
		close(fd1);
		return NULL;
	...
	ret = connect(fd1, (struct sockaddr *) &myaddr_sync, sizeof(struct sockaddr_in));
	if(ret < 0) {
	...
	fd2 = accept(async_server_sock, (struct sockaddr *) &peeraddr, &peeraddrlen);
	if(fd2 < 0) {
...
static int ospf_apiclient_send_request(struct ospf_apiclient *oclient, struct msg *msg) {
	u_int32_t reqseq;
 ...
 */
int ospf_apiclient_register_opaque_type(struct ospf_apiclient *cl, u_char ltype, u_char otype) {
	struct msg *msg;
	int rc;

	/* just put 1 as a sequence number. */
	msg = new_msg_register_opaque_type(ospf_apiclient_get_seqnr(), ltype, otype);
	if(!msg) {
...
int ospf_apiclient_lsa_originate(struct ospf_apiclient *oclient, struct in_addr ifaddr, struct in_addr area_id, u_char lsa_type, u_char opaque_type, u_int32_t opaque_id, void *opaquedata, int opaquelen) {
	struct msg *msg;
	int rc;
	...
	memcpy(((u_char *) lsah) + sizeof(struct lsa_header), opaquedata, opaquelen);

	msg = new_msg_originate_request(ospf_apiclient_get_seqnr(), ifaddr, area_id, lsah);
	if(!msg) {
...
int ospf_apiclient_lsa_delete(struct ospf_apiclient *oclient, struct in_addr area_id, u_char lsa_type, u_char opaque_type, u_int32_t opaque_id) {
	struct msg *msg;
	int rc;
   ...
   * to network byte order by new_msg_delete_request */
	msg = new_msg_delete_request(ospf_apiclient_get_seqnr(), area_id, lsa_type, opaque_type, opaque_id);
...
static void ospf_apiclient_handle_ready(struct ospf_apiclient *oclient, struct msg *msg) {
...
static void ospf_apiclient_handle_new_if(struct ospf_apiclient *oclient, struct msg *msg) {
	struct msg_new_if *n;
...
static void ospf_apiclient_handle_del_if(struct ospf_apiclient *oclient, struct msg *msg) {
	struct msg_del_if *d;
...
static void ospf_apiclient_handle_ism_change(struct ospf_apiclient *oclient, struct msg *msg) {
...
static void ospf_apiclient_handle_nsm_change(struct ospf_apiclient *oclient, struct msg *msg) {
...
static void ospf_apiclient_handle_lsa_update(struct ospf_apiclient *oclient, struct msg *msg) {
	...
	if(oclient->update_notify) {
		(oclient->update_notify)(cn->ifaddr, cn->area_id, cn->is_self_originated, lsa);
	}
...
static void ospf_apiclient_handle_lsa_delete(struct ospf_apiclient *oclient, struct msg *msg) {
	struct msg_lsa_change_notify *cn;
	struct lsa_header *lsa;
	int lsalen;
	...
	if(oclient->delete_notify) {
		(oclient->delete_notify)(cn->ifaddr, cn->area_id, cn->is_self_originated, lsa);
	}
...
static void ospf_apiclient_msghandle(struct ospf_apiclient *oclient, struct msg *msg) {
	/* Call message handler function. */
	switch(msg->hdr.msgtype) {
		case MSG_READY_NOTIFY: ospf_apiclient_handle_ready(oclient, msg); break;
		case MSG_NEW_IF: ospf_apiclient_handle_new_if(oclient, msg); break;
		case MSG_DEL_IF: ospf_apiclient_handle_del_if(oclient, msg); break;
		case MSG_ISM_CHANGE: ospf_apiclient_handle_ism_change(oclient, msg); break;
		case MSG_NSM_CHANGE: ospf_apiclient_handle_nsm_change(oclient, msg); break;
		case MSG_LSA_UPDATE_NOTIFY: ospf_apiclient_handle_lsa_update(oclient, msg); break;
		case MSG_LSA_DELETE_NOTIFY: ospf_apiclient_handle_lsa_delete(oclient, msg); break;
		default: fprintf(stderr, "ospf_apiclient_read: Unknown message type: %d\n", msg->hdr.msgtype); break;
	}
}
...
void ospf_apiclient_register_callback(
	struct ospf_apiclient *oclient, void (*ready_notify)(u_char lsa_type, u_char opaque_type, struct in_addr addr), void (*new_if)(struct in_addr ifaddr, struct in_addr area_id), void (*del_if)(struct in_addr ifaddr),
	void (*ism_change)(struct in_addr ifaddr, struct in_addr area_id, u_char status), void (*nsm_change)(struct in_addr ifaddr, struct in_addr nbraddr, struct in_addr router_id, u_char status),
	void (*update_notify)(struct in_addr ifaddr, struct in_addr area_id, u_char self_origin, struct lsa_header *lsa), void (*delete_notify)(struct in_addr ifaddr, struct in_addr area_id, u_char self_origin, struct lsa_header *lsa)
) {
	assert(oclient);
