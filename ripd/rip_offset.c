...
static int rip_offset_list_unset(struct vty *vty, const char *alist, const char *direct_str, const char *metric_str, const char *ifname) {
	int direct;
	int metric;
	struct rip_offset_list *offset;
	...
	if(offset) {
		...
		if(offset->direct[RIP_OFFSET_LIST_IN].alist_name == NULL && offset->direct[RIP_OFFSET_LIST_OUT].alist_name == NULL) {
...
int rip_offset_list_apply_in(struct prefix_ipv4 *p, struct interface *ifp, u_int32_t *metric) {
	struct rip_offset_list *offset;
	struct access_list *alist;
...
/* If metric is modifed return 1. */
int rip_offset_list_apply_out(struct prefix_ipv4 *p, struct interface *ifp, u_int32_t *metric) {
	struct rip_offset_list *offset;
	struct access_list *alist;
...
DEFUN(rip_offset_list, rip_offset_list_cmd, "offset-list WORD (in|out) <0-16>",
      "Modify RIP metric\n"
      "Access-list name\n"
...
DEFUN(no_rip_offset_list_ifname, no_rip_offset_list_ifname_cmd, "no offset-list WORD (in|out) <0-16> IFNAME",
      NO_STR "Modify RIP metric\n"
	     "Access-list name\n"
	     "For incoming updates\n"
	     "For outgoing updates\n"
...
static int offset_list_cmp(struct rip_offset_list *o1, struct rip_offset_list *o2) {
	...
	for(ALL_LIST_ELEMENTS(rip_offset_list_master, node, nnode, offset)) {
		if(!offset->ifname) {
			if(offset->direct[RIP_OFFSET_LIST_IN].alist_name) {
				vty_out(vty, " offset-list %s in %d%s", offset->direct[RIP_OFFSET_LIST_IN].alist_name, offset->direct[RIP_OFFSET_LIST_IN].metric, VTY_NEWLINE);
			}
			if(offset->direct[RIP_OFFSET_LIST_OUT].alist_name) {
				vty_out(vty, " offset-list %s out %d%s", offset->direct[RIP_OFFSET_LIST_OUT].alist_name, offset->direct[RIP_OFFSET_LIST_OUT].metric, VTY_NEWLINE);
			}
		} else {
			if(offset->direct[RIP_OFFSET_LIST_IN].alist_name) {
				vty_out(vty, " offset-list %s in %d %s%s", offset->direct[RIP_OFFSET_LIST_IN].alist_name, offset->direct[RIP_OFFSET_LIST_IN].metric, offset->ifname, VTY_NEWLINE);
			}
			if(offset->direct[RIP_OFFSET_LIST_OUT].alist_name) {
				vty_out(vty, " offset-list %s out %d %s%s", offset->direct[RIP_OFFSET_LIST_OUT].alist_name, offset->direct[RIP_OFFSET_LIST_OUT].metric, offset->ifname, VTY_NEWLINE);
			}
		}
	}
