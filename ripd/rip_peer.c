...
static struct rip_peer *rip_peer_get(struct in_addr *addr) {
	...
	peer->t_timeout = thread_add_timer(master, rip_peer_timeout, peer, RIP_PEER_TIMER_DEFAULT);
...
#define RIP_UPTIME_LEN 25
	...
	for(ALL_LIST_ELEMENTS(peer_list, node, nnode, peer)) {
		vty_out(vty, "    %-16s %9d %9d %9d   %s%s", inet_ntoa(peer->addr), peer->recv_badpackets, peer->recv_badroutes, ZEBRA_RIP_DISTANCE_DEFAULT, rip_peer_uptime(peer, timebuf, RIP_UPTIME_LEN), VTY_NEWLINE);
	}
}
