...
/* Hook functions. */
static u_char *rip2Globals(struct variable *, oid[], size_t *, int, size_t *, WriteMethod **);
static u_char *rip2IfStatEntry(struct variable *, oid[], size_t *, int, size_t *, WriteMethod **);
static u_char *rip2IfConfAddress(struct variable *, oid[], size_t *, int, size_t *, WriteMethod **);
static u_char *rip2PeerTable(struct variable *, oid[], size_t *, int, size_t *, WriteMethod **);
...
struct variable rip_variables[] = {
 ...
 /* RIP Interface Tables. */
	...
	{ RIP2IFCONFADDRESS,
	  IPADDRESS,			   RONLY,
	  rip2IfConfAddress,					      /* RIP Interface Configuration Table. */
	  3,				     { 3, 1, 1 }},
...
static u_char *rip2Globals(struct variable *v, oid name[], size_t *length, int exact, size_t *var_len, WriteMethod **write_method) {
	...
	switch(v->magic) {
		case RIP2GLOBALROUTECHANGES: return SNMP_INTEGER(rip_global_route_changes); break;
		case RIP2GLOBALQUERIES: return SNMP_INTEGER(rip_global_queries); break;
		default: return NULL; break;
	}
	return NULL;
}

void rip_ifaddr_add(struct interface *ifp, struct connected *ifc) {
	struct prefix *p;
...
void rip_ifaddr_delete(struct interface *ifp, struct connected *ifc) {
	struct prefix *p;
	struct route_node *rn;
	struct interface *i;
...
static struct interface *rip2IfLookup(struct variable *v, oid name[], size_t *length, struct in_addr *addr, int exact) {
	int len;
	struct interface *ifp;
...
static struct rip_peer *rip2PeerLookup(struct variable *v, oid name[], size_t *length, struct in_addr *addr, int exact) {
	int len;
	...
	if(exact) {
		...
		peer = rip_peer_lookup(addr);
		if(peer) {
			if((len < (int) sizeof(struct in_addr) + 1) || (peer->domain > (int) name[v->namelen + sizeof(struct in_addr)])) {
...
static u_char *rip2IfStatEntry(struct variable *v, oid name[], size_t *length, int exact, size_t *var_len, WriteMethod **write_method) {
	struct interface *ifp;
	...
	switch(v->magic) {
		case RIP2IFSTATADDRESS: return SNMP_IPADDRESS(addr); break;
		case RIP2IFSTATRCVBADPACKETS: *var_len = sizeof(long); return (u_char *) &ri->recv_badpackets;

		case RIP2IFSTATRCVBADROUTES: *var_len = sizeof(long); return (u_char *) &ri->recv_badroutes;

		case RIP2IFSTATSENTUPDATES: *var_len = sizeof(long); return (u_char *) &ri->sent_updates;
...
static long rip2IfConfReceive(struct rip_interface *ri) {
	...
	recvv = (ri->ri_receive == RI_RIP_UNSPEC) ? rip->version_recv : ri->ri_receive;
...
static u_char *rip2IfConfAddress(struct variable *v, oid name[], size_t *length, int exact, size_t *val_len, WriteMethod **write_method) {
	static struct in_addr addr;
	...
	if(smux_header_table(v, name, length, exact, val_len, write_method) == MATCH_FAILED) {
		return NULL;
	}
	...
	switch(v->magic) {
		case RIP2IFCONFADDRESS: *val_len = sizeof(struct in_addr); return (u_char *) &addr;

		case RIP2IFCONFDOMAIN: *val_len = 2; return (u_char *) &domain;
		...
		case RIP2IFCONFAUTHKEY: *val_len = 0; return (u_char *) &domain;
		case RIP2IFCONFSEND:
		...
		case RIP2IFCONFSTATUS:
			*val_len = sizeof(long);
			v->type = ASN_INTEGER;
			return (u_char *) &valid;
		case RIP2IFCONFSRCADDRESS: *val_len = sizeof(struct in_addr); return (u_char *) &addr;
...
static u_char *rip2PeerTable(struct variable *v, oid name[], size_t *length, int exact, size_t *val_len, WriteMethod **write_method) {
	static struct in_addr addr;
	static int domain = 0;
	static int version;
	...
	switch(v->magic) {
		case RIP2PEERADDRESS: *val_len = sizeof(struct in_addr); return (u_char *) &peer->addr;
      ...
      *val_len = sizeof (time_t);
      ...
      return (u_char *) &uptime;
		...
		case RIP2PEERRCVBADPACKETS: *val_len = sizeof(int); return (u_char *) &peer->recv_badpackets;

		case RIP2PEERRCVBADROUTES: *val_len = sizeof(int); return (u_char *) &peer->recv_badroutes;
