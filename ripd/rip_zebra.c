...
static void rip_zebra_ipv4_send(struct route_node *rp, u_char cmd) {
	...
	if(vrf_bitmap_check(zclient->redist[ZEBRA_ROUTE_RIP], VRF_DEFAULT)) {
		...
		if(IS_RIP_DEBUG_ZEBRA) {
			if(rip->ecmp) {
				zlog_debug("%s: %s/%d nexthops %d", (cmd == ZEBRA_IPV4_ROUTE_ADD) ? "Install into zebra" : "Delete from zebra", inet_ntoa(rp->p.u.prefix4), rp->p.prefixlen, count);
			} else {
				zlog_debug("%s: %s/%d", (cmd == ZEBRA_IPV4_ROUTE_ADD) ? "Install into zebra" : "Delete from zebra", inet_ntoa(rp->p.u.prefix4), rp->p.prefixlen);
			}
		}
...
static int rip_zebra_read_ipv4(int command, struct zclient *zclient, zebra_size_t length, vrf_id_t vrf_id) {
	struct stream *s;
	struct zapi_ipv4 api;
	...
	if(command == ZEBRA_IPV4_ROUTE_ADD) {
		rip_redistribute_add(api.type, RIP_ROUTE_REDISTRIBUTE, &p, ifindex, &nexthop, api.metric, api.distance, api.tag);
	} else {
...
#define DONT_CARE_METRIC_RIP 17
	if(metric != DONT_CARE_METRIC_RIP && rip->route_map[type].metric != metric) {
		return 1;
	}
...
static int rip_routemap_unset(int type, const char *name) {
	if(!rip->route_map[type].name || (name != NULL && strcmp(rip->route_map[type].name, name))) {
		return 1;
	}
...
void rip_redistribute_clean(void) {
	...
	for(i = 0; redist_type[i].str; i++) {
		if(vrf_bitmap_check(zclient->redist[redist_type[i].type], VRF_DEFAULT)) {
			if(zclient->sock > 0) {
				zebra_redistribute_send(ZEBRA_REDISTRIBUTE_DELETE, zclient, redist_type[i].type, VRF_DEFAULT);
			}
...
DEFUN(rip_redistribute_rip, rip_redistribute_rip_cmd, "redistribute rip",
...
DEFUN(no_rip_redistribute_rip, no_rip_redistribute_rip_cmd, "no redistribute rip",
...
DEFUN(rip_redistribute_type, rip_redistribute_type_cmd, "redistribute " QUAGGA_REDIST_STR_RIPD, REDIST_STR QUAGGA_REDIST_HELP_STR_RIPD) {
	int i;

	for(i = 0; redist_type[i].str; i++) {
		if(strncmp(redist_type[i].str, argv[0], redist_type[i].str_min_len) == 0) {
			zclient_redistribute(ZEBRA_REDISTRIBUTE_ADD, zclient, redist_type[i].type, VRF_DEFAULT);
			return CMD_SUCCESS;
		}
	}
...
DEFUN(no_rip_redistribute_type, no_rip_redistribute_type_cmd, "no redistribute " QUAGGA_REDIST_STR_RIPD, NO_STR REDIST_STR QUAGGA_REDIST_HELP_STR_RIPD) {
	int i;
...
DEFUN(rip_redistribute_type_routemap, rip_redistribute_type_routemap_cmd, "redistribute " QUAGGA_REDIST_STR_RIPD " route-map WORD",
      REDIST_STR QUAGGA_REDIST_HELP_STR_RIPD "Route map reference\n"
...
DEFUN(no_rip_redistribute_type_routemap, no_rip_redistribute_type_routemap_cmd, "no redistribute " QUAGGA_REDIST_STR_RIPD " route-map WORD",
      NO_STR REDIST_STR QUAGGA_REDIST_HELP_STR_RIPD "Route map reference\n"
						    "Pointer to route-map entries\n") {
	int i;
...
DEFUN(rip_redistribute_type_metric, rip_redistribute_type_metric_cmd, "redistribute " QUAGGA_REDIST_STR_RIPD " metric <0-16>",
      REDIST_STR QUAGGA_REDIST_HELP_STR_RIPD "Metric\n"
					     "Metric value\n") {
	int i;
	int metric;
...
DEFUN(no_rip_redistribute_type_metric, no_rip_redistribute_type_metric_cmd, "no redistribute " QUAGGA_REDIST_STR_RIPD " metric <0-16>",
      NO_STR REDIST_STR QUAGGA_REDIST_HELP_STR_RIPD "Metric\n"
						    "Metric value\n") {
	int i;
...
DEFUN(rip_redistribute_type_metric_routemap, rip_redistribute_type_metric_routemap_cmd, "redistribute " QUAGGA_REDIST_STR_RIPD " metric <0-16> route-map WORD",
      REDIST_STR QUAGGA_REDIST_HELP_STR_RIPD "Metric\n"
					     "Metric value\n"
					     "Route map reference\n"
					     "Pointer to route-map entries\n") {
...
DEFUN(no_rip_redistribute_type_metric_routemap, no_rip_redistribute_type_metric_routemap_cmd, "no redistribute " QUAGGA_REDIST_STR_RIPD " metric <0-16> route-map WORD",
      NO_STR REDIST_STR QUAGGA_REDIST_HELP_STR_RIPD "Metric\n"
						    "Metric value\n"
						    "Route map reference\n"
						    "Pointer to route-map entries\n") {
...
DEFUN(rip_default_information_originate, rip_default_information_originate_cmd, "default-information originate",
	...
	if(!rip->default_information) {
		...
		rip_redistribute_add(ZEBRA_ROUTE_RIP, RIP_ROUTE_DEFAULT, &p, 0, NULL, 0, 0, 0);
	}

	return CMD_SUCCESS;
}

DEFUN(no_rip_default_information_originate, no_rip_default_information_originate_cmd, "no default-information originate",
      NO_STR "Control distribution of default route\n"
	     "Distribute a default route\n") {
	struct prefix_ipv4 p;
...
int config_write_rip_redistribute(struct vty *vty, int config_mode) {
	...
	for(i = 0; i < ZEBRA_ROUTE_MAX; i++) {
		if(i != zclient->redist_default && vrf_bitmap_check(zclient->redist[i], VRF_DEFAULT)) {
			if(config_mode) {
				if(rip->route_map[i].metric_config) {
					if(rip->route_map[i].name) {
						vty_out(vty, " redistribute %s metric %d route-map %s%s", zebra_route_string(i), rip->route_map[i].metric, rip->route_map[i].name, VTY_NEWLINE);
					} else {
						vty_out(vty, " redistribute %s metric %d%s", zebra_route_string(i), rip->route_map[i].metric, VTY_NEWLINE);
					}
				} else {
					if(rip->route_map[i].name) {
						vty_out(vty, " redistribute %s route-map %s%s", zebra_route_string(i), rip->route_map[i].name, VTY_NEWLINE);
					} else {
