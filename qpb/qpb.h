...
static inline int qpb__address_family__set(Qpb__AddressFamily *pb_family, u_char family) {
	switch(family) {
		case AF_INET: *pb_family = QPB__ADDRESS_FAMILY__IPV4; return 1;

		case AF_INET6: *pb_family = QPB__ADDRESS_FAMILY__IPV6; return 1;
...
static inline int qpb__address_family__get(Qpb__AddressFamily pb_family, u_char *family) {
	switch(pb_family) {
		case QPB__ADDRESS_FAMILY__IPV4: *family = AF_INET; return 1;

		case QPB__ADDRESS_FAMILY__IPV6: *family = AF_INET6; return 1;
...
static inline Qpb__L3Prefix *qpb__l3_prefix__create(qpb_allocator_t *allocator, struct prefix *p) {
...
static inline int qpb__l3_prefix__get(const Qpb__L3Prefix *pb_prefix, u_char family, struct prefix *prefix) {
	switch(family) {
		case AF_INET: memset(prefix, 0, sizeof(struct prefix_ipv4)); break;

		case AF_INET6: memset(prefix, 0, sizeof(struct prefix_ipv6)); break;

		default: memset(prefix, 0, sizeof(*prefix));
	}
...
static inline int qpb__protocol__set(Qpb__Protocol *pb_proto, int route_type) {
	switch(route_type) {
		case ZEBRA_ROUTE_KERNEL: *pb_proto = QPB__PROTOCOL__KERNEL; break;

		case ZEBRA_ROUTE_CONNECT: *pb_proto = QPB__PROTOCOL__CONNECTED; break;

		case ZEBRA_ROUTE_STATIC: *pb_proto = QPB__PROTOCOL__STATIC; break;

		case ZEBRA_ROUTE_RIP: *pb_proto = QPB__PROTOCOL__RIP; break;

		case ZEBRA_ROUTE_RIPNG: *pb_proto = QPB__PROTOCOL__RIPNG; break;

		case ZEBRA_ROUTE_OSPF:
		case ZEBRA_ROUTE_OSPF6: *pb_proto = QPB__PROTOCOL__OSPF; break;

		case ZEBRA_ROUTE_ISIS: *pb_proto = QPB__PROTOCOL__ISIS; break;

		case ZEBRA_ROUTE_BGP: *pb_proto = QPB__PROTOCOL__BGP; break;
		...
		default: *pb_proto = QPB__PROTOCOL__OTHER;
	}
 ...
 */
static inline Qpb__Ipv4Address *qpb__ipv4_address__create(qpb_allocator_t *allocator, struct in_addr *addr) {
	Qpb__Ipv4Address *v4;
 ...
 */
static inline int qpb__ipv4_address__get(const Qpb__Ipv4Address *v4, struct in_addr *addr) {
 ...
 */
static inline Qpb__Ipv6Address *qpb__ipv6_address__create(qpb_allocator_t *allocator, struct in6_addr *addr) {
	Qpb__Ipv6Address *v6;
 ...
 */
static inline int qpb__ipv6_address__get(const Qpb__Ipv6Address *v6, struct in6_addr *addr) {
	if(v6->bytes.len != 16) {
		return 0;
	}
...
static inline Qpb__L3Address *qpb__l3_address__create(qpb_allocator_t *allocator, union g_addr *addr, u_char family) {
...
static inline int qpb__l3_address__get(const Qpb__L3Address *l3_addr, u_char *family, union g_addr *addr) {
	if(l3_addr->v4) {
...
static inline Qpb__IfIdentifier *qpb__if_identifier__create(qpb_allocator_t *allocator, uint if_index) {
...
static inline int qpb__if_identifier__get(Qpb__IfIdentifier *if_id, uint *if_index, char **name) {
	char *str;
	uint ix;
