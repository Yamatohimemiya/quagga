...
static void nhrp_interface_interface_notifier(struct notifier_block *n, unsigned long cmd) {
	...
	switch(cmd) {
		...
		case NOTIFY_INTERFACE_ADDRESS_CHANGED:
			...
			debugf(NHRP_DEBUG_IF, "%s: NBMA change: address %s", nifp->ifp->name, sockunion2str(&nifp->nbma, buf, sizeof buf));
			break;
	}
}
...
static void nhrp_interface_update_address(struct interface *ifp, afi_t afi, int force) {
	...
	if(best && if_ad->configured && best->address->prefixlen != 8 * prefix_blen(best->address)) {
		zlog_notice("%s: %s is not a host prefix", ifp->name, prefix2str(best->address, buf, sizeof buf));
		best = NULL;
	}
	...
	if(sockunion_family(&if_ad->addr) != AF_UNSPEC) {
		...
		if(nc) {
			nhrp_cache_update_binding(nc, NHRP_CACHE_LOCAL, -1, NULL, 0, NULL);
		}
	}

	debugf(NHRP_DEBUG_KERNEL, "%s: IPv%d address changed to %s", ifp->name, afi == AFI_IP ? 4 : 6, best ? prefix2str(best->address, buf, sizeof buf) : "(none)");
	if_ad->addr = addr;

	if(if_ad->configured && sockunion_family(&if_ad->addr) != AF_UNSPEC) {
		nc = nhrp_cache_get(ifp, &addr, 1);
		if(nc) {
			nhrp_cache_update_binding(nc, NHRP_CACHE_LOCAL, 0, NULL, 0, NULL);
		}
	}
...
void nhrp_interface_update(struct interface *ifp) {
	...
	for(afi = 0; afi < AFI_MAX; afi++) {
		...
		if(sockunion_family(&nifp->nbma) == AF_UNSPEC || ifp->ifindex == IFINDEX_INTERNAL || !if_is_up(ifp) || !if_ad->network_id) {
			if(if_ad->configured) {
				if_ad->configured = 0;
...
int nhrp_interface_add(int cmd, struct zclient *client, zebra_size_t length, vrf_id_t vrf_id) {
	...
	debugf(NHRP_DEBUG_IF, "if-add: %s, ifindex: %u, hw_type: %d %s", ifp->name, ifp->ifindex, ifp->ll_type, if_link_type_str(ifp->ll_type));
...
int nhrp_interface_address_add(int cmd, struct zclient *client, zebra_size_t length, vrf_id_t vrf_id) {
	...
	debugf(NHRP_DEBUG_IF, "if-addr-add: %s: %s", ifc->ifp->name, prefix2str(ifc->address, buf, sizeof buf));
...
int nhrp_interface_address_delete(int cmd, struct zclient *client, zebra_size_t length, vrf_id_t vrf_id) {
	...
	debugf(NHRP_DEBUG_IF, "if-addr-del: %s: %s", ifc->ifp->name, prefix2str(ifc->address, buf, sizeof buf));
