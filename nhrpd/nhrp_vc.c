...
static unsigned int nhrp_vc_key(void *peer_data) {
	...
	return jhash_2words(sockunion_hash(&vc->local.nbma), sockunion_hash(&vc->remote.nbma), 0);
}
...
static int nhrp_vc_cmp(const void *cache_data, const void *key_data) {
	...
	return sockunion_same(&a->local.nbma, &b->local.nbma) && sockunion_same(&a->remote.nbma, &b->remote.nbma);
}
...
int nhrp_vc_ipsec_updown(uint32_t child_id, struct nhrp_vc *vc) {
	...
	if(sa->vc && vc) {
		...
		debugf(NHRP_DEBUG_COMMON, "IPsec NBMA change of %s to %s", sockunion2str(&sa->vc->remote.nbma, buf[0], sizeof buf[0]), sockunion2str(&vc->remote.nbma, buf[1], sizeof buf[1]));
...
void nhrp_vc_reset(void) {
	...
	for(i = 0; i < ZEBRA_NUM_OF(childlist_head); i++) {
		list_for_each_entry_safe(sa, n, &childlist_head[i], childlist_entry) nhrp_vc_ipsec_updown(sa->id, 0);
	}
}
