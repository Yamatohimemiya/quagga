...
static void __nhrp_peer_check(struct nhrp_peer *p) {
	...
	if(p->online != online) {
		...
		if(online && notifier_active(&p->notifier_list)) {
			 ...
			 * SPDs and SAs. */
			THREAD_TIMER_MSEC_ON(master, p->t_fallback, nhrp_peer_notify_up, p, 50);
		} else {
			nhrp_peer_ref(p);
			p->online = online;
			if(online) {
...
static void nhrp_peer_vc_notify(struct notifier_block *n, unsigned long cmd) {
	...
	switch(cmd) {
		case NOTIFY_VC_IPSEC_CHANGED: __nhrp_peer_check(p); break;
...
static void nhrp_peer_ifp_notify(struct notifier_block *n, unsigned long cmd) {
	...
	switch(cmd) {
		...
		case NOTIFY_INTERFACE_DOWN: __nhrp_peer_check(p); break;
		...
		case NOTIFY_INTERFACE_ADDRESS_CHANGED: notifier_call(&p->notifier_list, NOTIFY_PEER_IFCONFIG_CHANGED); break;
		case NOTIFY_INTERFACE_MTU_CHANGED: notifier_call(&p->notifier_list, NOTIFY_PEER_MTU_CHANGED); break;
	}
	nhrp_peer_unref(p);
}
...
static int nhrp_peer_request_timeout(struct thread *t) {
	...
	if(nifp->ipsec_fallback_profile && !p->prio && !p->fallback_requested) {
		...
		vici_request_vc(nifp->ipsec_fallback_profile, &vc->local.nbma, &vc->remote.nbma, p->prio);
...
int nhrp_peer_check(struct nhrp_peer *p, int establish) {
	...
	THREAD_TIMER_ON(master, p->t_fallback, nhrp_peer_request_timeout, p, (nifp->ipsec_fallback_profile && !p->prio) ? 15 : 30);
...
void nhrp_peer_send(struct nhrp_peer *p, struct zbuf *zb) {
	...
	debugf(NHRP_DEBUG_KERNEL, "PACKET: Send %s -> %s", sockunion2str(&p->vc->local.nbma, buf[0], sizeof buf[0]), sockunion2str(&p->vc->remote.nbma, buf[1], sizeof buf[1]));

	os_sendmsg(zb->head, zbuf_used(zb), p->ifp->ifindex, sockunion_get_addr(&p->vc->remote.nbma), sockunion_get_addrlen(&p->vc->remote.nbma));
	zbuf_reset(zb);
}
...
static void nhrp_handle_resolution_req(struct nhrp_packet_parser *p) {
	...
	if(p->if_ad->network_id && p->route_type == NHRP_ROUTE_OFF_NBMA && p->route_prefix.prefixlen < 8) {
...
static void nhrp_handle_registration_request(struct nhrp_packet_parser *p) {
	...
	/* Create reply */
	zb = zbuf_alloc(1500);
	hdr = nhrp_packet_push(zb, NHRP_PACKET_REGISTRATION_REPLY, &p->src_nbma, &p->src_proto, &p->if_ad->addr);
	...
	while((ext = nhrp_ext_pull(&p->extensions, &payload)) != NULL) {
		switch(htons(ext->type) & ~NHRP_EXTENSION_FLAG_COMPULSORY) {
			case NHRP_EXTENSION_NAT_ADDRESS:
				...
				if(natted) {
					nhrp_cie_push(zb, NHRP_CODE_SUCCESS, &p->peer->vc->remote.nbma, &p->src_proto);
				}
				nhrp_ext_complete(zb, ext);
				break;
			default:
		...
		case ETH_P_IP:
			{
				...
				if(iph) {
					if(src) {
						sockunion_set(src, AF_INET, (uint8_t *) &iph->saddr, sizeof(iph->saddr));
					}
					if(dst) {
						sockunion_set(dst, AF_INET, (uint8_t *) &iph->daddr, sizeof(iph->daddr));
					}
				}
			}
			break;
		case ETH_P_IPV6:
			{
				...
				if(iph) {
					if(src) {
						sockunion_set(src, AF_INET6, (uint8_t *) &iph->saddr, sizeof(iph->saddr));
					}
					if(dst) {
						sockunion_set(dst, AF_INET6, (uint8_t *) &iph->daddr, sizeof(iph->daddr));
					}
				}
			}
			break;
...
void nhrp_peer_send_indication(struct interface *ifp, uint16_t protocol_type, struct zbuf *pkt) {
	...
	if(!(if_ad->flags & NHRP_IFF_REDIRECT)) {
		debugf(NHRP_DEBUG_COMMON, "Send Traffic Indication to %s about packet to %s ignored", sockunion2str(&p->vc->remote.nbma, buf[0], sizeof buf[0]), sockunion2str(&dst, buf[1], sizeof buf[1]));
		return;
	}

	debugf(NHRP_DEBUG_COMMON, "Send Traffic Indication to %s (online=%d) about packet to %s", sockunion2str(&p->vc->remote.nbma, buf[0], sizeof buf[0]), p->online, sockunion2str(&dst, buf[1], sizeof buf[1]));
...
static void nhrp_handle_error_ind(struct nhrp_packet_parser *pp) {
	...
	debugf(NHRP_DEBUG_COMMON, "Error Indication from %s about packet to %s ignored", sockunion2str(&pp->src_proto, buf[0], sizeof buf[0]), sockunion2str(&dst_proto, buf[1], sizeof buf[1]));
...
static void nhrp_handle_traffic_ind(struct nhrp_packet_parser *p) {
	...
	debugf(NHRP_DEBUG_COMMON, "Traffic Indication from %s about packet to %s: %s", sockunion2str(&p->src_proto, buf[0], sizeof buf[0]), sockunion2str(&dst, buf[1], sizeof buf[1]),
...
static void nhrp_packet_debug(struct zbuf *zb, const char *dir) {
	...
	debugf(NHRP_DEBUG_COMMON, "%s %s(%d) %s -> %s", dir, packet_types[hdr->type].name ?: "Unknown", hdr->type, reply ? buf[1] : buf[0], reply ? buf[0] : buf[1]);
}
...
void nhrp_peer_recv(struct nhrp_peer *p, struct zbuf *zb) {
	...
	debugf(NHRP_DEBUG_KERNEL, "PACKET: Recv %s -> %s", sockunion2str(&vc->remote.nbma, buf[0], sizeof buf[0]), sockunion2str(&vc->local.nbma, buf[1], sizeof buf[1]));
	...
	if(hdr->type > ZEBRA_NUM_OF(packet_types) || hdr->version != NHRP_VERSION_RFC2332 || nbma_afi >= AFI_MAX || proto_afi == AF_UNSPEC || packet_types[hdr->type].type == PACKET_UNKNOWN || htons(hdr->packet_size) > realsize) {
		zlog_info(
			"From %s: error: packet type %d, version %d, AFI %d, proto %x, size %d (real size %d)", sockunion2str(&vc->remote.nbma, buf[0], sizeof buf[0]), (int) hdr->type, (int) hdr->version, (int) nbma_afi,
			(int) htons(hdr->protocol_type), (int) htons(hdr->packet_size), (int) realsize
		);
		goto drop;
	}
	...
	switch(pp.route_type) {
		...
		case NHRP_ROUTE_OFF_NBMA:
			...
			break;
		case NHRP_ROUTE_NBMA_NEXTHOP: nhrp_peer_forward(peer, &pp); break;
		case NHRP_ROUTE_BLACKHOLE: break;
	}

drop:
	if(info) {
		zlog_info("From %s: error: %s", sockunion2str(&vc->remote.nbma, buf[0], sizeof buf[0]), info);
	}
	if(peer) {
		nhrp_peer_unref(peer);
	}
