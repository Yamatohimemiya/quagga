...
static void nhrp_reg_reply(struct nhrp_reqid *reqid, void *arg) {
	...
	while((cie = nhrp_cie_pull(&p->payload, p->hdr, &cie_nbma, &cie_proto)) != NULL) {
		...
		debugf(NHRP_DEBUG_COMMON, "NHS: CIE registration: %s: %d", sockunion2str(proto, buf, sizeof(buf)), cie->code);
		if(!((cie->code == NHRP_CODE_SUCCESS) || (cie->code == NHRP_CODE_ADMINISTRATIVELY_PROHIBITED && nhs->hub))) {
			ok = 0;
		}
	}
	...
	while((ext = nhrp_ext_pull(&p->extensions, &extpl)) != NULL) {
		switch(htons(ext->type) & ~NHRP_EXTENSION_FLAG_COMPULSORY) {
			case NHRP_EXTENSION_NAT_ADDRESS:
				...
				if(nhrp_cie_pull(&extpl, p->hdr, &cie_nbma, &cie_proto) && nhrp_cie_pull(&extpl, p->hdr, &cie_nbma, &cie_proto)) {
					nifp->nat_nbma = cie_nbma;
					debugf(NHRP_DEBUG_IF, "%s: NAT detected, real NBMA address: %s", ifp->name, sockunion2str(&nifp->nbma, buf, sizeof(buf)));
				}
				break;
		}
	}
	...
	if(c) {
		nhrp_cache_update_binding(c, NHRP_CACHE_NHS, holdtime, nhrp_peer_ref(r->peer), 0, NULL);
	}
}
...
static void nhrp_reg_peer_notify(struct notifier_block *n, unsigned long cmd) {
	...
	switch(cmd) {
		...
		case NOTIFY_PEER_MTU_CHANGED:
			debugf(NHRP_DEBUG_COMMON, "NHS: Flush timer for %s", sockunion2str(&r->peer->vc->remote.nbma, buf, sizeof buf));
...
static int nhrp_reg_send_req(struct thread *t) {
	...
	if(!nhrp_peer_check(r->peer, 2)) {
		debugf(NHRP_DEBUG_COMMON, "NHS: Waiting link for %s", sockunion2str(&r->peer->vc->remote.nbma, buf1, sizeof buf1));
...
static struct nhrp_registration *nhrp_reg_by_nbma(struct nhrp_nhs *nhs, const union sockunion *nbma_addr) {
	...
	list_for_each_entry(r, &nhs->reglist_head, reglist_entry) if(sockunion_same(&r->peer->vc->remote.nbma, nbma_addr)) return r;
	return NULL;
}
...
static void nhrp_nhs_resolve_cb(struct resolver_query *q, int n, union sockunion *addrs) {
	...
	list_for_each_entry(reg, &nhs->reglist_head, reglist_entry) reg->mark = 1;
...
int nhrp_nhs_add(struct interface *ifp, afi_t afi, union sockunion *proto_addr, const char *nbma_fqdn) {
	...
	if(sockunion_family(proto_addr) != AF_UNSPEC && sockunion_family(proto_addr) != afi2family(afi)) {
		return NHRP_ERR_PROTOCOL_ADDRESS_MISMATCH;
	}

	list_for_each_entry(nhs, &nifp->afi[afi].nhslist_head, nhslist_entry) {
		if(sockunion_family(&nhs->proto_addr) != AF_UNSPEC && sockunion_family(proto_addr) != AF_UNSPEC && sockunion_same(&nhs->proto_addr, proto_addr)) {
...
void nhrp_nhs_terminate(void) {
	...
	for(ALL_LIST_ELEMENTS_RO(iflist, node, ifp)) {
		...
		for(afi = 0; afi < AFI_MAX; afi++) {
			list_for_each_entry_safe(nhs, tmp, &nifp->afi[afi].nhslist_head, nhslist_entry) nhrp_nhs_free(nhs);
		}
	}
}
...
void nhrp_nhs_foreach(struct interface *ifp, afi_t afi, void (*cb)(struct nhrp_nhs *, struct nhrp_registration *, void *), void *ctx) {
	...
	list_for_each_entry(nhs, &nifp->afi[afi].nhslist_head, nhslist_entry) {
		if(!list_empty(&nhs->reglist_head)) {
			list_for_each_entry(reg, &nhs->reglist_head, reglist_entry) cb(nhs, reg, ctx);
		} else {
			cb(nhs, 0, ctx);
		}
	}
