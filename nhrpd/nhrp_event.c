...
static void evmgr_put(struct zbuf *zb, const char *fmt, ...) {
	...
	for(pos = fmt; (nxt = strchr(pos, '%')) != NULL; pos = nxt + 2) {
		...
		switch(nxt[1]) {
			case '%': zbuf_put8(zb, '%'); break;
			case 'u': zb->tail += snprintf((char *) zb->tail, zbuf_tailroom(zb), "%u", va_arg(va, uint32_t)); break;
			case 's':
...
static int evmgr_reconnect(struct thread *t) {
	...
	if(fd < 0) {
		zlog_warn("%s: failure connecting nhrp-event socket: %s", __PRETTY_FUNCTION__, strerror(errno));
		zbufq_reset(&evmgr->obuf);
...
void evmgr_notify(const char *name, struct nhrp_cache *c, void (*cb)(struct nhrp_reqid *, void *)) {
	...
	evmgr_put(
		...
		"interface=%s\n"
		"local_addr=%U\n",
		name, nhrp_cache_type_str[c->new.type], nhrp_cache_type_str[c->cur.type], (unsigned int) nhrp_cache_counts[NHRP_CACHE_NHS], c->ifp->name, &nifp->afi[afi].addr
	);
	...
	if(vc) {
		evmgr_put(
			...
			"remote_nbma=%U\n"
			"remote_cert=%H\n",
			c->new.peer->requested ? "yes" : "no", &vc->local.nbma, vc->local.cert, vc->local.certlen, &c->remote_addr, &vc->remote.nbma, vc->remote.cert, vc->remote.certlen
		);
	}
