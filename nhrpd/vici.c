...
static void vici_parse_message(struct vici_conn *vici, struct zbuf *msg, void (*parser)(struct vici_message_ctx *ctx, enum vici_type_t msgtype, const struct blob *key, const struct blob *val), struct vici_message_ctx *ctx) {
	uint8_t *type;
	struct blob key;
	struct blob val;
	...
	while((type = zbuf_may_pull(msg, uint8_t)) != NULL) {
		switch(*type) {
			...
			case VICI_LIST_ITEM:
				...
				parser(ctx, *type, &key, &val);
				break;
			case VICI_LIST_END: debugf(NHRP_DEBUG_VICI, "VICI: List end"); break;
			default: debugf(NHRP_DEBUG_VICI, "VICI: Unsupported message component type %d", *type); return;
		}
	}
}
...
static void parse_sa_message(struct vici_message_ctx *ctx, enum vici_type_t msgtype, const struct blob *key, const struct blob *val) {
			...
			switch(key->ptr[0]) {
				...
				case 's':
					if(blob_equal(key, "state") && ctx->nsections == 3) {
						sactx->child_ok = (sactx->event == 0 && (blob_equal(val, "INSTALLED") || blob_equal(val, "REKEYED")));
					}
					break;
			}
			break;
...
static void parse_cmd_response(struct vici_message_ctx *ctx, enum vici_type_t msgtype, const struct blob *key, const struct blob *val) {
	char buf[512];
...
static void vici_recv_sa(struct vici_conn *vici, struct zbuf *msg, int event) {
	...
	if(ctx.kill_ikesa && ctx.ike_uniqueid) {
		...
		vici_submit_request(vici, "terminate", VICI_KEY_VALUE, "ike-id", strlen(buf), buf, VICI_END);
	}
}
...
static void vici_recv_message(struct vici_conn *vici, struct zbuf *msg) {
	...
	switch(msgtype) {
		case VICI_EVENT:
			...
			if(blob_equal(&name, "list-sa") || blob_equal(&name, "child-updown") || blob_equal(&name, "child-rekey")) {
				vici_recv_sa(vici, msg, 0);
			} else if(blob_equal(&name, "child-state-installed") || blob_equal(&name, "child-state-rekeyed")) {
				vici_recv_sa(vici, msg, 1);
			} else if(blob_equal(&name, "child-state-destroying")) {
				vici_recv_sa(vici, msg, 2);
			}
			break;
		case VICI_CMD_RESPONSE: vici_parse_message(vici, msg, parse_cmd_response, 0); break;
		case VICI_EVENT_UNKNOWN:
		case VICI_CMD_UNKNOWN: zlog_err("VICI: StrongSwan does not support mandatory events (unpatched?)"); break;
		case VICI_EVENT_CONFIRM: break;
		default: zlog_notice("VICI: Unrecognized message type %d", msgtype); break;
	}
}
...
static int vici_reconnect(struct thread *t) {
	...
	if(fd < 0) {
		zlog_warn("%s: failure connecting VICI socket: %s", __PRETTY_FUNCTION__, strerror(errno));
		THREAD_TIMER_ON(master, vici->t_reconnect, vici_reconnect, vici, 2);
		return 0;
	}
...
void vici_request_vc(const char *profile, union sockunion *src, union sockunion *dst, int prio) {
	...
	vici_submit_request(
		vici, "initiate", VICI_KEY_VALUE, "child", strlen(profile), profile, VICI_KEY_VALUE, "timeout", (size_t) 2, "-1", VICI_KEY_VALUE, "async", (size_t) 1, "1", VICI_KEY_VALUE, "init-limits", (size_t) 1, prio ? "0" : "1",
		VICI_KEY_VALUE, "my-host", strlen(buf[0]), buf[0], VICI_KEY_VALUE, "other-host", strlen(buf[1]), buf[1], VICI_END
	);
}
