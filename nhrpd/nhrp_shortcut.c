...
static void nhrp_shortcut_check_use(struct nhrp_shortcut *s) {
	...
	if(s->expiring && s->cache && s->cache->used) {
		debugf(NHRP_DEBUG_ROUTE, "Shortcut %s used and expiring", prefix2str(s->p, buf, sizeof buf));
...
static void nhrp_shortcut_cache_notify(struct notifier_block *n, unsigned long cmd) {
	...
	switch(cmd) {
		case NOTIFY_CACHE_UP:
			...
			break;
		case NOTIFY_CACHE_USED: nhrp_shortcut_check_use(s); break;
...
static void nhrp_shortcut_delete(struct nhrp_shortcut *s) {
	...
	debugf(NHRP_DEBUG_ROUTE, "Shortcut %s purged", prefix2str(s->p, buf, sizeof buf));
...
static struct nhrp_shortcut *nhrp_shortcut_get(struct prefix *p) {
	...
	if(!rn->info) {
		...
		debugf(NHRP_DEBUG_ROUTE, "Shortcut %s created", prefix2str(s->p, buf, sizeof buf));
	} else {
		s = rn->info;
		route_unlock_node(rn);
	}
...
static void nhrp_shortcut_recv_resolution_rep(struct nhrp_reqid *reqid, void *arg) {
	...
	if(pp->hdr->type != NHRP_PACKET_RESOLUTION_REPLY) {
		if(pp->hdr->type == NHRP_PACKET_ERROR_INDICATION && pp->hdr->u.error.code == NHRP_ERROR_PROTOCOL_ADDRESS_UNREACHABLE) {
	...
	while((ext = nhrp_ext_pull(&pp->extensions, &extpl)) != NULL) {
		switch(htons(ext->type) & ~NHRP_EXTENSION_FLAG_COMPULSORY) {
			case NHRP_EXTENSION_NAT_ADDRESS: nhrp_cie_pull(&extpl, pp->hdr, &nat_nbma, &cie_proto); break;
		}
	}
	...
	if(!sockunion_same(&cie_proto, &pp->dst_proto)) {
		debugf(NHRP_DEBUG_COMMON, "Shortcut: Warning dst_proto altered from %s to %s", sockunion2str(&cie_proto, buf[0], sizeof buf[0]), sockunion2str(&pp->dst_proto, buf[1], sizeof buf[1]));
	}
	...
	debugf(NHRP_DEBUG_COMMON, "Shortcut: %s is at proto %s cie-nbma %s nat-nbma %s cie-holdtime %d", prefix2str(&prefix, bufp, sizeof bufp), sockunion2str(proto, buf[0], sizeof buf[0]), sockunion2str(&cie_nbma, buf[1], sizeof buf[1]),
	       sockunion2str(&nat_nbma, buf[2], sizeof buf[2]), htons(cie->holding_time));
	...
	if(sockunion_family(nbma)) {
		...
		if(c) {
			nhrp_cache_update_binding(c, NHRP_CACHE_CACHED, holding_time, nhrp_peer_get(pp->ifp, nbma), htons(cie->mtu), nbma_natoa);
		}
	}
...
static void nhrp_shortcut_send_resolution_req(struct nhrp_shortcut *s) {
	...
	/* Create request */
	zb = zbuf_alloc(1500);
	hdr = nhrp_packet_push(zb, NHRP_PACKET_RESOLUTION_REQUEST, &nifp->nbma, &nifp->afi[family2afi(sockunion_family(&s->addr))].addr, &s->addr);
	hdr->u.request_id = htonl(nhrp_reqid_alloc(&nhrp_packet_reqid, &s->reqid, nhrp_shortcut_recv_resolution_rep));
	hdr->flags = htons(NHRP_FLAG_RESOLUTION_SOURCE_IS_ROUTER | NHRP_FLAG_RESOLUTION_AUTHORATIVE | NHRP_FLAG_RESOLUTION_SOURCE_STABLE);
