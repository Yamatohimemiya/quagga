...
const char *const nhrp_cache_type_str[] = {
	[NHRP_CACHE_INVALID] = "invalid", [NHRP_CACHE_INCOMPLETE] = "incomplete", [NHRP_CACHE_NEGATIVE] = "negative", [NHRP_CACHE_CACHED] = "cached", [NHRP_CACHE_DYNAMIC] = "dynamic",
	[NHRP_CACHE_NHS] = "nhs",	  [NHRP_CACHE_STATIC] = "static",	  [NHRP_CACHE_LOCAL] = "local",
};
...
static void nhrp_cache_peer_notifier(struct notifier_block *n, unsigned long cmd) {
	...
	switch(cmd) {
		case NOTIFY_PEER_UP: nhrp_cache_update_route(c); break;
...
static void nhrp_cache_authorize_binding(struct nhrp_reqid *r, void *arg) {
	...
	debugf(NHRP_DEBUG_COMMON, "cache: %s %s: %s", c->ifp->name, sockunion2str(&c->remote_addr, buf, sizeof buf), (const char *) arg);
...
static void nhrp_cache_newpeer_notifier(struct notifier_block *n, unsigned long cmd) {
	...
	switch(cmd) {
		case NOTIFY_PEER_UP:
			...
			break;
		case NOTIFY_PEER_DOWN:
		case NOTIFY_PEER_IFCONFIG_CHANGED: nhrp_cache_reset_new(c); break;
	}
}
...
int nhrp_cache_update_binding(struct nhrp_cache *c, enum nhrp_cache_type type, int holding_time, struct nhrp_peer *p, uint32_t mtu, union sockunion *nbma_oa) {
	...
	if(c->cur.type == type && c->cur.peer == p && c->cur.mtu == mtu) {
		if(holding_time > 0) {
			c->cur.expires = recent_relative_time().tv_sec + holding_time;
		}
		if(nbma_oa) {
			c->cur.remote_nbma_natoa = *nbma_oa;
		} else {
			memset(&c->cur.remote_nbma_natoa, 0, sizeof c->cur.remote_nbma_natoa);
		}
		nhrp_peer_unref(p);
	} else {
		c->new.type = type;
		...
		if(c->new.type == NHRP_CACHE_INVALID || c->new.type >= NHRP_CACHE_STATIC || c->map) {
