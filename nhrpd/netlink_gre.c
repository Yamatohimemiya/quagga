...
static int __netlink_gre_get_data(struct zbuf *zb, struct zbuf *data, int ifindex) {
	...
	*ifi = (struct ifinfomsg) {
	...
	if(zbuf_send(zb, netlink_req_fd) < 0 || zbuf_recv(zb, netlink_req_fd) < 0) {
		return -1;
	}
	...
	debugf(NHRP_DEBUG_KERNEL, "netlink-link-gre: ifindex %u, receive msg_type %u, msg_flags %u", ifi->ifi_index, n->nlmsg_type, n->nlmsg_flags);
...
void netlink_gre_get_info(unsigned int ifindex, uint32_t *gre_key, unsigned int *link_index, struct in_addr *saddr) {
	...
	*gre_key = 0;
	...
	while((rta = znl_rta_pull(&data, &rtapl)) != NULL) {
		switch(rta->rta_type) {
			case IFLA_GRE_LINK: *link_index = zbuf_get32(&rtapl); break;
			case IFLA_GRE_IKEY:
			case IFLA_GRE_OKEY: *gre_key = zbuf_get32(&rtapl); break;
			case IFLA_GRE_LOCAL: saddr->s_addr = zbuf_get32(&rtapl); break;
		}
	}
err:
	zbuf_free(zb);
